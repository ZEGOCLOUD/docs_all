...
declare interface ZegoCloudRoomConfig {
  ...
  language?: ZegoUIKitLanguage; // 设置 UIKit 语言
	sendMessageChannel?: "RTC" | "ZIM"; // 消息发送通道配置
	videoScreenConfig?: {
  ...
  showBackgroundProcessButton?: boolean; // 是否显示背景虚化及虚拟背景开关按钮。默认不显示。
  ...
  // 成员列表配置
  memberViewConfig?: {
		operationListCustomButton?: () => Element // 操作列表按钮自定义
	}
  showRotatingScreenButton?: boolean; // 是否显示旋转屏幕按钮，默认不显示。
	requireRoomForegroundView?: () => HTMLElement; // 房间内自定义view，位于视频上方
  addInRoomMessageAttributes?: () => any // 添加房间内消息消息属性。返回自定义消息属性。
  customMessageUI?: (msg: InRoomMessageInfo) => HTMLElement // 自定义消息UI
  ...
  onReturnToHomeScreenClicked?: () => void; // 点击“返回主屏幕”按钮时触发此回调函数。设置此回调函数后，点击按钮将不会导航到主屏幕；而是您可以在此处添加自己的页面导航逻辑。
	onSendMessageResult?: (response: { errCode: number, message: string, timestamp?: string, fromUser?: ZegoUser, sendTime?: number, messageID?: number }) => void // 发送消息回调
  onScreenRotation?: (currentScreen: 'landscape' | 'portrait') => void; // 屏幕旋转回调
  onUserStateUpdated?: (status: ZegoUserState) => void // 用户状态更新回调
  onStreamUpdate?: (streamId: string) => void // 流新增回调，可获取到流新增时的流id
	onLocalStreamUpdated?: (state: "created" | "published" | "stopped", streamId: string, stream?) => void // 本地流状态更新回调
	onScreenSharingStreamUpdated?: (state: "created" | "published" | "closed", streamId: string, stream?) => void // 屏幕共享流状态更新回调
	onWhiteboardUpdated?: (state: "created" | "closed", whiteboardId: string) => void // 白板状态更新回调
	onCameraStateUpdated?: (state: "ON" | "OFF") => void // 摄像头状态更新回调
	onMicrophoneStateUpdated?: (state: "ON" | "OFF") => void // 麦克风状态更新回调
}
