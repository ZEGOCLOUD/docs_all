...
如果您使用的是即时通讯（ZIM）服务，请参考 [控制台 - 云市场 - 数美内容审核\|_blank](#18891) 开通权限、完成相关配置，无需其他操作。
</div>

## 开通权限

1. 在 [ZEGO 控制台\|_blank](https://console.zego.im) 创建项目，并申请有效的账号，详情请参考 [控制台 - 项目管理\|_blank](#12107) 中的“项目信息”。
2. 参考 [控制台 - 云市场 - 数美内容审核\|_blank](#18891)，按照页面指引，自助开通 `数美内容审核` 相关权限。
...
- 已在 [ZEGO 云市场\|_blank](https://www.zego.im/cloudmarket) 购买套餐包，或已与 ZEGO 商务人员签署合同，采用后付费的计费方式。
...
<td><ul><li><a href="/article/18683#serverAPI">向 ZEGO 服务端发起请求进行审核</a></li><li><a href="!Interface_audit/Start_audio_audit" target="blank">开始音频流审核</a></li></ul></td>
</tr>
<tr>
<td>智能视频流</td>
<td>向 ZEGO 服务端发起请求进行审核。</td>
<td><ul><li><a href="/article/18683#serverAPI">向 ZEGO 服务端发起请求进行审核</a></li><li><a href="!Interface_audit/Start_video_audit" target="blank">开始视频流审核</a></li></ul></td>
</tr>
<tr>
<td>智能文本</td>
<td rowspan="4">直接请求数美服务端 API 进行审核，请查看 <a href="/article/18683#other">必读信息</a>。</td>
<td><a href="https://help.ishumei.com/docs/tj/text/newest/developDoc/">数美智能文本识别产品 API 文档</a></td>
</tr>
<tr>
<td>智能图片</td>
...
<tr>
<td>智能音频文件</td>
<td><a href="https://help.ishumei.com/docs/tj/audio/newest/developDoc/">数美智能音频文件识别产品 API 文档</a></td>
</tr>
<tr>
<td>智能视频文件</td>
...
<details class="zg-primary">
    <summary>向 ZEGO 服务端发起请求进行审核</summary>
...
- 已经在 [ZEGO 控制台\|_blank](https://console.zego.im) 配置了接收审核结果的回调地址。

#### 1. 配置公共请求参数

开发者调用每个 ZEGO 服务端 API 前，需要先配置 API 的公共请求参数。

公共请求参数，是指每个接口都需要使用到的请求参数，包含了 AppId、Signature（指 2 生成的签名信息）、SignatureNonce（随机字符串）、Timestamp（时间戳）等参数，请根据实际情况修改。公共参数的具体介绍，请参考 [调用方式\|_blank](!Making_API_Requests/Making_API_Requests#4) 中的 “2.1 公共请求参数”。
...
- 开始音频流审核：[StartCensorAudio\|_blank](18720#3) 中的 “3 请求参数”。
- 开始视频流审核：[StartCensorVideo\|_blank](18724#3) 中的 “3 请求参数”。
...
返回信息字段中，如果 Code 为 “0” 表示请求成功；如果不为 “0” 则表示请求失败，请参考 [全局返回码\|_blank](!Return_Codes/Return_Codes) 处理。

通过您在 [ZEGO 控制台\|_blank](https://console.zego.im) 配置的回调地址，获取详细的审核结果，回调结果说明请参考：

- [音频流审核回调\|_blank](!Callback/Audio_audit_callback)
- [视频流审核回调\|_blank](!Callback/Video_audit_callback)
...
<details class="zg-primary">
    <summary>请求数美服务端 API 进行审核（必读信息）</summary>

`音频流识别` 和 `视频流识别` 之外的 4 类内容，ZEGO 没有封装对应的接口和使用方案，需要您参考数美的文档，自行发起服务端请求，并通过返回值或回调结果，进行业务侧逻辑处理。

在您自行接入数美审核服务前，请确保：

- 已参考 [控制台 - 云市场 - 数美内容审核\|_blank](#18891)，按照页面指引，自助开通 `数美内容审核` 相关权限。
