...
请参考 [控制台 - 云市场 - 大饼 AI 变声](#18973)，按照页面指引，自助开通 `大饼 AI 变声` 相关权限；并在自己的项目中集成 `大饼 AI 变声` 的 SDK 包。

### 2 初始化和登录房间

初始化和登录房间的具体流程，请参考实现视频通话文档中的 “[3.1 创建引擎](!ExpressVideoSDK-Integration/Solution_Implementation#CreateEngine)” 及 “[3.2 登录房间](!ExpressVideoSDK-Integration/Solution_Implementation#createroom)”。
...
调用 [IZegoAIVoiceChanger.setSpeaker](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~cpp_windows~class~IZegoAIVoiceChanger#set-speaker) 接口，设置音色，音色的选择可以通过 [5 获取音色列表](!implementation#2_5) 获取。
