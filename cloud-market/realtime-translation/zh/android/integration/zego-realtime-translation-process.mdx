...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时音视频功能，详情请参考 [实时音视频 - 快速开始 - 集成 SDK\|_blank](https://doc-zh.zego.im/article/195)。
...
2. 创建引擎。调用 ZEGO Express SDK 的 [createEngine\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#create-engine) 接口，将申请到的 AppID 和 AppSign 分别传入参数“appID”和“appSign”，创建引擎单例对象。 
...
调用 ZEGO Express SDK 的 [loginRoom\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#login-room) 接口登录房间。roomID 和 user 的参数由开发者的本地业务生成，但是需要满足以下条件：同一个 AppID 内，需保证 “roomID” 全局唯一。  
...
1. 退出房间。主持人调用 ZEGO Express SDK 的 [logoutRoom\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#logout-room) 接口，退出房间。
...
3. 销毁引擎。如果退出房间，不需要使用到引擎资源，可以调用 ZEGO Express SDK 的 [destroyEngine\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#destroy-engine) 接口，销毁引擎。
...
主持人登录房间、并且播放器设置完成后，可以调用  ZegoRealtimeTranslation SDK 的 [translate\|_blank](@translate)。
