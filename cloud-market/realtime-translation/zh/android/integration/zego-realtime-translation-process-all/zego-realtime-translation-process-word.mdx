...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时音视频功能，详情请参考 [实时音视频 - 快速开始 - 集成 SDK](https://doc-zh.zego.im/article/195)。
...
ZEGO 提供了 [示例源码](https://zego-public.oss-cn-shanghai.aliyuncs.com/zegorealtimetranslation/Android/SourceCode/ZegoRealtimeTranslationDemo.zip)，以供开发者进一步了解 ZEGO 实时传译功能。
...
2. 创建引擎。 
  调用 ZEGO Express SDK 的 [createEngine](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#create-engine) 接口，将申请到的 AppID 和 AppSign 分别传入参数“appID”和“appSign”，创建引擎单例对象。 
  
      注册回调，可将实现了 [IZegoEventHandler](/article/api?doc=Express_Video_SDK_API~java_android~class~IZegoEventHandler) 的对象传入参数 “eventHandler”。    
...
调用 ZEGO Express SDK 的 [loginRoom](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#login-room) 接口登录房间。roomID 和 user 的参数由开发者的本地业务生成，但是需要满足以下条件：同一个 AppID 内，需保证 “roomID” 全局唯一。  
...
主持人调用 ZEGO Express SDK 的 [logoutRoom](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#logout-room) 接口，退出房间。
...
如果退出房间，不需要使用到引擎资源，可以调用 ZEGO Express SDK 的 [destroyEngine](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#destroy-engine) 接口，销毁引擎。
