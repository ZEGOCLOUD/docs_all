...
本文介绍如何实现实时语音识别的功能，包括与 Express 的接口对接，主播与观众之间的通信方式。
...
![](http://doc.oa.zego.im/Pics/ZegoRealtimeTranslation/ZegoRealtimeTranslation_Sequence_diagram_voice.png)
...
调用 [loginRoom](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#login-room-user) 接口登录房间。如果房间不存在，调用该接口时会创建并登录此房间。roomID 和 userID 的参数由您本地生成，但是需要满足以下条件：
