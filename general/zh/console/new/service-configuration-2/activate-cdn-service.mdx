...
3. 按照如下流程开通和配置 ZEGO CDN。
    ...
    CNAME 域名本身不能直接访问，在 ZEGO 返回 CNAME 信息后，需要您自行在域名服务提供商处完成 CNAME 配置，详情可参考 [配置域名 CNAME](/console/domain-name-configuration-cname)。
    </Step>
...
3. CNAME 域名本身不能直接访问，在 ZEGO 返回新的 CNAME 信息后，需要您自行在域名服务提供商处完成 CNAME 配置，详情可参考 [配置域名 CNAME](/console/domain-name-configuration-cname)。
...
4. 在 ZEGO 返回 CNAME 信息后，CNAME 域名本身不能直接访问，需要您自行在域名服务提供商处完成 CNAME 配置，详情可参考 [配置域名 CNAME](/console/domain-name-configuration-cname)。  
