...
<ArticleMetadata language="zh" product="Video Call / Audio Call / Live streaming" platform="iOS / Android / macOS / Windows / Flutter / Unity3D" />
...
如果用户 A 短暂断线，但是在 100s 内重连成功了，那么用户 B 不会收到 `onRoomUserUpdate` 的回调通知。
...
- T5 = T3 + time（小于 100s）：用户 A 在重连时间内恢复网络，重连成功。会收到 `onRoomStateChanged` 回调以通知客户端用户 A 重连成功。
...
- T4' ≈ T3 + 100s：用户 B 此时收到 `onRoomUserUpdate` 回调以通知用户 A 已断线。

- T5' = T3 + time（大于 100s，小于 20 min）：用户 A 在重连时间内恢复网络，重连成功。会收到 `onRoomStateChanged` 回调以通知客户端用户 A 重连成功。
