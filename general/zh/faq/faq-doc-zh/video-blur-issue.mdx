...
- 确认 [setVideoConfig\|_blank](/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#set-video-config) 接口设置的 [ZegoVideoConfig\|_blank](/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-entity-zego-video-config)（例如分辨率、码率），尝试配置更高的分辨率和码率看是否可以解决问题。
