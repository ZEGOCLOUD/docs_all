...
[Address Sanitizer (ASan)\|_blank](https://github.com/google/sanitizers/wiki/AddressSanitizer) 是一种基于编译器的快速检测工具，用于检测原生代码中的内存错误。
...
1. 请根据 [集成 SDK\|_blank](#195) 文档的 “方式二：复制 SDK 文件手动集成” 进行集成 SDK。

    <img src="https://doc-media.zego.im/sdk-doc/Pics/Common/ASAN/express_android_asan_runtime_library.png">

2. 请将 SDK 包内带有的 ASan 运行时库一并集成到 App 内。

    将 “wrap.sh” 集成到 App 内。您可以从 [跑通示例源码\|_blank](#3125) 中下载的 Android 示例代码中获取 “wrap.sh” 文件，解压示例源码后，找到 “ZegoExpressExample/main/asan/lib” 目录，将整个 `lib` 目录拷贝到您的应用工程的 “app/src/main/resources” 目录下，结构如下所示。

    <div class="mk-hint">

    您也可以通过 [Android ASan 文档\|_blank](https://developer.android.com/ndk/guides/asan) ，获取谷歌 NDK 提供的 “wrap.sh`” 文件进行集成。
    </div>
    ...
    <img src="https://doc-media.zego.im/sdk-doc/Pics/Common/ASAN/android_asan_wrap_sh_crlf_or_lf.png">
...
4. 如果您的 App 的 minSdkVersion >= 23、且工程使用的 Android Gradle Plugin 版本 >= 7.0，请在 “app/build.gradle” 文件中将 `useLegacyPackaging` 设置为 `true`，如下所示，详情请参考 [文档\|_blank](https://developer.android.com/ndk/guides/wrap-script#packaging_wrapsh)。
...
5. 如果运行 App 时报错 “The APKs are invalid.” ，请参考 [GitHub 文档\|_blank](https://github.com/android/ndk/issues/988)，在 “app/build.gradle” 文件中添加如下配置，并参考 [配置文档\|_blank](https://developer.android.com/ndk/guides/asan#building) 进行处理。
...
1. 请根据 [集成 SDK](#196) 文档的 “方式三：复制 SDK 文件手动集成” 进行集成 SDK。

    <img src="https://doc-media.zego.im/sdk-doc/Pics/Common/ASAN/express_ios_asan_runtime_library.png">
...
2. SDK 包内带有 ASan 运行时库，请根据情况选择是否将其集成到 App 内。
    ...
    <img src="https://doc-media.zego.im/sdk-doc/Pics/Common/ASAN/express_ios_xcode_embed_asan_runtime_library.png">
    ...
    可通过如下链接查询 Xcode 历史版本中附带的 clang 版本：
        - [https://en.wikipedia.org/wiki/Xcode\|_blank](https://en.wikipedia.org/wiki/Xcode)
        - [https://gist.github.com/yamaya/2924292\|_blank](https://gist.github.com/yamaya/2924292)
    </div>

    <img src="https://doc-media.zego.im/sdk-doc/Pics/Common/ASAN/ios_asan_lib_clang_version.png">
    ...
    您可以使用 [XcodesApp\|_blank](https://github.com/RobotsAndPencils/XcodesApp) 在本机同时安装多个版本的 Xcode。
    </div>

    2. 打开 Xcode 工程，在菜单栏选择 ”Product > Scheme > Edit Scheme“，在侧边栏点击 “Run”，在 ”Diagnostics“ 选项卡中，勾选 ”Address Sanitizer“ ，即可开启 ASan。

    <img src="https://doc-media.zego.im/sdk-doc/Pics/Common/ASAN/express_ios_xcode_scheme_enable_asan.png">

    3. 单击 Xcode 的运行按钮，开始构建并调试 App。

    <div class="mk-warning">
        - 此时 Xcode 会自动集成 ASan 运行时库到 App 中，因此不能再手动集成 ZEGO SDK 附带的 ASan 运行时库，否则会冲突。
        - 若使用命令行构建 App 而不是 Xcode GUI，请在 `xcodebuild` 命令中，添加 `-enableAddressSanitizer YES` 参数，详情请参考 [文档\|_blank](https://developer.apple.com/documentation/xcode/diagnosing-memory-thread-and-crash-issues-early) 进行处理。
    </div>
...
<img src="https://doc-media.zego.im/sdk-doc/Pics/Common/ASAN/express_mac_asan_runtime_library.png">
...
目前我们使用微软 [MSVC 的 ASan\|_blank](https://learn.microsoft.com/en-us/cpp/sanitizers/asan)。请参考 [官方文档\|_blank](https://learn.microsoft.com/en-us/cpp/sanitizers/asan) 为您的 App 开启 ASan。
...
- App 的所有 C/C++ 代码需要以 Static CRT (`/MT`) 的方式编译。（因为 ZEGO SDK 使用了 Static CRT，详情请参考 [文档\|_blank](https://learn.microsoft.com/en-us/answers/questions/650660/open-address-sanitizer-flagmy-app-crash.html?childToView=657116#comment-657116) 进行操作）
- 当 EXE 链接了开启 ASan 的 DLL 时，该 EXE 也必须开启 ASan （[参考文档\|_blank](https://devblogs.microsoft.com/cppblog/asan-for-windows-x64-and-debug-build-support/#comment-1163)）。因此若要使用开启了 ASan 的 ZEGO SDK，您的 App 也必须开启 ASan。
...
当崩溃发生时，命令提示符的控制台中会输出 ASan 的错误报告。另外建议设置 ASan 的崩溃转储文件路径，当发生崩溃时将产生 Crash Dump 文件（[参考文档\|_blank](https://learn.microsoft.com/en-us/cpp/sanitizers/asan#crash-dumps)）。
...
若您从 [跑通示例源码\|_blank](#3125) 中获取 `wrap.sh` 文件，则无需做额外修改，示例源码中提供的 `wrap.sh` 是已修改完成的。
...
<img src="https://doc-media.zego.im/sdk-doc/Pics/Common/ASAN/express_ios_xcode_scheme_set_asan_options.png">
