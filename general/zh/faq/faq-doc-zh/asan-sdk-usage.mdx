...
1. 请根据 [集成 SDK](https://doc-zh.zego.im/article/195) 文档的 “方式二：复制 SDK 文件手动集成” 进行集成 SDK。
...
2. 请将 SDK 包内带有的 ASan 运行时库一并集成到 App 内。
    ...
    将 “wrap.sh” 集成到 App 内。您可以从 [跑通示例源码](https://doc-zh.zego.im/article/3125) 中下载的 Android 示例代码中获取 “wrap.sh” 文件，解压示例源码后，找到 “ZegoExpressExample/main/asan/lib” 目录，将整个 `lib` 目录拷贝到您的应用工程的 “app/src/main/resources” 目录下，结构如下所示。
...
若您从 [跑通示例源码](https://doc-zh.zego.im/article/3125) 中获取 `wrap.sh` 文件，则无需做额外修改，示例源码中提供的 `wrap.sh` 是已修改完成的。
