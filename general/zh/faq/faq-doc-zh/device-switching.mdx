...
<td>- `enableCamera` 会真正的开关摄像头设备，不建议频繁使用。
- 当需要解除占用摄像头设备时，可调用此接口。</td>
</tr>
<tr>
<td rowspan="2">视频画面</td>
<td>`mutePublishStreamVideo`</td>
<td>推流时，是否发送视频数据，true 代表不发送。</td>
<td>- 在主播不想发布自己的画面时，可调用此接口。
- 此接口不会真正开关设备，也不会真正停止推流。</td>
</tr>
<tr>
...
<td>- 与 `muteMicrophone` 的区别是，本接口会真正开启或关闭麦克风设备。
- 如果需要频繁开关，建议使用 `muteMicrophone` 接口。</td>
<tr>
<td rowspan="3">音频</td>
