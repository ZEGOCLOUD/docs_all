...
2. 如果拉流端花屏/绿屏，请排查推流端的采集模块。
3. 检查 [enableCustomVideoCapture\|_blank](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-zego-express-engine&jumpType=route#enable-custom-video-capture) 函数的 [ZegoCustomVideoCaptureConfig\|_blank](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-entity-zego-custom-video-capture-config) 参数中的 [ZegoVideoBufferType\|_blank](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~Java~enum~im-zego-zegoexpress-constants-zego-video-buffer-type) 中指定的视频数据格式和传回 SDK 的视频数据格式是否一致。
4. 检查采集图像参数是否正确。
