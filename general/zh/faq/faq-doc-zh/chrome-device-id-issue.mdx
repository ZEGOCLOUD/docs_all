...
在 Chrome 81 浏览器上调用 [enumDevices](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#enum-devices) 接口，获取到的 “deviceID” 为空。
...
在 ZEGO Express Web SDK 中，如果在调用 [creatStream](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#create-stream) 接口成功之前调用 [enumDevices](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#enum-devices)接口，则获得的 “deviceID” 可能为空。
...
2. 采集完音视频流后，调用 [enumDevices](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#enum-devices) 接口即可获得完整的设备信息。
