...
- Linux：没有默认的存储路径，开发者需要主动调用 [setLogConfig\|_blank](/article/api?doc=Express_Audio_SDK_API~cpp_linux~class~ZegoExpressSDK#set-log-config) 接口，自定义 SDK 日志文件的大小和路径。例如：“~/zegolog”
...
必须在调用 [createEngine\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#create-engine) 之前，调用 setLogConfig 接口设置日志的属性才生效，若在 [createEngine\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#create-engine) 之后设置，则在 [destroyEngine\|_blank](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#destroy-engine) 后的下一次 [createEngine\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#create-engine) 时生效。
</div>
...
API 参考：[ZegoLogConfig\|_blank](/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-entity-zego-log-config)、[setLogConfig\|_blank](/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-zego-express-engine#set-log-config)
...
API 参考：
- iOS：[ZegoLogConfig\|_blank](/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-log-config)、[setLogConfig\|_blank](/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#set-log-config)
- macOS：[ZegoLogConfig\|_blank](/article/api?doc=Express_Video_SDK_API~ObjectiveC_macos~class~zego-log-config)、[setLogConfig\|_blank](/article/api?doc=Express_Video_SDK_API~ObjectiveC_macos~class~zego-express-engine#set-log-config)
...
API 参考：[ZegoLogConfig\|_blank](/article/api?doc=Express_Video_SDK_API~CPP_windows~struct~zego-express-zego-log-config)、[setLogConfig\|_blank](/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-zego-express-sdk#set-log-config)
...
- iOS/iPadOS
    ...
    ![xcode_device_window_menu](https://doc-media.zego.im/sdk-doc/Pics/FAQ/xcode_device_window_menu.png)

    2. 在左侧选择指定设备，然后在 “INSTALLED APPS” 中找到需要取日志的 App， 单击下方齿轮图标，选择 “Download Container...” 并保存。
    <img src="https://doc-media.zego.im/sdk-doc/Pics/FAQ/xcode-device-window.png">
...
- Linux
    ...
    Linux 平台没有默认的存储路径，开发者请根据在 [setLogConfig\|_blank](/article/api?doc=Express_Audio_SDK_API~cpp_linux~class~ZegoExpressSDK#set-log-config) 接口中自定义的 SDK 日志文件的路径进行查找。
...
- Windows
    ...
    <img src="http://doc.oa.zego.im/Pics/FAQ/expressSDKlogwindows.png">
...
调用 [setLogConfig\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#set-log-config) 接口，通过 [ZegoLogConfig\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoLogConfig) 对象中的 “logLevel” 属性设置本地控制台打印的日志级别。
...
使用 Futter、Electron、uni-app、Uniy3D、React Native 等跨平台框架时，可参考上文 [Native 平台](/faq/express_sdkLog#1) 设置和获取日志的方式。
