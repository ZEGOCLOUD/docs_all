...
这个问题是因为按Home键回到桌面后房间界面会被最小化，而回到应用时未调用 RoomKit SDK 的 [displayRoomView\|_blank](/article/api?doc=RoomKit_API~java_android~class~ZegoInRoomService#display-room-view) 接口展示房间界面，所以用户看不到房间界面。<br>
