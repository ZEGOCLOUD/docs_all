...
当用户网络环境差时，SDK 会有内部重试的流程。若需关注房间连接情况，请监听 [onRoomStateChanged\|_blank](/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-callback-i-zego-event-handler&jumpType=route#on-room-state-changed) 回调。
