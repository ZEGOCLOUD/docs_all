...
- [Claude Code](https://code.claude.com/docs/en/mcp#option-1:-add-a-remote-http-server)（建议直接使用我们提供的 [Claude Code 插件](https://github.com/ZEGOCLOUD/zego-claude-code-plugins)，插件自动配置 MCP 服务。）
...
为配合 ZEGO MCP 服务实现最佳效果，我们建议：
- 在 AI 编程工具中设置对应的规则文件并在给 AI 指定开发任务时指定使用该规则文件。
- 在 AI 编程工具中设置对应的 Skills 并在给 AI 指定开发任务时指定使用该 Skills。请将以下 Skills 内容复制并粘贴至 IDE 的 Skills 中。

<Tabs>
<Tab title="使用规则文件">
请将以下规则文件内容复制并粘贴至 IDE 的规则文件中。
...
<Note title="说明">仅列出一些热门 IDE 的规则文件配置方法，其他 IDE 的配置方法请参考对应说明文档。</Note>

- [Augment Code](https://docs.augmentcode.com/setup-augment/guidelines)
- [Cursor](https://cursor.com/cn/docs/context/rules)
- Claude Code：建议使用 Skills 配置方式
- [CodeBuddy](https://www.codebuddy.ai/docs/zh/ide/Rules)
- [Trae](https://docs.trae.ai/ide/rules?_lang=zh)
</Tab>
<Tab title="使用 Skills">
Skill 是由 AI 编程工具智能触发的，在需要时加载。我们提供了用于集成 ZEGO 产品/修复 ZEGO 相关问题的 Skills。大部分 AI 编程工具都兼容 [Claude Code skills](https://code.claude.com/docs/zh-CN/skills) 协议。

- 如果您使用的是 Claude Code，请按照我们提供的插件安装指南安装 [ZEGO Claude Code Plugins](https://github.com/ZEGOCLOUD/zego-claude-code-plugins)安装。

<Video src="https://doc-media.zego.im/general/zh/faq/zego-claude-code-plugin.mp4" />

- 如果您使用的是其他 AI 编程工具，请将[ZEGO Claude Code Plugins](https://github.com/ZEGOCLOUD/zego-claude-code-plugins)下载到本地，然后参考对应说明文档进行配置。
</Tab>
</Tabs>
