...
<ArticleMetadata language="zh" product="Video Call / Audio Call / Live streaming" platform="iOS / Android / macOS / Windows / Linux / HarmonyOS / Web / Flutter / Electron / ReactNative / uni-app / Unity3D" />
...
- Linux：按优先级存放（高优先级路径无读写权限时，自动降级至下一优先级路径）：
    - 优先级 1（普通用户）："/home/[用户名]/zegolog"
    - 优先级 1（Root 超级管理员）："/root/zegolog"
    - 优先级 2："/var/lib/zegolog"
    - 优先级 3："/tmp/zegolog"
...
以设置日志存储路径为 “/data/storage/el2/base/files”，单个日志文件大小上限为 5MB 为例：
...
API 参考：[ZegoLogConfig](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/ets_ohos/classes/_zegoexpressdefines_.zegologconfig.html)、[setLogConfig](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/ets_ohos/classes/_zegoexpressengine_.zegoexpressengine.html#setlogconfig)


### 获取日志信息

Android、iOS、macOS、Windows、Linux 和 HarmonyOS 平台根据以下方式，可获取 SDK 的日志信息。
...
**Linux**
    ...
    Linux 平台的日志默认按优先级存放在 “/home/[用户名]/zegolog”（普通用户）、“/root/zegolog”（Root 超级管理员）、“/var/lib/zegolog”、“/tmp/zegolog” 中（高优先级路径无读写权限时，自动降级至下一优先级路径）。如果开发者自定义了日志路径，请根据在 [setLogConfig](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~cpp_linux~class~ZegoExpressSDK#set-log-config) 接口中自定义的 SDK 日志文件的路径进行查找。

**HarmonyOS**

    鸿蒙设备连上 Mac/PC 打开 devEco-studio，选择 Device File Browser 模块，打开目录 "/data/app/el2/100/base/\{package name\}/files/" 找到以 "zegoavlog" 开头的 txt 或 zip 文件。

### 获取堆栈信息

发生崩溃时，SDK 会产生崩溃的堆栈信息，Android、iOS、macOS、Windows、Linux 和 HarmonyOS 平台根据以下方式，可获取 SDK 的堆栈信息。
...
**HarmonyOS**
    ...
    鸿蒙设备连上 Mac/PC 打开 devEco-studio，选择 Log 模块，打开 FaultLog 栏找到对应的包名。里面有 cppcrash 和 jscrash 两个目录，内部就包含了原生层的崩溃信息和 ets 层的崩溃信息，右键点击文件就可以导出崩溃堆栈信息。
