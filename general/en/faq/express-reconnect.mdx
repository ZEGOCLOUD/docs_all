...
If all stream playing nodes have been attempted and still fail, the user will receive the [onPlayerStateUpdate](https://docs.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-callback-i-zego-event-handler&jumpType=route#on-player-state-update)
 callback, with "state" as `ZegoPlayerStateNoPlay` and "errorCode" not equal to 0, indicating that the SDK's internal reconnection has failed. The specific error should be determined based on the value of "errcode". You can refer to [Common Error Codes](/real-time-video-android-java/client-sdk/error-code). At this point, the business side can consider waiting a few seconds (such as 3-5 seconds) before starting to play the stream again, but do not continuously retry, which will lead to an infinite loop.
...
If the host application crashes and the stream isn’t started again within 90 seconds, the audience will receive
a "stream deleted" notification via [onRoomStreamUpdate](https://docs.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-callback-i-zego-event-handler&jumpType=route#on-room-stream-update). You can handle the 90-seconds case and after-90-seconds case as follows:

- If the host restarts the stream within 90 seconds, the audience will first receive a "stream deleted" notification,
...
- If the host still hasn’t published after 90 seconds, the audience will receive a "stream deleted" notification;
