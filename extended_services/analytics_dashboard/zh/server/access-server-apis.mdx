...
以下为 [`GetBizUsage`](./operation-data/get-biz-usage.mdx)（查询业务规模）接口的请求结构为例：
...
此外，ZEGO 提供多种编程语言的示例代码，开发者可根据实际情况进行参考。以下是 [`GetBizUsage`](./operation-data/get-biz-usage.mdx) 接口的 Node.js 代码：
