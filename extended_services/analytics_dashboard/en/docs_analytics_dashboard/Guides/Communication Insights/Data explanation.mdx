...
**Resolution**, **Bitrate** and **Frame Rate** are related to each other. Generally, when the resolution is fixed, the higher the bitrate, the clearer the image; and if the bitrate is fixed, the larger the resolution, the blurrier the image.合理的设置分辨率、码率和帧率，可以达到更好的画面质量。
</Note>
