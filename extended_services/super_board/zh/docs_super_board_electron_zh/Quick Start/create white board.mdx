...
调用 ZEGO Express SDK 接口 [createEngine](/article/api?doc=express-video-sdk_API~javascript_electron~class~ZegoExpressEngine#create-engine) 初始化 SDK。
...
2. 使用 [ZegoSuperBoardManager](@-ZegoSuperBoardManager) 的 [getInstance](@getInstance) 方法获取 ZegoSuperBoard 实例。
...
3. 使用 ZegoSuperBoard 实例的 [init](@init) 方法初始化 ZegoSuperBoard SDK。
...
初始化 ZegoExpress-Video SDK 和 ZegoSuperBoard SDK 成功之后，调用 ZEGO Express SDK 的 [loginRoom](/article/api?doc=express-video-sdk_API~javascript_electron~class~ZegoExpressEngine#login-room) 接口，登录 Express 房间，推荐在初始化成功之后调用。
...
登录房间成功（通过 Express 回调接口 [onRoomStateUpdate](/article/api?doc=express-video-sdk_API~javascript_electron~class~ZegoExpressEngine#on-room-state-update) 知道房间状态为连接成功）后，即可创建白板。

<a id="fileWhiteboard"></a>
超级白板 SDK 支持创建纯白板和文件白板。

- 纯白板：指定宽、高和页数创建的白板，用户在指定的白板画布上进行实时绘制，调用 [createWhiteboardView](@createWhiteboardView) 接口即可创建。
- 文件白板：基于文件创建的白板，白板宽高和页数由文件决定，实现在文件上绘制图元的业务需求，调用 [createFileView](@createFileView) 接口即可创建。

    <Warning title="注意">
    
    1. 创建白板完成后，超级白板 SDK 会返回 [ZegoSuperBoardSubViewModel](@-ZegoSuperBoardSubViewModel)，请保存其中的 [uniqueID](@uniqueID-ZegoSuperBoardSubViewModel)，以便后续用于白板切换或销毁白板。
    2. 创建文件白板前需要先获取文件的 fileID，可参考 [共享文件管理](../basic%20Func/File%20manage.mdx) 进行上传。
    </Warning>
...
- 请通过 [querySuperBoardSubViewList](@querySuperBoardSubViewList) 获取房间内当前的白板数量。
</Warning>

### 5 挂载当前白板

如果用户进房时，房间已经存在白板，请主动调用 [querySuperBoardSubViewList](@querySuperBoardSubViewList) 接口通知 ZegoSuperBoard SDK 当前白板容器已经存在，并通过 [switchSuperBoardSubView](@switchSuperBoardSubView) 接口挂载当前白板到父容器（parentDomID）上，以便展示白板。
...
如需销毁某个白板，调用 [destroySuperBoardSubView](@destroySuperBoardSubView) 接口，传入该白板的 [uniqueID](@uniqueID-ZegoSuperBoardSubViewModel)，即可销毁。
...
调用 [ZegoExpressEngine](/article/api?doc=Express_Video_SDK_API~javascript_electron~class~ZegoExpressEngine) 的 [logoutRoom](/article/api?doc=Express_Video_SDK_API~javascript_electron~class~ZegoExpressEngine#logout-room) 接口退出房间。
...
调用 [ZegoSuperBoardManager](@-ZegoSuperBoardManager) 的 [unInit](@unInit)，反初始化 ZegoSuperBoard SDK。
...
如果无需再使用 ZEGO Express Video SDK 的能力，即可调用 [ZegoExpressEngine](/article/api?doc=Express_Video_SDK_API~javascript_electron~class~ZegoExpressEngine) 的 [destroyEngine](/article/api?doc=Express_Video_SDK_API~javascript_electron~class~ZegoExpressEngine#destroy-engine) 销毁引擎，释放麦克风、摄像头、内存、CPU 等资源。
...
- [白板绘制](../basic%20Func/sketch.mdx)
- [白板翻页](../basic%20Func/scale%20And%20flip.mdx)
- [白板缩放](../basic%20Func/Withe%20board%20Scale.mdx)
- [白板切换](../basic%20Func/Switch.mdx)
- [获取白板列表](!ZegoSuperBoard-BasicFunction/Whiteboardlist)
- [共享文件管理](../basic%20Func/File%20manage.mdx)