...
- ZegoSuperBoardSubView：ZegoSuperBoardView 的子集，开发者实际创建的 View。ZegoSuperBoardView 会自动呈现最新创建或通过 [switchSuperBoardSubView](@switchSuperBoardSubView) 指定的 ZegoSuperBoardSubView。
...
- 使用超级白板前，请参考 [控制台 - 服务配置 - 文件共享\|_blank](#14338) 在 [ZEGO 控制台\|_blank](https://console.zego.im) 自助开通文件共享功能（或联系 ZEGO 技术支持开通相关功能权限），否则超级白板服务无法正常使用。 
...
请参考 [Electron 文档 - 快速启动指南\|_blank](https://github.com/electron/electron-quick-start) 创建一个 Electron 项目。
</details> 
...
1. 在 Electron 项目中创建一个 index.html 文件和 package.json 文件，并将以下代码拷贝到对应文件中。
    ...
    <div class="multiple-select-codes">
        ...
        <div class="scroll-box">
            <span class="tab-item">
            <span>创建超级白板 - index.html</span>
            </span>
            <span class="tab-item">
            <span>安装依赖 - package.json</span>
            </span> 
        </div>
        </div>
    <div class="code-list"> 
...
- 如果您的项目为全新项目，没有配置过 package.json，可参考本文档 [最简代码](#18266#minimal_sample) 相关内容进行配置。
</Note>
...
调用 ZEGO Express SDK 接口 [createEngine\|_blank](/article/api?doc=express-video-sdk_API~javascript_electron~class~ZegoExpressEngine#create-engine) 初始化 SDK。
...
2. 使用 [ZegoSuperBoardManager\|_blank](@-ZegoSuperBoardManager) 的 [getInstance\|_blank](@getInstance) 方法获取 ZegoSuperBoard 实例。
...
3. 使用 ZegoSuperBoard 实例的 [init\|_blank](@init) 方法初始化 ZegoSuperBoard SDK。
...
[on\|_blank](@on): 注册回调事件，通过 event 指定监听事件名。
...
初始化 ZegoExpress-Video SDK 和 ZegoSuperBoard SDK 成功之后，调用 ZEGO Express SDK 的 [loginRoom\|_blank](/article/api?doc=express-video-sdk_API~javascript_electron~class~ZegoExpressEngine#login-room) 接口，登录 Express 房间，推荐在初始化成功之后调用。
...
登录房间成功（通过 Express 回调接口 [onRoomStateUpdate\|_blank](/article/api?doc=express-video-sdk_API~javascript_electron~class~ZegoExpressEngine#on-room-state-update) 知道房间状态为连接成功）后，即可创建白板。

<a id="fileWhiteboard"></a>
超级白板 SDK 支持创建纯白板和文件白板。

- 纯白板：指定宽、高和页数创建的白板，用户在指定的白板画布上进行实时绘制，调用 [createWhiteboardView\|_blank](@createWhiteboardView) 接口即可创建。
- 文件白板：基于文件创建的白板，白板宽高和页数由文件决定，实现在文件上绘制图元的业务需求，调用 [createFileView\|_blank](@createFileView) 接口即可创建。

    <Warning title="Warning">
    
    1. 创建白板完成后，超级白板 SDK 会返回 [ZegoSuperBoardSubViewModel\|_blank](@-ZegoSuperBoardSubViewModel)，请保存其中的 [uniqueID\|_blank](@uniqueID-ZegoSuperBoardSubViewModel)，以便后续用于白板切换或销毁白板。
    2. 创建文件白板前需要先获取文件的 fileID，可参考 [共享文件管理\|_blank](!BasicFunction/File_manage) 进行上传。
    </Warning>
...
- 请通过 [querySuperBoardSubViewList\|_blank](@querySuperBoardSubViewList) 获取房间内当前的白板数量。
</Warning>

### 5 挂载当前白板

如果用户进房时，房间已经存在白板，请主动调用 [querySuperBoardSubViewList\|_blank](@querySuperBoardSubViewList) 接口通知 ZegoSuperBoard SDK 当前白板容器已经存在，并通过 [switchSuperBoardSubView\|_blank](@switchSuperBoardSubView) 接口挂载当前白板到父容器（parentDomID）上，以便展示白板。
...
如需销毁某个白板，调用 [destroySuperBoardSubView\|_blank](@destroySuperBoardSubView) 接口，传入该白板的 [uniqueID\|_blank](@uniqueID-ZegoSuperBoardSubViewModel)，即可销毁。
...
调用 [ZegoExpressEngine\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_electron~class~ZegoExpressEngine) 的 [logoutRoom\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_electron~class~ZegoExpressEngine#logout-room) 接口退出房间。
...
调用 [ZegoSuperBoardManager\|_blank](@-ZegoSuperBoardManager) 的 [unInit](@unInit)，反初始化 ZegoSuperBoard SDK。
...
如果无需再使用 ZEGO Express Video SDK 的能力，即可调用 [ZegoExpressEngine\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_electron~class~ZegoExpressEngine) 的 [destroyEngine\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_electron~class~ZegoExpressEngine#destroy-engine) 销毁引擎，释放麦克风、摄像头、内存、CPU 等资源。
