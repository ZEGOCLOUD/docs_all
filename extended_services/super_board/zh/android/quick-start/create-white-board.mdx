...
- 已在 [ZEGO 控制台](https://console.zego.im) 创建项目，并申请有效的 AppID 和 AppSign，详情请参考 [控制台 - 项目管理](https://doc-zh.zego.im/console/project-info) 中的“项目信息”。

<Warning title="注意">

- 使用超级白板前，请参考 [控制台 - 服务配置 - 文件共享](https://doc-zh.zego.im/console/service-configuration/file-sharing) 在 [ZEGO 控制台](https://console.zego.im) 自助开通文件共享功能（或联系 ZEGO 技术支持开通相关功能权限），否则超级白板服务无法正常使用。
- `2.3.0 或以上` 版本的 SDK 支持 Token 鉴权，若您需要升级鉴权方式，可参考 [如何从 AppSign 鉴权升级为 Token 鉴权](https://doc-zh.zego.im/faq/express-token-upgrade?product=ExpressVideo)。
</Warning>
...
1. 请参考 [下载 SDK 包](../download-sdk.mdx)，下载最新版本的 SDK，下载完成后进行解压。
...
定义 SDK 引擎对象，调用 [createEngine](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-zego-express-engine#create-engine) 接口，将申请到的 AppID 传入参数 “appID”，创建引擎单例对象。
...
如果您需要切换鉴权方式，请参考 [如何从 AppSign 鉴权升级为 Token 鉴权](https://doc-zh.zego.im/faq/express-token-upgrade?product=ExpressVideo)。
</Note>
...
如果回调 onInit 中的 errorCode 为 ZegoSuperBoardError.ZegoSuperBoardSuccess，代表初始化成功，可进行更多操作。errorCode 可参考 [常见错误码](../error-code.mdx)。
...
- [onError](@onError)：SDK 抛出的错误码。errorCode 可参考 [常见错误码](../error-code.mdx)。
...
错误码详情请参考 [登录房间错误码 ](https://doc-zh.zego.im/real-time-video-android-java/client-sdk/error-code)。
...
创建文件白板前需要先获取文件的 fileID，可参考 [共享文件管理](../basic-func/file-manage.mdx) 进行上传。
