...
开发者可参考本文中的实现以及 ZEGO 提供的 [示例源码\|_blank](!Sample_Codes/Sample_Codes)，根据自己的业务场景组合代码以满足业务需求。
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/WhiteboardView/Superboard_bestpractice.png" /></Frame>

流程中以 A 端创建并操作白板，B 端观看白板为例；图中的虚线表示 A 端进行操作时，B 端可在收到的对应回调中获取相关信息。

### 1. 初始化 SDK 并登录房间

请参考 [快速开始\|_blank](!QuickStart/CreateWboard) 完成 SDK 的集成与初始化，并登录房间。
...
创建文件白板使用的文件需要先上传，详见 [共享文件管理\|_blank](!BasicFunction/File_manage)。
</Note>
...
白板依次创建完成后，SuperBoardView 自动多端同步，默认显示最新创建的白板，即 Excel 文件白板, B 端在 A 端每次创建白板后都会收到 [remoteSuperBoardSubViewAdded\|_blank](@remoteSuperBoardSubViewAdded) 通知，在对应回调内刷新本地 UI 逻辑即可。
...
通过 [setOperationMode\|_blank](@setOperationMode) 设置权限可控制客户端是否可操作白板。

拥有权限的客户端可以在白板上进行绘制，详情请参考 [白板绘制\|_blank](!BasicFunction/Sketch)。
...
B 端在 A 端切换 Excel sheet 后会自动同步切换，同时收到 [scrollChange\|_blank](@scrollChange) 通知，在对应回调内刷新本地 UI 逻辑即可。
...
切换至动态 PPT 后，可通过跳步/翻页接口或点击工具进行 PPT 演示，详情请参考 [白板翻页\|_blank](!BasicFunction/flip)。

B 端在 A 端切换白板后会自动同步切换，同时收到 [remoteSuperBoardSubViewSwitched\|_blank](@remoteSuperBoardSubViewSwitched) 通知，在对应回调内刷新本地 UI 逻辑即可。
...
B 端在 A 端切换白板后会自动同步切换，同时收到 [remoteSuperBoardSubViewSwitched\|_blank](@remoteSuperBoardSubViewSwitched) 通知，上一步中已实现该回调。

## API 参考列表
| 方法        | 描述           |
| ----------- | -------------- |
| [createEngineWithProfile\|_blank](/unique-api/express-video-sdk/zh/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#createenginewithprofile)  | 创建 ZegoExpressEngine 单例对象并初始化 SDK  |
|[getInstance\|_blank](@getInstance)| 获取 ZegoSuperBoardManager 实例对象  |
| [init\|_blank](@init)| 初始化 ZegoSuperBoard SDK  |
| [loginRoom\|_blank](/unique-api/express-video-sdk/zh/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#loginroom) | 登录房间  |
| [createWhiteboardView\|_blank](@createWhiteboardView) | 创建纯白板  |
|[createFileView\|_blank](@createFileView)| 创建文件白板|
|[getSuperBoardView\|_blank](@getSuperBoardView)| 获取 BoardView 对象|
|[switchSuperBoardSubView\|_blank](@switchSuperBoardSubView)| 切换到指定的SuperBoardSubView|
|[enableSyncScale\|_blank](@enableSyncScale)| 设置是否将缩放同步给房间内其他成员|
|[enableResponseScale\|_blank](@enableResponseScale)| 是否响应房间内其他成员的缩放|
|[setToolType\|_blank](@setToolType)| 设置白板工具类型|
