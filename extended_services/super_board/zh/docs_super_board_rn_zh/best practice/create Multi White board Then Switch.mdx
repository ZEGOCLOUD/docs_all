...
开发者可参考本文中的实现以及 ZEGO 提供的 [示例源码](../Quick%20Start/Run%20Demo.mdx)，根据自己的业务场景组合代码以满足业务需求。
...
请参考 [快速开始](../Quick%20Start/create%20white%20board.mdx) 完成 SDK 的集成与初始化，并登录房间。
...
创建文件白板使用的文件需要先上传，详见 [共享文件管理](../basic%20Func/File%20manage.mdx)。
</Note>
...
白板依次创建完成后，SuperBoardView 自动多端同步，默认显示最新创建的白板，即 Excel 文件白板, B 端在 A 端每次创建白板后都会收到 [remoteSuperBoardSubViewAdded](@remoteSuperBoardSubViewAdded) 通知，在对应回调内刷新本地 UI 逻辑即可。
...
通过 [setOperationMode](@setOperationMode) 设置权限可控制客户端是否可操作白板。

拥有权限的客户端可以在白板上进行绘制，详情请参考 [白板绘制](../basic%20Func/sketch.mdx)。
...
B 端在 A 端切换 Excel sheet 后会自动同步切换，同时收到 [scrollChange](@scrollChange) 通知，在对应回调内刷新本地 UI 逻辑即可。
...
切换至动态 PPT 后，可通过跳步/翻页接口或点击工具进行 PPT 演示，详情请参考 [白板翻页](../basic%20Func/scale%20And%20flip.mdx)。

B 端在 A 端切换白板后会自动同步切换，同时收到 [remoteSuperBoardSubViewSwitched](@remoteSuperBoardSubViewSwitched) 通知，在对应回调内刷新本地 UI 逻辑即可。
...
B 端在 A 端切换白板后会自动同步切换，同时收到 [remoteSuperBoardSubViewSwitched](@remoteSuperBoardSubViewSwitched) 通知，上一步中已实现该回调。
...
| [getInstance](@getInstance)                                                                                                                                    | 获取 ZegoSuperBoardManager 实例对象         |
| [init](@init)                                                                                                                                                  | 初始化 ZegoSuperBoard SDK                   |
| [loginRoom](/unique-api/express-video-sdk/zh/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#loginroom)                             | 登录房间                                    |
| [createWhiteboardView](@createWhiteboardView)                                                                                                                  | 创建纯白板                                  |
| [createFileView](@createFileView)                                                                                                                              | 创建文件白板                                |
| [getSuperBoardView](@getSuperBoardView)                                                                                                                        | 获取 BoardView 对象                         |
| [switchSuperBoardSubView](@switchSuperBoardSubView)                                                                                                            | 切换到指定的SuperBoardSubView               |
| [enableSyncScale](@enableSyncScale)                                                                                                                            | 设置是否将缩放同步给房间内其他成员          |
| [enableResponseScale](@enableResponseScale)                                                                                                                    | 是否响应房间内其他成员的缩放                |
| [setToolType](@setToolType)                                                                                                                                    | 设置白板工具类型                            |
