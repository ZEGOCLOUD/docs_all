...
调用 Express Video SDK 的 [createEngineWithProfile ](/unique-api/express-video-sdk/zh/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#createenginewithprofile) 接口，将申请到的 AppID 和 AppSign 传入参数 “appID” 和 “appSign”，创建引擎单例对象。


如果需要注册回调方法，开发者可根据实际需要，实现 Express Video SDK 的 [ZegoEventListener](/unique-api/express-video-sdk/zh/javascript_react-native/interfaces/_zegoexpresseventhandler_.zegoeventlistener.html) 中的某些方法，创建引擎后可通过调用 [on](/unique-api/express-video-sdk/zh/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#on) 接口设置回调。
...
使用 [ZegoSuperBoardManager](@-ZegoSuperBoardManager) 的 [getInstance](@getInstance) 方法获取 ZegoSuperBoard 实例。
...
您可以调用 Express Video SDK 的 [loginRoom ](/unique-api/express-video-sdk/zh/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#loginroom) 接口登录房间，推荐在初始化成功之后调用。如果房间不存在，调用该接口时会创建并登录此房间。
...
调用 [ZegoExpressEngine](/unique-api/express-video-sdk/zh/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html) 的 [logoutRoom](/unique-api/express-video-sdk/zh/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#logoutroom) 接口退出房间。
...
调用 [ZegoSuperBoardManager](@-ZegoSuperBoardManager) 的 [unInit](@unInit-ZegoSuperBoardManager)，反初始化 ZegoSuperBoard SDK。
...
如果无需再使用 ZEGO Express Video SDK 的能力，即可调用 [ZegoExpressEngine](/unique-api/express-video-sdk/zh/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html) 的 [destroyEngine](/unique-api/express-video-sdk/zh/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#destroyengine) 销毁引擎，释放麦克风、摄像头、内存、CPU 等资源。
