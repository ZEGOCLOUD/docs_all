...
B 端加入房间并确认用于挂载白板的父容器的存在（物理像素宽高不为0）后，需要判断房间内是否存在白板，如果存在，请参考 [挂载当前白板](../Quick%20Start/create%20white%20board.mdx#5-挂载当前白板) 进行手动挂载。
...
| [loginRoom](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#login-room) | 登录房间                            |
| [createWhiteboardView](@createWhiteboardView)                                                         | 创建纯白板                          |
| [createFileView](@createFileView)                                                                     | 创建文件白板                        |
| [getSuperBoardView](@getSuperBoardView)                                                               | 获取 SuperBoardView 对象            |
| [switchSuperBoardSubView](@switchSuperBoardSubView)                                                   | 切换到指定的 SuperBoardSubView      |
...
| [setScaleFactor](@setScaleFactor)                                                                     | 缩放 SuperBoardSubView              |
| [on](@on)                                                                                             | 注册监听事件                        |
