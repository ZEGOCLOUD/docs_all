...
首先，调用 [getInstance](@getInstance) 获取超级白板实例。随后，调用 [getCurrentSuperBoardSubView](@getCurrentSuperBoardSubView) 获取当前展示 superBoardSubView。最后，调用 [switchSpeaker](@switchSpeaker) 切换动态文件中的扬声器。
...
- 在 [createFileView](@createFileView)之后调用 [switchSpeaker](@switchSpeaker)。
- 在 [switchSuperBoardSubView](@switchSuperBoardSubView) 成功之后调用 [switchSpeaker](@switchSpeaker)。
