...
首先，调用 [getInstance\|_blank](@getInstance) 获取超级白板实例。随后，调用 [getCurrentSuperBoardSubView\|_blank](@getCurrentSuperBoardSubView) 获取当前展示 superBoardSubView。最后，调用 [switchSpeaker\|_blank](@switchSpeaker) 切换动态文件中的扬声器。
...
- 在 [createFileView\|_blank](@createFileView)之后调用 [switchSpeaker\|_blank](@switchSpeaker)。
- 在 [switchSuperBoardSubView\|_blank](@switchSuperBoardSubView) 成功之后调用 [switchSpeaker\|_blank](@switchSpeaker)。
