...
## 实现流程
默认开启缩放模式。可通过调用 [setScaleFactor\|_blank](@setScaleFactor) 进行缩放。

<Note title="Note">

当 [setToolType\|_blank](@setToolType) 为 [Click\|_blank](@Click) 时，[setScaleFactor\|_blank](@setScaleFactor) 接口无效，无法缩放白板。
