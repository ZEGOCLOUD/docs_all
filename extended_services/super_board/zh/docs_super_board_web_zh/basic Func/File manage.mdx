...
<Tab title="上传 H5 文件">
通过 [ZegoSuperboardCustomH5Config](@-ZegoSuperboardCustomH5Config) 设置上传 H5 文件的配置参数，并调用 [uploadH5File](@uploadH5File) 方法上传 H5 文件。
...
文件上传分为上传中，排队中，转换中，和转换成功等多个阶段。具体上传状态请查看 [ZegoSuperBoardUploadFileState](@-ZegoSuperBoardUploadFileState)
...
调用 [cacheFile](@cacheFile) 方法提前请求该文件中的资源。
``` javascript
