...
1. 安装 CocoaPods，安装方法及常见问题请参考 [CocoaPods 常见问题 - 安装 CocoaPods](https://doc-zh.zego.im/faq/cocoapods-issues)。
...
如果回调 onInit 中的 errorCode 为 0，代表初始化成功，可进行更多操作。errorCode 可参考 [常见错误码](../error-code.mdx)。
...
错误码详情请参考 [登录房间错误码 ](https://doc-zh.zego.im/real-time-video-ios-oc/client-sdk/error-code)。
...
- (void)loginRoom {
    ...
    [[ZegoExpressEngine sharedEngine] loginRoom:roomID user:user config:roomConfig callback:^(int errorCode, NSDictionary * _Nullable extendedData) {
        ...
        if (errorCode == 0) {
            ...
            // 登录失败，请参考 errorCode 说明 https://doc-zh.zego.im/real-time-video-ios-oc/client-sdk/error-code
            NSLog(@"房间登录失败");
        }
    }];
}
