...
ZEGO 服务端 API 支持全球就近接入，使用全球统一域名 ${SERVICE}-api.zego.im，按区域解析。
...
<tbody>
  <tr>
    <td>中国⼤陆（上海）</td>
    <td>${SERVICE}-api-sha.zego.im</td>
  </tr>
  <tr>
    <td>港澳台（⾹港）</td>
    <td>${SERVICE}-api-hkg.zego.im</td>
  </tr>
  <tr>
    <td>欧洲（法兰克福）</td>
    <td>${SERVICE}-api-fra.zego.im</td>
  </tr>
  <tr>
    <td>美⻄（加州）</td>
    <td>${SERVICE}-api-lax.zego.im</td>
  </tr>
  <tr>
    <td>亚太（孟买）</td>
    <td>${SERVICE}-api-bom.zego.im</td>
  </tr>
</tbody>
</table>


域名中的 ${SERVICE} 为 ZEGO 提供的各个服务，对应域名如下：

| 产品 | ${SERVICE} 值 | 接入域名 |
|-|-|-|
| 超级白板| whiteboard | whiteboard-api.zego.im |
...
<tbody>
  ...
  <tr>
    ...
    <td>String</td>
    <td>是</td>
    <td>签名，签名的生成请参考 <a href="#3">3 签名机制</a>。
  </tr>
  <tr>
    <td>SignatureNonce</td>
    <td>String</td>
    <td>是</td>
    <td>随机数。要与生成签名时使用的 SignatureNonce 保持一致。</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>Int64</td>
    <td>是</td>
    <td>Unix 时间戳，单位为秒。最多允许 10 分钟的误差。要与生成签名时使用的 Timestamp 保持一致。
  </tr>
  <tr>
    <td>IsTest</td>
    <td>String</td>
    <td>否</td>
    <td>是否为测试环境。取值如下：<ul><li>true（忽略大小写）：测试环境</li><li>false（忽略大小写）：正式环境（默认值）</li></ul></td>
  </tr>
</tbody>
</table>
...
开发者可到 [ZEGO 控制台\|_blank](https://console.zego.im/) 获取 AppID 和 ServerSecret，在 “我的项目 > 项目 > 配置 > 基本信息” 中查看相关信息。

![](/Pics/server_v2/appid.png)

![](/Pics/server_v2/server.png)
...
<tbody>
  <tr>
    <td>AppId</td>
    <td>应用 ID。公共参数里的AppId，从 <a target="_blank" href="https://console.zego.im/">ZEGO 控制台</a> 获取。</td>
  </tr>
  <tr>
    <td>SignatureNonce</td>
    <td>随机数。公共参数里的 SignatureNonce，生成算法可参考下方签名示例。</td>
  </tr>
  <tr>
    <td>ServerSecret</td>
    <td>应⽤秘钥，从 <a target="_blank" href="https://console.zego.im/">ZEGO 控制台</a> 获取。</td>
  </tr>
  <tr>
    <td>Timestamp</td>
