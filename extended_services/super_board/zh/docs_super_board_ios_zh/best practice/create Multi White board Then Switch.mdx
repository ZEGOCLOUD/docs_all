...
请参考 [快速开始](../Quick%20Start/Run%20Demo.mdx) 完成 SDK 的集成与初始化，并登录房间。
...
| 方法        | 描述           |
| ----------- | -------------- |
|[createEngineWithProfile](/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#create-engine-with-profile-event-handler)  | 创建 ZegoExpressEngine 单例对象并初始化 SDK  |
|[sharedInstance](@sharedInstance)  | 获取 ZegoSuperBoardManager 实例对象  |
|[initWithConfig](@initWithConfig)| 初始化 ZegoSuperBoard SDK  |
|[loginRoom](/article/api?doc=Express_Video_SDK_API~ObjectiveC~class~zego-express-engine#login-room-user) | 登录房间  |
|[createWhiteboardView](@createWhiteboardView) | 创建纯白板  |
|[createFileView](@createFileView)| 创建文件白板|
|[superBoardView](@superBoardView)| 获取 SuperBoardView 对象|
