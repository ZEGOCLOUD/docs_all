...
当 [toolType](@toolType) 为 [ZegoSuperBoardToolClick](@ZegoSuperBoardToolClick) 时，手势会被关闭，无法缩放白板。

</Note>

ZegoSuperBoard 的缩放功能控制开关如下。

- 将 [setOperationMode](@setOperationMode) 设置为 `ZegoSuperBoardOperationModeZoom`，即可开启 ZegoSuperBoard 的缩放功能。
- 将 [setOperationMode](@setOperationMode) 设置为 `ZegoSuperBoardOperationModeDraw | ZegoSuperBoardOperationModeZoom`，会开启绘制功能并同时打开缩放功能。
