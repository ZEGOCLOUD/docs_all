...
<Tabs>
<Tab title="上传普通文件">
调用 [uploadFile](@uploadFile) 方法上传普通文件。
...
<Tab title="上传 H5 文件">
通过 [ZegoUploadCustomH5Config](@-ZegoUploadCustomH5Config) 设置上传 H5 文件的配置参数，并调用 [uploadH5File](@uploadH5File) 方法上传 H5 文件。
...
调用 [cacheFileWithFileID](@cacheFileWithFileID) 方法将文件缓存到本地。
``` objc
...
调用 [cancelCacheFileWithSeq](@cancelCacheFileWithSeq) 方法取消掉正在缓存的文件。
``` objc
...
调用 [queryFileCachedWithFileID](@queryFileCachedWithFileID) 方法查询文件缓存是否存在。
``` objc
