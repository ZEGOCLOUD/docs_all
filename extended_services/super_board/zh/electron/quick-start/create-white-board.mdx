    ...
    <script>
        ...
        var zegoConfig = {
            appID: YOUR APPID, // 在 ZEGO 控制台（https://console.zego.im/）创建项目后即可获得。
                               // 此 AppID 需要开通文件共享权限，开通步骤请参考控制台文档 [服务配置 - 文件共享](https://doc-zh.zego.im/console/service-configuration/file-sharing)，或联系 ZEGO 技术支持开通。
...
- 文件白板：基于文件创建的白板，白板宽高和页数由文件决定，实现在文件上绘制图元的业务需求，调用 [createFileView](@createFileView) 接口即可创建。
    ...
    2. 创建文件白板前需要先获取文件的 fileID，可参考 [共享文件管理](../basic-func/file-manage.mdx) 进行上传。
    </Warning>
...
- [白板绘制](../basic-func/sketch.mdx)
- [白板翻页](../basic-func/scale-and-flip.mdx)
- [白板缩放](../basic-func/withe-board-scale.mdx)
- [白板切换](../basic-func/switch.mdx)
- [获取白板列表](../basic-func/white-board-list.mdx)
- [共享文件管理](../basic-func/file-manage.mdx)