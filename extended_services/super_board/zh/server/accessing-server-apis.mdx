...
以下为 [StartMix](https://doc-zh.zego.im/real-time-video-server/api-reference/stream-mixing/start-mix)（开始混流）接口的请求结构为例：
...
此外，ZEGO 提供多种编程语言的示例代码，开发者可根据实际情况进行参考。以下是 [StartMix](https://doc-zh.zego.im/real-time-video-server/api-reference/stream-mixing/start-mix)接口的 Node.js 代码：
