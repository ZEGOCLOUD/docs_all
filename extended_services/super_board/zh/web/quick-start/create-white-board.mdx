...
- 已生成 Token，详情请参考 [使用 Token 鉴权](https://doc-zh.zego.im/super-board-web/quick-start/user-access-control)。也可以参考 [控制台 - 开发辅助](https://doc-zh.zego.im/console/development-assistance/temporary-token)，在 ZEGO 控制台获取临时 Token（有效期为 24 小时）。
...
请参考 [下载 SDK 包](../download-sdk.mdx)，下载最新版本的 SDK，下载完成后进行解压。
...
初始化 SDK 需要用于验证身份的 Token，其获取方式请参考 [用户权限控制](./user-access-control.mdx#42-生成-token) 中的 “4.2 生成 Token”。
...
<Warning title="注意">
临时 Token 仅供调试，正式上线时，请从开发者的业务服务器生成 token，详情可参考 使用 [Token 鉴权](./user-access-control.mdx)。如果 Token 错误，请参考 [错误码](../error-code.mdx) 文档中的 1002067 和 1003072 排查问题。
</Warning>
