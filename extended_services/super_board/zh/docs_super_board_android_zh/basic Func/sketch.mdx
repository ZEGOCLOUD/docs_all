...
已参考 [快速开始\|_blank](!QuickStart/CreateWboard) 创建了一个白板。
...
- 将 [setOperationMode\|_blank](@setOperationMode) 设置为 ZegoSuperBoardOperationMode.Draw，即可开启 ZegoSuperBoard 的绘制功能并同时关闭滚动功能。
- 将 [setOperationMode\|_blank](@setOperationMode) 设置为 ZegoSuperBoardOperationMode.Scroll，会关闭绘制功能并同时打开滚动功能。
...
设置 [ZegoSuperBoardManager\|_blank](@-ZegoSuperBoardManager) 类的 [setToolType\|_blank](@setToolType) 方法可修改 ZegoSuperBoard 的工具类型，目前支持 10 种工具。
...
开发者可通过 [enableHandwriting\|_blank](@enableHandwriting) 开启笔锋模式，文首的图先后演示了无笔锋时的绘制效果和有笔锋时的绘制效果。
...
ZEGO SDK 已内置默认画笔图标，开发者可通过 [setCustomCursorAttribute\|_blank](@setCustomCursorAttribute) 设置画笔的光标样式。
...
ZEGO 超级白板 SDK 本身不包含业务操作逻辑，需要您在业务侧通过信令传达操作权限数据，并结合 [setOperationMode\|_blank](@setOperationMode) 接口实现。

例如，当教师想要允许学生在白板上涂鸦，则需要教师端 App 向学生端 App 发送信令，学生端 App 调用 [setOperationMode\|_blank](@setOperationMode) 接口，将 `mode` 设置为 `ZegoSuperBoardOperationMode.Draw`。当教师不再允许学生对白板操作时，则教师端 App 再次向学生端 App 发送信令，学生端 App 再次调用 [setOperationMode\|_blank](@setOperationMode) 接口，将 `mode` 设置为 `ZegoSuperBoardOperationMode.None`。
