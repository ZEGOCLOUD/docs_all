...
当 [setToolType](@setToolType) 为 [Click](@Click) 时，手势会被关闭，无法缩放白板。

</Note>

- 将 [setOperationMode](@setOperationMode) 设置为 ZegoSuperBoardOperationMode.Zoom，即可开启 ZegoSuperBoard 的缩放功能。
- 将 [setOperationMode](@setOperationMode) 设置为 ZegoSuperBoardOperationMode.Draw.getMode() | ZegoSuperBoardOperationMode.Zoom.getMode()，会开启绘制功能并同时打开缩放功能。
