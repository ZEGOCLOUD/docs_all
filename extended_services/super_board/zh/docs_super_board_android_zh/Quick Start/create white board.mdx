...
1. 请参考 [下载 SDK 包](../Download%20SDK.mdx)，下载最新版本的 SDK，下载完成后进行解压。
...
使用 [ZegoSuperBoardManager](@-ZegoSuperBoardManager) 的 [init](@init) 方法初始化 ZegoSuperBoard SDK。

如果回调 onInit 中的 errorCode 为 ZegoSuperBoardError.ZegoSuperBoardSuccess，代表初始化成功，可进行更多操作。errorCode 可参考 [常见错误码](!ErrorCodes/ErrorCodes)。
...
- [onError](@onError)：SDK 抛出的错误码。errorCode 可参考 [常见错误码](../Error%20Code.mdx)。
- [onRemoteSuperBoardSubViewAdded](@onRemoteSuperBoardSubViewAdded)：远端新增文件白板通知。
- [onRemoteSuperBoardSubViewRemoved](@onRemoteSuperBoardSubViewRemoved)：远端销毁文件白板通知。
- [onRemoteSuperBoardSubViewSwitched](@onRemoteSuperBoardSubViewSwitched)：远端切换白板文件通知。
...
调用 [ZegoExpressEngine](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine) 的 [loginRoom](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#login-room) 接口登录房间，推荐在初始化成功之后调用。
...
创建文件白板前需要先获取文件的 fileID，可参考 [共享文件管理](!BasicFunction/File_manage) 进行上传。
...
调用 [ZegoExpressEngine](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine) 的 [logoutRoom](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#logout-room-2) 接口退出房间。
...
如果无需再使用 ZEGO Express Video SDK 的能力，即可调用 [ZegoExpressEngine](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine) 的 [destroyEngine](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#destroy-engine) 销毁引擎，释放麦克风、摄像头、内存、CPU 等资源。
