...
1. &nbsp;Import the libraries of ZEGOCLOUD Super Board SDK for Android
    Download and unzip the [sample code](/super-board-flutter/sample-app) to get the “superboard_view_flutter” folder.
...
If the return value is 0, it means the initialization is successful, and you can perform more operations. For the errorCode, you can refer to [Error codes](/super-board-flutter/error-codes).
...
- [onError](https://pub.dev/documentation/zego_superboard/latest/zego_superboard_event_handler/ZegoSuperBoardEventHandler/onError.html): Error codes thrown by the SDK. For errorCode reference, see [Error codes](/super-board-flutter/error-codes).
...
You can call the [loginRoom](https://www.zegocloud.com/docs/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngineRoom/loginRoom.html) interface of [ZegoExpressEngine](https://www.zegocloud.com/docs/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngine-class.html) to log in to the room. If the room does not exist, the room will be created and logged in when this interface is called.
...
- The constructor of [ZegoUser](https://www.zegocloud.com/docs/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoUser/ZegoUser.html) will set the userName to be the same as the parameter userID you passed.
