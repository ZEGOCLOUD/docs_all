...
When logging in to a room, you need to pass in a **token for user authentication**. To obtain the login token, see [User privilege control](https://www.zegocloud.com/docs/real-time-video-web/communication/using-token-authentication).
