...
- Call [`setOperationMode`\|_blank](@setOperationMode) method to set the operation mode to `ZegoSuperBoardOperationModeDraw` to enable the drawing feature and disable the scrolling feature of Super Board.
- Call [`setOperationMode`\|_blank](@setOperationMode) to set the operation mode to `ZegoSuperBoardOperationModeScroll` to disable the drawing feature and enable the scrolling feature of Super Board.
...
Set the [`toolType`\|_blank](@toolType) property of the [`ZegoSuperBoardManager`\|_blank](@-ZegoSuperBoardManager) class to modify the tool type of ZEGO Super Board. Currently, 10 tools are supported.
