...
30 seconds before a Token expires, the SDK sends out a notification through the [onRoomTokenWillExpire](/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-room-token-will-expire-room-id) callback.
