...
To change the authentication mode, refer to [Upgrade the authentication mode from using the AppSign to Token](https://www.zegocloud.com/docs/faq/express-token-upgrade).
</Note>
...
If `errorCode` in the callback `onInit` is `ZegoSuperBoardError.ZegoSuperBoardSuccess`, initialization is successful and more operations can be performed. For details about `errorCode`, refer to [Common Error Codes](/super-board-android/error-codes).
...
- [onError](@onError): Error code thrown by the SDK. For details about `errorCode`, refer to [Common Error Codes](/super-board-android/error-codes).
