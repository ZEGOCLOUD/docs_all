...
Clients with permissions can draw on the whiteboard. For more details, please refer to [Draw\|_blank](!ZegoSuperBoard-BasicFunction/Sketch).
...
After the A end switches the whiteboard, the B end will automatically switch synchronously and receive the [remoteSuperBoardSubViewSwitched\|_blank](@remoteSuperBoardSubViewSwitched)notification.This callback has been implemented in the previous step.
