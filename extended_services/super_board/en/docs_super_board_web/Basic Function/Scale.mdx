...
The zooming function is enabled by default. The [setScaleFactor\|_blank](@setScaleFactor) API can be called to zoom in and out.

<Note title="Note">

If the [`setToolType`\|_blank](@setToolType) is set to [`Click`\|_blank](@Click), the [setScaleFactor\|_blank](@setScaleFactor) API is unavailable and the whiteboard cannot be zoomed in or out.
