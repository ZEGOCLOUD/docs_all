...
- Create a project in the [ZEGOCLOUD Console\|_blank](https://console.zegocloud.com/) and apply a valid AppID for the project. 
...
Download the latest SDK package with reference to [SDK downloads\|_blank](!download), and decompress the package after the download.
...
1. Call the [`getInstance`\|_blank](@getInstance) method in [ZegoSuperBoardManager\|_blank](@-ZegoSuperBoardManager) to obtain the ZegoSuperBoard instance.
2. Call the [init\|_blank](@init) method of the ZegoSuperBoard instance to initialize ZegoSuperBoard SDK. The instance of ZegoExpressEngine needs to be transferred.

For details about how to obtain the token used for authentication during SDK initialization, refer to [Generate a token\|_blank](14927#5).
...
[`on`\|_blank](@on): Registers an event callback. A listening-for event name can be specified by setting event callbacks.
...
- Use [querySuperBoardSubViewList\|_blank](@querySuperBoardSubViewList) to obtain the current number of whiteboards in a room.
...
2. After the login to a room is successful, ensure that the parent container corresponding to **parentDomID** exists (none of the physical pixel, width, and height is 0), follow the steps below to call the [`querySuperBoardSubViewList`\|_blank](@querySuperBoardSubViewList) API to notify the ZegoSuperBoard SDK that the current whiteboard container already exists, and use the [`switchSuperBoardSubView`\|_blank](@switchSuperBoardSubView) API to mount the current whiteboard to the parent container.
...
Call the [logoutRoom](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#logout-room) interface of [ZegoExpressEngine](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine) to leave the room.
...
If you no longer need the capabilities of the ZEGO Express Video SDK, you can call the [destroyEngine](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#destroy-engine) method of [ZegoExpressEngine](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine) to destroy the engine and release resources such as microphone, camera, memory, CPU, etc.
