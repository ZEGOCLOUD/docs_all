...
When logging in to a room, you need to pass the Token, user, and roomID to the [`loginRoom`\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#login-room) method. Otherwise, the login will fail.
...
30 seconds before a Token expires, the SDK sends out a notification through the [`onRoomTokenWillExpire`\|_blank](@onRoomTokenWillExpire) callback.
