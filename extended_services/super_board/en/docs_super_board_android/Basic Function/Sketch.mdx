...
A whiteboard is created by referring to [Getting started\|_blank](!QuickStart/CreateWboard).
...
- Call [`setOperationMode`\|_blank](@setOperationMode) to set the operation mode to `ZegoSuperBoardOperationMode.Draw` to enable the drawing feature and disable the scrolling feature of ZEGO Super Board.
- Call [`setOperationMode`\|_blank](@setOperationMode) to set the operation mode to `ZegoSuperBoardOperationMode.Scroll` to disable the drawing feature and enable the scrolling feature of ZEGO Super Board.
...
Call the [setToolType\|_blank](@setToolType) method of the [ZegoSuperBoardManager\|_blank](@-ZegoSuperBoardManager) class to modify the tool type of ZEGO Super Board. Currently, 10 tools are supported.
...
You can call the [enableHandwriting\|_blank](@enableHandwriting) method to enable the handwriting mode. The figures at the beginning of this document show the drawing effects when the handwriting mode is enabled and disabled.
...
A default pen icon is embedded in the ZEGO SDK. You can call the [setCustomCursorAttribute\|_blank](@setCustomCursorAttribute) method to set the cursor style for the pen.
