...
| EventTime  | String | Unix timestamp (in milliseconds) when the event occurred on the cloud player server. Developers can use this field to determine the order of callbacks. |
...
| Reason    | Number | Reason for the destruction of the cloud player.<ul><li>1: Manually destroyed by calling the [DeletePlayer](../delete-player.mdx) interface.</li><li>2: The idle time exceeds MaxIdleTime.</li><li>3: The media resource playback has ended.</li><li>4: Streaming failed.</li><li>5: Failed to enter or was kicked out of the room.</li></ul> |
</Tab>
<Tab title="3">
| Parameter | Type   | Description |
| :-------- | :----- | :---------- |
...
| PlayIndex | Number | Index of the current playback media resource (only valid when Status is 4 or 5). |
</Tab>
<Tab title="4">
| Parameter | Type   | Description |
| :-------- | :----- | :---------- |
| Code      | Number | Exception types.<ul><li>-1: Resource loading timeout.</li><li>-2: Seek timeout.</li><li>-3: Seek failed.</li><li>-4: Abnormal audio rendering frame rate.</li><li>-5: Decoding data error.</li><li>-6: Audio and video out of sync.</li><li>-7: Playback stuttering.</li><li>1: Abnormal playback progress.</li><li>2: Abnormal video streaming frame rate (normal detection).<Note title="Note">Normal detection means that when the streaming frame rate is less than 90% of the baseline frame rate for two consecutive times in a non-flow control state, ZEGOCLOUD will trigger an exception callback to you after two consecutive occurrences.</Note></li><li>3: Abnormal video streaming frame rate (strict detection).<Note title="Note"><ul><li>Strict detection means that, in addition to normal detection, if the streaming frame rate is lower than 40% of the configured frame rate for three consecutive times, or if the streaming frame rate is 0 once, ZEGOCLOUD will immediately trigger an exception callback to you.</li><li>Strict detection is disabled by default. If you need to enable it, please contact ZEGOCLOUD technical support.</li></ul></Note></li><li>4: Abnormal audio streaming frame rate.</li><li>5: Flow control state lasts for more than 5 minutes.</li><li>6: Decoding error.</li><li>7: Failed to load resource.<Warning title="Warning"><ul><li>This exception type is triggered when creating a player with the `StreamUrlList` parameter and one of the resources in the list fails to load.</li><li>If the player has no resource loading failures and no other resources, only then will a callback with `EventType` 3 and `Status` 6 be triggered.</li><li>This exception type can only be triggered when creating a player with the `StreamUrlList` parameter; it will not be triggered when using the `StreamUrl` parameter.</li></ul></Warning></li></ul> |
| PlayIndex | Number | Index of the current playback media resource (only valid when Code=7). |
</Tab>
...
Here is an example of a successful callback after calling the [CreatePlayer](./../create-player.mdx) interface to create a cloud player.
...
Here is an example of a successful callback after calling the [CreateUploadTask](./../create-upload-task.mdx) interface to create an upload task.
