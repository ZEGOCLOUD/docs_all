...
The following list only lists the interface request parameters. For common parameters, please refer to [Accessing server APIs - Common request parameters](./accessing-server-apis.mdx#common-parameters).

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| PlayerId | String | Yes | Unique identifier ID for the cloud player, returned by [CreatePlayer](./create-player.mdx).<strong>Make sure that PlayerId is an existing one when calling this interface, otherwise errors may occur.</strong> |
...
| StreamUrl | String | No<Warning title="Warning"> You need to select according to whether a single player or a playlist player was created in [Create Player](./create-player.mdx#request-parameters). <ul><li>When creating a single player, the `StreamUrl` parameter is optional, and `StreamUrlList` is not allowed;</li><li>When creating a playlist player, the `StreamUrlList` parameter is optional, and `StreamUrl` is not allowed.</li></ul> **If `StreamUrl` or `StreamUrlList` is filled in here, other parameters will be ignored.**</Warning> | The address of the media resource, which must be a valid HTTP/HTTPS address and within 1024 bytes in length. |
| StreamUrlList | Array of String | !mu | A list of address of a media resource, used to continuously play multiple media resources. The maximum number of incoming file addresses is supported and the total byte length is not greater than 50,000. <Note title="Note">This feature is turned off by default. If you need to enable and use, please contact ZEGO Technical Support configuration. </Note> |
...
| IsPause | Number | No | Whether to pause the playback.<ul><li>0: Play the source file normally.</li><li>1: Pause the playback of the source file.<Warning title="Warning"> When setting pause playback, the `StreamUrl` parameter cannot be passed in, otherwise other parameter configurations will be ignored and pause playback will fail. </Warning></li></ul> |
...
The following are only the return codes related to the interface business logic. For the complete list of return codes, please refer to [Return Codes](./return-codes.mdx).
