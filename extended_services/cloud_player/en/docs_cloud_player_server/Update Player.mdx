...
| StreamUrl | String | No(Only one parameter in `StreamUrl` and `StreamUrlList` can be selected) | The address of the media resource, which must be a valid HTTP/HTTPS address and within 1024 bytes in length. |
