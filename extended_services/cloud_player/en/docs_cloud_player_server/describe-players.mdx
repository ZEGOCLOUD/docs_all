...
| PlayerId | String | No | Unique identifier ID of the cloud player, returned by [CreatePlayer](./create-player.mdx).<strong>If PlayerId is filled in, please make sure that the PlayerId already exists, otherwise the query result may be empty.</strong> |
...
| DestroyReason | Number | Reason for destroying the cloud player.<ul><li>1: Actively destroyed by calling the [DeletePlayer](./delete-player.mdx) interface.</li><li>2: The idle time exceeded <code>MaxIdleTime</code>.</li><li>3: Media resource playback has ended.</li><li>4: Failed to push stream.</li><li>5: Failed to enter or was kicked out of the room.</li></ul> |
| StreamUrlList | Array of String |Playback media resource list. |
| PlayIndex | Number | Index of the current playback media resource. Returns 0 after the player is destroyed. |
