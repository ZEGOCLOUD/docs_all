...
- 取消结果请通过 [回调](./Callback/Player%20Related.mdx) 或 [查询上传任务](./cloud%20player%20query.mdx) 接口判断。
</Note>
...
| 参数 | 类型 | 是否必填 | 描述 |
|------|------|----------|------|
| UploadTaskId | String | 是 | 上传任务 ID，上传任务的唯一标识。在调用 [CreateUploadTask](./Create%20upload%20task.mdx) 接口创建上传任务后，您可通过响应参数获取此 ID。 |
