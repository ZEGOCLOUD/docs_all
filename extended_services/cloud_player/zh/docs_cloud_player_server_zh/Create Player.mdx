...
| MaxIdleTime | Number | 否 | 云端播放器处于<strong>空闲状态（指 Status 为 2、5、6，具体请参考 [查询云端播放器任务列表](./Describe%20Players.mdx) 的返回参数）</strong>的最大时长，即媒体流为非播放状态的最大时长。取值范围为 [5, 3000]，单位为秒，默认值为 300 秒，大于 3000 时，自动调整为 3000。<ul><li>空闲状态超过设置的 MaxIdleTime 后，该播放器会被自动销毁。</li><li>MaxIdleTime 如果取值小于 5，ZEGO 服务器会自动调整为 5；如果取值大于 600，ZEGO 服务器会自动调整为 600。</li></ul> |
...
| └RoomUserUpdate | Number | 否 | 当您在创建云端播放器时因 RoomId 不存在而自动创建房间时，用户进出房间是否会触发房内其他用户的客户端 onRoomUserUpdate 回调通知：<ul><li>0：不会（默认）。</li><li>1：会。</li></ul><Note title="Note">若 RoomId 存在，此参数无效。</Note> |
