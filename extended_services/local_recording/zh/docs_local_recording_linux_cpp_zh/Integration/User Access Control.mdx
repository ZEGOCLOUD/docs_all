...
基础鉴权 Token 指的是开发者在登录房间前必须通过 [SetToken](https://doc-zh.zego.im/API/ServerRecord/html/namespace_z_e_g_o_1_1_l_i_v_e_r_o_o_m.html#aa0b8eca203fb7b2f40b69f1c853d29f4) 接口将 Token 传给 SDK，来验证用户的合法性。
:::
:::if{props.platform=java}
基础鉴权 Token 指的是开发者在登录房间前必须通过 [setToken](https://doc-zh.zego.im/API/ServerRecord-java/html/com/zego/zegoliveroom/ZegoLiveRoom.html#setToken-java.lang.String-java.lang.String-) 接口将 Token 传给 SDK，来验证用户的合法性。
:::
...
- 已在项目中集成 ZEGO 本地服务端录制 SDK，请参考 [快速开始 - 集成](./SDK%20Integration.mdx)。
...
3. 开发者客户端携带申请到的 Token 和 roomID 信息，通过 SetToken（C++）/ setToken（Java）接口传入 SDK。
...
<Warning title="注意">运行生成 Token 的 Java 源码时，如果出现 “java.security.InvalidKeyException:illegal Key Size” 异常提示，请参考 [相关常见问题文档](https://doc-zh.zego.im/faq/java-token-key-size-error) 解决。</Warning>
...
:::if{props.platform=undefined}
用户携带获取到的 Token 信息，在登录房间之前通过 [SetToken](https://doc-zh.zego.im/API/ServerRecord/html/namespace_z_e_g_o_1_1_l_i_v_e_r_o_o_m.html#aa0b8eca203fb7b2f40b69f1c853d29f4) 接口将 Token 传入给 SDK。

```cpp
LIVEROOM::SetToken("Token", "roomID"); // 将 Token 和 roomID 替换为实际获取到的信息
```

:::

:::if{props.platform=java}
用户携带获取到的 Token 信息，在登录房间之前通过 [setToken](https://doc-zh.zego.im/API/ServerRecord-java/html/com/zego/zegoliveroom/ZegoLiveRoom.html#setToken-java.lang.String-java.lang.String-) 接口将 Token 传入给 SDK。

```java
public void setToken(String token, String roomID); // 将 Token 和 roomID 替换为实际获取到的信息
```
:::