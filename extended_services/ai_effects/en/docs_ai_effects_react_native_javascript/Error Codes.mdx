...
Generally, the errors are returned through the callback [on\|_blank](@on). 
...
| Error Code | Description |
| ---------- | ----------- |
| 5000100    | The AI model file is missing, please check if the model path passed in is correct and if the file exists, or refer to [Import Resources and Models\|_blank](!Quick_Starts/Import_Resources) for reconfiguration. |
| 5000103    | Failed to initialize AI inference engine, model file is corrupted, please check if the model file is set correctly, or refer to [Import Resources and Models\|_blank](!Quick_Starts/Import_Resources) for reconfiguration. |
| 5000104    | Missing color lookup table (LUT) resource, may be due to incorrect settings of resources such as whitening, ruddiness, teeth whitening, style filters, etc., please refer to [Import Resources and Models\|_blank](!Quick_Starts/Import_Resources) for reconfiguration. |
| 5000105    | Failed to load color lookup table (LUT), may be due to file not found or file corruption, please refer to [Import Resources and Models\|_blank](!Quick_Starts/Import_Resources) for reconfiguration.  |
...
| 5000111    | AI model format error or corruption, please refer to [Import Resources and Models\|_blank](!Quick_Starts/Import_Resources) for reconfiguration.           |
...
| 5000114    | Beauty makeups resource does not exist, please confirm if the resource has been imported and if the path is correct, or refer to [Import Resources and Models\|_blank](!Quick_Starts/Import_Resources) for reconfiguration. |
| 5000120    | "Original Skin" function cannot run on this device.  |
| 5000121    | "Skin Replacement" function cannot run on this device.  |
| 5000203    | Failed to run portrait segmentation model, this error will be thrown every time an image is processed, please refer to [Import Resources and Models\|_blank](!Quick_Starts/Import_Resources) for reconfiguration. |
| 5000304    | Failed to run Face detection model, this error will be thrown every time an image is processed, please refer to [Import Resources and Models\|_blank](!Quick_Starts/Import_Resources) for reconfiguration. |
