...
1. Before using the AI features provided by ZegoEffects SDK, please download the latest version of AI resources and models (files with the suffix bundle or model) from the [downloads\|_blank](!ZegoEffects_downloads) page of the SDK, and copy them to your project for later use.
    <Note title="Note">
...
2. Specify the absolute path of the AI resources and models. **Please download the latest version of AI resources and models (files with the suffix bundle or model) from the [downloads\|_blank](!ZegoEffects_downloads) page.**
...
3. Before calling the [create\|_blank](/article/api?doc=Effects_SDK_API~objectivec_ios~class~ZegoEffects#create-appid-app-sign-callback-1) interface to create an object, call the [setResources\|_blank](@setResources) interface to pass in the model and resource path list to load the models and resources.
