...
2. Before calling the [create\|_blank](@create) interface to create a ZegoEffects object, pass the device performance level to the SDK through [setAdvancedConfig\|_blank](@setAdvancedConfig) to run the corresponding AI Face beautification effects based on the device performance level.
