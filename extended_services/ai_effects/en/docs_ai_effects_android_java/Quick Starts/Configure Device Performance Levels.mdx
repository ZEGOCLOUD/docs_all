...
1. (Optional) Use [getDeviceLevel\|_blank](@getDeviceLevel) to get the current device performance level.
    ```java
    int deviceLevel = ZegoEffects.getDeviceLevel().value();
    ```


2. Before calling the [create\|_blank](@create) interface to create the ZegoEffects object, pass the device performance level to the SDK through [setAdvancedConfig\|_blank](@setAdvancedConfig), and run the corresponding AI Face beautification effect according to the device performance level.
