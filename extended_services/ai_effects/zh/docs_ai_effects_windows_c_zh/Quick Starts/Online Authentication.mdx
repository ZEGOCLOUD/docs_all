...
开始实现鉴权之前，请在 [ZEGO 控制台](https://console.zego.im) 创建项目，获取到了接入 ZegoEffects SDK 服务所需的 AppID 和 AppSign，详情请参考 [控制台 - 项目管理](#12107) 中的“项目信息”；并联系 ZEGO 技术支持，开通 ZegoEffects 相关套餐服务权限。
...
1. 客户端向 ZegoEffects SDK 申请授权。
    ...
    调用 [zego_effects_get_auth_info](@zego_effects_get_auth_info) 接口，传入申请到的 AppSign，获取授权信息；ZegoEffects SDK 会返回授权信息 AuthInfo 到客户端。
...
| AuthInfo | string | 是 | 加密数据，由 SDK 产生的设备相关标识，通过 [zego_effects_get_auth_info](@zego_effects_get_auth_info) 接口获得。 |
...
开发者需要从返回字段 “Data” 的 “License” 字段中获取鉴权文件内容，然后调用 [zego_effects_create](@zego_effects_create) 接口，并将鉴权文件传给 ZegoEffects SDK，创建 Effects 对象。
...
在调用 [zego_effects_create](@zego_effects_create) 接口创建 Effects 对象之前，需要先调用 [zego_effects_set_resources](@zego_effects_set_resources) 接口导入资源和模型。
</Warning>
