...
1. 将 [下载](./../Downloads.mdx) 后的 SDK 目录下的 "ZegoEffects.xcframework" 文件夹，拷贝或替换到 "Example > Libs" 路径下。
2. 将根目录下的 "Models"，"Resources" 两个文件夹，拷贝或替换到 "Example > Resources" 路径下。
</Step>
<Step title="配置鉴权信息">
下载的示例源码中缺少 SDK 初始化所需的鉴权信息（AppID、AppSign），需要修改 "ZegoEffectsExampleLegacy" 目录下的 "ZegoLicense.m" 文件。请使用本文 [2 前提条件](9566#2) 所获取到的信息正确填写，否则源码无法正常运行。
<Warning>
