...
- 2.1.0 及以上版本，可直接在调用 [create](@create__1) 接口创建 Effects 对象时，使用 AppID 及 AppSign 鉴权，且无需关心鉴权逻辑，已在 SDK 内部实现。
</Warning>
...
1. 客户端向 ZegoEffects SDK 申请授权。
    ...
    调用 [getAuthInfo](@getAuthInfo) 接口，传入申请到的 AppSign，获取授权信息；ZegoEffects SDK 会返回授权信息 AuthInfo 到客户端。
