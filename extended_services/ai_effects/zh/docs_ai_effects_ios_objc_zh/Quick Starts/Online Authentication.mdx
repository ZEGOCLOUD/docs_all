...
- 2.1.0 及以上版本，可直接调用 [create\|_blank](/article/api?doc=Effects_SDK_API~objectivec_ios~class~ZegoEffects#create-appid-app-sign-callback-1) 接口创建 Effects 对象时，使用 AppID 及 AppSign 鉴权，且无需关心鉴权逻辑，已在 SDK 内部实现。
</Warning>
