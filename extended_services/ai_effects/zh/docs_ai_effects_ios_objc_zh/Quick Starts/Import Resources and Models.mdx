...
1. 在使用 ZegoEffects SDK 提供的 AI 功能前，请在 [下载\|_blank](#15898) 页面的 SDK 内部，获取最新版本的 AI 资源与模型（后缀为 bundle 或 model 的文件），并将其拷贝到您的工程中，以在后续步骤中使用。
    <Note title="Note">

    如果需要使用美颜，美型，美白，白牙，红润，挂件贴纸、大眼、瘦脸、人像分割等功能，请先导入 SDK 包内对应的资源与模型。
    </Note>
2. 指定 AI 资源和模型的绝对路径。**请在 [下载\|_blank](!ZegoEffects_downloads) 页面，获取最新版本的 AI 资源与模型（后缀为 bundle 或 model 的文件）。**
...
3. 在调用 [create\|_blank](https://doc-zh.zego.im/article/api?doc=Effects_SDK_API~objectivec_ios~class~ZegoEffects#create-appid-app-sign-callback-1) 接口创建对象之前，先调用 [setResources\|_blank](@setResources) 接口传入模型和资源路径列表，加载模型和资源。
