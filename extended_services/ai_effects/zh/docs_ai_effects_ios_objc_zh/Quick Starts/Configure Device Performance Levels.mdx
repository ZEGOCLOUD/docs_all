...
1. （可选）通过 [getDeviceLevel\|_blank](@getDeviceLevel) 获取当前设备性能等级。
    ```objc
    ZegoEffectsDeviceLevel deviceLevel = [ZegoEffects getDeviceLevel];
    ```

2. 在调用 [create\|_blank](@create) 接口创建 ZegoEffects 对象之前，通过 [setAdvancedConfig\|_blank](@setAdvancedConfig) 将设备性能等级传入 SDK，根据设备性能等级运行相应的 AI 美颜效果。
