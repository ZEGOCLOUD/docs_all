...
2. 使用 Xcode 打开 “ZegoEffectsExample.xcworkspace” 文件。
...
4. 修改 Bundle Identifier 和开发者证书。
    ...
    ![/Pics/ZegoEffects/Apple/effects_team_signing.png](http://doc.oa.zego.im/Pics/ZegoEffects/Apple/effects_team_signing.png)
    
    3. 在上图中找到 Bundle Identifier 选项，填入与自己开发者证书对应的包名。

5. 添加 SDK 依赖项和资源。 
    1. 将 [下载\|_blank](!ZegoEffects_downloads) 后的 SDK 目录下的 "ZegoEffects.xcframework" 文件夹，拷贝或替换到 “Example > Libs” 路径下。
    2. 将根目录下的 “Models”，“Resources” 两个文件夹，拷贝或替换到 “Example > Resources” 路径下。
    
6. 下载的示例源码中缺少 SDK 初始化所需的鉴权信息（AppID、AppSign），需要修改 “ZegoEffectsExampleLegacy” 目录下的 “ZegoLicense.m” 文件。请使用本文 [2 前提条件](9566#2) 所获取到的信息正确填写，否则源码无法正常运行。

    **请注意：此处需要填写鉴权文件，SDK 校验通过才可以正常使用 SDK 的功能。ZEGO 建议开发者定时更新、拉取鉴权数据，防止鉴权文件过期，导致 SDK 校验不通过的问题。**

    <Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/ZegoEffects/Apple/effects_enter_license_objc.png" /></Frame>

7. 将 iOS 设备连接到开发电脑，单击 Xcode 左上角的 “Generic iOS Device” 选择该 iOS 设备（或者模拟器）。
