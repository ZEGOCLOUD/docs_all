...
- 已在 [ZEGO 控制台](https://console.zego.im/) 开通了云端语音识别服务，详细请参考 [控制台 - 云端实时语音识别](/console/service-configuration/cloud-realtime-asr)。

## 使用步骤
### 时序图
```mermaid
sequenceDiagram
    participant Client as 客户端(ZEGO Express SDK)
    participant RTCServer as ZEGO 实时音视频(RTC) 后台
    participant Server as 业务后台
    participant ASRServer as ZEGO 云端实时语音识别后台

    Client->>RTCServer: 登录房间并推音频流
    rect rgb(125, 168, 237)
        Server->>ASRServer: 开启识别 (StartRealtimeASRTask)
        ASRServer-->>Server: 返回 TaskId
        ASRServer->>RTCServer: 从 RTC 房间拉取音视频流
        RTCServer-->>ASRServer: 音视频流数据
        ASRServer-->>Server: 识别结果回调 (Event: ASRResult)
        ASRServer->>ASRServer: 根据音视频流数据进行识别/翻译
        Note right of Server: 通过 TaskId 关联任务
        ASRServer-->>Client: 实时识别结果 (onRecvExperimentalAPI)
        Note right of Client: 业务后台开启识别时需设置 SubtitleType 为 1、2 或 3
    Server->>ASRServer: 停止识别 (StopRealtimeASRTask, TaskId)
    end
```
...
<Warning title="注意">
如果 RTC 房间 120 秒（默认值，可配置）后没有真实用户存在，则云端实时语音识别服务会自动停止，并触发 Event 为 Exception 的回调，Data.Code 为 1202。详情请参考 [返回码](./api-reference/return-codes.mdx)。
</Warning>
...
const data = JSON.stringify({
  ...
  "SubtitleType": 1 // 字幕下发类型，默认值为 0。0: 不下发；1: 仅下发识别结果（如用户说中文“你好”，则下发“你好”）；2: 仅下发翻译结果（如用户说中文“你好”，则下发翻译后的“Hello”）；3: 同时下发识别结果和翻译结果（如用户说中文“你好”，则同时下发“你好”和“Hello”）。若需要客户端展示UI，则推荐配置 1、2 或 3，详情见[展示字幕](./guides/display-subtitles.mdx)
});
...
const data = JSON.stringify({
  ...
  "Translation": {
    ...
    "TargetLanguage": "en",
    "LLM": {
      "//": "请自行开通并获取doubao模型的APIKey",
      "URL": "https://ark.cn-beijing.volces.com/api/v3/responses",
      "APIKey": "",
      "Model": "doubao-seed-translation-250915"
    }
  },
  "SubtitleType": 3 // 字幕下发类型，默认值为 0。0: 不下发；1: 仅下发识别结果；2: 仅下发翻译结果；3: 同时下发识别结果和翻译结果
});
