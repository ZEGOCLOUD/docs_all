...
sequenceDiagram
    ...
    rect rgb(40, 211, 60)
        ...
        Server->>Server: 将识别结果通过 RTC 房间消息或自定义消息通道传递给客户端
        Server-->>Client: 识别结果
        Server->>ASRServer: 停止云端实时语音识别
```

使用实时语音识别服务的核心步骤如图绿色块标注步骤所示。在您的业务后台调用[开启云端实时语音识别](./api-reference/start.mdx)接口后，云端实时语音识别后台会识别房间内所有音频流，通过提前配置好的[回调](./callbacks/receiving-callback.mdx)地址将识别结果回调给您的业务后台。

如果您需要实现实时字幕等需要在客户端显示识别内容的场景，您需要将识别结果通过 RTC 房间消息或自定义消息通道传递给客户端。
