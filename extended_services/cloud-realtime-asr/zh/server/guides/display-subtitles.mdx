...
- 已按照 [快速开始](./../quick-start.mdx) 文档实现基础功能：
    ...
    - 基于云端实时语音识别，开启云端实时语音识别，并配置 `SubtitleType` 为 `1`，`2` 或 `3`，从而将字幕通过房间信令下发。
<Warning title="注意">
必须使用 [下载 SDK 及 Demo](/aiagent-ios/introduction/download) 页面针对 Cloud ASR 优化的 ZEGO Express SDK 版本，否则无法正常显示字幕。
</Warning>
...
<Warning title="注意">建议您默认使用字幕组件实现。</Warning>
