...
| PauseInterval          | number | 否       | 用于设置用户说话停顿多少毫秒内，将两句话视为一句，即 ASR 多句拼接。范围 [200，2000]。仅当此值大于 VADSilenceSegmentation，才会开启 ASR 多句拼接。详细说明请参考[断句识别](#speech-segmentation-control)。 |
...
|配置|问答结果|
|---|---|
|VADSilenceSegmentation = 500ms，<br/>PauseInterval 不填|用户被判断为说了 2 句话：<br/>第1句：今天天气真好啊。我想出去玩<br/>第2句：你呢？<Note title="说明">由于400ms \< VADSilenceSegmentation 所以前两段记作第1句；800ms > VADSilenceSegmentation所以第三段被记作独立的第 2 句。</Note>|
|VADSilenceSegmentation = 500ms，<br/>PauseInterval = 1000ms|用户被判断为说了 1 句话：<br/>今天天气真好啊。我想出去玩。你呢？<Note title="说明">由于400ms \< PauseInterval 所以前两段被记作1句话， 800ms \< PauseInterval 所以第三段跟前一段被合并为同一句，所以最终结果为1句话。</Note>|
