...
<Frame width="100px" height="auto" caption="">
  <img src="https://media-resource.spreading.io/docuo/workspace564/27e54a759d23575969552654cb45bf89/a78bc0cbf0.png" alt="翻译字幕效果图"/>
</Frame>
...
sequenceDiagram
    ...
    participant ExpressSDK as Express SDK
...
| 步骤 | 说明 |
| --- | --- |
| 1 | 两位用户使用 Express SDK 加入同一房间，并分别以 `s1`、`s2` 为流 ID 推流。 |
...
3. 已下载并集成 [ZEGO Express SDK](/cloud-realtime-asr/introduction/download)。
    <Warning title="注意">
    必须使用[下载 SDK 及 Demo](/cloud-realtime-asr/introduction/download) 页面针对 Cloud ASR 优化的 ZEGO Express SDK 版本，否则无法正常显示字幕。
    </Warning>
...
请参考 [展示字幕](/cloud-realtime-asr/guides/display-subtitles#使用字幕组件) 文档，下载并集成字幕组件。
