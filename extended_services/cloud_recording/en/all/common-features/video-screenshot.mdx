...
To capture screenshots, set the `OutputFileFormat` to "jpg" when calling the [StartRecord](/cloud-recording-server/start-record) method. You can control the screenshot interval by setting the `SnapshotInterval` parameter. The service takes a screenshot of all streams every 10 seconds by default.
...
<Note title="Note">
When taking manual screenshots during mixed-stream recording, you will still only be charged according to the mixed-stream recording tier corresponding to the screenshot resolution, and no double recording fees will be incurred.
</Note>