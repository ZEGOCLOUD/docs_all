...
Integrated either the ZEGOCLOUD [Video Call](https://www.zegocloud.com/docs/real-time-video-android-java/introduction/overview) or [Voice Call](https://www.zegocloud.com/docs/real-time-voice-android/introduction/overview) SDK in your project.
...
Call all interfaces need to carry common parameters, please refer to [Making API requests](/cloud-recording-server/accessing-server-apis), signature information please refer to [Making API requests - Signature mechanism](/cloud-recording-server/making-api-requests#signing-the-requests), and the following is not repeated.
</Warning>
<Note title="Note">
...
To update the layout of the mixed streams during video recording, call the [`UpdateLayout`](/cloud-recording-server/update-layout) method multiple times. For more details, see [Set mixed-stream layout](./common-features/set-the-mixed-flow-layout.mdx).


### Update the whiteboard

To update the Whiteboard ID during recording, call the [`UpdateWhiteboard`](/cloud-recording-server/update-whiteboard) method multiple times. For more details, see [Multi-sector recording (Whiteboard)](./common-features/whiteboard-recording.mdx).
