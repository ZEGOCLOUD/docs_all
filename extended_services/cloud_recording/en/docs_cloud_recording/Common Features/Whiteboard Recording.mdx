...
To record the whiteboard, set the `HasWhiteboard` to `true` when calling the [`StartRecord`](/cloud-recording-server/start-recording) method.
...
To change a whiteboard, call the [`UpdateWhiteboard`](/cloud-recording-server/whiteboard-update) method, and set the `WhiteboardId` parameter with the ID of the whiteboard you want to change to.
