...
To record the whiteboard, set the `HasWhiteboard` to `true` when calling the [`StartRecord`](#7427) method.
...
To change a whiteboard, call the [`UpdateWhiteboard`](#7430) method, and set the `WhiteboardId` parameter with the ID of the whiteboard you want to change to.
