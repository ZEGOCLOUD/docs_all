...
To capture screenshots, set the `OutputFileFormat` to `jpg` when calling the [`StartRecord`](#7427) method. 
...
* When querying screenshot task status through the [DescribeRecordStatus](!API_Documents/query) method, the response parameters will not include the file list.
</Warning>
