...
To start cloud recording, use the [`StartRecord`](/cloud-recording-server/start-recording) method to get the room ID to join a specific room. 
...
- <b>ZEGOCLOUD recommends that you stop each recording task by calling the [StopRecord](/cloud-recording-server/stop-recording) method to avoid additional charges from continuous recording.</b>
</Note>
...
During recording, call the [`DescribeRecordStatus`](/cloud-recording-server/recording-status-check) method multiple times. After calling this method successfully, you can get the current recording status and the address to be uploaded from the response body.


### Update mixed-stream layout

To update the layout of the mixed streams during video recording, call the [`UpdateLayout`](/cloud-recording-server/update-the-mixed-stream-layout) method multiple times. For more details, see [Set mixed-stream layout](./Common%20Features/Set%20the%20Mixed%20Flow%20Layout.mdx).


### Update the whiteboard

To update the Whiteboard ID during recording, call the [`UpdateWhiteboard`](/cloud-recording-server/whiteboard-update) method multiple times. For more details, see [Multi-sector recording (Whiteboard)](./Common%20Features/Whiteboard%20Recording.mdx).

### Stop cloud recording

To stop the cloud recording, call the [`StopRecord`](/cloud-recording-server/stop-recording) method.
...
To check the uploading details, call the [`DescribeRecordStatus`](/cloud-recording-server/recording-status-check) method.
...
The recording file name contains information such as StreamId, timestamp of start recording. For more details, refer to the `OutputFileRule` parameter of the [`StartRecord`](/cloud-recording-server/start-recording) method.
