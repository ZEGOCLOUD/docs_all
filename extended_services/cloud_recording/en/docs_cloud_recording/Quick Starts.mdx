...
Integrated either the ZEGOCLOUD [Video Call\|_blank](https://www.zegocloud.com/docs/video-call/overview?platform=android&language=java) or [Voice Call\|_blank](https://www.zegocloud.com/docs/voice-call/overview?platform=android&language=java) SDK in your project.
...
To start cloud recording, use the [`StartRecord`\|_blank](#7427) method to get the room ID to join a specific room. 
...
- <b>ZEGOCLOUD recommends that you stop each recording task by calling the [StopRecord\|_blank](!API_Documents/stop) method to avoid additional charges from continuous recording.</b>

</Note>

And you will need to use this taskID as a request parameter for the subsequent requests. For details, see [Accessing Server APIs\|_blank](#7425).

### Query recording status

During recording, call the [`DescribeRecordStatus`\|_blank](#7431) method multiple times. After calling this method successfully, you can get the current recording status and the address to be uploaded from the response body.


### Update mixed-stream layout

To update the layout of the mixed streams during video recording, call the [`UpdateLayout`\|_blank](#7429) method multiple times. For more details, see [Set stream mixing layout\|_blank](#7412).


### Update the whiteboard

To update the Whiteboard ID during recording, call the [`UpdateWhiteboard`\|_blank](#7430) method multiple times. For more details, see [Whiteboard Recording\|_blank](#7413).

### Stop cloud recording

To stop the cloud recording, call the [`StopRecord`\|_blank](#7428) method.
...
To check the uploading details, call the [`DescribeRecordStatus`\|_blank](#7431) method.
...
The recording file name contains information such as StreamId, timestamp of start recording. For more details, refer to the `OutputFileRule` parameter of the [`StartRecord`\|_blank](#7427) method.
