...
|300001001|The recording task has stopped. The reason may be that you call the [Stop recording](/cloud-recording-server/stop-record) method or the recording conditions trigger the auto-stop recording. To know the conditions that trigger the auto-stop recording, refer to the [MaxIdleTime](/cloud-recording-server/start-record#maxidletime) and [MaxRecordTime](/cloud-recording-server/start-record#maxrecordtime) parameters. |
