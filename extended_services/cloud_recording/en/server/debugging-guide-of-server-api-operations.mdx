...
<Warning title="Note">
This document is no longer updated. We recommend using [How to call server API online](/cloud-recording-server/call-api-online) to debug.
</Warning>
...
1. Choose **StartRecord_Single** in **Collections** and view the **Body** tab.
    ...
    Modify the request parameters as required and perform the commissioning test. For more information about the parameters, see [Start recording](./start-record.mdx).
...
When you call the [Start recording](./start-record.mdx) API, you can start a screenshot capture task by setting **RecordMode** to **1** and **OutputFileFormat** to **jpg**. You can set **SnapshotInterval** to specify the screenshot interval. For more information, see [Capture screenshots](/cloud-recording/common-features/video-screenshot).
...
2. Click **Send** to check the response. If the error code in the response is not 0, modify the request based on the error description of the response and send the request again. For more information about the return parameters, see [Query recording status](/cloud-recording-server/describe-record-status).
