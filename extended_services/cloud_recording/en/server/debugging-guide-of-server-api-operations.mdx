...
1. Choose **StartRecord_Single** in **Collections** and view the **Body** tab.
    ...
    Modify the request parameters as required and perform the commissioning test. For more information about the parameters, see [Start recording](./start-recording.mdx).
...
2. Click **Send** to check the response. If the error code in the response is not 0, modify the request based on the error description of the response and send the request again. For more information about the return parameters, see [Query recording status](./recording-status-check.mdx).
