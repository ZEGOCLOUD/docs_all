...
请在 [ZEGO 控制台](https://console.zego.im) 完成注册、创建项目并在开启云录制录制服务。如需了解服务开通流程，请参考控制台文档 [服务配置 - 云端录制](https://doc-zh.zego.im/console/service-configuration/cloud-recording)。

### 集成 SDK

请确保您的项目已接入 ZEGO [实时音视频](https://doc-zh.zego.im/real-time-video-android-java/quick-start/integrating-sdk)/[实时语音](https://doc-zh.zego.im/real-time-voice-android/quick-start/integrating-sdk) SDK。
...
<Warning title="注意">
调用所有接口都需要带上公共参数，详见 [调用方式](/cloud-recording-server/making-api-requests)，签名信息详见 [调用方式 - 签名机制](/cloud-recording-server/making-api-requests#签名机制)，之后不再赘述。
</Warning>
<Note title="说明">
...
混流录制过程中可以多次调用 [UpdateLayout](/cloud-recording-server/update-layout) 方法更新混流布局，详情请参考 [设置混流布局](../common-features/set-the-mixed-flow-layout.mdx)。

### 4 更新白板

白板录制过程中可以多次调用 [UpdateWhiteboard](/cloud-recording-server/update-whiteboard) 方法更新白板 Id，详情请参考 [白板录制](../common-features/whiteboard-recording.mdx)。
...
- [录制结束后没有在云存储中看到录制文件？](https://doc-zh.zego.im/faq/recording-missing)
- [支持录制动态 PPT 吗？](https://doc-zh.zego.im/faq/dynamic-ppt-recording)
- [支持录制 IM 信息吗？](https://doc-zh.zego.im/faq/im-recording)
- [什么时候会收到回调通知？](https://doc-zh.zego.im/faq/recording-callback-timing)
- [用户程序崩溃对云录制会有什么影响？](https://doc-zh.zego.im/faq/crash-impact-recording)
- [第三方云存储参数（StorageParams）支持传入临时授权的 Token 吗？](https://doc-zh.zego.im/faq/storage-temp-token)
- [录制 PPT 时，能隐藏 PPT Notes 吗？](https://doc-zh.zego.im/faq/ppt-notes-hiding)
- [云端录制服务器出现断网、异常崩溃时，会如何处理？](https://doc-zh.zego.im/faq/cloud-recording-failure-handling)
