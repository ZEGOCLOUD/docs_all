...
在调用 [StartRecord](/cloud-recording-server/start-record) 方法时，将 `OutputFileFormat` 设置为 “jpg” 即可进行视频截图。您可以通过设置 `SnapshotInterval` 参数控制截图的周期，默认为每 10 秒对房间内的所有流截一次图。
...
<Note title="说明">
在混流录制时进行手动截图，仍只会按照截图分辨率对应的混流录制档位收费，不会收取两次录制费用。
</Note>