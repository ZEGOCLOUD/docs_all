...
- [接入地址](#11-接入地址)：ZEGO 服务端的接入地址，根据不同的产品和地域会有所不同。
- [公共参数](#2-公共参数)：每个请求都须有一系列公共参数。
- [签名](#3-签名机制)：签名也属于公共参数，需要根据相应的签名算法生成。
...
<tbody>
  ...
  <tr>
    <td>SignatureNonce</td>
    <td>随机字符串，即公共参数里的 SignatureNonce，生成算法可参考 <a href="#3-3-签名示例">3.3 签名示例</a>。</td>
  </tr>
  <tr>
    <td>ServerSecret</td>
