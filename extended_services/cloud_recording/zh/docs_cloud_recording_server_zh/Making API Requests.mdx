...
- 参考 [使用 Postman 调测指南\|_blank](!Server_APIs_v2-Postman/Postman)，使用 ZEGO 提供的 Postman API 集合，使用 Postman 快速调试相应接口。

## 请求方式概述

服务端 API 请求由不同内容构成，有固定的请求结构：

- [接入地址](!Making_API_Requests/Making_API_Requests#3_1)：ZEGO 服务端的接入地址，根据不同的产品和地域会有所不同。
- [公共参数](!Making_API_Requests/Making_API_Requests#4)：每个请求都须有一系列公共参数。
- [签名](!Making_API_Requests/Making_API_Requests#5)：签名也属于公共参数，需要根据相应的签名算法生成。
- 请求参数：您需要通过 Action 参数指定目标操作，例如 Action = StartMix；还需要指定接口的其他参数。

我们根据您的签名验证了请求后，会返回结果给您。接口调用成功会显示返回参数，调用失败则显示相应报错，您可以根据 [全局返回码\|_blank](!Server_APIs_v2-Return_Codes/Return_Codes) 分析排查。

<details class="zg-primary">
    <summary>服务端 API 请求示例</summary>

以下为 [`StartMix`\|_blank](#8791)（开始混流）接口的请求结构为例：
...
- 其它参数：即公共请求参数，是每个接口都需要使用到的请求参数，包含 AppId、SignatureNonce、Signature 等，具体请参考 [公共请求参数](!Making_API_Requests/Making_API_Requests#4_1)。


此外，ZEGO 提供多种编程语言的示例代码，开发者可根据实际情况进行参考。以下是 [`StartMix`\|_blank](#8791)接口的 Node.js 代码：
...
var url = `https://rtc-api.zego.im/?Action=StartMix&AppId=${appId}&SignatureNonce=${signatureNonce}&Timestamp=${timeStamp}&Signature=${sig}&SignatureVersion=2.0`;
var body = {
    'TaskId': '123',
    'Sequence': 123,
    'UserId': '123',
...
<tr>
<td>中国⼤陆（上海）</td>
<td>cloudrecord-api-sha.zego.im</td>
</tr>
<tr>
<td>港澳台（⾹港）</td>
<td>cloudrecord-api-hkg.zego.im</td>
</tr>
<tr>
<td>欧洲（法兰克福）</td>
<td>cloudrecord-api-fra.zego.im</td>
</tr>
<tr>
<td>美⻄（加州）</td>
<td>cloudrecord-api-lax.zego.im</td>
</tr>
<tr>
<td>亚太（孟买）</td>
<td>cloudrecord-api-bom.zego.im</td>
</tr>
<tr>
<td>东南亚（新加坡）</td>
<td>cloudrecord-api-sgp.zego.im</td>
</tr>
<tr>
<td>统一接入地址（不区分区域）</td>
<td>cloudrecord-api.zego.im</td>
</tr>
</tbody></table>
...
<tbody>
  ...
  <tr>
    <td>SignatureNonce</td>
    <td>随机字符串，即公共参数里的 SignatureNonce，生成算法可参考 <a href="!Making_API_Requests/Making_API_Requests#5_3">3.3 签名示例</a>。</td>
  </tr>
  <tr>
    <td>ServerSecret</td>
  ...
  <tr>
    <td>Timestamp</td>
    <td>当前 Unix 时间戳，单位为秒。生成算法可参考下方签名示例，最多允许 10 分钟的误差。
    </td>
  </tr>
</tbody>
</table>
