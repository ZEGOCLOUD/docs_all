...
调用 [开始录制](./start.mdx) 接口时，设置 RecordMode 为 1 并设置 OutputFileFormat 为 “jpg” 即可启动视频截图任务，并可通过 SnapshotInterval 设置截图的间隔，详见 [视频截图](/cloud-recording/common-features/video-screenshot)。
...
2. 单击 “Send”，在下方查看响应内容。若响应消息的错误码不是 0，请根据响应消息中的错误描述修改请求再次发送。返回参数的具体含义请参考 [查询录制状态](./query.mdx)。
