...
| 参数 | 类型 | 是否必选 | 描述 |
|------|------|----------|------|
| <p id="OutputFileFormat">OutputFileFormat</p> | String | 否 | 指定录制生成文件的格式，默认为 "mp4"。目前支持 "mp4"、"flv"、"hls"、"jpg" 和 "mp3"。如果录制 "mp4" 或 "flv"，且 <code>StreamType</code> 设为 4，则录制生成的音频文件格式为 aac。 |
