...
To use the skin tone enhancement feature, import the related resource file `FaceWhiteningResources` first; otherwise, this feature can't take effect. For details, see [Import resources and models](9920#1).
...
For this feature to work correctly, you need to import the related resource files first. For details, see [Import resources](9920#1).
...
The value range of the "intensity" parameter in the [ZegoEffectsAcneRemovingParam](@-ZegoEffectsAcneRemovingParam) object is [0, 100], with a default value of "0". A larger value indicates a stronger acne and spot removal effect, and developers can customize the value according to their needs.
...
Before using the clarity enhancement feature, please import the corresponding "ClarityResources" resource file, otherwise the feature will not work. For details, please refer to [Quick Starts - Import Resources and Models](!Quick_Starts/Import_Resources).
</div>
...
The value range of the "intensity" parameter in the [ZegoEffectsClarityParam](@-ZegoEffectsClarityParam) object is [0, 100], with a default value of "0". A larger value indicates a clearer image, and developers can customize the value according to their needs.
...
Before using the skin color adjustment feature, please import and load the corresponding skin resources file, otherwise the feature will not work. For details, please refer to [Quick Starts - Import Resources and Models](!Quick_Starts/Import_Resources).
</div>

Call the [enableSkinColor](@enableSkinColor) interface to enable the skin color adjustment feature, use the [setSkinColorResPath](@setSkinColorResPath) interface to load the supported skin resources, and then use the [setSkinColorParam](@setSkinColorParam) interface to set the intensity of the skin color effect.

The value range of the "intensity" parameter in the [ZegoEffectsSkinColorParam](@-ZegoEffectsSkinColorParam) object is [0, 100], with a default value of "0". A larger value indicates a more noticeable change in skin color, and developers can customize the value according to their needs.
