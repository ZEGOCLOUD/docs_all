...
<Steps>
<Step title="获取资源和模型">
在使用 ZegoEffects SDK 提供的 AI 功能前，请在 [下载](./../Downloads.mdx) 页面的 SDK 内部，获取最新版本的 AI 资源与模型（后缀为 bundle 或 model 的文件），并将其拷贝到您的工程中，以在后续步骤中使用。
<Note title="说明">
...
<Step title="加载资源和模型">
在调用 [zego_effects_create](@zego_effects_create) 接口创建对象之前，先调用 [zego_effects_set_resources](@zego_effects_set_resources) 接口传入模型和资源路径列表，加载模型和资源。
