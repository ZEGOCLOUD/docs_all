...
1. 开启滤镜。
    ...
    调用 [zego_effects_set_filter](@zego_effects_set_filter) 接口开启滤镜功能，传入对应的滤镜绝对路径。调用 [zego_effects_set_filter_param](@zego_effects_set_filter_param) 接口设置滤镜强度。

    [zego_effects_filter_param](@-zego_effects_filter_param) 对象的滤镜强度参数 “intensity” 的取值范围为 [0, 100]。取值越大，效果呈现的程度越大，开发者可以根据需求自定义取值。
...
2. 移除滤镜。
    ...
    如果需要移除滤镜，可将滤镜资源路径设置为空，传入 [zego_effects_set_filter](@zego_effects_set_filter) 接口。
