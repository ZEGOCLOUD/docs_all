...
- 已获取到 Effects SDK 唯一的鉴权文件，详情请参考 [快速开始 - 在线鉴权](./online-authentication.mdx)。
- 登录 [ZEGO 控制台](https://console.zego.im) 创建项目，获取接入 ZegoEffects SDK 服务所需的 AppID 和 AppSign，详情请参考 [控制台 - 项目信息](/console/project-info)。
...
使用 Effects 的 AI 相关功能前，必须调用 [setResources](@setResources) 接口导入 AI 资源或模型，详情请参考 [快速开始 - 导入资源和模型](/ai-effects-android-java/quick-starts/import-resources-and-models)。
...
调用 [setAdvancedConfig](@setAdvancedConfig) 接口，部署高级配置项，如配置设备性能等级，详情请参考 [配置设备性能等级](./configure-device-performance-levels.mdx)。
...
将 [前提条件](#前提条件) 中申请到的 AppID、AppSign 直接传入 [create](@create__2) 接口，由 SDK 内部鉴权之后，创建 Effects 对象并返回相关 [错误码](./error-codes.mdx)。
...
传入待处理的原始图像宽、高，调用 [initEnv](@initEnv) 接口初始化 Effects 对象。
...
<Step title="开启 AI 功能">
调用 [enableWhiten](@enableWhiten)/[enableBigEyes](@enableBigEyes)/[setPortraitSegmentationBackgroundPath](@setPortraitSegmentationBackgroundPath)/[enablePortraitSegmentation](@enablePortraitSegmentation) 接口开启各项 AI 功能。
```java
// 1. 开启美白功能
// 2. 开启大眼功能
...
在 1.4.11 及以后版本，通过 [processTexture](@processTexture) 接口对 OES 类型纹理进行图像处理：
```java
ZegoEffectsVideoFrameParam zegoEffectsVideoFrameParam = new ZegoEffectsVideoFrameParam();
