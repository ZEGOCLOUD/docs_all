...
- 已在项目中集成 ZEGO MiniGameEngine SDK，详情请参考 [集成 SDK\|_blank](#17681)。
- 已在项目中集成 ZEGO Express SDK，详情请参考实时音视频的 [集成 SDK\|_blank](#196)。

## 示例源码

ZEGO 提供了 [示例源码\|_blank](#17677)，以供开发者进一步了解如何实现弹幕游戏。
...
请参考 [服务端 API - 调用方式\|_blank](#17807) 了解如何接入 ZEGO 小游戏产品的服务端，并在您的服务端上实现 [获取游戏启动码\|_blank](#18357)，以便后续 <a href="#startCloudGame">主播启动弹幕游戏</a> 前获取游戏启动码。

ZEGO 为您提供了[服务端示例源码\|_blank](#17647)，可供参考。但为了您的数据安全，请勿将其直接用于线上生产环境。
...
请参考实时音视频的 [实现视频通话 - 创建引擎\|_blank](#7628#CreateEngine)，创建 ZegoExpressEngine。

完成初始化之后，请参考 实时音视频的 [实现视频通话 - 登录房间\|_blank](#7628#createroom)，登录 RTC 房间。
...
调用 [ZegoMiniGameEngine > sharedEngine\|_blank](@sharedEngine-ZegoMiniGameEngine) 接口创建 ZegoMiniGameEngine 单例对象。调用 [init\|_blank](@init) 接口，传入申请到的 AppID、您服务端生成的 Token、自定义的用户 ID 和用户名称、以及头像图片 URL 参数，初始化 ZEGO MiniGameEngine SDK。

调用 [ZegoCloudGameEngine > sharedEngine\|_blank](@sharedEngine-ZegoCloudGameEngine) 接口获取 ZegoCloudGameEngine 实例。
...
调用 [setCloudGameContainer\|_blank](@setCloudGameContainer) 设置游戏父容器，以便后续启动游戏。调用 [setCloudGameEngineHandler\|_blank](@setCloudGameEngineHandler) 接口注册监听小游戏事件，相关事件在 <a href="#startCloudGame">启动弹幕游戏</a> 后通过 [ZegoCloudGameEngineHandler\|_blank](@-ZegoCloudGameEngineHandler) 回调返回。 
...
调用 [getGameList\|_blank](@getGameList) 接口，传入 `ZegoGameModeCloudGame`，指定游戏模式为弹幕游戏模式，获取弹幕游戏列表，在列表中选择想玩的弹幕游戏 ID。
