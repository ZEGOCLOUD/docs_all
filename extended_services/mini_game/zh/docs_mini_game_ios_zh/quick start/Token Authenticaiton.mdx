...
用户在调用 [init](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoMiniGameEngine) 接口初始化 ZEGO MiniGameEngine SDK 时传入 Token。
...
在 Token 过期前 30 秒，SDK 会通过 [onTokenWillExpire](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoGameEngineHandler) 回调发出通知。

收到该回调后，您需要从自己的服务端获取新的有效 Token，并调用 SDK 提供的 [updateToken](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoMiniGameEngine) 接口更新 Token。
