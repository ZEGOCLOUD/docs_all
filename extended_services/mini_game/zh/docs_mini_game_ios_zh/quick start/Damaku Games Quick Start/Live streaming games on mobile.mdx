...
- 已在 [ZEGO 控制台](https://console.zego.im) 创建项目，获取项目的 AppID，详情请参考 [控制台 - 项目信息](/console/project-info)。
- 已联系 ZEGO 商务人员开通小游戏服务，不同类型的游戏权限需要分别开通，否则可能会导致无法体验。
- 已在项目中集成 ZEGO MiniGameEngine SDK，详情请参考 [集成 SDK](https://doc-zh.zego.im/faq/mini-game-alternatives)。
- 已在项目中集成 ZEGO Express SDK，详情请参考实时音视频的 [集成 SDK](https://doc-zh.zego.im/real-time-video-ios-oc/quick-start/integrating-sdk)。

## 示例源码

ZEGO 提供了 [示例源码](https://doc-zh.zego.im/faq/mini-game-alternatives)，以供开发者进一步了解如何实现弹幕游戏。
...
为便捷项目测试，您可以从 [ZEGO 控制台](https://console.zego.im/) 获取临时 Token，详情请参考控制台的 [项目信息 - 辅助工具](/console/project-info#2_3)。但项目上线时，请从您的服务端生成 Token，以保障安全。
</Note>
...
请参考实时音视频的 [实现视频通话 - 创建引擎](https://doc-zh.zego.im/real-time-video-ios-oc/quick-start/implementing-video-call)，创建 ZegoExpressEngine。

完成初始化之后，请参考 实时音视频的 [实现视频通话 - 登录房间](https://doc-zh.zego.im/real-time-video-ios-oc/quick-start/implementing-video-call)，登录 RTC 房间。
...
调用 [ZegoMiniGameEngine > sharedEngine](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoMiniGameEngine) 接口创建 ZegoMiniGameEngine 单例对象。调用 [init](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoMiniGameEngine) 接口，传入申请到的 AppID、您服务端生成的 Token、自定义的用户 ID 和用户名称、以及头像图片 URL 参数，初始化 ZEGO MiniGameEngine SDK。

调用 [ZegoCloudGameEngine > sharedEngine](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoCloudGameEngine) 接口获取 ZegoCloudGameEngine 实例。
...
调用 [setCloudGameContainer](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoCloudGameEngine) 设置游戏父容器，以便后续启动游戏。调用 [setCloudGameEngineHandler](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoCloudGameEngine) 接口注册监听小游戏事件，相关事件在 <a href="#33-启动弹幕游戏">启动弹幕游戏</a> 后通过 [ZegoCloudGameEngineHandler](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoCloudGameEngineHandler) 回调返回。 
...
调用 [getGameList](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoMiniGameEngine) 接口，传入 `ZegoGameModeCloudGame`，指定游戏模式为弹幕游戏模式，获取弹幕游戏列表，在列表中选择想玩的弹幕游戏 ID。
...
调用 [startCloudGame](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoCloudGameEngine) 接口，传入游戏房间 ID 和游戏 ID，启动游戏。
...
游戏启动后，调用 [startPublishCloudGameStream](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoCloudGameEngine) 接口，将弹幕游戏视频流转推到 RTC 房间时。
...
当需要停止转推游戏视频流时，请调用 [stopPublishCloudGameStream](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoCloudGameEngine) 接口。
...
当需要停止游戏时，调用 [stopCloudGame](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoCloudGameEngine) 接口停止弹幕游戏。
...
请参考实时音视频的 [实现视频通话 - 退出房间](https://doc-zh.zego.im/real-time-video-ios-oc/quick-start/implementing-video-call)，完成退出 RTC 房间。

####  销毁 ZegoExpressEngine

请参考实时音视频的 [实现视频通话 - 销毁引擎](https://doc-zh.zego.im/real-time-video-ios-oc/quick-start/implementing-video-call)，完成销毁。

<a id="unInit"></a>

####  反初始化 ZEGO MiniGameEngine SDK

调用 [unInit](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoMiniGameEngine) 接口反初始化 ZEGO MiniGameEngine SDK。
...
主播在移动端开播弹幕游戏后，如果游戏内没有提供键盘，需要调用 [sendKeyboardEvent](https://doc-zh.zego.im/faq/general-product-inquiry/mini-game-alternative?doc=ZegoMiniGameEngine_API~objective-c_ios~class~ZegoCloudGameEngine) 接口，传入 key（键盘的 code 码），实现键盘事件，如前进、调整视角等，以便正常玩游戏。如需了解键盘 code 码，请参考 <a href="https://www.toptal.com/developers/keycode" target="_blank" rel="noopener noreferrer">JavaScript Key Code Event Tool</a>。
