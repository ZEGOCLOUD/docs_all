...
- 已在项目中集成 ZEGO MiniGameEngine SDK，详情请参考 [集成 SDK\|_blank](#17682)。
- 已在项目中集成 ZEGO Express SDK，详情请参考实时音视频的 [集成 SDK\|_blank](#195)。

## 示例源码

ZEGO 提供了 [示例源码\|_blank](#17678)，以供开发者进一步了解如何实现弹幕游戏。
...
请参考 [服务端 API - 调用方式\|_blank](#17807) 了解如何接入 ZEGO 小游戏产品的服务端，并在您的服务端上实现 [获取游戏启动码\|_blank](#18358)，以便后续 <a href="#startCloudGame">主播启动弹幕游戏</a> 前获取游戏启动码。

ZEGO 为您提供了[服务端示例源码\|_blank](#17648)，可供参考。但为了您的数据安全，请勿将其直接用于线上生产环境。

#### 1.2 获取 Token

当您的用户在使用 ZEGO 小游戏服务时，ZEGO 服务端会根据用户初始化 SDK 时携带的 Token 参数判断用户是否有权限，避免因权限控制缺失或操作不当引发的风险问题。

请参考 [使用 Token 鉴权\|_blank](#17686) 文档，在您的服务端生成 Token 并返回给向用户客户端。
...
请参考实时音视频的 [实现视频通话 - 创建引擎\|_blank](#7627#CreateEngine)，创建 ZegoExpressEngine。

完成初始化之后，请参考 实时音视频的 [实现视频通话 - 登录房间\|_blank](#7627#createroom)，登录 RTC 房间。

<a id="initMiniGameEngine"></a>

#### 2.2 初始化 ZEGO MiniGameEngine SDK  

调用 [ZegoMiniGameEngine > getInstance\|_blank](@getInstance-ZegoMiniGameEngine) 接口创建 ZegoMiniGameEngine 实例对象。调用 [init\|_blank](@init) 接口，传入申请到的 AppID、您服务端生成的 Token、自定义的用户 ID 和用户名称、以及头像图片 URL 参数，初始化 ZEGO MiniGameEngine SDK。

调用 [ZegoCloudGameEngine > getInstance\|_blank](@getInstance-ZegoCloudGameEngine) 接口创建弹幕游戏引擎实例对象。
...
调用 [setCloudGameContainer\|_blank](@setCloudGameContainer) 设置游戏父容器，以便后续启动游戏。调用 [setCloudGameEngineHandler\|_blank](@setCloudGameEngineHandler) 接口注册监听小游戏事件，相关事件在 [启动弹幕游戏](18351#startCloudGame) 后通过 [IZegoCloudGameEngineHandler\|_blank](@-IZegoCloudGameEngineHandler) 回调返回。 
...
调用 [getGameList\|_blank](@getGameList) 接口，传入 `ZegoGameMode.ZegoGameModeCloudGame`，指定游戏模式为弹幕游戏模式，获取弹幕游戏列表，在列表中选择想玩的弹幕游戏 ID。
...
请参考实时音视频的 [实现视频通话 - 退出房间\|_blank](#7627#logoutRoom)，完成退出 RTC 房间。

#### 5.2 销毁 ZegoExpressEngine

请参考实时音视频的 [实现视频通话 - 销毁引擎\|_blank](#7627#destroy)，完成销毁。
...
主播在移动端开播弹幕游戏后，如果游戏内没有提供键盘，需要调用 [sendKeyboardEvent\|_blank](@sendKeyboardEvent) 接口，传入 key（键盘的 code 码），实现键盘事件，如前进、调整视角等，以便正常玩游戏。如需了解键盘 code 码，请参考 <a href="https://www.toptal.com/developers/keycode" target="_blank" rel="noopener noreferrer">JavaScript Key Code Event Tool</a>。
