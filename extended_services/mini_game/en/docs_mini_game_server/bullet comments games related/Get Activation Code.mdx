...
The following request parameter list only lists interface request parameters. Please refer to [Make API requests\|_blank](!ZegoMiniGameEngine-Making_API_Requests#2_1) for the public parameter list.
...
Only the return codes related to the interface business logic are listed below. For complete return codes, please refer to [Global return codes\|_blank](!ZegoMiniGameEngine-Return_Codes).

| Return code | Description | Handling suggestions |
| -- | -- | -- |
| 7202001 |  The game was not found. Possible reasons include: <ul><li>MiniGameId input error.</li><li>The permissions for this game are not activated.</li> | <ul><li>Please check if the MiniGameId is correct.</li><li> Please contact ZEGOCLOUD business personnel to activate permission for this game.</li></ul> |
