...
Before you begin, make sure you complete the integration by referring to the [Quick start with co-hosting](https://www.zegocloud.com/docs/uikit/live-streaming-kit-flutter/quick-start/quick-start-(with-cohosting)).
...
To display gift animations to the host and viewers, send a notification using the [send in-room messages](https://www.zegocloud.com/docs/zim-server/messaging/send-room-messages) API with `MessageType: 2`. 
...
To access the sample code for this feature, download it from [here](https://github.com/ZEGOCLOUD/zego_virtual_gift/tree/main/samples/flutter/live_audio_room).
