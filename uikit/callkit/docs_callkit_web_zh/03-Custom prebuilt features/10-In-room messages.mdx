...
- 您可以调用服务端 API [发送房间消息](https://doc-zh.zego.im/zim-server/messaging/send-room-messages) 发送 MessageType=2 的信令消息。
...
1. 发送礼物：您可以调用服务端 API [发送房间消息](https://doc-zh.zego.im/zim-server/messaging/send-room-messages) 发送 MessageType=2 的命令消息，以及对命令消息进行编码以表示不同的礼物。
