...
On top of the initialization code added in Quick Start, you need to set `ZegoUIKitBeautyPlugin` to the `plugins` parameter. Additionally, please refer to [Online authentication](https://www.zegocloud.com/docs/ai-effects-android-java/quick-starts/online-authentication) to obtain the license and set it to the `beauty` parameter of `ZegoUIKitPrebuiltLiveStreamingConfig`.
...
For portrait segmentation in background segmentation, you need to provide a background image. You need to create a new **background.bundle** folder to store background images and set it up through the config.


```dart
resources/background.bundle/image1.jpg
```
