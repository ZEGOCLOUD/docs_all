...
```swift
let liveVC = ZegoUIKitPrebuiltLiveStreamingVC(yourAppID, appSign: yourAppSign, userID: yourUserID, userName: yourUserName, liveID: yourLiveID, config: config)
...
To access the sample code for this feature, download it from [here](https://github.com/ZEGOCLOUD/zego_virtual_gift/tree/main/samples/ios/live_streaming).
