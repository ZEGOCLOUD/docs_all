...
- 在 [ZEGO 控制台](https://console.zego.im/ProjectManage) 中注册并创建项目，并联系销售或售前 开通 AI Agent、数字人服务和相关接口权限
...
数字人会根据绑定的配置，在指定的 roomId 下，以 agent_user_id 作为 userId 推流（streamId 为 agent_stream_id），同时根据绑定的配置拉取 user_id 的流（streamId 为 user_stream_id）。如果这些 ID 配置不一致，将导致推流或拉流失败。
...
<Note title="说明">以下以 Android 为例，其他平台请参考对应平台的 SDK 文档。</Note>
...
<Note title="说明">建议在开始大班课时预下载数字人资源。如果数字人形象没有变更，在 APP 启动后仅预下载一次即可。</Note>
<CodeGroup>
```java title="向后台请求 Token"
var userToken = ""
...
```java title="initDigitalMobileSDK"
...
```java title="初始化 ZEGO Express SDK"
//1.创建引擎前，设置相关参数配置
