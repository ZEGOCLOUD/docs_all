...
本文基于 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice) 来介绍的如何实现语聊房功能，因此文档中的部分方法为示例代码封装方法。
</div>
...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成 SDK\|_blank](!ExpressAudioSDK-Integration/SDK_Integration)。
- 下载本文档附带的 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice)。
- 在 [管理控制台\|_blank](https://console.zego.im/ProjectManage) 中注册并创建项目，同时开通 “即时通讯” 服务。
  ![](/Pics/InappChat/ActivateZIMinConsole2_zh.png)

## 预览效果

您可以通过本文档提供的 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice) 实现以下效果：

|主页|房主页面|观众页面|观众点击上麦请求|房主查看请求|
|--- | --- | --- |--- |--- |
|<img src="/Pics/zegocloud/audioroom/home_page.jpg" width=80%>|<img src="/Pics/zegocloud/audioroom/host.jpg" width=80%>|<img src="/Pics/zegocloud/audioroom/audience.jpg" width=80%>|<img src="/Pics/zegocloud/audioroom/audience_takeseat.jpg" width=80%>|<img src="/Pics/zegocloud/audioroom/host_requests.jpg" width=80%>|

房主可以单击右下角的“锁定图标”来更改房间模式。
- 自由模式：观众点击麦位后，即可成为发言人。
- 请求模式：观众点击麦位触发上麦请求后，必须等待房主同意才可成为发言人。


## 技术原理

如果您需要了解推流及拉流的相关接口使用，请参考 [快速开始 - 实现音频通话文档\|_blank](!ExpressAudioSDK-Integration/Solution_Implementation)。

在一个语聊房中：

- 所有观众进入房间后可以开始拉流，以听取房间中发言人的声音。
- 发言人在坐上麦位后即可开始推流，将本地音频传输给房间内的观众。

### 如何管理发言人麦位

除了实现上述逻辑外，语聊房还需要管理发言人麦位。发言人麦位管理功能通常可以使用 ZIM SDK 的 [房间属性管理](!IM-Advanced_Features/Room_Advanced_Features) 特性实现。

该特性允许开发者客户端在房间中设置和同步自定义的房间属性。房间属性以键值对的方式存储在 ZEGO 服务器上，由 ZEGO 服务器解决写入冲突和其他问题，以确保数据一致性。

同时，开发者客户端对房间属性的修改会通过 ZEGO 服务器实时同步给房间中的所有其他观众。

<div class="mk-warning">

- 每个房间最多允许设置 20 个属性，`key` 的长度限制为 16 字节，`value` 的长度限制为 1024 字节。 开发者如果需要提高属性上限，请联系 ZEGO 技术支持。
...
您可以使用语聊房中的发言人麦位号，作为房间属性的键，并使用 `userID` 作为房间属性的值，来表示房间的发言人麦位状态。

例如，如果 userID 为 “user123” 的用户位于 0 号发言人麦位上，而 userID 为 “user456” 的用户位于 1 号发言人麦位上，则其房间属性如下：

```json
{
  "0":"user123", // 表示 user123 位于 0 号发言人麦位上
  "1":"user456", // 表示 user456 位于 1 号发言人麦位上
}
```

**房间属性功能的设计，可以解决语聊房场景中，一些常见发言人麦位管理问题：**
...
<tr>
<td>所有者</td>
<td>第一个设置键的观众将成为该键的所有者。默认情况下，只有所有者可以修改键。</td>
<td>可用于避免抢占发言人麦位时的冲突。</td>
</tr>
<tr>
<td>自动删除</td>
<td>在设置键时，可以将键配置为“所有者离开房间后自动删除”。</td>
<td>可用于实现“发言人离线后发言人麦位自动更新”的功能，避免由于开发者客户端断开连接导致发言人麦位混乱的问题。</td>
</tr>
<tr>
<td>强制修改</td>
<td>支持忽略所有者并强制修改键值。</td>
<td>可用于实现“房主强制将观众从发言人麦位上移除”的功能。</td>
</tr>
<tr>
<td>组合操作</td>
<td>可以将对不同键值的多个操作组合成一个组合操作，避免其他用户操作相关键值时产生冲突。</td>
...
- 自由模式：观众点击麦位后，即可成为发言人。
- 请求模式：麦下观众点击麦位触发上麦请求后，必须等待房主同意才可成为发言人。

房间模式可使用 Express SDK 的 [setRoomExtraInfo\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#set-room-extra-info-for-key-room-id-callback) 实现。“房间附加信息”与上述的“房间属性”类似，也存储在 ZEGO 服务器上，但是“房间附加信息”的使用更简单：

没有复杂的参数，只支持设置键值字符串（键最大 10 字节，值最大 128 字节），更适合与房间绑定的简单业务操作，如房间模式、房间公告等。

您可以将任何业务字段封装成 JSON 协议，并将其设置为“房间附加信息”以实现房间模式等业务逻辑。

当房主调用 [setRoomExtraInfo\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#set-room-extra-info-for-key-room-id-callback) 方法时，房间内的用户可以通过 [onRoomExtraInfoUpdate\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-room-extra-info-update-room-id) 接收到设置的`房间附加信息`。

<pre class="mermaid" style="max-width: 100%;">
sequenceDiagram
    participant 用户 A
    participant sdk1 as Express SDK
    participant server as ZEGO 服务器
    participant sdk2 as Express SDK
    participant 用户 B
...
ZIM SDK 提供了丰富的功能来发送和接收消息，详情可参考 [收发消息\|_blank](!IM-sendMessages)。此处，您需要使用可自定义的房间请求消息：[ZIMCommandMessage\|_blank](/article/api?doc=zim_API~objectivec_ios~class~ZIMCommandMessage)

<div class="mk-hint">

此部分的完整示例代码，可参考 [ZIMService+Room.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/master/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService%2BRoomRequest.swift)。
</div>


1. 通过调用 [sendMessage\|_blank](/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#send-message-message-to-conversation-id-conversation-type-config-notification-callback) 方法在房间中发送房间请求信令（[ZIMCommandMessage\|_blank](/article/api?doc=zim_API~objectivec_ios~class~ZIMCommandMessage)）：
...
2. 发送后成功后，房间中的其他用户将通过 [receiveRoomMessage\|_blank](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-receive-room-message-from-room-id) 回调接收房间请求信令。您可以通过以下方式监听此回调：
...
此部分的完整示例代码，可参考 [ZIMService.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/master/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService.swift)。
</div>
...
- 静音观众：在收到相应的房间请求信令后，可阻止用户发送弹幕消息。
- 发送虚拟礼物：在收到房间请求信令后，可显示礼物特效。
- 移除观众：在收到房间请求信令后，提示观众已被移除并退出房间。
...
本文档中的演示是一个纯客户端 API + ZEGOCLOUD 解决方案。如果您有自己的业务服务端，并希望进行更多的逻辑扩展，您可以使用我们的 [服务端 API\|_blank](#12197) 来传递房间信令，并结合您服务端的房间业务逻辑，以提高您的应用的可靠性。
...
<details class="zg-primary">
    <summary>1. 导入 ZIM SDK</summary>

开发者可使用 Swift Package Manager 自动集成 SDK，更多导入方式，请参考 [即时通讯-实现基本消息收发\|_blank](11567#2_2) 文档。
...
<details class="zg-primary">
    <summary>2. 创建和管理 SDK 实例</summary>
...
<details class="zg-primary">
    <summary>1. 为每个 SDK 创建一个包装层，以便尽可能地重用代码。</summary>

为 `ZIM SDK` 创建一个 `ZIMService` 类，它管理与 SDK 的交互并存储所需数据，完整代码请参考 [ZIMService.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/master/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService.swift)。
...
同样，为 `Express SDK` 创建一个 `ExpressService` 类，它管理与 SDK 的交互并存储所需数据，完整代码请参考 [ExpressService.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/Internal/SDK/Express/ExpressService.swift). 中的完整代码。
...
<details class="zg-primary">
    <summary>2. 完成服务封装后，您可以通过创建 ZEGOSDKManager 进一步简化代码，以管理这些服务。</summary>

完整示例代码，请参考 [ZegoSDKManager.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/Internal/ZegoSDKManager.swift)。
...
如需让观众上麦，请调用 [setRoomAttributes\|_blank](/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#set-room-attributes-room-attributes-room-id-config-callback) 并将发言人麦位号作为键，观众的 `userID` 作为房间附加属性的值。如果设置成功，观众会成功上麦并可以开始推流。

完整的示例代码，请参考 [RoomSeatService.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/Internal/Business/AudioRoom/RoomSeatService.swift)，其中主要示例代码如下。

<div class="mk-hint">

观众上麦时，若将 [ZIMRoomAttributesSetConfig\|_blank](/article/api?doc=zim_API~objective-c_ios~class~ZIMRoomAttributesSetConfig) 的`isForce`属性设置为`false`。当多个观众同时尝试占用同一个麦位时，服务器将接收到第一个请求并返回成功响应，将该键的所有者设置为第一个发出请求的用户。其他用户的后续修改请求将失败。
</div>
...
当发言人下麦时，可调用 [deleteRoomAttributesByKeys](/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#delete-room-attributes-by-keys-room-id-config-callback) 来删除发言人使用的麦位号，并停止推流。
...
<details class="zg-primary"><summary>在示例代码中，该方法被封装为 switchSeat 方法。</summary>
...
房主可以调用示例代码中的自定义方法 [setRoomExtraInfo\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/44169b0962cf1a08fbb6e798eb452de6c66554f6/best_practice/ZegoCloudSDKDemo/Internal/SDK/Express/ExpressService%2BRoom.swift#L59)，在自由模式和请求模式之间切换。
...
`lockSeat` 的具体定义，参考 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/44169b0962cf1a08fbb6e798eb452de6c66554f6/best_practice/ZegoCloudSDKDemo/ZegoLiveAudioRoomManager.swift#L79)。
</div>

完整的示例代码，请参考 [ZegoLiveAudioRoomManager.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/ZegoLiveAudioRoomManager.swift)，其中主要示例代码如下。
...
在示例代码中，从**观众的角度**来看，上麦请求按钮已放置在屏幕的右下角，当点击按钮时，将执行以下操作。


1. 调用示例代码中自定义的 [sendRoomRequest\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/44169b0962cf1a08fbb6e798eb452de6c66554f6/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService%2BRoomRequest.swift#L9) 发送房间请求信令。（[sendRoomRequest\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/44169b0962cf1a08fbb6e798eb452de6c66554f6/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService%2BRoomRequest.swift#L9) 简化了`ZIM SDK`的 [sendMessage\|_blank](/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#send-message-message-to-conversation-id-conversation-type-config-notification-callback) 接口。）
    - 如果方法调用成功：本地端（即观众）的申请状态将切换为申请上麦，并且“上麦请求”按钮将切换为“取消上麦”。
    - 如果方法调用失败：将提示错误消息。**在实际的应用开发中，您应该使用更友好的用户界面来提示上麦请求失败。**

2. 调用示例代码中自定义的 [cancelRoomRequest\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/44169b0962cf1a08fbb6e798eb452de6c66554f6/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService%2BRoomRequest.swift#L67) 发送信号。（[cancelRoomRequest\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/44169b0962cf1a08fbb6e798eb452de6c66554f6/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService%2BRoomRequest.swift#L67) 简化了`ZIM SDK`的 [`sendMessage`\|_blank](/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#send-message-message-to-conversation-id-conversation-type-config-notification-callback)接口。）
    - 如果方法调用成功：本地端（即观众）的申请状态将切换为申请上麦，并且“上麦请求”按钮将切换为“申请成为发言人”。
    - 如果方法调用失败：将提示错误消息。**在实际的应用开发中，您应该使用更友好的用户界面来提示上麦请求失败。**
...
3. 本地端（观众端）将等待房主的响应。
  ...
  - 如果房主接受上麦请求：观众将成为发言人，并可以开始推流。

### 接受和拒绝上麦请求


1. 在示例代码中，当房主收到一个上麦请求时，请求的观众将显示在请求列表中，房主可以在单击请求列表后，查看列表并选择接受或拒绝观众的上麦请求。
2. 房主回应后，将发送一个接受或拒绝的房间请求信令，发送房间请求信令的相关逻辑在此不再详述。
...
<details class="zg-primary"><summary>如何知道发言人是否打开了麦克风？</summary>

您可以监听 Express SDK 的 [onRemoteMicStateUpdate\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-remote-mic-state-update-stream-id) 回调通知，以确定远程推流设备的麦克风设备是否正常工作或关闭，并根据相应的状态初步了解设备问题的原因。
...
<details class="zg-primary"><summary>如何确认发言人是否在说话？</summary>

您可以监听 Express SDK 的 [onRemoteSoundLevelUpdate\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-remote-sound-level-update) 回调通知，以获取发言人声音的音量级别，详情请参考 [音量变化与音频频谱\|_blank](!ExpressAudioSDK-AudioAdvanced/SoundLevelSpectrum)。
