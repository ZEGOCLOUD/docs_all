...
本文基于 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/tree/master/best_practice) 来介绍的如何实现语聊房功能，因此文档中的部分方法为示例代码封装方法。
</div>
...
- 下载本文档附带的 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/tree/master/best_practice)。
- 在 [管理控制台\|_blank](https://console.zego.im/ProjectManage) 中注册并创建项目，同时开通 “即时通讯” 服务。
  ![](/Pics/InappChat/ActivateZIMinConsole2_zh.png)

## 预览效果

您可以通过本文档提供的 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/tree/master/best_practice) 实现以下效果：
...
房间模式可使用 Express SDK 的 [setRoomExtraInfo\|_blank](/article/api?doc=Express_Audio_SDK_API~java_android~class~ZegoExpressEngine#set-room-extra-info) 实现。“房间附加信息”与上述的“房间属性”类似，也存储在 ZEGO 服务器上，但是“房间附加信息”的使用更简单：
...
当房主调用 [setRoomExtraInfo\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#set-room-extra-info) 方法时，房间内的用户可以通过 [onRoomExtraInfoUpdate\|_blank](/article/api?doc=Express_Audio_SDK_API~java_android~class~IZegoEventHandler#on-room-extra-info-update) 接收到设置的`房间附加信息`。
...
ZIM SDK 提供了丰富的功能来发送和接收消息，详情可参考 [收发消息\|_blank](!IM-sendMessages)。此处，您需要使用可自定义的房间请求消息：[ZIMCommandMessage\|_blank](/article/api?doc=zim_API~java_android~struct~ZIMCommandMessage)

<div class="mk-hint">

此部分的完整示例代码，可参考 [ZIMService.java\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/master/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/sdk/zim/ZIMService.java)。
</div>


1. 通过调用以下方法在房间中发送房间请求信令（[ZIMCommandMessage\|_blank](/article/api?doc=zim_API~java_android~struct~ZIMCommandMessage)）：
...
2. 发送后成功后，房间中的其他用户将通过 [onReceiveRoomMessage\|_blank](/article/api?doc=zim_API~java_android~class~ZIMEventHandler#on-receive-room-message) 回调接收房间请求信令。您可以通过以下方式监听此回调：
...
此部分的完整示例代码，可参考 [ZIMService.java\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/master/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/sdk/zim/ZIMService.java) 和 [RoomRequest.java\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/master/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/sdk/zim/RoomRequest.java)。
</div>
...
本文档中的演示是一个纯客户端 API + ZEGOCLOUD 解决方案。如果您有自己的业务服务端，并希望进行更多的逻辑扩展，您可以使用我们的 [服务端 API\|_blank](!IM-Server_APIs/API_Overview) 来传递房间信令，并结合您服务端的房间业务逻辑，以提高您的应用的可靠性。
...
1. 配置 repositories 源
    ...
    - 当您的 Android Gradle Plugin 为 v7.1.0 或以上版本：进入项目根目录，打开 “settings.gradle” 文件，在 “dependencyResolutionManagement” 中加入如下代码。
        ...
        相关信息请参考 [Android Gradle Plugin Release Note v7.1.0\|_blank](https://developer.android.google.cn/build/releases/past-releases/agp-7-1-0-release-notes#settings-gradle)。
        </div>
...
2. 声明依赖项：
    ...
    进入 “app” 目录，打开 “build.gradle” 文件，在 “dependencies” 中添加 `implementation 'im.zego:zim:x.y.z'`，请从 [发布日志\|_blank](!IM-Download_SDK/Change_Log) 查询 SDK 最新版本，并将 `x.y.z` 修改为具体的版本号。
...
为 `ZIM SDK` 创建一个 `ZIMService` 类，它管理与 SDK 的交互并存储所需数据，完整代码请参考 [ZIMService.java\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/master/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/sdk/zim/ZIMService.java)。
...
同样，为 `Express SDK` 创建一个 `ExpressService` 类，它管理与 SDK 的交互并存储所需数据，完整代码请参考 [ExpressService.java\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/master/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/sdk/express/ExpressService.java) 中的完整代码。
...
完整代码，请参考 [ZEGOSDKManager.java\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/master/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/sdk/ZEGOSDKManager.java)。
...
如需让听众上麦，请调用 [setRoomAttributes\|_blank](/article/api?doc=zim_API~java_android~class~ZIM#set-room-attributes) 并将麦位序号作为 Key，听众的 `userID` 作为房间附加属性的值。如果设置成功，听众会成功上麦并可以开始推流。

完整的示例代码，请参考 [RoomSeatService.java\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/master/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/business/audioroom/RoomSeatService.java)，其中主要示例代码如下。

<div class="mk-hint">

听众上麦时，若将 [ZIMRoomAttributesSetConfig\|_blank](/article/api?doc=zim_API~java_android~struct~ZIMRoomAttributesSetConfig) 的`isForce`属性设置为`false`。当多个听众同时尝试占用同一个麦位时，服务器将接收到第一个请求并返回成功响应，将该 Key 的所有者设置为第一个发出请求的用户。其他用户的后续修改请求将失败。
</div>
...
当发言人下麦时，可调用 [deleteRoomAttributes](/article/api?doc=zim_API~java_android~class~ZIM#delete-room-attributes) 来删除发言人使用的麦位号，并停止推流。
...
当房主需要将发言人从麦位上移除时，请调用 [deleteRoomAttributes\|_blank](/article/api?doc=zim_API~java_android~class~ZIM#delete-room-attributes)，并将 [ZIMRoomAttributesDeleteConfig\|_blank](/article/api?doc=zim_API~java_android~struct~ZIMRoomAttributesDeleteConfig) 的`isForce`字段设置为 `true`，强制清除相应发言人麦位的房间属性，从而将发言人从麦位上移除。
...
房主可以调用示例代码中的自定义方法 [setRoomExtraInfo\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/master/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/sdk/express/ExpressService.java#L817)，在自由模式和请求模式之间切换。
...
`lockSeat` 的具体定义，参考 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/dd8062399252502f3f91b46ae2147a43160f73b5/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/ZEGOLiveAudioRoomManager.java#L115C17-L115C25)
</div>
...
3. 调用示例代码中自定义的 [sendRoomRequest\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/dd8062399252502f3f91b46ae2147a43160f73b5/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/sdk/zim/ZIMService.java#L719) 发送房间请求信令。（[sendRoomRequest\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/dd8062399252502f3f91b46ae2147a43160f73b5/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/sdk/zim/ZIMService.java#L719) 简化了`ZIM SDK`的[sendMessage\|_blank](/article/api?doc=zim_API~java_android~class~ZIM#send-message) 接口。）
    - 如果方法调用成功：本地端（即听众）的申请状态将切换为申请上麦，并且`上麦请求`按钮将切换为`取消上麦`。
    - 如果方法调用失败：将提示错误消息。**在实际的应用开发中，您应该使用更友好的用户界面来提示上麦请求失败。**

完整的示例代码，请参考 [TakeSeatButton.java\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/master/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/components/audioroom/TakeSeatButton.java)，其中主要示例代码如下。
...
完整的示例代码，请参考 [RoomRequestListDialog.java\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/master/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/components/RoomRequestListDialog.java) ，其中主要示例代码如下。
...
您可以监听 Express SDK 的 [onRemoteMicStateUpdate\|_blank](/article/api?doc=Express_Audio_SDK_API~java_android~class~IZegoEventHandler#on-remote-mic-state-update) 回调通知，以确定远程推流设备的麦克风设备是否正常工作或关闭，并根据相应的状态初步了解设备问题的原因。
...
您可以监听 Express SDK 的 [onRemoteSoundLevelUpdate\|_blank](/article/api?doc=Express_Audio_SDK_API~java_android~class~IZegoEventHandler#on-remote-sound-level-update) 回调通知，以获取发言人声音的音量级别，详情请参考 [音量变化与音频频谱\|_blank](!ExpressAudioSDK-AudioAdvanced/SoundLevelSpectrum)。
