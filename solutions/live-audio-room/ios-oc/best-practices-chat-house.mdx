...
本文基于 [Swift 示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice) 来介绍的如何实现语聊房功能，因此文档中的部分方法为示例代码封装方法。
</div>
...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成 SDK\|_blank](#3574)。
- 下载本文档附带的 [Swift 示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice)。
- 在 [管理控制台\|_blank](https://console.zego.im/ProjectManage) 中注册并创建项目，同时开通 “即时通讯” 服务。
  ![](/Pics/InappChat/ActivateZIMinConsole2_zh.png)

## 预览效果

您可以通过本文档提供的 [Swift 示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice) 实现以下效果：
...
如果您需要了解推流及拉流的相关接口使用，请参考 [快速开始 - 实现音频通话文档\|_blank](#7631)。
...
除了实现上述逻辑外，语聊房还需要管理发言人麦位。发言人麦位管理功能通常可以使用 ZIM SDK 的 [房间属性管理](#12879) 实现。
...
ZIM SDK 提供了丰富的功能来发送和接收消息，详情可参考 [收发消息\|_blank](#14062)。此处，您需要使用可自定义的房间请求消息：[ZIMCommandMessage\|_blank](/article/api?doc=zim_API~objectivec_ios~class~ZIMCommandMessage)
...
如需让听众上麦，请调用 [setRoomAttributes\|_blank](/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#set-room-attributes-room-attributes-room-id-config-callback) 并将麦位序号作为 Key，听众的 `userID` 作为房间附加属性的值。如果设置成功，听众会成功上麦并可以开始推流。
...
听众上麦时，若将 [ZIMRoomAttributesSetConfig\|_blank](/article/api?doc=zim_API~objective-c_ios~class~ZIMRoomAttributesSetConfig) 的`isForce`属性设置为`false`。当多个听众同时尝试占用同一个麦位时，服务器将接收到第一个请求并返回成功响应，将该 Key 的所有者设置为第一个发出请求的用户。其他用户的后续修改请求将失败。
</div>
...
您可以监听 Express SDK 的 [onRemoteSoundLevelUpdate\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-remote-sound-level-update) 回调通知，以获取发言人声音的音量级别，详情请参考 [音量变化与音频频谱\|_blank](#5075)。
