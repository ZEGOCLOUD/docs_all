...
ZEGO 针对视频通话场景提供了 [示例源码\|_blank](#4457)，以供开发者进一步体验相关场景，了解 ZEGO 方案。

## 3 前提条件

在实现基本的音视频通话前，请确保：

- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成\|_blank](#196)。
- @@@console_new_guide@@@
...
![/Pics/Video_call/Audio_video.png](/Pics/Video_call/Audio_video.png)
...
更多初始化 SDK 的细节请参考：[快速开始 - 实现流程\|_blank](7627#3_1) 的 “3.1 初始化”。   
...
更多使用 SDK 实现登录房间的细节请参考：[快速开始 - 实现流程\|_blank](7627#3_2) 的 “3.2 登录房间”。   
...
更多使用 SDK 实现预览和推流的细节请参考：[快速开始 - 实现流程\|_blank](7627#3_3) 的 “3.3 预览自己的画面，并推送到 ZEGO 音视频云”。
...
更多使用 SDK 实现拉流的细节请参考：[快速开始 - 实现流程\|_blank](7627#3_4) 的 “3.4 拉取其他用户的音视频”。
...
更多使用 SDK 实现停止推流的细节请参考：[快速开始 - 实现流程\|_blank](7627#4_2) 的 “4.2 停止音视频通话”。
...
```
更多使用 SDK 实现停止拉流的细节请参考：[快速开始 - 实现流程\|_blank](7627#4_2) 的 “4.2 停止音视频通话”。
...
ZEGO 支持在视频通话中加入实时消息互动，实时展示房间内的消息，例如发消息、进退房提示、互动通知等。有关实现实时消息互动的流程，详情请参考 [实时消息互动\|_blank](#15743)。
...
如果您的项目需要实现美颜美型效果，需要先向项目集成 ZegoEffects SDK，详情请参考 [集成 SDK\|_blank](#9573)。

#### 5.2.2 导入资源和模型

在使用 ZegoEffects SDK 提供的 AI 功能前，您需要先导入 AI 资源或模型，详情请参考 [导入资源和模型\|_blank](#10184)。

#### 5.2.3 在线鉴权

在使用 ZegoEffects SDK 的 AI 功能前，您需要完成在线鉴权，详情请参考 [在线鉴权\|_blank](#11987)。

#### 5.2.4 实现图像处理

完成上述准备后，您可以开始进行美颜美型，详情请参考 [实现图像处理\|_blank](#9577)。
