...
调用 [login_room](/large-class/edu-cloud-service-paa-s/log-in-to-go-classroom) 接口登录教室，会自动初始化教室，如果输入了相同的 `room_id` 则会进入同一教室。客户端需保证 `uid` 唯一性，相同的 `uid` 后端会认为是同一个用户。
...
调用 [set_user_info](/large-class/edu-cloud-service-paa-s/set-user-attribute) 接口可以修改自己的设备、共享状态，教师也可以通过此接口修改学生的设备、共享状态。
...
调用 [leave_room](/large-class/edu-cloud-service-paa-s/leave-class) 接口可以离开教室，用户状态信息会被删除，当所有人都离开教室一段时间后，教室会自动销毁。
...
调用 [end_teaching](/large-class/edu-cloud-service-paa-s/end-of-teaching) 接口可以结束教学，将教室内所有用户踢出教室，然后立即销毁教室。
