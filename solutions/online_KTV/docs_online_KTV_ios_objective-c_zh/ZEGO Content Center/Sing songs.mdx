...
1. 调用 [sendExtendedRequest](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#send-extended-request-callback-params) 接口，command 传入 `"/tag/list"`，params 传入 `"{vendor_id：xx}"`，即版权方 ID（具体信息请联系 ZEGO 技术支持），在回调中获取到标签列表。
    ...
    [sendExtendedRequest](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#send-extended-request-callback-params) 接口完整参数与返回结果的说明请参考 [发送扩展请求接口说明](./../Client%20API/Send%20Extended%20Request.mdx)。
    </Note>
...
点播歌曲完成后，需要推送歌声音频流，以供远端用户听到演唱用户的歌声。请根据实际需求，参考 [独唱方案](./../Implementation/KTV%20Solutions/Solo.mdx)、[实时合唱方案](./../Implementation/KTV%20Solutions/Chorus/Concurrent.mdx)、[串行合唱方案](./../Implementation/KTV%20Solutions/Chorus/Serial.mdx) 或 [抢唱方案](./../Implementation/KTV%20Solutions/Grab%20the%20Mic.mdx) 实现推流。 
