...
获取音高线数据需要调用 SDK 的 [getStandardPitch\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#get-standard-pitch-callback) 接口，解析为包含 ZegoPitchModel 的数组，根据不同的音乐资源类型，需要调用 ZegoPitchModel 的不同接口进行解析。
...
如果需要在控件上展示音高分数，则需要在歌曲开始前调用 ZegoExpresssEngine SDK 版权音乐模块的 [resetScore\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#reset-score) 和 [startScore\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#start-score-pitch-value-interval) 接口开启打分。

设置 [歌词组件\|_blank](!Copyright_Music_Lyrics_Control) 的换行监听，在换行回调中判断当前播放进度时间是否大于音高线开始的时间，如果大于则添加分数。上一行分数可以通过 SDK 的 [getPreviousScore\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#get-previous-score) 接口获取。
