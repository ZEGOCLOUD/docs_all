...
通过 [requestResource\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#request-resource-callback-type) 接口请求歌曲资源，再从返回的歌曲信息中获取 krcToken，向 [getKrcLyricByToken\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#get-krc-lyric-by-token-callback) 传入 krcToken，获取逐字歌词。
...
演唱者调用 [startPublishingStream\|_blank](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#start-publishing-stream) 开始推流，随后调用 [enableAux\|_blank](/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-media-player&jumpType=route#enable-aux) 接口开启混流，将播放歌曲的音频流推送到远端，以供观众收听。

<Note title="Note">

实现基本的音频通话功能，将音频流推送到远端，可参考 [实时语音 - 快速开始 - 实现流程\|_blank](7631#2_3)。
</Note>
...
演唱者通过在媒体播放器的进度回调中，调用 [sendSEI\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#send-sei) 发送 SEI，向观众端同步歌曲的当前播放进度，详细内容请参考 [实时音视频 - 进阶功能 - 媒体补充增强信息\|_blank](#6064)。
...
观众只要通过拉流 [startPlayingStream\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#start-playing-stream-canvas) 就可以听到歌曲的声音，不需要再去做点歌、下载、播放等的一系列操作。
...
观众端将从演唱者端获取的 krcToken 传入 [getKrcLyricByToken\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#get-krc-lyric-by-token-callback)，获取逐字歌词。

示例代码请参考本文的 [4.1.2 获取歌词](!Solo#4_1)。

观众端也可以通过 [getSharedResource\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#get-shared-resource-callback-type) 接口，传入演唱者端通过 [sendExtendedRequest\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#send-extended-request-callback-params) 接口请求资源时获取到的 songID（请参考 [4.1.1 点歌](!Solo#4_1)），获取对应的歌词信息，详情请参考 [进阶功能 - 分享歌曲](!Advanced_features_KTV#3)。

2. 同步演唱者进度信息

观众通过 [onPlayerRecvSEI\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-player-recv-sei-stream-id) 回调收到 SEI 信息，获取进度信息，用于歌词同步。
