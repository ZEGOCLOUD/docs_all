...
用户通过调用 [sendExtendedRequest\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoCopyrightedMusic#send-extended-request-callback-params) 接口，传入对应的 command、params，可以获取标签列表、搜索歌曲、榜单歌曲、歌词分片等信息，对应结果将通过 [ZegoCopyrightedMusicSendExtendedRequestCallback\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressDefines#zego-copyrighted-music-send-extended-request-callback) 回调获取。
