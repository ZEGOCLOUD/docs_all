...
1. 安装 CocoaPods，安装时的常见问题请参考 [CocoaPods 常见问题 - 安装 CocoaPods\|_blank](!Old_Live_Room-Common_Problem/CocoaPods#1)。

2. 打开终端，进入项目根目录，执行 `pod init` 命令创建 Podfile 文件。

3. 打开 Podfile 文件，在 “target” 下添加 `pod 'ZegoExpressPrivate', '3.20.2.43680'`，需要将 “MyProject” 替换为开发者的 Target 名称。

    ```ruby
    target 'MyProject' do
        use_frameworks!
        # 请填写具体的 SDK 版本号
        pod 'ZegoExpressPrivate', '3.20.2.43680'
    end
    ```
...
1. 请参考 [下载\|_blank](!online_ktv_download)，下载最新版本的 SDK 并解压。
...
4. 选择 “TARGETS > General > Frameworks,Libraries,and Embedded Content” 菜单，添加 “ZegoExpressEngine.xcframework” 和 “ZegoExpressVideoCopyrightedMusicPlugin.xcframework”，将 “Embed” 设置为 “Embed & Sign”。
    ...
    <Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/ktv/plugin/ios_video.png" /></Frame>
...
- [从 2.8.0 之前的版本升级以后，Xcode 编译报错如何处理？\|_blank](/faq/Express_xcframework)
- [打包 iOS 时 Xcode 报错提示 Bitcode 版本不兼容？\|_blank](https://doc-zh.zego.im/faq/ios_bitcode?product=ExpressVideo&platform=ios)
