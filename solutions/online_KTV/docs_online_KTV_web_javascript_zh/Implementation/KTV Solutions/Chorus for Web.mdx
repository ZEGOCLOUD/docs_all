...
登录房间成功会触发回调 [roomStateUpdate](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoRTMEvent#room-state-update)，推流必须在登录房间成功后进行。

#### 1.2 推流配置

当 `state =='CONNECTED' && errorCode == 0` 时，登录房间成功：

1. 点歌的合唱者调用 [createZegoStream](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#create-zego-stream) 接口，创建推流对象。

2. 调用 [startPublishingStream](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#start-publishing-stream) 接口，开始推流。**点歌的合唱者需要开启 SEI，用于同步歌词进度，其他合唱者不需要开启。**

3. 推流成功后，需要 [setStreamAlignmentProperty](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#set-stream-alignment-property) 执行混流 NTP 时间对齐。
...
每当房间内有用户开始推流或者停止推流时，房间内的其他用户都会收到 [roomStreamUpdate](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoRTCEvent#room-stream-update) 流变更通知，麦上用户需要在 [roomStreamUpdate](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoRTCEvent#room-stream-update) 中处理拉流事件。
...
调用 [startPublishingStream](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#start-publishing-stream) 接口时，将 [ZegoWebPublishOption](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPublishOption) 中 forceSynchronousNetworkTime 设置为 1，开始推流。
...
在收到 [publisherStateUpdate](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoRTCEvent#publisher-state-update) 流状态更新回调后，调用 [setStreamAlignmentProperty](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#set-stream-alignment-property) 接口，开启混流精准对齐。
...
实现此功能也可以使用其他方式，只需通知到位即可。收到歌曲信息后，参与合唱的用户开始下载歌曲。如何搜索歌词、展示歌单、下载歌曲和下载歌词，请参考 [ZEGO 内容中心 - 点歌](./../../../ZEGO%20Content%20Center/Sing%20songs.mdx)。
...
麦上合唱用户的进度，以点歌用户的伴奏进度为准，通过 [playerRecvSEI](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoRTCEvent#player-recv-sei) 监听点歌用户 SEI 中的同步伴奏信息，调节本端的伴奏进度。由于插拔耳机会导致本地伴奏进度滞后，可以以此来保证进度一致。
...
通过媒体播放器的进度回调进行同步即可，歌词控件请参考 [下载](./../../../Downloads.mdx) 页面获取。

- 点歌合唱者通过定时获取 \<audio> 标签的进度，设置自己的歌曲进度；其他合唱者则通过 [playerRecvSEI](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoRTCEvent#player-recv-sei) 监听点歌合唱者的 SEI，获取的歌词进度。

- 观众端通过 [playerRecvSEI](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoRTCEvent#player-recv-sei) 监听解析混流中的 SEI，获取播放点歌用户的播放进度，以此同步歌词进度。
