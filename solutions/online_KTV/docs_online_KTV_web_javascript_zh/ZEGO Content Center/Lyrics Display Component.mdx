...
获取歌词数据需要调用 SDK 的 [getKrcLyricByToken](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoCopyrightedMusic#get-krc-lyric-by-token) 接口获取到 lyrics 歌词信息，再调用 ZegoLyricView 的 `setupKrcLyric` 接口设置歌词数据源。
...
- 原唱资源，指的是调用 Express SDK [requestResource](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoCopyrightedMusic#request-resource) 接口，type 参数传入 0 为原曲音乐资源类型请求的资源。

- 伴奏资源，指的是调用 Express SDK [requestResource](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoCopyrightedMusic#request-resource) 接口，type 参数传入 1 为伴奏资源类型请求的资源。
