...
} from '/solutions/online_KTV/snippets/api-links.mdx'
...
- 已从 [下载](./../Downloads.mdx) 获取 Express SDK（含版权音乐功能）并在项目中集成 SDK，详情请参考 [SDK 集成指引](https://doc-zh.zego.im/article/197)。
...
7. 调用 {getPlatformData2(props,enableAuxMap)} 将当前播放的歌曲音频流推送到远端，参考 [实时语音 - 快速开始 - 实现流程](https://doc-zh.zego.im/article/7637)。
:::
:::if{props.platform="linux"}
7. 调用 {getPlatformData2(props,enableAuxMap)} 将当前播放的歌曲音频流推送到远端，参考 [实时语音 - 快速开始 - 实现流程](https://doc-zh.zego.im/article/8228)。
:::
...
<Note title="Note">
:::if{props.platform=undefined}
初始化 ZEGO Express SDK 时，内部处理的操作较多，建议用户在 App 启动时候进行。详情参考：[实时语音 - 快速开始 - 实现流程](https://doc-zh.zego.im/article/7637)。
:::
:::if{props.platform="linux"}
初始化 ZEGO Express SDK 时，内部处理的操作较多，建议用户在 App 启动时候进行。详情参考：[实时语音 - 快速开始 - 实现流程](https://doc-zh.zego.im/article/8228)。
:::
</Note>
...
1. 调用 {getPlatformData2(props,sendExtendedRequestnMap)} 接口，command 传入 `"/tag/list"`，params 传入 `"{vendor_id：xx}"`，即版权方 ID（具体信息请联系 ZEGO 技术支持），在回调中获取到标签列表。
    ...
    {getPlatformData2(props,sendExtendedRequestnMap)} 接口完整参数与返回结果的说明请参考 [发送扩展请求接口说明|_blank](./../Client%20API/Send%20Extended%20Request.mdx)。
    </Note>
...
如果包含人声的歌曲资源有版权，就可以调用 {getPlatformData2(props,requestResourceMap)} 接口获取到歌曲的资源信息，包括歌曲名、歌曲时长、resourceID 等，接口具体回调的信息参考 [获取歌曲与歌词资源接口说明](./../Client%20API/APIs%20to%20Obtain%20Songs%20and%20Lyrics.mdx)。
...
:::if{props.platform=undefined}
1. 实现基本的音频通话功能，将音频流推送到远端，可参考 [实时语音 - 快速开始 - 实现流程 ](https://doc-zh.zego.im/article/7637#2_3)。
:::
:::if{props.platform="linux"}
1. 实现基本的音频通话功能，将音频流推送到远端，可参考 [实时语音 - 快速开始 - 实现流程 ](https://doc-zh.zego.im/article/8228#2_3)。
:::
