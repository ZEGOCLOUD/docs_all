...
另外为了K歌效果好，可以考虑添加混响。混流为枚举值。详情见 [setReverbPreset\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#set-reverb-preset)。
...
在推流之前，需要同步 NTP 时间，等同步完后，再推流，开发者可以在推流配置中设置同步完后再推流即可。推流必须在登录房间成功后进行。登录房间成功会触发回调 [onRoomStateUpdate\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~IZegoEventHandler#on-room-state-update)。
...
每当房间内有人开始推流或者停止推流，其他房间内的人都会收到流变更通知，麦上用户在 [onRoomStreamUpdate\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~IZegoEventHandler#on-room-stream-update) 中处理拉流事件。
...
点歌用户需要发起混流任务。人声和伴奏流都需要设置对齐。[ZegoMixerTask\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoMixerTask) 类中 [setStreamAlignmentMode\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoMixerTask#set-stream-alignment-mode) 设置为 ZegoStreamAlignmentModeTry。
...
完成 [4.2 混流任务设置对齐模式](!online_ktv_solution_chorus_concurrent#2_4) 创建混流任务后，接下来设置混流任务的输入和输出。混流输出的流 ID 由业务侧自定义，混流 ID 的如何下发也由业务侧决定。
