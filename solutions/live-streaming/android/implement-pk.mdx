...
- 启动混流：[开始混流](/zh/real-time-video-server/api-reference/stream-mixing/start-mix)，[混流开始回调](/zh/real-time-video-server/callback/stream-mixing/started)
- 停止混流：[停止混流](/zh/real-time-video-server/api-reference/stream-mixing/stop-mix)，[混流结束回调](/zh/real-time-video-server/callback/stream-mixing/stopped)

<Note title="说明">


- 在服务端发起的混流方法中，服务端需要使用 [用户退出房间回调](/zh/real-time-video-server/callback/room/logged-out) 监控主播的客户端状态。如果服务端检测到主播异常退出，需要及时停止相应的混流任务，并向主播发送通知以结束 PK 对战。
