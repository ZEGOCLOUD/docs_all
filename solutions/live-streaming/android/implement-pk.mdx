...
- 已集成 SDK，详情请参考 [CDN 直播快速开始](/solution-live-streaming-android/implement/cdn-live-streaming/quick-start) 或 [超低延迟直播快速开始](/solution-live-streaming-android/implement/l3-live-streaming/quick-start) 文档。
...
此外，如果您的服务器没有信令通道向客户端发送通知，我们建议使用 ZIM 服务端 API 的`Command message (signaling message)` 实现，详情请参考 [ZIM 服务端 API-发送房间消息](/zim-server/messaging/send-room-messages)。
</Note>

此处使用类似呼叫邀请的方法，实现 PK 对战邀请：

基于 ZIM SDK 提供的 [呼叫邀请](/zim-android/guides/call-invitation-signaling) 功能。该功能提供了呼叫邀请的能力，允许您发送、取消、接受和拒绝邀请，您可以实现 PK 对战邀请、房间邀请。您可以使用 [ZIMCallInviteConfig](https://doc-zh.zego.im/article/api?doc=zim_API~java_android~struct~ZIMCallInviteConfig) 提供的`extendedData`字段，该字段允许您自定义此邀请的类型，从而实现不同的功能。
...
有关这些接口的具体用法，请参考 [呼叫邀请](/zim-android/guides/call-invitation-signaling)。
...
2. 什么是混流服务？
    ...
    更多详细信息，请参考 [混流](/real-time-video-android-java/live-streaming/stream-mixing)。
</Accordion>

PK 解决方案需要使用 [混流](/real-time-video-android-java/live-streaming/stream-mixing)：混流指将多个流合并为一个流，这样观众只需播放该流，即可观看多个主播的画面。使用混流的必要性和重要优势如下：
...
- **混流布局**：通常，在 PK 对战中，每个观众在界面左侧看到自己房间的画面。因此，在 PK 开始后，需要发起混流任务，即每个房间中的每个主播都需要发起一个混流任务。在混合流时，每个主播需要将自己的视频放在流混流布局的左侧。`layout` 参数可以参考下方代码，或者您可以参考 [混流](/real-time-video-android-java/live-streaming/stream-mixing) 文档，了解更多关于混流布局的详细信息。
...
如果您需要更复杂的自定义布局，请参考 [混流布局文档](/real-time-video-android-java/live-streaming/stream-mixing)，了解混流布局的方式，并在示例代码中使用 [ZEGOLiveStreamingManager.setMixLayoutProvider](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/blob/38cc405f591e42412cfb0542bfe4682b9a311b76/best_practice/app/src/main/java/com/zegocloud/demo/bestpractice/internal/ZEGOLiveStreamingManager.java#L283) 来修改布局：
...
- 如果您的服务器没有信令通道用于向客户端发送通知，我们建议使用 ZIM 的 [发送房间消息](/zim-server/messaging/send-room-messages) 实现。
</Note>

### 3 通知观众 PK 对战开始

当 PK 对战开始时，需要通知观众 PK 对战已经开始。观众在收到通知后，可以处理观看 PK 的逻辑。我们建议使用 ZIM SDK 的 [房间属性](/zim-android/guides/room/room-properties) 功能通知观众。
...
1. 停止播放另一个主播的单流。
2. 结束混流任务。
3. 从 [房间属性](/zim-android/guides/room/room-properties) 中删除与 PK 相关的属性。
...
- 有关 SEI 功能的详细说明，请参考 [媒体补充增强信息（SEI）](/real-time-video-android-java/communication/sei)。
...
通常，可以使用 [音量变化与音频频谱](/real-time-video-android-java/audio/sound-level-spectrum) 来渲染音量级别。
