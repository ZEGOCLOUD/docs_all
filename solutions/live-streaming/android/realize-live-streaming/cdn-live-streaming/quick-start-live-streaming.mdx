...
当您开发应用程序时，可下载 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/tree/master/live_streaming/quick_start) 进行参考。
...
调用 [createEngine|_blank](/article/api?doc=express-video-sdk_API~java_android~class~ZegoExpressEngine#create-engine) 方法初始化 Express Video SDK，并配置以下内容：
...
- [onRoomStreamUpdate\|_blank](/article/api?doc=express_video_sdk_API~java_android~class~IZegoEventHandler#on-room-stream-update): 房间内流的状态更新回调。当新的流推到房间或房间内现有流停止时，SDK 通过此回调发出事件通知。您可以在此回调中，调用 `startPlayingStream()` 和 `stopPlayingStream()` 方法。
...
- 调用 [loginRoom\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#login-room) 登录房间。
- 调用 [logoutRoom\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#logout-room) 退出房间。
...
- 调用 [stopPreview\|_blank](/article/api?doc=express_video_sdk_API~objective-c_ios~class~ZegoExpressEngine#stop-preview) 方法，停止渲染及预览。
...
- 调用 [startPlayingStream\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#start-playing-stream) 方法，开始拉流。
...
更多详细的演示源代码，请查看 [此处\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/tree/master/live_streaming/quick_start)。
