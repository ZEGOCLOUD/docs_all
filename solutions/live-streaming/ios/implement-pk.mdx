...
- 本文是基于 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_android/tree/master/best_practice) 来介绍的如何实现PK功能，因此文档中的部分方法为示例代码封装方法。
- 示例代码把多个SDK进行了封装以方便管理，请参考：[管理多个 SDK\|_blank](20744#4_2)
</div>
...
- 已集成 SDK，详情请参考 [CDN 直播快速开始\|_blank](!QuickStart_LiveStreaming) 或 [超低延迟直播快速开始\|_blank](!QuickStart_L3_LiveStreaming) 文档。
- 下载与本文档配套的 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice)。
- 请联系 ZEGO 技术支持开通混流服务。
- 在 [管理控制台\|_blank](https://console.zego.im/ProjectManage) 中注册并创建项目，同时开通 “即时通讯” 服务。
  ![](/Pics/InappChat/ActivateZIMinConsole2_zh.png)


## 预览效果

您可以使用本文档提供的 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice) 实现以下效果：


|主页|直播页面|接收PK对战请求|PK对战|
|--- | --- | --- |--- |
|<img src="/Pics/A_LiveStreamingSDK/livestream_page.png" width=80%>|<img src="/Pics/A_LiveStreamingSDK/homepage.png" width=80%>|<img src="/Pics/A_LiveStreamingSDK/receive_pk_battle.png" width=80%>|<img src="/Pics/A_LiveStreamingSDK/start_pk_battle.png" width=80%>|
...
1. [如何发送 PK 对战邀请](!Implement_PK#4)：解释如何利用 [呼叫邀请\|_blank](!IM-Call_Invitation) 发送 PK 邀请的过程。
2. [PK 拉流和推流的介绍](!Implement_PK#5)：基于流的概念，介绍 PK 对战的推流和拉流解决方案的框架。
3. [实现主播端逻辑](!Implement_PK#6)：提供主播端 PK 的解决方案的详细信息，包括如何与其他主播互动，为观众开启混流，以及在 PK 对战开始时通知观众。
4. [实现观众端逻辑](!Implement_PK#7)：提供观众端 PK 的解决方案的细节，包括如何观看主播之间的互动及如何处理单流场景。
5. [如何结束 PK 对战](!Implement_PK#8)：描述结束 PK 会话，并恢复正常的单主播直播的步骤。
6. [如何检测 PK 对战中的异常情况](!Implement_PK#9)：描述如何通过 SEI 检测 PK 主机的异常情况。
7. [其他功能介绍](!Implement_PK#10)：包括使用 SEI 检测主机设备和流的状态，处理客户端断开连接或崩溃等异常情况。
8. [常见问题解答](!Implement_PK#11)：解答与 PK 对战相关的常见问题，如渲染主播声音或暂时静音其他主播。
...
此外，如果您的服务器没有信令通道向客户端发送通知，我们建议使用 ZIM 服务端 API 的`Command message (signaling message)` 实现，详情请参考 [ZIM 服务端 API-发送房间消息\|_blank](!IM-Server_APIs/send_Room_Message)。
</div>

此处使用类似呼叫邀请的方法，实现 PK 对战邀请：

基于 ZIM SDK 提供的 [呼叫邀请\|_blank](!IM-Call_Invitation) 功能。该功能提供了呼叫邀请的能力，允许您发送、取消、接受和拒绝邀请，您可以实现 PK 对战邀请、房间邀请。您可以使用 [ZIMCallInviteConfig\|_blank](/article/api?doc=zim_API~objectivec_ios~class~ZIMCallInviteConfig) 提供的`extendedData`字段，该字段允许您自定义此邀请的类型，从而实现不同的功能。
...
有关这些接口的具体用法，请参考 [呼叫邀请](!IM-Call_Invitation)。
...
<details class="zg-primary"><summary>什么是房间、流及混流？</summary>
...
2. 什么是混流服务？
    ...
    更多详细信息，请参考 [混流\|_blank](!ExpressVideoSDK-Other_Functions/Mixer)。

</details>

PK 解决方案需要使用 [混流\|_blank](!ExpressVideoSDK-Other_Functions/Mixer)：混流指将多个流合并为一个流，这样观众只需播放该流，即可观看多个主播的画面。使用混流的必要性和重要优势如下：
...
通常，流 ID 与房间 ID 和用户 ID 相关。例如，在本文档的附带示例代码中，流 ID 规则为 `"${roomID}_${userID}_main_host"`。因此，您可以使用此规则，连接每个主播的流 ID，然后调用 [startPlayingStream\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/44169b0962cf1a08fbb6e798eb452de6c66554f6/best_practice/ZegoCloudSDKDemo/Internal/SDK/Express/ExpressService%2BStream.swift#L44) 来拉对方的流。双方的信息可以从邀请接口的回调和双方之间传递的 `extendedData` 中获取。
...
- **混流布局**：通常，在 PK 对战中，每个观众在界面左侧看到自己房间的画面。因此，在 PK 开始后，需要发起混流任务，即每个房间中的每个主播都需要发起一个混流任务。在混合流时，每个主播需要将自己的视频放在流混流布局的左侧。`layout` 参数可以参考下方代码，或者您可以参考 [混流\|_blank](!ExpressVideoSDK-Other_Functions/Mixer) 文档，了解更多关于混流布局的详细信息。
...
如果您想设置混流的布局，可以使用`ZegoMixerTask`的 [setInputList\|_blank](/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-mixer-task#set-input-list) 方法自定义布局。此处我们展示一些简单的设置规则。
...
如果您需要更复杂的自定义布局，请参考 [混流布局文档\|_blank](!ExpressVideoSDK-Other_Functions/Mixer)，了解混流布局的方式，并在示例代码中使用`ZEGOLiveStreamingManager.shared.eventDelegates.add(self)`来修改布局：
```objectivec
...
- 启动混流：[开始混流\|_blank](#19595)，[混流开始回调\|_blank](#19682)
- 停止混流：[停止混流\|_blank](#19597)，[混流结束回调\|_blank](#19684)

<div class="mk-hint">

- 在服务端发起的混流方法中，服务端需要使用 [用户退出房间回调\|_blank](#19672) 监控主播的客户端状态。如果服务端检测到主播异常退出，需要及时停止相应的混流任务，并向主播发送通知以结束 PK 对战。
- 如果您的服务器没有信令通道用于向客户端发送通知，我们建议使用 ZIM 的 [发送房间消息\|_blank](!IM-Server_APIs/send_Room_Message) 来实现。
</div>

### 3. 通知观众 PK 对战开始

当 PK 对战开始时，需要通知观众 PK 对战已经开始。观众在收到通知后，可以处理观看 PK 的逻辑。我们建议使用 ZIM SDK 的 [房间属性\|_blank](!IM-Advanced_Features/Room_Advanced_Features) 功能通知观众。

<details class="zg-primary"><summary>如果您之前没有使用过该功能，可以点击这里查看详细介绍</summary>
...
当 PK 开始时，主播需要调用 [setRoomAttributes\|_blank](/article/api?doc%253Dzim_API~objectivec_ios~protocol~ZIM#set-room-attributes-room-attributes-room-id-config-callback) 来设置房间的附加属性，表示房间已进入 PK 状态。建议在房间的附加属性中包含以下信息：
...
设置完成后，观众将收到 [onRoomAttributesUpdated\|_blank](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-room-attributes-updated-room-id) 回调。

## 开始 PK 对战 - 观众逻辑

在收到 [onRoomAttributesUpdated\|_blank](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-room-attributes-updated-room-id) 回调后，如果观众发现有与 PK 对战相关的新添加字段，可以开始处理观看 PK 的逻辑。
...
需要处理以下两个细节：
- 需要监听 [onPlayerStateUpdate\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-player-state-update-error-code-extended-data-stream-id) 回调来判断拉流是否成功。如果拉混流失败，需要向用户提示加载消息，并实现相应的重试逻辑。
- 由于生成混流需要一定的时间，观众可能无法立即拉混流。为了优化用户体验，避免黑屏，拉混流后，需要监听 [onPlayerRecvVideoFirstFrame\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-player-recv-video-first-frame) 回调。在收到这两个回调中的任意一个后，可以渲染混流。这样可以避免黑屏。

### 2. 在 PK 过程中，观众需要静音主播的单流

一旦观众成功拉混流，就可以开始观看 PK 对战。由于混流已经包含了两个主播的音视频，观众不需要再渲染主播的单流。

因此，观众可以使用 [mutePlayStreamAudio\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#mute-play-stream-audio-stream-id) 和 [mutePlayStreamVideo\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#mute-play-stream-video-stream-id) 来暂停播放主播的单流的音频和视频。这样可以进一步降低成本，避免观众端不必要的流量消耗和性能损失。

<div class="mk-warning">

不建议此时使用 [stopPlayingStream\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#stop-playing-stream) 来停止拉主播的单流。如果这样做，观众在 PK 结束后需要重新拉流，而流切换的速度比使用 `mute` 要慢得多，这会导致用户体验较差。 
</div>
...
当主播点击结束按钮时，还需要通知另一个主播 PK 已经结束，可以通过 `ZEGOLivesSreamingManager` 的 [endPKBattle\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/44169b0962cf1a08fbb6e798eb452de6c66554f6/best_practice/ZegoCloudSDKDemo/ZegoLiveStreamingManager.swift#L134) 方法来实现。当另一个主播收到此通知时，也需要处理结束 PK 的逻辑。

[quitPKBattle\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/44169b0962cf1a08fbb6e798eb452de6c66554f6/best_practice/ZegoCloudSDKDemo/ZegoLiveStreamingManager.swift#L141) 和 [endPKBattle\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/44169b0962cf1a08fbb6e798eb452de6c66554f6/best_practice/ZegoCloudSDKDemo/ZegoLiveStreamingManager.swift#L134) 的区别在于，前者只允许玩家退出 PK，而后者会让所有人停止 PK。结束 PK 对战需要执行以下操作：

**主播：**

1. 停止播放另一个主播的单流。
2. 结束混流任务。
3. 从 [房间属性\|_blank](!IM-Advanced_Features/Room_Advanced_Features) 中删除与 PK 相关的属性。
4. 调整 UI，返回到单个主播的直播流状态。

**观众：**

当观众收到 [onRoomAttributesUpdated\|_blank](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-room-attributes-updated-room-id) 回调，表示与 PK 相关的属性已被删除时，可以开始处理以下逻辑：

1. 调用 [stopPlayingStream\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#stop-playing-stream) 停止播放混流，返回到单个主播的直播流状态。
2. 由于观众在 PK 开始时静音了主播的单流，现在需要再次调用 [mutePlayStreamAudio\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#mute-play-stream-audio-stream-id) 和 [mutePlayStreamVideo\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#mute-play-stream-video-stream-id) 来取消静音。
...
当检测到主播异常时，将在主播的视频画面上出现“主播重新连接”的提示，详情请参考 [PKBattleView.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/master/best_practice/ZegoCloudSDKDemo/Components/PK/PKBattleView.swift)。
...
此外，您还需要定义一个最大超时时间。例如，在示例代码中，如果一个 PK 主播的 SEI 消息超过 60 秒没有收到，所有参与 PK 的用户将把该异常主播从 PK 中移除，详情请参考 [LiveStreamingViewController.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/master/best_practice/ZegoCloudSDKDemo/ViewControllers/LiveStreamingViewController.swift)。
...
在这种实时流媒体互动场景中，主播可以使用 [onRemoteCameraStateUpdate\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-remote-camera-state-update-stream-id) 和 [onRemoteMicStateUpdate\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-remote-mic-state-update-stream-id) 来获取对方的摄像头和麦克风状态。

需要注意的是，需要在调用 [createEngineWithProfile\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#create-engine-with-profile-event-handler) 后调用 [setEngineConfig\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#set-engine-config) 来启用此功能。以下是示例代码：
...
在这种情况下，观众无法接收到“场景 1”中提到的回调。因此，当主播推流时，他们需要使用 [sendSEI\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#send-sei) 更新自己的设备状态，而拉流端将通过回调 [onPlayerRecvSEI\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-player-recv-sei-stream-id) 接收。

<details class="zg-primary"><summary>什么是 SEI？</summary>

- 有关 SEI 功能的详细说明，请参考 [媒体补充增强信息（SEI）\|_blank](!ExpressVideoSDK-Other_Functions/SEI)。
...
您可以参考示例代码中的 [相关代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/Internal/Business/PK/PKService.swift) 来具体实现此部分。


@@@Resolution_And_Pricing_Attention@@@
...
<details class="zg-primary"><summary>如何在 PK 对战期间处理共同主持功能？</summary>
...
<details class="zg-primary"><summary>如何渲染主播的音量级别？</summary>

通常，可以使用 [音量变化与音频频谱\|_blank](!ExpressVideoSDK-AudioAdvanced/SoundLevelSpectrum) 来渲染音量级别。

然而，当观众端拉混流时，方法略有不同：

1. 在开始混流任务时，将 `enableSoundLevel` 设置为 `true`，以便混流包含输入流的音量级别信息。
2. 在开始混流任务时，为每个输入流分配一个 `soundLevelID`，以便拉流端可以确定其属于哪个音量级别数据。
3. 拉流端可以监听 [onMixerSoundLevelUpdate\|_blank](/article/api?doc=express-video-sdk_API~objectivec_ios~protocol~ZegoEventHandler#on-mixer-sound-level-update) 来接收音量级别数据，可以使用 `soundLevelID` 来确定其属于哪个音量级别。

</details>


<details class="zg-primary"><summary>如何在 PK 期间临时静音另一个主播？</summary>

在 PK 期间，可能需要临时静音另一个主播的音频。静音后，房间中的主播和观众都无法听到另一个主播的音频。

1. 主播可以使用 [mutePlayStreamAudio\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#mute-play-stream-audio-stream-id) 静音另一个主播的音频流。
2. 为确保房间中的观众也无法听到另一个主播的音频，可以通过再次调用 [startMixerTask\|_blank](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#start-mixer-task-callback) 来更新混流任务，将另一个主播的 [inputstream\|_blank](/article/api?doc=express-video-sdk_API~objectivec_ios~class~ZegoMixerInput) 的 `contentType` 设置为 `ZegoMixerInputContentTypeVideoOnly`（无需调用`StopMixerTask`，直接调用`startMixerTask`）。

</details>



<details class="zg-primary"><summary>业务服务端如何了解房间状态？</summary>
...
<details class="zg-primary"><summary>迁移指南：我已经集成了仅支持 1v1 PK 版本的演示，如何升级到最新版本？</summary>

您可以使用 [git diff\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/compare/master..pk_1v1) 查看这些更改，并按照我们的文档逐项检查更改并将其添加到您的项目中。

![/Pics/zegocloud/live/diff_tree_android.png](//doc.oa.zego.im/Pics/zegocloud/live/diff_tree_android.png)
