...
ZIM SDK 提供了丰富的发送和接收消息的功能，详情请参考 [收发消息](/zim-ios/guides/messaging/send-and-receive-messages)。此处需要您使用可自定义的信令消息：`ZIMCommandMessage`，完整示例代码在 [ZIMService.swift](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/master/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService.swift) 中找到。
...
本文档中的示例代码是纯客户端 API + ZEGO 解决方案。如果您有自己的业务服务端，并希望进行更多的逻辑扩展，您可以使用我们的 [服务端 API](/zim-server/overview) 来传递信令，并结合您服务端的房间业务逻辑，以提高您的应用的可靠性。
...
开始连麦后的逻辑与 [快速开始](/solution-live-streaming-ios/implement/cdn-live-streaming/quick-start) 相同。如果您不熟悉如何推流和拉流以及如何渲染它们，请参考 [快速开始](/solution-live-streaming-ios/implement/cdn-live-streaming/quick-start)。
</Note>
