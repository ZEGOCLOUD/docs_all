...
本文是基于 [示例代码\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice) 来介绍的如何实现连麦功能，因此文档中的部分方法为示例代码封装方法。
</div>
...
### 2. 如何通过 ZIM SDK 接口发送和接收信令消息

ZIM SDK 提供了丰富的发送和接收消息的功能，详情请参考 [收发消息\|_blank](!IM-sendMessages)。此处需要您使用可自定义的信令消息：`ZIMCommandMessage`，完整示例代码在 [ZIMService.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/master/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService.swift) 中找到。

1. 在房间中通过调用 [sendMessage\|_blank](/article/api?doc=zim_API~objectivec_ios~protocol~ZIM#send-message-message-to-conversation-id-conversation-type-config-notification-callback) 发送信令消息 (`ZIMCommandMessage`)：
    ...
    ```
2. 发送消息后，房间中的其他用户将通过 [onReceiveRoomMessage\|_blank](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-receive-room-message-from-room-id) 回调接收到信令消息。您可以通过以下方式监听此回调：
...
此处完整示例代码，可以在 [ZIMService+RoomRequest.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/master/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService%2BRoomRequest.swift) 中查看。
</div>
...
本文档中的示例代码是纯客户端 API + ZEGO 解决方案。如果您有自己的业务服务端，并希望进行更多的逻辑扩展，您可以使用我们的 [服务端 API\|_blank](!IM-Server_APIs/API_Overview) 来传递信令，并结合您服务端的房间业务逻辑，以提高您的应用的可靠性。
...
为 `ZIM SDK` 创建一个 `ZIMService` 类，它管理与 SDK 的交互并存储所需数据，详情请参考 [ZIMService.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService.swift) 中的完整代码。
...
同样，为 `Express SDK` 创建一个 `ExpressService` 类，它管理与 SDK 的交互并存储所需数据，详情请参考 [ExpressService.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/Internal/SDK/Express/ExpressService.swift) 中的完整代码。
...
完整代码，请参考 [ZegoSDKManager.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/Internal/ZegoSDKManager.swift)。
...
2. 调用示例代码中封装的 [sendRoomRequest\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/44169b0962cf1a08fbb6e798eb452de6c66554f6/best_practice/ZegoCloudSDKDemo/Internal/SDK/ZIM/ZIMService%2BRoomRequest.swift#L9) 发送信令。（`sendRoomRequest` 简化了 `ZIM SDK` 的 [sendMessage\|_blank](/article/api?doc=zim_API~objectivec_ios~protocol~ZIM#send-message-message-to-conversation-id-conversation-type-config-notification-callback) 接口。）
...
3. 本地端（观众端）将等待主播的响应。
   ...
   - 如果主播接受了连麦请求：将开始连麦（有关开始和结束连麦的详细信息，请参考 [开始或结束连麦部分](20744#4_5)）。

### 4. 接受或拒绝连麦请求

1. 在示例代码中，当主播收到连麦请求的信令时，将弹出确认对话框，等待主播选择接受或拒绝。
2. 主播响应后，会发送接受或拒绝的信令。此处不再进一步描述发送信令的相关逻辑。完整代码可以在 [ApplyCoHostListViewController.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/ViewControllers/ApplyCoHostListViewController.swift) 中查看。

相关代码片段如下，完整代码可在 [ApplyCoHostListViewController.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/ViewControllers/ApplyCoHostListViewController.swift) 中找到。
...
当观众接收到主播同意连麦的信令时，他们可以通过调用 `Express SDK` 的相关方法来预览和推流，从而成为连麦主播。完整代码可以在  [LiveStreamingViewController.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/ViewControllers/LiveStreamingViewController.swift) and [ExpressService+Stream.swift\|_blank](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/Internal/SDK/Express/ExpressService%2BStream.swift) 中查看。
