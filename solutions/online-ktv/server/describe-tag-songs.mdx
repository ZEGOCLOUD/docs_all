...
本接口调用需通过 [获取标签列表](./describe-tags.mdx) 中的标签 ID，可获取对应的歌曲 ID、歌曲名、歌曲专辑、以及版权相关信息等。
...
<td>String</td>
<td>是</td>
<td>标签 ID，通过 [获取标签列表](./describe-tags.mdx) 接口获取。</td>
</tr>
<tr>
<td>Page</td>
<td>Number</td>
...
以下仅列出了接口业务逻辑相关的部分返回码，完整返回码请参考 [全局返回码](./return-code.mdx)。
