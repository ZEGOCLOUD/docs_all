...
SDK 支持 Token 鉴权。若您需要升级鉴权方式，可参考 [如何从 AppSign 鉴权升级为 Token 鉴权](https://doc-zh.zego.im/faq/express-token-upgrade)。
</Note>
...
engine->loginRoom(roomID, user, roomConfig, [=](int errorCode, std::string extendedData)
        {
            if (error == 0) {
                ...
                // 登录失败，请参考 errorCode 说明 https://doc-zh.zego.im/real-time-video-windows-cpp/client-sdk/error-code
            }
        });
