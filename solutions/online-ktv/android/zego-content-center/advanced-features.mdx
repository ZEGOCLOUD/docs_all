...
在使用以下“进阶功能”之前，请先实现 [点歌](./sing-songs.mdx) 功能。
...
- 点歌后或播放歌曲前，用户可以调用获取歌词的方法获取整首歌的歌词。具体回调的信息参考 [获取歌曲与歌词资源接口说明](./../client-api/apis-to-obtain-songs-and-lyrics.mdx)。
- 成功获取歌词之后，需要将歌词放入歌词控件中进行展示。ZEGO 提供了独立的歌词 UI 组件，开发者可将该组件导入项目中，即可与 SDK 搭配使用，快速实现歌曲歌词的效果展示，详情请参考 [歌词组件](./lyrics-display-component.mdx)。
...
点歌用户点完歌曲后，其他用户可以调用 [getSharedResource](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoCopyrightedMusic#get-shared-resource) 获取对应的歌曲资源，接口回调的信息参考 [获取歌曲与歌词资源接口说明](./../client-api/apis-to-obtain-songs-and-lyrics.mdx)。

获取成功后，可参考 [点歌 - 下载歌曲](./sing-songs.mdx#下载歌曲) 获取歌曲资源。
...
1. 通过 [实现歌词与歌曲进度的同步](./advanced-features.mdx#实现歌词与歌曲进度的同步) 中获取逐字歌词。
