...
| 参数                   | 类型 | 是否必选 | 描述                                                                                                                 |
| ---------------------- | ---- | -------- | -------------------------------------------------------------------------------------------------------------------- |
| EnableASRServerMessage | int  | 否       | 是否开启服务端回调 ASR结果。<ul><li>0：不开启（默认）。</li><li>1：开启。ZEGO 服务端将会按照每句话返回 ASR 输出结果。请参考 [通用回调](./../Callbacks/General%20Callback.mdx) 了解如何配置回调地址和具体字段说明。</li></ul> |
| EnableLLMServerMessage | Int  | 否       | 是否开启服务端回调 LLM结果。<ul><li>0：不开启（默认）。</li><li>1：开启。ZEGO 服务端将会按照每句话返回 LLM 输出结果。请参考 [通用回调](./../Callbacks/General%20Callback.mdx) 了解如何配置回调地址和具体字段说明。</li></ul> |
|                        |      |          |                                                                                                                      |
