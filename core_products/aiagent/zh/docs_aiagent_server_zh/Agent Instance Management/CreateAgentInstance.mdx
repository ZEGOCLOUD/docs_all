...
| ChatConfig       | Object  | 否       | 对话配置参数。本参数结构，请参考 [ChatConfig](#chatconfig)。                           |
...
| WelcomeMessage  | String  | 否       | 语音对话欢迎语。如果配置此参数，每次用户进入对话时，智能体实例都会发送此语音。若此参数为空，则不发送欢迎语。     |
...
| 参数                   | 类型 | 是否必选 | 描述                                                                                                                 |
| ---------------------- | ---- | -------- | -------------------------------------------------------------------------------------------------------------------- |
| EnableASRServerMessage | Number  | 否       | 是否开启服务端回调 ASR结果。<ul><li>0：不开启（默认）。</li><li>1：开启。ZEGO 服务端将会按照每句话返回 ASR 输出结果。请参考 [通用回调](./../Callbacks/General%20Callback.mdx) 了解如何配置回调地址和具体字段说明。</li></ul> |
| EnableLLMServerMessage | Number  | 否       | 是否开启服务端回调 LLM结果。<ul><li>0：不开启（默认）。</li><li>1：开启。ZEGO 服务端将会按照每句话返回 LLM 输出结果。请参考 [通用回调](./../Callbacks/General%20Callback.mdx) 了解如何配置回调地址和具体字段说明。</li></ul> |
|                        |      |          |                                                                                                                      |
