...
| AgentStreamId  | String  | 是       | 智能体实例推流使用的流 ID。<Note title="说明">请确保当前运行中的多个智能体实例使用不同的流 ID，否则会导致后创建的智能体实例推流失败。</Note>                                                      |
| AgentUserId    | String  | 是       | 智能体实例的用户 ID。<Note title="说明">需确保同时在运行中的多个智能体实例使用不同的用户 id，否则先创建的智能体实例会被踢出 RTC 房间。</Note>                                          |
