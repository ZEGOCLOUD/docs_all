...
<Note title="说明">
对于非公共请求参数中的 ID 类参数，包括 ConversationId、UserId、AgentId、RoomId、StreamId、AgentStreamId、AgentTemplateId 等，应当遵循以下规则：
- AgentId 的字符限制：数字、英文字符、'-'、'_'、'@'。
- 其它 ID 类参数的字符限制：数字、英文字符、'-'、'_'。
- 长度上限：
  - ConversationId、RoomId、StreamId、AgentStreamId、AgentTemplateId：128 字节。
