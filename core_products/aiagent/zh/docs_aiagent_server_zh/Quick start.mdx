...
您需要在客户端使用 ZEGO Express SDK 实现真实用户进入房间并推流。然后配合业务后台调用 AI Agent 提供的服务端 API 接口，实现将智能体加入房间并与真实用户进行实时互动。

请按实际业务需求参考 [Android 端快速开始](/aiagent-android/quick-start) 、 [iOS 端快速开始](/aiagent-ios/quick-start) 或 [Web 端快速开始](/aiagent-web/quick-start) 文档实现客户端功能。
...
通常智能体是相对比较固定的，一旦设定好智能体的相关参数（人设形象）就不会经常改动。所以建议按照业务流程需要在适当时机注册智能体即可。
...
- 如遇智能体无法输出文本内容或语音，请先检查 LLM 和 TTS 参数配置是否完全正确，或参考 [获取 AI Agent 服务状态 - 监听服务端异常事件](./Guides/Get%20AI%20Agent%20status.mdx#监听服务端异常事件) 确定具体的问题。  
</Note>
...
<Step title="删除智能体实例">
[删除智能体实例](./API%20reference/Agent%20Instance%20Management/Delete%20Agent%20instance.mdx)后，智能体实例会自动退出房间并停止推流。用户再停止推流和退出房间后，一次完整的互动就结束了。
