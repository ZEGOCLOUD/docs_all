...
在注册Agent智能体（[RegisterAgent](./../API%20reference/Agent%20Configuration%20Management/Register%20Agent.mdx)）或创建Agent实例（[CreateAgentInstance](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx)）时，设置 TTS 结构体中填写Params字段，该字段将透传给第三方 TTS 的接口，其中就包括音色信息：
