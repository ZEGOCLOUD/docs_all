...
本文档旨在指导开发者如何利用 [ZIM (ZEGO Instant Messaging)](https://doc-zh.zego.im/zim-server/overview) 的实时通讯能力与 LLM 的自然语言处理能力，实现与 AI 进行 IM 聊天并结合 AI Agent的功能发起语音通话。
...
AI Agent 服务需要通过 ZIM 机器人来接收和发送消息。因此，您需要在 ZIM 后台注册一个机器人，并获取其 `UserId`。
...
#### 1.2 调用 LLM 服务生成 AI 回复

从回调收到用户消息后，业务后台需调用您设置的 LLM 服务以生成 AI 回复。您可以使用如 DeepSeek、火山方舟（豆包）、MiniMax、火山引擎、阿里云、阶跃星辰、 OpenAI (GPT 系列)或自己部署的 LLM 服务。

调用通常通过 HTTP API 完成。请务必查阅相应 LLM 提供商的 API 文档，了解请求构造、认证方式及响应解析。

以下是使用 Node.js 调用 DeepSeek 模型的示例代码：

```javascript title="Node.js 调用 DeepSeek 示例"
...
#### 1.3 将 AI 回复通过 ZIM 发送回客户端

获取到 LLM 的回复后，业务后台需使用 ZIM Server API 将此回复内容以 AI 身份发送给真实用户。

具体接口使用请参考 ZIM Server API 文档中的 [发送单聊消息](https://doc-zh.zego.im/zim-server/messaging/send-a-one-to-one-message)接口。

以下是使用 Node.js 通过 ZIM Server API 发送消息的示例：
...
AI 回复的消息会通过 ZIM 服务发送到客户端。您需要在客户端实现消息接收逻辑，并将内容展示在用户界面上。以下是各平台实现接收消息的示例代码：
...
-   **目的**: 避免因 LLM 处理耗时较长导致的用户界面卡顿或无响应。
-   **方法**:
...
在实际应用场景中，用户可能会在文字互动和语音互动之间切换。为了保持对话的连贯性和上下文的完整性，您可以通过以下方式将 IM 聊天的上下文关联到语音互动中：

1. 在[创建智能体实例](https://doc-zh.zego.im/aiagent-server/api-reference/agent-instance-management/create-agent-instance)并开始语音互动时，配置 MessageHistory 参数：
...
2. 完成以上配置后，创建的智能体实例在语音互动中将自动获取 IM 聊天的上下文作为 LLM 的历史消息输入。

详细步骤请参考[快速开始](./../Quick%20start.mdx)文档实现与 AI 进行语音通话功能。
