...
| LLM             | Object  | 否       | 大语言模型参数。本参数结构，请见 [智能体参数说明 - LLM](./../Common%20Parameter%20Description.mdx#llm)。                                               |
| TTS             | Object  | 否       | 文本转语音参数。本参数结构，请见 [智能体参数说明 - TTS](./../Common%20Parameter%20Description.mdx#tts)。                                               |
...
在配置以下参数前，你需要参考 [接收回调](./../../Callbacks/Receiving%20Callback.mdx) 回调地址，并了解具体字段说明。
</Note>
| 参数             | 类型   | 是否必选 | 描述                                                                                                                                                                                                                         |
| ---------------- | ------ | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Interrupted      | Number | 否       | 是否开启服务端回调智能体被打断结果。若开启。<ul><li>0：不开启。（默认）</li><li>1：开启。</li></ul>                                                         |
...
| 参数 | 类型 | 是否必选 | 描述 |
|------|------|----------|------|
| InterruptMode | Number | 否 | 智能体在回复时是否会被用户语音打断：<ul><li>0：会（默认）。</li><li>1: 不会，即等待智能体语音输出完成后，才开始 ASR。</li></ul> |
