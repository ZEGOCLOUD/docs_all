...
| Round | Number | <p>对话轮次，每次用户主动说话，轮次都会增加（有序，非递增）。</p><p>您可以基于对话轮次处理打断逻辑，例如：</p><ol><li>如果当您收到 `Round` 为 1 的回调时，您未及时响应该回调（回复有效 System）。</li><li>随即，用户说话打断了智能体 `Round` 为 1 的回复（此时 Round 为 2），您才响应 Round 为 1 的回调。</li><li>此时， ZEGO AI Agent 服务器不会处理该应答。</li></ol> |
