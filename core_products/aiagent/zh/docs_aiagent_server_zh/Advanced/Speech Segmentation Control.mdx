...
影响用户说话结束判断的两个参数在注册/更新智能体、创建/更新智能体实例的 [ASR 参数](./../API%20reference/Common%20Parameter%20Description.mdx#asr)中。详细说明如下：
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| VADSilenceSegmentation | Number | 否 | 用于设置用户说话停顿多少毫秒后，不再将两句话视为一句。单位为 ms，范围 [200，2000]，默认为 500。 |
| PauseInterval | Number | 否 | 用于设置用户说话停顿多少毫秒内，将两句话视为一句，即 ASR 多句拼接。单位为 ms，范围 [200，2000]。仅当此值大于 VADSilenceSegmentation，才会开启 ASR 多句拼接。 |
