...
| ConversationId   | String | 是    | 目标会话的唯一标识。仅支持数字、英文字符、'-'、'_'。长度上限为 128 字节。在同一 AppID 下全局唯一。           |
| AgentId | String | 是 | 智能体 ID（与目标会话中的智能体 ID 一致）。用于登录 IM 或 RTC 服务。仅支持 数字、英文字符、'-'、'_'、'@'。长度上限为 32 字节。                              |
...
```https
https://aigc-chat-api.zegotech.cn?Action=SendAgentTTSInput
&<公共请求参数>
```
