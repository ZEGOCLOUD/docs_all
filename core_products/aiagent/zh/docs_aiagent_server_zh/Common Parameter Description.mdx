...
| IgnoreCustomBracketText | Array of String | 否       | <p>将 LLM 返回内容中两个相同内容之间的文字过滤后，再进行语音合成。</p><p>数组大小限制最大5个。示例：</p><ul><li>配置：["###"]</li><li>效果：<br/>###<br/>被忽略的内容<br/>###</li></ul>    |
| SilenceDuration         | Number          | 否       | 在基于 ZEGO 断句进行 TTS 时，断句停顿额外增加对应的耗时。默认为 0，单位为毫秒。取值范围 [0, 5000]。 |
| ExtensionTTSParams      | String          | 否       | <p>第三方 TTS 请求参数，一般为第三方请求体。</p><p>格式为：转义之后的 JSON 字符串。</p>比如：`\{\"model\":\"model_name\",\"text\":\"text_content\",\"temperature\":0.1,\"top_p\":1.0\}`<p>目前支持第三方的参数：</p><ul><li>MiniMax 大模型 TTS。</li><li>火山引擎 TTS（大模型语音合成API)。</li><li>火山引擎 TTS（双向流式 API）。</li></ul>本参数结构，请见 [ExtensionTTSParams（各第三方 TTS 扩展透传参数说明）](#extensionttsparams各第三方-tts-扩展透传参数说明)。    |
...
<Note title="说明">以下示例虽然完整展示了 MiniMax 支持的所有参数，但是 ZEGO 仅支持高亮参数。</Note>
...
示例如下：
<Note title="说明">以下示例虽然完整展示了火山引擎 TTS（大模型语音合成API) 支持的所有参数，但是 ZEGO 仅支持高亮参数。</Note>
...
示例如下：
<Note title="说明">以下示例虽然完整展示了火山引擎 TTS（双向流式 API）支持的所有参数，但是 ZEGO 仅支持高亮参数。</Note>
