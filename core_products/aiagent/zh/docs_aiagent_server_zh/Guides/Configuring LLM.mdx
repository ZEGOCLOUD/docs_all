...
| AddAgentInfo | Bool   | 否       | 如果该值为 true ，在 AI Agent 后台向 LLM 服务发起请求时，请求参数中会包含智能体信息 `agent_info`。该值默认为 false。在使用自定义 LLM 时可根据此参数内容做额外的业务逻辑。 |
...
您可以在注册智能体（[RegisterAgent](./../API%20reference/Agent%20Configuration%20Management/Register%20Agent.mdx)）或创建智能体实例（[CreateAgentInstance](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx)）时设置 LLM 参数。
...
<Accordion title="请求体示例" defaultOpen="false">
```json
{
