...
您可以在注册智能体（[RegisterAgent](/aiagent-server/agent-configuration-management/register-agent)）或创建智能体实例（[CreateAgentInstance](/aiagent-server/agent-instance-management/create-agent-instance)）时设置 LLM 参数。常用参数如下：
...
在注册智能体（[RegisterAgent](/aiagent-server/agent-configuration-management/register-agent)）或创建智能体实例（[CreateAgentInstance](/aiagent-server/agent-instance-management/create-agent-instance)）时，设置自定义 LLM 的配置。
