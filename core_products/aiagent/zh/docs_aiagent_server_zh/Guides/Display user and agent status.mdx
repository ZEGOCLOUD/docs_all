...
请参考 [接收回调](./../Callbacks/Receiving%20Callback.mdx) 文档开发好用于接收 AI Agent 事件通知的回调，并提供地址联系 ZEGO 技术支持进行配置。
<Note title="说明">

要接收用户和智能体状态的回调结果，请在[创建智能体实例](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx)时，配置相应的 [CallbackConfig.UserSpeakAction](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx#callbackconfig) 和 [CallbackConfig.AgentSpeakAction](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx#callbackconfig) 参数为 `1`：
