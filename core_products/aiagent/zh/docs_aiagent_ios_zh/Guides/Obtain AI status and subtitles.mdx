...
本文介绍如何通过监听 ZEGO Express SDK 的 [onIMRecvCustomCommand](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-im-recv-custom-command-from-user-room-id) 回调获取 AI 状态和字幕。

## 功能概述

通过监听 [onIMRecvCustomCommand](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-im-recv-custom-command-from-user-room-id) 回调，您可以：
...
通过 ZEGO Express SDK 的 [onIMRecvCustomCommand](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-im-recv-custom-command-from-user-room-id) 回调接收到的消息格式如下：
...
2. 收到 ZEGO Express SDK 的 [onIMRecvCustomCommand](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-im-recv-custom-command-from-user-room-id) 回调后开始进行消息的处理：
...
- (void)addMessage:(ZegoAITextMessage *)message {
    ...
    if (existingIndex >= 0) {
        ...
        if ([message isKindOfClass:[ZegoAIAITextMessage class]]) {
            ...
            // 更新序列号
            if (message.seqId > existingMsg.seqId) {
                existingMsg.seqId = message.seqId;
            }
        } else {
            // 如果是用户消息，我们比较序列号
            if (message.seqId > existingMsg.seqId) {
