...
2. 初始化 ZIM SDK，使用 [createWithAppConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objectivec_ios~protocol~ZIM#create-with-app-config) 创建 ZIM 实例
```objective-c
...
通过 ZIM SDK 的 [setEventHandler](https://doc-zh.zego.im/article/api?doc=zim_API~objectivec_ios~protocol~ZIM#set-event-handler) 监听 [peerMessageReceived](https://doc-zh.zego.im/article/api?doc=zim_API~objectivec_ios~protocol~ZIMEventHandler#zim-peer-message-received-info-from-user-id) 回调接收单聊消息，实现基本的 AI 文本互动功能
