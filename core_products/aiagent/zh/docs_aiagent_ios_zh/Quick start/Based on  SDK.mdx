...
2. 登录 Express 房间并且推流， 具体过程见 [](https://doc-zh.zego.im/article/7628#4)。推流成功后，调用 AIAgentSDK 接口 `[ZegoAIAgent sharedInstance] startChat`来开启会话，并且在开启会话成功以后，主动去拉 AI 的语音流：
```objective-c
    ...
    [[ZegoExpressEngine sharedEngine] loginRoom:roomID
        ...
        NSString* agentStreamID = @"user_stream_id"; // AI的推流 ID，由用户自己定义生成规则
...
```
3. 此时会话已经成功建立，用户可以通过 Express SDK 的 `onIMRecvCustomCommand` 回调来获取并且展示语音聊天的文本内容,具体的展示逻辑见 [获取AI状态 & 字幕](../Guides/Obtain%20AI%20status%20and%20subtitles.mdx)
```objective-c
```
...
- (void)startRtcChatInternal:(NSString*)conversationID
    ...
    NSString* userId = DEFAULT_USER_ID;
    NSLog(@"StartRtcChat params, userId=%@, conversationId=%@, roomID=%@, userStreamID=%@, agentStreamID=%@",
            userId,
            conversationID,
            roomID, streamID,
            agentStreamID);
...
- (void)onRoomStreamUpdate:(ZegoUpdateType)updateType
    ...
    if (updateType == ZegoUpdateTypeAdd) {
        for (int i=0; i<streamList.count; i++) {
            ...
            NSLog(@"onRoomStreamUpdate, ZegoUpdateTypeAdd userID=%@, userName=%@, streamID=%@",
                  item.user.userID,
                  item.user.userName,
                  item.streamID);
...
- (void)startPlayStream:(NSString*)streamId {
    NSLog(@"Start play stream: %@", streamId);
