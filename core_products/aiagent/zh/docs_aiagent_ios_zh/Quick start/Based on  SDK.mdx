...
通过[createEngineWithProfile](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#create-engine-with-profile-event-handler) 接口初始化 ZegoExpress SDK ，具体过程见 [创建引擎](https://doc-zh.zego.im/article/7631#2_1)
```objc
...
此时会话已经成功建立，用户可以通过 ZEGO Express SDK 的 [onIMRecvCustomCommand](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoEventHandler#on-im-recv-custom-command-from-user-room-id) 回调来获取并且展示语音聊天的文本内容,具体的展示逻辑见 [获取 AI 状态和字幕](../Guides/Obtain%20AI%20status%20and%20subtitles.mdx)
</Step>
</Steps>
...
1. 调用 AIAgent SDK 接口 [stopChat](../Client%20API/API%20reference.mdx#stopchat) 结束会话。
...
<Tabs>
<TabItem title="AudioChatViewController.h">
```objc
...
<TabItem title="AudioChatViewController.m">
```objc
