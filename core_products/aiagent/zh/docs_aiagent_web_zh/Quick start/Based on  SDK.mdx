...
- 已在 [ZEGO 控制台](https://console.zego.im) 创建项目，并申请有效的 AppId 和 Server [控制台 - 项目信息](https://doc-zh.zego.im/article/12107)。
...
1. 使用 AI Agent SDK 进行初始化，通过 [getInstance](../Client%20API/API%20reference.mdx#getinstance) 方法获取单例。
...
使用 [createConversation](../Client%20API/API%20reference.mdx#createconversation) 创建会话，传入自定义的智能体配置，包括设置智能体模板 id、智能体名称、性别、系统角色、欢迎语、TTS 配置等，创建会话成功后，会返回会话Id。 
