...
数字人 SDK 依赖 Express 的视频帧和 SEI 数据，需要开启 Express 的自定义视频渲染能力并且将 Express 的视频帧和 SEI 数据同步给数字人 SDK。

<Warning title="注意">
- 开启 Express 自定义视频渲染能力需要在调用 Express [startPublishingStream]、[startPlayingStream] 接口之前设置，否则无效。
</Warning>
