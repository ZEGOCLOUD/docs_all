...
以下视频演示了如何跑通服务端和客户端（Web）示例代码并跟智能体进行视频互动。
...
sequenceDiagram
    ...
    客户端->>客户端: 初始化数字人 SDK 并设置数字人配置（web没有这一步骤）
    ...
    客户端->>客户端: 用户拉智能体流并且把帧数据和 SEI 数据传给数字人 SDK（web这里是"用户拉智能体流"）
...
<Step title="初始化数字人 SDK 实例">
需要先在 android 布局文件中添加数字人预览视图，数字人会渲染到该视图上。
...
```java
String digitalHumanConfig = xxx;            // 数字人配置，从业务后台获取
IZegoDigitalMobile digitalMobile = ZegoDigitalMobileFactory.create(this);
digitalMobile.start(digitalHumanConfig, null);
digitalMobile.attach(previewView);
```
</Step>
</Steps>
</div>
...
<Steps>
  <Step title="下载最新版本的SDK">
    请下载最新版本的 [SDK](https://storage.zego.im/AIEffect/DigitalHuman/sdk/ios/ZegoDigitalMobile_1.0.1.0.xcframework.zip)。
  </Step>
  <Step title="解压SDK">
...
请求业务后台的时候需要带上数字人参数，参数包括 **digital_human_id** 和 **config_id**。**digital_human_id** 是数字人 ID，请联系 ZEGO 技术支持获取。**config_id** 是数字人的配置 ID，不同平台使用不同的数字人配置， 数字人服务会根据 config_id 优化不同平台上的性能和效果，Android/iOS 请填写 **mobile**，Web 请填写 **web**。
...
private void start() {
    ...
    Request request = new Request.Builder().url(YOUR_SERVER_URL + "/api/start-digital-human").post(body).build();
...
恭喜你🎉！完成这一步骤后，您已经可以用语音问智能体任何问题，智能体都会用视频回答您的问题！
...
- 开启传统音频 3A 处理（回声消除AEC、自动增益控制AGC、噪声抑制ANS）
- 设置房间的使用场景为高品质语聊房场景，SDK 会针对不同的场景采取不同的优化策略
- 加载 [AI 降噪](https://doc-zh.zego.im/article/16459)模块，设置均衡模式模式（该功能需要联系 ZEGO 技术支持获取对应版本的 ZEGOExpress SDK）
