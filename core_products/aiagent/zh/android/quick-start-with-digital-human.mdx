...
private void stop() {
    RequestBody body = RequestBody.create("", MediaType.parse("application/json; charset=utf-8"));
    Request request = new Request.Builder().url(YOUR_SERVER_URL + "/api/stop-digital-human").post(body).build();
...
Future<Map<String, dynamic>> stopCall() async {
  try {
    final response = await http.post(
      Uri.parse('$_currentBaseUrl/api/stop-digital-human'),
      headers: {'Content-Type': 'application/json'},
    );
...
async function stopCall() {
  try {
    const response = await fetch(`${YOUR_SERVER_URL}/api/stop-digital-human`, { // YOUR_SERVER_URL 为您的业务后台地址
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      }
