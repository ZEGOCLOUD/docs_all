...
| Cmd | Int | 1: 用户说话状态<br />3: 识别的 ASR 文本<br />4: LLM 回答文本（增量下发）<br />6: 智能体实例状态<br />102: 元数据信息 |
| Data | Object | 具体内容。见Data |

<Accordion title="消息示例" defaultOpen="false">
```json
{
...
```json
{
    "method": "liveroom.room.on_recive_room_channel_message",
    "params": {
        "msg_content": "{\"Timestamp\":1765510398,\"TimestampMs\":1765510398029,\"SeqId\":278800725,\"Round\":0,\"Cmd\":102,\"Legacy\":false,\"Data\":{\"action\":\"动作1\"}}",
        "msg_type": 1,
        "roomid": "ir_20p158E0",
        "send_idname": "@RBT#38475_xiaozhi-sx_174722027",
        "send_nickname": ""
...
```json
{
    ...
    "content": {
        ...
        "sendNickName": "小智",
        "msgType": 1,
        "msgContent": "{\"Timestamp\":1765790414,\"TimestampMs\":1765790414022,\"SeqId\":558853069,\"Round\":0,\"Cmd\":102,\"Legacy\":false,\"Data\":{\"action\":\"动作1\"}}"
    }
}
```
</Accordion>
...
与 `msg_content` 同级的其他状态信息参数说明如下：

| 参数 | 类型 | 描述 |
|------|------|------|
| msy_type | int | 1：表示房间自定义消息 |
| roomid | string | 房间id |
| send_idname | string | 自定义消息发送者的 UserID |
| send_nickname | string | 自定义消息发送者的昵称 |
:::

:::if{props.platform="web"}

与 `msgContent` 同级的其他状态信息参数说明如下：

| 参数 | 类型 | 描述 |
|------|------|------|
| msgType | int | 1：表示房间自定义消息 |
| roomID | string | 房间id |
| sendIDName | string | 自定义消息发送者的 UserID |
| sendNickName | string | 自定义消息发送者的昵称 |
