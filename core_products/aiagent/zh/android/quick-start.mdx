...
/// 生成 RTC Token [参考文档]（/real-time-video-flutter/communication/using-token-authentication）
      ...
      if (updateType === "ADD" && streamList.length > 0) {
        try {
          for (const stream of streamList) {
            ...
            if (remoteView) {
             // 这里需要页面上有个id为remoteSteamView的容器接收智能体流 [参考文档]（/real-time-voice-web/client-sdk/api-reference/class#zegostreamview）
