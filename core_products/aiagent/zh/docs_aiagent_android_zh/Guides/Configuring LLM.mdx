...
您可以在注册Agent智能体（[RegisterAgent](/aiagent-server/agent-configuration-management/registeragent)）或创建Agent实例（[CreateAgentInstance](/aiagent-server/agent-instance-management/createagentinstance)）时设置 LLM 参数。常用参数如下：
