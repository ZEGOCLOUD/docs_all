...
本文介绍如何通过监听 ZEGO Express SDK 的 [onIMRecvCustomCommand](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~java_android~class~IZegoEventHandler#on-im-recv-custom-command) 回调获取 AI 状态和字幕。

## 功能概述

通过监听 [onIMRecvCustomCommand](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~java_android~class~IZegoEventHandler#on-im-recv-custom-command) 回调，您可以：
...
通过 ZEGO Express SDK 的 [onIMRecvCustomCommand](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~java_android~class~IZegoEventHandler#on-im-recv-custom-command) 回调接收到的消息格式如下：
...
2. 收到 ZEGO Express SDK 的 [onIMRecvCustomCommand](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~java_android~class~IZegoEventHandler#on-im-recv-custom-command) 回调后开始进行消息的处理：
...
<Tabs>
<Tab title="cmd == 1 或者 cmd == 2">
对于 cmd == 1 或者 cmd == 2 ,可以在 ui 上展示当前的状态：
```java
...
<Tab title="cmd == 3">
对于 cmd == 3, 相同的 message_id 取 seq_id 最大的消息进行覆盖，只显示 seq_id最大 的 message 的文本：
```java
...
<Tab title="cmd == 4">
对于 cmd == 4, 相同的 message_id 按照 seq_id 进行排序，然后合并文本进行显示。
```java
/**
 * 消息列表的所有数据
 */
