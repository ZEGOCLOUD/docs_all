...
您可以在注册 Agent 智能体（[RegisterAgent](/aiagent-server/agent-configuration-management/registeragent)）或创建 Agent 实例（[CreateAgentInstance](/aiagent-server/agent-instance-management/createagentinstance)）时通过 `ASR` 参数中的 `HotWord` 设置热词表。
