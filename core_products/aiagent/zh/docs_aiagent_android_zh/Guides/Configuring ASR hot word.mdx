...
您可以在注册 Agent 智能体（[RegisterAgent](/aiagent-server/agent-configuration-management/register-agent)）或创建 Agent 实例（[CreateAgentInstance](/aiagent-server/agent-instance-management/create-agent-instance)）时通过 `ASR` 参数中的 `HotWord` 设置热词表。
...
<Note title="注意">热词权重设置为11时，当前热词将升级为超级热词，建议仅将重要且必须生效的热词设置到11，设置过多权重为11的热词将影响整体字准率。</Note>
