...
| AgentId          | String | 是    | AgentId。机器人id，会被用于登录IM\RTC。注意保证App下AgentId唯一。必须以@RBT#开头。                                                                 |
| AgentTemplateId  | String | 否    | Agent模版id。zego提供或用户自定义，对应一个默认的agent配置。AgentTemplateId与CustomAgentConfig必须传一个。                                           |
| ChatHistoryMode  | Int    | 否    | 是否保存会话历史消息，历史消息会作为上下文输入给 LLM，默认为 0。<br/>0：保存会话历史消息至 zego 的 zim（需要自行集成 ZIM SDK 进行注册）。<br/>1：保存会话历史消息至 zego 的 zim（AI 陪伴服务端会自动注册 ZIM 用户）。<br/>2：不保存会话历史消息。 |
