...
2. 如果没有创建过会话，则需要使用 AIAgent SDK 接口 [createConversation](../Client%20API/API%20reference.mdx#createconversation) 来创建会话。
```java
...
通过上面的步骤，我们就创建了 AI Agent 会话，以下是完整代码：

<Accordion title="AI Agent 初始化和创建会话完整代码" defaultOpen="false">
```java
...
### 实现 AI 语音互动
完成会话创建后，就可以继续以下步骤来进行语音互动了：
<Steps>
<Step title="初始化 ZegoExpress SDK">
通过 [createEngine](https://doc-zh.zego.im/article/api?doc=express-audio-sdk_API~java_android~class~ZegoExpressEngine#create-engine) 接口初始化 ZegoExpress SDK
```java
...
<Step title="登录 Express 房间并且推流">
完成 ZegoExpress SDK 初始化后，调用 [loginRoom](https://doc-zh.zego.im/article/api?doc=express-audio-sdk_API~java_android~class~ZegoExpressEngine#login-room-2) 登录 Express 房间并且通过 [startPublishingStream](https://doc-zh.zego.im/article/api?doc=express-audio-sdk_API~java_android~class~ZegoExpressEngine#start-publishing-stream) 推流， 具体过程见[实时语音-使用步骤](https://doc-zh.zego.im/article/7636#2)
```java
//  roomID  由用户自己定义生成规则
...
<Step title="开启会话并且拉流">
推流成功后，调用 AIAgentSDK 的 [startChat](../Client%20API/API%20reference.mdx#startchat) 接口来开启会话，并且在开启会话成功以后，主动去拉 AI 的语音流：
```java
//...
ZegoExpressEngine.getEngine().loginRoom(roomID, new ZegoUser(userProfile.userID, userProfile.userName), roomConfig, new IZegoRoomLoginCallback() {
    @Override
...
<Step title="获取AI状态 & 字幕">

此时会话已经成功建立，用户可以通过 ZEGO Express SDK 的 [onIMRecvCustomCommand](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~java_android~class~IZegoEventHandler#on-im-recv-barrage-message) 回调来获取并且展示语音聊天的文本内容,具体的展示逻辑见 [获取AI状态 & 字幕](../Guides/Obtain%20AI%20status%20and%20subtitles.mdx)
...
1. 调用 AIAgent SDK 的 [stopChat](../Client%20API/API%20reference.mdx#stopchat) 接口结束会话。
2. 结束会话时 ZEGO Express SDK 也需要调用 [logoutRoom](https://doc-zh.zego.im/article/api?doc=express-audio-sdk_API~java_android~class~ZegoExpressEngine#logout-room) 和 [destroyEngine](https://doc-zh.zego.im/article/api?doc=express-audio-sdk_API~java_android~class~ZegoExpressEngine#destroy-engine) 退出房间并且销毁。
```java
