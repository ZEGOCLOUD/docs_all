...
- 联系 ZEGO 技术支持开通 AI Agent 相关服务。
- 已在第三方 LLM 厂商开通账号并获取 API Key。
- 已在第三方 TTS 厂商开通账号并获取相关配置信息。
<Note title="注意">
文本转语音（TTS）服务供应商。可选：
- Aliyun：阿里云。
- Bytedance：火山引擎。
...
<Note title="注意">请联系 ZEGO 技术支持获取支持 AI 降噪的 ZEGO Express SDK 版本，并在集成使用时开启 AI 降噪功能以获得更好的语音交互效果。</Note>
...
<Note title="注意">一个智能体只能注册一次（同一个ID），如果重复注册会返回错误码 410001008。</Note>
...
<CardGroup cols={2}>
<Card title="Android 示例代码" href="https://github.com/ZEGOCLOUD/ai_agent_quick_start/tree/master/android" target="_blank">
