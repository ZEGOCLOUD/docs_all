...
音色克隆属于增值能力，计费方式请参考：[TTS 定价](./../introduction/pricing.mdx#tts文本转语音) 说明。

<Note title="说明">目前支持火山引擎、MiniMax、阿里等多家厂商的声音复刻及文本转语音能力。</Note>

## 前提条件

- 已经参考[快速开始](./../quick-start.mdx)接入 AI Agent 服务。
...
2. 克隆音色
    ...
    - 方式2：在 MiniMaxAPI 调试台页面完成声音克隆
        ...
        <img src="https://doc-media.zego.im/docuo/ac1402eb31.png" alt="minimax-api-voice-clone.png"/>
        </Frame>
3. 克隆完成后，请保管好 voice_id 。
</Tab>
<Tab title="火山引擎V3版本单向流式 TTS">
...
2. 克隆完成后，请保管好 speaker 和声音复刻的集群（cluster）。
</Tab>
</Tabs>
</Step>
<Step title="在语音对话中使用克隆的音色">
在注册Agent智能体（[RegisterAgent](./../api-reference/agent-configuration-management/register-agent.mdx)）或创建智能体实例（[CreateAgentInstance](./../api-reference/agent-instance-management/create-agent-instance.mdx)）时，设置 TTS 结构体中填写Params字段，该字段将透传给第三方 TTS 的接口，其中就包括音色信息：
...
```json title="火山引擎 - V3版本单向流式TTS" {9-11,14}
// 1. resource_id：用于选择不同的声音复刻版本效果和计费方式，默认使用 seed-icl-2.0
//       seed-icl-2.0（默认，对应计费商品"声音复刻2.0字符版"）
//       seed-icl-1.0（对应计费商品"声音复刻1.0字符版"）
//       seed-icl-1.0-concurr（对应计费商品"声音复刻1.0并发版"）
//       更多详情请参考火山引擎官方文档发音人列表
// 2. speaker 填写为使用克隆的speaker
"TTS": {
    "Vendor": "ByteDanceV3",
    "Params": {
        "app": {
