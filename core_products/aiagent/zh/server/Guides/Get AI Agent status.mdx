...
- 智能体延迟数据：
    - 大语言模型LLM相关耗时：
        - LLM首token耗时（毫秒）。请求大语言模型～大语言模型返回首个非空token的耗时。
        - LLM输出速度（tokens/秒）。大语言模型平均每秒输出的token数量。
    - 文字转语音TTS相关耗时：
        - TTS音频首帧耗时（毫秒）
            - 单向流式：AI Agent判断LLM输出组句成功～TTS返回首个非静音音频帧的耗时。
            - 双向流式：LLM输出首token并开始发送TTS～TTS返回首个非静音音频帧的耗时。其中组句逻辑为TTS服务实现
    - AI Agent服务端总耗时（毫秒）。AI Agent服务端拉流并判断用户说话结束～TTS返回首帧并开始推流的耗时，所有服务端产生的耗时，至少包含语音识别ASR耗时、大语言模型LLM相关耗时、文字转语音TTS相关耗时等。
    - 客户端&服务端耗时。可通过RTC侧获取：https://doc-zh.zego.im/article/1074
这里还有个图片，不知道怎么加！！！！
