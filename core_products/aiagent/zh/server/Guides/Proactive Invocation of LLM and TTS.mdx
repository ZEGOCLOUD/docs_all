...
您可以调用 [SendAgentInstanceLLM](./../API%20reference/Agent%20Instance%20Control/Send%20Agent%20instance%20LLM.mdx) 接口，主动让 LLM 输出文本和语音。
...
| 参数            | 类型   | 是否必选 | 描述                                                                                                                                                                                                                                          |
| --------------- | ------ | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AgentInstanceId | String | 是       | 智能体实例的唯一标识，通过 [创建智能体实例](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx) 接口的响应参数获取。                                                                                                               |
| Text            | String | 是       | 发送给 LLM 服务的文本内容。                                                                                                                                                                                                                   |
| SystemPrompt    | String | 否       | 本次对话的临时智能体系统提示词。如果不填则使用[注册智能体](./../API%20reference/Agent%20Configuration%20Management/Register%20Agent.mdx)或者[创建智能体实例](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx)时的 LLM 参数中的 `SystemPrompt`。 |
...
您可以通过调用 [SendAgentInstanceTTS](./../API%20reference/Agent%20Instance%20Control/Send%20Agent%20instance%20TTS.mdx) 接口，主动让智能体用语音说一段文本内容。
...
| InterruptMode   | Int | 否 | 智能体回复时被用户语音打断的模式：<ul><li>0：语音打断，用户说话时，如果 AI 在说话，立刻打断 AI（默认）。</li><li>1: 语音不打断，如果 AI 在说话时，用户说话了，会忽略用户说的内容，知道 AI 说完话。</li></ul> |
