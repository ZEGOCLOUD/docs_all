...
在[创建智能体实例](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx)时可设置智能体实例的初始记忆（即智能体初始上下文）。记忆来源可分为“ZIM 的会话历史记录”和“自定义外部上下文”，
可通过创建智能体实例接口的 [MessageHistory](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx#messagehistory) 参数控制。相关参数详细结构如下：
...
2. 在[创建智能体实例](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx)时，将设置 `SyncMode` 为 0 ，并且填写对应的 `ZIM` 参数。示例如下：
...
1. 业务需要自己存储上下文信息。
2. 在[创建智能体实例](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx)时，将设置 `SyncMode` 为 1 ，并且填写对应的 `Messages` 参数。示例如下：
...
调用获取智能体实例上下文列表接口（[GetAgentInstanceMsgList](./../API%20reference/Agent%20Instance%20Control/Get%20Agent%20Instance%20Msg%20List.mdx)），传入创建智能体实例接口返回的 `AgentInstanceId` ，服务端将返回该实例的上下文列表，消息按照聊天的时间升序返回，示例如下：
...
调用重置智能体实例上下文列表接口（[ResetAgentInstanceMsgList](./../API%20reference/Agent%20Instance%20Control/Reset%20Agent%20Instance%20Msg%20List.mdx)），传入创建智能体实例接口返回的 `AgentInstanceId`，服务端将重置当前实例的上下文列表。
