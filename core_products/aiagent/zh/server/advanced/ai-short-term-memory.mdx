import MessagesStructure from '/snippets/Reuse/MessagesStructureforAIAgentCN.mdx'
...
- 语音通话过程中记忆：语音通话过程中产生的对话历史，会缓存在 AI Agent 服务端。
  ...
  - 若希望插入一条上下文信息到最近一轮对话中，可参考[插入消息到智能体实例上下文列表](#插入消息到智能体实例上下文列表)章节。
- 语音通话结束，记忆归档
...
在[创建智能体实例](./../api-reference/agent-instance-management/create-agent-instance.mdx)时可设置智能体实例的初始记忆（即智能体初始上下文）。记忆来源可分为“ZIM 的会话历史记录”和“自定义外部上下文”，
可通过创建智能体实例接口的 [MessageHistory](./../api-reference/agent-instance-management/create-agent-instance.mdx#messagehistory) 参数控制。相关参数详细结构如下：
...
2. 在[创建智能体实例](./../api-reference/agent-instance-management/create-agent-instance.mdx)时，将设置 `SyncMode` 为 0 ，并且填写对应的 `ZIM` 参数。示例如下：
...
1. 业务需要自己存储上下文信息。
2. 在[创建智能体实例](./../api-reference/agent-instance-management/create-agent-instance.mdx)时，将设置 `SyncMode` 为 1 ，并且填写对应的 `Messages` 参数。示例如下：
...
调用获取智能体实例上下文列表接口（[GetAgentInstanceMsgList](./../api-reference/agent-instance-control/get-agent-instance-msg-list.mdx)），传入创建智能体实例接口返回的 `AgentInstanceId` ，服务端将返回该实例的上下文列表，消息按照聊天的时间升序返回，示例如下：
...
调用重置智能体实例上下文列表接口（[ResetAgentInstanceMsgList](./../api-reference/agent-instance-control/reset-agent-instance-msg-list.mdx)），传入创建智能体实例接口返回的 `AgentInstanceId`，服务端将重置当前实例的上下文列表。

#### 插入消息到智能体实例上下文列表

调用插入消息到智能体实例上下文列表接口（[AddAgentInstanceMsg](./../api-reference/agent-instance-control/add-agent-instance-msg.mdx)），传入创建智能体实例接口返回的 `AgentInstanceId`，服务端将往当前实例的上下文列表最后一条消息后面插入 `Messages` 的值。
