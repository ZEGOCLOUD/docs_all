...
以豆包语音 2.0的自然语言描述（`context_texts` 标签），通过ZEGO `TTS-Vendor=ByteDanceV3` 调用"[单向流式语音合成 WebSocket](https://www.volcengine.com/docs/6561/2534913?lang=zh)"为例。

{/* TODO: Replace with actual flow diagram image */}

<Frame width="auto" height="auto" caption="">
  <img src="https://doc-media.zego.im/core_products/aiagent/zh/server/advanced/功能流程图zh.png" alt="controlling-tts-effects-flow"/>
</Frame>

## 具体使用流程

<Note title="示例说明">以豆包语音 2.0 的自然语言描述（`context_texts` 标签），通过 ZEGO `TTS-Vendor=ByteDanceV3` 调用"单向流式语音合成 WebSocket"为例，实现AI 自动判断并以指定语气 + 音调播报回复的效果。</Note>
...
通过 [`sendAgentInstanceLLM`](/aiagent-server/api-reference/agent-instance-control/send-agent-instance-llm) 主动触发 LLM 生成回复，LLM 同样按已配置的 `SystemPrompt` 输出带 `[[标签]]` 的文本，处理流程与触发方式1一致。

调用 [`sendAgentInstanceLLM`](/aiagent-server/api-reference/agent-instance-control/send-agent-instance-llm) 接口示例：
...
<Step title="ZEGO 自动处理并控制 TTS 播报">
...
<Note title="如何适配其他 TTS 厂商">

本示例以火山 `ByteDanceV3` 为例。若使用其他厂商，只需调整以下三处，其余流程（提取标签、触发方式、ZEGO 自动处理）完全不变：

- **配置2 的 `TTSParamPaths` 映射路径** — 替换为目标厂商的参数路径，见附录《[支持的厂商与参数](#支持的厂商与参数)》表"TTS 参数路径"列。例：阿里云 CosyVoice 的 `instruction` → `payload.parameters.instruction`
...
<Accordion title="早期实现方式（已废弃，仅供老用户参考）" defaultOpen="false">
