...
要实现该能力主要分为三步：1. 指定 LLM 文本中控制情绪的内容格式。2. 让 LLM 按照指定的控制情绪格式输出内容。3. 让 TTS 厂商根据情绪控制参数合成带情绪的语音（ZEGO AI Agent 自动处理）。
...
以下是使用 MiniMax 和豆包语音 TTS 时，对应的 LLM.SystemPrompt 示例，仅供参考，请根据实际需求调整：
<Tabs>
<Tab title="MiniMax">
```markdown
# 角色
