...
火山方舟大模型支持 OpenAI Chat Completions API 协议， 部分模型（如 Doubao-Seed 系列）支持 OpenAI Responses API 协议。请按实际需求配置：
...
仅支持 OpenAI Chat Completions API 协议。请按实际需求配置：
...
```json
{
    ...
    "max_tokens": 4096, // 对应 LLM.Params.max_tokens 参数
    "messages":[
        {
            "role": "system",
...
- 选择合适的 LLM 厂商、选择开发语言、填写 LLM 参数后即可点击校验按钮进行参数格式校验。
- 校验器会检查参数的 JSON 格式、必填项、参数拼写、大小写、取值范围等是否正确。
- “示例代码” Tab 为对应语言的示例代码。
- “实际请求LLM参数” 为将发送给 LLM 的完整请求参数预览，您可以根据对比所选厂商的官方文档，确保参数填写正确。
