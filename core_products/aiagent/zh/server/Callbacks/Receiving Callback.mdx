...
| TTSAudioFirstFrameTime | Int | TTS 音频首帧耗时（毫秒）。<ul><li>单向流式: 从LLM输出首Token开始，累积Token组成第一句话，到TTS返回收个非静音音频帧的耗时。</li><li>双向流式: LLM 输出首 token 并开始发送 TTS 至 TTS 返回首个非静音音频帧的耗时。(双向流式组句逻辑由 TTS 服务自己实现)</li></ul> |
| TotalCost | Int | 平均 AI Agent 服务端总耗时（毫秒）: AI Agent 服务端拉流并判断用户说话结束至TTS 返回首帧并开始推流的耗时。所有服务端产生的耗时，至少包含语音识别 ASR 耗时、大语言模型 LLM 相关耗时、文字转语音 TTS 相关耗时等。 |
</Tab>
</Tabs>
