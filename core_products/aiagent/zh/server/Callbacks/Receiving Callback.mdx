...
如您需要接收以下类型的回调结果，请在[创建智能体实例](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx)时，配置相应的 [CallbackConfig](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx#callbackconfig) 参数为 `1`：
...
| LatencyData | Object | 本次和智能体实例对话的延迟数据。 |
LatencyData
| 参数 | 类型 | 描述 |
|------|------|------|
| LLMTTFT | Int | LLM 首 token 平均耗时（毫秒）。请求大语言模型～大语言模型返回首个非空 token 的耗时。 |
| LLMTPS | Int | LLM 平均输出速度（tokens/秒）。大语言模型平均每秒输出的 token 数量 |
| TTSAudioFirstFrameTime | Int | TTS 音频首帧耗时（毫秒）。<ul><li>单向流式：AI Agent 判断 LLM 输出组句成功～TTS 返回首个非静音音频帧的耗时。</li><li>双向流式：LLM 输出首 token 并开始发送 TTS～TTS 返回首个非静音音频帧的耗时。其中组句逻辑为 TTS 服务实现</li></ul> |
| TotalCost | Int | 平均 AI Agent 服务端总耗时（毫秒）: AI Agent 服务端拉流并判断用户说话结束～TTS 返回首帧并开始推流的耗时，所有服务端产生的耗时，至少包含语音识别 ASR 耗时、大语言模型 LLM 相关耗时、文字转语音 TTS 相关耗时等。 |
</Tab>
</Tabs>
