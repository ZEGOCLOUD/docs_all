...
本文档所列参数被用在[注册智能体](./Agent%20Configuration%20Management/Register%20Agent.mdx)、[更新智能体](./Agent%20Configuration%20Management/Update%20Agent.mdx)、[创建智能体实例](./Agent%20Instance%20Management/Create%20Agent%20instance.mdx)、[更新智能体实例](./Agent%20Instance%20Management/Update%20Agent%20instance.mdx)等多个接口中。
...
| ApiKey          | String          | 否       | LLM 服务供应商用于鉴权的参数。默认为空，生产环境中务必传入。在接入测试期间，可以设置为“zego_test”以使用 ZEGO 提供的测试账号。             |
...
| AddAgentInfo | Bool   | 否       | 如果该值为 true ，在 AI Agent 后台向 LLM 服务发起请求时，请求参数中会包含智能体信息 `agent_info`。该值默认为 false。在使用自定义 LLM 时可根据此参数内容做额外的业务逻辑。`agent_info` 的结构详情请参考 [使用自定义 LLM](./../Guides/Configuring%20LLM.mdx#使用自定义-llm)。 |
...
| 参数 | 类型 | 是否必选 | 描述 |
|------|------|----------|------|
| app_key | String | 是 | 请参考阿里云文档 [智能语音交互 - 快速入门 - 从这里开始](https://help.aliyun.com/zh/isi/getting-started/start-here?spm=a2c4g.11186623.0.0.44f34b4f40XABA#659bb8103941q) 中 “步骤 4：管理项目” 获取 AppKey 并传入此处。在接入测试期间，可以设置为“zego_test”以使用 ZEGO 提供的测试账号。|
| ak_id | String | 是 | 请参考阿里云文档 [智能语音交互 - 快速入门 - 从这里开始](https://help.aliyun.com/zh/isi/getting-started/start-here?spm=a2c4g.11186623.0.0.44f34b4f40XABA#659bb8103941q) 中 “步骤 2：创建 AccessKey” 获取 AccessKey ID 并传入此处。在接入测试期间，可以设置为“zego_test”以使用 ZEGO 提供的测试账号。|
| ak_key | String | 是 | 请参考阿里云文档 [智能语音交互 - 快速入门 - 从这里开始](https://help.aliyun.com/zh/isi/getting-started/start-here?spm=a2c4g.11186623.0.0.44f34b4f40XABA#659bb8103941q) 中 “步骤 2：创建 AccessKey” 获取 AccessKey Secret 并传入此处。在接入测试期间，可以设置为“zego_test”以使用 ZEGO 提供的测试账号。|
</Tab>
<Tab title="ByteDance">

| 参数 | 类型 | 是否必选 | 描述 |
|------|------|----------|------|
| appid | String | 是 | 请参考火山引擎文档 [语音技术 - 快速入门 - 控制台使用 FAQ](https://www.volcengine.com/docs/6561/196768#q1%EF%BC%9A%E5%93%AA%E9%87%8C%E5%8F%AF%E4%BB%A5%E8%8E%B7%E5%8F%96%E5%88%B0%E4%BB%A5%E4%B8%8B%E5%8F%82%E6%95%B0appid%EF%BC%8Ccluster%EF%BC%8Ctoken%EF%BC%8Cauthorization-type%EF%BC%8Csecret-key-%EF%BC%9F) 中 “哪里可以获取到以下参数appid，cluster，token，authorization_type，secret_key ？”。在接入测试期间，可以设置为“zego_test”以使用 ZEGO 提供的测试账号。|
| token | String | 是 | 同上。 |
...
| 参数 | 类型 | 是否必选 | 描述 |
|------|------|----------|------|
| group_id | String | 是 | 请参考 MiniMax 文档 [快速开始](https://platform.minimaxi.com/document/Fast%20access?key=66701cf51d57f38758d581b2) 获取 group_id 并传入此处。在接入测试期间，可以设置为“zego_test”以使用 ZEGO 提供的测试账号。   |
| api_key | String | 是 | 请参考 MiniMax 文档 [快速开始](https://platform.minimaxi.com/document/Fast%20access?key=66701cf51d57f38758d581b2) 获取 api key 并传入此处。在接入测试期间，可以设置为“zego_test”以使用 ZEGO 提供的测试账号。   |
</Tab>
</Tabs>
...
- `MiniMax`：[语音模型 - T2A v2 - HTTP接口 - 接口请求参数](https://platform.minimaxi.com/document/T2A%20V2?key=66719005a427f0c8a5701643#YqSh1KAoyms1WH4XJrdeIrrb)。
...
| 参数 | 类型 | 是否必选 | 描述 |
|------|------|----------|------|
| HotWord | String | 否 | 热词表，用于提升识别准确率。格式：`热词1\|权重1,热词2\|权重2,热词3\|权重3`。<p />单个热词不超过 30 个字符（最多 10 个汉字），不能包含空格，权重范围：[-1, 11]。<p />最多支持 128 个热词。<Note title="说明">当权重为 11 时，表示该词为超级热词。建议仅将重要且必须生效的热词设置到 11，过多权重为 11 的热词会影响识别效果。</Note><br/>请参考[配置语音识别热词](./../Guides/Configuring%20ASR%20hot%20word.mdx)了解详细使用方法|
| Params | Object | 否 | 扩展参数，详情请联系 ZEGO 技术支持。 |
| VADSilenceSegmentation | Number | 否 | 用于设置用户说话停顿多少秒后，不再将两句话视为一句。单位为 ms，范围 [200，2000]，默认为 500。 <br/>请参考[语音识别断句](./../Advanced/Speech%20Segmentation%20Control.mdx)了解详细使用方法。|
| PauseInterval | Number | 否 | 用于设置用户说话停顿多少秒内，将两句话视为一句，即 ASR 多句拼接。单位为 ms，范围 [200，2000]。仅当此值大于 `VADSilenceSegmentation`，才会开启 ASR 多句拼接。<br/>请参考[语音识别断句](./../Advanced/Speech%20Segmentation%20Control.mdx)了解详细使用方法。 |
