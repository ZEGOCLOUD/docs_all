...
The two parameters that affect the determination of user's speech completion are in the ASR parameters of registering/updating agents and creating/updating agent instances. Please refer to [Register Agent > Body > ASR Parameters](/aiagent-server/api-reference/agent-configuration-management/register-agent) for details.
