...
- Memory During Voice Call: The chat history generated during voice calls and cached on the AI Agent server.
  ...
  - To insert a context message into the latest conversation round, read [Add Message to Agent Instance Context List](#add-message-to-agent-instance-context-list).
...
Call the [AddAgentInstanceMsg](./../api-reference/agent-instance-control/add-agent-instance-msg.mdx) API with the `AgentInstanceId` returned after creating an agent instance. The server will insert the `Messages` value after the last message in the current instance's context list.
