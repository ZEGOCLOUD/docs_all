...
- If the agent fails to output text or voice, first check if the LLM and TTS parameter configurations are completely correct, or refer to [Get AI Agent Status - Listen for Server Exception Events](./Guides/Get%20AI%20Agent%20status.mdx#listen-for-server-exception-events) to identify the specific issue.
</Warning>
