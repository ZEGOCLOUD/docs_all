...
| TTSAudioFirstFrameTime | Int | TTS audio first frame latency (in milliseconds).<ul><li>Unidirectional streaming: Time from LLM outputting first token and starting TTS until TTS returns the first non-silent audio frame. (Bidirectional streaming sentence composition logic is implemented by the TTS service)</li><li>Bidirectional streaming: Time from LLM outputting first token and starting TTS until TTS returns the first non-silent audio frame. (Bidirectional streaming sentence composition logic is implemented by the TTS service)</li></ul> |
