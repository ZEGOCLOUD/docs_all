...
| LLMTPS | Int | Average LLM Tokens Per Second. Number of tokens output by the large language model per second. |
| TTSAudioFirstFrameTime | Int | Average TTS audio first frame latency (in milliseconds).<ul><li>Unidirectional streaming: Time from LLM outputting first token and starting TTS until TTS returns the first non-silent audio frame. (Bidirectional streaming sentence composition logic is implemented by the TTS service)</li><li>Bidirectional streaming: Time from LLM outputting first token and starting TTS until TTS returns the first non-silent audio frame. (Bidirectional streaming sentence composition logic is implemented by the TTS service)</li></ul> |
| TotalCost | Int | Average total server-side latency (in milliseconds): Time from AI Agent server pulling stream and determining user speech end until TTS returns first frame and starts streaming. Includes all server-side processing time, including ASR latency, LLM-related latency, TTS-related latency, etc. |
</Tab>
</Tabs>
