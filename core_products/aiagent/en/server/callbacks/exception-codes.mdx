...
| 1004 | Server VAD detects human voice but ASR fails to recognize valid text | Statistics monitoring or ignore and do not process. |
...
| 1202 | The user who created the AI agent instance has exceeded 120 seconds (or the AdvancedConfig.MaxIdleTime parameter custom time) and is not in the RTC room. | Please check if the user who created the AI agent instance is in the RTC room. |
...
| 1208 | RTC failed to send room message | Please provide the `Message` field to ZEGOCLOUD Technical Support. |
