...
<Warning title="Note">**Please contact ZEGOCLOUD sales to enable ElevenLabs TTS before using it.**</Warning>
...
Except for the `app` parameter, other parameters are passed through to ElevenLabs. The `Url` parameter is optional and defaults to `wss://api.elevenlabs.io/v1/text-to-speech`.

#### Setting the API Key

The ElevenLabs API Key should be placed in the `app.api_key` field.

#### Setting the Model

- `model_id`: Model ID, optional. The `eleven_flash_v2_5` model is recommended.

#### Setting the Voice

- `voice_id`: Voice ID, required. The `voice_id` must be a voice under the user's account that matches the `model_id`.

#### Voice Settings (Optional)

`voice_settings` is used to adjust the voice effects. Some common parameters are as follows:

| Parameter         | Type    | Description                                                                                                          |
| ----------------- | ------- | -------------------------------------------------------------------------------------------------------------------- |
| stability         | float   | Voice stability. The lower the value, the more expressive the emotion; the higher the value, the more stable the voice. |
| similarity_boost  | float   | Similarity boost to the original voice. The higher the value, the more similar the synthesized voice is to the original voice. |
| speed             | float   | Speech rate. The default value is 1.                                                                                 |
| style             | float   | Speaking style intensity. The higher the value, the closer it is to the original speaking style of the voice.        |
| use_speaker_boost | boolean | Whether to enable speaker boost, which improves the similarity to the original voice when enabled.                   |

#### Usage Example

The following is an example of using the ElevenLabs TTS with the `eleven_flash_v2_5` model.
...
// !mark(10:26)
{
    ...
    "LLM": {
        ...
        "SystemPrompt": "You are Xiaozhi, an adult woman and a companion assistant created by ZEGOCLOUD."
    },
    "TTS": {
        "Vendor": "ElevenLabs",
        "Params": {
