...
| AgentExtraInfo  | Object          | No       | Agent extra information, which is passed to the LLM service when the server requests the LLM service. The example of the passed parameter is referred to [Using Custom LLM](#use-custom-llm). You can use this parameter to execute additional business logic in your custom LLM service. |
