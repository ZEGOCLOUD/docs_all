import LLMValidator from '../../../zh/server/LLMValidator.jsx';
...
|-----------------|-----------------|----------|--------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Vendor          | String          | No       | The vendor type of the LLM interface.<ul><li>OpenAIChat: OpenAI's Chat Completions interface type (default).</li><li>OpenAIResponses: OpenAI's Responses API interface type.</li></ul> |
| Url             | String          | Yes      | LLM callback address, which must be compatible with the OpenAI protocol.<ul><li>If Vendor is "OpenAIChat", it must be compatible with [OpenAI Chat Completions API](https://platform.openai.com/docs/api-reference/chat).</li><li>If Vendor is "OpenAIResponses", it must be compatible with [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses).</li></ul> |
...
| AddAgentInfo    | Bool            | No       | If this value is true, when the AI Agent backend sends requests to custom LLM services, the request parameters will include agent information `agent_info`. This value defaults to false. When using custom LLM, additional business logic can be implemented based on this parameter content. <Note title="Note">Only effective when Vendor is "OpenAIChat".</Note> |
| AgentExtraInfo  | Object          | No       | Agent extra information, which is passed to the LLM service when the server requests the LLM service. The example of the passed parameter is referred to [Using Custom LLM](#use-custom-llm). You can use this parameter to execute additional business logic in your custom LLM service. <Note title="Note">Only effective when Vendor is "OpenAIChat".</Note> |
...
Volcano Ark Large Model supports OpenAI Chat Completions API protocol, and some models (such as Doubao-Seed series) support OpenAI Responses API protocol. Please configure according to your actual needs:

For model usage docs, read [Chat Completions API](https://www.volcengine.com/docs/82379/1298454).
```json
// !mark(2)
"LLM": {
    ...
    "Model": "doubao-1-5-pro-32k-250115",    // Your inference access point created on the Volcano Ark Large Model Platform
...
```
For model usage docs, read [Responses API](https://www.volcengine.com/docs/82379/1585128?lang=zh).
```json
// !mark(2:3)
"LLM": {
    ...
    "ApiKey": "zego_test", // your api key (zego_test can be used during the integration testing period (within 2 weeks of AI Agent service activation))
    "Model": "doubao-seed-1-6-flash-250828",    // Your inference access point created on the Volcano Ark Large Model Platform
    "SystemPrompt": "You are Xiao Zhi, an adult female, a **companion assistant created by ZEGOCLOUD Technology**, knowledgeable in astronomy and geography, smart, wise, enthusiastic, and friendly.\nDialogue requirements: 1. Interact with users according to the character requirements.\n2. Do not exceed 100 words.",
    "Temperature": 1,
    "TopP": 0.7,
    "Params": {
...
Only supports OpenAI Chat Completions API protocol. Please configure according to your actual needs:
...
```json
{
    ...
    "max_tokens": 4096, // Corresponding to LLM.Params.max_tokens parameter
    "messages":[
        {
            "role": "system",
    ...
    // If the LLM.AgentExtraInfo parameter has a configuration value, it will contain agent_extra_info information
    "agent_extra_info":{
...
- Except for the last valid data that can contain `"finish_reason":"stop"`, other data must set `finish_reason` to an empty string or not set the `finish_reason` field, otherwise the AI Agent will not output content.
...
## How to Configure Multimodal LLM - Configure LLM with Text Input and Audio Output <a id="configure-multimodal-llm" />
...
<Note title="Note">
Please specify `DisableTTS:true` in `AdvancedConfig` when calling [CreateAgentInstance](./../api-reference/agent-instance-management/create-agent-instance.mdx) or [CreateDigitalHumanAgentInstance](./../api-reference/agent-instance-management/create-digital-human-agent-instance.mdx) to disable TTS functionality.
</Note>
...
- Select the appropriate LLM vendor, select the development language, fill in the LLM parameters, and click the validate button to check the parameter format.
- The validator checks JSON format, required fields, parameter spelling, case sensitivity, value ranges, etc.
- The "Sample Code" Tab is the sample code for the corresponding language.
- The "Actual Request LLM Parameters" is a preview of the complete request parameters that will be sent to the LLM. You can compare with the official documentation of the selected vendor to ensure the parameters are filled correctly.

<LLMValidator lang="en" />