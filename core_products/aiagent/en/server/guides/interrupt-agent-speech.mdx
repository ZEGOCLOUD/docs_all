...
> In addition to interrupting the agent via the server API, you can also interrupt the agent through the client-side **ZEGO Express SDK** by sending room signaling messages. For details, see the client-side document [Agent Custom Control](/aiagent-android/guides/agent-custom-control).
