...
- You have created a project in the [ZEGOCLOUD Console](https://console.zego.im/) and applied for a valid AppID and AppSign. For details, see [Console - Project Information](https://doc-zh.zego.im/article/12107).
...
[Register AI Agent](./API%20reference/Agent%20Configuration%20Management/Register%20Agent.mdx) is used to set the basic configuration of the AI Agent, including the AI Agent name, LLM, TTS, ASR, and other related configurations. After registration, you can create multiple instances with multiple real users as templates.
...
async registerAgent(agentId: string, agentName: string) {
    ...
    // sendRequest method encapsulates the request URL and public parameters. For details, see: https://doc-zh.zego.im/aiagent-server/api-reference/accessing-server-apis
...
- If the AI agent cannot output text content or voice, please first check if the LLM and TTS parameter configuration is completely correct, or refer to [Get AI Agent Status - Listen for Service-Side Exception Events](./Guides/Get%20AI%20Agent%20status.mdx#Listen%20for%20Service-Side%20Exception%20Events) to determine the specific problem.  
</Warning>

</Step>
<Step title="Create Digital Human Agent Instance">
You can create multiple digital human agent instances [Create Digital Human Agent Instance](./API%20reference/Agent%20Instance%20Management/create-digital-human-agent-instance.mdx) with the registered AI Agent as a template to join different rooms and interact with different users in real time. After creating the digital human agent instance, the digital human agent instance will automatically login the room and push the stream, and will also pull the real user's stream.
...
After [Delete AI Agent Instance](./API%20reference/Agent%20Instance%20Management/Delete%20Agent%20instance.mdx), the AI agent instance will automatically exit the room and stop the push stream. The real user will stop the push stream and exit the room after the client, and a complete interactive session will end.
