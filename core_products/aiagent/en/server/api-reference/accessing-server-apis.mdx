...
|-------------------|----------|----------|----------------------------------------|
| AppId             | Uint32  | Yes      | Application ID, assigned by ZEGOCLOUD. Get it from the [ZEGOCLOUD Admin Console](https://console.zegocloud.com/).      |
| Signature         | String  | Yes      | Signature, please refer to [Signing the requests](#signing-the-requests) for how to generate an API request signature.     |
| SignatureNonce    | String  | Yes      | A 16-character hexadecimal random string (hex encoding of 8-byte random number). Refer to [Signature sample code](#signature-sample-code) for how to generate.                           |
| SignatureVersion  | String  | Yes      | Signature version number, default value: 2.0.           |
| Timestamp         | Number   | Yes      | Current Unix timestamp, in seconds. Refer to [Signature sample code](#signature-sample-code) for how to generate, with a maximum error of 10 minutes. |
