...
- AI agent instance status:
    ...
    - Status that can be monitored via server callbacks: agent interruption events, which can be used to interrupt ongoing tasks such as terminating RAG tasks.
...
Call the Query Agent Instance Status API ([QueryAgentInstanceStatus](./../API%20reference/Agent%20Instance%20Control/Query%20Agent%20instance%20status.mdx)), pass in the corresponding `AgentInstanceId`, and the server will return the current status of the AI agent instance (such as idle, listening, thinking, speaking, etc.).
...
When the AI agent is interrupted, the AI Agent backend will send an interruption event notification (`Event` is `Interrupted`) to the configured address above. Here's an example of the content:
...
|--------|--------|--------------|
| Reason | Number | Interruption reason:<ul><li>1: User is speaking.</li><li>2: You actively [call LLM](./../API%20reference/Agent%20Instance%20Control/Send%20Agent%20instance%20LLM.mdx) on the server.</li><li>3: You actively [call TTS](./../API%20reference/Agent%20Instance%20Control/Send%20Agent%20instance%20TTS.mdx) on the server.</li><li>4: You [interrupt the agent instance](./../API%20reference/Agent%20Instance%20Control/Interrupt%20Agent%20Instance.mdx) on the server.</li></ul> |
</Tab>
...
| LLMTPS | Int | LLM average output speed (tokens/second). The average number of tokens output per second by the Large Language Model. |
| TTSAudioFirstFrameTime | Int | TTS audio first frame average latency (milliseconds). <ul><li>Unidirectional streaming: From when LLM outputs the first token, accumulating tokens to form the first sentence, to when TTS returns the first non-silent audio frame.</li><li>Bidirectional streaming: From when LLM outputs the first token and starts sending to TTS, to when TTS returns the first non-silent audio frame. (Bidirectional streaming sentence formation logic is implemented by the TTS service itself)</li></ul> |
| TotalCost | Int | AI Agent server average total latency (milliseconds): The time from when the AI Agent server pulls the stream and determines the user has finished speaking, to when TTS returns the first frame and starts pushing the stream. All server-generated latency, including at least Automatic Speech Recognition (ASR) latency, Large Language Model (LLM) related latency, Text-to-Speech (TTS) related latency, etc. |