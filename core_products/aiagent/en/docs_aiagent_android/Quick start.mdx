...
- A project has been created in the [ZEGOCLOUD Console](https://console.zegocloud.com/) and a valid AppID and AppSign have been applied for. For more details, please refer to [How to view project info - Admin Console ](https://www.zegocloud.com/docs/admin-console/view-project-information?platform=all&language=all).
...
Server example code. It includes the most basic capabilities such as obtaining ZEGOCLOUD Token, registering an AI agent, creating an AI agent instance, and deleting an AI agent instance.
</Card>
</CardGroup>
...
sequenceDiagram
    ...
    Client->>Business Backend: Register Agent
    ...
    Client->>Business Backend: Create Agent Instance
    Business Backend->>AI Agent Backend: Create Agent Instance
    AI Agent Backend->>AI Agent Backend: Agent logs into the room and streams, pulls user stream
    AI Agent Backend-->>Business Backend: 
    Business Backend-->>Client: 
    
    
    Client->>Client: User pulls Agent stream
    
    Client->>Business Backend: Delete Agent Instance
...
The registration of an AI agent is used to set up the basic configuration of the agent, including configurations related to the agent name, LLM, TTS, ASR, etc. After registration, the agent can be used as a template to create multiple instances for interaction with multiple real users.

AI agents are usually relatively fixed; once the relevant parameters (persona image) of the agent are set, they will not be changed frequently. Therefore, it is recommended to register the agent when initializing the application or during other steps according to the needs of the business process.

<Note title="Note">An AI agent can only be registered once (with the same ID); if registered repeatedly, error code 410001008 will be returned.</Note>

Below is an example of calling the business backend interface to register an AI agent:
...
async registerAgent(agentId: string, agentName: string) {
    ...
    // The sendRequest method encapsulates the Request endpoint and common parameters. For details, see: https://doc-zh.zego.im/aiagent-server/accessing-server-apis
    return this.sendRequest<any>(action, body);
}
```
</CodeGroup>
...
async createAgentInstance(agentId: string, userId: string, rtcInfo: RtcInfo, messages?: any[]) {
    ...
    // The sendRequest method encapsulates the Request endpoint and common parameters. For more details, refer to: https://doc-zh.zego.im/aiagent-server/accessing-server-apis
...
```objectivec Client pulls the AI agent's stream {12}
// Listen for room stream information update status, pull and play the AI agent's stream
...
```
```javascript title="Client pulls the AI agent stream" {9}
      ...
      if (updateType === "ADD" && streamList.length > 0) {
        try {
          for (const stream of streamList) {
            ...
            if (remoteView) {
             // A container with the id 'remoteSteamView' is required on the page to receive the AI agent stream [Reference Documentation]（https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoStreamView）
...
async deleteAgentInstance(agentInstanceId: string) {
    ...
    // The sendRequest method encapsulates the Request endpoint and common parameters. For details, refer to: https://doc-zh.zego.im/aiagent-server/accessing-server-apis
    return this.sendRequest(action, body);
}
```
</CodeGroup>
