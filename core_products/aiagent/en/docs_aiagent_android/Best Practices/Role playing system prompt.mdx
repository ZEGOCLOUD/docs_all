# Character Playing System Prompt

A System Prompt (system prompt word) is information provided to a large language model (LLM) so that the LLM knows what role it is currently playing. System prompts optimized for different scenarios and roles can enable large language models to better achieve the desired effect of the role they are playing.

A well-crafted System Prompt can be used through the following interfaces:
- Passed into the `LLM`-`SystemPrompt` parameter via the [Register Intelligent Agent](/en/aiagent-server/agent-configuration-management/register-agent) interface.
- Passed into the `LLM`-`SystemPrompt` parameter via the [Modify Intelligent Agent](/en/aiagent-server/agent-configuration-management/update-agent) interface.
- Passed into the `LLM`-`SystemPrompt` parameter via the [Create Intelligent Agent Instance](/en/aiagent-server/agent-instance-management/create-agent-instance) interface.
- Passed into the `LLM`-`SystemPrompt` parameter via the [Modify Intelligent Agent Instance](/en/aiagent-server/agent-instance-management/update-agent-instance) interface.
## Template

### Basic Template

The following is a system prompt template that can quickly implement role-playing:
```text
...
When precise control over a character is needed, Markdown syntax can be used to inform the LLM of the priorities and requirements.
...
Below are some System Prompt examples for different character styles that you may refer to:
...
## Best Practices for System Prompts of LLM Vendors
...
- [Volcano Ark (DouBao)](https://www.volcengine.com/docs/82379/1221660#prompt-engineering-introduction)
- [Step Stars](https://platform.stepfun.com/docs/guide/prompt#common-prompt-reference)
- [Tongyi Qwen](https://help.aliyun.com/zh/model-studio/use-cases/prompt-engineering-guide?spm=a2c4g.11186623.help-menu-2400256.d_2_1.5a06b0a83i6mYy&scm=20140722.H_2735998._.OR_help-T_cn~zh-V_1)