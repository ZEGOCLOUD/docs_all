...
export const expressSDKMap = {
  ...
  'MiniProgram': <a href='/real-time-voice-miniprogram/quick-start/integrating-sdk' target='_blank'>ZEGO Express SDK</a>,
}
...
Please refer to [Integrate the SDK > Method 1](https://www.zegocloud.com/docs/voice-call/sdk-integration?platform=flutter&language=dart#2) to integrate the SDK using "pub" dependency. After integrating the SDK, follow these steps to initialize ZegoExpressEngine.
...
Please refer to [Integrate SDK > Method 2](https://www.zegocloud.com/docs/video-call/sdk-integration?platform=web&language=javascript#2_2) to use npm to integrate the ZEGO Express SDK version optimized for AI Agent. After integrating the SDK, initialize ZegoExpressEngine as follows.
...
async function enterRoom() {
  try {
    // Generate RTC Token [Reference Documentation] (/real-time-video-web/communication/using-token-authentication)
    const token = await Api.getToken();
    // Login to room
    await zg.loginRoom(roomId, token, {
      userID: userId,
