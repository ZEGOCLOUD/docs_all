...
- Initial Memory: Whether chat history is needed when creating an instance.
  - Can be linked to [ZIM](https://doc-zh.zego.im/zim-android/introduction/overview) session history as initial memory;
...
| Parameter  | Type   | Required | Description                                                           |
|------------|--------|----------|-----------------------------------------------------------------------|
| RobotId    | String | No       | The `UserInfo.UserId` corresponding to the ZIM [register bot](https://doc-zh.zego.im/zim-server/bot/register-bots#request-parameters) API call. Used to load chat context between users and this ZIM bot, and synchronize messages generated during conversations to ZIM. If empty, the Real-Time AI Agent backend will generate one randomly. |
...
1. Ensure your app has [ZIM](https://doc-zh.zego.im/zim-android/introduction/overview) service enabled
...
<Note title="Note">`ZIM.RobotId` is the `UserInfo.UserId` corresponding to the ZIM [register bot](https://doc-zh.zego.im/zim-server/bot/register-bots#request-parameters) API call.</Note>
