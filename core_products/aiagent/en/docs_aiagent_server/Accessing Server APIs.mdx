...
Developers need to specify the corresponding access address based on the geographical location of their server, and send requests to the ZEGO server.

<Warning title="Warning">

To ensure the quality of your service connection, please prioritize using the domain name of the geographical location where your server is located as the access address to send requests to the ZEGO server.  
</Warning>

ZEGO supports request access from the following geographical regions:
...
All interfaces of the ZEGO server API communicate via HTTPS, providing secure communication services.
...
All request parameters (including common parameters and business parameters) are uniformly placed in the Query, using the GET request method. Special and complex API parameters are placed in the Body, using the POST request method.
</Note>
...
Public request parameters are the parameters required for every interface:
...
To ensure the secure invocation of APIs, the ZEGO server will authenticate each API access request. Developers must include the signature Signature information in the request when calling an API.
...
The Signature verifies the sender's identity of the request through a symmetric encryption method using AppID and ServerSecret. AppID is used to identify the visitor's identity, and ServerSecret is used as the key for encrypting the signature string and for server verification of the signature string. It must be kept strictly confidential to prevent leakage.

AppID and ServerSecret are obtained from the [ZEGO Console](https://console.zego.im). For more details, please refer to the console documentation [Project Information](https://doc-zh.zego.im/article/12107).
...
ZEGO provides signature example codes in various programming languages, and developers can refer to them according to their actual situations.
