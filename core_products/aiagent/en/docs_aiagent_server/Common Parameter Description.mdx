...
| FilterText     | Array of Object | No       | <p>Filters the text within specified punctuation marks from the content returned by LLM before performing speech synthesis. For the structure of this parameter, please refer to [FilterText](#filtertext).</p><Note title="Note"><ul><li>You need to define in [LLM > SystemPrompt](#LLM) which content should be placed within the specified punctuation marks.</li><li>This parameter cannot be updated when [Modifying An Agent Instance](./Agent%20Instance%20Management/Update%20Agent%20instance.mdx).</li></ul></Note> |
