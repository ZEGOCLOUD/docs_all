...
You can set the hot word list through the `HotWord` parameter in the `ASR` section when [registering an AI agent](/aiagent-server/agent-configuration-management/register-agent) or [creating an AI agent instance](/aiagent-server/agent-instance-management/create-agent-instance).
