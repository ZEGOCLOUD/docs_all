...
Please refer to the [Receiving Callback](./../Callbacks/Receiving%20Callback.mdx) documentation to develop callbacks for receiving AI Agent event notifications, and contact ZEGOCLOUD technical support to configure the callback address.

<Note title="Note">

To receive user and agent status callback results, when [creating an agent instance](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx), set the corresponding [CallbackConfig.UserSpeakAction](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx#callbackconfig) and [CallbackConfig.AgentSpeakAction](./../API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx#callbackconfig) parameters to `1`.
...
export class AgentCallbackController {
    ...
    callbackHandler = async (req: Request, res: Response) => {
        ...
        // For commonArgs generation, refer to https://doc-zh.zego.im/article/19456#3
