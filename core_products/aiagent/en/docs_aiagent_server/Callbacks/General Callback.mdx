...
- Request URL: Please contact ZEGO Technical Support for configuration.
- Transfer protocol: HTTPS/HTTP, it is recommended to use HTTPS.

## Callback Parameters

<Note title="Note">ZEGO will continuously optimize and update the relevant callback parameters in future iteration plans (for example: adding new fields or adding parameter values for certain fields). When developers integrate, please avoid hardcoding to ensure compatibility with new versions after updates.</Note>
