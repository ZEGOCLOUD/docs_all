...
To ensure system security, you must authenticate every callback request sent from the ZEGOCLOUD server to your application server. You can verify the callback request by comparing the signature included in the request with the locally calculated signature.
...
|-----------|-------------|
| callbacksecret | Server-side verification key. Generated when registering a project in the [ZEGOCLOUD Console](https://console.zego.im/), and can be viewed in "Console > Project Configuration > Project Information > Configuration Information". |
