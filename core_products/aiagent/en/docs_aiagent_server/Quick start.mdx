...
[Register Agent](./API%20reference/Agent%20Configuration%20Management/Register%20Agent.mdx) is used to set basic agent configurations, including agent name, LLM, TTS, ASR and other related settings. After registration, you can use this agent as a template to create multiple instances for interaction with multiple real users.
...
- If the agent fails to output text or voice, first check if the LLM and TTS parameter configurations are completely correct, or refer to [Get AI Agent Status - Listen for Server Exception Events](./Guides%5CGet%20AI%20Agent%20status.mdx#listen-for-server-exception-events) to identify the specific issue.
</Note>

</Step>
<Step title="Create Agent Instance">
You can use a registered agent as a template to [create multiple agent instances](./API%20reference/Agent%20Instance%20Management/Create%20Agent%20instance.mdx) that join different rooms to interact with different users in real-time. After an agent instance is created, it automatically logs into the room, publishes its stream, and plays the real user's stream.
...
After [deleting an agent instance](./API%20reference/Agent%20Instance%20Management/Delete%20Agent%20instance.mdx), the instance automatically leaves the room and stops publishing its stream. When the real user stops publishing and leaves the room on the client side, one complete interaction session ends.
