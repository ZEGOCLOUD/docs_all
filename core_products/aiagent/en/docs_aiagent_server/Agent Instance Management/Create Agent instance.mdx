...
| Parameter     | Type   | Required | Description                                                                 |
|---------------|--------|----------|-----------------------------------------------------------------------------|
| RobotId       | String | No  | ZIM robot ID, used to load the chat context between the user and the ZIM robot, and synchronize messages generated during the conversation to ZIM. If this parameter is empty, the ZEGOCLOUD AI Agent server will randomly generate one.<Note title="Note">How to register a ZIM robot to get the robot ID, please refer to the In-app Chat docs [Register Bots](https://www.zegocloud.com/docs/zim-server/bot/register-bots)。</Note> |
