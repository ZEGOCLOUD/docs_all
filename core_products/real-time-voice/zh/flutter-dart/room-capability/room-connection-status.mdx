...
用户可通过监听 [onRoomStateChanged](/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngine/onRoomStateChanged.html) 回调实时监控自己在本房间内的连接状态。当用户连接状态发生改变时，SDK 会触发该回调，并在回调中明确当前的连接状态和相关错误码。
...
房间连接状态会互相转化，开发者可通过 reason 判断各种情况，并在必要时结合 errorCode 处理相应的逻辑。reason 为 [ZegoRoomStateChangedReason](/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoRoomStateChangedReason.html) 数据，具体说明如下所列：
...
当 reason 为LoginFailed 时，表示第一次登录房间失败，此时 errorCode 详情可参考 [常见错误码](!Error_Code/Error_Code)。
...
- 由于开发者主动调用后台的 [踢出房间用户](#8784) 接口导致被踢出时，errorCode 为 1002055。
...
如果用户 A 短暂断线，但是在 90 秒内重连成功了，那么用户 B 不会收到 [onRoomUserUpdate](/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngine/onRoomUserUpdate.html) 的回调通知。
</div>


其中：
- T0 = 0s：ClientA 的 SDK 收到客户端发起的 [loginRoom](/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngineRoom/loginRoom.html) 请求。
- T1 ≈ T0 + 120ms：通常在调用 [loginRoom](/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngineRoom/loginRoom.html) 120 毫秒后，客户端可以加入房间。加入房间过程中，ClientA 的客户端会收到 `onRoomStateChanged(roomID, ZegoRoomStateChangedReason.Logining, 0, extendedData)` 和 `onRoomStateChanged(roomID, ZegoRoomStateChangedReason.Logined, 0, extendedData)` 回调，分别通知客户端正在连接房间以及连接房间成功。
