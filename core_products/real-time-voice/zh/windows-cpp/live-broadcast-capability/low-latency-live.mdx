...
设置 [ZegoPlayerConfig](/zh/api?doc=Express_Audio_SDK_API~CPP_windows~struct~zego-express-zego-player-config) 的 “resourceMode” 参数为 “ZEGO_STREAM_RESOURCE_MODE_ONLY_L3”，调用 [startPlayingStream](/zh/api?doc=Express_Audio_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#start-playing-stream) 接口，根据传入的流 ID 参数 “streamID”，拉取远端推送的低延迟直播流，即可实现低延迟直播功能。
...
| 方法 | 描述 |
|-------|--------|
| [startPlayingStream ](/zh/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#start-playing-stream) | 开始拉流 |
