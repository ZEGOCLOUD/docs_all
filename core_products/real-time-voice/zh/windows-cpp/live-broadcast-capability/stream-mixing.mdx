...
可参考 [快速开始 - 实现流程](7633#3_1) 的 “3.1 创建引擎” 和 “3.2 登录房间” 完成房间登录。
...
[ZegoMixerTask](@-ZegoMixerTask) 是 SDK 中定义的混流任务配置对象。
...
完成了 [ZegoMixerTask](@-ZegoMixerTask) 混流任务对象的配置后，调用开始混流的接口开始这个混流任务，并在回调中接收开始混流任务结果。
...
| 方法 | 描述 |
|-------|--------|
| [startMixerTask ](https://doc-zh.zego.im/zh/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#start-mixer-task) | 开始混流任务 |
| [stopMixerTask ](https://doc-zh.zego.im/zh/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#stop-mixer-task) | 停止混流任务 |
