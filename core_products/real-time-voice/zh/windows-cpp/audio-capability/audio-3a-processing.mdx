...
2. 开启回声消除后，开发者可通过调用 [setAECMode\|_blank](@setAECMode) 接口设置回声消除模式。SDK 支持以下三种回声消除模式：
...
3. 开启噪声抑制后，开发者可通过调用 [setANSMode\|_blank](@setANSMode) 接口设置噪声抑制模式，默认值为 “ZEGO_ANS_MODE_MEDIUM”。SDK 支持以下三种噪声抑制模式：
