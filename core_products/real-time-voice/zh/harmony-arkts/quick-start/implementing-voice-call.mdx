...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时音视频功能，详情请参考 [快速开始 - 集成](/real-time-voice-harmonyos/quick-start/integrating-sdk)。
...
调用 [createEngine ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/ets_ohos/classes/_zegoexpressengine_.zegoexpressengine.html#createengine) 接口，将 [前提条件](/real-time-voice-harmonyos/quick-start/implementing-voice-call) 中申请到的 AppID 传入参数 “appID”，创建引擎单例对象。

<Note title="说明">


如果需要将日志信息输出到 DevEco Studio 的 HiLog 窗口，可参考 [常见问题](/real-time-voice-harmonyos/quick-start/implementing-voice-call) 中的“鸿蒙系统如何打印调试日志？”实现。
...
调用登录房间接口之后，您可通过监听 [onRoomStateUpdate ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/ets_ohos/interfaces/_zegoexpresseventhandler_.zegoeventlistener.html#onroomstateupdate) 回调实时监控自己在本房间内的连接状态，详情请参考 [房间内的连接状态变化通知](/real-time-voice-harmonyos/quick-start/implementing-voice-call)。
...
如果您在音视频通话的过程中，遇到相关错误，可查询 [错误码](/real-time-voice-harmonyos/client-sdk/error-code)。
...
详情可参考 [房间连接状态说明 ](/real-time-voice-harmonyos/room/room-connection-status)。
