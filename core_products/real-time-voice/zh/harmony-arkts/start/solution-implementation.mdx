...
调用 [createEngine ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/ets_ohos/classes/_zegoexpressengine_.zegoexpressengine.html#createengine) 接口，将 [前提条件](!Integration/Solution_Implementation#2) 中申请到的 AppID 传入参数 “appID”，创建引擎单例对象。
...
调用登录房间接口之后，您可通过监听 [onRoomStateUpdate ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/ets_ohos/interfaces/_zegoexpresseventhandler_.zegoeventlistener.html#onroomstateupdate) 回调实时监控自己在本房间内的连接状态，详情请参考 [房间内的连接状态变化通知](!Integration/Solution_Implementation#4_1)。
