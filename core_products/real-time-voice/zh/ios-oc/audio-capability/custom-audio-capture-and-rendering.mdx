...
请参考 [快速开始 - 实现流程](7628#2_1) 的 “2.1 创建引擎”。
...
请参考 [快速开始 - 实现流程](7628#2_2) 的 “2.2 登录房间”、“2.3 推流” 和 “2.4 拉流”。
...
打开音频渲染设备，使用 [fetchCustomAudioRenderPCMData](@fetchCustomAudioRenderPCMData) 从引擎中获取要渲染的数据，拿到音频数据后再通过渲染设备播放。
...
| 方法 | 描述 |
|-------|--------|
| [enableCustomAudioIO ](/zh/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#enable-custom-audio-io-config) | 开启自定义音频采集渲染 |
| [sendCustomAudioCaptureAACData ](/zh/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#send-custom-audio-capture-aac-data-data-length-config-length-timestamp-param) | 向 SDK 发送自定义采集的 AAC 数据 |
| [sendCustomAudioCapturePCMData ](/zh/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#send-custom-audio-capture-pcm-data-data-length-param) | 向 SDK 发送自定义采集的 PCM 数据 |
| [fetchCustomAudioRenderPCMData ](/zh/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#fetch-custom-audio-render-pcm-data-data-length-param) | 从 SDK 获取要自定义渲染的 PCM 数据 |
