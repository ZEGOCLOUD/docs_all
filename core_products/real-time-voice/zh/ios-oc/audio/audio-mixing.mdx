...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时语音功能，详情请参考 [快速开始 - 集成](/real-time-voice-ios/quick-start/integrating-sdk) 和 [快速开始 - 实现流程](/real-time-voice-ios/quick-start/implementing-voice-call)。
...
“expectedDataLength” 为本次回调期望数据的长度，收到回调后应构造一个 [ZegoAudioMixingData](@-ZegoAudioMixingData) 对象，设置其 “param” 参数为待混音数据的真实采样率和声道数，将其 “audioData” 参数设置为期望长度的待混音数据。如果填写的数据不足 SDK 提供的长度值，则可设置 “audioData” 参数置空（丢弃末尾数据）；或者最后的尾音不足 SDK 提供的长度值时，可以用静音数据补齐长度。
...
| 方法 | 描述 |
|-------|--------|
| [enableAudioMixing ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~ObjectiveC~class~zego-express-engine) | 开启/关闭混音 |
| [setAudioMixingHandler ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~ObjectiveC~class~zego-express-engine) | 设置混音回调 |
| [setAudioMixingVolume ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~ObjectiveC~class~zego-express-engine) | 设置混音音量 |
| [muteLocalAudioMixing ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~ObjectiveC~class~zego-express-engine) | 静音混音本地播放 |
