...
请参考 [快速开始 - 实现流程](7628#2_3) 的  “2.3 推流”。
...
- 接口原型
    ...
    /// 当调用 [enablePublishDirectToCDN] 接口设置为 YES 将流直推到 CDN 时，再调用本接口将无效。
    ///
...
当音视频流转推 CDN 成功后，开发者希望用户从 CDN 进行拉流时，需要使用传入 URL 的自定义拉流方式进行拉流，而不能通过流 ID 进行拉流，具体操作请参考 [推拉流进阶 - 通过 URL 拉流](#3640)。
...
| [startPlayingStream](@startPlayingStream) | 自定义拉流 |
