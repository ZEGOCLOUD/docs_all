...
已到 [ZEGO 控制台](https://console-express.zego.im/account/login) 创建项目，申请有效的 AppID 和 AppSign，以及获取到接入服务器地址 Server，详情请参考 [控制台 - 项目管理](/console/project-info)。
...
通过 [Web 端调试示例 ](https://zegoim.github.io/express-demo-web/src/Examples/QuickStart/VideoTalk/index.html?lang=zh)（只支持在测试环境下使用），输入相同的 AppID 和 roomID，加入同一房间与真机设备互通。当成功开始音视频通话时，可以同时听到本端和远端音频，看到本端和远端视频画面。
