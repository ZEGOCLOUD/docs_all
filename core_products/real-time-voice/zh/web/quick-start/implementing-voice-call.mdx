...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成](/real-time-voice-web/quick-start/integrating-sdk)。
...
您可以监听 [roomStateChanged](@roomStateChanged) 回调实时监控自己与房间的连接状态。只有当房间状态是连接成功时，才能进行推流、拉流等操作。如果登录房间失败，可参考 [错误码](/real-time-voice-web/client-sdk/error-code) 进行操作。
