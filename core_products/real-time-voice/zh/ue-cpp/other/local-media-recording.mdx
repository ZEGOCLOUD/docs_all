...
1. 调用系统默认的播放器进行播放。
2. 调用 ZegoExpressEngine 中的 ZegoMediaPlayer 组件进行播放，详情可参考 [媒体播放器](/real-time-voice-ue/other/media-player)。
...
请参考 [原始音频数据获取](/real-time-voice-ue/audio/get-audio-raw-data)。
...
开发者自行集成 ZEGO 的服务端录制插件，并部署在自己的服务器上，然后开发者通过访问自己的服务器接口开始录制，具体说明请参考 [本地服务端录制](https://doc-zh.zego.im/local-recording-linux-cpp/overview)。
...
请参考 [本地服务端录制](https://doc-zh.zego.im/local-recording-linux-cpp/integration/sdk-integration)。
