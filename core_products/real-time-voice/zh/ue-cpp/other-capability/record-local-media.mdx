...
1. 调用系统默认的播放器进行播放。
2. 调用 ZegoExpressEngine 中的 ZegoMediaPlayer 组件进行播放，详情可参考 [媒体播放器](!Other_Functions/MediaPlayer)。
...
请参考 [开始 CDN 录制](/zh/real-time-voice-video-server/media-file/start-cdnrecrod)。
