...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时音视频功能，详情请参考 [快速开始 - 集成](/real-time-voice-ue/quick-start/integrating-sdk)。
...
调用 [createEngine ](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~CPP~class~zego-express-zego-express-sdk) 接口，将申请到到 AppID 和 AppSign 传入参数 “appID” 和 “appSign”。

根据 App 实际的音视频业务选择一个合适的场景，请参考 [场景化音视频配置](/real-time-voice-ue/quick-start/scenario-based-audio-video-configuration) 文档，把选择好的场景枚举传入参数 "scenario"。
...
传入用户 ID 参数 “userID” 和 “userName” 创建 ZegoUser 用户对象后，调用 [loginRoom](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~CPP~class~zego-express-i-zego-express-engine)，传入房间 ID 参数 “roomID” 和用户参数 “user”，登录房间。如果房间不存在，调用该接口时会创建并登录此房间。
...
- [onRoomStreamUpdate](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~CPP_windows~class~zego-express-i-zego-event-handler#on-room-stream-update)：流状态更新回调。登录房间后，当房间内有用户新推送或删除音频流时，SDK 会通过该回调通知。
...
调用 [startPublishingStream](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~CPP~class~zego-express-i-zego-express-engine)，传入流 ID 参数“streamID”，向远端用户发送本端的音频流。
...
调用 [startPlayingStream](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~CPP~class~zego-express-i-zego-express-engine) 接口，根据传入的流 ID 参数 “streamID”，拉取远端推送的音频流。
...
调用 [stopPublishingStream ](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~CPP~class~zego-express-i-zego-express-engine) 停止向远端用户发送本端的音频流。
...
调用 [stopPlayingStream ](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~CPP~class~zego-express-i-zego-express-engine) 停止拉取远端推送的音频流。
...
调用 [logoutRoom ](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~CPP~class~zego-express-i-zego-express-engine) 接口退出房间。
...
调用 [destoryEngine ](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~CPP~class~zego-express-zego-express-sdk) 接口销毁引擎，用于释放 SDK 使用的资源。
