...
1. 调用 [getVideoDeviceList](@getVideoDeviceList) 获取可用的视频设备列表。
2. 调用 [useVideoDevice](@useVideoDevice) 接口指定主路流的视频设备，调用 [startPublishingStream](@startPublishingStream) 接口推主路流。
3. 调用 [useVideoDevice](@useVideoDevice) 接口指定第二路流的视频设备，调用 [startPublishingStream](@startPublishingStream) 接口推第二路流。
