...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时音视频功能，详情请参考 [快速开始 - 集成](/real-time-voice-electron/quick-start/integrating-sdk)。
...
更多信息请参考文档 [实时音视频 SDK 与实时语音 SDK 的差异](/real-time-voice-electron/introduction/difference-between-audio-and-video-sdk)。

若您对 Express Electron SDK 包体大小有较高要求，可联系技术支持进行定制裁包，关于 SDK 包大小数据请参考 [概述](/real-time-voice-electron/introduction/overview) 文档。


**3. 设置回调**


关于回调的注册和取消，请参考 [设置回调](/real-time-voice-electron/quick-start/setting-callback)
...
- [onRoomStateUpdate](/real-time-voice-electron/quick-start/setting-callback)：房间状态更新回调。登录房间后，当房间连接状态发生变更（如出现房间断开，登录认证失败等情况），SDK 会通过该回调通知。
- [onRoomUserUpdate](/real-time-voice-electron/quick-start/setting-callback)：用户状态更新回调。登录房间后，当房间内有用户新增或删除时，SDK 会通过该回调通知。

    只有调用 [loginRoom](@loginRoom) 接口登录房间时传入 [ZegoRoomConfig](@-ZegoRoomConfig) 配置，且 “isUserStatusNotify” 参数取值为 “true” 时，用户才能收到 [onRoomUserUpdate](/real-time-voice-electron/quick-start/setting-callback) 回调。

- [onRoomStreamUpdate](/real-time-voice-electron/quick-start/setting-callback)：流状态更新回调。登录房间后，当房间内有用户新推送或删除音视频流时，SDK 会通过该回调通知。
...
[onPublisherStateUpdate](/real-time-voice-electron/quick-start/setting-callback)：推流状态更新回调。调用推流接口成功后，当推流状态发生变更（如出现网络中断导致推流异常等情况），SDK 在重试推流的同时，会通过该回调通知。
...
[onPlayerStateUpdate](/real-time-voice-electron/quick-start/setting-callback)：拉流状态更新回调。调用拉流接口成功后，当拉流状态发生变更（如出现网络中断导致推流异常等情况），SDK 在重试拉流的同时，会通过该回调通知。
