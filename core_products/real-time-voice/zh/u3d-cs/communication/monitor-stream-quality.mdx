...
请参考 [下载示例源码](/real-time-voice-u3d/quick-start/run-example-code) 获取源码。
...
- 已在项目中集成 ZEGO Express SDK，并实现了基本的音视频推拉流功能，详情请参考 [快速开始 - 集成](/real-time-voice-u3d/quick-start/integrating-sdk) 和 [快速开始 - 实现流程](/real-time-voice-u3d/quick-start/implementing-voice-call)。
...
[OnNetworkQuality](@OnNetworkQuality) 回调的逻辑为：
...
[OnNetworkQuality](@OnNetworkQuality) 不适用于使用 CDN 进行直播的场景，您可以参考 [进阶质量报告 - 推流质量报告](/real-time-voice-u3d/communication/monitor-stream-quality) 监测 CDN 的推流质量。
...
您可以通过注册 [OnPublisherQualityUpdate](@OnPublisherQualityUpdate) 接收推流质量回调，推流成功后每隔三秒会收到此回调。可根据 quality(ZegoPublishStreamQuality) 参数实时了解推送的音视频流的健康情况。
...
您可以通过注册 [OnPlayerQualityUpdate](@OnPlayerQualityUpdate) 接收拉流质量回调，拉流成功后每隔三秒会收到此回调。开发者可根据 quality(ZegoPlayStreamQuality) 参数实时了解拉取的音视频流的健康情况。
