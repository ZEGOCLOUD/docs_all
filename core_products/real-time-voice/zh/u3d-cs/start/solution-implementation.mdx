...
![](/Pics/QuickStart/quickstart_uml_audio_u3d.png)
...
调用 [CreateEngine\|_blank](@CreateEngine) 接口，将申请到到 AppID 和 AppSign 传入参数 “appId” 和 “appSign”，创建引擎单例对象。
...
若您对 Express Unity SDK 包体大小有较高要求，可联系技术支持进行定制裁包，关于 SDK 包大小数据请参考 [概述\|_blank](!Overview/Overview) 文档。
...
创建 ZegoUser 用户对象，设置用户信息 “userID” 和 “userName” 后，调用 [LoginRoom\|_blank](@LoginRoom)，传入房间 ID 参数 “roomId” 和用户参数 “user”，登录房间。如果房间不存在，调用该接口时会创建并登录此房间。
...
事件回调皆为 [ZegoExpressEngine\|_blank](@-ZegoExpressEngine) 的委托，开发者直接将自己实现的回调处理函数赋值给 [ZegoExpressEngine\|_blank](@-ZegoExpressEngine) 对应的委托，即可收到回调并进行处理。
...
远端用户推送的 “streamID” 可以从 [ZegoExpressEngine\|_blank](@-ZegoExpressEngine) 委托中的 [OnRoomStreamUpdate\|_blank](@OnRoomStreamUpdate) 回调中获得。
</div>
