...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时音视频功能，详情请参考 [快速开始 - 集成](/real-time-voice-u3d/quick-start/integrating-sdk)。
...
更多信息请参考文档 [实时音视频 SDK 与实时语音 SDK 的差异](/real-time-voice-u3d/introduction/difference-between-audio-and-video-sdk)。

若您对 Express Unity SDK 包体大小有较高要求，可联系技术支持进行定制裁包，关于 SDK 包大小数据请参考 [概述](/real-time-voice-u3d/introduction/overview) 文档。
...
事件回调皆为 [ZegoExpressEngine](@-ZegoExpressEngine) 的委托，开发者直接将自己实现的回调处理函数赋值给 [ZegoExpressEngine](@-ZegoExpressEngine) 对应的委托，即可收到回调并进行处理。
...
远端用户推送的 “streamID” 可以从 [ZegoExpressEngine](@-ZegoExpressEngine) 委托中的 [OnRoomStreamUpdate](@OnRoomStreamUpdate) 回调中获得。
