...
**2. 从此版本开始 Express 实时语音 Audio SDK 的 Gradle 仓库由 https://jitpack.io 变更为 https://storage.zego.im/maven，依赖的项目名称由 `com.github.zegolibrary:express-audio:x.y.z` 变更为 `im.zego:express-audio:x.y.z`，详情请查看 [集成 SDK](/real-time-video-android-java/start/integration) 文档。**
...
虚拟场景中，由于每个场景的地图大小、音视频互动玩法及规模不同，需要针对每个场景进行自定义配置。3.1.0 版本后，“万人范围音视频”及“多人实时状态同步”支持通过 SDK 接口，并使用模板 ID 指定场景。模板 ID 对应的配置项只能通过服务器 API 进行配置，详情请参考 [服务端 API - 场景模版配置](/zh/real-time-voice-video-server/scenario-service/scenario-template-configure)。
...
相关 API 请参考 [批量禁止 RTC 推流](/zh/real-time-voice-video-server/media/batch-prohibit-rtc-push-streaming) / [批量恢复 RTC 推流](/zh/real-time-voice-video-server/media/batch-restore-rtc-push-streaming)
