...
为方便体验，ZEGO 提供了一个 [Web 端调试示例](https://zegoim.github.io/express-demo-web/src/Examples/QuickStart/VideoTalk/index.html?lang=zh)，在该页面下，输入相同的 AppID、RoomID，输入不同的 UserID、以及对应的 [Token](/console/development-assistance/temporary-token)，即可加入同一房间与真机设备互通。当成功开始音频通话时，可以听到远端的音频，看到远端的视频画面。
