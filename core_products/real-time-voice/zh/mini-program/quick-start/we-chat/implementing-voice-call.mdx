...
- ZEGO 提供了 `<zego-pusher>` 和 `<zego-player>`，可以帮助开发者更快地实现实时音频。用户也可以选择使用微信原生小程序推拉流组件 `<live-pusher>` 和 `<live-player>` 进行推拉流，参考 [使用微信原生组件实现通话](/real-time-voice-miniprogram/other/implementing-voice-call-with-wechat-native-components)。
...
- 推流：把采集阶段封包好的音频数据流推送到 ZEGO 实时音频云的过程。
- 拉流：从 ZEGO 实时音频云将已有音频数据流拉取播放的过程。
...
await zegoPusher.startPush(zg, "streamID_xxx",undefined,{enableCamera: false});
```

<Warning title="注意">

- 您可通过监听 [publisherStateUpdate](@publisherStateUpdate) 回调知晓推流是否成功，具体请参考 [常见通知回调](#callback) 中的“用户推送音频流的状态通知”。
...
用户先获取 `<zego-player>` 组件实例对象，再调用 该对象的 `startPlay` 方法传入 SDK 实例和 `streamID` 参数即可发起推流。
...
如果需要了解 API 返回的错误码，请参考 [常见错误码](/real-time-voice-miniprogram/client-sdk/error-code)。
