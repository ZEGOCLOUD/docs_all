...
在实现基本的实时音频功能之前，请确保：
- 已在项目中集成 ZEGO Express SDK，详情请参考 [微信小程序 - 集成 SDK](/real-time-voice-miniprogram/quick-start/we-chat/integrating-sdk)。
...
两者均能实现一样的功能，但是推荐客户参考组件化的示例代码进行开发。因为小程序组件化后，使得代码更加简洁，具体请参考 [将自己的音视频流推送到 ZEGO 音视频云](/real-time-voice-miniprogram/quick-start/we-chat/implementing-voice-call) 中的 **“创建对应业务场景的 WXML”**。
...
您可通过监听 [roomStateUpdate ](@roomStateUpdate) 回调实时监控自己在本房间内的连接状态，具体请参考 [常见通知回调 ](/real-time-voice-miniprogram/quick-start/we-chat/implementing-voice-call) 中的“我在房间内的连接状态变化通知”。
...
为避免错过任何通知，您需要在登录房间前先设置所有的监听回调（如房间状态、用户状态、流状态、推拉流状态等），具体请参考 [常见通知回调 ](/real-time-voice-miniprogram/quick-start/we-chat/implementing-voice-call)。
...
用户调用 SDK 的 [createPusher](@createPusher) 接口创建推流实例，并通过调用实例对象上的 [start](@start) 接口，传入流 ID 参数 “streamID”。您可通过监听 [publisherStateUpdate ](@publisherStateUpdate) 回调知晓推流是否成功，具体请参考 [常见通知回调 ](/real-time-voice-miniprogram/quick-start/we-chat/implementing-voice-call) 中的“用户推送音频流的状态通知”。
...
用户先调用 [getPlayerInstance](@getPlayerInstance) 接口，根据传入的流 ID 参数 “streamID”，获取 streamID 对应的拉流实例，然后通过调用拉流实例对象的 [play](@play) 接口开始拉流。您可通过监听 [playerStateUpdate ](@playerStateUpdate) 回调知晓是否成功拉取音频，具体请参考 [常见通知回调 ](/real-time-voice-miniprogram/quick-start/we-chat/implementing-voice-call) 中的“用户拉取音频流的状态通知”。

远端用户推送的 “streamID” 可以从 [roomStreamUpdate](@roomStreamUpdate) 回调中获得，具体回调设置请参考 [常见通知回调 ](/real-time-voice-miniprogram/quick-start/we-chat/implementing-voice-call) 中的“房间内流状态变更的通知”。
...
如果用户在音频通话的过程中，遇到相关错误，可查询 [错误码](/real-time-voice-miniprogram/client-sdk/error-code)。
