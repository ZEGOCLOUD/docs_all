...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [支付宝小程序 - 集成 SDK](/real-time-voice-miniprogram/quick-start/alipay/integrating-sdk)。
...
您可通过监听 [roomStateUpdate ](@roomStateUpdate) 回调实时监控自己在本房间内的连接状态，具体请参考 [常见通知回调 ](/real-time-voice-miniprogram/quick-start/alipay/implementing-voice-call) 中的“我在房间内的连接状态变化通知”。
...
为避免错过任何通知，您需要在登录房间前先设置所有的监听回调（如房间状态、用户状态、流状态、推拉流状态等），具体请参考 [常见通知回调 ](/real-time-voice-miniprogram/quick-start/alipay/implementing-voice-call)。
...
您可通过监听 [playerStateUpdate ](@playerStateUpdate) 回调知晓是否成功拉取音视频，具体请参考 [常见通知回调](/real-time-voice-miniprogram/quick-start/alipay/implementing-voice-call) 中的“用户拉取音视频流的状态通知”。

远端用户推送的 “streamID” 可以从 [roomStreamUpdate](@roomStreamUpdate) 回调中获得，具体回调设置请参考 [常见通知回调](/real-time-voice-miniprogram/quick-start/alipay/implementing-voice-call) 中的“房间内流状态变更的通知”。
