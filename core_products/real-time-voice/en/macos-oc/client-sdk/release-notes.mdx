...
2. When PC captures system sound card, it can be used as an independent source for streaming
...
4. The desktop supports setting pictures as custom video collection sources.
...
1. Fixed the issue of macOS throwing a status error when disconnecting the currently used media device.
...
2. Support for calling back vocal-aligned PCM data from media player.
...
4. iOS supports Mac Catalyst architecture (arm64 + x86_64).
...
2. Fix the problem that the media player's playback volume or publish volume obtained for the first time is 0.
...
2. Fix the problem that the parameters obtained by the Preset construction method of the ZegoReverbParam are incorrect.
    
3. Fix the problem that the value obtained by the `currentProgress` getter of `ZegoMediaPlayer` is incorrect.
