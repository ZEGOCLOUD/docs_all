...
7. The media player supports chunked file storage to avoid excessive memory allocation at once
...
8. The pull stream switching function supports forced switching mode
...
9. Support switching to a non-system default network
...
10. Added a non-main thread (UI thread) audio and video first frame time callback interface, which can more accurately count the first frame time when the main thread is blocked
...
11. Added a callback for the data metrics of each link on the pull stream
...
12. The SDK player supports HLS protocol playback and supports resolution adaptation
...
13. Support sending cloud proxy configuration through cloud control
...
14. The push and pull stream functions add support for national encryption algorithms
...
1. Fixed the issue that the onAudioRouteChange callback is not triggered when switching between speaker and earpiece using AVRoutePickerView in the call volume mode.
    
2. Fixed the issue that the wireless microphone device can not collect sound correctly in the media mode.
...
1. Fixed the issue of iPad not connecting to Bluetooth headphones.
...
2. For KTV scenarios, optimize the alignment effect of Bluetooth earphones on vocals and accompaniment, enhancing the K-song experience.
    
3. Adapt to new rules for publishing Apple apps.
...
1. After fixing the initialization of the SDK, call the synchronous interface immediately. Occasionally, the UI thread may get stuck.
    
2. Fix the problem of abnormal time consumption of interface calls caused by not deinitializing the SDK after long-term use.
...
2. Supports throwing Audio Session exception callbacks, such as device without permission or preempted.
    
    For related API, please refer to [onLocalDeviceExceptionOccurred](@onLocalDeviceExceptionOccurred)
    
3. Direct-publish CDN supports updating the CDN address during publishing.
    
    For related API, please refer to [enablePublishDirectToCdn](@enablePublishDirectToCdn)
    
4. Supports  AI-balanced noise reduction mode.
...
2. Starting from this version, the CocoaPods project name of Express Voice Call SDK is changed from `ZegoExpressEngine/Audio` to `ZegoExpressAudio`, see [Integrate the SDK](#4121) documentation for more details.
...
2. Fixed an issue where the iOS SDK depended on CocoaPods could not package the Release simulator architecture.
