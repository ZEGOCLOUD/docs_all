...
- A project has been created in the [ZEGOCLOUD Console](https://console.zegocloud.com), and a valid AppID and AppSign have been applied. 
...
You can monitor and handle the event callbacks you are interested in by implementing a class that conforms to the [ZegoEventHandler](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~protocol~zego-event-handler) interface and implementing the required callback methods. Then pass the instantiated object (e.g., self) as the eventHandler parameter to [createEngineWithProfile ](@createEngineWithProfile) or to [setEventHandler](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~class~zego-express-engine#set-event-handler) to register the callback.
...
After passing the user ID parameter "userID" to create a [ZegoUser](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~class~zego-user#includes) user object, call the [loginRoom ](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~class~zego-express-engine#login-room-user) interface, pass in the room ID parameter "roomID" and user parameter "user", and log in to the room. If the room does not exist, calling this interface will create and log in to this room.
...
- The constructor method of ZegoUser [userWithUserID ](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~class~zego-user#user-with-user-id) will set "userName" to be the same as the passed parameter "userID". "userID" cannot be "nil", otherwise logging in to the room will fail.
...
Call the [startPublishingStream ](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~class~zego-express-engine#start-publishing-stream) interface, pass in the stream ID parameter "streamID", and send the local audio stream to remote users.
...
If you need to know about Express's microphone/audio/speaker related interfaces, please refer to [FAQ - How to implement switching camera/video/microphone/audio/speaker?](https://www.zegocloud.com/docs/faq/device-switching).
...
Call the [startPlayingStream ](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~class~zego-express-engine#start-playing-stream) interface, based on the passed stream ID parameter "streamID", to play remote published audio streams.

The "streamID" published by remote users can be obtained from the [onRoomStreamUpdate](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~protocol~zego-event-handler#on-room-stream-update-stream-list-extended-data-room-id) callback in [ZegoEventHandler](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC~protocol~zego-event-handler).
...
Call the [stopPublishingStream ](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~class~zego-express-engine#stop-publishing-stream) interface to stop sending local audio streams to remote users.
...
Call the [stopPlayingStream ](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~class~zego-express-engine#stop-playing-stream) interface to stop playing remote published audio streams.
...
Call the [logoutRoom ](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~class~zego-express-engine#logout-room) interface to log out of the room.
...
Call the [destroyEngine ](https://www.zegocloud.com/article/api?doc=Express_Audio_SDK_API~ObjectiveC_ios~class~zego-express-engine#destroy-engine) interface to destroy the engine, which is used to release resources used by the SDK.
