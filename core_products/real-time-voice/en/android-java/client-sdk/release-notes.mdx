...
5. Semi-automatic mixing supports mixing alignment capability
    
    For related API, please refer to [ZegoAutoMixerTask](@ZegoAutoMixerTask)
    
6. The media player supports chunked file storage to avoid excessive memory allocation at once
...
7. The pull stream switching function supports forced switching mode
...
8. Added OpenGL 3 related interfaces
...
2. Optimize the SDK initialization to join the room time performance
...
3. Optimize the ear-return delay performance
...
1. Fixed the issue of audio still playing through the device's speakers after occasionally plugging in headphones.
    
2. Fixed the issue of crashing caused by Bluetooth-related permissions.
