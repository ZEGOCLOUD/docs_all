...
For SEI-related concepts and principles, please refer to [How to Understand and Use Supplemental Enhancement Information](https://www.zegocloud.com/docs/faq/media-sei-usage) in "FAQ - Message Issues".
...
1. Call the [loginRoom](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#login-room) interface to log into the room.
2. Call the [startPublishingStream](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#start-publishing-stream) interface to publish the stream.
3. After successful publishing, call the [sendSEI](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#send-sei) interface to send SEI information.

### Playing Side Receiving SEI Message Call Flow

1. Create an [IZegoEventHandler](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-event-handler) object and rewrite the [onPlayerRecvSEI](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-event-handler##on-player-recv-sei) method to receive SEI information.
2. Call the [loginRoom](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#login-room) interface to log into the room.
3. Call the [startPlayingStream](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#start-playing-stream) interface to play the stream.
4. After successful playing, the [onPlayerRecvSEI](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-event-handler#on-player-recv-sei) callback will be triggered when receiving the SEI information sent from the publishing side.

### (Optional) Set SEI Type

<Accordion title="Set SEI Type" defaultOpen="false">

Since the SDK uses ZEGO's self-defined SEI (nalu type = 6, payload type = 243) type for packaging by default, and this type is not specified in the SEI standard, it does not conflict with SEI in video encoders or video files. However, when developers need to use third-party decoders for decoding (such as FFmpeg), it will result in the inability to extract the correct SEI. In this case, before publishing the stream [startPublishingStream](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#start-publishing-stream), you need to call the [setSEIConfig](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#set-sei-config) interface to change the SDK's sent SEI type to use UserUnregister SEI (nalu type = 6, payload type = 5) type packaging.
...
| Method | Description |
|-------|--------|
| [sendSEI](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#send-sei) | Send SEI information |
| [onPlayerRecvSEI](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-event-handler#on-player-recv-sei) | Receive SEI information |
| [setSEIConfig](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#set-sei-config) | Set SEI type |