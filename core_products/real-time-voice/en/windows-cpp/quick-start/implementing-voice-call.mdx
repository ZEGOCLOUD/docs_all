...
Call the [createEngine](@createEngine) interface, passing the obtained AppID and AppSign into the "appID" and "appSign" parameters.
...
The SDK also supports Token authentication. If you need to upgrade the authentication method, please refer to [How to Upgrade Express from AppSign Authentication to Token Authentication?](/faq/express-token-upgrade).
...
You can implement the required callback methods by implementing a class that inherits from the [IZegoEventHandler](@-IZegoEventHandler) interface, to listen for and handle the event callbacks you care about. Then, pass the instantiated object as the `eventHandler` parameter to the [createEngine](@createEngine) method or pass it to the [setEventHandler](@setEventHandler) method to register the callback.
...
- [onRoomStreamUpdate](@onRoomStreamUpdate): Stream status update callback. After logging in to the room, when users in the room add or remove audio/video streams, the SDK will notify through this callback.
...
Call the [startPublishingStream](@startPublishingStream) interface, passing in the stream ID parameter "streamID" to send your local audio stream to remote users.
...
Call the [startPlayingStream](@startPlayingStream) interface, passing in the stream ID parameter "streamID" to play the remote audio stream pushed by the remote user.
...
Call the [stopPublishingStream](@stopPublishingStream) interface to stop sending your local audio stream to remote users.
...
Call the [stopPlayingStream](@stopPlayingStream) interface to stop pulling the remote audio stream pushed by the remote user.
...
Call the [destroyEngine](@destroyEngine) interface to destroy the engine and release the resources used by the SDK.
