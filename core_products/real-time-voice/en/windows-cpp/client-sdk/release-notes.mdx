...
1. Pull stream supports local recording of remote audio and video streams
    
    The pull stream supports recording remote audio and video streams and saving them as local audio and video files. 
...
For related API, please refer to [startRecordingRemoteData](@startRecordingRemoteData), [stopRecordingRemoteData](@stopRecordingRemoteData), [setPlayStreamDecodeFrameMode](@setPlayStreamDecodeFrameMode), [onRemoteDataRecordStateUpdate](@onRemoteDataRecordStateUpdate), [onRemoteDataRecordProgressUpdate](@onRemoteDataRecordProgressUpdate)
...
6. The desktop supports setting pictures as custom video collection sources.
...
1. Fix the problem of over-the-ear and double-talk suppression in windows system
...
1. Semi-automatic mixing supports mixing alignment capability
    
    For related API, please refer to [ZegoAutoMixerTask](@ZegoAutoMixerTask)
    
2. The media player supports chunked file storage to avoid excessive memory allocation at once
...
3. The pull stream switching function supports forced switching mode
...
4. Added a non-main thread (UI thread) audio and video first frame time callback interface, which can more accurately count the first frame time when the main thread is blocked
...
5. Added a callback for the data metrics of each link on the pull stream
...
6. The SDK player supports HLS protocol playback and supports resolution adaptation
...
7. Support sending cloud proxy configuration through cloud control
...
8. The push and pull stream functions add support for national encryption algorithms
...
9. Support mixing third-party audio data
...
1. Fixed the issue of occasional playback crashes after enabling local caching in the media player.
...
1. Fix the issue of the monitoring module crashing and causing the main thread to freeze during de-initialization
    
2. Fix the issue of possible crashes when starting to score copyrighted music
    
3. Adjust the lifecycle to fix known crash issues
...
2. Supports determining in advance whether the device can run the AI voice changing function.
...
2. Fixed the problem of using the AI voice changing function to occasionally cause the audio and video to be out of sync, and causing words to be swallowed instantly when starting and stopping.
...
3. Optimize multi-device login logic.
...
3. Windows supports the hard coding of the specified graphics card, and the texture input and output of hard coding and hard decoding.
...
3. Scenario-based audio and video configuration adds [StandardVoiceCall] standard voice call scenario.
...
3. Added switch interface for acquiring GPS information
...
2. Optimize the audio and video experience in extremely weak networks
...
2. Supports querying the features of the current SDK.
...
1. Local media recording supports file paths in utf8 format.
...
7. The server supports ServerSecret smooth migration capability.
...
8. Server Token capability upgrade.
...
9. Support batch prohibition of RTC streaming and recovery of RTC streaming.
...
4. Support to obtain the audio device information currently in use.
...
1. Support to obtain local and remote uplink and downlink network quality.
...
1. Support mute or unmute audio devices.
...
2. Supports monitoring of audio device volume.
...
3. Support setting the collection volume of the system sound card.
...
4. Support setting and obtaining audio quality configuration.
...
5. Support for setting the factors of concern that trigger flow control.
...
6. Media player adds precise search function.
...
7. Support to mute or resume all streams' audio data being playing
...
3. Added [setAllPlayStreamVolume] interface to set the volume of all streaming sounds.
...
4. Added [startNetworkProbe] interface for starting network probes. Added [stopNetworkProbe] interface to stop network probing.
...
2. Added desktop sound card capture function `enableMixSystemPlayout` (only supports macOS/Windows)
...
2. Fixed the issue that `setEventHandler` cannot set to empty.
