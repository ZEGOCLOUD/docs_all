...
<Note title="Note">
If you need to understand Express's microphone/audio/speaker related interfaces, please refer to [FAQ - How to implement switching camera/video screen/microphone/audio/speaker?](/faq/device-switching).
</Note>
...
Call the [startPlayingStream](@startPlayingStream) interface, based on the passed stream ID parameter "streamID", to pull the remote published audio and video stream.
...
[onPlayerStateUpdate](@onPlayerStateUpdate): Playing status update callback. After the playing interface is called successfully, when the playing status changes (such as network interruption causing publishing exceptions, etc.), the SDK will notify through this callback while retrying to play.
...
For how to solve this problem, please refer to [FAQ](/faq/macos-monterey-device-access-issue).
