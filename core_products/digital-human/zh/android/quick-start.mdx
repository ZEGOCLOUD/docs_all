...
本文将介绍如何通过 ZEGO 相关 SDK 快速实现数字人视频流播报。

## 功能简介

数字人视频流播报是指通过数字人后台 API 创建数字人视频流任务并驱动数字人播报后，客户端通过 ZEGO Express SDK 拉取视频流进行渲染展示的过程。

<Note title="说明">
- 在移动端（Android/iOS），通过数字人 SDK 进行画面渲染。
- 在 Web 端，通过 ZEGO Express SDK 进行画面渲染。
</Note>

## 前提条件

在实现数字人功能之前，请确保：
- 已部署集成了数字人后台 API 的业务后台。详情请参考业务后台 [快速跑通示例代码](/aigc-digital-human-server/quick-start/run-example-code)，并确保业务后台 API 可正常访问。
- 已联系 ZEGO 技术支持开通数字人服务和相关接口的权限。
:::if{props.platform="undefined|iOS"}
- 已根据 [集成 SDK](./integrating-sdk.mdx) 完成 SDK 的集成和权限配置。
:::
...
- **RTC 房间**：是指 ZEGO 提供的音视频空间服务，用于组织用户群，同一房间内的用户可以互相收发实时音视频及消息。详情请参考 [登录实时音视频房间](/real-time-video-android-java/quick-start/implementing-video-call#createroom)。
- **Base64Config**：启动数字人 SDK 所需的核心配置字符串。由业务后台在调用[创建数字人视频流任务](/aigc-digital-human-server/streaming-apis/digital-human-streaming/create-digital-human-stream-task) API 时获得。
- **SEI**：视频补充增强信息。数字人 SDK 依赖视频流中携带的 SEI 数据来实现数字人动作、口型与音频的精准同步。

## 示例代码

我们提供了实现数字人视频流播报基本流程的客户端和业务后台示例代码，您可以参考其实现逻辑来实现自己的业务逻辑：
...
包含初始化、自定义渲染处理、调用业务后台创建数字人视频流任务、驱动数字人等核心逻辑。
</Card>
:::
...
:::if{props.platform="undefined|iOS"}
1. **初始化 SDK**：分别初始化 ZEGO Express SDK 和数字人 SDK。
2. **创建数字人视频流任务**：向业务后台发起请求，调用数字人后台 API 创建数字人视频流任务并获取 `Base64Config`。
3. **登录房间并拉流**：开启**自定义视频渲染**模式，登录 RTC 房间并拉取数字人视频流。
4. **启动数字人渲染引擎**：调用数字人 SDK 的 `start` 方法，传入 `Base64Config` 启动数字人客户端渲染引擎。
5. **驱动数字人播报**：业务后台使用文本或音频数据驱动数字人讲话。
6. **停止任务**：停止拉流、登出房间并销毁相关 SDK 实例。
:::
:::if{props.platform="Web"}
1. **初始化 SDK**：初始化 ZEGO Express SDK。
2. **创建数字人视频流任务**：向业务后台发起请求，调用数字人后台 API 创建数字人视频流任务。
3. **登录房间并拉流**：登录 RTC 房间并拉取数字人视频流。
4. **驱动数字人播报**：业务后台使用文本或音频数据驱动数字人讲话。
5. **停止任务**：停止拉流、登出房间并销毁相关 SDK 实例。
:::
...
sequenceDiagram
    ...
    participant PaaS as 数字人/ 实时音视频 后台
    ...
    Note over Client, DH_SDK: 2. 预加载资源 (可选)
    Client->>Server: 获取 Token：GET /api/ZegoToken
    Server-->>Client: 返回 Token
    Client->>DH_SDK: ZegoDigitalHumanResource.preload(auth, id, callback)

    Note over Client, PaaS: 3. 创建流任务
    Client->>Server: 创建数字人任务：POST /api/CreateDigitalHumanStreamTask(RoomID, StreamID, OutputMode=2)
    Server->>PaaS: CreateDigitalHumanStreamTask
    PaaS-->>Server: 返回 Base64Config, TaskID
    Server-->>Client: 返回 Base64Config

    Note over Client, PaaS: 4. 登录房间并拉流
    ...
    Note over Client, DH_SDK: 5. 启动数字人
    ...
    Note over Client, PaaS: 6. 驱动数字人播报
    Client->>Server: 发起播报指令 (Text / Audio / WS)
    Server->>PaaS: 驱动数字人【DriveByText(Text)、DriveByAudio(AudioUrl)】
    Server->>PaaS: 驱动数字人【WebSocket(音频数据)】
    PaaS-->>DH_SDK: (通过流内信令驱动口型与动作)
    PaaS-->>Client: (通过音视频流输出画面与声音)

    Note over Client, PaaS: 7. 停止任务
    Client->>Server: 停止任务：POST /api/StopDigitalHumanStreamTask(TaskID)
...
sequenceDiagram
    ...
    Note over Client, DH_SDK: 2. 预加载资源 (可选)
    Client->>Server: 获取数字人信息 (DigitalHumanID)
    Server-->>Client: 返回 AppID, Token
...
sequenceDiagram
    ...
    participant Express as Express SDK
    participant Server as 业务服务端
    participant PaaS as 数字人/ 实时音视频 服务端
...
为了提升数字人启动速度，减少拉流后的等待时间，建议在创建任务前，先预加载数字人资源。




<Note title="说明">
- 需要先调用业务后台获取 Token 用于预加载数字人资源。可参考 [GET /api/ZegoToken](https://github.com/ZEGOCLOUD/digital_human_paas_quick_start/blob/90d895e93d16bf8bddf7effe8034ed19d7a724ad/Server/internal/handler/token.go#L16) 实现。
- 通常数字人 ID 一旦选定就不会改变。我们这里假设已经调用过数字人后台接口[查询数字人形象列表](/aigc-digital-human-server/streaming-apis/digital-human-management/get-digital-human-list)选定了某个数字人 ID。
</Note>
...
业务客户端需要向业务服务端发起请求，由业务服务端调用数字人后台 API [创建数字人视频流任务](/aigc-digital-human-server/streaming-apis/digital-human-streaming/create-digital-human-stream-task)。在该步骤中，**业务服务端**会获得包含 `RoomID`、`Token`、`StreamID` 以及 `Base64Config` 在内的关键信息。
...
| **StreamID** | 数字人推流的唯一标识。客户端根据此 ID 识别并拉取对应的数字人视频流。 |
...
任务创建成功后，需要通过业务服务端调用 PaaS API 驱动数字人播报。支持以下驱动方式：

|驱动方式|业务后台 API|数字人后台 API|说明|
|:---|:---|:---|:---|
|文本驱动|POST /api/DriveByText|DriveByText(Text)|[文本驱动数字人](/aigc-digital-human-server/streaming-apis/digital-human-streaming/drive-by-text)|客户端发送文本，由业务后台调用数字人后台API驱动数字人播报。|
|音频驱动|POST /api/DriveByAudio|DriveByAudio(AudioUrl)|[音频驱动数字人](/aigc-digital-human-server/streaming-apis/digital-human-streaming/drive-by-audio)|客户端发送音频文件URL，由业务后台调用数字人后台API驱动数字人播报。|
|WebSocket 驱动|POST /api/DriveByWsStream|DriveByWsStream(音频数据)|[WebSocket 音频流驱动数字人](/aigc-digital-human-server/streaming-apis/digital-human-streaming/drive-by-ws-stream)|客户端通过WebSocket实时传输音频数据，由业务后台调用数字人后台API驱动数字人播报。|
...
:::
- [服务端 API - 创建数字人视频流任务](/aigc-digital-human-server/streaming-apis/digital-human-streaming/create-digital-human-stream-task)
