...
- **Base64Config**：启动数字人 SDK 所需的核心配置字符串。由业务后台在调用[创建数字人流任务](/aigc-digital-human-server/server-apis/digital-human-streaming/create-digital-human-stream-task) API 时获得。
...
sequenceDiagram
    ...
    participant PaaS as 数字人 PaaS / RTC
    participant DH_SDK as 数字人 SDK

    Note over Client, DH_SDK: 1. 初始化 SDK
    Client->>Client: ZegoExpressEngine.createEngine(profile)
    Client->>DH_SDK: ZegoDigitalHuman.create(context)

    Note over Client, DH_SDK: 2. 创建流任务
    ...
    Note over Client, DH_SDK: 3. 登录房间并拉流
    Client->>Client: enableCustomVideoRender(true, config)
    Client->>PaaS: loginRoom(RoomID, Token)
    Client->>PaaS: startPlayingStream(StreamID)
    PaaS-->>Client: 视频帧 / SEI 回调
    ...
    loop 持续渲染
        Client->>DH_SDK: onRemoteVideoFrameRawData(data, dataLength, param, streamID)
        Client->>DH_SDK: onPlayerSyncRecvSEI(streamID, data)
    end

    Note over Client, DH_SDK: 5. 驱动数字人播报
    Client->>Server: 发起播报指令 (Text / Audio)
    Server->>PaaS: DriveDigitalHuman (TaskID, Text / AudioUrl)
    PaaS-->>DH_SDK: (通过流内信令驱动口型与动作)
    PaaS-->>Client: (通过音视频流输出画面与声音)

    Note over Client, DH_SDK: 6. 停止任务
    Client->>Server: 停止任务
    Server->>PaaS: StopDigitalHumanStreamTask (TaskID)
    Client->>Client: logoutRoom() / digitalMobile.stop()
```
:::
...
sequenceDiagram
    ...
    Note over Client, DH_SDK: 1. 初始化 SDK
    Client->>Client: createEngineWithProfile(profile)
    ...
    Note over Client, DH_SDK: 3. 登录房间并拉流
    Client->>Client: enableCustomVideoRender:YES config:renderConfig
    Client->>PaaS: loginRoom:RoomID config:roomConfig
    Client->>PaaS: startPlayingStream:StreamID
    PaaS-->>Client: 视频帧 / SEI 回调
    ...
    loop 持续渲染
        Client->>DH_SDK: onRemoteVideoFrameRawData:data dataLength:dataLength param:param streamID:streamID
...
sequenceDiagram
    ...
    Note over Client, Express: 2. 创建流任务
    Client->>Server: 请求启动数字人 (RoomID, StreamID)
    ...
    Note over Client, Express: 4. 驱动数字人播报
    Client->>Server: 发起播报指令 (Text / Audio)
    Server->>PaaS: DriveDigitalHuman (TaskID, Text / AudioUrl)
    PaaS-->>Express: (音视频流内容更新)

    Note over Client, Express: 5. 停止任务
    Client->>Server: 停止任务
    Server->>PaaS: StopDigitalHumanStreamTask (TaskID)
    Client->>Express: stopPlayingStream() / logoutRoom()
```
:::
...
业务客户端需要向业务服务端发起请求，由业务服务端调用数字人 PaaS API [创建数字人流任务](/aigc-digital-human-server/server-apis/digital-human-streaming/create-digital-human-stream-task)。在该步骤中，**业务服务端**会获得包含 `RoomID`、`Token`、`StreamID` 以及 `Base64Config` 在内的关键信息。
...
任务启动后，需要通过业务服务端调用 PaaS API 驱动数字人播报。支持文本驱动、音频驱动等方式。
