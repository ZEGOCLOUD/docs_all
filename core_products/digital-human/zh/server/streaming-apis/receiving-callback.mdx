...
通过本回调,您可以监听数字人视频流任务相关的信息,包含视频流任务状态、视频流驱动任务状态。对于一次数字人驱动任务,您也可以结合驱动任务状态判断数字人说话开始和结束。
...
| Status      | Number       | 与 DriveId 关联的数字人视频流驱动任务状态:<ul><li>1:排队中。</li><li>2:驱动中。</li><li>3:驱动失败。</li><li>4:驱动结束。</li></ul> |
...
如果您的业务需要在服务端感知数字人说话状态,可直接监听 `EventType = 4` 的驱动任务回调,并按如下方式映射:

- `Detail.Status = 2`:数字人开始说话
- `Detail.Status = 4`:数字人说话结束

这种方式适合在服务端统一汇总状态后,再通过 WebSocket、IM 或自定义信令转发给客户端。
