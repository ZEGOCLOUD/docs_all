...
通过本接口，您可以使数字人播报指定的输入文本。如果您没有接入任何的 TTS 厂商，您可以直接通过传入文本信息来驱动数字人，即构支持使用 火山引擎（豆包）、MiniMax 厂商的公共音色来驱动数字人。
...
| 参数               | 类型   | 是否必填 | 描述             |
| ----------------|------|------|-------------------------|
| TimbreId | String | 是    | 音色 ID。通过 [获取音色列表](../timbre-management/get-timbre-list.mdx) 接口的响应参数获取。 |
