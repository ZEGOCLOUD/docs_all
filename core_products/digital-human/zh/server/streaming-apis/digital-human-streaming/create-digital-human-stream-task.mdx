...
| RTCConfig      | Object | 是    | RTC（[实时音视频](/real-time-video-android-java/introduction/overview)） 相关配置。 本参数结构，请参考 [RTCConfig](#rtcconfig)。           |
...
| ExtraConfig | Object | 否    | 额外配置。 本参数结构，请参考 [ExtraConfig](#extraconfig)。     |
...
| StreamId      | String | 是    | 数字人往 RTC 房间推流的流 ID，长度在 128 字节以内，仅支持数字，英文字符 和 ‘-’，’_’。<Note title="注意">请注意，StreamId 必须是 AppID 全局内唯一，用户自定义传入。</Note>    |

### ExtraConfig
| 参数               | 类型   | 是否必填 | 描述             |
|------------------|------|------|-------------------------|
| OutputMode       | Number | 否    | 服务端推流输出模式。 <ul><li>1：Web 模式。使用 Web 端拉流时使用该模式。</li><li>2：Mobile 模式。使用 iOS/ Android 端拉流时使用该模式。需要配合 SDK 使用，性能更好。</li></ul>如果不填，取值为 1。 |
