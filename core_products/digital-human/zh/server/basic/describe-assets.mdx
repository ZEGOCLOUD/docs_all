...
调用 [查询音色列表](./../server-apis/digital-human-management/get-timbre-list.mdx) 接口，传入所获取的 `DigitalHumanId`，查询与数字人形象关联的音色信息列表，从中选择所需音色 ID（`TimbreId`），以便后续用于 [合成实时流式数字人视频](./digital-human-livestreaming.mdx)。
