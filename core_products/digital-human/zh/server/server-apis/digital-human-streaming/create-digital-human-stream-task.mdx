...
| VideoConfig      | Object | 否    | 视频相关配置。 本参数结构，请参考 [VideoConfig](#videoconfig)。           |
| Assets      | Array of Object | 否    | 素材相关配置，可通过此配置设置背景图片或挂件等。 本参数结构，请参考 [Asset](#asset)。           |
...
| 参数     | 类型     | 是否必填 | 描述                         |
| ------ | ------ | ---- | -------------------------- |
| Top    | Number | 否    | 数字人/资源左上角的纵坐标，单位为像素。        |
| Left   | Number | 否    | 数字人/资源左上角的横坐标，单位为像素。        |
| Width  | Number | 否    | 数字人/资源宽度，单位为像素。               |
| Height | Number | 否    | 数字人/资源高度，单位为像素。               |
...
| 参数      | 类型     | 是否必填 | 描述                                                        |
| ------- | ------ | ---- | --------------------------------------------------------- |
| Width   | Number | 否    | 视频流的宽度，单位为像素，最大 1920。且 `Width × Height` 不得超过 1920 × 1080。 |
| Height  | Number | 否    | 视频流的高度，单位为像素，最大 1920。且 `Width × Height` 不得超过 1920 × 1080。 |
