# 获取数字人驱动任务状态

---

##  描述

通过本接口，您可以获取数字人实时视频流驱动任务的状态。

import PostPrototype from '/snippets/common/zh/server/post-prototype.mdx'

<PostPrototype link="https://aigc-digitalhuman-api.zegotech.cn?Action=GetDriveStatus" />

## 请求参数

以下请求参数列表仅列出了接口请求参数和部分公共参数，完整公共参数列表请参考 [调用方式 - 公共请求参数](./../Accessing%20Server%20APIs.mdx#公共请求参数)。

| 参数               | 类型   | 是否必选 | 描述             |
|------------------|------|------|-------------------------|
| TaskId      | String | 否    | 数字人实时视频流任务 ID，如果仅填入此字段，表示拉取所有的驱动任务列表。 通过 [创建数字人实时视频流任务](./Create%20Digital%20Human%20Stream%20Task.mdx) 接口的响应参数获取。    |
| DriveId      | String | 否    | 数字人驱动任务 ID。通过 [文本驱动数字人](./Drive%20By%20Text.mdx) 或其他驱动接口的响应参数获取。    |

## 请求示例

- 请求地址 URL：

    ```https
    https://aigc-digitalhuman-api.zegotech.cn?Action=GetDriveStatus
    &<公共请求参数>
    ```

- 请求消息体：

    ```json
    {
        "DriveId": "1d4e048a-7520-4b7b-b762-5d8266bb8b7f"
    }
    ```

## 响应参数

| 参数 | 类型 | 描述 |
|------|------|------|
| Code | Number | 返回码。0 表示成功，其他值表示失败。如需了解错误码及响应处理建议，请参考 [返回码](./../Return%20codes.mdx)。 |
| Message | String | 请求结果的说明信息。 |
| RequestId | String | 请求 ID。 |
| Data | Object | 返回数据。 |
| └DriveList | Array of Object | 数字人驱动任务列表。 |
| &nbsp;&nbsp;└DriveId      | String       | 数字人驱动任务 ID。 |
| &nbsp;&nbsp;└Status      | Number       | 数字人驱动任务状态：<br/>1：排队中。<br/>2：播报中。<br/>3：播报失败。<br/>4：播报结束。 |


## 响应示例

```json
{
    "Code": 0,
    "Message": "success",
    "RequestId": "831234c7-8385-4051-8d3f-d491fe89cc2f",
    "Data": {
        "DriveList": [
            {
                "DriveId": "1d4e048a-7520-4b7b-b762-5d8266bb8b7f",
                "Status": 1
            }
        ]
    }
}
```