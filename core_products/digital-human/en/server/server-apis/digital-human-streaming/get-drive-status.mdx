...
| 参数               | 类型   | 是否必填 | 描述             |
|------------------|------|------|-------------------------|
| TaskId      | String | 否    | 数字人视频流任务 ID，如果仅填入此字段，表示拉取此视频流任务的所有驱动任务状态。 通过 [创建数字人视频流任务](./create-digital-human-stream-task.mdx) 接口的响应参数获取。    |
| DriveId      | String | 否    | 数字人视频流驱动任务 ID。您可通过 [音频驱动数字人](./drive-by-audio.mdx)、[文本驱动数字人](./drive-by-text.mdx) 和 [WebSocket 驱动数字人](./drive-by-ws-stream.mdx) 接口的响应参数获取。    |
