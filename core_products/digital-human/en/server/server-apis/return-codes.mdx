...
After calling the Digital Human API, the server will return a response result with a Code field, which can be used to determine whether the request was successful.
After calling the Digital Human API, the server will return a response result with a Code field, which can be used to determine whether the request was successful.
...
| 400000002 | Digital Human API platform service not enabled. | Please contact ZEGOCLOUD technical support to enable permissions. |
