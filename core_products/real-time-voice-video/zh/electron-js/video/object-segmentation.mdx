...
sequenceDiagram
    ...
    拉流端->>ZEGO Express SDK: 创建引擎 <br/> createEngine
    ...
    ZEGO Express SDK-->>拉流端: 拉流 <br/> startPlayingStream
    推流端->>ZEGO Express SDK: 关闭主体分割 <br/> (可选) enableVideoObjectSegmentation
...
初始化和登录房间的具体流程，请参考实现视频通话文档中的 “[创建引擎](/real-time-video-electron-js/quick-start/implementing-video-call)”及“[登录房间](/real-time-video-electron-js/quick-start/implementing-video-call)”。

### 监听主体分割状态回调

调用 [onVideoObjectSegmentationStateChanged](/real-time-video-electron-js/quick-start/setting-callback) 接口，监听主体分割状态回调。

<Warning title="注意">


主体分割的状态回调依赖于开启预览或者推流，即如需监听 [onVideoObjectSegmentationStateChanged](/real-time-video-electron-js/quick-start/setting-callback) 回调，需要调用预览 [startPreview](@startPreview) 或推流 [startPublishingStream](@startPublishingStream)。
...
开发者如需更新主体分割类型、背景处理类型，需要修改 [ZegoObjectSegmentationConfig](@-ZegoObjectSegmentationConfig) 的配置，并再次调用 [enableVideoObjectSegmentation](@enableVideoObjectSegmentation) 接口开启主体分割，即可更新主体分割效果；更新结果将通过 [onVideoObjectSegmentationStateChanged](/real-time-video-electron-js/quick-start/setting-callback) 回调通知给开发者。
