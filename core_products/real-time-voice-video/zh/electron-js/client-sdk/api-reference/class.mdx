...
| toUserList | [ZegoUser](./struct#zegouser)[] | 信令的接收者列表。是否必填：是。取值范围：用户列表或者 [nullptr]。注意事项：为 [nullptr] 时 SDK 回向房间内所有用户发送自定义信令。 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| visible | boolean | 是否显示光标。true 为显示光标，false 不显示光标，默认 false。 |
