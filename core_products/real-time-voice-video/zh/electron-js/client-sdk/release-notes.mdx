...
1. Windows 新增在屏幕采集时选择是否采集音频

    在屏幕采集时，可以设置是否采集音频以满足不同场景下的需求。

    相关 API 请参考 [enableAudioCaptureWithConfig](@enableAudioCaptureWithConfig)

**改进优化**

1. 更新集成 Express Native SDK 至 3.24.1 版本
...
1. Windows 新增在窗口采集时选择是否采集音频
    ...
    相关 API 请参考 [enableAudioCapture](@enableAudioCapture)
...
1. 修复媒体播放器 [loadResource](@loadResource) 时展示首页的问题
2. 修复拉流时可能出现花屏的问题
...
3. 媒体播放器支持在本地缓存网络资源
    ...
    相关 API 请参考 [enableLocalCache](@enableLocalCache)、[onMediaPlayerLocalCache](/real-time-video-electron-js/quick-start/setting-callback)
...
6. 媒体播放器支持回调视频分辨率改变事件
    ...
    相关 API 请参考 [onMediaPlayerVideoSizeChanged](/real-time-video-electron-js/quick-start/setting-callback)
...
3. 在断网导致的推拉流重试状态中，支持回调本地网络质量
    ...
    相关 API 请参考 [onNetworkQuality](/real-time-video-electron-js/quick-start/setting-callback)
...
4. 修复 [onNetworkSpeedTestQualityUpdate](/real-time-video-electron-js/quick-start/setting-callback) 回调中网络测速质量参数 quality 接收不到数据的问题
...
2. 请在创建引擎之前，配置地理围栏信息。
    ...
    将音视频及信令数据访问限定在某一区域，用以满足地区数据隐私安全相关法规，即限定用户访问某一特定区域的音视频服务，详情请参考 [地理围栏](/real-time-video-electron-js/communication/geofencing)。
...
3. 新增音视频推流的首帧回调
    ...
    在进行音视频推流时，通过 [onPublisherSendAudioFirstFrame](/real-time-video-electron-js/quick-start/setting-callback)、[onPublisherSendVideoFirstFrame](/real-time-video-electron-js/quick-start/setting-callback) 回调，监听“首帧音频”或“首帧视频”的发布时机。该功能可用于统计音视频推流的耗时、或更新 UI 表现等。
...
6. 媒体播放器支持播放音乐时获取声浪和频谱
    ...
    创建媒体播放器后，调用 [enableSoundLevelMonitor](@enableSoundLevelMonitor) 接口可以开启声浪监听，开启后可以通过 [onMediaPlayerSoundLevelUpdate](/real-time-video-electron-js/quick-start/setting-callback)  回调监听声浪的变化。

    创建媒体播放器后，调用 [enableFrequencySpectrumMonitor](@enableFrequencySpectrumMonitor) 接口可以开启频谱监听，开启后可以通过 [onMediaPlayerFrequencySpectrumUpdate](/real-time-video-electron-js/quick-start/setting-callback) 回调监听频谱的变化。
...
1. 支持多源采集能力
    ...
    多源采集能力对屏幕共享、混音等常见能力的实现路径，进行缩短优化及归一化设计，从 3.4.2 版本后，您可以不用再通过自定义采集实现上述复杂的能力，详情请参考 [多源采集文档](/real-time-video-electron-js/communication/multi-source-capture)。
...
12. 新增设备音量变更、语音状态变化事件的回调接口
    ...
    - [onAudioDeviceVolumeChanged](/real-time-video-electron-js/quick-start/setting-callback)：接收音频设备音量变更事件的回调。调用 [startAudioDeviceVolumeMonitor](@startAudioDeviceVolumeMonitor) 开启音频设备的音量监控后，如果音量发生变化，会触发此回调。

    - [onAudioVADStateUpdate](/real-time-video-electron-js/quick-start/setting-callback)：检测音频数据的稳态语音状态的回调。调用 [startAudioVADStableStateMonitor](@startAudioVADStableStateMonitor) 启动语音状态检测，且处于正在推流的状态或预览预览状态，会触发此回调。
...
本次更新包含不兼容改动，详情请参考 [3.0.0 及以上版本升级指南](/real-time-video-electron-js/client-sdk/upgrade-guide/upgrade-to-v3)。
...
1. 废弃了 [ZegoScenario] 的三种旧版本场景
    废弃 [ZegoScenario] 场景枚举中的 [General]，[Communication]， [Live] 三种场景，详情请参考 [场景化音视频配置](/real-time-video-electron-js/quick-start/scenario-based-audio-video-configuration) 文档。

2. 删除了 [setDebugVerbose]、[setPlayStreamVideoLayer]、[loginMultiRoom] 等接口，详情请参考 [3.0.0 及以上版本升级指南](/real-time-video-electron-js/client-sdk/upgrade-guide/upgrade-to-v3)。
...
1. Express-Video SDK 修复了 [onAudioDeviceStateChanged](/real-time-video-electron-js/quick-start/setting-callback) 回调接口参数错误的问题。
...
5. 新增开启人声检测功能以及人声部分的声浪回调
    ...
    相关 API 请参考 [startSoundLevelMonitorWithConfig](@startSoundLevelMonitorWithConfig), [onCapturedSoundLevelInfoUpdate](/real-time-video-electron-js/quick-start/setting-callback), [onRemoteSoundLevelInfoUpdate](/real-time-video-electron-js/quick-start/setting-callback)
...
7. 新增 Token 过期管理机制
    ...
    相关 API 请参考 [renewToken](@renewToken), [onRoomTokenWillExpire](/real-time-video-electron-js/quick-start/setting-callback)

8. 新增本地设备异常回调

    通过 [onLocalDeviceExceptionOccurred] 回调可以设置要检测的设备类型，如摄像头、扬声器、麦克风等，开发者可以根据不同设备类型的错误回调进行相应的处理。

    相关 API 请参考 [onLocalDeviceExceptionOccurred](/real-time-video-electron-js/quick-start/setting-callback)
...
1. 去测试环境
    ...
    为了降低开发者对环境的理解成本，ZEGO 已统一环境概念，从该版本开始，废弃了测试环境，统一使用正式环境。在 0.25.3 版本之前已接入过 SDK 的开发者，可以参考 [测试环境废弃说明](https://doc-zh.zego.im/real-time-voice-android/introduction/abandon-test-environment) 进行 SDK 升级及代码调整。
