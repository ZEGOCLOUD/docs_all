...
请参考 [下载示例源码](/real-time-video-electron-js/integration/download-demo) 获取源码。
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/subject_segmentation/subject_segmentation_electron.png" /></Frame>
...
调用 [onVideoObjectSegmentationStateChanged](!setting_callback#onVideoObjectSegmentationStateChanged) 接口，监听主体分割状态回调。

<Warning title="注意">


主体分割的状态回调依赖于开启预览或者推流，即如需监听 [onVideoObjectSegmentationStateChanged](!setting_callback#onVideoObjectSegmentationStateChanged) 回调，需要调用预览 [startPreview](@startPreview) 或推流 [startPublishingStream](@startPublishingStream)。
...
开发者如需更新主体分割类型、背景处理类型，需要修改 [ZegoObjectSegmentationConfig](@-ZegoObjectSegmentationConfig) 的配置，并再次调用 [enableVideoObjectSegmentation](@enableVideoObjectSegmentation) 接口开启主体分割，即可更新主体分割效果；更新结果将通过 [onVideoObjectSegmentationStateChanged](!setting_callback#onVideoObjectSegmentationStateChanged) 回调通知给开发者。 
...
调用 [enableVideoObjectSegmentation](@enableVideoObjectSegmentation) 接口，开启主体分割，并设置背景处理类型为“图片”。
...
通过 [startPlayingStream](@startPlayingStream) 接口，开始拉流。
