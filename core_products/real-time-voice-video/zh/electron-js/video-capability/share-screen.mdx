...
- 已在项目中集成 ZEGO Express Electron SDK，并实现了基本的音视频推拉流功能，详情请参考 [快速开始 - 集成\|_blank](!ExpressVideoSDK-Integration/SDK_Integration) 和 [快速开始 - 实现视频通话\|_blank](!ExpressVideoSDK-Integration/Solution_Implementation)。
...
调用 [setExcludeWindowList\|_blank](@setExcludeWindowList) 接口可以过滤掉共享的屏幕中指定窗口画面，只适用于共享整个屏幕时设置。
...
通过监听 [onExceptionOccurred\|_blank](!setting_callback#onExceptionOccurred) 回调，接收采集异常相关通知。
