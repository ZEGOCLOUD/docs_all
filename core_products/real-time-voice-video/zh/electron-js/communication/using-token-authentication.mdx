...
在 Token 过期前 30 秒，SDK 会通过 [onRoomTokenWillExpire](!setting_callback#onRoomTokenWillExpire) 回调发出通知；在 Token 过期后再进行登录房间操作时会通过 `onDebugError` 或 `onRoomStateChanged` 收到 `1002078` （Token 过期）错误码。
