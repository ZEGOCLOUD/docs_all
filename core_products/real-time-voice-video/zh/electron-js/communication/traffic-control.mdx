...
SDK 会根据 [创建引擎](/real-time-video-electron-js/quick-start/implementing-video-call) 时，开发者选择的场景 [`scenario`](/real-time-video-electron-js/quick-start/scenario-based-audio-video-configuration)，自动设置适合该场景的流量控制属性。例如，秀场直播场景需要保证视频流畅性，推荐选择自适应（降低）视频分辨率；教育场景需要保证清晰度，推荐选择自适应（降低）视频帧率。
