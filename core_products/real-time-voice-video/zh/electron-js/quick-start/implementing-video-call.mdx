...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时音视频功能，详情请参考 [快速开始 - 集成](/real-time-video-electron-js/quick-start/integrating-sdk)。
...
使用 [createEngine](@createEngine) 方法，将申请到的 AppID 和 AppSign 传入参数 “appID” 和 “appSign”。
...
关于回调的注册和取消，请参考 [设置回调](/real-time-video-electron-js/quick-start/setting-callback)
...
调用 [loginRoom ](@loginRoom) 接口，传入房间 ID 参数 “roomID” 和用户参数 “user”，登录房间。如果房间不存在，调用该接口时会创建并登录此房间。
...
- [onRoomStateUpdate](/real-time-video-electron-js/quick-start/setting-callback)：房间状态更新回调。登录房间后，当房间连接状态发生变更（如出现房间断开，登录认证失败等情况），SDK 会通过该回调通知。
- [onRoomUserUpdate](/real-time-video-electron-js/quick-start/setting-callback)：用户状态更新回调。登录房间后，当房间内有用户新增或删除时，SDK 会通过该回调通知。

    只有调用 [loginRoom](@loginRoom) 接口登录房间时传入 [ZegoRoomConfig](@-ZegoRoomConfig) 配置，且 “isUserStatusNotify” 参数取值为 “true” 时，用户才能收到 [onRoomUserUpdate](/real-time-video-electron-js/quick-start/setting-callback) 回调。


- [onRoomStreamUpdate](/real-time-video-electron-js/quick-start/setting-callback)：流状态更新回调。登录房间后，当房间内有用户新推送或删除音视频流时，SDK 会通过该回调通知。
...
[onPublisherStateUpdate](/real-time-video-electron-js/quick-start/setting-callback)：推流状态更新回调。调用推流接口成功后，当推流状态发生变更（如出现网络中断导致推流异常等情况），SDK 在重试推流的同时，会通过该回调通知。
...
[onPlayerStateUpdate](/real-time-video-electron-js/quick-start/setting-callback)：拉流状态更新回调。调用拉流接口成功后，当拉流状态发生变更（如出现网络中断导致推流异常等情况），SDK 在重试拉流的同时，会通过该回调通知。
...
3. **macOS Monterey(12.2.1) 及以上版本运行 electron 应用导致摄像头、麦克风等设备不能使用或者 crash？**
    ...
    如何解决此问题，详情请参考 [FAQ](https://doc-zh.zego.im/faq/macos-monterey-device-access-issue)。
