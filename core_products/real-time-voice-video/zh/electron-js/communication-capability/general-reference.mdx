...
- 已在项目中集成 ZEGO Express SDK（2.17.3 及以上版本），并实现了基本的音视频推拉流功能，请参考 [快速开始 - 集成\|_blank](!Integration/SDK_Integration) 和 [快速开始 - 实现流程\|_blank](!Integration/Solution_Implementation)。
...
在 Token 过期前 30 秒，SDK 会通过 [onRoomTokenWillExpire\|_blank](!setting_callback#onRoomTokenWillExpire) 回调发出通知。

收到该回调后，开发者需要从自己的服务端获取新的有效 Token，并调用 SDK 提供的 [renewToken\|_blank](@renewToken) 接口更新 Token。

若您接入的是 **2.17.3** 及以上版本的 ZEGO Express SDK，在 Token 过期后，未调用 [renewToken\|_blank](@renewToken) 接口更新 Token，则当权限过期时：
  - 已登录的用户不会被踢出房间。
...
| [onRoomTokenWillExpire\|_blank](!setting_callback#onRoomTokenWillExpire) | Token 过期回调 |
