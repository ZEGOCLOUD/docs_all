...
- 已在项目中集成 ZEGO Express SDK（2.17.3 及以上版本），并实现了基本的音视频推拉流功能，请参考 [快速开始 - 集成](!Integration/SDK_Integration) 和 [快速开始 - 实现流程](!Integration/Solution_Implementation)。
...
在 Token 过期前 30 秒，SDK 会通过 [onRoomTokenWillExpire](!setting_callback#onRoomTokenWillExpire) 回调发出通知。
...
| [onRoomTokenWillExpire](!setting_callback#onRoomTokenWillExpire) | Token 过期回调 |
