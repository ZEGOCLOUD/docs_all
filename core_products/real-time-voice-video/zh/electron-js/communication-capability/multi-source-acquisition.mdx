...
相关源码请查看官网示例源码，详情请参考 [跑通示例源码](!DownloadDemo/DownloadDemo) 获取源码。
...
- 如果需要使用屏幕共享作为视频采集源，需设置视频采集源类型为 `ScreenCapture`，并开启屏幕共享向 SDK 推屏幕共享视频数据，详情请参考 [屏幕共享](!ExpressVideoSDK-Other_Functions/Share_Screen) 。
...
登录房间及推流流程，请参考实现视频通话的 [登录房间](!Integration/Solution_Implementation#loginRoom) 及 [推流](!Integration/Solution_Implementation#startPublishingStream) 。
