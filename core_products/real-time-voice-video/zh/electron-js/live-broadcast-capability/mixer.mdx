...
[ZegoMixerTask \|_blank](@-ZegoMixerTask) 是 ZegoExpressEngine SDK 中定义的混流任务配置对象，其中包含输入流、输出流等信息。
...
创建 [ZegoMixerVideoConfig \|blank](@-ZegoMixerVideoConfig) 对象，设置混流视频相关配置，包含视频分辨率、帧率和码率，开始混流后不支持修改视频帧率。
...
创建 [ZegoMixerAudioConfig \|blank](@-ZegoMixerAudioConfig) 对象，设置混流音频配置，包含音频码率、声道和编码 ID，开始混流后不支持修改音频码率。
...
创建 [ZegoMixerOutput \|blank](@-ZegoMixerOutput) 对象，设置混流输出信息。
...
创建 [ZegoMixerInput \|blank](@-ZegoMixerInput) 对象，设置混流输入信息，包含流 ID、混流内容类型、混流布局等。
...
创建 [ZegoWatermark \|blank](@-ZegoWatermark) 对象，设置混流水印，水印的“imageURL”需要联系 ZEGO 技术支持获取。
...
创建 [ZegoMixerTask \|_blank](@-ZegoMixerTask) 对象，设置混流任务。
...
混流任务创建完成后，调用 [startMixerTask \|_blank](@startMixerTask) 接口启动该混流任务，若请求开启混流任务发生异常，例如混流的输入流不存在，将会从 callback 回调的错误码给出。
...
当混流信息发生变更时，例如混流的输入流列表发生增减、调整混流视频输出码率等，则修改该混流任务对象的参数后再调用一次 [startMixerTask \|_blank](@startMixerTask) 接口即可更新配置。
...
调用 [stopMixerTask \|_blank](@stopMixerTask) 接口停止混流任务。
...
若需要将混流推到第三方 CDN，可在 [ZegoMixerOutput \|_blank](@-ZegoMixerOutput) 接口的“target”参数填写 CDN 的 URL，且 URL 格式需要为 RTMP 格式，例如“rtmp://xxxxxxxx”。

推多路 CDN 需要创建 N 个输出流对象，将 [ZegoMixerOutput \|_blank](@-ZegoMixerOutput) 放入 [ZegoMixerTask \|_blank](@-ZegoMixerTask) 的 “outputList”输出列表中。

**2. 如何设置混流中每条流的布局？**

通过 [ZegoMixerInput \|_blank](@-ZegoMixerInput) 对象的“layout”参数进行设置，使用示例如下：

1. 假设指定某条流的左上角坐标为 (50，300)，右下角坐标为 (200，450)，则“layout”参数为[ZegoRect rectWithLeft:50 top:300 right:200 bottom:450]。
2. 假设 [ZegoMixerTask \|_blank](@-ZegoMixerTask) 对象的“setVideoConfig”参数中的分辨率“resolution”为“CGSizeMake(375, 667)”，则这条流在最终的输出混流中的位置如下所示：
...
SDK 会做等比缩放。假设一条输入流的分辨率为“720x1280”，即比例为“9:16”，同时这条流的 [ZegoMixerInput \|_blank](@-ZegoMixerInput) 的“layout”参数为“[left:0 top:0 right:100 bottom:100]”， 即比例为 “1:1” 时，画面将会显示这条流的中间部分，即裁剪了上下部分。
