...
若指定场景后，开发者有对 SDK 音视频配置精细化控制的需求，可通过其他 API 进行配置，优先级高于场景化配置，详情请参考 [常用视频配置](!ExpressVideoSDK-AdvancedModule/Common_Video_Configuration)、[流量控制](!ExpressVideoSDK-Other_Functions/Network_Adaptation) 文档。
...
- 登录房间后，不再允许修改房间场景。如需进行场景切换，需先退出房间，若登录了多个房间，则需要退出所有房间后才能进行场景切换。若在已登录房间的情况下调用 [setRoomScenario](@setRoomScenario) ，该次调用无效，并会通过 [onDebugError](!setting_callback#onDebugError) 回调错误码 1000067。
