...
调用 [createMediaPlayer](@createMediaPlayer) 创建媒体播放器，并调用 [setPlayerCanvas](@setPlayerCanvas-ZegoMediaPlayer) 设置媒体播放器 canvas 的 alphaBlend 属性为 true。
...
调用 [createMediaPlayer](@createMediaPlayer) 创建媒体播放器后，通过 [loadResourceWithConfig](@loadResourceWithConfig) 接口，通过**文件路径**或**二进制内存数据**，播放拼接后的特效素材。

<Warning title="注意">


虽然 [loadResourceWithConfig](@loadResourceWithConfig) 接口支持通过版权音乐资源 ID 加载多媒体资源，但是版权音乐目前不支持视频，因此无法通过版权音乐资源 ID 加载方式来加载礼物特效。
</Warning>
