...
sequenceDiagram
    ...
    App->>SDK: 设置自定义视频渲染配置 <br/> enableCustomVideoRender
    App->>SDK: 设置渲染回调对象,<br>实现对应回调方法 <br/> setCustomVideoRenderHandler
    App->>SDK: 登录房间 <br/> loginRoom

    rect rgb(255, 253, 230)
        note over App,SDK: 调用推拉流接口，获取相关视频数据后，进行自定义渲染
        App->>SDK: 预览、推流 <br/> startPreview、startPublishingStream
        SDK-->>App: 本地预览采集的视频数据通知 <br/> onCapturedVideoFrameRawData
        App->>SDK: 拉流 <br/> startPlayingStream
        SDK-->>App: 远端拉流的视频数据通知 <br/> onRemoteVideoFrameRawData
    end
```
{/*
