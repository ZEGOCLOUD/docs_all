...
sequenceDiagram
    ...
    P->>Z: 初始化引擎 <br/> createEngine
    P->>Z: 登录房间 <br/> loginRoom
    L->>Z: 初始化引擎 <br/> createEngine
    L->>Z: 登录房间 <br/> loginRoom
    P->>Z: 设置渲染的 view <br/> setOpaque、setFormat

    P-->>Z:  (可选) 自定义渲染及回调 <br/> enableCustomVideoRender、<br/>setCustomVideoRenderHandler
    P->>Z: 监听主体分割状态回调 <br/> onVideoObjectSegmentationStateChanged
    P->>Z: 开启主体分割 <br/> enableVideoObjectSegmentation<br/>(背景虚化/虚拟背景/透明背景)
    P-->>Z: (可选) 使用 Alpha 通道传输分割出的主体 <br/> enableAlphaChannelVideoEncoder
    P->>Z: 开始预览和推流 <br/> startPreview、<br/>startPublishingStream

    L-->>Z: (可选) 在拉流端设置 Alpha 通道渲染并拉流 <br/> ZegoCanvas.alphaBlend = true、<br/>startPlayingStream

    P->>Z: 关闭主体分割 <br/> enableVideoObjectSegmentation

    P->>Z: 销毁引擎 <br/> destroyEngine
    L->>Z: 销毁引擎 <br/> destroyEngine
```
{/*
...
初始化和登录房间的具体流程，请参考实现视频通话文档中的 “[创建引擎](/real-time-video-android-java/quick-start/implementing-video-call)”及“[登录房间](/real-time-video-android-java/quick-start/implementing-video-call)”。
...
调用 [enableCustomVideoRender](@enableCustomVideoRender) 接口，设置引擎进阶配置并开启自定义渲染，同时调用 [setCustomVideoRenderHandler](@setCustomVideoRenderHandler) 接口设置自定义视频渲染回调，详情请参考 [自定义视频渲染](/real-time-video-android-java/video/custom-video-rendering)。
