...
- 已在项目中集成 ZegoSuperBoard SDK，实现白板相关功能，详情请参考 [创建超级白板](/super-board-android/quick-start/create-white-board)。
...
具体流程请参考“实现视频通话”文档的 [初始化](/real-time-video-android-java/quick-start/implementing-video-call) 和 [登录房间](/real-time-video-android-java/quick-start/implementing-video-call) 章节完成初始化并登录房间。
...
具体流程请参考混流文档的 [创建混流任务对象](/real-time-video-android-java/live-streaming/stream-mixing) 进行创建。

#### 设置混流输入流

具体流程请参考混流文档的 [设置混流输入流](/real-time-video-android-java/live-streaming/stream-mixing) 进行设置。

#### 设置混流输出

具体流程请参考混流文档的 [设置混流输出](/real-time-video-android-java/live-streaming/stream-mixing) 进行设置。
...
调用 [setWhiteboard](@setWhiteboard) 接口，将 ZEGO 超级白板内容合入混流：需填入 [whiteboardID](@whiteboardID-ZegoMixerWhiteboard) 白板 ID 等参数，即本文 [前提条件](/real-time-video-android-java/other/push-the-whiteboard) 中，集成 ZegoSuperBoard SDK 并创建白板时返回的 ID 等信息。
...
具体流程请参考混流文档的 [开始混流任务](/real-time-video-android-java/live-streaming/stream-mixing) 章节。


### 4 更新混流任务配置

Express SDK 支持更新混流任务中的白板配置，例如，需要切换要混入的 ZEGO 超级白板实例或不再混入 ZEGO 超级白板时，可通过调用 [startMixerTask](@startMixerTask) 接口更新配置。如需更新混流任务的其它配置，请参考混流文档中的 [5.4 更新混流任务的配置](/real-time-video-android-java/live-streaming/stream-mixing)。
...
具体流程请参考混流文档的 [停止混流](/real-time-video-android-java/live-streaming/stream-mixing)。
