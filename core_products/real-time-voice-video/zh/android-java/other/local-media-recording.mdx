...
- [ZegoDataRecordConfig](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-entity-zego-data-record-config) 指定录制的配置，可以指定录制的路径以及录制的类型 [ZegoDataRecordType](@-ZegoDataRecordType)。
...
* 调用 ZegoExpressEngine 中的 ZegoMediaPlayer 组件进行播放，详情可参考 [媒体播放器](/real-time-video-android-java/other/media-player)。
...
请参考 [原始音频数据获取](/real-time-video-android-java/audio/get-audio-raw-data)。
