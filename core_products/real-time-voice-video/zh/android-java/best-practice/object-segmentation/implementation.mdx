...
如需了解渲染部分的实现原理，请参考 [播放透明礼物特效](/real-time-video-android-java/other/play-transparent-gift-effects)。
...
- 已联系 ZEGO 技术支持，获取包含主体分割功能的实时音视频 SDK，并参考实时音视频的 [快速开始 - 集成](/real-time-video-android-java/quick-start/integrating-sdk) 集成 SDK。**如果您的项目曾集成了官网版本的实时音视频 SDK，需要替换。**
...
sequenceDiagram
    ...
    App->>ZIM SDK: 初始化 SDK<br/>create
    App->>ZIM SDK: 设置事件通知回调<br/>setEventHandler
    App->>ZEGO Express SDK: 初始化 SDK 并设置事件通知回调<br/>createEngine(profile, eventHandler)
```
{/*
...
sequenceDiagram
    ...
    participant App
    participant ZEGO Express SDK
    note over ZIM SDK,ZEGO Express SDK: 加入房间
    App->>ZEGO Express SDK: 登录 RTC 房间<br/>loginRoom
    App->>ZIM SDK: 登录 ZIM 服务<br/>login
    App->>ZIM SDK: 创建/加入 ZIM 房间<br/>createRoom/joinRoom
```
{/*
...
sequenceDiagram
    ...
    App->>ZIM SDK: 查询房间所有属性（包含麦位信息）<br>queryRoomAllAttributes
    ZIM SDK-->>App: 返回房间所有属性信息<br>ZIMRoomAttributesQueriedCallback
    App->>ZIM SDK: 设置房间属性（上麦）<br>setRoomAttributes
    ZIM SDK-->>App: 通知用户本端已上麦<br>onRoomAttributesUpdated
```
{/*
...
sequenceDiagram
    ...
    participant App
    participant ZEGO Express SDK
    note over ZIM SDK, ZEGO Express SDK: 使用主体分割
    App->>ZEGO Express SDK: 设置透明通道 <br/>enableAlphaChannelVideoEncoder
    App->>ZEGO Express SDK: 开启屏幕镜像 <br/>setVideoMirrorMode
    App->>ZEGO Express SDK: 设置采集视频的朝向 <br/>setAppOrientationMode
    App->>ZEGO Express SDK: 开始预览和推流 <br/>startPreview & startPublishingStream <br/>仅预览时，需设置 TextureView 的 opaque 为透明色
    App->>ZEGO Express SDK: 开启主体分割 <br/>enableVideoObjectSegmentation
    ZEGO Express SDK-->>App: 通知主体分割结果 <br/>onVideoObjectSegmentationStateChanged
```
{/*
...
sequenceDiagram
    ...
    ZIM SDK->>App: 通知本端房间内有其他用户上麦 <br/>nonRoomAttributesUpdated
    App->>ZEGO Express SDK: 开始拉流 <br/>startPlayingStream <br/>仅预览时，需调用 setOpaque 接口用于渲染的 TextureView 的 opaque 属性为 false（透明色）
