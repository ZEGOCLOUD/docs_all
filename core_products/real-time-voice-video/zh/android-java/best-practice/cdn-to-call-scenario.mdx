...
<img src="/Pics/Common/ZegoExpressEngine/CDN_To_Call_Scenario.png" width=50% >
...
如需从 CDN 拉流切换到连麦场景，请确保当前音视频流，已实现 [通过 URL 拉流\|_blank](!Publisher_Player_Advanced/PlayingStreamByCDN)。

## 实现步骤

### 1 观众端切换拉流方式

当从 CDN 拉流切换到连麦场景时，观众端需调用 [stopPlayingStream\|_blank](@stopPlayingStream) 停止当前 CDN 拉流，并调用 [startPlayingStream\|_blank](@startPlayingStream__2) 切换为 RTC 拉流。
...
传入观众端的流 ID，并开启预览 [startPreview\|_blank](@startPreview) 和推流 [startPublishingStream\|_blank](@startPublishingStream)，即可连麦成功。
...
若连麦结束，可在观众端调用 [stopPreview\|_blank](@stopPreview) 和 [stopPublishingStream\|_blank](@stopPublishingStream) 接口，停止预览和推流。
...
连麦结束，传入主播的流 ID，并在观众端调用 [stopPlayingStream\|_blank](@stopPlayingStream) 接口，停止 RTC 拉流 ，重新切换为 CDN 拉流，并通过 [ZegoViewMode\|_blank](@-ZegoViewMode) 设置 CDN 参数，详情请参考 [通过 URL 拉流\|_blank](!Publisher_Player_Advanced/PlayingStreamByCDN)。
...
主播端通过 [IZegoEventHandler\|_blank](@-IZegoEventHandler) 中的 [onRoomStreamUpdate\|_blank](@onRoomStreamUpdate) 回调，收到观众流新增通知后，可开启 RTC 拉流。当收到观众流删除通知后，则停止 RTC 拉流。
