...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/Common/ZegoExpressEngine/CDN_To_Call_Scenario.png" /></Frame>
...
如需从 CDN 拉流切换到连麦场景，请确保当前音视频流，已实现 [通过 URL 拉流](!Publisher_Player_Advanced/PlayingStreamByCDN)。
...
连麦结束，传入主播的流 ID，并在观众端调用 [stopPlayingStream](@stopPlayingStream) 接口，停止 RTC 拉流 ，重新切换为 CDN 拉流，并通过 [ZegoViewMode](@-ZegoViewMode) 设置 CDN 参数，详情请参考 [通过 URL 拉流](!Publisher_Player_Advanced/PlayingStreamByCDN)。
