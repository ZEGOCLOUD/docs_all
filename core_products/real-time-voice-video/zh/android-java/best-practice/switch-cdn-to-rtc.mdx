...
sequenceDiagram
    ...
    note over 主播,ZEGO Express SDK: 观众上麦
    观众->>ZEGO Express SDK: 停止 CDN 拉流，切换 RTC 拉流
    观众->>ZEGO Express SDK: 开启预览和推流
    ZEGO Express SDK-->>主播: 房间观众流新增通知
    主播->>ZEGO Express SDK: 开启 RTC 拉流

    note over 主播,ZEGO Express SDK: 观众下麦
    观众->>ZEGO Express SDK: 停止预览和推流
    观众->>ZEGO Express SDK: 停止 RTC 拉流，切换 CDN 拉流
    ZEGO Express SDK-->>主播: 房间观众流删除通知
    主播->>ZEGO Express SDK: 停止 RTC 拉流
```
{/*
...
如需从 CDN 拉流切换到连麦场景，请确保当前音视频流，已实现 [通过 URL 拉流](/real-time-video-android-java/live-streaming/playing-stream-by-url)。

## 实现步骤

### 观众端切换拉流方式

当从 CDN 拉流切换到连麦场景时，观众端需调用 [stopPlayingStream](@stopPlayingStream) 停止当前 CDN 拉流，并调用 [startPlayingStream](@startPlayingStream__2) 切换为 RTC 拉流。
...
传入观众端的流 ID，并开启预览 [startPreview](@startPreview) 和推流 [startPublishingStream](@startPublishingStream)，即可连麦成功。
...
若连麦结束，可在观众端调用 [stopPreview](@stopPreview) 和 [stopPublishingStream](@stopPublishingStream) 接口，停止预览和推流。
...
连麦结束，传入主播的流 ID，并在观众端调用 [stopPlayingStream](@stopPlayingStream) 接口，停止 RTC 拉流 ，重新切换为 CDN 拉流，并通过 [ZegoViewMode](@-ZegoViewMode) 设置 CDN 参数，详情请参考 [通过 URL 拉流](/real-time-video-android-java/live-streaming/playing-stream-by-url)。
...
主播端通过 [IZegoEventHandler](@-IZegoEventHandler) 中的 [onRoomStreamUpdate](@onRoomStreamUpdate) 回调，收到观众流新增通知后，可开启 RTC 拉流。当收到观众流删除通知后，则停止 RTC 拉流。
