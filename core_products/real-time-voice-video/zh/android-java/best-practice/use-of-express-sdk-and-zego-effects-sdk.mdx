...
- 已在项目中集成 ZEGO Effects SDK，详情请参考“AI 美颜”的 [快速开始 - 集成](https://doc-zh.zego.im/article/9573)。
- 已获取到 ZEGO Effects SDK 唯一的鉴权文件，详情请参考“AI 美颜”的 [快速开始 - 在线鉴权](https://doc-zh.zego.im/article/11987)。
...
2. 获取视频原始数据，可通过 ZEGO Express SDK 的 [自定义视频采集](https://doc-zh.zego.im/article/3677) 或 [自定义视频前处理](https://doc-zh.zego.im/article/7656) 两种方式获取。
...
1. 导入 Effects 模型和资源。
    ...
    ZEGO Effects SDK 支持的所有资源和模型请参考“AI 美颜”的 [快速开始 - 导入资源和模型](https://doc-zh.zego.im/article/10184)。
...
- **方式一：自定义视频前处理**
    ...
    具体的原理可参考“实时音视频”的 [自定义视频前处理](https://doc-zh.zego.im/article/7656)。

<a id="CustomCollection"></a>

- **方式二：自定义视频采集**

    自定义视频采集，主要依赖开发者自行采集视频数据，具体方式请参考“实时音视频”的 [自定义视频采集](https://doc-zh.zego.im/article/3677)。
...
- **方式一：自定义视频前处理**
    ...
    在 [onCapturedUnprocessedTextureData](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-callback-i-zego-custom-video-process-handler#on-captured-unprocessed-texture-data) 回调中，获取到视频原始数据后，调用 ZEGO Effects SDK 的相关接口，进行 AI 美颜处理（请参考 [美颜](https://doc-zh.zego.im/article/9638)、[美型](https://doc-zh.zego.im/article/9642)、[背景分割](https://doc-zh.zego.im/article/9646)、[人脸检测](https://doc-zh.zego.im/article/9650)、[挂件](https://doc-zh.zego.im/article/10172)、[滤镜](https://doc-zh.zego.im/article/10727)），并将处理后的数据，返回给 ZEGO Express SDK。
...
- **方式二：自定义视频采集**
    ...
    在接收到自定义采集的 [onStart](@onStart) 回调后，开发者通过自定义采集获取视频数据，再调用 ZEGO Effects SDK 的相关接口，进行 AI 美颜处理（请参考 [美颜](https://doc-zh.zego.im/article/9638)、[美型](https://doc-zh.zego.im/article/9642)、[背景分割](https://doc-zh.zego.im/article/9646)、[人脸检测](https://doc-zh.zego.im/article/9650)、[挂件](https://doc-zh.zego.im/article/10172)、[滤镜](https://doc-zh.zego.im/article/10727)），并将处理后的数据，返回给 ZEGO Express SDK（可参考 [自定义视频采集](3677#4_3) 中的 “3 向 SDK 发送视频帧数据”）。
