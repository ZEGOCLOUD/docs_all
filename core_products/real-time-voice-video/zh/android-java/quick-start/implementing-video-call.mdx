...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时音视频功能，详情请参考 [快速开始 - 集成](/real-time-video-android-java/quick-start/integrating-sdk)。
...
根据 App 实际的音视频业务选择一个合适的场景，请参考 [场景化音视频配置](/real-time-video-android-java/quick-start/scenario-based-audio-video-configuration) 文档，把选择好的场景枚举传入参数 "scenario"。
...
我们可以在该回调中，调用 [startPlayingStream](@startPlayingStream__1)，传入 “streamID” 拉取播放该用户的音视频。您可通过监听 [onPlayerStateUpdate ](@onPlayerStateUpdate) 回调知晓是否成功拉取音视频。
...
如果用户在音视频通话的过程中，遇到相关错误，可查询 [错误码](/real-time-video-android-java/client-sdk/error-code)。
...
sequenceDiagram
    ...
    App->>ZEGO Express SDK: 创建引擎 <br/> createEngine

    App->>ZEGO Express SDK: 登录房间 <br/> loginRoom
    ZEGO Express SDK-->>App: 房间/用户/流状态更新回调 <br/> onRoomStateChanged / onRoomUserUpdate / onRoomStreamUpdate

    App->>ZEGO Express SDK: 预览/推流 <br/> startPreview / startPublishingStream
    ZEGO Express SDK-->>App: 推流状态更新回调 <br/> onPublisherStateUpdate

    App->>ZEGO Express SDK: 拉流 <br/> startPlayingStream
    ZEGO Express SDK-->>App: 拉流状态更新回调 <br/> onPlayerStateUpdate
    ...
    ZEGO Express SDK-->>App: 房间状态更新回调 <br/> onRoomStateChanged
...
## 相关文档
- [常见错误码](/real-time-video-android-java/client-sdk/error-code)
...
- [为什么 Android 9 应用锁屏或切后台后采集音视频无效](https://doc-zh.zego.im/faq/android-background-capture-issue)
