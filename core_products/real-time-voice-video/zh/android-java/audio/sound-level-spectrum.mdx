...
- 调用 [startSoundLevelMonitor](@startSoundLevelMonitor) 接口启动声浪的监听：
    ...
    在 1.15.0 及以上 版本，调用 [startSoundLevelMonitor](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#start-sound-level-monitor-1) 接口时，可以设置 `millisecond` 参数来调整声浪回调的间隔。
    ```java
    // millisecond：声浪的监控时间周期，单位为毫秒，取值范围 [100, 3000]。默认 100 ms。
