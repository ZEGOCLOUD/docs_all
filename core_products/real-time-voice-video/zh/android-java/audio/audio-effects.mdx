...
如果需要设置自定义变声，请查看 [设置自定义变声](/real-time-video-android-java/audio/audio-effects)。
</Note>
...
如果需要设置预设变声，请查看 [设置预设变声](/real-time-video-android-java/audio/audio-effects)。
</Note>

若 SDK 预置的变声效果无法满足需求，开发者可以调用 [setVoiceChangerParam](@setVoiceChangerParam) 中 [ZegoVoiceChangerParam ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-entity-zego-voice-changer-param) 方法，通过音高参数 "pitch" 设置自定义变声。该参数取值范围为 [-12.0, 12.0]，值越大声音越尖锐，默认值为 "0.0"（即无变声）。
...
如果需要设置自定义混响，请查看 [设置自定义混响](/real-time-video-android-java/audio/audio-effects)。
</Note>
...
如果需要设置预设混响，请查看 [设置预设混响](/real-time-video-android-java/audio/audio-effects)。
</Note>

若 SDK 预设的混响类型无法满足需求，开发者可以调用 [setReverbAdvancedParam](@setReverbAdvancedParam) 中 [ZegoReverbAdvancedParam ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-entity-zego-reverb-advanced-param) 方法，通过相关参数搭配设置，实现开发者需要的混响效果（详细参数说明请参考 API 文档）。
