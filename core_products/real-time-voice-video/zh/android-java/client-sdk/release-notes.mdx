...
1. 新增 I 帧丢失通知回调
    ...
    外部采集模块新增 onRequestKeyFrame 回调，当 I 帧丢失时会触发回调通知，提醒补请求关键帧。

    相关 API 请参考： [onRequestKeyFrame](@onRequestKeyFrame)

2. 新增 Opus 音频自定义输入接口

    新增 sendCustomAudioCaptureOpusData 接口，支持输入 Opus 编码格式的音频数据。

    相关 API 请参考： [sendCustomAudioCaptureOpusData](@sendCustomAudioCaptureOpusData)

3. 新增 L3 与 RTC 切换接口

    新增 switchPlayingResource 接口，支持 L3（超低延迟直播）与 RTC 之间的切换，并优化切换流程的流畅性。

    相关 API 请参考： [switchPlayingResource](@switchPlayingResource)

4. 支持自定义音频采集与自定义屏幕采集组合使用

    支持自定义管理音频采集和屏幕采集，适用于多通道推流及通道切换场景。

    相关 API 请参考： [createCustomAudioSource](@createCustomAudioSource)、 [destroyCustomAudioSource](@destroyCustomAudioSource)

5. Android 和 iOS 端支持枚举外部摄像头

    Android 和 iOS 端 SDK 支持枚举外部摄像头，满足在 Mac App Store、Google Play Games on PC 等场景下识别外接摄像头的需求。
    <Warning title="注意">
...
6. 自有版权打分支持内存资源模式
    ...
    自有版权打分能力支持内存资源模式，适配通过内存方式加载资源播放的场景。

    相关 API 请参考： [loadResourceFromMediaData](@loadResourceFromMediaData)

7. 近距离拍摄时支持自动切换广角/微距镜头

    当摄像头与被摄物距离低于主摄最近对焦距离时，SDK 支持自动切换到广角或微距镜头，以获得更佳拍摄效果。
    <Warning title="注意">
    如需使用此功能，请联系 ZEGO 技术支持。
    </Warning>
...
1. 减少 SDK 所创建的 TLS Key 数量，降低资源占用

2. 默认最大拉流路数从 12 路提升至 20 路

3. 优化低延迟模式下影响字准率的问题

4. 优化 Android ClassLoader 加载逻辑，确保 SDK 初始化时的稳定性

5. 优化画质增强算法，提升图像锐化和超分辨率效果


6. 超分辨率性能优化

    优化传统超分辨率场景下的内存占用和帧率表现。

    相关 API 请参考： [enableVideoSuperResolution](@enableVideoSuperResolution)
...
1. 修复创建线程时的低概率崩溃问题
    ...
    优化 SDK 的线程管理，避免因创建线程导致的低概率崩溃问题。

2. 修复一个低概率出现的拉流异常问题

3. 修复外部滤镜偶现的崩溃问题

4. 修复版权音乐打分场景下获取不到分数的问题
...
1. 支持音视频流自定义加密
    ...
    SDK 开放音视频传输数据自定义加解密能力，支持业务自主对原始音视频数据进行前置加密处理。
    <Warning title="注意">
    此功能需要特殊编包，如需使用请联系 ZEGO 技术支持。
    </Warning>

    相关 API 请参考： [IZegoCustomMediaCrypto](@-IZegoCustomMediaCrypto)、 [setCustomMediaCryptoFactory](@setCustomMediaCryptoFactory)


**改进优化**

1. 优化 Express SDK 的大小

2. 本地设备异常通知回调支持监听设备从异常状态恢复的异常类型

    新增设备从异常状态恢复的异常类型，当本地音频或视频设备功能从其他的异常状态恢复正常时会触发本地设备异常通知回调。
    <Warning title="注意">
    如需监听此异常类型，请联系 ZEGO 技术支持进行配置。
    </Warning>

    相关 API 请参考： [onLocalDeviceExceptionOccurred](@onLocalDeviceExceptionOccurred)

3. 本地设备异常通知回调支持监听采集音量过低的异常类型

    新增采集音量过低的异常类型，支持设置音量阈值和自定义检测周期，便于业务灵活识别并做出低音量提示。

    相关 API 请参考： [onLocalDeviceExceptionOccurred](@onLocalDeviceExceptionOccurred)

4. 优化人脸检测回调延迟

    优化 Android 低端设备的人脸检测兼容性，支持自定义回调延迟模式，大幅降低响应耗时。同时人脸检测回调新增设备不支持人脸检测的错误码 1003082。

    相关 API 请参考： [onPublisherFaceDetectInfo](@onPublisherFaceDetectInfo)

5. 优化媒体播放器播放预加载资源时的内存占用
...
1. 修复 SDK 在反初始化过程中，解码器未及时停止极小概率出现崩溃的问题

2. 修复开启本地代理连接不上 RTC 服务的问题

3. 修复 32 位系统开启云代理连接不上 RTC 服务的问题
...
1. 修复 CPU 软件解码处理色彩范围为 full range 的视频时可能出现绿屏的问题
...
1. 移动端支持视频采集后、美颜前、美颜后、编码后的实时转储
    ...
    相关 API 请参考： [startDumpData](@startDumpData)

2. 新增视频编码前处理

    新增视频编码前处理功能，开启后可提升画面的清晰度。

    相关 API 请参考： [enableVideoEncoderEnhancement](@enableVideoEncoderEnhancement)
...
3. onPublisherQualityUpdate 回调提供流量控制状态信息
    ...
    相关 API 请参考： [onPublisherQualityUpdate](@onPublisherQualityUpdate)
...
1. 流量控制支持任意分辨率
    ...
    优化流控逻辑，移除画面比例限制，当前可支持任意分辨率。

2. 流量控制状态回调接口支持区分断网状态

    新增返回值 -1 表示断网重连，从没有流量控制的状态（返回值 0） 中分离出断网重连的状态。

3. OpenH.264 编码器质量优化

    优化 OpenH.264 编码器性能，提升编码画质。

4. 媒体播放器资源解析优化

    SDK 支持 HTTP DNS 解析，提升媒体播放器资源域名解析成功率。
...
2. 屏幕共享提供启动回调和异常中断回调 
    ...
    相关 API 请参考：[onScreenCaptureStart](@onScreenCaptureStart)、[onScreenCaptureExceptionOccurred](@onScreenCaptureExceptionOccurred)
...
5. 新增静态图片采集视频源 
    ...
    通过设置 [setVideoSource](@setVideoSource) 视频源为 `Picture`，即可推流一张指定的图片。
...
7. 媒体播放器实例个数限制放开至 10 个
    ...
    详细请参考 [媒体播放器](../other/media-player.mdx)。
...
1. 支持开发者在自定义音频采集时指定当前音频输出路由以优化 AEC 效果
    ...
    相关 API 请参考 [setCustomAudioIOAudioRoute](@setCustomAudioIOAudioRoute)
...
3. 人脸识别功能支持后置摄像头以及回调人脸个数和坐标信息
    ...
    人脸识别功能支持后置摄像头，且新增回调 [onPublisherFaceDetectInfo](@onPublisherFaceDetectInfo) 支持获取人脸个数和坐标信息。

    <Note title="说明">该回调仅当摄像头启动和人脸个数变化时触发，无法与自定义视频前处理配合使用。</Note>


    相关 API 请参考 [enableFaceDetection](@enableFaceDetection), [onPublisherFaceDetectInfo](@onPublisherFaceDetectInfo)
...
2. 优化频繁调用 enableCamera 时摄像头状态回调次数过多的问题
        ...
        ZEGO 为 [enableCamera](@enableCamera__2) 接口新增 `notifyMode` 参数，用于控制关闭摄像头时是否通知本端以及远端。在切换前后置摄像头场景下，该参数可减少设备开关状态的回调次数，避免触发不必要的业务逻辑。

    相关 API 请参考 [enableCamera](@enableCamera__2)
...
1. 新增 AI 低照度增强功能
    ...
    开发者可通过 [setLowlightEnhancementParams](@setLowlightEnhancementParams) 指定使用 AI 算法，当设备性能不足以支持开启 AI 算法时，SDK 会自动使用传统算法。一般中高端机型可稳定运行低照度增强 AI 算法，详细请参考 [推流视频增强](/real-time-video-android-java/video/publish-video-enhancement) 文档。
...
5. 移动端屏幕共享功能支持共享指定画面区域
    ...
    使用移动端的屏幕共享功能时，可通过  [ZegoScreenCaptureConfig-cropRect](@cropRect-ZegoScreenCaptureConfig) 参数指定共享的画面区域，详情请参考 [屏幕共享](/real-time-video-android-java/video/screen-sharing) 文档。

    相关 API 请参考 [ZegoScreenCaptureConfig > cropRect](@cropRect-ZegoScreenCaptureConfig)
...
2. 支持设置视频降噪功能
    ...
    新增视频降噪接口`setVideoDenoiseParams`，适用于摄像头的采集效果不佳，画面出现明显的噪点时，需要单独视频降噪等场景。开发者可以根据业务场景，选择不同的视频降噪模式 [ZegoVideoDenoiseMode](@-ZegoVideoDenoiseMode) 及视频降噪强度 [ZegoVideoDenoiseStrength](@-ZegoVideoDenoiseStrength)，详情可参考 [推流视频增强](/real-time-video-android-java/video/publish-video-enhancement) 文档。

    相关 API 请参考 [setVideoDenoiseParams](@setVideoDenoiseParams)
...
4. 支持将混流后的输出流加入到目标房间
    ...
    混流功能支持将混流后的输出流，加入到指定房间，即支持设置输出流的目标房间信息 [setTargetRoom](@setTargetRoom)。每条输出流只支持加入一个房间，且一旦添加，混流过程中不支持动态更新房间。如需使用服务端接口实现该功能，可参考 [开始混流](/real-time-video-server/api-reference/stream-mixing/start-mix) 文档。

    相关 API 请参考 [setTargetRoom](@setTargetRoom)
...
4. 媒体播放器播放透明特效时，支持 Alpha 数据布局在 RGB 数据右上方
        ...
        媒体播放器播放透明特效时，新增 [ZegoAlphaLayoutType > ZegoAlphaLayoutTypeRightTop](@ZegoAlphaLayoutTypeRightTop-ZegoAlphaLayoutType)  枚举，以支持 Alpha 数据拼接在 RGB 数据右上方，设置此枚举时，仅支持 0.5x 的缩放倍率，详情请参考  [播放透明礼物特效](/real-time-video-android-java/other/play-transparent-gift-effects) 文档。
...
1. 视频超分辨率支持 1.33 倍和 1.5 倍放大倍数
        ...
        视频分辨率高于设备支持上限会引入额外损伤，导致视频质量下降。因此视频超分辨率能力新增 1.33 倍和 1.5 倍的放大倍数，以适配不同设备的最佳效果，更多内容请参考 [超分辨率](/real-time-video-android-java/video/super-resolution)。

2. AI 降噪新增低延迟模式

        注意：如需使用该功能，请联系 ZEGO 技术支持。

        在 10ms 延迟下，依然保持纯净的降噪效果以及高保真的人声音质，适用于游戏语音、游戏开黑、实时合唱等对延迟较为敏感的场景，目前 AI 降噪已支持均衡模式、低延迟模式以及轻量模式，详情请参考 [场景化 AI 降噪](/real-time-video-android-java/audio/scenario-based-ai-noise-reduction)。
...
4. 新增屏幕采集权限的申请结果回调通知
        ...
        开发者可根据回调结果做后续业务处理，包括 UI 提示或应用跳转等，更多内容请参考 [屏幕共享](/real-time-video-android-java/video/screen-sharing)。

        相关 API 请参考 [onScreenCaptureStart](@onScreenCaptureStart)

5. 媒体播放器播放倍速最低支持至 0.3 倍速

        媒体播放器的播放速度区间从 [0.5,4.0]，扩大为 [0.3,4.0]，更多内容请参考 [媒体播放器](/real-time-video-android-java/other/media-player)。
...
1. 优化 AI 场景化降噪中，“均衡模式”的降噪效果
        ...
        优化 AI 场景化降噪中，“均衡模式”的降噪效果，在性能不变的情况下，人声清晰度、平稳度进一步提升，且噪音抑制更干净，详情请参考 [场景化 AI 降噪](/real-time-video-android-java/audio/scenario-based-ai-noise-reduction)。
...
5. 支持获取 SDK 前处理后的视频数据，向 SDK 传入不同的视频数据分别用于预览和推流
        ...
        相关 API 请参考 [sendCustomVideoProcessedTextureData](@sendCustomVideoProcessedTextureData)
...
2. 推流视频支持色彩增强
        ...
        针对各种摄像头等设备采集到的画面色彩偏灰、或饱和度偏低的情况，支持在保护人体肤色的同时，增强画面色彩，使其更加鲜艳明亮，更符合人眼真实的视觉感受，详情请参考 [推流视频增强](/real-time-video-android-java/video/publish-video-enhancement)。
...
11. 支持获取音频前处理之前的数据
        ...
        相关 API 请参考 [enableBeforeAudioPrepAudioData](@enableBeforeAudioPrepAudioData), [onBeforeAudioPrepAudioData](@onBeforeAudioPrepAudioData)
...
2. 当前官网 SDK 不包含此功能，如有需要，请联系 ZEGO 技术支持特殊编包。
        ...
        新增 AI 变声功能，实时通话中的“柯南变声领结”，完美重现目标角色的音色与韵律，同时保留用户的语速、情感、语调，随心所欲切换音色，超低延迟让用户畅享社交语聊、直播、游戏语音等场景，详情请参考 [AI 变声](/real-time-video-android-java/audio/ai-voice-changer)。
...
2. 虚拟背景的视频填充方式为居中及等比例缩放，视频过大时，超出部分会被裁减。
        ...
        使用主体分割功能时，虚拟背景支持使用视频素材，视频素材的最终帧率会与编码帧率保持一致，且循环播放。视频素材的限制说明，请参考 [主体分割](/real-time-video-android-java/video/object-segmentation)。
...
4. 支持获取并上传音频的 Dump 文件
        ...
        注意：由于音频 Dump 文件属于用户的隐私敏感数据，因此开发者实现该能力时，请务必认真阅读 [《即构隐私政策》](https://doc-zh.zego.im/policies-and-agreements/zego-privacy-policy) 中关于 “使用音频 Dump 功能” 的内容。此外，在收集音频 Dump 文件时，请在获得用户授权同意时，同步注明 Express SDK 收集目的。

        支持将处理前后的音频数据保存下来并上传，用于定位音频相关问题、提高问题排查效率、缩短接入时间，详情请参考 [如何获取、上传音频的 Dump 文件？](https://doc-zh.zego.im/faq/audio-dump-handling)
...
2. 修复本地混音时，期望数据长度可能不对的问题
        ...
        相关 API 请参考 [onAudioMixingCopyData](@onAudioMixingCopyData)
...
2. 转码会造成额外的延迟，不建议您在使用 RTC 拉流的麦上场景中使用该功能。
        ...
        RTC 拉流时，支持通过预设的转码模板触发单流转码任务，输出不同分辨率的转码流，详情请参考 [单流转码](/real-time-video-android-java/live-streaming/single-stream-transcoding)。
...
6. 支持均衡型 AI 降噪模式
        ...
        支持均衡型 AI 降噪模式，与原有模式相比，在相同的人声保真效果前提下，噪音抑制效果明显提升，可以达到干净无噪音或不扰人的程度；但性能消耗稍微增加。适用于街道、马路、市场等较为嘈杂（信噪比低）的户外环境中，详情请参考 [场景化 AI 降噪](/real-time-video-android-java/audio/scenario-based-ai-noise-reduction)。
...
1. 新增支持“智能云代理”模式
        ...
        开发者设置“智能云代理”模式后，在 RTC 或 L3 拉流时，会优先使用直连网络模式进行尝试。如果直连网络不可用、且当前是蜂窝网络，则继续留在直连模式重试；如果直连网络不可用、且当前是非蜂窝网络，则切到云代理模式，详情请参考 [云代理](/real-time-video-android-java/communication/cloud-proxy)。

2. 自定义视频前处理功能支持双输出

        自定视频前处理功能支持“双输出”，即支持输出“内存数据”和 “2D 纹理数据”。开发者可以通过这些数据，实现第三方美颜功能，更高性能地进行人脸检测和美颜。

        相关 API 请参考 [onCapturedUnprocessedRawData](@onCapturedUnprocessedRawData), [ZegoVideoBufferTypeGLTexture2DAndRawData](@ZegoVideoBufferTypeGLTexture2DAndRawData)
...
2. 设置大流、小流的分辨率的 “比例” 需要保持一致，否则调用接口会出错。
        ...
        在指定编码格式为 “大小流编码” 的情况下，支持分别设置大流和小流的分辨率、帧率和码率，详情请参考 [视频大小流和分层编码](/real-time-video-android-java/video/small-large-video-stream-and-layered-encoding)。
...
3. 新增错误码提示
        ...
        新增语音检测（1018xxxxx）、万人范围音视频（1019xxxxx）、屏幕采集（1020xxxxx）三个模块的错误码，详情请参考 [常见错误码](/real-time-video-android-java/client-sdk/error-code)。
...
1. 新增支持背景虚化、虚拟背景功能
        ...
        在实景或绿幕场景中，开发者可以通过该功能对用户的背景进行虚化模糊处理，或替换为自定义图片背景，详情请参考 [主体分割](/real-time-video-android-java/video/object-segmentation)。
...
2. 屏幕采集功能支持采集系统音频
        ...
        相关 API 请参考 [startScreenCapture](@startScreenCapture)
...
1. Android SDK 提供 AAR 包
        ...
        为了方便开发者使用屏幕共享功能，从 3.6.0 版本开始，在本地下载的 SDK 中提供了 AAR 包，并在 Maven 在线集成方式的 AAR 包中携带了屏幕共享的 Service 声明，开发者无需手动在自己工程中声明，详情请参考 [屏幕共享](/real-time-video-android-java/video/screen-sharing)。
...
2. 请在创建引擎之前，配置地理围栏信息。
        ...
        将音视频及信令数据访问限定在某一区域，用以满足地区数据隐私安全相关法规，即限定用户访问某一特定区域的音视频服务，详情请参考 [地理围栏](/real-time-video-android-java/communication/geofencing)。
...
3. 媒体播放器支持播放透明特效
        ...
        媒体播放器支持通过渲染 Alpha 通道，实现播放透明特效文件功能，详情请参考 [播放透明礼物特效](/real-time-video-android-java/other/play-transparent-gift-effects)。
...
8. 场景化音视频配置新增 [StandardVoiceCall] 标准语音通话场景
        ...
        场景化音视频配置新增 [StandardVoiceCall] 标准语音通话场景，适用于 1v1 纯语音通话场景，详情请参考 [场景化音视频配置](/real-time-video-android-java/quick-start/scenario-based-audio-video-configuration)。
...
2. 优化超分逻辑，新增初始化、反初始化接口
        ...
        注意：[enableVideoSuperResolution] 修改了调用时机，需要在 [initVideoSuperResolution] 后才能调用，详情请参考 [超分辨率](/real-time-video-android-java/video/super-resolution)。

        相关 API 请参考 [initVideoSuperResolution](@initVideoSuperResolution), [uninitVideoSuperResolution](@uninitVideoSuperResolution)
...
2. 支持视频大小流功能
        ...
        通过视频大小流编码（H.264 DualStream）对视频进行码流分层时，相比较于分层视频编码（H.264 SVC），视频大小流编码（H.264 DualStream）支持使用硬件编码，即 [ZegoVideoCodecID] 新增 [ZegoVideoCodecIDH264DualStream] 字段，详情请参考 [设置视频编码方式](/real-time-video-android-java/video/set-video-encoding)。
...
4. 支持多源采集能力
        ...
        多源采集能力对屏幕共享、混音等常见能力的实现路径，进行缩短优化及归一化设计，从 3.1.0 版本后，您可以不用再通过自定义采集实现上述复杂的能力，详情请参考 [多源采集](/real-time-video-android-java/communication/multi-source-capture) 。
...
5. SDK 支持设置云代理
        ...
        通过设置 SDK 的云代理接口，将 SDK 对应的所有流量通过云端的代理服务器进行中转，实现与 RTC 通信，详情请参考  [云代理](/real-time-video-android-java/communication/cloud-proxy) 。
...
本次更新包含不兼容改动，详情请参考 [v3.0.0 升级指南](/real-time-video-android-java/client-sdk/upgrade-guide/upgrade-to-v3)。
</Warning>
...
1. 新增视频超分辨率能力
        ...
        新增 [enableVideoSuperResolution] 接口支持对某一条视频流进行超分辨率处理，从而得到更好的画质。超分辨率，简称超分，是客户端对拉取的视频流的宽和高进行实时倍增处理的技术，例如，从 640x360 超分为 1280x720，详情请参考 [超分辨率](/real-time-video-android-java/video/super-resolution)。

        相关 API 请参考 [enableVideoSuperResolution](@enableVideoSuperResolution), [onPlayerVideoSuperResolutionUpdate](@onPlayerVideoSuperResolutionUpdate)

2. 场景化 AI 降噪新增在音乐场景下降噪的能力

        注意：如需使用该功能，请联系 ZEGO 技术支持。

        场景化 AI 降噪功能，在之前针对所有非人声进行降噪的基础上，新增支持在音乐场景下的降噪能力，通过识别音乐，智能调整降噪效果还原音乐音质。SDK 会实时对麦克风输入内容进行音乐检测，在声卡、弹唱或近场音乐场景下，自动调整降噪等级，保证音乐的高保真音质，详情请参考 [场景化 AI 降噪](/real-time-video-android-java/audio/scenario-based-ai-noise-reduction)。
...
3. 新增多人状态实时同步功能
        ...
        大型虚拟世界中，用户一般不需要感知较远的场景或远端用户，ZEGO 提供 AOI（Area Of Interest）能力，减免用户可见范围外的信息，极大减少客户流量成本以及用户端流量及性能消耗，详情请参考 [多人状态实时同步](/real-time-video-android-java/communication/sync-users-status)。
...
4. 新增万人范围音视频
        ...
        依赖多人状态实时同步服务，根据云端用户位置自动拉取收听范用内的远端音视频并提供空间音效。单场景内最大支持 1 万用户同时开启麦克风及摄像头，用户默认拉取距离最近的 12 路（可配置）音视频，详情请参考 [万人范围音视频](/real-time-video-android-java/communication/mass-scale-range-audio-video)。
...
5. 新增房间维度的场景 Scenario
        ...
        当前支持场景包括秀场直播、KTV、标准 1v1 音视频通话、高画质 1v1 音视频通话、标准语聊房、高音质语聊房，详情请参考 [场景化音视频配置](/real-time-video-android-java/quick-start/scenario-based-audio-video-configuration)。
...
1. 废弃了 [ZegoScenario] 的三种旧版本场景
        ...
        废弃 [ZegoScenario] 场景枚举中的 [General]，[Communication]， [Live] 三种场景，详情请参考 [场景化音视频配置](/real-time-video-android-java/quick-start/scenario-based-audio-video-configuration)。

2. 删除了 [setDebugVerbose]、[setPlayStreamVideoLayer]、[enableAudioDataCallback] 等接口，详情请参考 [v3.0.0升级指南](/real-time-video-android-java/client-sdk/upgrade-guide/upgrade-to-v3)。
...
1. 移动端视频采集旋转功能新增 3 种模式
        ...
        当前 SDK 对各种配置进行封装，对外提供简单的模式选择，在原有自定义模式基础上，新增固定比例模式、自适应模式及对齐模式，可有效降低开发者接入成本，详情请参考 [视频采集旋转](/real-time-video-android-java/video/video-capture-rotation)。

        相关 API 请参考 [setAppOrientationMode](@setAppOrientationMode)
...
2. 提升 Express Android Java SDK 的开发体验
        ...
        为了提升 Android 端开发体验，从 v2.23.0 版本开始以 jar 包的形式随 SDK 包一并交付 Java 源码，开发者可通过 Android Studio 导入源码 jar 包以获得 API 注释，详情请参考 <a href="https://doc-zh.zego.im/faq/android-api-docs" target="_blank" rel="noopener noreferrer">Express Android SDK 如何查看 API 注释和文档？</a>。
...
3. 支持动态修改 AudioDeviceMode
        ...
        新增 [setAudioDeviceMode] 接口进行动态修改设备的音频模式，该配置决定设备的音量模式、前处理模式以及 Mic 占用逻辑，您可以根据具体场景进行选择，详情请参考 <a href="https://doc-zh.zego.im/faq/audio-device-mode-setting" target="_blank" rel="noopener noreferrer">如何设置音频设备模式 ZegoAudioDeviceMode？</a>

        相关 API 请参考 [setAudioDeviceMode](@setAudioDeviceMode)
...
4. 支持指定采集/播放的音频设备
        ...
        相关 API 请参考 [useAudioDevice](@useAudioDevice)
...
1. 废弃了旧版本的基础美颜相关接口
        ...
        相关 API 请参考 [enableBeautify](@enableBeautify), [enableEffectsBeauty](@enableEffectsBeauty), [setBeautifyOption](@setBeautifyOption), [setEffectsBeautyParam](@setEffectsBeautyParam)
...
6. 支持设置自定义采集画面的转换矩阵和旋转角度
        ...
        相关 API 请参考 [setCustomVideoCaptureRotation](@setCustomVideoCaptureRotation), [setCustomVideoCaptureTransformMatrix](@setCustomVideoCaptureTransformMatrix)
...
7. 支持设置摄像头对焦和曝光模式
        ...
        相关 API 请参考 [isCameraFocusSupported](@isCameraFocusSupported), [setCameraFocusMode](@setCameraFocusMode), [setCameraExposureMode](@setCameraExposureMode)
...
4. 去测试环境
        ...
        为了降低开发者对环境的理解成本，ZEGO 已统一环境概念，从该版本开始，废弃了测试环境，统一使用正式环境。在 2.14.0 版本之前已接入过 SDK 的开发者，可以参考 [测试环境废弃说明](https://doc-zh.zego.im/real-time-voice-android/introduction/abandon-test-environment) 进行 SDK 升级及代码调整。
...
1. 优化了混流接口
        ...
        相关 API 请参考 [ZegoMixerTask](@ZegoMixerTask)
...
2. 修复日志上传可能失败的问题
        ...
        相关 API 请参考 [uploadLog](@uploadLog)
...
5. 新增摄像头曝光补偿功能
        ...
        相关 API 请参考 [setCameraExposureCompensation](@setCameraExposureCompensation)
...
1. 兼容 Android 10 和 Android 11 对文件访问的限制
        ...
        相关 API 请参考 [setPublishWatermark](@setPublishWatermark), [startRecordingCapturedData](@startRecordingCapturedData), [loadResource](@loadResource)
...
2. 修复了当调用 [sendCustomCommand] 接口时，用户列表参数 [toUserList] 传 null 时崩溃的问题。
        ...
        相关 API 请参考 [sendCustomCommand](@sendCustomCommand)
...
3. 新增摄像头变焦功能
        ...
        相关 API 请参考 [getCameraMaxZoomFactor](@getCameraMaxZoomFactor), [setCameraZoomFactor](@setCameraZoomFactor)

4. 新增音频设备路由变更通知回调

        当有耳机插拔、扬声器和听筒切换等音频路由发生变化时会抛出此回调。

        相关 API 请参考 [onAudioRouteChange](@onAudioRouteChange)
...
5. 自定义视频采集功能新增支持 [GL_TEXTURE_EXTERNAL_OES] 类型
        ...
        相关 API 请参考 [enableCustomVideoCapture](@enableCustomVideoCapture), [sendCustomVideoCaptureTextureData](@sendCustomVideoCaptureTextureData)
...
2. 修复多次使用 [setEventHandler] 更换 `IZegoEventHandler` 后，某些回调可能引起 crash 的问题。
        ...
        相关 API 请参考 [setEventHandler](@setEventHandler)
