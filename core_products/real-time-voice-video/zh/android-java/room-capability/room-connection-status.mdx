...
- T6' ≈ T5' + 100 ms：因网络传输延迟，用户 B 约在 100 ms 后收到 [onRoomUserUpdate\|_blank](@onRoomUserUpdate) 回调以通知客户端用户 A 加入房间。
...
[Express SDK 是否支持断线重连机制？\|_blank](https://doc-zh.zego.im/faq/reconnect?product=ExpressVideo&platform=android)
