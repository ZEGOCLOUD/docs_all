...
sequenceDiagram
    ...
    客户端 App->>ZEGO Express SDK: 创建范围场景模块 <br/> createRangeScene

    客户端 App->>ZEGO Express SDK: 监听范围场景事件回调 <br/> setEventHandler

    critical 关键步骤
    客户端 App->>ZEGO Express SDK: 设置范围场景流管理参数 <br/> setReceiveRange、enableRangeSpatializer

    客户端 App->>ZEGO Express SDK: 登录场景 <br/> loginScene

    客户端 App->>ZEGO Express SDK: 推流到场景内 <br/> startPublishingStreamInScene

    客户端 App->>ZEGO Express SDK: 更新用户位置 <br/> updateUserPosition

    客户端 App->>ZEGO Express SDK: 退出场景 <br/> logoutScene
    end

    客户端 App->>ZEGO Express SDK: 销毁范围场景模块 <br/> destroyRangeScene

    客户端 App->>ZEGO Express SDK: 销毁引擎 <br/> destroyEngine
```
{/*
