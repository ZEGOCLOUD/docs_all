...
sequenceDiagram
    ...
    participant socks5代理服务器
    participant ZEGO音视频云
    note right of ZEGO音视频云:───> 由开发者处理<br>   ┄┄> 由ZEGO内部处理
    客户端APP->>ZEGO Express SDK: 设置代理
    ZEGO Express SDK-->>socks5代理服务器: 请求连接代理
    socks5代理服务器-->>ZEGO Express SDK: 连接代理成功
    ZEGO Express SDK-->>socks5代理服务器: 发送数据
    socks5代理服务器-->>ZEGO音视频云: 发送数据
    ZEGO音视频云-->>socks5代理服务器: 接收数据
    socks5代理服务器-->>ZEGO Express SDK: 接收数据
```
{/*
