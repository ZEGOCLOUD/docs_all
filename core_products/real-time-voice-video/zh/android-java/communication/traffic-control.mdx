...
SDK 会根据 [创建引擎](/real-time-video-android-java/quick-start/implementing-video-call) 时，开发者选择的场景 [scenario](/real-time-video-android-java/quick-start/scenario-based-audio-video-configuration)，自动设置适合该场景的流量控制属性。例如，秀场直播场景需要保证视频流畅性，推荐选择自适应（降低）视频分辨率；教育场景需要保证清晰度，推荐选择自适应（降低）视频帧率。
...
开启流量控制后，调用 [setMinVideoBitrateForTrafficControl](@setMinVideoBitrateForTrafficControl) 方法设置视频码率最小值（默认值为 “0”）及视频发送模式，可以使 SDK 在网络未达到发送视频的最小码率时采用该方法设置视频发送模式（不发送视频或以极低的帧率发送）。

<Warning title="注意">

可以在初始化 SDK 后且推流 [startPublishingStream](@startPublishingStream) 之前的任意时间调用 [setMinVideoBitrateForTrafficControl](@setMinVideoBitrateForTrafficControl)，但需要开启流量控制后该设置才生效。
</Warning>
