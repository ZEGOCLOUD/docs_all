...
<p>上图仅以范围语音模式为“全世界”时为例，更多不同模式组合关系下的声音可达情况请参考 <a href="#set-general-voice-mode">设置通用语音模式</a>。</p>
</td>
</tr>
<tr>
<td>3D 音效</td>
...
<Note title="说明">
<ul><li>队友之间的通话不受“范围”和 “3D 音效” 的影响，详情请查看 <a href="#set-general-voice-mode">设置通用语音模式</a>。</li><li>如果您希望自定义语音收发能力，请参考 <a href="#set-custom-voice-mode">设置自定义语音模式</a>。</li></ul>
</Note>
</td>
</tr>
</tbody></table>
...
可以根据需要调用 [setEventHandler ](@setEventHandler-ZegoRangeAudio) 接口为麦克风设置事件回调，用于监听麦克风的开启状态 [onRangeAudioMicrophoneStateUpdate ](@onRangeAudioMicrophoneStateUpdate) 通知。
...
登录房间成功后，需要通过调用 [updateAudioSource](@updateAudioSource) 接口，添加或更新发声者的位置信息。
...
2. 设置本地播放器的 3D 音效
    - 媒体播放器：
        ...
        请参考 [媒体播放器](/real-time-video-android-java/other/media-player) 了解如何创建媒体播放器与加载媒体资源。
      </Note>

        1. 调用 [createMediaPlayer](@createMediaPlayer) 接口创建媒体播放器。
        2. 调用 [ZegoMediaPlayer.updatePosition](@updatePosition-ZegoMediaPlayer-class) 接口设置媒体播放器在世界坐标系中的位置。
    ...
    - 音效播放器：
        ...
        请参考 [音效文件播放器](/real-time-video-android-java/other/audio-effect-player) 了解如何创建音效播放器与加载音效资源。
        </Note>

        1. 调用 [createAudioEffectPlayer](@createAudioEffectPlayer) 接口创建音效播放器。
        2. 调用 [ZegoAudioEffectPlayer.start](@start-ZegoAudioEffectPlayer-class) 接口播放音效资源。
        3. 在收到 [onAudioEffectPlayStateUpdate](@onAudioEffectPlayStateUpdate) 回调状态为 `ZegoAudioEffectPlayState.Playing` 后，调用 [ZegoAudioEffectPlayer.updatePosition](@updatePosition-ZegoAudioEffectPlayer-class) 接口设置正在播放的音效资源在世界坐标系中的位置。
...
#### 设置通用语音模式 <a id="set-general-voice-mode" />
...
#### 设置自定义语音模式 <a id="set-custom-voice-mode" />
