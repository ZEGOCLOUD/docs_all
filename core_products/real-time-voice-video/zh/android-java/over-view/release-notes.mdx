...
人脸识别功能支持后置摄像头，且新增回调 [onPublisherFaceDetectInfo](@onPublisherFaceDetectInfo) 支持获取人脸个数和坐标信息。<div class="mk-note">
...
混流功能支持将混流后的输出流，加入到指定房间，即支持设置输出流的目标房间信息 [setTargetRoom](@setTargetRoom)。每条输出流只支持加入一个房间，且一旦添加，混流过程中不支持动态更新房间。如需使用服务端接口实现该功能，可参考 [开始混流](#19595) 文档。
...
#### 2. 从此版本开始 Express 实时音视频 Video SDK 的 Gradle 仓库由 https://jitpack.io 变更为 https://storage.zego.im/maven，依赖的项目名称由 `com.github.zegolibrary:express-video:x.y.z` 变更为 `im.zego:express-video:x.y.z`，详情请查看 [集成 SDK](#195) 文档。
...
虚拟场景中，由于每个场景的地图大小、音视频互动玩法及规模不同，需要针对每个场景进行自定义配置。3.1.0 版本后，“万人范围音视频”及“多人实时状态同步”支持通过 SDK 接口，并使用模板 ID 指定场景。模板 ID 对应的配置项只能通过服务器 API 进行配置，详情请参考 [服务端 API - 场景模版配置](#19659)。
