...
混流功能支持将混流后的输出流，加入到指定房间，即支持设置输出流的目标房间信息 [setTargetRoom](@setTargetRoom)。每条输出流只支持加入一个房间，且一旦添加，混流过程中不支持动态更新房间。如需使用服务端接口实现该功能，可参考 [开始混流](https://doc-zh.zego.im/article/19595) 文档。
...
详情请参考 [屏幕共享](https://doc-zh.zego.im/article/9216) 文档。
...
**2.从此版本开始 Express 实时音视频 Video SDK 的 Gradle 仓库由 https://jitpack.io 变更为 https://storage.zego.im/maven，依赖的项目名称由 `com.github.zegolibrary:express-video:x.y.z` 变更为 `im.zego:express-video:x.y.z`，详情请查看 [集成 SDK](https://doc-zh.zego.im/article/195) 文档。**
