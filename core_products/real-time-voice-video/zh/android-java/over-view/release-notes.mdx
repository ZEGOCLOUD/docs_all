...
相关 API 请参考 [onMediaPlayerStateUpdate\|_blank](@onMediaPlayerStateUpdate)
...
ZEGO 为 [enableCamera\|_blank](@enableCamera__2) 接口新增 `notifyMode` 参数，用于控制关闭摄像头时是否通知本端以及远端。在切换前后置摄像头场景下，该参数可减少设备开关状态的回调次数，避免触发不必要的业务逻辑。
...
相关 API 请参考 [onEvent\|_blank](@onEvent)
...
相关 API 请参考 [onScreenCaptureStart\|_blank](@onScreenCaptureStart)
...
相关 API 请参考 [sendCustomVideoProcessedTextureData\|_blank](@sendCustomVideoProcessedTextureData)
...
相关 API 请参考 [onMediaDataPublisherFileClose\|_blank](@onMediaDataPublisherFileClose)
...
相关 API 请参考 [enableLocalCache\|_blank](@enableLocalCache), [onMediaPlayerLocalCache\|_blank](@onMediaPlayerLocalCache)
...
相关 API 请参考 [onMediaPlayerVideoSizeChanged\|_blank](@onMediaPlayerVideoSizeChanged)
...
相关 API 请参考 [enableBeforeAudioPrepAudioData\|_blank](@enableBeforeAudioPrepAudioData), [onBeforeAudioPrepAudioData\|_blank](@onBeforeAudioPrepAudioData)
...
相关 API 请参考 [onCapturedUnprocessedRawData\|_blank](@onCapturedUnprocessedRawData), [ZegoVideoBufferTypeGLTexture2DAndRawData\|_blank](@ZegoVideoBufferTypeGLTexture2DAndRawData)
...
相关 API 请参考 [startScreenCapture\|_blank](@startScreenCapture)
...
为了方便开发者使用屏幕共享功能，从 3.6.0 版本开始，在本地下载的 SDK 中提供了 AAR 包，并在 Maven 在线集成方式的 AAR 包中携带了屏幕共享的 Service 声明，开发者无需手动在自己工程中声明，详情请参考 [屏幕共享\|_blank](!ExpressVideoSDK-Other_Functions/Share_Screen)。
...
相关 API 请参考 [onMediaPlayerFirstFrameEvent\|_blank](@onMediaPlayerFirstFrameEvent)
...
#### 2. 从此版本开始 Express 实时音视频 Video SDK 的 Gradle 仓库由 https://jitpack.io 变更为 https://storage.zego.im/maven，依赖的项目名称由 `com.github.zegolibrary:express-video:x.y.z` 变更为 `im.zego:express-video:x.y.z`，详情请查看 [集成 SDK\|_blank](#195) 文档。
...
#### 2. 删除了 [setDebugVerbose]、[setPlayStreamVideoLayer]、[enableAudioDataCallback] 等接口，详情请参考 [v3.0.0升级指南\|_blank](!upgrade_guide4)。
...
相关 API 请参考 [useAudioDevice\|_blank](@useAudioDevice)
...
相关 API 请参考 [setCustomVideoCaptureRotation\|_blank](@setCustomVideoCaptureRotation), [setCustomVideoCaptureTransformMatrix\|_blank](@setCustomVideoCaptureTransformMatrix)
...
相关 API 请参考 [ZegoMixerTask\|_blank](@ZegoMixerTask)
...
相关 API 请参考 [uploadLog\|_blank](@uploadLog)
...
相关 API 请参考 [setPublishWatermark\|_blank](@setPublishWatermark), [startRecordingCapturedData\|_blank](@startRecordingCapturedData), [loadResource\|_blank](@loadResource)
...
相关 API 请参考 [sendCustomCommand\|_blank](@sendCustomCommand)
...
相关 API 请参考 [enableCustomVideoCapture\|_blank](@enableCustomVideoCapture), [sendCustomVideoCaptureTextureData\|_blank](@sendCustomVideoCaptureTextureData)
...
相关 API 请参考 [setEventHandler\|_blank](@setEventHandler)
