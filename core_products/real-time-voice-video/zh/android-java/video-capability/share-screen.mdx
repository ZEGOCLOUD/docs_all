...
如需使用屏幕共享功能，请务必 [获取用户录制屏幕授权](9216#SERVICE)，否则将无法使用该功能。
</div>
...
调用 [startScreenCapture\|_blank](@startScreenCapture) 接口共享整个系统的画面、采集第三方应用的音频。
...
调用 [updateScreenCaptureConfig\|_blank](@updateScreenCaptureConfig) 接口，可以更新屏幕共享的配置。
...
开发者需要监听 [IZegoEventHandler\|_blank](@-IZegoEventHandler) 类中的以下回调： 
- [onScreenCaptureStart\|_blank](@onScreenCaptureStart) 回调，接收开始屏幕采集成功的通知，方便做后续业务处理，如 UI 提示或应用跳转等。
- [onScreenCaptureExceptionOccurred\|_blank](@onScreenCaptureExceptionOccurred) 回调，接收屏幕共享过程中的异常信息通知，从而可分析屏幕共享失败的原因。
...
完成以上步骤之后，其他用户可以使用 [startPlayingStream\|_blank](/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#start-playing-stream) 接口拉取屏幕共享流。
