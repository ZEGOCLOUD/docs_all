...
通过调用 [startPlayingStream\|_blank](@startPlayingStream__1) 接口开始拉流。
...
如果不是使用 ZEGO SDK 进行推流，而是使用第三方推流工具直接进行推流、但是使用 ZEGO SDK 进行拉流，这种场景下推流方没有使用 ZEGO SDK 加入房间，拉流方默认收不到 [onRoomStreamUpdate\|_blank](@onRoomStreamUpdate) 的回调，可以使用 [增加房间流\|_blank](!room/AddStream) 与 [删除房间流\|_blank](!room/DeleteStream) 的功能，让拉流端可以收到 [onRoomStreamUpdate \|_blank](@onRoomStreamUpdate) 的回调。
</div>
