...
根据需要调用 [ZegoRangeScene.setEventHandler\|_blank](@setEventHandler-ZegoRangeScene) 
接口设置范围场景，用于监听范围场景状态、登录状态、进出 AOI 通知等。调用 [ZegoRangeSceneItem.setEventHandler\|_blank](@setEventHandler-ZegoRangeSceneItem) 接口设置范围场景物品管理类事件回调，用于监听物品进入离开 AOI 范围、物品绑定状态变更、物品状态和命令更新事件。
