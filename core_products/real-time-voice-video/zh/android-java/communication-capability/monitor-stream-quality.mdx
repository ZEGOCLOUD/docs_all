...
[onNetworkQuality\|_blank](@onNetworkQuality) 不适用于使用 CDN 进行直播的场景，您可以参考 [进阶质量报告 - 推流质量报告\|_blank](!Publisher_Player_Advanced/MonitorStreamQuality#5_1) 监测 CDN 的推流质量。  
</div>
...
您可以通过注册 [onPublisherQualityUpdate\|_blank](@onPublisherQualityUpdate) 接收推流质量回调，推流成功后每隔三秒会收到此回调。可根据 quality（[ZegoPublishStreamQuality\|_blank](@-ZegoPublishStreamQuality)) 参数实时了解推送的音视频流的健康情况。

- 大多数情况下，您只需关注 “quality” 的 “level” 参数，以 “level” 枚举值来判断推流的综合质量，详情可参考 [ZegoStreamQualityLevel\|_blank](@-ZegoStreamQualityLevel)。
- 如果您想关注更详细的推流质量参数，可以参考 [ZegoPublishStreamQuality\|_blank](@-ZegoPublishStreamQuality)。
...
您可以通过注册 [onPlayerQualityUpdate\|_blank](@onPlayerQualityUpdate) 接收拉流质量回调，拉流成功后每隔三秒会收到此回调。开发者可根据 quality（[ZegoPlayStreamQuality\|_blank](@-ZegoPlayStreamQuality)) 参数实时了解拉取的音视频流的健康情况。

- 大多数情况下，您只需关注 “quality” 的 “level” 参数，以 “level” 枚举值来判断拉流的综合质量，详情可参考 [ZegoStreamQualityLevel\|_blank](@-ZegoStreamQualityLevel)。
- 如果您想关注更详细的拉流质量参数，可以参考 [ZegoPlayStreamQuality\|_blank](@-ZegoPlayStreamQuality)。
...
在推流成功后，您可以通过 [onPublisherStateUpdate\|_blank](@onPublisherStateUpdate) 获取推流状态变更的通知。
...
您可以通过注册 [onPlayerRecvVideoFirstFrame\|_blank](@onPlayerRecvVideoFirstFrame) 监听拉流端接收视频首帧回调。调用拉流接口成功后，SDK 拉流拉到第一帧视频数据时会收到此回调。
...
您可以通过注册 [onPlayerRenderVideoFirstFrame\|_blank](@onPlayerRenderVideoFirstFrame) 监听拉流端渲染完视频首帧回调。调用拉流接口成功后，SDK 拉流并渲染完第一帧视频数据后会收到此回调。
