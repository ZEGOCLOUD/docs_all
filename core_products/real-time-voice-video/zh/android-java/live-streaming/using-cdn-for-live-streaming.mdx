...
为防止攻击者盗取或伪造您的推流 URL 地址，您可以参考 [CDN 推流鉴权](/real-time-video-android-java/live-streaming/cdn-stream-publishing-authentication)，提升您推流使用的安全性。
...
<Note title="说明">
若选择使用直推 CDN 功能，则无需执行本节所有步骤，请参考 [直推 CDN](/real-time-video-android-java/live-streaming/using-cdn-for-live-streaming)。
</Note>

### 初始化和登录房间

请参考 [快速开始 - 实现流程](/real-time-video-android-java/quick-start/implementing-video-call) 的 “创建引擎”、“登录房间”。

### 开始推流

请参考 [快速开始 - 实现流程](/real-time-video-android-java/quick-start/implementing-video-call) 的 “推流”。
...
转推 CDN 信息 [ZegoStreamRelayCDNInfo](@-ZegoStreamRelayCDNInfo) 包含了 CDN 推流的 URL、转推状态、转推状态变更的原因、状态发生的时间。[ZegoStreamRelayCDNInfo](@-ZegoStreamRelayCDNInfo) 内所有参数如下：
...
调用 [removePublishCdnUrl](@removePublishCdnUrl) 接口即可删除动态转推至 CDN 的 URL。**调用 [removePublishCdnUrl](@removePublishCdnUrl) 接口停止转推时，请确保当前流 streamID 是存在的。**
...
若选择使用转推 CDN 功能，则无需执行本节所有步骤，请参考 [转推 CDN](/real-time-video-android-java/live-streaming/using-cdn-for-live-streaming)。
</Note>
...
- 当推流方直推 CDN 时，拉流方可以直接通过 streamID 进行拉流，请参考 [快速开始 - 实现流程](/real-time-video-android-java/quick-start/implementing-video-call) 的 “拉流”。

- 当音视频流转推 CDN 成功后，开发者希望用户从 CDN 进行拉流时，需要使用传入 URL 的自定义拉流方式进行拉流，而不能通过流 ID 进行拉流。URL 拉流的操作步骤可参考 [通过 URL 拉流](/real-time-video-android-java/live-streaming/playing-stream-by-url) 中的 “1 配置拉流参数” 和 “2 开始拉流”。
