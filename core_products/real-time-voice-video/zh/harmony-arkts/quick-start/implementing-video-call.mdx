...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成](/real-time-video-harmonyos-arkts/quick-start/integrating-sdk)。
...
调用 [createEngine ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/ets_ohos/classes/_zegoexpressengine_.zegoexpressengine.html#createengine) 方法，将 [前提条件](/real-time-video-harmonyos-arkts/quick-start/implementing-video-call) 中申请到的 AppID 传入参数 “appID”，创建引擎单例对象。

<Note title="说明">


如果需要将日志信息输出到 DevEco Studio 的 HiLog 窗口，可参考 [常见问题](/real-time-video-harmonyos-arkts/quick-start/implementing-video-call) 中的“鸿蒙系统如何打印调试日志？”实现。

</Note>


<Warning title="注意">
创建引擎时，[ZegoEngineProfile](http://doc-zh.zego.im/unique-api/express-video-sdk/zh/ets_ohos/classes/_zegoexpressdefines_.zegoengineprofile.html) 必须传入 applicationContext 作为上下文对象，以确保注册的回调能够正常触发。
</Warning>
...
详情可参考 [房间连接状态说明 ](/real-time-video-harmonyos-arkts/room/room-connection-status)。
...
调用登录房间接口之后，您可通过监听 [onRoomStateUpdate ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/ets_ohos/interfaces/_zegoexpresseventhandler_.zegoeventlistener.html#onroomstateupdate) 回调实时监控自己在本房间内的连接状态，详情请参考 [房间内的连接状态变化通知](/real-time-video-harmonyos-arkts/quick-start/implementing-video-call)。
...
如果您在音视频通话的过程中，遇到相关错误，可查询 [错误码](/real-time-video-harmonyos-arkts/client-sdk/error-code)。
