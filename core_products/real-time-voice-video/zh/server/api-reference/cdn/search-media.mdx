...
<Warning title="注意">
首次使用本接口之前，请确认是否已经开通 CDN 直播服务。若未开通，请前往 [ZEGO 控制台](https://console.zego.im) 自助开通，详情请参考 [控制台 - 服务配置 - CDN](/console/service-configuration-2/activate-cdn-service)，或联系 ZEGO 技术支持开通。
</Warning>
...
| └ FileId | String | 文件唯一标识，即 <a href="!Callback/Record_File_Create_Callback" target="blank">录制文件生成回调</a> 中的 FileID。 |
...
| Key | String | 媒体文件的唯一标识，即 <a href="!Callback/Record_File_Create_Callback" target="blank">录制文件生成回调</a> 中的 ObsObject。 |
...
|---|---|---|
| ETag | String | 媒体文件的 md5 值。 |
| Key | String | 媒体文件的唯一标识，即 <a href="!Callback/Record_File_Create_Callback" target="blank">录制文件生成回调</a> 中的 PersistentId。 |
| LastModified | String | 媒体文件的最后一次修改时间。 |
| Size | Number | 媒体文件的大小。 |
| MediaUrl | String | 媒体文件的地址。 |
