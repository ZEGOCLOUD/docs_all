...
- 参考 [使用 Postman 调测指南\](!Server_APIs_v2-Postman/Postman)，使用 ZEGO 提供的 Postman API 集合，使用 Postman 快速调试相应接口。
...
我们根据您的签名验证了请求后，会返回结果给您。接口调用成功会显示返回参数，调用失败则显示相应报错，您可以根据 [全局返回码\](!Server_APIs_v2-Return_Codes/Return_Codes) 分析排查。

<details class="zg-primary">
    <summary>服务端 API 请求示例</summary>

以下为 [`StartMix`\](#8791)（开始混流）接口的请求结构为例：
...
此外，ZEGO 提供多种编程语言的示例代码，开发者可根据实际情况进行参考。以下是 [`StartMix`\](#8791)接口的 Node.js 代码：
