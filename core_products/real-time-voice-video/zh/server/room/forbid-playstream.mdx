...
<tr>
<td><a href="!forbid_playstream#3_1">设置音视频流封禁规则</a></td>
<td rowspan="3">GET</td>
...
<tr>
<td><a href="!forbid_playstream#3_2">查询音视频流封禁规则</a></td>
<td>https://rtc-api.zego.im/?Action=<b>DescribeForbidStreamRules</b></td>
</tr>
<tr>
<td><a href="!forbid_playstream#3_3">删除音视频流封禁规则</a></td>
...
- 响应结果中，Code 为 0 表示设置成功，不为 0 表示设置失败，请参考 [4 返回码](!forbid_playstream#4) 进行处理。
- 设置成功后，您可以 [查询音视频流封禁规则](!forbid_playstream#3_2) 或 [删除音视频流封禁规则](!forbid_playstream#3_3)。
</div>
...
调用 `DescribeForbidStreamRules` 接口，可以查询到当前处于有效期内的规则，也可以查询到已失效的规则。<b>请注意，ZEGO 会不定期清理数据库中的已失效的封禁规则，调用本接口查询得到的失效规则列表可能不完整。</b></p>
</div>
...
<td>ForbidStreamRuleList</td>
<td>Object</td>
<td>查询到的音视频流封禁规则信息列表。</td>
</tr>
...
<td>DisableAudio</td>
<td>Int32</td>
<td><p>流 ID 对应的音频是否封禁。</p><ul><li>0：不封禁。</li><li>1：封禁。</li></ul></td>
</tr>
<tr data-row-level="5-2-3">
<td>DisableVideo</td>
<td>Int32</td>
<td><p>流 ID 对应的视频是否封禁。</p><ul><li>0：不封禁。</li><li>1：封禁。</li></ul></td>
</tr>
<tr data-row-level="5-2-4">
<td>CreateTime</td>
<td>Int64</td>
<td>音视频流封禁规则的创建时间，Unix 时间戳，单位：秒。</td>
</tr>
<tr data-row-level="5-2-5">
<td>EffectiveTime</td>
<td>Int64</td>
<td>音视频流封禁规则的有效时间，Unix 时间戳，单位：秒。</td>
</tr>
</tbody></table>
...
- 响应结果中，Code 为 0 表示查询成功，不为 0 表示查询失败，请参考 [4 返回码](!forbid_playstream#4) 进行处理。
...
- 响应结果中，Code 为 0 表示删除成功，不为 0 表示删除失败，请参考 [4 返回码](!forbid_playstream#4) 进行处理。
- 您也可以通过 [查询音视频流封禁规则](!forbid_playstream#3_2) 查看对应的音视频流封禁规则是否删除成功。
