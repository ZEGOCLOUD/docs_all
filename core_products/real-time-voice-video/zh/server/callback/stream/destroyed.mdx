...
<Warning title="注意">
停止混流时不会触发此回调，监听混流相关的回调请参考 [混流停止回调](./../stream-mixing/stopped.mdx)。
</Warning>
...
<td>signature</td>
<td>String</td>
<td>检验串，请参考 <a target="_blank" href="/real-time-video-server/callback/receiving-callback">回调说明 - 检验说明</a>。</td>
</tr>
<tr>
<th>业务参数</th>
<th>类型</th>
...
<td>type</td>
<td>String</td>
<td>关闭类型。<ul><li>0：正常关闭（用户调用 SDK 的 <a href="https://doc-zh.zego.im/real-time-video-android-java/client-sdk/api-reference/class#stoppublishingstream" target="blank">stopPublishingStream</a> 接口停止推流）。</li><li>非 0 为异常关闭：<ul><li>1：用户心跳超时关闭。</li><li>2：用户重复登录同一个房间，关闭上次登录会话的流。</li><li>3：服务端接口 kickout 用户，关闭用户创建的流。</li><li>4：tcp 掉线关闭流（可配置通知时间）。</li><li>5：房间被清除关闭流。</li><li>100：调用服务端接口 <a href="/real-time-video-server/api-reference/room/delete-stream" target="blank">DeleteStream</a> 关闭流。</li></ul></li></ul></td>
</tr>
<tr>
<td>stream_alias</td>
<td>String</td>
...
<td>
流关闭时的流附加信息。
客户端可通过 [setStreamExtraInfo](/real-time-video-android-java/client-sdk/api-reference/class#setstreamextrainfo-zegoexpressengine) 设置流附加信息。

<Warning title="注意">
如果在流关闭前，客户端未设置流附加信息，则该字段为空字符串。
</Warning>
</td>
</tr>
<tr>
