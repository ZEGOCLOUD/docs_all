...
当开发者需要了解当前 App 的单流转码任务，可以参考 [回调配置说明](/real-time-video-server/callback/receiving-callback) 配置相关回调接口。当发起 [开始单流转码](/real-time-video-server/api-reference/stream-mixing/start-cloud-transcode) 请求成功后，可以通过该回调，查看转码任务的结果。
