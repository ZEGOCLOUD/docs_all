...
开发者通过 [开始点播转码](/real-time-video-server/api-reference/cdn/start-on-demand-transcoding) 接口完成媒体文件转码后，可以通过该回调、或 [查询媒体文件任务](/real-time-video-server/api-reference/cdn/describe-media-task) 接口，查看转码任务的状态、文件回放地址等详细信息。
...
| nonce | String | 随机数。 |
| signature | String | 检验串，详情见 [检验说明](/real-time-video-server/callback/receiving-callback)。 |

| 业务参数 | 类型 | 描述 |
|---|---|---|
| task_id | String | 转码任务的唯一标识 ID，与 <a href="/real-time-video-server/api-reference/cdn/start-on-demand-transcoding" target="balnk">开始点播转码</a> 响应结果中的 task_id 保持一致。 |
...
| video | Object | 转码后的视频信息，与 <a href="/real-time-video-server/api-reference/cdn/start-on-demand-transcoding" target="balnk">开始点播转码</a> 请求参数中的 Resolution 配置相关。 |
...
| audio | Object | 转码后的音频信息，与 <a href="/real-time-video-server/api-reference/cdn/start-on-demand-transcoding" target="balnk">开始点播转码</a> 请求参数中的 Resolution 配置相关。 |
