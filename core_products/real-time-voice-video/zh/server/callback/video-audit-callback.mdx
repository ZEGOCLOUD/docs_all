...
- 请求地址：请参考 [控制台 - 云市场 - 数美内容审核](https://doc-zh.zego.im/article/18891)，按照页面指引，完成相关回调地址的配置。
...
| TaskId | String | 审核任务的 TaskId。<br/>对应 [开始视频流审核](!Interface_audit/Start_video_audit) 响应参数中返回的 TaskId。 |
...
| Code | Number | 返回码。0 为成功。 |
| Message | String | 操作结果描述。 |
| TaskId | String | 审核任务的 TaskId。<br/>对应 [开始视频流审核](!Interface_audit/Start_video_audit) 中响应参数中返回的 TaskId。 |
| ResultTaskId | String | 此次回调信息的 ID。 |
...
| Message | String | 操作结果描述。 |
| TaskId | String | 审核任务的 TaskId。<br/>对应 [开始视频流审核](!Interface_audit/Start_video_audit) 返回的 TaskId。 |
...
| Code | Number | 返回码，0 表示成功。 |
| Message | String | 操作结果描述。 |
| TaskId | String | 审核任务的 TaskId，对应 [开始视频流审核](!Interface_audit/Start_video_audit) 返回的 TaskId。 |
| Status | Number | 审核状态。<br/>0：审核结束。 |
| AuxInfo | Object | 辅助信息。 |
| └ RoomId | String | 房间 ID。 |
| └ CensorStreamTime | Number | 本次任务审核流的总时长（单位：秒）。 |
