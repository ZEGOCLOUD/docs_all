...
4. 加载示例工程后，控制台会报大量错误，原因是下载的示例源码中缺少 SDK，因此需要先 [下载 SDK 包](/real-time-video-cocos-creator/client-sdk/download-sdk) （无需解压）。
...
7. 示例源码中缺少 SDK 初始化所需的 AppID 和 AppSign，请使用 VS Code 打开示例源码工程，修改 assets/KeyCenter.ts 文件，请使用本文 [前提条件](/real-time-video-cocos-creator/quick-start/run-example-code) 已获取的 AppID 和 AppSign 正确填写，否则示例源码无法正常运行。
...
<Warning title="注意">
        ...
        - 若要构建 iOS 模拟器架构，请参考 [集成 SDK](/real-time-video-cocos-creator/quick-start/integrating-sdk) 文档 “2.2 导入 SDK” 中关于 iOS 工程的额外处理步骤。
        - 若生成失败，请参考 [集成 SDK](/real-time-video-cocos-creator/quick-start/integrating-sdk) 文档中的常见问题 7 进行操作。
