...
- [Cocos Creator\|_blank](https://www.cocos.com/creator-download) v3.6.0 或以上版本（建议通过 Cocos Dashboard 下载最新的版本）。
...
- 根据需要运行的平台，确保对应的开发环境和设备。
  ...
  - 满足 Express Web SDK 兼容性的浏览器（具体请参考 [浏览器兼容性和已知问题\|_blank](#12047)），推荐使用最新版本的 Google Chrome 浏览器。
...
<img src="/Pics/CocosCreator/Common/cocos_dashboard_new_project.png">
...
1. 请参考 [下载 SDK\|_blank](!DownloadSDK/DownloadSDK) 文档，下载最新版本的 SDK。

2. 打开 “Extensions > Extensions Manager”，在项目选项卡下，单击加号，添加扩展，选择上一步下载下来的 SDK 压缩包。

    <img src="/Pics/CocosCreator/Express/cocos_express_extension_manager.png">
...
3. 添加完成后，确认扩展管理器中有 zego_express_cocos_creator_sdk 并处于启用状态，然后查看资源管理器，在 assets 目录下，应当存在一个 “zego_express_engine” 目录，里面为 SDK 的 TypeScript 层源码。
    ...
    <img src="/Pics/CocosCreator/Express/cocos_express_scripts_in_assets.png">
    ...
    <img src="/Pics/CocosCreator/Express/cocos_express_gitignore.png">

4. 先根据 Cocos Creator [安装配置原生开发环境\|_blank](https://docs.cocos.com/creator/manual/zh/editor/publish/setup-native-development.html) 文档配置好开发环境，然后再根据 [发布到原生平台\|_blank](https://docs.cocos.com/creator/manual/zh/editor/publish/native-options.html) 文档和实际情况，构建出原生平台工程。
...
5. 将 “native/engine” 原生工程配置文件纳入 git 跟踪。
    ...
    <a name="step6"></a>
    <img src="/Pics/CocosCreator/Common/cocos_gitignore_native_engine.png">
...
6. 部分平台需要做一些额外处理。
    ...
    - Android
        ...
        若使用 macOS 开发，请勿直接打开 Android Studio，而是需要打开终端，并输入 `open -a "Android Studio"` 来启动 Android Studio 以解决潜在问题，请参考 [Issue\|_blank](https://github.com/cocos/cocos-engine/issues/12456#issuecomment-1220338692) 以获得更多信息。
        ...
        <img src="/Pics/CocosCreator/Express/cocos_express_gradle_jni_libs.png">
        ...
        <img src="/Pics/CocosCreator/Express/cocos_express_gradle_jar.png">
    ...
    - iOS
        ...
        <img src="/Pics/CocosCreator/Common/cocos_cmake_set_ios_variant.png">
        ...
        <img src="/Pics/CocosCreator/Common/cocos_ios_target_os_config.png">
...
以下简单示例在 App 启动时申请权限，也可以根据实际情况在合适的时机申请（在调用 SDK 的 [createEngine\|_blank](@createEngine) 前申请即可）。
...
1. 在 Xcode 中，选择 “TARGETS > Info > Custom iOS Target Properties” 菜单。
    ...
    <img src="/Pics/iOS/ZegoExpressEngine/Common/privacy-description.png">
...
2. 单击 “+” 添加按钮，根据实际应用需要，添加摄像头和麦克风权限。
    ...
    <img src="/Pics/iOS/ZegoExpressEngine/Common/privacy-description-done.png">
...
<img src="/Pics/CocosCreator/Common/cocos_apple_privacy_info_plist.png">
...
2. 若发现有 “NodeJS is not found in $PATH” 的输出，说明 Android Studio 内执行的 cmake 无法找到 node 导致 SDK 无法加载，请参考 [Issue\|_blank](https://github.com/cocos/cocos-engine/issues/12456#issuecomment-1220338692)。
...
<img src="/Pics/CocosCreator/Express/cocos_express_android_src.png">
...
<img src="/Pics/CocosCreator/Common/cocos_project_setting_antialias.png">
...
Cocos Creator 3.6.2 版本存在一个 [bug\|_blank](https://github.com/cocos/cocos-engine/issues/13696)，建议使用 Cocos Creator 3.6.1 / 3.6.3 或更高版本，或者您也可以根据 Cocos 引擎的 [Pull Request\|_blank](https://github.com/cocos/cocos-engine/pull/13721) 自行修复处理。
