...
1. Android 新增 engineUpdateState 状态通知的回调。

相关 API 请参考 [engineUpdateState](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/javascript_react-native/interfaces/_zegoexpresseventhandler_.zegoeventlistener.html#enginestateupdate)

**改进优化**

1. 更新集成 Express Native SDK 至 3.22.0 版本。
...
1. 删除 AndroidManifest.xml 文件中声明的屏幕共享默认权限
    ...
    详情请参考 [屏幕共享](/real-time-video-rn/video/screen-sharing) 文档。
...
1. 废弃拉流接口中关于 CDN Plus 的配置
    ...
    下架畅直播概念，废弃 CDN Plus 直播相关接口。如需实现直播功能，推荐使用 ZEGO 自研 [超低延迟直播产品](/live-streaming-rn/introduction/overview)，实现更高质量的直播体验。
...
1. 新增本地媒体录制功能
    ...
    开发者可将音视频流录制成本地文件，便于日后作回放等用途，详情请参考 [音视频录制](/real-time-video-rn/other/local-media-recording)。
...
2. 去测试环境
    ...
    为了降低开发者对环境的理解成本，ZEGO 已统一环境概念，从该版本开始，废弃了测试环境，统一使用正式环境。在 1.5.0 版本之前已接入过 SDK 的开发者，可以参考 [测试环境废弃说明](https://doc-zh.zego.im/real-time-voice-android/introduction/abandon-test-environment) 进行 SDK 升级及代码调整。
