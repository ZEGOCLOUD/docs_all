...
sequenceDiagram
    ...
    Note over APP,ZEGO Express SDK: 创建 ZegoExpressEngine 引擎
    APP ->> ZEGO Express SDK: createEngineWithProfile

    Note over APP,ZEGO Express SDK: 设置自定义视频渲染配置并开启
    APP ->> ZEGO Express SDK: enableCustomVideoRender

    Note over APP,ZEGO Express SDK: 设置自定义视频渲染器对象并实现回调方法
    APP ->> ZEGO Express SDK: setCustomVideoRenderHandler(IZegoCustomVideoRenderHandler)

    Note over APP,ZEGO Express SDK: 登录房间后开始预览/拉流, 收到<br/>自定义视频渲染视频帧数据回调
    ...
    Note over APP,ZEGO Express SDK: 停止预览或停止拉流
    APP ->> ZEGO Express SDK: stopPreview / stopPlayingStream

    Note over APP,ZEGO Express SDK: 退出房间
    APP ->> ZEGO Express SDK: logoutRoom

    Note over APP,ZEGO Express SDK: 销毁引擎
...
### 1 设置自定义视频渲染配置
 ...
 ####  创建 [ZegoCustomVideoRenderConfig](@-ZegoCustomVideoRenderConfig) 对象并配置参数。

`bufferType` 参数是枚举 [ZegoVideoBufferType](@-ZegoVideoBufferType)，可指定开发者需要的自定义视频渲染视频帧数据类型，目前仅支持裸数据类型 `ZegoVideoBufferTypeRawData`、CVPixelBuffer 类型 `ZegoVideoBufferTypeCVPixelBuffer` 及 `ZegoVideoBufferTypeEncodedData`，设置其他不支持的枚举值，将不会回调视频帧数据。

`frameFormatSeries` 参数是枚举 [ZegoVideoFrameFormatSeries](@-ZegoVideoFrameFormatSeries)，可指定开发者需要的自定义视频渲染视频帧数据格式，此参数只能指定 RGB 或 YUV 颜色空间大类，具体的数据格式不同平台间不一致，以回调中的参数为准。
<Warning title="注意">
...
[enableEngineRender](@enableEngineRender) 表示是否在要自定义视频渲染的同时，SDK 内部也渲染。设置为 "NO" 时，引擎不会在预览接口 [startPreview](@startPreview) 和拉流接口 [startPlayingStream](@startPlayingStream) 设置的 View 上渲染。
...
#### 调用 [enableCustomVideoRender](@enableCustomVideoRender) 接口开启自定义视频渲染。

创建 [ZegoCustomVideoRenderConfig](@-ZegoCustomVideoRenderConfig) 对象并配置参数后，再调用 [enableCustomVideoRender](@enableCustomVideoRender) 接口开启自定义视频渲染。
...
调用 [setCustomVideoRenderHandler](@setCustomVideoRenderHandler) 接口设置自定义视频渲染回调。
...
- 接口原型
    ...
    其中的自定义视频渲染回调协议 [ZegoCustomVideoRenderHandler](@-ZegoCustomVideoRenderHandler) 定义如下：
...
<Note title="说明">
本地预览采集视频帧回调方法中的 `flipMode` 参数与镜像有关，通知开发者是否需要自行将视频帧画面做翻转，以使画面符合 [setVideoMirrorMode](@setVideoMirrorMode) 中设置的 [ZegoVideoMirrorMode](@-ZegoVideoMirrorMode) 枚举值的描述。
</Note>

以上回调方法中的 `param` 参数（[ZegoVideoFrameParam](@-ZegoVideoFrameParam) 对象）描述了该视频帧的一些参数，定义如下：
...
推流方首先需要调用启动预览接口，才能收到自定义视频渲染视频帧数据回调，如果 [ZegoCustomVideoRenderConfig](@-ZegoCustomVideoRenderConfig) 自定义视频渲染配置的 "enableEngineRender" 参数为 "NO"，启动预览的 `canvas` 参数可以传空，启动预览后即可开始推流。
...
2. **如果自定义视频渲染在配置 [ZegoCustomVideoRenderConfig](@-ZegoCustomVideoRenderConfig) 时 `enableEngineRender` 参数为 "NO" 时，预览接口 [startPreview](@startPreview) 及拉流接口 [startPlayingStream](@startPlayingStream) 中的 "canvas" 参数填什么？**
...
4. **本地采集预览自定义视频渲染的视频帧的宽高是多少？**
    ...
    本地采集预览自定义视频渲染的视频帧的宽高会在回调中的 [param](@-ZegoVideoFrameParam) 参数里返回，其值与 [setVideoConfig](@setVideoConfig) 设置的分辨率相同。

5. **自定义视频渲染出来的视频帧数据格式是什么？是否支持 YUV？**

    根据选择的渲染数据格式系列 [ZegoVideoFrameFormatSeries](@-ZegoVideoFrameFormatSeries) 以及软解/硬解码后直接返回的数据决定，即视频帧回调方法中的 [param](@-ZegoVideoFrameParam) 参数里的 `format` ([ZegoVideoFrameFormat](@-ZegoVideoFrameFormat)) 参数。

    视频帧数据格式 [ZegoVideoFrameFormat](@-ZegoVideoFrameFormat) 说明：
...
7. **自定义视频渲染如何拿到第一帧数据呢？**
    ...
    自定义视频渲染回调 [ZegoCustomVideoRenderHandler](@-ZegoCustomVideoRenderHandler) 回调第一次返回的数据即第一帧数据。

8. **是否支持可以本地采集预览渲染我们自己做，拉流渲染由 ZEGO SDK 做呢？**

    支持，自定义视频渲染在配置 [ZegoCustomVideoRenderConfig](@-ZegoCustomVideoRenderConfig) 时将 `enableEngineRender` 参数设为 "YES" 时，不仅自定义视频渲染器会回调视频帧数据，同时 SDK 会往预览接口 [startPreview](@startPreview) 及拉流接口 [startPlayingStream](@startPlayingStream) 中 `canvas` 里 View 上做内部渲染。

9. **渲染预览视图时，为什么收不到视频数据的回调？**

    如果需要渲染预览视图，需要在推流前先启动预览 [startPreview](@startPreview)，否则收不到视频数据的回调。
...
- [怎么处理视频黑屏问题？](https://doc-zh.zego.im/faq/video-black-screen)
