...
- 如果您已经通过 ZEGO 旧版 [屏幕共享](https://doc-zh.zego.im/article/11151) 产品实现屏幕共享功能，或者需要自己实现屏幕共享功能，请参考 [如何通过自定义采集实现屏幕共享？](http://doc-zh.zego.im/faq/Express_Share_Screen?product=HybridHierarchicalDeliverySystem&platform=windows)。
...
屏幕共享朝向支持跟随系统朝向和固定朝向，默认跟随系统朝向。可以设置 [ZegoScreenCaptureConfig](@-ZegoScreenCaptureConfig) 中的 `orientation` 参数来设置屏幕共享朝向。
...
如下系统回调的实现，可以在 [下载示例源码](/real-time-video-ios-oc/quick-start/run-example-code) 中的 “/ZegoExpressExample/Examples/Others/ScreenSharing/ZegoExpressExample-Broadcast/SampleHandler.m” 文件中查看：
