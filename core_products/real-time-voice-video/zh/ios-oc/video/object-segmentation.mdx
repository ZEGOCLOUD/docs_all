...
sequenceDiagram
    ...
    推流端->>ZEGO Express SDK: 设置渲染的 view <br/> setBackgroundColor

    推流端-->>ZEGO Express SDK: (可选) 自定<br>义渲染及回调 <br/> enableCustomVideoRender、<br>setCustomVideoRenderHandler
    ...
    推流端-->>ZEGO Express SDK: (可选) 使用 Alpha<br>通道传输分割出的主体 <br/> enableAlphaChannelVideoEncoder

    推流端->>ZEGO Express SDK: 开始预览和推流 <br/> startPreview、<br>startPublishingStream

    ZEGO Express SDK-->>拉流端: (可选) 在拉流端设置<br>Alpha 通道渲染并拉流 <br/> ZegoCanvas.alphaBlend = YES、<br>startPlayingStream

    推流端->>ZEGO Express SDK: 关闭主体分割 <br/> enableVideoObjectSegmentation

    推流端->>ZEGO Express SDK: 销毁引擎 <br/> destroyEngine

    ZEGO Express SDK->>拉流端: 销毁引擎 <br/> destroyEngine
```
{/*
...
初始化和登录房间的具体流程，请参考实现视频通话文档中的 “[创建引擎](/real-time-video-ios-oc/quick-start/implementing-video-call)”及“[登录房间](/real-time-video-ios-oc/quick-start/implementing-video-call)”。
...
调用 [enableCustomVideoRender](@enableCustomVideoRender) 接口，设置引擎进阶配置并开启自定义渲染，同时调用 [setCustomVideoRenderHandler](@setCustomVideoRenderHandler) 接口设置自定义视频渲染回调，详情请参考 [自定义视频渲染](/real-time-video-ios-oc/video/custom-video-rendering)。
