...
sequenceDiagram
    ...
    推流客户端 App->>ZEGO Express SDK: 初始化 SDK <br/> createEngineWithProfile
    推流客户端 App->>ZEGO Express SDK: 登录房间 <br/> loginRoom
    推流客户端 App->>ZEGO Express SDK: 开始推流 <br/> startPublishingStream

    拉流客户端 App->>ZEGO Express SDK: 初始化 SDK <br/> createEngineWithProfile
...
1. 调用 [createEngineWithProfile](@createEngineWithProfile) 接口创建 engine 对象。
...
2. 创建 [ZegoEventHandler](@-ZegoEventHandler) 对象，并重写接收 SEI 信息的 [onPlayerRecvSEI](@onPlayerRecvSEI) 方法，调用 [setEventHandler](@-ZegoEventHandler) 接口传入创建的 [ZegoEventHandler](@-ZegoEventHandler) 监听 [onPlayerRecvSEI](@onPlayerRecvSEI) 的回调。

3. 调用 [loginRoom](@loginRoom) 接口登录房间。

4. 调用 [startPlayingStream](@startPlayingStream) 接口拉流。

5. 在拉流成功后，接收到推流端发送的 SEI 信息之后触发 [onPlayerRecvSEI](@onPlayerRecvSEI) 回调。
