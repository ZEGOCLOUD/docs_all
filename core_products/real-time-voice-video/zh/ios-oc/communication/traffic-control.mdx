...
当只有一个 [RTC 推流](/glossary/term-explanation#rtc-publishing-stream) 的情况下，只有一个 [RTC 拉流](/glossary/term-explanation#rtc-playing-stream) 或只有一个 [L3 拉流](/glossary/term-explanation#rtc-playing-stream) 时，SDK 会自动开启下行流量控制。拉流端将自己的网络情况通知给推流端，推流端估算自己的上行带宽和拉流端的下行带宽，并从上行、下行带宽的估值中取最小值来调整自己的上行码率，以保证连麦的流畅性。
...
SDK 会根据 [创建引擎](/real-time-video-ios-oc/quick-start/implementing-video-call) 时，开发者选择的场景 [`scenario`](/real-time-video-ios-oc/quick-start/scenario-based-audio-video-configuration)，自动设置适合该场景的流量控制属性。例如，秀场直播场景需要保证视频流畅性，推荐选择自适应（降低）视频分辨率；教育场景需要保证清晰度，推荐选择自适应（降低）视频帧率。
