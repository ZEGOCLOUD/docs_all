...
<p>上图仅以范围语音模式为"全世界"时为例，更多不同模式组合关系下的声音可达情况请参考 [范围语音模式说明](/real-time-video-ios-oc/client-sdk/api/enum/zegorangeaudiomode)。</p>
</td>
</tr>
<tr>
<td>3D 音效</td>
...
<Note title="说明">
<ul><li>队友之间的通话不受"范围"和 "3D 音效" 的影响，详情请查看 [范围语音模式说明](/real-time-video-ios-oc/client-sdk/api/enum/zegorangeaudiomode)。</li><li>如果您希望自定义语音收发能力，请参考 [自定义语音模式相关API](/real-time-video-ios-oc/client-sdk/api/class/zegorangeaudio)。</li></ul>
</Note>
</td>
</tr>
</tbody></table>
...
sequenceDiagram
    ...
    A->>B: 创建范围语音实例对象 <br/> createRangeAudio
    A->>B: 监听范围语音事件回调 <br/> setEventHandler
    A->>B: 登录房间 <br/> loginRoom
    critical 关键步骤
        A->>B: 设置收听者的当前位置 <br/> updateSelfPosition
        A->>B: 添加/更新发声者位置信息 <br/> updateAudioSource
        A->>B: 开启麦克风、扬声器 <br/> enableMicrophone、enableSpeaker
        B-->>A: 回调麦克风使用状态 <br/> microphoneStateUpdate
    end

    A->>B: 销毁范围语音模块 <br/> destroyRangeAudio
...
2. 设置本地播放器的 3D 音效
    - 媒体播放器:
        ...
        请参考 [媒体播放器](/real-time-video-ios-oc/other/media-player) 了解如何创建媒体播放器与加载媒体资源。
        </Note>
    ...
    - 音效播放器:
        ...
        请参考 [音效文件播放器](/real-time-video-ios-oc/other/audio-effect-player) 了解如何创建音效播放器与加载音效资源。
      </Note>
