...
RTC SDK 支持不同的算法低照度增强模式，调用 [setLowlightEnhancement\|_blank](@setLowlightEnhancement) 接口进行设置低照度增强模式 [ZegoLowlightEnhancementMode\|_blank](@-ZegoLowlightEnhancementMode)：
...
低照度增强类型 [ZegoExpLowlightEnhancementType\|_blank](@-ZegoExpLowlightEnhancementType) 包括如下类型：
...
当环境光照度较低，且摄像头采集帧率过高时，每一帧画面的曝光时间不足，可能导致采集的视频画面偏暗，此时可以通过 [enableCameraAdaptiveFPS\|_blank](@enableCameraAdaptiveFPS) 开启摄像头自适应帧率。SDK 会根据设置的帧率范围匹配摄像头支持的采集帧率范围，在此范围内，根据环境亮度动态调整摄像头采集帧率，提升设置的帧率过高时的画面亮度。
...
调用 [enableColorEnhancement\|_blank](@enableColorEnhancement) 接口，设置色彩增强参数 [ZegoColorEnhancementParams\|_blank](@-ZegoColorEnhancementParams)（参数属性如下表）、以及其它参数，开启色彩增强功能。**默认不开启色彩增强功能。**
...
调用 [startPreview\|_blank](@startPreview) 接口开启预览后，可以设置不同的视频增强功能，并实时体验效果。
...
当预览效果达到预期后，可以调用 [startPublishingStream\|_blank](@startPublishingStream) 接口开始推流，推流画面效果将与预览效果一致。
