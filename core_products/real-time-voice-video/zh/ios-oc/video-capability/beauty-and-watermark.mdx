...
调用 [ZegoWatermark\|_blank](@-ZegoWatermark) 配置一个水印的图片 URL 以及该水印在画面中的大小方位。
...
[ZegoWatermark\|_blank](@-ZegoWatermark) 对象中的 [imageURL\|_blank](@imageURL) 参数支持传两种路径格式：绝对路径和 Assets。
...
1. **[ZegoWatermark\|_blank](@-ZegoWatermark) 中的 layout 如何指定？**

    水印的布局不能超过当前设置的推流的视频编码分辨率，对推流编码分辨率的设置可参考 [setVideoConfig\|_blank](@setVideoConfig) 接口。
