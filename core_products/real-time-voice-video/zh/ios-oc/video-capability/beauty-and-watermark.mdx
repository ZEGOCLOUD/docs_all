...
调用 [ZegoWatermark](@-ZegoWatermark) 配置一个水印的图片 URL 以及该水印在画面中的大小方位。

调用 [setPublishWatermark](@setPublishWatermark) 接口设置推流水印。
<Warning title="注意">
水印图片只支持 “PNG” 与 “JPEG” 两种图片格式，即 “.png”、“.jpg”、“.jpeg” 三种后缀的图片文件。
</Warning>

[ZegoWatermark](@-ZegoWatermark) 对象中的 [imageURL](@imageURL) 参数支持传两种路径格式：绝对路径和 Assets。
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/iOS/ZegoExpressEngine/Beautify/watermark-bundle.png" /></Frame>
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/iOS/ZegoExpressEngine/Beautify/watermark-assets.png" /></Frame>
...
1. 推流后，调用 [takePublishStreamSnapshot ](@takePublishStreamSnapshot) 接口对推流画面截图。
...
2. 拉流后，调用 [takePlayStreamSnapshot ](@takePlayStreamSnapshot) 接口对拉流画面截图。
...
1. **[ZegoWatermark](@-ZegoWatermark) 中的 layout 如何指定？**

    水印的布局不能超过当前设置的推流的视频编码分辨率，对推流编码分辨率的设置可参考 [setVideoConfig](@setVideoConfig) 接口。
