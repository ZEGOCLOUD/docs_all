...
* 调用 ZegoExpressEngine 中的 ZegoMediaPlayer 组件进行播放，详情可参考 [媒体播放器](/real-time-video-ios-oc/other/media-player)。
...
9. **为什么本地录制生成的文件打不开（无法正常播放）？**

  需要先确认是否有调用结束录制接口正常结束录制，如果没有正常结束录制，生成的录制文件是会打不开的。
...
请参考 [原始音频数据获取](/real-time-video-ios-oc/audio/get-audio-raw-data)。
