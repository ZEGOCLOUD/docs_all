...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/Express/RangeVoice_ios.png" /></Frame>

上图仅展示了实现游戏语音功能的核心步骤与接口，开发者可以根据业务需要，参考下文文档的详细介绍，实现其他相关接口。

### 1 创建引擎

调用 [createEngineWithProfile](@createEngineWithProfile) 接口，将申请到到 AppID 和 AppSign 传入参数 “appID” 和 “appSign”，创建引擎单例对象。引擎当前只支持同时创建一个实例，超出后将返回 null。
...
可以根据需要调用 [setEventHandler ](@setEventHandler-ZegoRangeAudio) 接口为麦克风设置事件回调，用于监听麦克风的开启状态 [microphoneStateUpdate ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoRangeAudioEventHandler#range-audio-microphone-state-update-error-code) 通知。
...
同时您也可以通过 [setAudioReceiveRangeWithParam](@setAudioReceiveRangeWithParam) 接口进一步控制衰减范围。距离小于 min 时，音量不会随着距离的增加而衰减；距离大于 max 时，将无法听到对方的声音。
...
2. 设置本地播放器的 3D 音效
    ...
    - 音效播放器：
        ...
        3. 在收到 [audioEffectPlayer](@audioEffectPlayer) 回调状态为 `ZegoAudioEffectPlayState.Playing` 后，调用 [ZegoAudioEffectPlayer.updatePosition](@updatePosition-ZegoAudioEffectPlayer-class) 接口设置正在播放的音效资源在世界坐标系中的位置。
