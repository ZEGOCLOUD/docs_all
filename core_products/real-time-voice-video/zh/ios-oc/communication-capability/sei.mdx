...
<img src="/Pics/Common/ZegoExpressEngine/send_and_recv_sei_iOS_new.png" />

推流端：

1. 调用 [createEngineWithProfile\|_blank](@createEngineWithProfile) 接口创建 engine 对象。

2. 调用 [loginRoom\|_blank](@loginRoom) 接口登录房间。

3. 调用 [startPublishingStream\|_blank](@startPublishingStream) 接口推流。

4. 在推流成功后，调用 [sendSEI\|_blank](@sendSEI) 接口发送 SEI 信息。

拉流端：

1. 调用 [createEngineWithProfile\|_blank](@createEngineWithProfile) 接口创建 engine 对象。

2. 创建 [ZegoEventHandler\|_blank](@-ZegoEventHandler) 对象，并重写接收 SEI 信息的 [onPlayerRecvSEI\|_blank](@onPlayerRecvSEI) 方法，调用 [setEventHandler\|_blank](@-ZegoEventHandler) 接口传入创建的 [ZegoEventHandler\|_blank](@-ZegoEventHandler) 监听 [onPlayerRecvSEI\|_blank](@onPlayerRecvSEI) 的回调。

3. 调用 [loginRoom\|_blank](@loginRoom) 接口登录房间。

4. 调用 [startPlayingStream\|_blank](@startPlayingStream) 接口拉流。

5. 在拉流成功后，接收到推流端发送的 SEI 信息之后触发 [onPlayerRecvSEI\|_blank](@onPlayerRecvSEI) 回调。
...
由于 SDK 默认使用 ZEGO 自行定义的 SEI（nalu type = 6, payload type = 243）类型打包，且此类型是 SEI 标准未规定的类型，因此跟视频编码器或者视频文件中的 SEI 不存在冲突。但当开发者需要使用第三方解码器解码时（如 FFmpeg），会导致解不出正确的 SEI，此时需要在推流前调用 [setSEIConfig\|_blank](@setSEIConfig) 接口更换 SDK 发送 SEI 的类型，使用 UserUnregister 的 SEI（nalu type = 6, payload type = 5）类型打包。
