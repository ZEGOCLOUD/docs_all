...
在 Token 过期前 30 秒，SDK 会通过 [tokenWillExpire](/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoRangeSceneEventHandler#range-scene-custom-command-update) 回调发出通知。
