...
- 大多数情况下，您只需要关注 “quality” 的 “level” 参数，以 “level” 枚举值来判断推流的综合质量，详情可参考 [ZegoStreamQualityLevel\|_blank](@-ZegoStreamQualityLevel)。
...
您可以通过注册 [onPlayerQualityUpdate\|_blank](@onPlayerQualityUpdate) 接收拉流质量回调，拉流成功后每隔三秒会收到此回调。可根据 quality（[ZegoPlayStreamQuality\|_blank](@-ZegoPlayStreamQuality)) 参数实时了解拉取的音视频流的健康情况。
...
您可以通过注册 [onPlayerRecvAudioFirstFrame\|_blank](@onPlayerRecvAudioFirstFrame) 监听拉流端音频接收首帧回调。调用拉流接口成功后，SDK 拉流拉到第一帧音频数据时会收到此回调。
