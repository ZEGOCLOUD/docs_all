...
此接口必须在调用 [createEngineWithProfile](@createEngineWithProfile) 接口创建引擎前调用。
</Warning>
...
- 确保在 [createEngineWithProfile](@createEngineWithProfile) 前正确设置云代理，不支持中途修改。

**是否提供本地代理方案？**

提供，详情请参考 [本地代理](/real-time-video-ios-oc/communication-capability/local-proxy)。
