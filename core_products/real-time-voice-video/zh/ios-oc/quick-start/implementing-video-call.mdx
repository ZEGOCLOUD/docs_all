...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时音视频功能，详情请参考 [快速开始 - 集成](/real-time-video-ios-oc/quick-start/integrating-sdk)。
...
调用 [createEngineWithProfile ](@createEngineWithProfile) 接口，将申请到的 AppID 和 AppSign 传入参数 “appID” 和 “appSign”。

根据 App 实际的音视频业务选择一个合适的场景，请参考 [场景化音视频配置](/real-time-video-ios-oc/quick-start/scenario-based-audio-video-configuration) 文档，把选择好的场景枚举传入参数 "scenario"。
...
您可以通过实例化实现 [ZegoEventHandler](@-ZegoEventHandler) 接口的类（例如 `self`）并实现需要的回调方法，以监听并处理所关注的事件回调。然后将实例化对象作为`eventHandler`参数传递给 [createEngineWithProfile ](@createEngineWithProfile) 或者传递给  [setEventHandler](@setEventHandler-ZegoExpressEngine) 注册回调
...
调用 [loginRoom ](@loginRoom) 接口登录房间。如果房间不存在，调用该接口时会创建并登录此房间。roomID 和 user 的参数由您本地生成，但是需要满足以下条件：
...
我们可以在该回调中，调用 [startPlayingStream](@startPlayingStream) ，传入 “streamID” 拉取拉取播放该用户的音视频。您可通过监听 [onPlayerStateUpdate ](@onPlayerStateUpdate) 回调知晓是否成功拉取音视频。
...
sequenceDiagram
    ...
    App->>ZEGO Express SDK: 开始推流 <br/> startPublishingStream
    ZEGO Express SDK-->>App: 推流状态更新回调 <br/> onPublisherStateUpdate

    App->>ZEGO Express SDK: 开始拉流 <br/> startPlayingStream
...
##### 如何排查视频通话中的错误？
如果用户在音视频通话的过程中，遇到相关错误，可查询 [错误码](/real-time-video-ios-oc/client-sdk/error-code)。


## 相关文档
- [常见错误码](/real-time-video-ios-oc/client-sdk/error-code)
