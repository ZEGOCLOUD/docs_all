...
在播放透明礼物特效前，需要设置用于播放器渲染透明特效的 view，即调用 [setBackgroundColor\|_blank](@setBackgroundColor) 将 `backgroundColor` 属性设置为 `clearColor`。
...
调用 [createMediaPlayer\|_blank](@createMediaPlayer) 创建媒体播放器。调用 [setPlayerCanvas\|_blank](@setPlayerCanvas-ZegoMediaPlayer) 设置播放器 canvas 的 `alphaBlend` 属性为 `YES`。
