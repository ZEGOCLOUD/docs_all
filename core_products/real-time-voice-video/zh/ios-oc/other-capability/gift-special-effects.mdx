...
在播放透明礼物特效前，需要设置用于播放器渲染透明特效的 view，即调用 [setBackgroundColor](@setBackgroundColor) 将 `backgroundColor` 属性设置为 `clearColor`。
