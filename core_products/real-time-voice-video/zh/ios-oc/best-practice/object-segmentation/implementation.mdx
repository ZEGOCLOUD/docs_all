...
如需了解渲染部分的实现原理，请参考 [播放透明礼物特效](/real-time-video-ios-oc/other/play-transparent-gift-effects)。
...
- 已联系 ZEGO 技术支持，获取包含主体分割功能的实时音视频 SDK，并参考实时音视频的 [快速开始 - 集成](/real-time-video-ios-oc/quick-start/integrating-sdk) 集成 SDK。**如果您的项目曾集成了官网版本的实时音视频 SDK，需要替换。**
...
- 已集成 ZIM SDK，详情请参考 [快速开始 - 实现基本收发消息](/zim-ios/send-and-receive-messages) 的 “集成 SDK”。
...
sequenceDiagram
    ...
    note over ZIM SDK,ZEGO Express SDK: 初始化 SDK
    App->>ZIM SDK: 初始化 SDK<br/>createWithAppConfig
    App->>ZIM SDK: 设置事件通知回调<br/>setEventHandler
    App->>ZEGO Express SDK: 初始化 SDK 并设置事件通知回调<br/>createEngineWithProfile:eventHandler:
```
{/*
...
sequenceDiagram
    ...
    note over ZIM_SDK,ZEGO_Express_SDK: 加入房间
    App->>ZEGO_Express_SDK: 登录 RTC 房间<br/>loginRoom
    App->>ZIM_SDK: 登录 ZIM 服务<br/>loginWithUserInfo
    App->>ZIM_SDK: 创建/加入 ZIM 房间<br/>createRoom/joinRoom
```
{/*
...
sequenceDiagram
    ...
    note over ZIM SDK,ZEGO Express SDK: 管理麦位

    App->>ZIM SDK: 查询房间所有属性（包含麦位信息）<br/>queryRoomAllAttributesByRoomID
    ZIM SDK-->>App: 返回房间所有属性信息<br/>ZIMRoomAttributesQueriedCallback
    App->>ZIM SDK: 设置房间属性（上麦）<br/>setRoomAttributes
    ZIM SDK-->>App: 通知用户本端已上麦<br/>roomAttributesUpdated
```
{/*
...
1. 为在推流时传输经主体分割后的图像，需要设置透明通道，请参考 [实时音视频 - 主体分割](/real-time-video-ios-oc/video/object-segmentation#7可选使用-alpha-通道传输分割出的主体) 的 “使用 Alpha 通道传输分割出的主体”，了解如何调用 [enableAlphaChannelVideoEncoder](@enableAlphaChannelVideoEncoder) 设置透明通道。
...
sequenceDiagram
    ...
    note over ZIM_SDK, ZEGO_Express_SDK: 使用主体分割

    App->>ZEGO_Express_SDK: enableAlphaChannelVideoEncoder <br/> 设置透明通道
    App->>ZEGO_Express_SDK: setVideoMirrorMode <br/> 开启屏幕镜像
    App->>ZEGO_Express_SDK: setAppOrientationMode <br/> 设置采集视频的朝向
    App->>ZEGO_Express_SDK: startPreview & startPublishingStream <br/> 开始预览和推流 <br/> 仅预览时，需设置 View 的 backgroundColor 为透明色
    App->>ZEGO_Express_SDK: enableVideoObjectSegmentation <br/> 开启主体分割
    ZEGO_Express_SDK-->>App: onVideoObjectSegmentationStateChanged <br/> 通知主体分割结果
```
{/*
...
sequenceDiagram
    ...
    note over ZIM SDK,ZEGO Express SDK: 拉流
    ZIM SDK-->>App: 通知本端房间内有其他用户上麦<br/>roomAttributesUpdated
    App->>ZEGO Express SDK: 开始拉流<br/>startPlayingStream <br/>调用 setBackgroundColor 接口将用于渲染的 View 的 backgroundColor 属性为 clearColor（透明色）
...
sequenceDiagram
    ...
    note over ZIM SDK, ZEGO Express SDK: 退出房间
    App->>ZEGO Express SDK: 停止预览 <br/> stopPreview
    App->>ZEGO Express SDK: 停止推流 <br/> stopPublishingStream
    App->>ZIM SDK: 离开 ZIM 房间 <br/> leaveRoom
    App->>ZEGO Express SDK: 退出 RTC 房间 <br/> logoutRoom
```
{/*
