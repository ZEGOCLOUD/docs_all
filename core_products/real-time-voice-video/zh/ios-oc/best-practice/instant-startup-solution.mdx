...
详情请参考 [快速开始 - 实现流程](/real-time-video-ios-oc/quick-start/implementing-video-call) 的“初始化”章节。
...
详情请参考 [快速开始 - 实现流程](/real-time-video-ios-oc/quick-start/implementing-video-call) 的“登录房间”章节。
...
- 推荐开发者使用超低延迟直播（Low-Latency Live Streaming，L3）进行拉流，实现更高质量的直播体验，具体请参考 [超低延迟直播](/real-time-video-ios-oc/live-streaming/low-latency-live-streaming) 。
- 当开发者使用 ZEGO 配置的 CDN 进行直推时，则可以直接通过 streamID 进行拉流，请参考 [快速开始 - 实现视频通话](/real-time-video-ios-oc/quick-start/implementing-video-call) 的 “拉流”。
- 当开发者使用第三方 CDN 进行直推时，则可以通过 URL 进行拉流，请参考 [通过 URL 拉流 ](/real-time-video-ios-oc/live-streaming/playing-stream-by-url)。
</Note>
