...
如需从 CDN 拉流切换到连麦场景，请确保当前音视频流，已实现 [通过 URL 拉流](/real-time-video-ios-oc/live-streaming/playing-stream-by-url)。
...
连麦结束，传入主播的流 ID，并在观众端调用 [stopPlayingStream](@stopPlayingStream) 接口，停止 RTC 拉流 ，重新切换为 CDN 拉流，并通过 [ZegoViewMode](@-ZegoViewMode) 设置 CDN 参数，详情请参考 [通过 URL 拉流](/real-time-video-ios-oc/live-streaming/playing-stream-by-url)。
...
主播端通过 [ZegoEventHandler](@-ZegoEventHandler) 中的 [onRoomStreamUpdate](@onRoomStreamUpdate) 回调，收到观众流新增通知后，可开启 RTC 拉流。当收到观众流删除通知后，则停止 RTC 拉流。
