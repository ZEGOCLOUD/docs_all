...
请参考 [快速开始 - 实现流程](!Integration/Solution_Implementation#CreateEngine) 的 “创建引擎” 和 “登录房间” 完成房间登录。
...
音频的码率 [bitrate](@bitrate-ZegoMixerAudioConfig) 默认值为 48 kbps。
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/iOS/ZegoExpressEngine/Mixer/mixer_layout_2.png" /></Frame>
...
可通过设置 [enableSoundLevel](@enableSoundLevel-ZegoMixerTask) 参数选择是否开启混流的声浪回调通知，开启后（取值为 “YES”）用户拉混流时可通过 [onMixerSoundLevelUpdate](@onMixerSoundLevelUpdate) 回调收到每条单流的声浪信息。
...
|枚举值|说明|适用场景|
|-|-|-|
|ZegoAudioCodecIDDefault|默认值。|根据调用 [createEngineWithProfile](@createEngineWithProfile) 时的 [scenario] 决定。|
...
可通过设置 [enableSoundLevel](@enableSoundLevel-ZegoAutoMixerTask) 参数选择是否开启自动混流的声浪回调通知，开启后（取值为 “YES”）用户拉混流时可通过 [onAutoMixerSoundLevelUpdate](@onAutoMixerSoundLevelUpdate) 回调收到每条单流的声浪信息。
