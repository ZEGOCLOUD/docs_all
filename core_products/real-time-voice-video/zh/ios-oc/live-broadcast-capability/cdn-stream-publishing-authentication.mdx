...
- KEY：指您在 [生成鉴权密钥](#生成鉴权密钥) 中配置的加密 KEY。
- StreamName：流名称，用户自定义，用于标识直播流。
- txTime：指您在 [生成 txTime](#生成-txtime) 中生成的 txTime。
...
- wsABStime：指您在 [生成 wsABStime](#生成-wsabstime) 中生成的 wsABStime，例如 65A006AA。
- StreamName：路径，格式为 “/接入点/streamID”，例如 /live/streamid123。
- KEY：指您在 [生成鉴权密钥](#生成鉴权密钥-2) 中配置的加密 KEY，例如 KEY123。
- MD5：标准的 MD5 单向不可逆哈希算法。
...
- KEY：指您在 [获取鉴权密钥](#获取鉴权密钥) 中获取的鉴权密钥。
- StreamName：流名称，用户自定义，用于标识直播流。
- hwTime：指您在 [生成 hwTime](#生成-hwtime) 中生成的 hwTime。
