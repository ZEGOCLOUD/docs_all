...
因网络质量不佳而导致的房间连接中断，SDK 内部会自动进行重试，详情请参考 [房间断线重连](/real-time-video-ios-oc/room/room-connection-status) 章节。
...
- T0、T1、T2、T3 时刻的情况同 [情况一：在 ZEGO 服务端判断用户 A 心跳超时前重连成功](/real-time-video-ios-oc/room/room-connection-status) 中的 T0、T1、T2、T3。
