...
开发者可调用 [startAudioDataObserver ](@startAudioDataObserver) 接口，开启音频数据回调监测。回调的音频数据类型为 [ZegoAudioDataCallbackBitMask](@-ZegoAudioDataCallbackBitMask)，包括有 `ZegoAudioDataCallbackBitMaskCaptured`、`ZegoAudioDataCallbackBitMaskPlayback`、`ZegoAudioDataCallbackBitMaskMixed`、`ZegoAudioDataCallbackBitMaskPlayer`，这四个回调类型都需要开启。
...
如有需要可参考 [ZegoAudioDataCallbackBitMask](/real-time-video-ios-oc/client-sdk/api-reference/enum#zegoaudiodatacallbackbitmask) 了解位掩码的使用。
</Note>
