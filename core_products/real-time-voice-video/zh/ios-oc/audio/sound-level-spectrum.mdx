...
- 接口原型
    ...
    - 本地采集的声浪回调接口 [onCapturedSoundLevelUpdate](@onCapturedSoundLevelUpdate)、[onCapturedSoundLevelInfoUpdate](@onCapturedSoundLevelInfoUpdate)：
    ...
    - 远端音频声浪回调接口 [onRemoteSoundLevelUpdate](@onRemoteSoundLevelUpdate)、[onRemoteSoundLevelInfoUpdate](@onRemoteSoundLevelInfoUpdate)：
    ...
    - 本地采集的音频频谱回调接口 [onCapturedAudioSpectrumUpdate](@onCapturedAudioSpectrumUpdate)：

        ```objc
        // 本地采集音频频谱回调
        //
        // @param audioSpectrum 本地采集的音频频谱值数组，频谱值范围为 [0-2^31]
        - (void)onCapturedAudioSpectrumUpdate:(NSArray<NSNumber *> *)audioSpectrum;
        -
        ```

    - 远端拉流音频频谱回调接口 [onRemoteAudioSpectrumUpdate](@onRemoteAudioSpectrumUpdate)：

        ```objc
        // 远端拉流音频频谱回调
        //
        // @param audioSpectrums 远端音频频谱键值对，key 是流 ID，value 为对应的流的音频频谱值数组，频谱值范围为 [0-2^31]
...
- 调用示例
    ...
    可先通过 [onRoomStreamUpdate ](@onRoomStreamUpdate) 回调方法获取到当前房间内存在的流列表，并保存起来，然后通过保存的流列表来索引 NSDictionary 取得每条流对应的声浪/音频频谱数据。
...
- 调用  [startSoundLevelMonitor](@startSoundLevelMonitor) 接口启动声浪的监听：
    ...
    ```
    <Note title="说明">配置 [ZegoSoundLevelConfig](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objectivec_ios~class~ZegoSoundLevelConfig) 的 `millisecond` 属性可以设置声浪回调的间隔。 声浪的监控时间周期，单位为毫秒，取值范围 [100, 3000]。默认 100 ms。</Note>

    在调用上述接口后，[onCapturedSoundLevelUpdate](@onCapturedSoundLevelUpdate) 回调方法会立刻触发，未推流且未预览时回调值为 0。[onRemoteSoundLevelUpdate](@onRemoteSoundLevelUpdate) 需要在调用 [startPlayingStream](@startPlayingStream) 开始拉流接口之后，才会有回调。
...
- 调用  [startAudioSpectrumMonitor](@startAudioSpectrumMonitor) 接口启动音频频谱的监听：
    ...
    在调用上述接口之后，[onCapturedAudioSpectrumUpdate](@onCapturedAudioSpectrumUpdate) 回调方法会立刻触发，未推流且未预览时回调值为 0；[onRemoteAudioSpectrumUpdate](@onRemoteAudioSpectrumUpdate) 需要在调用 [startPlayingStream ](@startPlayingStream) 开始拉流接口之后，才会有回调。
...
- 调用  [stopSoundLevelMonitor](@stopSoundLevelMonitor) 接口停止声浪的监听:
    ...
    在调用上述接口后，[onCapturedSoundLevelUpdate](@onCapturedSoundLevelUpdate) 与 [onRemoteSoundLevelUpdate](@onRemoteSoundLevelUpdate) 不再回调。
...
- 接口原型
    ...
    <Warning title="注意">
    登录了自动混流所在的房间拉取本房间的混流时，才能在回调接口中拿到 `streamID`。
    {/* 需要在没有登录自动混流所在房间的情况下也能拿到 `streamID`，请联系 ZEGO 技术支持进行配置。 */}
    </Warning>
        ```objc
        /**
         * 自动混流中每条单流的声浪更新回调
...
- 手动混流场景
  ...
  调用 [startMixerTask](@startMixerTask) 接口发起一个手动混流任务时，设置 `enableSoundLevel` 参数为 `YES` 可启动声浪的监听，并为每条输入流指定设置唯一的 `soundLevelID`：
...
- 自动混流场景
  ...
  调用 [startAutoMixerTask](@startAutoMixerTask) 接口发起一个自动混流任务时，设置 `enableSoundLevel` 参数为 `YES` 可启动声浪的监听：
...
- 手动混流场景
  ...
  在调用 [startMixerTask](@startMixerTask) 的客户端接口更新一个混流任务时，设置 `enableSoundLevel` 参数为 `NO` 可停止声浪的监听：
...
- 自动混流场景
  ...
  调用 [startAutoMixerTask](@startAutoMixerTask) 的客户端接口更新一个自动混流任务时，设置 `enableSoundLevel` 参数为 `NO` 可停止声浪的监听：
...
- [媒体音量和通话音量有什么区别？](https://doc-zh.zego.im/faq/media-vs-call-volume)
- [怎么处理无声问题？](https://doc-zh.zego.im/faq/no-audio-issue)
- [怎么处理音量太小问题？](https://doc-zh.zego.im/faq/low-audio-volume-issue)
