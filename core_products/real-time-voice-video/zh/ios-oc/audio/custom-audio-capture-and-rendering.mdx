...
sequenceDiagram
    ...
    客户端App->>ZEGO Express SDK: createEngineWithProfile <br/> 创建引擎

    客户端App->>ZEGO Express SDK: enableCustomAudioIO <br/> 开启自定义音频采集渲染

    客户端App->>ZEGO Express SDK: loginRoom <br/> 登录房间

    客户端App->>ZEGO Express SDK: startPublishingStream <br/> 开始推流

    客户端App->>ZEGO Express SDK: sendCustomAudioCaptureAACData、sendCustomAudioCapturePCMData <br/> 采集音频数据

    客户端App->>ZEGO Express SDK: startPlayingStream <br/> 开始拉流

    客户端App->>ZEGO Express SDK: fetchCustomAudioRenderPCMData <br/> 渲染音频数据
```
{/*
...
请参考 [快速开始 - 实现流程](/real-time-video-ios-oc/quick-start/implementing-video-call) 的 “创建引擎”。
...
可调用 [ZegoCustomAudioConfig](@-ZegoCustomAudioConfig) 设置 `sourceType = ZegoAudioSourceTypeCustom`，再调用 [enableCustomAudioIO](@enableCustomAudioIO) 接口开启自定义音频 IO 功能。
...
请参考 [快速开始 - 实现流程](/real-time-video-ios-oc/quick-start/implementing-video-call) 的 “登录房间”、“推流” 和 “拉流”。
...
打开音频渲染设备，使用 [fetchCustomAudioRenderPCMData](@fetchCustomAudioRenderPCMData) 从引擎中获取音频数据，拿到音频数据后再通过渲染设备播放。
