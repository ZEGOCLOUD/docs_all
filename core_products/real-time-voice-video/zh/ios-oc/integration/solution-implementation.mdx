...
- (void)loginRoom {
    ...
    [[ZegoExpressEngine sharedEngine] loginRoom:roomID user:user config:roomConfig callback:^(int errorCode, NSDictionary * _Nullable extendedData) {
        ...
        if (errorCode == 0) {
            ...
            // 登录失败，请参考 errorCode 说明 /real-time-video-ios-oc/error-code
            NSLog(@"房间登录失败");
        }
    }];
}
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/QuickStart/quickstart_uml_createEngineWithProfile.png" /></Frame>
