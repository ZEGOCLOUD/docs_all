...
3. 废弃了 [ZegoEngineConfig > advancedConfig\|_blank](@advancedConfig-ZegoEngineConfig) 的配置项 "audio_device_mode"，请使用 [setAudioDeviceMode\|_blank](@setAudioDeviceMode) 接口代替。
...
setReverbParam | 设置混响的具体参数。此函数在 1.18.0 版本以后已废弃，请使用 [setReverbPreset\|_blank](@setReverbPreset) 或者 [setReverbAdvancedParam](@setReverbAdvancedParam) 代替。
setBuiltInSpeakerOn | 是否使用内置扬声器播放声音。此函数在 2.3.0 版本以后已废弃，请使用 [setAudioRouteToSpeaker\|_blank](@setAudioRouteToSpeaker) 代替。
onRoomStreamUpdate | 相同房间内其他用户推的流增加或减少的通知。此回调函数自 1.18.0 起已废弃，请使用带 [extendedData] 参数的 [onRoomStreamUpdate\|_blank](@onRoomStreamUpdate) 函数。
onDeviceError | 设备异常通知。此函数在 2.15.0 版本及以上已废弃，请使用 [onLocalDeviceExceptionOccurred\|_blank](@onLocalDeviceExceptionOccurred) 代替。
onNetworkQuality | 回报房间中流的网络质量。此回调函数在 2.10.0 版本及以上已废弃，请使用带 [ZegoStreamQualityLevel\|_blank](@-ZegoStreamQualityLevel) 枚举参数的 [onNetworkQuality\|_blank](@onNetworkQuality) 函数。
onProcessCapturedAudioData | 自定义音频处理本地采集 PCM 音频帧回调。此回调函数在 2.13.0 版本及以上已废弃，请使用带 [timestamp] 参数的 [onProcessCapturedAudioData\|_blank](@onProcessCapturedAudioData) 函数。
onProcessRemoteAudioData | 自定义音频处理远端拉流 PCM 音频帧回调。此回调函数在 2.13.0 版本及以上已废弃，请使用带 [timestamp] 参数的 [onProcessRemoteAudioData\|_blank](@onProcessRemoteAudioData) 函数。
onProcessPlaybackAudioData | 自定义音频处理 SDK 播放音频的 PCM 音频帧回调。此回调函数在 2.13.0 版本及以上已废弃，请使用带 [timestamp] 参数的 [onProcessPlaybackAudioData\|_blank](@onProcessPlaybackAudioData) 函数。
...
相关接口使用可参考 [实时音视频 - 登录多房间\|_blank](!ExpressVideoSDK-Other_Functions/Multiple_Rooms) 或 [实时语音 - 登录多房间\|_blank](!ExpressAudioSDK-Other_Functions/Multiple_Rooms)文档。
...
相关接口使用可参考 [设置视频编码方式\|_blank](!ExpressVideoSDK-VideoAdvanced/video_codec_svc) 文档。
