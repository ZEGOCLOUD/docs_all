...
相关 API 请参考 [setCustomAudioIOAudioRoute\|_blank](@setCustomAudioIOAudioRoute)
...
若要在直播列表等场景展示直播标题，SDK 支持开发者在主播调用 [startPublishingStream\|_blank](@startPublishingStream) 接口开始推流前，通过 [ZegoPublisherConfig\|_blank](@-ZegoPublisherConfig) 中的 streamTitle 参数配置标题。

相关 API 请参考 [startPublishingStream\|_blank](@startPublishingStream), [ZegoPublisherConfig > streamTitle\|_blank](@streamTitle-ZegoPublisherConfig)

#### 4. 媒体播放器新增有关资源文件权限的错误码

优化媒体播放器的回调通知逻辑，针对“播放资源无权限”的错误新增错误码 1008015，帮助开发者快速定位问题。

相关 API 请参考 [mediaPlayer:stateUpdate:errorCode\|_blank](/article/api?doc=Express_Video_SDK_API~objectivec_macos~protocol~ZegoMediaPlayerEventHandler#media-player-state-update-error-code)
...
新增 [onRtcStats\|_blank](@onRtcStats) 回调，开发者可用于衡量用户维度的 RTC 网络质量，该接口将本端用户所有推、拉流的质量数据进行聚合统计并回调，包括：<ul><li>上行 & 下行：占用带宽、RTT、丢包率。</li><li>端到端延迟。</li></ul>

相关 API 请参考 [onRtcStats\|_blank](@onRtcStats)

#### 3. 人脸识别功能支持后置摄像头以及回调人脸个数和坐标信息

人脸识别功能支持后置摄像头，且新增回调 [onPublisherFaceDetectInfo\|_blank](@onPublisherFaceDetectInfo) 支持获取人脸个数和坐标信息。<div class="mk-note">
...
相关 API 请参考 [enableFaceDetection\|_blank](@enableFaceDetection), [onPublisherFaceDetectInfo\|_blank](@onPublisherFaceDetectInfo)
...
ZEGO 为 [enableCamera\|_blank](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#enable-camera-notify-mode-channel) 接口新增 `notifyMode` 参数，用于控制关闭摄像头时是否通知本端以及远端。在切换前后置摄像头场景下，该参数可减少设备开关状态的回调次数，避免触发不必要的业务逻辑。
...
开发者可通过 [setLowlightEnhancementParams\|_blank](@setLowlightEnhancementParams) 指定使用 AI 算法，当设备性能不足以支持开启 AI 算法时，SDK 会自动使用传统算法。一般中高端机型可稳定运行低照度增强 AI 算法，详细请参考 [推流视频增强\|_blank](!Publish_Video_Enhanced) 文档。

相关 API 请参考 [setLowlightEnhancementParams\|_blank](@setLowlightEnhancementParams)

#### 2. 独唱场景下支持动态响度均衡功能

开启动态响度均衡后，根据媒体播放器伴奏的实时表现，SDK 动态调整人声响度，使人声与伴奏响度时刻保持恰当比例，提升人声与伴奏融合度，观众体验更佳，适用于实时 KTV 的独唱场景。

相关 API 请参考 [enableAuxBgmBalance\|_blank](@enableAuxBgmBalance)

#### 3. 媒体播放器播放在线资源时，支持缓存资源文件

开发者可通过配置 [ZegoMediaPlayerResource-onlineResourceCachePath\|_blank](@onlineResourceCachePath-ZegoMediaPlayerResource) 参数，设置缓存资源文件路径。此外可通过 [ZegoMediaPlayerResource-maxCachePendingLength\|_blank](@maxCachePendingLength-ZegoMediaPlayerResource) 参数设置从当前播放时间点，最多可预缓存的数据长度（单位 bytes）。从而使媒体播放器播放在线资源时，可以减少用户资源下载频次，提高资源复用率与用户体验。 <div class="mk-warning">

生成缓存文件的同时，SDK 同时会生成一个 ".info "后缀的文件，用于记录缓存信息（如缓存的起始位置等）。用户使用该功能时，需要自行负责清理"缓存文件"及"缓存信息文件"。

</div>

相关 API 请参考 [ZegoMediaPlayerResource > onlineResourceCachePath\|_blank](@onlineResourceCachePath-ZegoMediaPlayerResource), [ZegoMediaPlayerResource > maxCachePendingLength\|_blank](@maxCachePendingLength-ZegoMediaPlayerResource)

#### 4. 摄像头停止采集时，支持自动推送静态图片

当用户主动操作或系统前后台等限制，导致摄像头无法继续采集时，支持自动推送开发者配置的静态图片。

相关 API 请参考 [setDummyCaptureImageParams\|_blank](@setDummyCaptureImageParams)

#### 5. 移动端屏幕共享功能支持共享指定画面区域

使用移动端的屏幕共享功能时，可通过  [ZegoScreenCaptureConfig-cropRect\|_blank](@cropRect-ZegoScreenCaptureConfig) 参数指定共享的画面区域，详情请参考 [屏幕共享\|_blank](!Other_Functions/Share_Screen) 文档。

相关 API 请参考 [ZegoScreenCaptureConfig > cropRect\|_blank](@cropRect-ZegoScreenCaptureConfig)

#### 6. 自定义视频采集功能支持由开发者控制外部时间戳

使用自定义视频采集时，支持由开发者自行对齐音频帧和视频帧的外部时间戳，保证音画同步以及播放器正常工作。

相关 API 请参考 [ZegoVideoEncodedFrameParam > isExternalClock\|_blank](@isExternalClock-ZegoVideoEncodedFrameParam)
...
相关 API 请参考 [ZegoAECModeAI\|_blank](@ZegoAECModeAI)

#### 2. 支持设置视频降噪功能

新增视频降噪接口`setVideoDenoiseParams`，适用于摄像头的采集效果不佳，画面出现明显的噪点时，需要单独视频降噪等场景。开发者可以根据业务场景，选择不同的视频降噪模式 [ZegoVideoDenoiseMode\|_blank](@-ZegoVideoDenoiseMode) 及视频降噪强度 [ZegoVideoDenoiseStrength\|_blank](@-ZegoVideoDenoiseStrength)，详情可参考 [推流视频增强\|_blank](!Publish_Video_Enhanced) 文档。

相关 API 请参考 [setVideoDenoiseParams\|_blank](@setVideoDenoiseParams)
...
混流功能支持将混流后的输出流，加入到指定房间，即支持设置输出流的目标房间信息 [setTargetRoom\|_blank](@setTargetRoom)。每条输出流只支持加入一个房间，且一旦添加，混流过程中不支持动态更新房间。如需使用服务端接口实现该功能，可参考 [开始混流\|_blank](#19595) 文档。

相关 API 请参考 [setTargetRoom\|_blank](@setTargetRoom)
...
相关 API 请参考 [aiVoiceChanger\|_blank](@aiVoiceChanger)
...
媒体播放器播放透明特效时，新增 [ZegoAlphaLayoutType > ZegoAlphaLayoutTypeRightTop\|_blank](@ZegoAlphaLayoutTypeRightTop-ZegoAlphaLayoutType)  枚举，以支持 Alpha 数据拼接在 RGB 数据右上方，设置此枚举时，仅支持 0.5x 的缩放倍率，详情请参考  [播放透明礼物特效\|_blank](!gift_special_effects) 文档。

相关 API 请参考 [loadResourceWithConfig\|_blank](@loadResourceWithConfig), [ZegoAlphaLayoutTypeRightTop\|_blank](@ZegoAlphaLayoutTypeRightTop)
...
相关 API 请参考 [startPlayingStream\|_blank](@startPlayingStream), [ZegoStreamResourceModeCustom\|_blank](@ZegoStreamResourceModeCustom), [ZegoPlayerConfig > customResourceConfig\|_blank](@customResourceConfig-ZegoPlayerConfig)

<p class="hthree">改进优化</p>

#### 1. 登录房间时，userName 字段改为非必填字段

调用 [loginRoom\|_blank](@loginRoom) 接口登录房间时，`userName` 原来为必填字段，本次优化为非必填的可选字段。
...
相关 API 请参考 [setVideoSource\|_blank](@setVideoSource), [useFrontCamera\|_blank](@useFrontCamera), [ZegoVideoSourceType > ZegoVideoSourceTypeSecondaryCamera\|_blank](@ZegoVideoSourceTypeSecondaryCamera-ZegoVideoSourceType), [ZegoVideoSourceType > ZegoVideoSourceTypeCamera\|_blank](@ZegoVideoSourceTypeCamera-ZegoVideoSourceType)
...
相关 API 请参考 [setCameraZoomFactor\|_blank](@setCameraZoomFactor)
...
相关 API 请参考 [setANSMode\|_blank](@setANSMode)
...
相关 API 请参考 [startPublishingStream\|_blank](@startPublishingStream), [ZegoPublisherConfig > streamCensorFlag\|_blank](@streamCensorFlag-ZegoPublisherConfig)

#### 6. 媒体播放器播放倍速最低支持至 0.3 倍速

媒体播放器的播放速度区间从 [0.5,4.0]，扩大为 [0.3,4.0]，更多内容请参考 [媒体播放器\|_blank](!Other_Functions/MediaPlayer)。

相关 API 请参考 [setPlaySpeed\|_blank](@setPlaySpeed)
...
相关 API 请参考 [startPlayingStream\|_blank](@startPlayingStream), [adaptiveSwitch\|_blank](@adaptiveSwitch), [adaptiveTemplateIDList\|_blank](@adaptiveTemplateIDList)
...
相关 API 请参考 [switchPlayingStream\|_blank](@switchPlayingStream)
...
相关 API 请参考 [loginRoom\|_blank](@loginRoom), [startPublishingStream\|_blank](@startPublishingStream), [ZegoPublisherConfig > codecNegotiationType\|_blank](@codecNegotiationType-ZegoPublisherConfig), [ZegoRoomConfig > capabilityNegotiationTypes\|_blank](@capabilityNegotiationTypes-ZegoRoomConfig)
...
相关 API 请参考 [ZegoStreamResourceModeCDNPlus\|_blank](@ZegoStreamResourceModeCDNPlus)
...
相关 API 请参考 [ZegoMediaPlayer > enableVoiceChanger\|_blank](@enableVoiceChanger-ZegoMediaPlayer)
...
相关 API 请参考 [enableViewMirror\|_blank](@enableViewMirror)
...
相关 API 请参考 [isAIVoiceChangerSupported\|_blank](@isAIVoiceChangerSupported)
...
相关 API 请参考 [setCameraStabilizationMode\|_blank](@setCameraStabilizationMode)
...
相关 API 请参考 [ZegoCDNConfig > quicConnectMode\|_blank](@quicConnectMode-ZegoCDNConfig)
...
相关 API 请参考 [addPublishCdnUrl\|_blank](@addPublishCdnUrl)

#### 4. 支持回调本地录制的质量数据

ZegoDataRecordProgress 新增 quality 属性，在本地录制过程中，通过该属性回调录制文件的帧率、码率等质量数据。

相关 API 请参考 [onCapturedDataRecordProgressUpdate\|_blank](@onCapturedDataRecordProgressUpdate)

#### 5. 自定义视频渲染支持独立通道控制

自定义视频渲染支持独立通道控制。例如，针对指定流 ID 只进行 SDK 内部渲染、但不执行自定义渲染。

相关 API 请参考 [enableCapturedVideoCustomVideoRender\|_blank](@enableCapturedVideoCustomVideoRender), [enableRemoteVideoCustomVideoRender\|_blank](@enableRemoteVideoCustomVideoRender)
...
相关 API 请参考 [sendCustomVideoProcessedCVPixelBuffer\|_blank](@sendCustomVideoProcessedCVPixelBuffer)
...
相关 API 请参考 [setLowlightEnhancement\|_blank](@setLowlightEnhancement), [enableColorEnhancement\|_blank](@enableColorEnhancement)
...
相关 API 请参考 [getRoomStreamList\|_blank](@getRoomStreamList)
...
相关 API 请参考 [getPlaybackStatistics\|_blank](@getPlaybackStatistics)

#### 5. 媒体播放器支持在本地缓存网络资源

支持在本地缓存网络资源，如果需要播放同一个网络资源时，将优先使用缓存数据，提升用户体验。

相关 API 请参考 [enableLocalCache\|_blank](@enableLocalCache)
...
相关 API 请参考 [ZegoMixerOutputVideoConfig > enableLowBitrateHD\|_blank](@enableLowBitrateHD-ZegoMixerOutputVideoConfig)

#### 2. 推流视频支持色彩增强

针对各种摄像头等设备采集到的画面色彩偏灰、或饱和度偏低的情况，支持在保护人体肤色的同时，增强画面色彩，使其更加鲜艳明亮，更符合人眼真实的视觉感受，详情请参考 [推流视频增强\|_blank](!Publish_Video_Enhanced)。

相关 API 请参考 [enableColorEnhancement\|_blank](@enableColorEnhancement)
...
相关 API 请参考 [sendTransparentMessage\|_blank](@sendTransparentMessage)
...
相关 API 请参考 [ZegoAutoMixerTask > minPlayStreamBufferLength\|_blank](@minPlayStreamBufferLength-ZegoAutoMixerTask)
...
相关 API 请参考 [ZegoMixerInput > advancedConfig\|_blank](@advancedConfig-ZegoMixerInput)
...
相关 API 请参考 [ZegoAudioEffectPlayer > setPublishVolume\|_blank](@setPublishVolume-ZegoAudioEffectPlayer), [ZegoAudioEffectPlayer > setPlayVolume\|_blank](@setPlayVolume-ZegoAudioEffectPlayer), [ZegoAudioEffectPlayer > setPublishVolumeAll\|_blank](@setPublishVolumeAll-ZegoAudioEffectPlayer), [ZegoAudioEffectPlayer > setPlayVolumeAll\|_blank](@setPlayVolumeAll-ZegoAudioEffectPlayer)
...
相关 API 请参考 [createAIVoiceChanger\|_blank](@createAIVoiceChanger), [destroyAIVoiceChanger\|_blank](@destroyAIVoiceChanger)
...
相关 API 请参考 [enableVideoObjectSegmentation\|_blank](@enableVideoObjectSegmentation)

#### 3. 媒体播放器支持伴奏音质增强

媒体播放器支持伴奏音质增强，提升伴奏的音质以及现场的氛围感，适用于语聊房、K 歌等场景中。

相关 API 请参考 [enableLiveAudioEffect\|_blank](@enableLiveAudioEffect)
...
相关 API 请参考 [startDumpData\|_blank](@startDumpData), [stopDumpData\|_blank](@stopDumpData), [uploadDumpData\|_blank](@uploadDumpData), [removeDumpData\|_blank](@removeDumpData), [onRequestDumpData\|_blank](@onRequestDumpData), [onStartDumpData\|_blank](@onStartDumpData), [onStopDumpData\|_blank](@onStopDumpData), [onUploadDumpData\|_blank](@onUploadDumpData)

#### 5. 自定义视频采集支持透明通道传输

支持提取、编码和传输开发者自定义采集的 RGBA 通道中的 Alpha 通道数据，从而在拉流端渲染出透明背景的主体，实现更加沉浸、真实的视频场景。

相关 API 请参考 [enableAlphaChannelVideoEncoder\|_blank](@enableAlphaChannelVideoEncoder)
...
相关 API 请参考 [startNetworkSpeedTest\|_blank](@startNetworkSpeedTest)
...
相关 API 请参考 [muteAllPlayAudioStreams\|_blank](@muteAllPlayAudioStreams), [muteAllPlayVideoStreams\|_blank](@muteAllPlayVideoStreams), [mutePlayStreamAudio\|_blank](@mutePlayStreamAudio), [mutePlayStreamVideo\|_blank](@mutePlayStreamVideo)
...
相关 API 请参考 [setPlayMediaStreamType\|_blank](@setPlayMediaStreamType)
...
相关 API 请参考 [onAudioMixingCopyData\|_blank](@onAudioMixingCopyData)
...
相关 API 请参考 [onPlayerRecvMediaSideInfo\|_blank](@onPlayerRecvMediaSideInfo)
...
相关 API 请参考 [ZegoPlayerConfig > codecTemplateID\|_blank](@codecTemplateID-ZegoPlayerConfig)

#### 3. 支持抛出 Audio Session 异常回调，例如设备无权限、被抢占等

相关 API 请参考 [onLocalDeviceExceptionOccurred\|_blank](@onLocalDeviceExceptionOccurred)

#### 4. 支持抛出 [setDummyCaptureImagePath] 异常回调

相关 API 请参考 [onPublisherDummyCaptureImagePathError\|_blank](@onPublisherDummyCaptureImagePathError)

#### 5. 直推 CDN 支持在推流过程中更新 CDN 地址

相关 API 请参考 [enablePublishDirectToCdn\|_blank](@enablePublishDirectToCdn)
...
相关 API 请参考 [ZegoANSModeAIBalanced\|_blank](@ZegoANSModeAIBalanced)
...
相关 API 请参考 [setLogConfig\|_blank](@setLogConfig), [setEngineConfig\|_blank](@setEngineConfig)
...
相关 API 请参考 [ZegoVideoBufferTypeNV12CVPixelBuffer\|_blank](@ZegoVideoBufferTypeNV12CVPixelBuffer)
...
相关 API 请参考 [onPlayerLowFpsWarning\|_blank](@onPlayerLowFpsWarning), [onPublisherLowFpsWarning\|_blank](@onPublisherLowFpsWarning)

#### 4. 新增同步抛出视频网络首帧的回调

新增从非 UI 线程抛出视频网络首帧的回调 [onPlayerSyncRecvVideoFirstFrame]，该回调不受 UI 卡顿的影响，能够更准确的统计视频首帧数据。

相关 API 请参考 [onPlayerSyncRecvVideoFirstFrame\|_blank](@onPlayerSyncRecvVideoFirstFrame)

#### 5. 媒体播放器支持设置网络资源的 Http Headers

媒体播放器支持设置网络资源的 Http Headers，开发者可基于该配置，自定义限定网络资源的访问方式，加强资源的安全防护。

相关 API 请参考 [setHttpHeader\|_blank](@setHttpHeader)
...
相关 API 请参考 [ZegoExpressEngine > setVideoConfig\|_blank](@setVideoConfig-ZegoExpressEngine), [setPublishDualStreamConfig\|_blank](@setPublishDualStreamConfig)
...
相关 API 请参考 [setReceiveRange\|_blank](@setReceiveRange), [setAudioReceiveRange\|_blank](@setAudioReceiveRange)

#### 3. 新增错误码提示

新增语音检测（1018xxxxx）、万人范围音视频（1019xxxxx）、屏幕采集（1020xxxxx）三个模块的错误码，详情请参考 [常见错误码\|_blank](!Error_Code/Error_Code)。
...
相关 API 请参考 [onNetworkQuality\|_blank](@onNetworkQuality)

#### 4. 支持在调用 [destroyEngine] 接口后，生成日志上传任务

相关 API 请参考 [submitLog\|_blank](@submitLog)
...
相关 API 请参考 [setReverbPreset\|_blank](@setReverbPreset)

#### 3. 游戏语音功能支持媒体播放器&音效播放器使用 3D 音效

开发者可以通过设置媒体播放器、音效播放器的位置和朝向，实现本地音频、在线音频资源的 3D 音效。该功能可用于在虚拟场景中设置物品的音效、以及指定位置的背景音乐等。

相关 API 请参考 [ZegoMediaPlayer > updatePosition\|_blank](@updatePosition-ZegoMediaPlayer), [ZegoAudioEffectPlayer > updatePosition\|_blank](@updatePosition-ZegoAudioEffectPlayer)

#### 4. 媒体播放器支持获取媒体流视频信息

针对媒体播放器正在播放的视频文件，开发者可以主动获取视频的分辨率、帧率等信息。

相关 API 请参考 [getMediaInfo\|_blank](@getMediaInfo)

#### 5. 媒体播放器倍速功能最大支持 4 倍速

媒体播放器倍速的上限提升到 4 倍速。例如，用户在播放音视频文件时，如果已设置为 2 倍速播放，长按屏幕时可以加速至 4 倍速。

相关 API 请参考 [ZegoMediaPlayer > setPlaySpeed\|_blank](@setPlaySpeed-ZegoMediaPlayer)
...
相关 API 请参考 [setEngineConfig\|_blank](@setEngineConfig)
...
相关 API 请参考 [onPublisherSendAudioFirstFrame\|_blank](@onPublisherSendAudioFirstFrame), [onPublisherSendVideoFirstFrame\|_blank](@onPublisherSendVideoFirstFrame)
...
相关 API 请参考 [sendReferenceAudioPCMData\|_blank](@sendReferenceAudioPCMData)
...
相关 API 请参考 [setBlockDataHandler\|_blank](@setBlockDataHandler)
...
相关 API 请参考 [enableTrafficControl\|_blank](@enableTrafficControl), [setMinVideoBitrateForTrafficControl\|_blank](@setMinVideoBitrateForTrafficControl), [setMinVideoFpsForTrafficControl\|_blank](@setMinVideoFpsForTrafficControl), [setMinVideoResolutionForTrafficControl\|_blank](@setMinVideoResolutionForTrafficControl)
...
相关 API 请参考 [setPlayStreamBufferIntervalRange\|_blank](@setPlayStreamBufferIntervalRange)
...
相关 API 请参考 [onPlayerSyncRecvSEI\|_blank](@onPlayerSyncRecvSEI), [onPlayerRecvSEI\|_blank](@onPlayerRecvSEI)
...
相关 API 请参考 [enableSpeechEnhance\|_blank](@enableSpeechEnhance)

#### 2. 媒体播放器支持播放透明特效

媒体播放器支持通过渲染 Alpha 通道，实现播放透明特效文件功能，详情请参考 [播放透明礼物特效\|_blank](!gift_special_effects)。

相关 API 请参考 [loadResourceWithConfig\|_blank](@loadResourceWithConfig)
...
相关 API 请参考 [setWhiteboard\|_blank](@setWhiteboard), [startMixerTask\|_blank](@startMixerTask)
...
注意：[enableVideoSuperResolution] 修改了调用时机，需要在 [initVideoSuperResolution] 后才能调用，详情请参考 [超分辨率\|_blank](!super_resolution)。

相关 API 请参考 [initVideoSuperResolution\|_blank](@initVideoSuperResolution), [uninitVideoSuperResolution\|_blank](@uninitVideoSuperResolution)
...
相关 API 请参考 [setAppGroupID\|_blank](@setAppGroupID), [setupWithDelegate\|_blank](@setupWithDelegate)
...
相关 API 请参考 [enablePlayInRange\|_blank](@enablePlayInRange), [enablePublishToWorld\|_blank](@enablePublishToWorld)

#### 2. 支持视频大小流功能

通过视频大小流编码（H.264 DualStream）对视频进行码流分层时，相比较于分层视频编码（H.264 SVC），视频大小流编码（H.264 DualStream）支持使用硬件编码，即 [ZegoVideoCodecID] 新增 [ZegoVideoCodecIDH264DualStream] 字段，详情请参考 [设置视频编码方式\|_blank](!VideoAdvanced/video_codec_svc)。

相关 API 请参考 [ZegoVideoCodecIDH264DualStream\|_blank](@ZegoVideoCodecIDH264DualStream)
...
#### 2. 从此版本开始 Express 实时音视频 Video SDK 的 CocoaPods 的项目名称由 `ZegoExpressEngine/Video` 变更为 `ZegoExpressEngine`，详情请查看 [集成 SDK\|_blank](#196) 文档。
...
相关 API 请参考 [createItem\|_blank](@createItem), [bindItem\|_blank](@bindItem), [unbindItem\|_blank](@unbindItem), [updateItemStatus\|_blank](@updateItemStatus), [updateItemCommand\|_blank](@updateItemCommand)

#### 2. “万人范围音视频”及“多人实时状态同步”功能支持使用场景模板

注意：如需使用该功能，请联系 ZEGO 技术支持。

虚拟场景中，由于每个场景的地图大小、音视频互动玩法及规模不同，需要针对每个场景进行自定义配置。3.1.0 版本后，“万人范围音视频”及“多人实时状态同步”支持通过 SDK 接口，并使用模板 ID 指定场景。模板 ID 对应的配置项只能通过服务器 API 进行配置，详情请参考 [服务端 API - 场景模版配置\|_blank](#19659)。

相关 API 请参考 [templateID\|_blank](@templateID)
...
相关 API 请参考 [ZegoSceneParam > token\|_blank](@token-ZegoSceneParam), [ZegoRangeScene > renewToken\|_blank](@renewToken-ZegoRangeScene)

#### 4. 支持多源采集能力

面向在线 KTV、一起看电影、看比赛等、视频会议、在线教育等音视频源丰富多样的互动场景，多源采集提供了灵活易用的音视频采集源与通道管理能力，大量减少开发者的开发及维护成本。

多源采集能力对屏幕共享、混音等常见能力的实现路径，进行缩短优化及归一化设计，从 3.1.0 版本后，您可以不用再通过自定义采集实现上述复杂的能力，详情请参考 [多源采集\|_blank](!Multi_source_acquisition) 。
...
相关 API 请参考 [enableHardwareDecoder\|_blank](@enableHardwareDecoder)
...
本次更新包含不兼容改动，详情请参考 [v3.0.0 升级指南\|_blank](!upgrade_guide4)。
</div>
...
新增 [enableVideoSuperResolution] 接口支持对某一条视频流进行超分辨率处理，从而得到更好的画质。超分辨率，简称超分，是客户端对拉取的视频流的宽和高进行实时倍增处理的技术，例如，从 640x360 超分为 1280x720，详情请参考 [超分辨率\|_blank](!super_resolution)。

相关 API 请参考 [enableVideoSuperResolution\|_blank](@enableVideoSuperResolution), [onPlayerVideoSuperResolutionUpdate\|_blank](@onPlayerVideoSuperResolutionUpdate)

#### 2. 场景化 AI 降噪新增在音乐场景下降噪的能力

注意：如需使用该功能，请联系 ZEGO 技术支持。

场景化 AI 降噪功能，在之前针对所有非人声进行降噪的基础上，新增支持在音乐场景下的降噪能力，通过识别音乐，智能调整降噪效果还原音乐音质。SDK 会实时对麦克风输入内容进行音乐检测，在声卡、弹唱或近场音乐场景下，自动调整降噪等级，保证音乐的高保真音质，详情请参考 [场景化 AI 降噪\|_blank](!AIDenoise)。
...
大型虚拟世界中，用户一般不需要感知较远的场景或远端用户，ZEGO 提供 AOI（Area Of Interest）能力，减免用户可见范围外的信息，极大减少客户流量成本以及用户端流量及性能消耗，详情请参考 [多人状态实时同步\|_blank](!state_sync)。

相关 API 请参考 [createRangeScene\|_blank](@createRangeScene)
...
依赖多人状态实时同步服务，根据云端用户位置自动拉取收听范用内的远端音视频并提供空间音效。单场景内最大支持 1 万用户同时开启麦克风及摄像头，用户默认拉取距离最近的 12 路（可配置）音视频，详情请参考 [万人范围音视频\|_blank](!Real_time_range)。
...
当前支持场景包括秀场直播、KTV、标准 1v1 音视频通话、高画质 1v1 音视频通话、标准语聊房、高音质语聊房，详情请参考 [场景化音视频配置\|_blank](!video_proflie)。
...
相关 API 请参考 [isVideoEncoderSupported\|_blank](@isVideoEncoderSupported), [isVideoDecoderSupported\|_blank](@isVideoDecoderSupported)
...
相关 API 请参考 [onPlayerRenderCameraVideoFirstFrame\|_blank](@onPlayerRenderCameraVideoFirstFrame)
...
废弃 [ZegoScenario] 场景枚举中的 [General]，[Communication]， [Live] 三种场景，详情请参考 [场景化音视频配置\|_blank](!video_proflie)。

#### 2. 废弃了对 bitcode 的支持

从 3.0.0 版本开始，iOS SDK 不再支持 bitcode，请参考 [Xcode 14 Release Notes\|_blank](https://developer.apple.com/documentation/xcode-release-notes/xcode-14-release-notes#Deprecations) 中关于废弃 bitcode 的说明。

#### 3. 删除了 [setDebugVerbose]、[setPlayStreamVideoLayer]、[enableAudioDataCallback] 等接口，详情请参考 [v3.0.0升级指南\|_blank](!upgrade_guide4)。
...
当前 SDK 对各种配置进行封装，对外提供简单的模式选择，在原有自定义模式基础上，新增固定比例模式、自适应模式及对齐模式，可有效降低开发者接入成本，详情请参考 [视频采集旋转\|blank](!collection_rotate)。

相关 API 请参考 [setAppOrientationMode\|_blank](@setAppOrientationMode)
...
相关 API 请参考 [enableCustomVideoCapture\|_blank](@enableCustomVideoCapture)
...
相关 API 请参考 [setAudioDeviceMode\|_blank](@setAudioDeviceMode)
...
相关 API 请参考 [enableAlignedAudioAuxData\|_blank](@enableAlignedAudioAuxData), [onAlignedAudioAuxData\|_blank](@onAlignedAudioAuxData), [enableAux\|_blank](@enableAux)

#### 6. 支持查询当前 SDK 具备的功能特性

由于 SDK 支持特性裁包，部分特性可能已被裁剪；可以使用此函数快速判断当前 SDK 是否支持指定的功能特性。

相关 API 请参考 [isFeatureSupported\|_blank](@isFeatureSupported)
...
相关 API 请参考 [onPlayerQualityUpdate\|_blank](@onPlayerQualityUpdate), [onPublisherQualityUpdate\|_blank](@onPublisherQualityUpdate), [onNetworkQuality\|_blank](@onNetworkQuality)
...
相关 API 请参考 [ZegoRangeAudio > setPositionUpdateFrequency\|_blank](@setPositionUpdateFrequency-ZegoRangeAudio)
...
相关 API 请参考 [enableCustomVideoProcessing\|_blank](@enableCustomVideoProcessing)
...
相关 API 请参考 [setActiveAudioChannel\|_blank](@setActiveAudioChannel)
...
相关 API 请参考 [ZegoMediaPlayer > clearView\|_blank](@clearView-ZegoMediaPlayer)
...
相关 API 请参考 [enableCameraAdaptiveFPS\|_blank](@enableCameraAdaptiveFPS)
...
相关 API 请参考 [initCopyrightedMusic\|_blank](@initCopyrightedMusic), [loginRoom\|_blank](@loginRoom)
...
相关 API 请参考 [onPlayerRecvAudioSideInfo\|_blank](@onPlayerRecvAudioSideInfo), [sendAudioSideInfo\|_blank](@sendAudioSideInfo)
...
相关 API 请参考 [ZegoAudioEffectPlayer > setPlaySpeed\|_blank](@setPlaySpeed-ZegoAudioEffectPlayer)
...
相关 API 请参考 [onPublisherStreamEvent\|_blank](@onPublisherStreamEvent), [onPlayerStreamEvent\|_blank](@onPlayerStreamEvent)
...
相关 API 请参考 [loginRoom\|_blank](@loginRoom), [logoutRoom\|_blank](@logoutRoom)

#### 7. 新增房间状态变化通知 [onRoomStateChanged]

当房间的连接状态发生变化时会触发 [onRoomStateChanged] 回调，通过 [ZegoRoomStateChangedReason] 参数提供更加详细的连接状态及状态变化原因。

相关 API 请参考 [onRoomStateChanged\|_blank](@onRoomStateChanged)
...
相关 API 请参考 [setMinVideoFpsForTrafficControl\|_blank](@setMinVideoFpsForTrafficControl), [setMinVideoResolutionForTrafficControl\|_blank](@setMinVideoResolutionForTrafficControl)

#### 2. 支持稳态语音设置检测周期参数

稳态语音默认检测周期为 3 秒，用户如需修改默认检测周期，可通过 [startAudioVADStableStateMonitor] 接口自定义检测周期参数。

相关 API 请参考 [startAudioVADStableStateMonitor\|_blank](@startAudioVADStableStateMonitor)
...
相关 API 请参考 [enableDebugAssistant\|_blank](@enableDebugAssistant)
...
相关 API 请参考 [muteUser\|_blank](@muteUser)
...
相关 API 请参考 [onPlayerQualityUpdate\|_blank](@onPlayerQualityUpdate)
...
相关 API 请参考 [setCustomVideoCaptureRegionOfInterest\|_blank](@setCustomVideoCaptureRegionOfInterest)
...
相关 API 请参考 [startEffectsEnv\|_blank](@startEffectsEnv), [stopEffectsEnv\|_blank](@stopEffectsEnv), [enableEffectsBeauty\|_blank](@enableEffectsBeauty), [setEffectsBeautyParam\|_blank](@setEffectsBeautyParam)
...
相关 API 请参考 [enableBeautify\|_blank](@enableBeautify), [enableEffectsBeauty\|_blank](@enableEffectsBeauty), [setBeautifyOption\|_blank](@setBeautifyOption), [setEffectsBeautyParam\|_blank](@setEffectsBeautyParam)
...
相关 API 请参考 [setCustomVideoCaptureDeviceState\|_blank](@setCustomVideoCaptureDeviceState)
...
相关 API 请参考 [enableSoundLevelMonitor\|_blank](@enableSoundLevelMonitor), [enableFrequencySpectrumMonitor\|_blank](@enableFrequencySpectrumMonitor)
...
相关 API 请参考 [sendSEISyncWithCustomVideo\|_blank](@sendSEISyncWithCustomVideo)
...
相关 API 请参考 [enableVirtualStereo\|_blank](@enableVirtualStereo)
...
相关 API 请参考 [setCustomVideoCaptureRotation\|_blank](@setCustomVideoCaptureRotation)
...
相关 API 请参考 [createRealTimeSequentialDataManager\|_blank](@createRealTimeSequentialDataManager)
...
相关 API 请参考 [onPlayerLowFpsWarning\|_blank](@onPlayerLowFpsWarning), [onPublisherStateUpdate\|_blank](@onPublisherStateUpdate), [onPlayerStateUpdate\|_blank](@onPlayerStateUpdate)
...
相关 API 请参考 [callExperimentalAPI\|_blank](@callExperimentalAPI)

#### 5. 媒体播放器支持在加载媒体资源时指定开始播放进度

媒体播放器新增 [loadResourceWithPosition] 接口，支持加载媒体资源时指定开始播放进度，单位为毫秒。

相关 API 请参考 [ZegoMediaPlayer > loadResourceWithPosition\|_blank](@loadResourceWithPosition-ZegoMediaPlayer)
...
相关 API 请参考 [isCameraFocusSupported\|_blank](@isCameraFocusSupported), [setCameraFocusMode\|_blank](@setCameraFocusMode), [setCameraExposureMode\|_blank](@setCameraExposureMode)
...
相关 API 请参考 [setPlayStreamsAlignmentProperty\|_blank](@setPlayStreamsAlignmentProperty)
...
相关 API 请参考 [startAudioVADStableStateMonitor\|_blank](@startAudioVADStableStateMonitor), [stopAudioVADStableStateMonitor\|_blank](@stopAudioVADStableStateMonitor), [onAudioVADStateUpdate\|_blank](@onAudioVADStateUpdate)
...
相关 API 请参考 [setElectronicEffects\|_blank](@setElectronicEffects)
...
相关 API 请参考 [renewToken\|_blank](@renewToken), [onRoomTokenWillExpire\|_blank](@onRoomTokenWillExpire)
...
相关 API 请参考 [setReplayKitMicrophoneVolume\|_blank](@setReplayKitMicrophoneVolume), [setReplayKitApplicationVolume\|_blank](@setReplayKitApplicationVolume)
...
相关 API 请参考 [setStreamAlignmentProperty\|_blank](@setStreamAlignmentProperty)
...
相关 API 请参考 [startPublishingStream\|_blank](@startPublishingStream), [setStreamAlignmentProperty\|_blank](@setStreamAlignmentProperty), [onNetworkTimeSynchronized\|_blank](@onNetworkTimeSynchronized)
...
相关 API 请参考 [onProcessCapturedAudioData\|_blank](@onProcessCapturedAudioData)
...
相关 API 请参考 [onProcessRemoteAudioData\|_blank](@onProcessRemoteAudioData)
...
相关 API 请参考 [onProcessPlaybackAudioData\|_blank](@onProcessPlaybackAudioData)
...
相关 API 请参考 [isVideoEncoderSupported\|_blank](@isVideoEncoderSupported), [isVideoDecoderSupported\|_blank](@isVideoDecoderSupported), [enableH265EncodeFallback\|_blank](@enableH265EncodeFallback), [onPublisherVideoEncoderChanged\|_blank](@onPublisherVideoEncoderChanged)
...
相关 API 请参考 [enableCustomAudioPlaybackProcessing\|_blank](@enableCustomAudioPlaybackProcessing), [setCustomAudioProcessHandler\|_blank](@setCustomAudioProcessHandler), [onProcessPlaybackAudioData\|_blank](@onProcessPlaybackAudioData)

#### 3. 支持回调远端扬声器设备状态

与远端用户连麦成功后，当远端扬声器设备状态发生变更时，例如开/关扬声器，可以通过 [onRemoteSpeakerStateUpdate] 回调监听。

相关 API 请参考 [onRemoteSpeakerStateUpdate\|_blank](@onRemoteSpeakerStateUpdate)
...
相关 API 请参考 [ZegoMediaPlayer > loadResource\|_blank](@loadResource-ZegoMediaPlayer)
...
相关 API 请参考 [createRangeAudio\|_blank](@createRangeAudio), [destroyRangeAudio\|_blank](@destroyRangeAudio), [setEventHandler\|_blank](@setEventHandler), [setAudioReceiveRange\|_blank](@setAudioReceiveRange), [updateSelfPosition\|_blank](@updateSelfPosition), [updateAudioSource\|_blank](@updateAudioSource), [enableSpatializer\|_blank](@enableSpatializer), [enableMicrophone\|_blank](@enableMicrophone), [enableSpeaker\|_blank](@enableSpeaker), [setRangeAudioMode\|_blank](@setRangeAudioMode), [setTeamID\|_blank](@setTeamID)
...
相关 API 请参考 [startSoundLevelMonitor\|_blank](@startSoundLevelMonitor), [onCapturedSoundLevelInfoUpdate\|_blank](@onCapturedSoundLevelInfoUpdate), [onRemoteSoundLevelInfoUpdate\|_blank](@onRemoteSoundLevelInfoUpdate)

#### 4. 媒体播放器支持播放二进制文件

当开发者已经将播放数据写入内存后，可直接使用媒体播放器进行播放，无需再写成文件进行播放。

相关 API 请参考 [ZegoMediaPlayer > loadResourceFromMediaData\|_blank](@loadResourceFromMediaData-ZegoMediaPlayer)

#### 5. 新增摄像头曝光补偿功能

开发者可以在开启预览后调用 [setCameraExposureCompensation] 接口设置摄像头曝光补偿数值，取值范围为 [-1, 1]，取值越小画面越暗，取值越大画面越亮。

相关 API 请参考 [setCameraExposureCompensation\|_blank](@setCameraExposureCompensation)
...
相关 API 请参考 [setDummyCaptureImagePath\|_blank](@setDummyCaptureImagePath)
...
相关 API 请参考 [setRoomMode\|_blank](@setRoomMode), [loginRoom\|_blank](@loginRoom)

#### 2. 优化了退出房间逻辑

开发者不需要填写 roomID 就可以调用 [logoutRoom] 接口退出当前所在的房间。若使用了多房间功能，则调用该接口会退出所有房间。

相关 API 请参考 [logoutRoom\|_blank](@logoutRoom)
...
相关 API 请参考 [loginRoom\|_blank](@loginRoom), [renewToken\|_blank](@renewToken), [onRoomTokenWillExpire\|_blank](@onRoomTokenWillExpire)
...
相关 API 请参考 [enablePlayStreamVirtualStereo\|_blank](@enablePlayStreamVirtualStereo)
...
相关 API 请参考 [startAudioDataObserver\|_blank](@startAudioDataObserver)
...
相关 API 请参考 [ZegoMediaPlayer > setNetWorkResourceMaxCache\|_blank](@setNetWorkResourceMaxCache-ZegoMediaPlayer), [ZegoMediaPlayer > getNetWorkResourceCache\|_blank](@getNetWorkResourceCache-ZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > setNetWorkBufferThreshold\|_blank](@setNetWorkBufferThreshold-ZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > takeSnapshot\|_blank](@takeSnapshot-ZegoMediaPlayer)

#### 2. 支持设置和获取音频质量配置

可根据需要设置和获取音频编码类型、码率，音频声道的组合值。

相关 API 请参考 [setAudioConfig\|_blank](@setAudioConfig), [getAudioConfig\|_blank](@getAudioConfig)

#### 3. 支持设置触发流量控制的关注因素

当通过 [enableTrafficControl] 接口开启了指定推流通道的流量控制后，可通过 [setTrafficControlFocusOn] 接口控制是否因为远端网络状况差而启动流量控制。

相关 API 请参考 [setTrafficControlFocusOn\|_blank](@setTrafficControlFocusOn)

#### 4. 新增本地预览首帧渲染回调

第一帧视频数据被渲染完之后会收到此回调。

相关 API 请参考 [onPublisherRenderVideoFirstFrame\|_blank](@onPublisherRenderVideoFirstFrame)

#### 5. 媒体播放器新增精确 seek 功能

普通 seek 指定的时间戳可能是非 I 帧，进而返回指定时间戳前后的 I 帧，不是很精确。而精准 seek 当指定的时间戳不是 I 帧时，会通过指定时间戳前后的 I 帧去解指定时间戳的那帧数据。

相关 API 请参考 [ZegoMediaPlayer > enableAccurateSeek\|_blank](@enableAccurateSeek-ZegoMediaPlayer)

#### 6. 支持停止或恢复所有正在拉流的音频数据

当需要同时停止拉流中所有流的音频数据时，可以使用此功能。

相关 API 请参考 [muteAllPlayStreamAudio\|_blank](@muteAllPlayStreamAudio)

#### 7. 支持停止或恢复所有正在拉流的视频数据

当需要同时停止拉流中所有流的视频数据时，可以使用此功能。

相关 API 请参考 [muteAllPlayStreamVideo\|_blank](@muteAllPlayStreamVideo)
...
相关 API 请参考 [setApiCalledCallback\|_blank](@setApiCalledCallback)
...
相关 API 请参考 [setPlayStreamVideoType\|_blank](@setPlayStreamVideoType)
...
相关 API 请参考 [setAudioRouteToSpeaker\|_blank](@setAudioRouteToSpeaker)

#### 5. 新增 [setAllPlayStreamVolume] 接口，用于设置所有拉流声音大小

本端用户可控制所有音频流的播放音量。

相关 API 请参考 [setAllPlayStreamVolume\|_blank](@setAllPlayStreamVolume)

#### 6. 新增 [startNetworkProbe] 接口，用于开始网络探测。新增 [stopNetworkProbe] 接口，用于结束网络探测

在推拉流之前通过网络探测定位一些可能存在的网络问题。

相关 API 请参考 [startNetworkProbe\|_blank](@startNetworkProbe), [stopNetworkProbe\|_blank](@stopNetworkProbe)
...
相关 API 请参考 [setPublishWatermark\|_blank](@setPublishWatermark)

#### 2. 修复了在极端情况下发送自定义视频采集数据时，可能导致崩溃的问题

#### 3. 修复了音频自定义采集向 SDK 塞数据时开发者音频通道不合理传参导致的崩溃问题

相关 API 请参考 [sendCustomAudioCaptureAACData\|_blank](@sendCustomAudioCaptureAACData), [sendCustomAudioCapturePCMData\|_blank](@sendCustomAudioCapturePCMData)

#### 4. 修复了抛解码的码流时在某些设备可能会出现渲染画面任意旋转的问题

相关 API 请参考 [onRemoteVideoFrameEncodedData\|_blank](@onRemoteVideoFrameEncodedData)
...
相关 API 请参考 [onRoomStateUpdate\|_blank](@onRoomStateUpdate)
...
低延迟直播专注于提供稳定可靠的直播服务，相比于标准视频直播产品，音画延迟更低，同步性更强，弱网抗性更好，能为用户带来毫秒级的直播体验。通常用于教育大班课、秀场直播、电商直播、一起看、在线拍卖等场景。详情请参考：[低延迟直播](#6777)。
...
旧接口跟随了 [setEngineConfig] 接口的生命周期，不够灵活。在允许了引擎启动前设置 [自定义视频采集] 之后，Express SDK 新增了独立的 [enableCustomVideoCapture] 接口用于设置自定义视频采集，详情请参考 [接口变更说明](#6762)。

相关 API 请参考 [enableCustomVideoCapture\|_blank](@enableCustomVideoCapture)

#### 2. 删除 [ZegoEngineConfig] 中的 [customVideoRenderConfig]，请使用 [ZegoExpressEngine] 的 [enableCustomVideoRender] 接口代替

旧接口跟随了 [setEngineConfig] 接口的生命周期，不够灵活。在允许了引擎启动前设置 [自定义视频渲染] 之后，Express SDK 新增了独立的 [enableCustomVideoRender] 接口用于设置自定义视频渲染，详情请参考 [接口变更说明](#6762)。

相关 API 请参考 [enableCustomVideoRender\|_blank](@enableCustomVideoRender)

#### 3. 删除 [destroyEngine()] 接口，请使用 [destroyEngine(callback)] 代替

新增了一个带 [callback] 参数的销毁引擎接口，若开发者有切换多个音视频 SDK 的需求，则可以通过收到回调时认为 ZEGO SDK 对设备硬件资源的占用已释放完成。若无监听必要，对 [callback] 参数传 null 即可，详情请参考 [接口变更说明](#6762)。

相关 API 请参考 [destroyEngine\|_blank](@destroyEngine)

#### 4. 删除 [onRemoteAudioData] 回调，请使用 [onPlaybackAudioData] 代替

旧回调命名与实际功能不符，因此删除改名，详情请参考 [接口变更说明](#6762)。

相关 API 请参考 [onPlaybackAudioData\|_blank](@onPlaybackAudioData)

#### 5. 删除 [muteAudioOutput] 接口，请使用 [muteSpeaker] 代替

旧接口命名风格和语义不清晰，[muteSpeaker] 使用 Speaker 的定义与 Microphone 对应，详情请参考 [接口变更说明](#6762)。

相关 API 请参考 [muteSpeaker\|_blank](@muteSpeaker)

#### 6. 删除 [ZegoMediaPlayer] 类中的 [createMediaPlayer] 接口，请使用 [ZegoExpressEngine] 类中的 [createMediaPlayer] 代替

媒体播放器的生命周期跟随引擎，故变更为 ZegoExpressEngine 类的同名实例方法，详情请参考 [接口变更说明](#6762)。

相关 API 请参考 [createMediaPlayer\|_blank](@createMediaPlayer)

#### 7. 删除 [ZegoMediaPlayer] 中的 [volume] 和 [getVolume]，请使用 [setPublishVolume]、[getPublishVolume]、[setPlayVolume]、[getPlayVolume] 代替

引擎提供了分别获取播放器推流音量和本地播放音量的功能，更加精准的获取音量，故废弃原来的统一获取接口，详情请参考 [接口变更说明](#6762)。

相关 API 请参考 [ZegoMediaPlayer > setPublishVolume\|_blank](@setPublishVolume-ZegoMediaPlayer), [ZegoMediaPlayer > setPlayVolume\|_blank](@setPlayVolume-ZegoMediaPlayer)
...
相关 API 请参考 [startNetworkSpeedTest\|_blank](@startNetworkSpeedTest), [stopNetworkSpeedTest\|_blank](@stopNetworkSpeedTest), [onNetworkSpeedTestQualityUpdate\|_blank](@onNetworkSpeedTestQualityUpdate)

#### 2. 新增网络模式变更通知回调

当设备的网络模式改变时，例如从 WiFi 切换到 5G，或断网等情况时，将会抛出此回调。

相关 API 请参考 [onNetworkModeChanged\|_blank](@onNetworkModeChanged)

#### 3. 新增摄像头变焦功能

通过 SDK 设置摄像头的变焦倍数，可实现拍摄时放大远方物体的效果，详细功能实现请参考 [进阶功能 - 变焦]。

相关 API 请参考 [getCameraMaxZoomFactor\|_blank](@getCameraMaxZoomFactor), [setCameraZoomFactor\|_blank](@setCameraZoomFactor)

#### 4. 新增音频设备路由变更通知回调

当有耳机插拔、扬声器和听筒切换等音频路由发生变化时会抛出此回调。

相关 API 请参考 [onAudioRouteChange\|_blank](@onAudioRouteChange)

<p class="hthree">问题修复</p>

#### 1. 修复了本地录制功能在异常情况下偶现崩溃的问题

相关 API 请参考 [startRecordingCapturedData\|_blank](@startRecordingCapturedData), [stopRecordingCapturedData\|_blank](@stopRecordingCapturedData)
...
相关 API 请参考 [loginRoom\|_blank](@loginRoom), [logoutRoom\|_blank](@logoutRoom), [switchRoom\|_blank](@switchRoom)

<p class="hthree">问题修复</p>

#### 1. 修复了音效播放器设置指定的播放进度时偶现爆音的问题

相关 API 请参考 [ZegoAudioEffectPlayer > seekTo\|_blank](@seekTo-ZegoAudioEffectPlayer)
...
相关 API 请参考 [startPerformanceMonitor\|_blank](@startPerformanceMonitor), [stopPerformanceMonitor\|_blank](@stopPerformanceMonitor)

#### 2. 新增流媒体加密功能

支持使用 AES-128/192/256 对流媒体数据加密。

相关 API 请参考 [setPublishStreamEncryptionKey\|_blank](@setPublishStreamEncryptionKey), [setPlayStreamDecryptionKey\|_blank](@setPlayStreamDecryptionKey)
...
相关 API 请参考 [sendBroadcastMessage\|_blank](@sendBroadcastMessage)

#### 3. 本地媒体录制新增支持 AAC 文件格式

相关 API 请参考 [startRecordingCapturedData\|_blank](@startRecordingCapturedData)
...
相关 API 请参考 [setReverbAdvancedParam\|_blank](@setReverbAdvancedParam), [setReverbPreset\|_blank](@setReverbPreset), [setVoiceChangerPreset\|_blank](@setVoiceChangerPreset)
...
相关 API 请参考 [onRoomStreamUpdate\|_blank](@onRoomStreamUpdate)

#### 4. 新增按流 ID 形式获取拉流端的音频原始数据

相关 API 请参考 [onPlayerAudioData\|_blank](@onPlayerAudioData)
...
相关 API 请参考 [mutePlayStreamAudio\|_blank](@mutePlayStreamAudio)

#### 3. 优化 [mutePlayStreamVideo] 调用时机，在拉流前和拉流后设置均能生效

相关 API 请参考 [mutePlayStreamVideo\|_blank](@mutePlayStreamVideo)
...
为了支持更多混响参数，设置更加丰富的混响效果，在 1.18.0 及以上版本废弃 [setReverbParam] 接口，请使用带 `ZegoReverbAdvancedParam` 类型参数的 [setReverbAdvancedParam] 接口代替，详情请参考 [接口变更说明](#6762)。

相关 API 请参考 [setReverbAdvancedParam\|_blank](@setReverbAdvancedParam)
...
相关 API 请参考 [loginRoom\|_blank](@loginRoom), [sendBroadcastMessage\|_blank](@sendBroadcastMessage), [sendBarrageMessage\|_blank](@sendBarrageMessage), [sendCustomCommand\|_blank](@sendCustomCommand)

<p class="hthree">问题修复</p>

#### 1. 修复辅流发送 SEI 时，对端接收不到的问题

相关 API 请参考 [sendSEI\|_blank](@sendSEI)

#### 2. 修复 iOS 屏幕采集推流偶现音画不同步的问题

相关 API 请参考 [handleReplayKitSampleBuffer\|_blank](@handleReplayKitSampleBuffer)
...
相关 API 请参考 [setReverbEchoParam\|_blank](@setReverbEchoParam)

#### 3. 新增屏幕采集 (ReplayKit) 自适应画面旋转功能

默认在推流端处理自适应旋转画面，也支持在拉流端做旋转处理以降低推流端的内存开销，具体使用方法请参考 [视频进阶-自定义视频采集]。

相关 API 请参考 [prepareForReplayKit\|_blank](@prepareForReplayKit), [handleReplayKitSampleBuffer\|_blank](@handleReplayKitSampleBuffer)

#### 4. 媒体播放器新增支持设置变声效果

通过改变用户的音调，使输出的声音在感官上与原始声音不同，实现男声变女生等多种效果。

相关 API 请参考 [ZegoMediaPlayer > setVoiceChangerParam\|_blank](@setVoiceChangerParam-ZegoMediaPlayer)

#### 5. 新增对推流或拉流画面截图的功能

支持在推拉流过程中对画面进行截图，可用于鉴黄等场景。

相关 API 请参考 [takePublishStreamSnapshot\|_blank](@takePublishStreamSnapshot), [takePlayStreamSnapshot\|_blank](@takePlayStreamSnapshot)

#### 6. 新增支持瞬态噪声抑制

该功能可用于抑制敲击键盘、桌子等瞬态噪声。

相关 API 请参考 [enableTransientANS\|_blank](@enableTransientANS)

#### 7. 媒体播放器新增支持切换音轨

当媒体文件中包含多个音轨时（例如原声和伴奏），支持切换音轨进行播放。

相关 API 请参考 [ZegoMediaPlayer > setAudioTrackIndex\|_blank](@setAudioTrackIndex-ZegoMediaPlayer)
...
相关 API 请参考 [setVoiceChangerPreset\|_blank](@setVoiceChangerPreset), [setVoiceChangerParam\|_blank](@setVoiceChangerParam)

#### 2. 废弃设置混响参数函数 [setReverbParam] 的 `param` 参数的预设枚举构造函数，该函数仅用于微调设置具体的混响参数值，如需使用预设枚举来设置混响，请使用新增的 [setReverbPreset] 函数。

相关 API 请参考 [setReverbPreset\|_blank](@setReverbPreset), [setReverbAdvancedParam\|_blank](@setReverbAdvancedParam)
...
相关 API 请参考 [createAudioEffectPlayer\|_blank](@createAudioEffectPlayer), [destroyAudioEffectPlayer\|_blank](@destroyAudioEffectPlayer)

<p class="hthree">问题修复</p>

#### 1. 修复 [onRoomStreamExtraInfoUpdate] 可能会在预期外回调空列表的问题

相关 API 请参考 [onRoomStreamExtraInfoUpdate\|_blank](@onRoomStreamExtraInfoUpdate)
...
相关 API 请参考 [onRoomUserUpdate\|_blank](@onRoomUserUpdate)
...
相关 API 请参考 [setAudioCaptureStereoMode\|_blank](@setAudioCaptureStereoMode), [setAudioConfig\|_blank](@setAudioConfig)

#### 3. 开启声浪/音频频谱监控的接口支持设置回调间隔时间

开发者可自行控制声浪/音频频谱监控的回调间隔，默认为 100 ms，取值范围为 [100, 3000]。

相关 API 请参考 [startSoundLevelMonitor\|_blank](@startSoundLevelMonitor), [startAudioSpectrumMonitor\|_blank](@startAudioSpectrumMonitor)
...
相关 API 请参考 [onEncodedDataTrafficControl\|_blank](@onEncodedDataTrafficControl)
...
相关 API 请参考 [enableCustomVideoCapture\|_blank](@enableCustomVideoCapture), [enableCustomVideoRender\|_blank](@enableCustomVideoRender), [enableCustomAudioIO\|_blank](@enableCustomAudioIO)

#### 2. 修复 onPlayerRecvSEI 会接收到编码器或者视频文件本身附带的 SEI 的问题。

相关 API 请参考 [onPlayerRecvSEI\|_blank](@onPlayerRecvSEI)
...
相关 API 请参考 [setRoomExtraInfo\|_blank](@setRoomExtraInfo)

#### 3. 新增音频前/后处理功能

允许开发者在采集音频数据后，或者拉取远端音频数据渲染前，对音频数据进行自定义处理。该功能通常用于 “变声”、“美声” 等场景。

相关 API 请参考 [enableCustomAudioCaptureProcessing\|_blank](@enableCustomAudioCaptureProcessing), [enableCustomAudioRemoteProcessing\|_blank](@enableCustomAudioRemoteProcessing)
...
相关 API 请参考 [setCaptureVolume\|_blank](@setCaptureVolume), [setPlayVolume\|_blank](@setPlayVolume)
...
相关 API 请参考 [setAudioEqualizerGain\|_blank](@setAudioEqualizerGain)
...
相关 API 请参考 [enableVirtualStereo\|_blank](@enableVirtualStereo), [setVoiceChangerParam\|_blank](@setVoiceChangerParam), [setReverbAdvancedParam\|_blank](@setReverbAdvancedParam)
...
相关 API 请参考 [enableCustomAudioIO\|_blank](@enableCustomAudioIO)
...
相关 API 请参考 [sendBarrageMessage\|_blank](@sendBarrageMessage)
