...
可调用 [ZegoCustomAudioConfig\|_blank](@-ZegoCustomAudioConfig) 设置 `sourceType = ZegoAudioSourceTypeCustom`，再调用 [enableCustomAudioIO\|_blank](@enableCustomAudioIO) 接口开启自定义音频 IO 功能。
...
打开音频渲染设备，使用 [fetchCustomAudioRenderPCMData\|_blank](@fetchCustomAudioRenderPCMData) 从引擎中获取音频数据，拿到音频数据后再通过渲染设备播放。
