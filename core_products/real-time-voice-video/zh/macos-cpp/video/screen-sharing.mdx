...
- 需要自己实现屏幕共享功能，请参考 [如何通过自定义采集实现屏幕共享？](https://doc-zh.zego.im/faq/custom-capture-screen-share)。
</Warning>
...
```cpp
engine->setAudioSource(ZEGO_AUDIO_SOURCE_TYPE_SCREEN_CAPTURE, ZEGO_PUBLISH_CHANNEL_MAIN);
```
...
source->enableAudioCapture(true, config);
```
<a id="callback"></a>

### 设置事件回调

调用 [setEventHandler](@setEventHandler-IZegoScreenCaptureSource) 接口设置共享源事件回调。
