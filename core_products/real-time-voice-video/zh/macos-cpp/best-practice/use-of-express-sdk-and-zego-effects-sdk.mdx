...
- 已在项目中集成 ZEGO Effects SDK，详情请参考“AI 美颜”的 [快速开始 - 集成\|_blank](#11766)。
- 已获取到 ZEGO Effects SDK 唯一的鉴权文件，详情请参考“AI 美颜”的 [快速开始 - 在线鉴权\|_blank](#12199)。
...
2. 获取视频原始数据，可通过 ZEGO Express SDK 的 [自定义视频采集\|_blank](#11795) 或 [自定义视频前处理\|_blank](#11761) 两种方式获取。
...
1. 导入 Effects 模型和资源。
    ...
    ZEGO Effects SDK 支持的所有资源和模型请参考“AI 美颜”的 [快速开始 - 导入资源和模型\|_blank](#11768)。
...
3. 初始化 Effects 对象。
    ...
    调用 [zego_effects_init_env\|_blank](/article/api?doc=Effects_SDK_API~c_macos~class~ZegoEffects#zego-effects-init-env) 接口初始化 Effects 对象，需要传入待处理视频图像数据的宽高。
...
调用 [createEngine\|_blank](/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-zego-express-sdk#create-engine) 接口，初始化 ZEGO Express SDK。
...
- **方式一：自定义视频前处理**
    ...
    开发者通过调用 [enableCustomVideoProcessing\|_blank](/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#enable-custom-video-processing) 接口，开启自定义视频前处理；开启后，ZEGO Express SDK 内部会采集视频数据；采集完成后，可以通过 [onCapturedUnprocessedCVPixelBuffer\|_blank](/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-custom-video-process-handler#on-captured-unprocessed-cv-pixel-buffer) 回调接口，获取采集到的视频原始数据。
    ...
    具体的原理可参考“实时音视频”的 [自定义视频前处理\|_blank](#11761)。

<a id="CustomCollection"></a>

- **方式二：自定义视频采集**

    自定义视频采集，主要依赖开发者自行采集视频数据，具体方式请参考“实时音视频”的 [自定义视频采集\|_blank](#11795)。
...
- **方式一：自定义视频前处理**
    ...
    在 [onCapturedUnprocessedCVPixelBuffer\|_blank](/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-custom-video-process-handler&jumpType=route#on-captured-unprocessed-cv-pixel-buffer) 回调中，获取到视频原始数据后，调用 ZEGO Effects SDK 的相关接口，进行 AI 美颜处理（请参考 [美颜\|_blank](#11769)、[美型\|_blank](#11770)、[背景分割\|_blank](#11771)、[人脸检测\|_blank](#11772)、[挂件\|_blank](#11773)、[滤镜\|_blank](#11774)），并将处理后的数据，返回给 ZEGO Express SDK。
...
ZEGO Express SDK 调用 [startPublishingStream\|_blank](/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-express-engine#start-publishing-stream) 接口，传入处理后的数据流 streamID，开始推流，发送给云服务器。
...
ZEGO Express SDK 开始推流后，远端用户可以调用 [startPlayingStream\|_blank](/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-express-engine#start-playing-stream) 接口，传入处理后的数据流 streamID，拉取视频数据，进行播放。
