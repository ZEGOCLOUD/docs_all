...
在调用 [startPublishingStream\|_blank](@startPublishingStream) 之前调用 [setVideoConfig\|_blank](@setVideoConfig) 设置 [zegoVideoConfig\|_blank](@zegoVideoConfig) 类中的参数 “codecID”，即可开启/关闭分层视频编码功能。
...
在推流端开启了分层视频编码后，拉流端调用 [setPlayStreamVideoType\|_blank](@setPlayStreamVideoType)，拉流端默认会根据网络情况拉取合适的视频分层，例如弱网只拉取基本层。开发者也可以传入具体的拉流参数以拉取特定的视频分层。该接口在拉流前后皆可调用。
...
#### 4.2.1 开启硬件编码
若开发者需要开启硬件编码，可调用 [enableHardwareEncoder\|_blank](@enableHardwareEncoder)。
...
#### 4.2.2 开启硬件解码
若开发者需要开启硬件解码，可调用 [enableHardwareDecoder\|_blank](@enableHardwareDecoder)。
...
### 4.3 设置视频编码方式
在调用 [startPublishingStream\|_blank](@startPublishingStream) 之前调用 [setVideoConfig\|_blank](@setVideoConfig) 设置 “VideoConfig” 类下的参数 “codecID”，即可设置视频编码方式。目前，支持的视频编码方式如下：
...
|方法 | 描述  |
| ---- | ---- |
| [setVideoConfig \|_blank](@setVideoConfig) | 设置视频参数 |
|[enableHardwareEncoder\|_blank](@enableHardwareEncoder)|开/关硬件编码|
|[enableHardwareDecoder\|_blank](@enableHardwareDecoder)|开/关硬件解码|
