...
- 需要自己实现屏幕共享功能，请参考 [如何通过自定义采集实现屏幕共享？\|_blank](http://doc-zh.zego.im/faq/Express_Share_Screen?product=HybridHierarchicalDeliverySystem&platform=windows)。
</div>
...
SDK 可以通过 [getScreenCaptureSources\|_blank](@getScreenCaptureSources) 获取当前可共享的所有窗口信息。
...
通过上述窗口信息里的窗口 ID 与窗口类型，调用 [createScreenCaptureSource\|_blank](@createScreenCaptureSource) 接口创建屏幕共享源对象。
...
SDK 推流的视频源默认为摄像头源，如果需要推屏幕共享源，需要通过 [setVideoSource\|_blank](@setVideoSource) 进行切换为屏幕共享。
...
调用 [startCapture\|_blank](@startCapture) 接口可以共享窗口画面。
...
调用 [updateCaptureSource\|_blank](@updateCaptureSource) 接口可以更新共享窗口画面。
...
用户可以调用 [updateCaptureRegion\|_blank](@updateCaptureRegion) 接口动态更新共享窗口区域，其中设置为 (0, 0, 0, 0) 可恢复整个窗口共享。
...
调用 [setExcludeWindowList\|_blank](@setExcludeWindowList) 接口可以过滤掉共享的屏幕中指定窗口画面，只适用于共享整个屏幕时进行设置。
...
用户可以调用 [enableWindowActivate\|_blank](@enableWindowActivate) 接口来激活当前共享的窗口。
...
调用 [enableCursorVisible\|_blank](@enableCursorVisible) 接口进行显示或隐藏鼠标。
...
调用 [setEventHandler\|_blank](@setEventHandler-IZegoScreenCaptureSource) 接口设置共享源事件回调。
...
用户可以调用 [stopCapture\|_blank](@stopCapture) 接口停止共享。
...
不再需要使用屏幕采集功能时，可调用 [destroyScreenCaptureSource\|_blank](@destroyScreenCaptureSource) 接口销毁屏幕采集源对象。
