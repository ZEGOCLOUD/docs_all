...
请参考 [快速开始 - 实现流程](9976#2_1) 的 “2.1 创建引擎”、“2.2 登录房间”。      

### 开始推流

请参考 [快速开始 - 实现流程](9976#2_3) 的 “2.3 推流”。

### 开始转推

当推流成功后，调用 [addPublishCdnUrl](@addPublishCdnUrl) 增加动态转推至 CDN 的 URL，即可将已经成功推向 ZEGO 实时云的音视频流动态向第三方 CDN 进行转推。支持的转推地址格式为 “rtmp”。                      
...
- 若开发者有转推到多家第三方 CDN 厂商的需求，可使用同一个流 ID 多次调用 [addPublishCdnUrl](@addPublishCdnUrl)（URL 需要不同）。
...
<Accordion title="监听 CDN 回调的状态通知" defaultOpen="false">
#### 添加/删除转推 CDN 地址状态回调
开发者可通过注册 [onPublisherRelayCDNStateUpdate](@onPublisherRelayCDNStateUpdate) 获取添加/删除转推 CDN 地址状态回调。在 ZEGO 实时音视频云将音视频流转推到 CDN 后，如果 CDN 转推状态发生变化，例如出现转推停止或转推重试，将会收到此回调。
...
调用 [removePublishCdnUrl](@removePublishCdnUrl) 即可删除动态转推至 CDN 的 URL。
...
若要使用直推 CDN 功能，必须在推流前调用 [enablePublishDirectToCDN](@enablePublishDirectToCDN)。

<div class = 'mk-warning'>

调用此接口将音视频流直接推往 CDN 之后，调用 [addPublishCdnUrl](@addPublishCdnUrl) 与 [removePublishCdnUrl](@removePublishCdnUrl) 动态转推至 CDN 不再生效，因为这两个接口是从 ZEGO 实时音视频云将音视频流转推或停止转推到 CDN，若将音视频流直接推往 CDN 则无法通过 ZEGO 实时音视频云将音视频流再动态转推至 CDN。
...
- 当开发者使用 ZEGO 配置的 CDN 进行直推时，则可以直接通过 streamID 进行拉流，请参考 [快速开始 - 实现流程](9976#2_4) 的 “2.4 拉流”。

- 当音视频流转推 CDN 成功后，开发者希望用户从 CDN 进行拉流时，需要使用传入 URL 的自定义拉流方式进行拉流，而不能通过流 ID 进行拉流。

本节将简要介绍通过 URL 拉流的操作步骤。详细内容可参考 [推拉流进阶 - 通过 URL 拉流](https://doc-zh.zego.im/article/1183)。

### 配置拉流参数

直接通过 CDN 的 URL 地址拉流，需要使用 [ZegoCDNConfig](@-ZegoCDNConfig) 对象来填入 URL 参数，当从 CDN 拉流时存在拉流鉴权时，还需要通过 “authParam” 字段填入鉴权参数。
...
调用 [startPlayingStream](@startPlayingStream) 并传入设置好的拉流参数来从 CDN 拉流。
...
拉流时，如果出现错误，请参考 [常见错误码 - 1004xxx 拉流相关的错误码](4378#5)。

## 7 API 参考列表
	
| 方法 | 描述 |
|-------|--------|
| [enablePublishDirectToCDN](@enablePublishDirectToCDN) | 使能直推 CDN |
| [startPublishingStream](@startPublishingStream) | 开始推流 |
| [addPublishCDNURL](@addPublishCDNURL) | 增加转推至 CDN 的 URL |
| [removePublishCDNURL](@removePublishCDNURL) | 删除转推至 CDN 的 URL |
| [stopPublishingStream](@stopPublishingStream) | 停止推流 |
| [startPlayingStream ](@startPlayingStream) | 开始通过 URL 拉流 |
