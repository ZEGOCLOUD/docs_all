...
- 如果您已经通过 ZEGO 旧版 [屏幕共享](https://doc-zh.zego.im/article/3169) 产品实现屏幕共享功能，或者需要自己实现屏幕共享功能，请参考 [如何通过自定义采集实现屏幕共享？](http://doc-zh.zego.im/faq/Express_Share_Screen?product=HybridHierarchicalDeliverySystem&platform=windows)。
</Warning>
...
SDK 可以通过 [getScreenCaptureSourcesWithThumbnailSize](@getScreenCaptureSourcesWithThumbnailSize) 获取当前可共享的所有窗口信息。
...
SDK 推流的声音源默认为麦克风源，如果需要推屏幕共享源，需要通过 [setAudioSource](@setAudioSource) 进行切换为屏幕共享。

```objc
[engine setAudioSource:ZegoAudioSourceTypeScreenCapture channel:ZegoPublishChannelMain];
```
...
用户可以调用 [updateCaptureRegion](@updateCaptureRegion) 接口动态更新共享窗口区域，其中设置为 (0, 0, 0, 0) 可恢复整个窗口共享。
...
调用 [setExcludeWindowList](@setExcludeWindowList) 接口可以过滤掉共享的屏幕中指定窗口画面，只适用于共享整个屏幕时进行设置。
...
用户可以调用 [enableWindowActivate](@enableWindowActivate) 接口来激活当前共享的窗口。
...
调用 [enableCursorVisible](@enableCursorVisible) 接口进行显示或隐藏鼠标。
...
调用 [enableAudioCapture](@enableAudioCapture) 接口开启系统声卡采集。

```objc
ZegoScreenCaptureAudioConfig *config = [[ZegoScreenCaptureAudioConfig alloc] init];
[source enableAudioCapture:true config:config];
```
...
调用 [setEventHandler](@setEventHandler-ZegoScreenCaptureSource) 接口设置共享源事件回调。
...
不再需要使用屏幕采集功能时，可调用 [destroyScreenCaptureSource](@destroyScreenCaptureSource) 接口销毁屏幕采集源对象。
...
在屏幕共享时，您可以调用 [enableHightLight](@enableHightLight) 接口对屏幕区域或者窗口启用描边，设置描边的颜色和宽度。
...
## 注意事项
<Warning title="注意">Mac 平台需使用虚拟声卡才能采集系统音频。</Warning>

ZEGO Express SDK 提供自实现的虚拟音频插件，请按以下步骤将插件集成到项目：
1. 调用 [callExperimentalAPI](@callExperimentalAPI) 接口安装插件
    - 参数值为 `{"method": "liveroom.audio.install_mac_virtual_audio_device", "params":{}}`
2. 监听 [onRecvExperimentalAPI](@onRecvExperimentalAPI) 回调安装结果：`{"method":"liveroom.audio.install_mac_virtual_audio_device","result":0}`，即安装成功

<Note title="说明">
- 只需要安装一次即可，重复安装会从 [onRecvExperimentalAPI](@onRecvExperimentalAPI) 接收到安装成功结果。
- 需要在创建引擎后，预览/推流/拉流前调用
- 插件会自动安装到 `/Library/Audio/Plug-Ins/HAL` 目录并重启系统音频，音频 MIDI 设置应用中会显示Zego的虚拟音频插件
- 此插件不能在 App Sandbox 环境中使用
</Note>
