...
## 注意事项
### Mac 平台需使用虚拟声卡才能采集系统音频。
1. Zego提供自实现的虚拟音频插件，将插件集成到业务工程。
2. 调用 [callExperimentalAPI](@callExperimentalAPI) 接口安装插件
  - 输入 `{"method": "liveroom.audio.install_mac_virtual_audio_device", "params":{}}`
  - 监听 [onRecvExperimentalAPI](@onRecvExperimentalAPI) 回调安装结果：`{"method":"liveroom.audio.install_mac_virtual_audio_device","result":0}`，即安装成功
  - 只需要安装一次，如已经安装也会从 [onRecvExperimentalAPI](@onRecvExperimentalAPI) 返回正确的安装结果
  - 需要在创建引擎后，预览/推流/拉流前调用
3. 插件将自动安装到 `/Library/Audio/Plug-Ins/HAL` 目录并重启系统音频，音频 MIDI 设置应用中会显示Zego的虚拟音频插件
4. 自动安装插件要求应用不能使用 App Sandbox 环境
5. 手动安装将插件放置 `/Library/Audio/Plug-Ins/HAL`，终端执行重启音频指令 `sudo killall coreaudiod`
