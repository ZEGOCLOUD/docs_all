...
- 已在项目中集成 ZEGO Express SDK，并实现了基本的音视频推拉流功能，详情请参考 [快速开始 - 集成](/real-time-video-macos-oc/quick-start/integrating-sdk) 和 [快速开始 - 实现流程](/real-time-video-macos-oc/quick-start/implementing-video-call)。
...
sequenceDiagram
    ...
    推流端->>ZEGO Express SDK: 初始化引擎 <br/> createEngineWithProfile

    推流端->>ZEGO Express SDK: 登录房间 <br/> loginRoom

    拉流端->>ZEGO Express SDK: 初始化 <br/> createEngineWithProfile
    ...
    opt (可选) 使用 Alpha 通道传输分割出的主体
        推流端-->>ZEGO Express SDK: enableAlphaChannelVideoEncoder
    end

    推流端->>ZEGO Express SDK: 开始预览和推流 <br/> startPreview、<br>startPublishingStream

    opt (可选) 在拉流端设置<br>Alpha 通道渲染并拉流
        拉流端-->>ZEGO Express SDK: ZegoCanvas.alphaBlend = YES、<br>startPlayingStream
    end
...
初始化和登录房间的具体流程，请参考实现视频通话文档中的 “[创建引擎](/real-time-video-macos-oc/quick-start/implementing-video-call)”及“[登录房间](/real-time-video-macos-oc/quick-start/implementing-video-call)”。
...
调用 [enableCustomVideoRender](@enableCustomVideoRender) 接口，设置引擎进阶配置并开启自定义渲染，同时调用 [setCustomVideoRenderHandler](@setCustomVideoRenderHandler) 接口设置自定义视频渲染回调，详情请参考 [自定义视频渲染](/real-time-video-macos-oc/video/custom-video-rendering)。
