...
由于微信小程序原生推拉流组件使用起来比较复杂，推荐开发者使用 ZEGO 封装的 \<zego-push> 和 \<zego-player> 组件实现视频通话，可参考 [微信小程序 - 实现视频通话\|_blank](!Solution_Implementation_WeChat)。
</div>
...
- 已在 [ZEGO 控制台\|_blank](https://console.zego.im/) 创建项目，申请有效的 AppID 和 ServerSecret，详情请参考 [控制台 - 项目管理\|_blank](#12107) 中的“项目信息”。
...
- [服务器域名配置\|_blank](https://developers.weixin.qq.com/miniprogram/dev/framework/ability/network.html)：在“小程序后台 > 开发管理 > 开发设置 > 服务器域名”中，按照协议分类，将即构 Server 地址、LogUrl、以及用户业务需要用到的地址填到指定的“request合法域名”或“socket合法域名”中。
...
如果需要注册回调，开发者可根据实际需要，实现 ZegoEvent 中的某些方法，创建引擎后可通过调用 [on\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~class~ZegoExpressEngine#on) 接口设置回调。
...
登录房间需要用于验证身份的 Token，获取方式请参考 [用户权限控制\|_blank](#8940)。如需快速调试，建议使用控制台生成的临时 Token，生成临时 Token 的具体操作请参考 [控制台 - 项目管理\|_blank](#12107)。

**2. 登录房间**

您可以调用 SDK 的 [loginRoom \|_blank](@loginRoom) 接口，传入房间 ID 参数 “roomID”、“token” 和用户参数 “user”，登录房间。您可通过监听 [roomStateUpdate \|_blank](@roomStateUpdate) 回调实时监控自己在本房间内的连接状态，具体请参考 [4.1 常见通知回调](14945#4_1) 中的“4.1.1 我在房间内的连接状态变化通知”。
...
为避免错过任何通知，您需要在登录房间前先设置所有的监听回调（如房间状态、用户状态、流状态、推拉流状态等），具体请参考 [4.1 常见通知回调](14945#4_1)。  
</div>
...
- 通过 [wx.createLivePusherContext](https://developers.weixin.qq.com/miniprogram/dev/api/media/live/wx.createLivePusherContext.html)，可以创建 [\<live-pusher>\|_blank](https://developers.weixin.qq.com/miniprogram/dev/component/live-pusher.html) 上下文 [LivePusherContext](https://developers.weixin.qq.com/miniprogram/dev/api/media/live/LivePusherContext.html) 对象。
- 通过 [wx.createLivePlayerContext](https://developers.weixin.qq.com/miniprogram/dev/api/media/live/wx.createLivePlayerContext.html) 传入 [\<live-player>\|_blank](https://developers.weixin.qq.com/miniprogram/dev/component/live-player.html) 组件 ID，可以创建 [\<live-player>\|_blank](https://developers.weixin.qq.com/miniprogram/dev/component/live-player.html) 上下文 [LivePlayerContext](https://developers.weixin.qq.com/miniprogram/dev/api/media/live/LivePlayerContext.html) 对象。
...
#### 3.4.2 推送音视频流到 ZEGO 音视频云

调用 [startPublishingStream](@startPublishingStream) 获取推流地址，并将推流地址赋值给 [\<live-pusher>\|_blank](https://developers.weixin.qq.com/miniprogram/dev/component/live-pusher.html) 组件的 url 属性。

推送音视频流到 ZEGO 音视频云：

- `autopush` 为 true，自动推送音视频。
- `autopush` 为 false，通过 [wx.createLivePusherContext](https://developers.weixin.qq.com/miniprogram/dev/api/media/live/wx.createLivePusherContext.html)，可以创建 [\<live-pusher>\|_blank](https://developers.weixin.qq.com/miniprogram/dev/component/live-pusher.html) 上下文 [LivePusherContext](https://developers.weixin.qq.com/miniprogram/dev/api/media/live/LivePusherContext.html) 对象，调用 [LivePusherContext.start](https://developers.weixin.qq.com/miniprogram/dev/api/media/live/LivePusherContext.start.html) 推送音视频。

您可通过监听 [publisherStateUpdate \|_blank](@publisherStateUpdate) 回调知晓是否成功推送音视频，具体回调设置请参考 [4.1 常见通知回调](14945#4_1) 中的“4.1.3 房间内流状态变更的通知”。
...
通过调用[startPlayingStream](@startPlayingStream) 获取拉流地址，并将拉流地址赋值给 [\<live-player>\|_blank](https://developers.weixin.qq.com/miniprogram/dev/component/live-player.html) 组件的 url 属性。
...
您可通过监听 [playerStateUpdate \|_blank](@playerStateUpdate) 回调知晓是否成功拉取音视频，具体请参考 [4.1 常见通知回调](14945#4_1) 中的“4.1.5 用户拉取音视频流的状态通知”。
   
远端用户推送的 “streamID” 可以从 [roomStreamUpdate\|_blank](@roomStreamUpdate) 回调中获得，具体回调设置请参考 [4.1 常见通知回调](14945#4_1) 中的“4.1.3 房间内流状态变更的通知”。
...
[roomStateUpdate\|_blank](@roomStateUpdate)：本地调用 [loginRoom\|_blank](@loginRoom) 加入房间时，您可通过监听该回调实时监控自己在该房间内的连接状态。
...
只有调用 [loginRoom\|_blank](@loginRoom) 接口登录房间时传入 [ZegoRoomConfig\|_blank](@-ZegoRoomConfig) 配置，且 “userUpdate” 参数取值为 “true” 时，用户才能收到 [roomUserUpdate\|_blank](@roomUserUpdate) 回调。
</div>
...
[roomStreamUpdate\|_blank](@roomStreamUpdate)：流状态更新回调。登录房间后，当房间内有用户新推送或删除音视频流时，SDK 会通过该回调通知。
...
a. 在 [bindstatechange\|_blank](https://developers.weixin.qq.com/miniprogram/dev/component/live-pusher.html) 绑定的回调函数中，调用 SDK 的 [updatePlayerState\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~class~ZegoExpressEngine#update-player-state) 接口将推流状态事件透传给 SDK。

b. 在 SDK 的 [publisherStateUpdate\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~interface~ZegoWechatMiniEvent#publisher-state-update) 回调中处理推流的开始、失败状态。
...
微信小程序会在 \<live-pusher> 的 [bindnetstatus\|_blank](https://developers.weixin.qq.com/miniprogram/dev/component/live-pusher.html) 绑定的方法中通知出推流网络事件，开发者需要在对应的小程序回调中，调用 SDK 的 [updatePlayerNetStatus\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~class~ZegoExpressEngine#update-player-net-status) 接口将推流网络事件透传给 SDK。
...
a. 在 [bindstatechange\|_blank](https://developers.weixin.qq.com/miniprogram/dev/component/live-player.html) 绑定的回调函数中，调用 SDK 的 [updatePlayerState\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~class~ZegoExpressEngine#update-player-state) 接口将拉流状态事件透传给 SDK。
...
调用拉流组件上下文的 [LivePusherContext.stop](https://developers.weixin.qq.com/miniprogram/dev/api/media/live/LivePusherContext.stop.html) 方法停止推流。
...
调用推流组件上下文的 [LivePlayerContext.stop](https://developers.weixin.qq.com/miniprogram/dev/api/media/live/LivePlayerContext.stop.html) 方法停止拉流。
