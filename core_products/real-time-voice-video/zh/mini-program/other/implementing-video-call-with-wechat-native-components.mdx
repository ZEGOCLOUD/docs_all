...
您可通过监听 [publisherStateUpdate ](@publisherStateUpdate) 回调知晓是否成功推送音视频，具体回调设置请参考 [常见通知回调](#callback) 中的“房间内流状态变更的通知”。
...
您可通过监听 [playerStateUpdate ](@playerStateUpdate) 回调知晓是否成功拉取音视频，具体请参考 [常见通知回调](#callback) 中的“用户拉取音视频流的状态通知”。
...
sequenceDiagram
    ...
    App->>ZEGO Express SDK: 创建流（采集） <br/> createPusher

    App->>ZEGO Express SDK: 推流 <br/> getPusherInstance().start()
    ZEGO Express SDK-->>App: 推流状态更新/质量回调 <br/> publisherStateUpdate/publishQualityUpdate

    App->>ZEGO Express SDK: 拉流 <br/> getPlayerInstance().play()
    ZEGO Express SDK-->>App: 拉流状态更新/质量回调 <br/> playerStateUpdate/playQualityUpdate

    App->>ZEGO Express SDK: 停止推流 <br/> getPusherInstance().stop()
    ZEGO Express SDK-->>App: 推流状态更新回调 <br/> publisherStateUpdate

    App->>ZEGO Express SDK: 停止拉流 <br/> getPlayerInstance().stop()
