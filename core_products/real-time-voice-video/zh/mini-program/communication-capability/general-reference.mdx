...
- 已在项目中集成 ZEGO Express SDK，并实现了基本的音视频推拉流功能，详情请参考：
    - [微信小程序 - 集成 SDK](!SDK_Integration_WeChat) 和 [微信小程序 - 实现流程](!Solution_Implementation_WeChat)，**微信小程序 SDK 需要在 2.10.0 或以上版本**。
    - [支付宝小程序 - 集成 SDK](!SDK_Integration_AliPay) 和 [支付宝小程序 - 实现流程](!Solution_Implementation_AliPay)。
...
为方便开发者体验使用 Token 鉴权，[ZEGO 控制台](https://console.zego.im/) 提供生成临时 Token 的功能，开发者可直接获取临时 Token 来使用，详情请参考 [控制台 - 开发辅助](/console/development-assistance/app-development-assistance)。但是在开发者自己的线上环境中，一定要通过自己的服务端生成 Token。
...
调用 [loginRoom](@loginRoom) 接口登录房间时使用的 userID，必须和 “4.2 生成 Token” 时使用的 userID 保持一致。 
...
收到该回调后，开发者需要从自己的服务端获取新的有效 Token，并调用 SDK 提供的 [renewToken](@renewToken) 接口更新 Token。否则：
- 已登录的用户不会被踢出房间。
