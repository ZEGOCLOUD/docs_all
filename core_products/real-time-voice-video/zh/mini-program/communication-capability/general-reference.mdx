...
- 已在项目中集成 ZEGO Express SDK，并实现了基本的音视频推拉流功能，详情请参考：
    - [微信小程序 - 集成 SDK\|_blank](!SDK_Integration_WeChat) 和 [微信小程序 - 实现流程\|_blank](!Solution_Implementation_WeChat)，**微信小程序 SDK 需要在 2.10.0 或以上版本**。
...
为方便开发者体验使用 Token 鉴权，[ZEGO 控制台\|_blank](https://console.zego.im/) 提供生成临时 Token 的功能，开发者可直接获取临时 Token 来使用，详情请参考 [控制台 - 开发辅助\|_blank](#16309)。但是在开发者自己的线上环境中，一定要通过自己的服务端生成 Token。



### 4.3 使用 Token 

用户携带获取到的 Token 和 userID、roomID 信息，通过 [loginRoom\|_blank](@loginRoom) 接口登录对应的房间。

<div class="mk-warning">

调用 [loginRoom\|_blank](@loginRoom) 接口登录房间时使用的 userID，必须和 “4.2 生成 Token” 时使用的 userID 保持一致。 
</div>
...
如果开发者需要在登录房间后修改推流权限位，可以调用 [renewToken\|_blank](@renewToken) 接口更新 Token，更新后会影响下一次登推流的权限，之前已经成功的推流不受影响。
...
收到该回调后，开发者需要从自己的服务端获取新的有效 Token，并调用 SDK 提供的 [renewToken\|_blank](@renewToken) 接口更新 Token。否则：
- 已登录的用户不会被踢出房间。
...
| [tokenWillExpire\|_blank](@tokenWillExpire) | Token 过期回调 |
