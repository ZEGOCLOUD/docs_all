...
在直推 CDN 前，请确保已在项目中实现了基本的音视频推拉流功能，详情请参考 [微信小程序 - 集成 SDK](/real-time-video-miniprogram/quick-start/we-chat/integrating-sdk) 和 [微信小程序 - 实现流程](/real-time-video-miniprogram/quick-start/we-chat/implementing-video-call)。

## 使用步骤

### 直推 CDN

调用 [startPublishingStream](@startPublishingStream) 接口，设置 `publishOption` 属性的 `sourceType` 为 CDN，推流到 CDN。
