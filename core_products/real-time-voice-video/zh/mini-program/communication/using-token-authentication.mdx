...
- 已在项目中集成 ZEGO Express SDK，并实现了基本的音视频推拉流功能，详情请参考：
    - [微信小程序 - 集成 SDK](/real-time-video-miniprogram/quick-start/we-chat/integrating-sdk) 和 [微信小程序 - 实现流程](/real-time-video-miniprogram/quick-start/we-chat/implementing-video-call)，**微信小程序 SDK 需要在 2.10.0 或以上版本**。
...
用户携带获取到的 Token 和 userID、roomID 信息，通过 [loginRoom](@loginRoom) 接口登录对应的房间。

<Warning title="注意">


调用 [loginRoom](@loginRoom) 接口登录房间时使用的 userID，必须和 “生成 Token” 时使用的 userID 保持一致。
...
在 Token 过期前 30 秒，SDK 会通过 [tokenWillExpire](@tokenWillExpire) 回调发出通知；在 Token 过期后再进行登录房间操作时会收到 `1102018` （Token 过期）错误码。

收到 Token 即将过期回调或者收到 Token 过期错误码后，开发者需要从自己的服务端获取新的有效 Token，并调用 SDK 提供的 [renewToken](@renewToken) 接口更新 Token。否则：
- 已登录的用户不会被踢出房间。
