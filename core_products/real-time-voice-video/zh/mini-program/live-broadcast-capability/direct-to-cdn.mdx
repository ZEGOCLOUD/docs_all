...
在直推 CDN 前，请确保已在项目中实现了基本的音视频推拉流功能，详情请参考 [微信小程序 - 集成 SDK\|_blank](!SDK_Integration_WeChat) 和 [微信小程序 - 实现流程\|_blank](!Solution_Implementation_WeChat)。

## 使用步骤

### 直推 CDN

调用 [startPublishingStream\|_blank](@startPublishingStream) 接口，设置 `publishOption` 属性的 `sourceType` 为 CDN，推流到 CDN。
