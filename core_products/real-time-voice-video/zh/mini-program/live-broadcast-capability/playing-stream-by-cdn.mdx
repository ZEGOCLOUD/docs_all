...
在通过 URL 拉流前，请确保已在项目中实现了基本的音视频推拉流功能，详情请参考 [微信小程序 - 集成 SDK\|_blank](!SDK_Integration_WeChat) 和 [微信小程序 - 实现流程\|_blank](!Solution_Implementation_WeChat)。

## 使用步骤

### 1 开始拉流

调用 [startPlayingStream\|_blank](@startPlayingStream) 接口获取 `streamID` 对应的 URL 播放地址。
