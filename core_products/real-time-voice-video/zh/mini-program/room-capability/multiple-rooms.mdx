...
- 已在项目中集成 ZEGO Express SDK（2.9.0 版本及以上），实现基本的实时音视频功能，详情请参考 [微信小程序 - 集成 SDK\|_blank](!SDK_Integration_WeChat) 和 [微信小程序 - 实现流程\|_blank](!Solution_Implementation_WeChat)。
...
请参考 [微信小程序 - 实现流程\|_blank](!Solution_Implementation_WeChat#initialization) 的 “初始化” 进行 SDK 的初始化。

### 2 设置多房间模式

必须在初始化 SDK 之后，登录房间前调用，调用 [enableMultiRoom\|_blank](@enableMultiRoom) 设置为 true 开启多房间。
...
传入房间 ID 等参数，调用 [createPusher\|_blank](@createPusher) 接口在指定房间内推流。
...
传入的流 ID，调用 [getPlayerInstance\|_blank](@getPlayerInstance) 接口即可进行拉流。
...
调用 [sendBroadcastMessage\|_blank](@sendBroadcastMessage) 接口发送广播消息，房间其他用户通过 [IMRecvBroadcastMessage\|_blank](@IMRecvBroadcastMessage) 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
调用 [sendBarrageMessage\|_blank](@sendBarrageMessage) 接口发送弹幕消息，房间其他用户通过 [IMRecvBarrageMessage\|_blank](@IMRecvBarrageMessage) 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
调用 [sendCustomCommand\|_blank](@sendCustomCommand) 接口发送自定义信令，房间其他用户通过 [IMRecvCustomCommand\|_blank](@IMRecvCustomCommand) 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
调用 [setRoomExtraInfo\|_blank](@setRoomExtraInfo) 接口发送房间附加信息，房间其他用户通过 [roomExtraInfoUpdate\|_blank](@roomExtraInfoUpdate) 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
