...
2. 将 app.js 文件中的 zegoAppID 及 server 分别修改为 [前提条件](/real-time-video-miniprogram/quick-start/alipay/run-example-code) 中获取到的 `AppID` 和 `Server 地址`，同时在 userID 和 token 中，传入正确的值，否则示例代码无法正常运行。
