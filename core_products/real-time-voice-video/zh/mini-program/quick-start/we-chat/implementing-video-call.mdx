...
- ZEGO 提供了 `<zego-pusher>` 和 `<zego-player>`，可以帮助开发者更快地实现实时音视频。用户也可以选择使用微信原生小程序推拉流组件 `<live-pusher>` 和 `<live-player>` 进行推拉流，参考 [使用微信原生组件实现通话](/real-time-video-miniprogram/other/implementing-video-call-with-wechat-native-components)。
...
sequenceDiagram
    ...
    用户A->>ZEGO: 1.用户A创建引擎<br/>new ZegoExpressEngine
    用户A->>ZEGO: 2.用户A登录房间<br/>loginRoom
    用户B->>ZEGO: 3.用户B创建引擎<br/>new ZegoExpressEngine
    用户B->>ZEGO: 4.用户B登录房间<br/>loginRoom
    用户B->>ZEGO: 5.用户B推流<br/>createPusher 与<br/>getPusherInstance().start()
    ZEGO->>用户A: 6.用户A收到roomStreamUpdate回调<br/>(获取 用户B 的streamID)
    用户A->>ZEGO: 7.用户A传入用户B的streamID拉流<br/>getPlayerInstance().play()
```
{/*
...
### 配置微信小程序后台 <a id="configure-wechat-mini-program-backend" />
...
- [服务器域名配置](https://developers.weixin.qq.com/miniprogram/dev/framework/ability/network.html)：在“小程序后台 > 开发管理 > 开发设置 > 服务器域名”中，按照协议分类，将即构 `Server 地址`、`LogUrl`、以及用户业务需要用到的地址填到指定的“socket合法域名”或“request合法域名”中，详情请参考 [控制台 - 项目信息](/console/project-info)。
    ...
    <Warning title="注意">
    - 控制台提供的 `Server 地址`格式为：wss://xxxxxxxxxx.com/ws。在 [微信公众平台](https://mp.weixin.qq.com/?token=&lang=zh_CN) 填写时，不能直接复制原地址，需要删除原地址的 “/ws”，应填入地址的格式为：wss://xxxxxxxxxx.com。
    ...
    - 如果您使用 3.0.0 或以上版本（包括 3.6.0 及以上版本）的 SDK，还需要添加一些 socket 域名，详情请参考 [3.0.0 及以上版本升级指南](../../client-sdk/upgrade-guide/upgrade-to-v3.mdx)。
    - 如果您使用 3.6.0 或以上版本的 SDK，还需要添加一些 request 域名，详情请参考 [3.6.0 及以上版本升级指南](../../client-sdk/upgrade-guide/upgrade-to-v3-6.mdx)。
    </Warning>
...
创建 [ZegoExpressEngine ](@-ZegoExpressEngine) 引擎实例，将申请到的 `AppID` 传入参数 “appID”，将获取到的 `Server 地址`传入参数 “server”。
...
<Warning title="注意">
若使用 3.6.0 及以上版本 SDK，`server` 参数可填写控制台获取的`Server 地址`或者直接填空字符串。
</Warning>
...
[roomStreamUpdate](@roomStreamUpdate)：流状态更新回调。登录房间后，当房间内有用户新推送或删除音视频流时，SDK 会通过该回调通知。
...
您可通过监听 [roomStateUpdate ](@roomStateUpdate) 回调实时监控自己在本房间内的连接状态，具体请参考 [常见通知回调](#callback) 中的“我在房间内的连接状态变化通知”。
...
调用 [initContext ](@initContext) 接口初始化小程序组件。
...
- 您可通过监听 [publisherStateUpdate](@publisherStateUpdate) 回调知晓推流是否成功，具体请参考 [常见通知回调](#callback) 中的“用户推送音视频流的状态通知”。

- `streamID` 由您本地生成，但是需要保证：
    1.  同一个 AppID 下，`streamID` 全局唯一。如果同一个 AppID 下，不同用户各推了一条 `streamID` 相同的流，后推流的用户推流失败。
    2.  `streamID` 长度不超过 256 字节的字符串。仅支持数字、英文字符和 "-"、"_"。
...
如果用户在音视频通话的过程中遇到相关错误，可查询 [常见错误码](/real-time-video-miniprogram/client-sdk/error-code)。
...
sequenceDiagram
    ...
    note over App,ZEGO Express SDK: 1. 初始化和房间登录
    App->>ZEGO Express SDK: new ZegoExpressEngine
    App->>ZEGO Express SDK: loginRoom
    ZEGO Express SDK-->>App: roomStateUpdate/roomUserUpdate/roomStreamUpdate<br/>(房间/用户/流状态更新回调)
    note over App,ZEGO Express SDK: 2. 推流流程
    App->>ZEGO Express SDK: createPusher (创建流/采集)
    App->>ZEGO Express SDK: getPusherInstance().start() (推流)
    ZEGO Express SDK-->>App: publisherStateUpdate/publishQualityUpdate<br/>(推流状态更新/质量回调)
    note over App,ZEGO Express SDK: 3. 拉流流程
    App->>ZEGO Express SDK: getPlayerInstance().play() (拉流)
    ZEGO Express SDK-->>App: playerStateUpdate/playQualityUpdate<br/>(拉流状态更新/质量回调)
    note over App,ZEGO Express SDK: 4. 停止推流
    App->>ZEGO Express SDK: getPusherInstance().stop() (停止推流)
    ZEGO Express SDK-->>App: publisherStateUpdate<br/>(推流状态更新回调)
    note over App,ZEGO Express SDK: 5. 停止拉流
    App->>ZEGO Express SDK: getPlayerInstance().stop() (停止拉流)
    ZEGO Express SDK-->>App: playerStateUpdate<br/>(拉流状态更新回调)
    note over App,ZEGO Express SDK: 6. 退出房间并销毁引擎
    App->>ZEGO Express SDK: logoutRoom (退出房间)
    ZEGO Express SDK-->>App: roomStateUpdate<br/>(房间状态更新回调)
    App->>ZEGO Express SDK: destroyEngine (销毁引擎)
```
{/*
...
- 如果需要了解 API 返回的错误码，请参考 [常见错误码](/real-time-video-miniprogram/client-sdk/error-code)。
