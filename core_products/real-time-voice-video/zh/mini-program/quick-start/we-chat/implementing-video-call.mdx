...
您可通过监听 [roomStateUpdate ](@roomStateUpdate) 回调实时监控自己在本房间内的连接状态，具体请参考 [常见通知回调 ](/real-time-video-miniprogram/quick-start/we-chat/implementing-video-call) 中的“我在房间内的连接状态变化通知”。
...
为避免错过任何通知，您需要在登录房间前先设置所有的监听回调（如房间状态、用户状态、流状态、推拉流状态等），具体请参考 [常见通知回调 ](/real-time-video-miniprogram/quick-start/we-chat/implementing-video-call)。
...
用户调用 SDK 的 [createPusher](@createPusher) 接口创建推流实例，并通过调用实例对象上的 [start](@start) 接口，传入流 ID 参数 “streamID”。您可通过监听 [publisherStateUpdate ](@publisherStateUpdate) 回调知晓推流是否成功，具体请参考 [常见通知回调 ](/real-time-video-miniprogram/quick-start/we-chat/implementing-video-call) 中的“用户推送音视频流的状态通知”。
...
用户先调用 [getPlayerInstance](@getPlayerInstance) 接口，根据传入的流 ID 参数 “streamID”，获取 streamID 对应的拉流实例，然后通过调用拉流实例对象的 [play](@play) 接口开始拉流。您可通过监听 [playerStateUpdate ](@playerStateUpdate) 回调知晓是否成功拉取音视频，具体请参考 [常见通知回调 ](/real-time-video-miniprogram/quick-start/we-chat/implementing-video-call) 中的“用户拉取音视频流的状态通知”。

远端用户推送的 “streamID” 可以从 [roomStreamUpdate](@roomStreamUpdate) 回调中获得，具体回调设置请参考 [常见通知回调 ](/real-time-video-miniprogram/quick-start/we-chat/implementing-video-call) 中的“房间内流状态变更的通知”。
...
如果用户在音视频通话的过程中，遇到相关错误，可查询 [错误码](/real-time-video-miniprogram/client-sdk/error-code)。
...
调用 SDK 的 [getPusherInstance](@getPusherInstance) 接口获取推流实例，并调用推流实例的 [stop](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~class~ZegoWxLivePusher#stop) 方法停止推流。
...
调用 SDK 的 [getPlayerInstance](@getPlayerInstance) 接口获取拉流实例，并调用推流实例的 [stop](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~class~ZegoWxLivePlayer#stop) 方法停止拉流。
...
sequenceDiagram
    ...
    Note over App,SDK: 1. 初始化和房间登录
    App->>SDK: new ZegoExpressEngine
    App->>SDK: loginRoom
    SDK-->>App: roomStateUpdate/roomUserUpdate/roomStreamUpdate<br/>(房间/用户/流状态更新回调)

    Note over App,SDK: 2. 推流流程
    App->>SDK: createPusher (创建流/采集)
    App->>SDK: getPusherInstance().start() (推流)
    SDK-->>App: publisherStateUpdate/publishQualityUpdate<br/>(推流状态更新/质量回调)

    Note over App,SDK: 3. 拉流流程
    App->>SDK: getPlayerInstance().play() (拉流)
    SDK-->>App: playerStateUpdate/playQualityUpdate<br/>(拉流状态更新/质量回调)

    Note over App,SDK: 4. 停止推流
    App->>SDK: getPusherInstance().stop() (停止推流)
    SDK-->>App: publisherStateUpdate<br/>(推流状态更新回调)

    Note over App,SDK: 5. 停止拉流
    App->>SDK: getPlayerInstance().stop() (停止拉流)
    SDK-->>App: playerStateUpdate<br/>(拉流状态更新回调)

    Note over App,SDK: 6. 退出房间并销毁引擎
    App->>SDK: logoutRoom (退出房间)
    SDK-->>App: roomStateUpdate<br/>(房间状态更新回调)
    App->>SDK: destroyEngine (销毁引擎)
``` */}

## 相关文档

- 如果需要了解 API 返回的错误码，请参考 [错误码文档](/real-time-video-miniprogram/client-sdk/error-code)。
