...
- 已在项目中集成 ZEGO Express SDK（2.9.0 版本及以上），实现基本的实时音视频功能，详情请参考 [微信小程序 - 集成 SDK](/real-time-video-miniprogram/quick-start/we-chat/integrating-sdk) 和 [微信小程序 - 实现流程](/real-time-video-miniprogram/quick-start/we-chat/implementing-video-call)。
...
sequenceDiagram
    ...
    客户端->>ZEGO Express SDK: 设置多房间模式 <br/> enableMultiRoom
    ...
    critical 多次调用 loginRoom，传入不同的 roomID、相同的 userID，即可实现同时登录多个房间。
        ...
        ZEGO Express SDK-->>客户端: 接收房间内的相关回调 <br/> on
...
请参考 [微信小程序 - 实现流程](/real-time-video-miniprogram/quick-start/we-chat/implementing-video-call) 的 “初始化” 进行 SDK 的初始化。

### 设置多房间模式

必须在初始化 SDK 之后，登录房间前调用，调用 [enableMultiRoom](@enableMultiRoom) 设置为 true 开启多房间。
...
通过 [loginRoom](@loginRoom) 接口成功登录第一个房间后，再次调用 [loginRoom](@loginRoom) 接口登录第二个房间。
...
传入房间 ID 等参数，调用 [createPusher](@createPusher) 接口在指定房间内推流。
...
传入的流 ID，调用 [getPlayerInstance](@getPlayerInstance) 接口即可进行拉流。
...
调用 [sendBroadcastMessage](@sendBroadcastMessage) 接口发送广播消息，房间其他用户通过 [IMRecvBroadcastMessage](@IMRecvBroadcastMessage) 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
调用 [sendBarrageMessage](@sendBarrageMessage) 接口发送弹幕消息，房间其他用户通过 [IMRecvBarrageMessage](@IMRecvBarrageMessage) 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
调用 [sendCustomCommand](@sendCustomCommand) 接口发送自定义信令，房间其他用户通过 [IMRecvCustomCommand](@IMRecvCustomCommand) 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
调用 [setRoomExtraInfo](@setRoomExtraInfo) 接口发送房间附加信息，房间其他用户通过 [roomExtraInfoUpdate](@roomExtraInfoUpdate) 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
<Note title="说明">
发送实时消息的大小和频率限制可参考 [限制说明](/real-time-video-miniprogram/best-practice/restrictions)。
</Note>
...
传入房间 ID ，调用 [logoutRoom](@logoutRoom) 接口退出房间。
