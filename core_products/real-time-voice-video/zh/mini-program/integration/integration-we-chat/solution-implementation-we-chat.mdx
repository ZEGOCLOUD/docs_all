...
<Warning title="注意">
    ...
    - 如果您使用 3.0.0 或以上版本（包括 3.6.0 及以上版本）的 SDK，还需要添加一些 socket 域名，详情请参考 [3.0.0 及以上版本升级指南](/real-time-video-miniprogram/over-view/upgrade-guide/upgrade-guide4)。
    - 如果您使用 3.6.0 或以上版本的 SDK，还需要添加一些 request 域名，详情请参考 [3.6.0 及以上版本升级指南](/real-time-video-miniprogram/over-view/upgrade-guide/upgrade-guide6)。
...
登录房间需要用于验证身份的 Token，获取方式请参考 [用户权限控制](/real-time-video-miniprogram/communication-capability/general-reference)。如需快速调试，建议使用控制台生成的临时 Token，生成临时 Token 的具体操作请参考 [控制台 - 项目管理](/console/guide-doc2)。
...
- 如果需要修改视频清晰度，请参考 [设置视频属性](/real-time-video-miniprogram/video-capability/video-attributes) 文档。
