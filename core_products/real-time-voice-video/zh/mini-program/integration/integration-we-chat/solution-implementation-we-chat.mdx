...
如下实现流程支持的是 SDK 需为 2.10.0 及以上版本，如使用 2.10.0 以下版本的 SDK 请参考旧版方案 [实现视频通话](#14192)。
</div>
...
- [服务器域名配置](https://developers.weixin.qq.com/miniprogram/dev/framework/ability/network.html)：在“小程序后台 > 开发管理 > 开发设置 > 服务器域名”中，按照协议分类，将即构 Server 地址、LogUrl、以及用户业务需要用到的地址填到指定的“socket合法域名”或“request合法域名”中，详情请参考 [控制台 - 项目信息](https://doc-zh.zego.im/article/12107)。
    ...
    - 如果您使用 3.0.0 或以上版本（包括 3.6.0 及以上版本）的 SDK，还需要添加一些 socket 域名，详情请参考 [3.0.0 及以上版本升级指南](#18284)。
    - 如果您使用 3.6.0 或以上版本的 SDK，还需要添加一些 request 域名，详情请参考 [3.6.0 及以上版本升级指南](#21275)。
    </div>
...
创建引擎后，开发者可根据实际需要，通过引擎实例的 [on](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~class~ZegoExpressEngine#on) 方法设置回调。
...
您可通过监听 [roomStateUpdate ](@roomStateUpdate) 回调实时监控自己在本房间内的连接状态，具体请参考 [常见通知回调 ](!Solution_Implementation_WeChat#3_2) 中的“我在房间内的连接状态变化通知”。
...
为避免错过任何通知，您需要在登录房间前先设置所有的监听回调（如房间状态、用户状态、流状态、推拉流状态等），具体请参考 [常见通知回调 ](!Solution_Implementation_WeChat#3_2)。  
</div>
...
- pusher 字段中的属性值请参考 [ZegoWxPusherAttributes](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~interface~ZegoWxPusherAttributes)。
- playerlist 字段中的属性值请参考 [ZegoWxPlayerAttributes](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~interface~ZegoWxPlayerAttributes)。
...
用户调用 SDK 的 [createPusher](@createPusher) 接口创建推流实例，并通过调用实例对象上的 [start](@start) 接口，传入流 ID 参数 “streamID”。您可通过监听 [publisherStateUpdate ](@publisherStateUpdate) 回调知晓推流是否成功，具体请参考 [常见通知回调 ](!Solution_Implementation_WeChat#3_2) 中的“用户推送音视频流的状态通知”。
...
用户先调用 [getPlayerInstance](@getPlayerInstance) 接口，根据传入的流 ID 参数 “streamID”，获取 streamID 对应的拉流实例，然后通过调用拉流实例对象的 [play](@play) 接口开始拉流。您可通过监听 [playerStateUpdate ](@playerStateUpdate) 回调知晓是否成功拉取音视频，具体请参考 [常见通知回调 ](!Solution_Implementation_WeChat#3_2) 中的“用户拉取音视频流的状态通知”。
   
远端用户推送的 “streamID” 可以从 [roomStreamUpdate](@roomStreamUpdate) 回调中获得，具体回调设置请参考 [常见通知回调 ](!Solution_Implementation_WeChat#3_2) 中的“房间内流状态变更的通知”。
...
调用 SDK 的 [getPusherInstance](@getPusherInstance) 接口获取推流实例，并调用推流实例的 [stop](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~class~ZegoWxLivePusher#stop) 方法停止推流。
...
调用 SDK 的 [getPlayerInstance](@getPlayerInstance) 接口获取拉流实例，并调用推流实例的 [stop](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~class~ZegoWxLivePlayer#stop) 方法停止拉流。

<div class="mk-warning">

如果开发者通过 [roomStreamUpdate](@roomStreamUpdate) 回调收到了音视频流 “减少” 的通知，请及时调用 [stop ](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~class~ZegoWxLivePlayer#stop) 接口停止拉流，避免拉到空流、产生额外的成本；或者，开发者可以根据自己的业务需求，选择合适的时机，主动调用 [stop](/article/api?doc=Express_Video_SDK_API~javascript_wxxcx~class~ZegoWxLivePlayer#stop) 接口停止拉流。
</div>
...
- 如果需要修改视频清晰度，请参考 [设置视频属性](#19447) 文档。
