...
- 需使用 [L3 拉流](/real-time-video-flutter/live-streaming/low-latency-live-streaming)、[转推 CDN 拉流（通过 streamID 拉流）](/real-time-video-flutter/live-streaming/using-cdn-for-live-streaming) 或 RTC 拉流，且未使用混流能力。
...
1. 添加转码模板。
2. 创建引擎并登录房间。
3. 按需拉取转码流，根据需要选择拉流方式为 [L3 拉流](/real-time-video-flutter/live-streaming/low-latency-live-streaming)、[转推 CDN 拉流（通过 streamID 拉流）](/real-time-video-flutter/live-streaming/using-cdn-for-live-streaming) 或 RTC，并传入转码模板 ID 触发转码。
...
具体流程请参考“实现视频通话”文档的 [创建引擎](/real-time-video-flutter/quick-start/implementing-video-call) 和 [登录房间](/real-time-video-flutter/quick-start/implementing-video-call) 章节。
...
配置 [ZegoPlayerConfig](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoPlayerConfig-class.html) 的 codecTemplateID 属性，填写 [步骤 1 添加转码配置模板](/real-time-video-flutter/live-streaming/single-stream-transcoding) 中获取的转码模板 ID，并开始拉流。
