...
```dart
ZegoExpressEngine.instance.setVideoSource(ZegoVideoSourceType.ScreenCapture, instanceID: source.getIndex(), channel: ZegoPublishChannel.Main);
```
...
调用 [enableAudioCaptureWithConfig](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoScreenCaptureSource/enableAudioCaptureWithConfig.html) 接口开启系统声卡采集。

```dart
source.enableAudioCaptureWithConfig(true);
```
...
在屏幕共享时，您可以调用 [enableHightLight]([enableHightLight](https://doc-zh.zego.im/unique-api/express-video-sdkhttps://doc-zh.zego.im/article/dart_flutter/zego_express_engine/ZegoScreenCaptureSource/enableHightLight.html)) 接口对屏幕区域或者窗口启用描边，设置描边的颜色和宽度。
