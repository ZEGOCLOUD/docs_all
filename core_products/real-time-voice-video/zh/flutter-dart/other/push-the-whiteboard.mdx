...
具体流程请参考“实现视频通话”文档的 [初始化](/real-time-video-flutter/quick-start/implementing-video-call) 和 [登录房间](/real-time-video-flutter/quick-start/implementing-video-call) 章节完成初始化并登录房间。
...
具体流程请参考混流文档的 [创建混流任务对象](/real-time-video-flutter/live-streaming/stream-mixing) 进行创建。

**设置混流输入流**

具体流程请参考混流文档的 [设置混流输入流](/real-time-video-flutter/live-streaming/stream-mixing) 进行设置。

**设置混流输出**

具体流程请参考混流文档的 [设置混流输出](/real-time-video-flutter/live-streaming/stream-mixing) 进行设置。
...
调用 [ZegoMixerTask > whiteboard](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoMixerTask/whiteboard.html) 接口，将 ZEGO 超级白板内容合入混流：需填入 [whiteboardID](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoMixerWhiteboard/whiteboardID.html) 白板 ID 等参数，即本文 [前提条件](/real-time-video-flutter/other/push-the-whiteboard) 中，集成 ZegoSuperBoard SDK 并创建白板时返回的 ID 等信息。
...
具体流程请参考混流文档的 [停止混流](/real-time-video-flutter/live-streaming/stream-mixing)。
