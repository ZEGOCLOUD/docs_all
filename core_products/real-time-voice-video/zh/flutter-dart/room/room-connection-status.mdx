...
如果因为网络质量不佳导致房间连接中断，SDK 内部会自动进行重试，，详情请参考 [房间断线重连](/real-time-video-flutter/room/room-connection-status) 章节。
...
如果用户 A 短暂断线，但是在 100s 内重连成功了，那么用户 B 不会收到 [onRoomUserUpdate](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngine/onRoomUserUpdate.html) 的回调通知。
...
- T5 = T3 + time（小于 100s）：用户 A 在重连时间内恢复网络，重连成功。会收到 [onRoomStateChanged](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngine/onRoomStateChanged.html) 回调以通知客户端用户 A 重连成功。
...
- T0、T1、T2、T3 时刻的情况同 [情况一：在 ZEGO 服务端判断用户 A 心跳超时前重连成功](/real-time-video-flutter/room/room-connection-status) 中的 T0、T1、T2、T3。

- T4' ≈ T3 + 100s：用户 B 此时收到 [onRoomUserUpdate](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngine/onRoomUserUpdate.html) 回调以通知用户 A 已断线。

- T5' = T3 + time（大于 100s，小于 20 min）：用户 A 在重连时间内恢复网络，重连成功。会收到 [onRoomStateChanged](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngine/onRoomStateChanged.html) 回调以通知客户端用户 A 重连成功。
