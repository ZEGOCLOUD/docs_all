...
在 Express SDK 中，通过 [ZegoMixerTask](/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoMixerTask-class.html) 定义混流任务配置对象，其中包含输入流布局、输出流和 ZEGO 超级白板配置等信息。
...
调用 [ZegoMixerTask > whiteboard](/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoMixerTask/whiteboard.html) 接口，将 ZEGO 超级白板内容合入混流：需填入 [whiteboardID](/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoMixerWhiteboard/whiteboardID.html) 白板 ID 等参数，即本文 [前提条件](!Push_the_whiteboard#2) 中，集成 ZegoSuperBoard SDK 并创建白板时返回的 ID 等信息。
...
Express SDK 支持更新混流任务中的白板配置，例如，需要切换要混入的 ZEGO 超级白板实例或不再混入 ZEGO 超级白板时，可通过调用 [startMixerTask](/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngineMixer/startMixerTask.html) 接口更新配置。如需更新混流任务的其它配置，请参考混流文档中的 [5.4 更新混流任务的配置](!Other_Functions/Mixer#5_4)。
