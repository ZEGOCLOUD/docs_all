...
1. 打开 “pubspec.yaml” 文件，添加 “zego_express_engine” 依赖，有以下两种形式：
    ...
    以上代码的版本号仅为示例，并非最新版本。最新版本号请参考 [下载\|_blank](!DownloadSDK/DownloadSDK)。
    </div>
...
如果在构建发行版 APK（flutter build apk）期间启用了混淆（默认启用），则需要配置与 ZEGO 相关的类，防止混淆，请参考 [4 常见问题](!Integration/SDK_Integration#4)。
</div>   
...
3. 权限添加完成后，如图所示：
    ...
    如果使用 Platform View，并且 Flutter 版本低于 1.22，则需要为 iOS 添加其他说明，请参考 [4 常见问题](!Integration/SDK_Integration#4)。
    </div>  
