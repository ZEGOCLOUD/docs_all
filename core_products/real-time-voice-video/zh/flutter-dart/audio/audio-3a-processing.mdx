...
- ANS（降噪）：识别声音中的背景噪声并进行消除，开启该功能后可以使人声更加清晰。如果用户需要更加激进的降噪策略，可使用 [场景化 AI 降噪](/real-time-video-flutter/audio/scenario-based-ai-noise-reduction) 功能。
...
<Warning title="注意">
    ...
    Express SDK 支持 AI 模式噪声抑制，并提供轻量模式、均衡模式及低延迟模式三种模式，在消除稳态噪声的基础上，有效消除瞬态噪声，如键盘、咳嗽、风声、汽车喇叭等瞬态噪声，详情请参考 [场景化 AI 降噪](/real-time-video-flutter/audio/scenario-based-ai-noise-reduction)。
