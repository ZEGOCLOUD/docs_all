...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成](/real-time-video-windows-cs/quick-start/integrating-sdk)。
...
如果用户在音视频通话的过程中，遇到相关错误，可查询 [错误码](/real-time-video-windows-cs/client-sdk/error-code)。
...
sequenceDiagram
    ...
    App->>ZEGO: 创建引擎 <br/> CreateEngine
    App->>ZEGO: 登录房间 <br/> LoginRoom
    ZEGO-->>App: 房间/用户/流状态更新回调 <br/> OnRoomStateUpdate/OnRoomUserUpdate/OnRoomStreamUpdate
    App->>ZEGO: 预览/推流 <br/> StartPreview/StartPublishingStream
    ZEGO-->>App: 推流状态更新回调 <br/> OnPublisherStateUpdate
    App->>ZEGO: 拉流 <br/> StartPlayingStream
    ZEGO-->>App: 拉流状态更新回调 <br/> OnPlayerStateUpdate
    App->>ZEGO: 停止预览/推流 <br/> StopPreview/StopPublishingStream
    ZEGO-->>App: 推流状态更新回调 <br/> OnPublisherStateUpdate
    App->>ZEGO: 停止拉流 <br/> StopPlayingStream
    ZEGO-->>App: 拉流状态更新回调 <br/> OnPlayerStateUpdate
    App->>ZEGO: 退出房间 <br/> LogoutRoom
    ZEGO-->>App: 房间状态更新回调 <br/> OnRoomStateUpdate
    App->>ZEGO: 销毁引擎 <br/> DestroyEngine
```
{/*
