...
初始化和登录房间的具体流程，请参考实现视频通话文档中的 “[3.1 创建引擎\|_blank](!Integration/Solution_Implementation#CreateEngine)” 及 “[3.2 登录房间\|_blank](!Integration/Solution_Implementation#createroom)”。
...
2. 调用 [IZegoAIVoiceChanger.setEventHandler\|_blank](@setEventHandler-IZegoAIVoiceChanger) 接口，设置 AI 变声引擎事件回调。
...
3. 调用 [IZegoAIVoiceChanger.initEngine\|_blank](@initEngine-IZegoAIVoiceChanger) 接口，初始化 AI 变声引擎实例。

    <div class = "mk-warning">

    [IZegoAIVoiceChanger.initEngine\|_blank](@initEngine-IZegoAIVoiceChanger) 接口需要在 [startPublishingStream\|_blank](@startPublishingStream) 接口之前调用才有效。
    </div>
...
调用 [IZegoAIVoiceChanger.update\|_blank](@update-IZegoAIVoiceChanger) 接口，更新 AI 变声引擎模型。**AI 变声引擎模型文件较大，首次更新时耗时会比较长，请您耐心等待。**
...
调用 [IZegoAIVoiceChanger.getSpeakerList\|_blank](@getSpeakerList-IZegoAIVoiceChanger) 接口，获取可用音色列表。
...
调用 [IZegoAIVoiceChanger.setSpeaker\|_blank](@setSpeaker-IZegoAIVoiceChanger) 接口，设置音色，音色的选择可以通过 [5 获取音色列表](!AI_VoiceChanger#3_5) 获取。
