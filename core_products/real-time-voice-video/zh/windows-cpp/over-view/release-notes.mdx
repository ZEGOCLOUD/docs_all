...
在桌面端使用屏幕共享时，可以为共享窗口添加一个高亮描边，并设置描边的颜色和宽度，提升共享窗口的识别度。详情请参考 [屏幕共享 - 描边](/real-time-video-windows-cpp/video-capability/share-screen#5_1)。
<Warning title="注意">
...
当屏幕共享的采集源（如窗口或屏幕区域）出现异常时，新增的 [onCaptureTypeExceptionOccurred](@onCaptureTypeExceptionOccurred) 回调将返回具体的异常采集源类型，帮助开发者快速定位问题场景。详情请参考 [屏幕共享 - 设置事件回调](!ExpressVideoSDK-Other_Functions/Share_Screen#callback)。

相关 API 请参考 [onCaptureTypeExceptionOccurred](@onCaptureTypeExceptionOccurred)
...
相关 API 请参考 [onMediaPlayerStateUpdate](@onMediaPlayerStateUpdate)
...
开发者可通过配置 [ZegoMediaPlayerResource-onlineResourceCachePath](@onlineResourceCachePath-ZegoMediaPlayerResource) 参数，设置缓存资源文件路径。此外可通过 [ZegoMediaPlayerResource-maxCachePendingLength](@maxCachePendingLength-ZegoMediaPlayerResource) 参数设置从当前播放时间点，最多可预缓存的数据长度（单位 bytes）。从而使媒体播放器播放在线资源时，可以减少用户资源下载频次，提高资源复用率与用户体验。

<Warning title="注意">
生成缓存文件的同时，SDK 同时会生成一个 ".info "后缀的文件，用于记录缓存信息（如缓存的起始位置等）。用户使用该功能时，需要自行负责清理"缓存文件"及"缓存信息文件"。
</Warning>

相关 API 请参考 [ZegoMediaPlayerResource > onlineResourceCachePath](@onlineResourceCachePath-IZegoMediaPlayerResource), [ZegoMediaPlayerResource > maxCachePendingLength](@maxCachePendingLength-IZegoMediaPlayerResource)
...
在 macOS 与 Windows 平台的 [createEngine](@createEngine) 接口中，新增 [callbackSwitchToMainThread] 参数，当其设置为 false 时，回调接口支持非主线程回调，适用于单元测试等无交互界面的测试场景。

相关 API 请参考 [createEngine](@createEngine)
...
混流功能支持将混流后的输出流，加入到指定房间，即支持设置输出流的目标房间信息 [targetRoom](@targetRoom)。每条输出流只支持加入一个房间，且一旦添加，混流过程中不支持动态更新房间。如需使用服务端接口实现该功能，可参考 [开始混流](/real-time-video-android-java/integration/sdk-integration95) 文档。

相关 API 请参考 [targetRoom](@targetRoom)
...
相关 API 请参考 [ZegoMediaPlayer > enableVoiceChanger](@enableVoiceChanger-IZegoMediaPlayer)
...
#### 支持提前判断设备能否运行 AI 变声功能

AI 变声功能对运行设备的性能有一定要求，开发者可以通过 [isAIVoiceChangerSupported] 接口提前判断设备能否支持运行 AI 变声功能。

相关 API 请参考 [isAIVoiceChangerSupported](@isAIVoiceChangerSupported)

<p class="hthree">问题修复</p>

#### 修复未定义全局变量的释放时机，导致进程退出异常的问题

#### 修复媒体播放器打开 M3U8 格式的文件耗时较长的问题

#### 修复使用 AI 变声功能，偶现音画不同步、启停瞬间吞字的问题
...
相关 API 请参考 [enableAudioCapture](@enableAudioCapture), [startcapture](@startcapture), [setAudioSource](@setAudioSource)
...
相关 API 请参考 [sendCustomVideoProcessedRawData](@sendCustomVideoProcessedRawData)
...
相关 API 请参考 [onMediaDataPublisherFileClose](@onMediaDataPublisherFileClose)
...
#### 修复 Windows 摄像头使用 MJPEG 格式采集时，偶现失败的问题

#### 修复硬编、硬解时兼容异常，偶现崩溃的问题
...
相关 API 请参考 [enableLocalCache](@enableLocalCache), [onMediaPlayerLocalCache](@onMediaPlayerLocalCache)
...
相关 API 请参考 [createAIVoiceChanger](@createAIVoiceChanger), [destroyAIVoiceChanger](@destroyAIVoiceChanger)
...
#### Windows 支持指定显卡硬编，支持硬编硬解的纹理输入输出
...
在 3.8.0 版本，对媒体推流器 [IZegoMediaDataPublisher] 类的一个 API 接口命名进行变更：将原来的成员函数 [setMediaDataPublisherEventHandler] 更名为 [setEventHandler]，接口名称修改后可能存在兼容性问题，详情请参考 [3.8.0 及以上版本升级指南](!upgrade_guide5)。
...
相关 API 请参考 [ZegoExpressEngine > setVideoConfig](@setVideoConfig-IZegoExpressEngine), [setPublishDualStreamConfig](@setPublishDualStreamConfig)
...
相关 API 请参考 [ZegoMediaPlayer > updatePosition](@updatePosition-IZegoMediaPlayer), [ZegoAudioEffectPlayer > updatePosition](@updatePosition-IZegoAudioEffectPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > setPlaySpeed](@setPlaySpeed-IZegoMediaPlayer)
...
相关 API 请参考 [onMediaPlayerFirstFrameEvent](@onMediaPlayerFirstFrameEvent)
...
相关 API 请参考 [createMediaDataPublisher](@createMediaDataPublisher)
...
#### 新增 [setSpeakerSimpleMute] 和 [getSpeakerSimpleMute] 接口，用于设置扬声器静音及获取扬声器是否静音
...
虚拟场景中，由于每个场景的地图大小、音视频互动玩法及规模不同，需要针对每个场景进行自定义配置。3.1.0 版本后，“万人范围音视频”及“多人实时状态同步”支持通过 SDK 接口，并使用模板 ID 指定场景。模板 ID 对应的配置项只能通过服务器 API 进行配置，详情请参考 [服务端 API - 场景模版配置](/real-time-video-ios-oc/integration/sdk-integration59)。
...
相关 API 请参考 [ZegoSceneParam > token](@token-ZegoSceneParam), [ZegoRangeScene > renewToken](@renewToken-IZegoRangeScene)
...
当前仅支持通过内部采集的方式使用该能力，详情请参考 [主体分割](!background_segment)。
...
#### 2. 删除了 [setDebugVerbose]、[setPlayStreamVideoLayer]、[enableAudioDataCallback] 等接口，详情请参考 [v3.0.0升级指南](!upgrade_guide4)。
...
#### 修复 Windows 平台枚举视频硬件编码能力时的小概率崩溃问题
...
若在内网或防火墙场景下，您可以通过代理服务器与公网交互，并通过 [setEngineConfig] 设置代理服务器地址，保证 ZEGO 音视频云服务正常，目前仅支持 SOCKS5 协议，详情请参考 <a href="/real-time-video-ios-oc/communication-capability/local-proxy" target="_blank" rel="noopener noreferrer">本地代理</a>。
...
#### H.264 及 H.265 硬件编解码支持使用 AMD/NVIDIA/Intel 系列显卡

注意：若要开启该功能，需要联系 ZEGO 技术支持。

支持 AMD/NVIDIA 独立显卡和 Intel 核芯显卡，您可以通过 [setEngineConfig] 修改默认显卡优先级。

相关 API 请参考 [setEngineConfig](@setEngineConfig), [enableHardwareEncoder](@enableHardwareEncoder), [enableHardwareDecoder](@enableHardwareDecoder)
...
相关 API 请参考 [ZegoRangeAudio > setPositionUpdateFrequency](@setPositionUpdateFrequency-IZegoRangeAudio)
...
拉流接口新增 CDN_PLUS 的拉流资源模式（ZegoStreamResourceMode），开发者可按流维度自行开启使用CDN_PLUS 拉流。CDN Plus 拉流是比 CDN 拉流直播质量更高，但是价格接近 CDN 的一种性价比高的拉流方式，详情请参考 <a href="/live-streaming-android/over-view/over-view" target="_blank" rel="noopener noreferrer">CDN Plus 拉流</a>。
...
新增 1002074、1002075、1002076、1002077、1002078、1002079、1002080 等错误码。开启强制登录鉴权后，如果 Token 错误时，会返回这些错误码，详情请参考 <a href="/real-time-video-ios-oc/error-code#3" target="_blank" rel="noopener noreferrer">常见错误码</a> 中的详细解释和处理建议。

#### Windows 支持 N 卡的 H.265 硬件编解码
...
相关 API 请参考 [ZegoMediaPlayer > clearView](@clearView-IZegoMediaPlayer)
...
AI 降噪指的是 SDK 会对麦克风采集的声音进行降噪处理，在原有的稳态噪声正常处理的情况下，还会处理非稳态噪声，主要包括鼠标、键盘声、敲击、空调、厨房碗碟、嘈杂餐厅、环境风声、咳嗽、吹气 等非人声噪声。通过 [setANSMode] 接口中的 [ZegoANSMode] 参数设置 AI 降噪模式，并且可以实时调整降噪模式，详情请参考 <a href="/real-time-video-android-java/audio-capability/a-id-enoise" target="_blank" rel="noopener noreferrer">AI 降噪</a>。
...
相关 API 请参考 [ZegoAudioEffectPlayer > setPlaySpeed](@setPlaySpeed-IZegoAudioEffectPlayer)
...
新增 1005000 错误码，表示未开通混流服务。出现此错误码时，请在 <a href="https://console.zego.im/" target="_blank" rel="noopener noreferrer">ZEGO 控制台</a> 自助开通混流服务（开通步骤请参考 <a href="/console/service-configuration-2/enable-mixed-streaming-service" target="_blank" rel="noopener noreferrer">项目管理 - 服务配置</a> 中的“混流”），或联系 ZEGO 技术支持开通。
...
2.17.0 及以上版本，在创建引擎时将 AppSign 传空或不传，并且在登录房间时必须传入 Token，鉴权通过后即可使用实时音视频功能，具体请参考 <a href="/real-time-video-android-java/communication-capability/general-reference" target="_blank" rel="noopener noreferrer">使用 Token 鉴权</a>。
...
相关 API 请参考 [ZegoMediaPlayer > loadResourceWithPosition](@loadResourceWithPosition-IZegoMediaPlayer)
...
相关 API 请参考 [批量禁止 RTC 推流](/real-time-video-ios-oc/integration/sdk-integration18) / [批量恢复 RTC 推流](/real-time-video-ios-oc/integration/sdk-integration20)
...
为了降低开发者对环境的理解成本，ZEGO 已统一环境概念，从该版本开始，废弃了测试环境，统一使用正式环境。在 2.14.0 版本之前已接入过 SDK 的开发者，可以参考 [测试环境废弃说明](/real-time-voice-android/summary/abandon-test-environment) 进行 SDK 升级及代码调整。
...
相关 API 请参考 [ZegoMediaPlayer > loadResource](@loadResource-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > loadResourceFromMediaData](@loadResourceFromMediaData-IZegoMediaPlayer)

#### 媒体播放器新增支持重复播放次数

开发者在使用媒体播放器时，可能需要统一文件预置播放多次，SDK 提供此功能进行循环播放，可以调用 [setPlayLoopCount] 接口设置重复播放次数。

相关 API 请参考 [setPlayLoopCount](@setPlayLoopCount)
...
#### C++ 增加自定义视频前处理类型 CVPixelBuffer，便于开发者在 macOS 平台上使用
...
相关 API 请参考 [ZegoMediaPlayer > setNetWorkResourceMaxCache](@setNetWorkResourceMaxCache-IZegoMediaPlayer), [ZegoMediaPlayer > getNetWorkResourceCache](@getNetWorkResourceCache-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > setNetWorkBufferThreshold](@setNetWorkBufferThreshold-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > takeSnapshot](@takeSnapshot-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > enableAccurateSeek](@enableAccurateSeek-IZegoMediaPlayer)
...
#### 修复了特定情况下拉流失败并且出现 1004099 错误码的问题
...
低延迟直播专注于提供稳定可靠的直播服务，相比于标准视频直播产品，音画延迟更低，同步性更强，弱网抗性更好，能为用户带来毫秒级的直播体验。通常用于教育大班课、秀场直播、电商直播、一起看、在线拍卖等场景。详情请参考：[低延迟直播](/real-time-video-android-java/live-broadcast-capability/low-latency-live)。
...
旧接口跟随了 [setEngineConfig] 接口的生命周期，不够灵活。在允许了引擎启动前设置 [自定义视频采集] 之后，Express SDK 新增了独立的 [enableCustomVideoCapture] 接口用于设置自定义视频采集，详情请参考 [接口变更说明](/real-time-voice-android/summary/api-changes-notes)。

相关 API 请参考 [enableCustomVideoCapture](@enableCustomVideoCapture)

#### 删除 [ZegoEngineConfig] 中的 [customVideoRenderConfig]，请使用 [ZegoExpressEngine] 的 [enableCustomVideoRender] 接口代替

旧接口跟随了 [setEngineConfig] 接口的生命周期，不够灵活。在允许了引擎启动前设置 [自定义视频渲染] 之后，Express SDK 新增了独立的 [enableCustomVideoRender] 接口用于设置自定义视频渲染，详情请参考 [接口变更说明](/real-time-voice-android/summary/api-changes-notes)。

相关 API 请参考 [enableCustomVideoRender](@enableCustomVideoRender)

#### 删除 [destroyEngine()] 接口，请使用 [destroyEngine(callback)] 代替

新增了一个带 [callback] 参数的销毁引擎接口，若开发者有切换多个音视频 SDK 的需求，则可以通过收到回调时认为 ZEGO SDK 对设备硬件资源的占用已释放完成。若无监听必要，对 [callback] 参数传 null 即可，详情请参考 [接口变更说明](/real-time-voice-android/summary/api-changes-notes)。

相关 API 请参考 [destroyEngine](@destroyEngine)

#### 删除 [onRemoteAudioData] 回调，请使用 [onPlaybackAudioData] 代替

旧回调命名与实际功能不符，因此删除改名，详情请参考 [接口变更说明](/real-time-voice-android/summary/api-changes-notes)。

相关 API 请参考 [onPlaybackAudioData](@onPlaybackAudioData)

#### 删除 [muteAudioOutput] 接口，请使用 [muteSpeaker] 代替

旧接口命名风格和语义不清晰，[muteSpeaker] 使用 Speaker 的定义与 Microphone 对应，详情请参考 [接口变更说明](/real-time-voice-android/summary/api-changes-notes)。
...
媒体播放器的生命周期跟随引擎，故变更为 ZegoExpressEngine 类的同名实例方法，详情请参考 [接口变更说明](/real-time-voice-android/summary/api-changes-notes)。
...
引擎提供了分别获取播放器推流音量和本地播放音量的功能，更加精准的获取音量，故废弃原来的统一获取接口，详情请参考 [接口变更说明](/real-time-voice-android/summary/api-changes-notes)。

相关 API 请参考 [ZegoMediaPlayer > setPublishVolume](@setPublishVolume-IZegoMediaPlayer), [ZegoMediaPlayer > setPlayVolume](@setPlayVolume-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoAudioEffectPlayer > seekTo](@seekTo-IZegoAudioEffectPlayer)
...
为了支持更多混响参数，设置更加丰富的混响效果，在 1.18.0 及以上版本废弃 [setReverbParam] 接口，请使用带 `ZegoReverbAdvancedParam` 类型参数的 [setReverbAdvancedParam] 接口代替，详情请参考 [接口变更说明](/real-time-voice-android/summary/api-changes-notes)。
...
#### 支持 [setPlayVolume] 方法传入的 streamID 参数为空字符串时，对所有拉流音量进行设置

相关 API 请参考 [setPlayVolume](@setPlayVolume)
...
相关 API 请参考 [ZegoMediaPlayer > setVoiceChangerParam](@setVoiceChangerParam-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > setAudioTrackIndex](@setAudioTrackIndex-IZegoMediaPlayer)
...
#### 修复 C++ 接口 在 Windows 在 32 位机器上 long 类型转换丢失精度的问题。
