...
在桌面端使用屏幕共享时，可以为共享窗口添加一个高亮描边，并设置描边的颜色和宽度，提升共享窗口的识别度。详情请参考 [屏幕共享 - 描边\|_blank](https://doc-zh.zego.im/article/6518#5_1)。<div class="mk-warning">Windows 平台部分系统窗口（例如任务管理器、以管理员权限启动的程序的窗口等）可能因权限限制无法显示描边，建议以管理员权限启动应用。</div>

相关 API 请参考 [enableHightLight\|_blank](@enableHightLight)

#### 2. 新增屏幕共享采集源异常类型回调

当屏幕共享的采集源（如窗口或屏幕区域）出现异常时，新增的 [onCaptureTypeExceptionOccurred\|_blank](@onCaptureTypeExceptionOccurred) 回调将返回具体的异常采集源类型，帮助开发者快速定位问题场景。详情请参考 [屏幕共享 - 设置事件回调\|_blank](!ExpressVideoSDK-Other_Functions/Share_Screen#callback)。

相关 API 请参考 [onCaptureTypeExceptionOccurred\|_blank](@onCaptureTypeExceptionOccurred)
...
相关 API 请参考 [ZegoMediaPlayerResource > onlineResourceCachePath\|_blank](@onlineResourceCachePath-IZegoMediaPlayerResource), [ZegoMediaPlayerResource > maxCachePendingLength\|_blank](@maxCachePendingLength-IZegoMediaPlayerResource)
...
在 macOS 与 Windows 平台的 [createEngine\|_blank](@createEngine) 接口中，新增 [callbackSwitchToMainThread] 参数，当其设置为 false 时，回调接口支持非主线程回调，适用于单元测试等无交互界面的测试场景。

相关 API 请参考 [createEngine\|_blank](@createEngine)
...
混流功能支持将混流后的输出流，加入到指定房间，即支持设置输出流的目标房间信息 [targetRoom\|_blank](@targetRoom)。每条输出流只支持加入一个房间，且一旦添加，混流过程中不支持动态更新房间。如需使用服务端接口实现该功能，可参考 [开始混流\|_blank](#19595) 文档。

相关 API 请参考 [targetRoom\|_blank](@targetRoom)
...
相关 API 请参考 [ZegoMediaPlayer > enableVoiceChanger\|_blank](@enableVoiceChanger-IZegoMediaPlayer)
...
相关 API 请参考 [enableAudioCapture\|_blank](@enableAudioCapture), [startcapture\|_blank](@startcapture), [setAudioSource\|_blank](@setAudioSource)
...
相关 API 请参考 [sendCustomVideoProcessedRawData\|_blank](@sendCustomVideoProcessedRawData)
...
相关 API 请参考 [ZegoAudioEffectPlayer > setPublishVolume\|_blank](@setPublishVolume-IZegoAudioEffectPlayer), [ZegoAudioEffectPlayer > setPlayVolume\|_blank](@setPlayVolume-IZegoAudioEffectPlayer), [ZegoAudioEffectPlayer > setPublishVolumeAll\|_blank](@setPublishVolumeAll-IZegoAudioEffectPlayer), [ZegoAudioEffectPlayer > setPlayVolumeAll\|_blank](@setPlayVolumeAll-IZegoAudioEffectPlayer)
...
相关 API 请参考 [updatePublishRegion\|_blank](@updatePublishRegion)
...
在 3.8.0 版本，对媒体推流器 [IZegoMediaDataPublisher] 类的一个 API 接口命名进行变更：将原来的成员函数 [setMediaDataPublisherEventHandler] 更名为 [setEventHandler]，接口名称修改后可能存在兼容性问题，详情请参考 [3.8.0 及以上版本升级指南\|_blank](!upgrade_guide5)。
...
相关 API 请参考 [ZegoExpressEngine > setVideoConfig\|_blank](@setVideoConfig-IZegoExpressEngine), [setPublishDualStreamConfig\|_blank](@setPublishDualStreamConfig)
...
相关 API 请参考 [onRectChanged\|_blank](@onRectChanged)
...
相关 API 请参考 [ZegoMediaPlayer > updatePosition\|_blank](@updatePosition-IZegoMediaPlayer), [ZegoAudioEffectPlayer > updatePosition\|_blank](@updatePosition-IZegoAudioEffectPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > setPlaySpeed\|_blank](@setPlaySpeed-IZegoMediaPlayer)
...
相关 API 请参考 [createMediaDataPublisher\|_blank](@createMediaDataPublisher)
...
相关 API 请参考 [ZegoSceneParam > token\|_blank](@token-ZegoSceneParam), [ZegoRangeScene > renewToken\|_blank](@renewToken-IZegoRangeScene)
...
当前仅支持通过内部采集的方式使用该能力，详情请参考 [主体分割\|_blank](!background_segment)。
...
相关 API 请参考 [setEngineConfig\|_blank](@setEngineConfig), [enableHardwareEncoder\|_blank](@enableHardwareEncoder), [enableHardwareDecoder\|_blank](@enableHardwareDecoder)
...
相关 API 请参考 [ZegoRangeAudio > setPositionUpdateFrequency\|_blank](@setPositionUpdateFrequency-IZegoRangeAudio)
...
相关 API 请参考 [ZegoMediaPlayer > clearView\|_blank](@clearView-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoAudioEffectPlayer > setPlaySpeed\|_blank](@setPlaySpeed-IZegoAudioEffectPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > loadResourceWithPosition\|_blank](@loadResourceWithPosition-IZegoMediaPlayer)
...
相关 API 请参考 [getCurrentAudioDevice\|_blank](@getCurrentAudioDevice)
...
相关 API 请参考 [ZegoMediaPlayer > loadResource\|_blank](@loadResource-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > loadResourceFromMediaData\|_blank](@loadResourceFromMediaData-IZegoMediaPlayer)

#### 5. 媒体播放器新增支持重复播放次数

开发者在使用媒体播放器时，可能需要统一文件预置播放多次，SDK 提供此功能进行循环播放，可以调用 [setPlayLoopCount] 接口设置重复播放次数。

相关 API 请参考 [setPlayLoopCount\|_blank](@setPlayLoopCount)
...
相关 API 请参考 [ZegoMediaPlayer > setNetWorkResourceMaxCache\|_blank](@setNetWorkResourceMaxCache-IZegoMediaPlayer), [ZegoMediaPlayer > getNetWorkResourceCache\|_blank](@getNetWorkResourceCache-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > setNetWorkBufferThreshold\|_blank](@setNetWorkBufferThreshold-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > takeSnapshot\|_blank](@takeSnapshot-IZegoMediaPlayer)

#### 3. 支持对音频设备静音或取消静音

可根据需要对音频输入/输出设备静音或取消静音。

相关 API 请参考 [muteAudioDevice\|_blank](@muteAudioDevice), [isAudioDeviceMuted\|_blank](@isAudioDeviceMuted)

#### 4. 支持对音频设备音量进行监控

可以监控音频输入/输出设备的音量。

相关 API 请参考 [startAudioDeviceVolumeMonitor\|_blank](@startAudioDeviceVolumeMonitor), [stopAudioDeviceVolumeMonitor\|_blank](@stopAudioDeviceVolumeMonitor)

#### 5. 支持设置系统声卡的采集音量

可根据需要设置系统声卡音量，有效范围为 [0, 200], 默认为 100。

相关 API 请参考 [setMixSystemPlayoutVolume\|_blank](@setMixSystemPlayoutVolume)
...
相关 API 请参考 [ZegoMediaPlayer > enableAccurateSeek\|_blank](@enableAccurateSeek-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > setPublishVolume\|_blank](@setPublishVolume-IZegoMediaPlayer), [ZegoMediaPlayer > setPlayVolume\|_blank](@setPlayVolume-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoAudioEffectPlayer > seekTo\|_blank](@seekTo-IZegoAudioEffectPlayer)
...
相关 API 请参考 [setPlayVolume\|_blank](@setPlayVolume)
...
相关 API 请参考 [ZegoMediaPlayer > setVoiceChangerParam\|_blank](@setVoiceChangerParam-IZegoMediaPlayer)
...
相关 API 请参考 [ZegoMediaPlayer > setAudioTrackIndex\|_blank](@setAudioTrackIndex-IZegoMediaPlayer)
...
相关 API 请参考 [setAudioDeviceVolume\|_blank](@setAudioDeviceVolume), [getAudioDeviceVolume\|_blank](@getAudioDeviceVolume), [setCaptureVolume\|_blank](@setCaptureVolume), [setPlayVolume\|_blank](@setPlayVolume)
