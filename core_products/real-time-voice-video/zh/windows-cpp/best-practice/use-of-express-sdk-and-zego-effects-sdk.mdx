...
- 已在 [ZEGO 控制台\|_blank](https://console.zego.im) 创建项目，并申请有效的 AppID 和 AppSign，详情请参考 [控制台 - 项目管理\|_blank](#12107) 中的“项目信息”；并联系 ZEGO 技术支持，开通 ZEGO Effects 相关套餐服务权限。
- 已在项目中集成 ZEGO Express SDK，并实现了基本的音视频推拉流功能，详情请参考 [快速开始 - 集成\|_blank](!ExpressVideoSDK-Integration/SDK_Integration) 和 [快速开始 - 实现视频通话\|_blank](!ExpressVideoSDK-Integration/Solution_Implementation)。
- 已在项目中集成 ZEGO Effects SDK，详情请参考“AI 美颜”的 [快速开始 - 集成](#9575)。
- 已获取到 ZEGO Effects SDK 唯一的鉴权文件，详情请参考“AI 美颜”的 [快速开始 - 在线鉴权](#11988)。
...
2. 获取视频原始数据，可通过 ZEGO Express SDK 的 [自定义视频采集](#3678) 或 [自定义视频前处理](#10357) 两种方式获取。
...
1. 导入 Effects 模型和资源。
    ...
    ZEGO Effects SDK 支持的所有资源和模型请参考“AI 美颜”的 [快速开始 - 导入资源和模型](#10189)。


2. 创建 Effects 对象。传入在 [前提条件](11690#3) 中获取到的鉴权文件，创建 Effects 对象。
...
3. 初始化 Effects 对象。
    ...
    调用 [zego_effects_init_env](/article/api?doc=Effects_SDK_API~c_windows~class~ZegoEffects#zego-effects-init-env) 接口初始化 Effects 对象，需要传入待处理视频图像数据的宽高。
...
调用 [createEngine](/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-zego-express-sdk#create-engine) 接口，初始化 ZEGO Express SDK。
...
- **方式一：自定义视频前处理**
    ...
    开发者通过调用 [enableCustomVideoProcessing](/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#enable-custom-video-processing) 接口，开启自定义视频前处理；开启后，ZEGO Express SDK 内部会采集视频数据；采集完成后，可以通过 [onCapturedUnprocessedRawData](/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-custom-video-process-handler#on-captured-unprocessed-raw-data) 回调接口，获取采集到的视频原始数据。
    ...
    具体的原理可参考“实时音视频”的 [自定义视频前处理](#10357)。

<a id="CustomCollection"></a>

- **方式二：自定义视频采集**

    自定义视频采集，主要依赖开发者自行采集视频数据，具体方式请参考“实时音视频”的 [自定义视频采集](#3678)。
...
- **方式一：自定义视频前处理**
    ...
    在 [onCapturedUnprocessedRawData](/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-custom-video-process-handler#on-captured-unprocessed-raw-data) 回调中，获取到视频原始数据后，调用 ZEGO Effects SDK 的相关接口，进行 AI 美颜处理（请参考 [美颜](#11769)、[美型](#11770)、[背景分割](#11771)、[人脸检测](#11772)、[挂件](#11773)、[滤镜](#11774)），并将处理后的数据，返回给 ZEGO Express SDK。
...
- **方式二：自定义视频采集**
    ...
    在接收到自定义采集的 [onStart](@onStart) 回调后，开发者通过自定义采集获取视频数据，再调用 ZEGO Effects SDK 的相关接口，进行 AI 美颜处理（请参考 [美颜](#11769)、[美型](#11770)、[背景分割](#11771)、[人脸检测](#11772)、[挂件](#11773)、[滤镜](#11774)），并将处理后的数据，返回给 ZEGO Express SDK（可参考 [自定义视频采集](11795#4_3) 中的 “3 向 SDK 发送视频帧数据”）。

### 4 推流处理后的数据

经由 ZEGO Effects SDK 处理完成后，将处理后的数据，返回给 ZEGO Express SDK。

ZEGO Express SDK 调用 [startPublishingStream](/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#start-publishing-stream) 接口，传入处理后的数据流 streamID，开始推流，发送给云服务器。
...
ZEGO Express SDK 开始推流后，远端用户可以调用 [startPlayingStream](/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#start-playing-stream) 接口，传入处理后的数据流 streamID，拉取视频数据，进行播放。
