...
如需从 CDN 拉流切换到连麦场景，请确保当前音视频流，已实现 [通过 URL 拉流](/real-time-video-windows-cpp/live-streaming/playing-stream-by-url)。

## 实现步骤

### 观众端切换拉流方式

当从 CDN 拉流切换到连麦场景时，观众端需调用 [stopPlayingStream](@stopPlayingStream) 停止当前 CDN 拉流，并调用 [startPlayingStream](@startPlayingStream) 切换为 RTC 拉流。
...
连麦结束，传入主播的流 ID，并在观众端调用 [stopPlayingStream](@stopPlayingStream) 接口，停止 RTC 拉流 ，重新切换为 CDN 拉流，并通过 [ZegoViewMode](@-ZegoViewMode) 设置 CDN 参数，详情请参考 [通过 URL 拉流](/real-time-video-windows-cpp/live-streaming/playing-stream-by-url)。
