...
<img src="/Pics/Common/ZegoExpressEngine/Second_opening_scheme.png" width=30% >
...
详情请参考 [快速开始 - 实现流程\|_blank](!Integration/Solution_Implementation#CreateEngine) 的“初始化”章节。
...
详情请参考 [快速开始 - 实现流程\|_blank](!Integration/Solution_Implementation#createroom) 的“登录房间”房间章节。
...
- 推荐开发者使用超低延迟直播（Low-Latency Live Streaming，L3）进行拉流，实现更高质量的直播体验，具体请参考 [超低延迟直播\|_blank](!Publisher_Player_Advanced/Low_Latency_Live#3) 。
- 当开发者使用 ZEGO 配置的 CDN 进行直推时，则可以直接通过 streamID 进行拉流，请参考 [快速开始 - 实现视频通话\|_blank](!Integration/Solution_Implementation#3_4) 的 “3.4 拉流”。
- 当开发者使用第三方 CDN 进行直推时，则可以通过 URL 进行拉流，请参考 [通过 URL 拉流 \|_blank](!Publisher_Player_Advanced/PlayingStreamByCDN)。
</div>
