...
- T0 = 0s：用户 A 的 SDK 收到客户端发起的 [loginRoom\|_blank](@loginRoom__2) 请求。
- T1 ≈ T0 + 120 ms：通常在调用 [loginRoom\|_blank](@loginRoom__2) 120 ms 后，客户端可以加入房间。加入房间过程中，用户 A 的客户端会收到 2 次 [onRoomStateChanged\|_blank](@onRoomStateChanged) 回调，分别通知客户端正在连接房间（Logining）以及连接房间成功（Logined）。
...
[Express SDK 是否支持断线重连机制？\|_blank](https://doc-zh.zego.im/faq/reconnect?product=ExpressVideo&platform=windows)
