...
如果需要设置自定义变声，请查看 [设置自定义变声](/real-time-video-windows-cpp/audio/audio-effects)。
</Note>
...
如果要使用 Autobot 和 OutOfPower 变声效果，请联系技术支持编包。
</Note>
...
如果需要设置预设变声，请查看 [设置预设变声](/real-time-video-windows-cpp/audio/audio-effects)。
</Note>
...
<Note title="说明">
如果需要设置自定义混响，请查看 [设置自定义混响](/real-time-video-windows-cpp/audio/audio-effects)。
</Note>
...
<Note title="说明">
如果需要设置预设混响，请查看 [设置预设混响](/real-time-video-windows-cpp/audio/audio-effects)。
</Note>

若 SDK 预设的混响类型无法满足需求，开发者可以调用 [setReverbAdvancedParam](@setReverbAdvancedParam) 中 [ZegoReverbAdvancedParam ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~CPP_windows~struct~zego-express-zego-reverb-advanced-param) 方法，通过相关参数搭配设置，实现开发者需要的混响效果（详细参数说明请参考 API 文档）。
