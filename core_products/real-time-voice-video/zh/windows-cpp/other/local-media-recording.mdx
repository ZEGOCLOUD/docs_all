...
sequenceDiagram
    ...
    note over APP,ZEGO Express SDK: 创建 ZegoExpressEngine 实例
    APP->>ZEGO Express SDK: createEngine
    note over APP,ZEGO Express SDK: 设置录制信息回调
    APP->>ZEGO Express SDK: setDataRecordEventHandler
    note over APP,ZEGO Express SDK: 登录房间
    APP->>ZEGO Express SDK: loginRoom
    note over APP,ZEGO Express SDK: 开始推流或启动预览以启动数据采集<br/>(开启麦克风、摄像头)
    APP->>ZEGO Express SDK: startPublishingStream/startPreview
    note over APP,ZEGO Express SDK: 开始录制
    APP->>ZEGO Express SDK: startRecordingCapturedData
    note over APP,ZEGO Express SDK: 处理录制信息的回调通知
    ZEGO Express SDK->>APP: onCapturedDataRecordStateUpdate
    ZEGO Express SDK->>APP: onCapturedDataRecordProgressUpdate
    note right of ZEGO Express SDK: 持续触发 onCapturedDataRecordProgressUpdate 回调
    note over APP,ZEGO Express SDK: 停止录制
    APP->>ZEGO Express SDK: stopRecordingCapturedData
    ZEGO Express SDK->>APP: onCapturedDataRecordStateUpdate
    note over APP,ZEGO Express SDK: 停止拉流或停止预览
...
- [ZegoDataRecordConfig ](@-ZegoDataRecordConfig) 指定录制的配置，可以指定录制的路径以及录制的类型 [ZegoDataRecordType](@-ZegoDataRecordType)。
...
- 在 [ZegoDataRecordConfig](@-ZegoDataRecordConfig) 指定录制的配置中，文件路径的后缀名必须是 “.mp4”、“.flv” 或 “.aac” 结尾以指定录制文件的格式。
- 录制的类型 [ZegoDataRecordType](@-ZegoDataRecordType) 与文件的后缀名无关。
</Warning>
...
在录制过程中，若退出房间 [logoutRoom](@logoutRoom) SDK 内部会主动停止录制。
</Warning>
...
* 调用系统默认的播放器进行播放。
* 调用 ZegoExpressEngine 中的 ZegoMediaPlayer 组件进行播放，详情可参考 [媒体播放器](/real-time-video-windows-cpp/other/media-player)。
...
请参考 [原始音频数据获取](/real-time-video-windows-cpp/audio/get-audio-raw-data)。
