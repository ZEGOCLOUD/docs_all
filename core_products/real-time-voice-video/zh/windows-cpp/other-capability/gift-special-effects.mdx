...
调用 [createMediaPlayer\|_blank](@createMediaPlayer) 创建媒体播放器，并调用 [setPlayerCanvas\|_blank](@setPlayerCanvas-IZegoMediaPlayer) 设置播放器 canvas 的 `alphaBlend` 属性为 `true`。
...
调用 [createMediaPlayer\|_blank](@createMediaPlayer) 创建媒体播放器后，通过 [loadResourceWithConfig\|_blank](@loadResourceWithConfig-ZegoMediaPlayer) 接口，通过**文件路径**或**二进制内存数据**，播放拼接后的特效素材。

<div class="mk-warning">

虽然 [loadResourceWithConfig\|_blank](@loadResourceWithConfig-ZegoMediaPlayer) 接口支持通过版权音乐资源 ID 加载多媒体资源，但是版权音乐目前不支持视频，因此无法通过版权音乐资源 ID 加载方式来加载礼物特效。
</div>
...
在调用 [loadResourceWithConfig\|_blank](@loadResourceWithConfig-IZegoMediaPlayer) 加载文件成功后，可调用 [start\|_blank](@start-IZegoMediaPlayer)、[pause\|_blank](@pause-IZegoMediaPlayer)、[resume\|_blank](@resume-IZegoMediaPlayer)、[stop\|_blank](@stop-IZegoMediaPlayer) 来启停播放。一旦播放器的内部状态改变，[IZegoMediaPlayer\|_blank](@-IZegoMediaPlayer) 的 [onMediaPlayerStateUpdate\|_blank](@onMediaPlayerStateUpdate-IZegoMediaPlayerEventHandler) 回调将会被触发。

开发者也可通过调用 [getCurrentState\|_blank](@getCurrentState-IZegoMediaPlayer) 随时获取播放器的当前状态。

如果 [enableRepeat\|_blank](@enableRepeat-IZegoMediaPlayer) 设置为 `true`，则播放器会在播放完文件后自动重播。
