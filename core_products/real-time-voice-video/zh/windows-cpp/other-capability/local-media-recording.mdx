...
开发者需要调用 [setDataRecordEventHandler\|_blank](@setDataRecordEventHandler) 来设置录制功能的回调以接收录制过程中的录制信息。
...
在调用 [startRecordingCapturedData\|_blank](@startRecordingCapturedData) 开始录制之后才会收到所设置的录制回调信息。
</div>
...
开发者需要调用 [startRecordingCapturedData\|_blank](@startRecordingCapturedData) 以开始录制。
...
开发者需要调用 [stopRecordingCapturedData\|_blank](@stopRecordingCapturedData) 来结束录制。
...
3. **录制过程如何获取到录制文件的大小？**
    ...
    录制进度回调 [onCapturedDataRecordProgressUpdate\|_blank](@onCapturedDataRecordProgressUpdate) 中可以获取得到录制时长和录制文件大小。
...
5. **使用外部采集时的录制需要注意什么？**
    ...
    使用外部采集时，也需要在录制之前调用 SDK 的 [startPreview\|_blank](@startPreview) 方法来启动。
