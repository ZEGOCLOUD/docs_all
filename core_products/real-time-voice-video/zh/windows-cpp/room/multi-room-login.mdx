...
- 已在项目中集成 ZEGO Express SDK（2.9.0 及以上版本），实现基本的实时音视频功能，详情请参考 [快速开始 - 集成](/real-time-video-windows-cpp/quick-start/integrating-sdk) 和 [快速开始 - 实现流程](/real-time-video-windows-cpp/quick-start/implementing-video-call)。
...
请参考 [快速开始 - 实现流程](/real-time-video-windows-cpp/quick-start/implementing-video-call) 的 “创建引擎”  进行 SDK 的初始化。
...
多房间与单房间发送、接收实时消息的实现一致，详情请参考 [实时消息与信令](/real-time-video-windows-cpp/room/messaging-and-signaling)。

### 7 退出房间

传入房间 ID（roomID），调用 [logoutRoom](@logoutRoom__3) 接口退出房间。

<Note title="说明">

如果想一次性退出所有已登录的房间，可以直接调用不含 “roomID” 的 [logoutRoom](@logoutRoom__3) 接口。
</Note>
