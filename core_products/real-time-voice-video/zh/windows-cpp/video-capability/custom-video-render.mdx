...
<img src="/Pics/Common/ZegoExpressEngine/custom_video_render_uml_Andriod_Windows_new.png" width=80% height=80%>

### 1 设置自定义视频渲染配置

#### 1.1 创建 [ZegoCustomVideoRenderConfig\|_blank](@-ZegoCustomVideoRenderConfig) 对象并配置参数。

`bufferType` 参数是枚举 [ZegoVideoBufferType \|_blank](@-ZegoVideoBufferType)，目前 SDK 仅支持 `ZEGO_VIDEO_BUFFER_TYPE_RAWDATA` 类型。

`frameFormatSeries` 参数是枚举 [ZegoVideoFrameFormatSeries \|_blank](@-ZegoVideoFrameFormatSeries)，可指定开发者需要的自定义视频渲染视频帧数据格式，此参数只能指定 `RGB` 或 `YUV` 颜色空间大类，具体的数据格式不同平台间不一致，以回调中的参数为准，本文下半部分将会提到。
...
[enableEngineRender\|_blank](@enableEngineRender) 表示是否在要自定义视频渲染的同时，SDK 内部也渲染。如果为 `false`，则引擎不会往预览接口 [startPreview \|_blank](@startPreview) 和拉流接口 [startPlayingStream \|_blank](@startPlayingStream) 设置的 canvas 上渲染。
...
调用 [setCustomVideoRenderHandler \|_blank](@setCustomVideoRenderHandler) 接口来设置自定义视频渲染回调 [IZegoCustomVideoRenderHandler \|_blank](@-IZegoCustomVideoRenderHandler)。
...
回调方法中的 `param` （[ZegoVideoFrameParam \|_blank](@-ZegoVideoFrameParam) 对象）描述了该视频帧的一些参数，定义如下：
...
开启本地预览或推流后，SDK 会将采集到的本地视频通过 [onCapturedVideoFrameRawData \|_blank](@onCapturedVideoFrameRawData)  回调给用户。

开启拉流后，SDK 会将获取的远端流的视频数据通过 [onRemoteVideoFrameRawData \|_blank](@onRemoteVideoFrameRawData) 回调给用户。
...
2. 如果自定义视频渲染在配置 [ZegoCustomVideoRenderConfig \|_blank](@-ZegoCustomVideoRenderConfig) 里的 [enableEngineRender \|_blank](@enableEngineRender) 参数为 `false` 时，预览接口 [startPreview \|_blank](@startPreview) 及拉流接口 [startPlayingStream \|_blank](@startPlayingStream) 中的 `canvas` 参数填什么？

   答：当 [enableEngineRender \|_blank](@enableEngineRender) 为 `false` 时，引擎不渲染，因此预览和拉流接口的 `canvas` 参数可置空 `nullptr`。
...
4. 本地采集预览自定义视频渲染的视频帧的宽高是多少？
   ...
   答：本地采集预览自定义视频渲染的视频帧的宽高会在回调中的 [param \|_blank](@-ZegoVideoFrameParam) 参数里返回，其值与 [setVideoConfig \|_blank](@setVideoConfig) 设置的分辨率相同。

5. 自定义视频渲染出来的视频帧数据格式是什么？

   答：根据选择的渲染数据格式系列 [ZegoVideoFrameFormatSeries \|_blank](@-ZegoVideoFrameFormatSeries) 以及软解/硬解码后直接返回的数据决定，即视频帧回调方法中的 [ZegoVideoFrameParam \|_blank](@-ZegoVideoFrameParam) 参数里的 `format` 参数。
...
7. 自定义视频渲染如何拿到第一帧数据呢？
   ...
   答：自定义视频渲染回调 [IZegoCustomVideoRenderHandler \|_blank](@-IZegoCustomVideoRenderHandler) 的第一次返回的数据即第一帧数据。

8. 为什么收不到本地视频数据的回调？

   答：需要在推流前先启动预览 [startPreview \|_blank](@startPreview)，否则收不到视频数据的回调。

9. 本地预览自定义视频渲染，视频帧数据的画面为什么没有默认做水平镜像翻转？

   答：自定义视频渲染的视频帧画面翻转需要开发者自行实现，可通过视频帧数据回调接口中的 [flipMode \|_blank](@-ZegoVideoFlipMode) 获知该帧是否需要翻转。
