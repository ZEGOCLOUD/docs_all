...
sequenceDiagram
    ...
    推流端->>ZEGO Express SDK: 初始化引擎 <br/> createEngine

    推流端->>ZEGO Express SDK: 登录房间 <br/> loginRoom

    拉流端->>ZEGO Express SDK: 初始化 <br/> createEngine

    拉流端->>ZEGO Express SDK: 登录房间 <br/> loginRoom

    alt 可选:自定义渲染及回调
        推流端-->>ZEGO Express SDK: enableCustomVideoRender、<br>setCustomVideoRenderHandler
    end

    推流端->>ZEGO Express SDK: 监听主体分割状态回调 <br/> onVideoObjectSegmentationStateChanged

    推流端->>ZEGO Express SDK: 开启主体分割 <br/> enableVideoObjectSegmentation<br>(背景虚化/虚拟背景/透明背景)

    alt 可选:使用 Alpha 通道传输分割出的主体
        推流端-->>ZEGO Express SDK: enableAlphaChannelVideoEncoder
    end

    推流端->>ZEGO Express SDK: 开始预览和推流 <br/> startPreview、<br>startPublishingStream

    alt 可选:在拉流端设置 Alpha 通道渲染并拉流
        拉流端-->>ZEGO Express SDK: ZegoCanvas.alphaBlend = true、<br>startPlayingStream
    end

    推流端->>ZEGO Express SDK: 关闭主体分割 <br/> enableVideoObjectSegmentation

    推流端->>ZEGO Express SDK: 销毁引擎 <br/> destroyEngine

    拉流端->>ZEGO Express SDK: 销毁引擎 <br/> destroyEngine
```
{/*
...
初始化和登录房间的具体流程，请参考实现视频通话文档中的 “[创建引擎](/real-time-video-windows-cpp/quick-start/implementing-video-call)”及“[登录房间](/real-time-video-windows-cpp/quick-start/implementing-video-call)”。
...
调用 [enableCustomVideoRender](@enableCustomVideoRender) 接口，设置引擎进阶配置并开启自定义渲染，同时调用 [setCustomVideoRenderHandler](@setCustomVideoRenderHandler) 接口设置自定义视频渲染回调，详情请参考 [自定义视频渲染](/real-time-video-windows-cpp/video/custom-video-rendering)。
...
调用 [enableVideoObjectSegmentation](@enableVideoObjectSegmentation__1) 接口，开启主体分割，并设置背景处理类型为“虚化”。
...
调用 [enableVideoObjectSegmentation](@enableVideoObjectSegmentation__1) 接口，开启主体分割，并设置背景处理类型为“图片”或“视频”。
...
调用 [enableVideoObjectSegmentation](@enableVideoObjectSegmentation__1) 接口，开启主体分割，并设置背景处理类型为“透明”。
...
- **方式 1**：使用内部渲染
    ...
    若使用内部渲染，开启预览前，请设置 [alphaBlend](@alphaBlend-ZegoCanvas) 为 true。
...
- **方式 1**：使用内部渲染
    ...
    如果使用内部渲染，拉流端在调用 [startPlayingStream](@startPlayingStream) 接口开启拉流前，需要将 [ZegoCanvas](@-ZegoCanvas) 的 [alphaBlend](@alphaBlend-ZegoCanvas) 属性设置为 true。
