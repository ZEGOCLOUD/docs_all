...
3. Windows 端支持采集非系统默认音频输出设备
    ...
    相关 API 请参考： [setAudioDevice](@setAudioDevice)
...
ZegoScreenCaptureSource 新增 enableAudioCaptureWithConfig 接口设置是否允许系统音频采集，使用前需调用 setAudioSource 将采集源设为 ZegoAudioSourceTypeScreenCapture ，且屏幕采集与推流通道需相同。
<Warning title="注意">
...
相关 API 请参考： [enableAudioCaptureWithConfig](@enableAudioCaptureWithConfig)、 [setAudioSource](@setAudioSource)
