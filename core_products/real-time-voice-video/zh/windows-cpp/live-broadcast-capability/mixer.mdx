...
可参考 [快速开始 - 实现流程](!Integration/Solution_Implementation#CreateEngine) 的 “创建引擎” 和 “登录房间” 完成房间登录。
...
通过 [ZegoMixerOutput](@-ZegoMixerOutput) 设置混流输出列表，用户可以从列表中的输出目标拉取混流。    
...
1. **能将混流推到第三方 CDN 吗？如何转推多路 CDN？**
    ...
    若需要将混流推到第三方 CDN，可在 [ZegoMixerOutput](@-ZegoMixerOutput) 的 “target” 参数填写 CDN 的 URL。

    填写的 URL 格式需要为 RTMP 格式：“rtmp://xxxxxxxx”。

    推多路 CDN 就创建 N 个输出流对象 [ZegoMixerOutput](@-ZegoMixerOutput) 放入 [ZegoMixerTask](@-ZegoMixerTask) 中的 “outputList” 输出列表中。
