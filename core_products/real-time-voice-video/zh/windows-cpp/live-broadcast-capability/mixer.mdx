...
可参考 [快速开始 - 实现流程\|_blank](!Integration/Solution_Implementation#CreateEngine) 的 “创建引擎” 和 “登录房间” 完成房间登录。
...
通过 [ZegoMixerAudioConfig\|_blank](@-ZegoMixerAudioConfig) 设置混流音频相关配置，主要包括音频码率、声道数、编码 ID。 
...
通过 [ZegoMixerOutput\|_blank](@-ZegoMixerOutput) 设置混流输出列表，用户可以从列表中的输出目标拉取混流。    
...
请参考 [快速开始 - 实现流程\|_blank](!Integration/Solution_Implementation#CreateEngine) 的 “创建引擎” 和 “登录房间” 完成房间登录。
