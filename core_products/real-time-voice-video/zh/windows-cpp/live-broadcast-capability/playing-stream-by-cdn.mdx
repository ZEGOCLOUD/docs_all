...
- 已将音视频流推送到 CDN，并知晓相应的 URL，详情请参考 [使用 CDN 直播\|_blank](!Publisher_Player_Advanced/RelayToCDN)。
...
直接通过 CDN 的 URL 地址拉流，需要使用 [ZegoCDNConfig\|_blank](@-ZegoCDNConfig) 对象来填入 URL 参数，如果对应的 CDN 配置了拉流鉴权，还需要通过 “authParam” 字段填入鉴权参数。  
...
通过调用 [startPlayingStream\|_blank](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~cpp_windows~class~IZegoExpressEngine#start-playing-stream-1) 接口开始拉流。
...
若要停止拉流，调用 [stopPlayingStream\|_blank](@stopPlayingStream) 接口。
