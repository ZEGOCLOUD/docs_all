...
1. 调用 [loginRoom](@loginRoom) 接口登录房间。
2. 调用 [startPublishingStream](@startPublishingStream) 接口推流。
3. 在推流成功后，调用 [sendSEI](@sendSEI) 接口发送 SEI 信息。

拉流端：

1. 创建 [IZegoEventHandler](@-IZegoEventHandler) 对象，并重写接收 SEI 信息的 [onPlayerRecvSEI](@onPlayerRecvSEI) 方法。
2. 调用 [loginRoom](@loginRoom) 接口登录房间。
3. 调用 [startPlayingStream](@startPlayingStream) 接口拉流。
4. 在拉流成功后，接收到推流端发送的 SEI 信息之后触发 [onPlayerRecvSEI](@onPlayerRecvSEI) 回调。
