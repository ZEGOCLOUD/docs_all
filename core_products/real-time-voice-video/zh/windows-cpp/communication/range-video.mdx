...
<p>上图仅以范围语音模式为“全世界”时为例，更多不同模式组合关系下的声音可达情况请参考 <a href="#设置通用语音模式">设置通用语音模式</a>。</p>
</td>
</tr>
<tr>
<td>3D 音效</td>
...
<Note title="说明">
<ul><li>队友之间的通话不受“范围”和 “3D 音效” 的影响，详情请查看 <a href="#设置通用语音模式">设置通用语音模式</a>。</li><li>如果您希望自定义语音收发能力，请参考 <a href="#设置自定义语音模式">设置自定义语音模式</a>。</li></ul>
</Note>
</td>
</tr>
</tbody></table>
...
- 范围语音功能模块中推拉流的相关回调（[onPublisherStateUpdate](@onPublisherStateUpdate) 、[onPlayerStateUpdate](@onPlayerStateUpdate)、[onPublisherQualityUpdate](@onPublisherQualityUpdate) 和 [onPlayerQualityUpdate](@onPlayerQualityUpdate)）不再生效。
...
sequenceDiagram
    ...
    客户端 App->>ZEGO Express SDK: 创建引擎 <br/> createEngine

    客户端 App->>ZEGO Express SDK: 创建范围语音模块 <br/> createRangeAudio

    客户端 App->>ZEGO Express SDK: 监听范围语音事件回调 <br/> setEventHandler

    客户端 App->>ZEGO Express SDK: 登录房间 <br/> loginRoom

    critical 关键步骤
    客户端 App->>ZEGO Express SDK: 设置收听者的当前位置 <br/> updateSelfPosition

    客户端 App->>ZEGO Express SDK: 添加/更新发声者位置信息 <br/> updateAudioSource

    客户端 App->>ZEGO Express SDK: 开启麦克风、扬声器 <br/> enableMicrophone、enableSpeaker

    ZEGO Express SDK-->>客户端 App: 回调麦克风使用状态 <br/> onRangeAudioMicrophoneStateUpdate
    end

    客户端 App->>ZEGO Express SDK: 销毁范围语音模块 <br/> destroyRangeAudio

    客户端 App->>ZEGO Express SDK: 退出房间 <br/> logoutRoom
```
{/*
...
调用的 [createRangeAudio ](@createRangeAudio) 方法创建范围语音实例。
...
可以根据需要调用 [setEventHandler ](@setEventHandler-IZegoRangeAudio) 接口为麦克风设置事件回调，用于监听麦克风的开启状态 [onRangeAudioMicrophoneStateUpdate ](@onRangeAudioMicrophoneStateUpdate) 通知。
...
传入用户 ID 参数 userID 和 userName 创建 ZegoUser 用户对象后，调用 [loginRoom](@loginRoom) 接口，传入房间 ID 参数 roomID 和用户参数 user，登录房间。
...
开发者可以通过调用 [updateSelfPosition](@updateSelfPosition) 接口，设置听者自身的所在位置和方位，或者在自身方位发生变化时更新自己在世界坐标系中的位置和朝向。
...
登录房间成功后，可以通过调用 [updateAudioSource](@updateAudioSource) 接口，添加或更新发声者的位置信息。
...
调用 [setAudioReceiveRange](@setAudioReceiveRange) 接口设置听者接收音频距离的最大范围，即以自身为起点，3D 空间中以设置的距离为立体空间。设置该范围后，在开启 3D 音效的情况下，声音将会随距离的增加而衰减，直至超出所设置的范围，则不再有声音。
...
同时您也可以通过 [setAudioReceiveRange](@setAudioReceiveRange__1) 接口进一步控制衰减范围。距离小于 min 时，音量不会随着距离的增加而衰减；距离大于 max 时，将无法听到对方的声音。
...
实现此功能完成以下步骤：
1. 调用 [enableSpatializer](@enableSpatializer) 接口开启 3D 音效。
2. 设置本地播放器的 3D 音效
    - 媒体播放器：

        <Note title="说明">
        请参考 [媒体播放器](/real-time-video-windows-cpp/other/media-player) 了解如何创建媒体播放器与加载媒体资源。
        </Note>

        1. 调用 [createMediaPlayer](@createMediaPlayer) 接口创建媒体播放器。
        2. 调用 [IZegoMediaPlayer.updatePosition](@updatePosition-IZegoMediaPlayer-class) 接口设置媒体播放器在世界坐标系中的位置。
        3. 调用 [loadResourceWithConfig](@loadResourceWithConfig) 接口加载媒体资源。
    - 音效播放器：

        <Note title="说明">
        请参考 [音效文件播放器](/real-time-video-windows-cpp/other/audio-effect-player) 了解如何创建音效播放器与加载音效资源。
        </Note>

        1. 调用 [createAudioEffectPlayer](@createAudioEffectPlayer) 接口创建音效播放器。
        2. 调用 [IZegoAudioEffectPlayer.start](@start-IZegoAudioEffectPlayer-class) 接口播放音效资源。
        3. 在收到 [onAudioEffectPlayStateUpdate](@onAudioEffectPlayStateUpdate) 回调状态为 `ZegoAudioEffectPlayState.Playing` 后，调用 [IZegoAudioEffectPlayer.updatePosition](@updatePosition-IZegoAudioEffectPlayer-class) 接口设置正在播放的音效资源在世界坐标系中的位置。
...
- 调用 [enableMicrophone](@enableMicrophone) 接口，设置是否开启麦克风。当 enable 取值为 true 时表示开启，此时 SDK 将会自动使用主通道推音频流，为 false 时表示关闭。（可随时开启或关闭）

    开发者可以通过监听 [onRangeAudioMicrophoneStateUpdate](@onRangeAudioMicrophoneStateUpdate) 事件回调，获取麦克风更新后的状态。

- 调用 [enableSpeaker](@enableSpeaker) 接口，设置是否开启扬声器。enable 取值为 true 时表示开启，此时将会自动拉取房间内的音频流，为 false 时表示关闭。（可随时开启或关闭）
...
当不再使用范围语音模块时可调用 [destroyRangeAudio ](@destroyRangeAudio) 接口销毁，释放范围语音模块占用的资源。
