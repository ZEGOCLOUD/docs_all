...
在 Token 过期前 30 秒，SDK 会通过 [onSceneTokenWillExpire](@onSceneTokenWillExpire) 回调发出通知。

收到该回调后，开发者需要从自己的服务端获取新的有效 Token，并调用 SDK 提供的 [IZegoRangeScene.renewToken](@renewToken-IZegoRangeScene) 接口更新 Token。若未处理，Token 过期的处理机制如下：
