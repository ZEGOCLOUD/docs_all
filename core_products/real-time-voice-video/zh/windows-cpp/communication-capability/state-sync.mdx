...
<Frame width="512" height="auto" caption="">
  <img src="https://storage.zego.im/sdk-doc/Pics/Express/state_sync_aoi.jpg" />
</Frame>
...
根据需要调用 [IZegoRangeScene.setEventHandler](@setEventHandler-IZegoRangeScene) 
接口设置范围场景事件回调，用于监听范围场景状态、登录状态、进出 AOI 通知等。调用 [IZegoRangeSceneItem.setEventHandler](@setEventHandler-IZegoRangeSceneItem) 接口设置范围场景物品管理类事件回调，用于监听物品进入离开 AOI 范围、物品绑定状态变更、物品状态和命令更新事件。
...
如果您需要自定义模板，详情请参考 [服务端 API - 场景模版配置](/real-time-video-ios-oc/integration/sdk-integration59)。
</Warning>

### 同步状态

通过 [updateUserStatus](@updateUserStatus)、[updateUserCommand](@updateUserCommand) 接口更新用户状态及用户命令。通过 [onUserStatusUpdate](@onUserStatusUpdate)、[onUserCommandUpdate](@onUserCommandUpdate) 回调接收 AOI 范围内的远端用户位置、命令等状态信息。
...
### （可选）获取场景内用户数、获取 AOI 范围内用户列表

<Accordion title="获取场景内用户数、获取 AOI 范围内用户列表" defaultOpen="false">
可以根据需要调用 [getUserCount](@getUserCount)、[getUserListInView](@getUserListInView) 接口获取场景内用户数、AOI 范围内用户列表。
...
根据需要调用 [createItem](@createItem) 接口在场景内创建物品。
...
如果需要对物品进行操作（即更新物品状态命令），首先需要调用 [bindItem](@bindItem) 绑定物品。
...
通过 [updateItemStatus](@updateItemStatus)、[updateItemCommand](@updateItemCommand) 接口更新物品状态及物品命令。通过 [onItemStatusUpdate](@onItemStatusUpdate)、[onItemCommandUpdate](@onItemCommandUpdate) 回调接收 AOI 范围内的远端物品位置、命令等状态信息。
...
如果无需对物品进行操作（即更新物品状态命令），可以调用 [unbindItem](@unbindItem) 解绑物品。
...
根据需要调用 [destroyItem](@destroyItem) 销毁场景内的物品。
...
当不再使用范围场景模块时，可以调用 [destroyRangeScene](@destroyRangeScene) 接口销毁范围场景模块。
