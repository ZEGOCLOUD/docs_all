...
您可以通过监听 [onNetworkQuality\|_blank](@onNetworkQuality) 回调，收到用户（包括您自己）的上下行网络质量。此回调每隔两秒会收到一次，网络质量等级请参考 [ZegoStreamQualityLevel\|_blank](@-ZegoStreamQualityLevel)。

不同版本的 [onNetworkQuality\|_blank](@onNetworkQuality) 回调逻辑有所不同：
...
在拉流成功后，开发者可通过 [onPlayerStateUpdate\|_blank](@onPlayerStateUpdate) 获取拉流状态变更的通知。
...
开发者可通过注册 [onPlayerRecvAudioFirstFrame\|_blank](@onPlayerRecvAudioFirstFrame) 监听拉流端音频接收首帧回调。调用拉流接口成功后，SDK 拉流拉到第一帧音频数据时会收到此回调。
...
开发者可通过注册 [onPlayerRecvVideoFirstFrame\|_blank](@onPlayerRecvVideoFirstFrame) 监听拉流端接收视频首帧回调。调用拉流接口成功后，SDK 拉流拉到第一帧视频数据时会收到此回调。
...
- [怎么处理视频卡顿问题？\|_blank](https://doc-zh.zego.im/faq/video_freeze)
- [怎么处理音频卡顿问题？\|_blank](https://doc-zh.zego.im/faq/audio_freeze)
