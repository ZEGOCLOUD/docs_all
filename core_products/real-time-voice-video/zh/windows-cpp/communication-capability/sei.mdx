...
1. 创建 [IZegoEventHandler\|_blank](@-IZegoEventHandler) 对象，并重写接收 SEI 信息的 [onPlayerRecvSEI\|_blank](@onPlayerRecvSEI) 方法。
2. 调用 [loginRoom\|_blank](@loginRoom) 接口登录房间。
3. 调用 [startPlayingStream\|_blank](@startPlayingStream) 接口拉流。
4. 在拉流成功后，接收到推流端发送的 SEI 信息之后触发 [onPlayerRecvSEI\|_blank](@onPlayerRecvSEI) 回调。
