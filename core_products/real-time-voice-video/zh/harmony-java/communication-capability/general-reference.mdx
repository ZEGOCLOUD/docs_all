...
调用 [loginRoom\|_blank](@loginRoom) 接口登录房间时使用的 roomID、userID，必须和 “4.2 生成 Token” 时使用的 roomID、userID 保持一致。 
</div>
...
收到该回调后，开发者需要从自己的服务端获取新的有效 Token，并调用 SDK 提供的 [renewToken\|_blank](@renewToken) 接口更新 Token。若未处理：
