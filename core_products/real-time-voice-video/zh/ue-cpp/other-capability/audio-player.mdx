...
调用 `ZegoExpressEngine` 的 [createAudioEffectPlayer \|_blank](@createAudioEffectPlayer) 方法创建音效播放器实例。
...
可以根据需要调用音效播放器的 [setEventHandler \|_blank](@setEventHandler) 方法为播放器设置事件回调，用于监听`音效播放状态改变`等通知。
...
调用 [start \|_blank](@start) 方法播放音效，目前仅支持同时播放 12 个，且只能为本地文件，不支持播放网络资源。 `audioEffectID` 需要保持全局唯一。

* 如果已通过 [loadResource \|_blank](@loadResource) 方法预先加载了音效，则只需要传入预加载时的 `audioEffectID` ，`path`（音效资源的路径） 字段传空即可。
* 若需要重复播放可以通过 `ZegoAudioEffectPlayConfig` 中 `playCount` 配置重复次数。如果设置为0，则表示无限重复播放，直到用户手动调用 [stop \|_blank](@stop) 停止。
...
1. 调用 [pause \|_blank](@pause) 方法可以暂停播放 audioEffectID 指定的音效，调用 [pauseAll \|_blank](/zh/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-audio-effect-player#pause-all) 方法则暂停所有正在播放的音效。
2. 音效暂停播放后，调用 [resume \|_blank](@resume) 方法可以恢复播放 audioEffectID 指定的音效，调用 [resumeAll \|_blank](@resumeAll) 方法则恢复所有已暂停音效。
3. 调用 [stop \|_blank](@stop) 方法可以停止播放 audioEffectID 指定的音效，调用 [stopAll \|_blank](@stopAll) 方法则停止播放所有音效。
...
1. 调用 [setVolume \|_blank](@setVolume) 方法可以设置 audioEffectID 指定的音效音量，取值范围为 [0-200]，默认值为 100。
...
3. 调用 [seekTo \|_blank](@seekTo) 方法可以根据需要设置播放进度。
...
调用 [loadResource \|_blank](@loadResource) 方法加载音效资源，可通过 `callback` 参数来监听加载的结果，显示加载成功后方可播放。最多支持同时预加载 15 个本地音效文件（不支持网络资源），并且单个音效文件时长不能超过 30s，否则加载会报错。

当加载的音效使用完成后，可以调用 [unloadResource \|_blank](@unloadResource) 接口卸载，以释放相关资源。否则 SDK 将在 `ZegoAudioEffectPlayer` 实例释放时会自动卸载已加载的音效。
