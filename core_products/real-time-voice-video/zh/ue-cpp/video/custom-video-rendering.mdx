...
请参考 [下载示例源码](/real-time-video-ue-cpp/quick-start/run-example-code) 获取源码。
...
<Warning title="注意">
    ...
    - Unreal Engine SDK 不支持使用 [enableEngineRender](@enableEngineRender) 功能。
...
调用 [setCustomVideoRenderHandler ](@setCustomVideoRenderHandler) 接口设置自定义视频渲染回调 [IZegoCustomVideoRenderHandler ](@-IZegoCustomVideoRenderHandler)。
...
5. 自定义视频渲染出来的视频帧数据格式是什么？
   ...
   答：根据选择的渲染数据格式系列 [ZegoVideoFrameFormatSeries ](@-ZegoVideoFrameFormatSeries) 以及软解或硬解码后，直接返回的数据决定，即视频帧回调方法中的 [ZegoVideoFrameParam ](@-ZegoVideoFrameParam) 参数里的 `format` 参数。
