...
sequenceDiagram
    ...
    %% 用户A初始化流程
    note over 客户端A,客户端B: 用户 A 加入
    客户端A->>ZEGO Express SDK: 用户 A 创建引擎 <br/> createEngineWithProfile
    客户端A->>ZEGO Express SDK: 用户 A 登录房间 <br/> loginRoom
    客户端A->>ZEGO Express SDK: 设置事件通知回调 <br/> on
    客户端A->>ZEGO Express SDK: 用户 A 预览推流画面 <br/> startPreview
    客户端A->>ZEGO Express SDK: 用户 A 开始推流 <br/> startPublishingStream
    客户端A->>ZEGO Express SDK: 用户 A 开始拉流 <br/> startPlayingStream

    %% 用户B加入阶段
    note over 客户端A,客户端B: 用户 B 加入
    客户端B->>ZEGO Express SDK: 用户 B 创建引擎 <br/> createEngineWithProfile
    客户端B->>ZEGO Express SDK: 用户 B 登录房间 <br/> loginRoom
    客户端B->>ZEGO Express SDK: 设置事件通知回调 <br/> on
    ZEGO Express SDK-->>客户端A: 通知 A 房间内的用户更新 <br/> roomUserUpdate
    客户端B->>ZEGO Express SDK: 用户 B 预览推流画面 <br/> startPreview
    客户端B->>ZEGO Express SDK: 用户 B 开始推流 <br/> startPublishingStream
    ZEGO Express SDK-->>客户端A: 通知 A 房间内的流更新 <br/> roomStreamUpdate
    客户端B->>ZEGO Express SDK: 用户 B 开始拉流 <br/> startPlayingStream

    %% 用户A退出阶段
    note over 客户端A,客户端B: 用户 A 退出
    客户端A->>ZEGO Express SDK: 用户 A 停止预览 <br/> stopPreview
    客户端A->>ZEGO Express SDK: 用户 A 停止推流 <br/> stopPublishingStream
    ZEGO Express SDK-->>客户端B: 通知 B 房间内的流更新 <br/> roomStreamUpdate
    客户端A->>ZEGO Express SDK: 用户 A 停止拉流 <br/> stopPlayingStream
    客户端A->>ZEGO Express SDK: 用户 A 退出房间 <br/> logoutRoom
    ZEGO Express SDK-->>客户端B: 通知 B 房间内的用户更新 <br/> roomUserUpdate

    %% 用户B退出阶段
    note over 客户端A,客户端B: 用户 B 退出
    客户端B->>ZEGO Express SDK: 用户 B 停止预览 <br/> stopPreview
    客户端B->>ZEGO Express SDK: 用户 B 停止推流 <br/> stopPublishingStream
    客户端B->>ZEGO Express SDK: 用户 B 停止拉流 <br/> stopPlayingStream
    客户端B->>ZEGO Express SDK: 用户 B 退出房间 <br/> logoutRoom
```
{/*
...
定义 SDK 引擎对象，调用 [createEngineWithProfile](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/javascript_uni-app/classes/_zegoexpressengineapp_.zegoexpressengine.html) 接口，将申请到的 AppID 和 AppSign 传入参数 “appID” 和 “appSign”，创建引擎单例对象。
...
为实现多人视频通话功能，需要监听房间内用户和流的增减并做出相应处理，可通过 [on](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/javascript_uni-app/classes/_zegoexpressengineapp_.zegoexpressengine.html) 设置相关回调。
...
请参考 [快速开始 - 实现流程](/real-time-video-uniapp/quick-start/implementing-video-call) 依次完成“推流”和“拉流”相关操作。
