...
- 如果您选择使用 Token 鉴权，Token 的获取方式请参考 [使用 Token 鉴权](/real-time-video-uniapp/communication/using-token-authentication)。
</Warning>

## 获取示例源码

<Card title="示例源码" href="https://artifact-demo.zego.im/core_products/real-time-voice-video/zh/uni-app/quick-start/ZegoExpressExample-UniApp.zip">
本地下载
</Card>
...
├── package.json
├── pages                                # demo 源码
│   ├── KeyCenter.js                     # 在里面的 KeyCenter.js 文件中填写申请的 AppID、AppSign、Token
│   ├── example                          # 示例代码
│   ├── getToken.js
│   ├── index
│   └── permission.js
...
2. 示例源码中缺少 SDK 初始化所需的 AppID、AppSign 或 Token，需要修改 “pages/KeyCenter.js” 文件，请使用本文 [前提条件](/real-time-video-uniapp/quick-start/run-example-code) 已获取的 AppID、AppSign 或 Token 正确填写，否则示例源码无法正常运行。
...
4. 在项目中导入插件，具体操作请参考 [在 uni-app 项目中导入插件](/real-time-video-uniapp/quick-start/integrating-sdk)。

5. 在项目根目录打开终端执行 `npm install` 安装项目依赖。
...
[在 Windows 平台上集成 Express uni-app SDK 后，使用 dev 证书云打包运行 iOS 应用时出错，该如何处理？](https://doc-zh.zego.im/faq/windows-uniapp-ios-packaging-issue)
