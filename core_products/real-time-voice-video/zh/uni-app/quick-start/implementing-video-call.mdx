...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时音视频功能，详情请参考 [快速开始 - 集成](/real-time-video-uniapp/quick-start/integrating-sdk)。
- 已在 [ZEGO 控制台](https://console.zego.im) 创建项目，并申请有效的 AppID 和 AppSign，详情请参考 [控制台 - 项目管理](/console/project-info) 中的“项目信息”。

<Warning title="注意">

SDK 同时也支持 Token 鉴权，若您对项目安全性有更高要求，建议您升级鉴权方式，详情请参考 [如何从 AppSign 鉴权升级为 Token 鉴权](https://doc-zh.zego.im/faq/express-token-upgrade?product=ExpressVideo)。
</Warning>
...
sequenceDiagram
    ...
    participant 用户B
    用户A->>ZEGO Express SDK: 1.用户 A 创建引擎
    用户A->>ZEGO Express SDK: 2.用户 A 登录房间
    用户B->>ZEGO Express SDK: 3.用户 B 创建引擎
    用户B->>ZEGO Express SDK: 4.用户 B 登录房间
    用户B->>ZEGO Express SDK: 5.用户 B 推流
    ZEGO Express SDK->>用户A: 6.用户 A 收到房间内流变化通知<br/>(获取用户 B 的 streamID)
    用户A->>ZEGO Express SDK: 7.用户 A 传入用户 B 的 streamID 拉流
```
{/*
...
sequenceDiagram
    participant App
    participant ZEGO Express SDK
    App->>ZEGO Express SDK: 创建引擎 <br/> createEngineWithProfile
    App->>ZEGO Express SDK: 登录房间 <br/> loginRoom
    ZEGO Express SDK-->>App: 房间/用户/流状态更新回调 <br/> roomStateUpdate/roomUserUpdate/roomStreamUpdate
    App->>ZEGO Express SDK: 预览/推流 <br/> starPreview/startPublishingStream
    ZEGO Express SDK-->>App: 推流状态更新回调 <br/> publisherStateUpdate
    App->>ZEGO Express SDK: 拉流 <br/> startPlayingStream
    ZEGO Express SDK-->>App: 拉流状态更新回调 <br/> playerStateUpdate
    App->>ZEGO Express SDK: 停止预览/推流 <br/> stopPreview/stopPublishingStream
    ZEGO Express SDK-->>App: 推流状态更新回调 <br/> publisherStateUpdate
    App->>ZEGO Express SDK: 停止拉流 <br/> stopPlayingStream
    ZEGO Express SDK-->>App: 拉流状态更新回调 <br/> playerStateUpdate
    App->>ZEGO Express SDK: 退出房间 <br/> logoutRoom
    ZEGO Express SDK-->>App: 房间状态更新回调 <br/> roomStateUpdate
    App->>ZEGO Express SDK: 销毁引擎 <br/> destroyEngine
```
{/*
...
<Warning title="注意">如果是语音场景，请务必调用 `enableCamera(false)` 关闭摄像头，避免启动视频采集推流产生额外的视频流量。</Warning>
...
调用 [startPublishingStream ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/javascript_uni-app/classes/_zegoexpressengineapp_.zegoexpressengine.html) 接口，传入流 ID 参数 “streamID”，向远端用户发送本端的音视频流。
...
<Note title="说明">如果您需要了解 ZEGO Express SDK 的摄像头/视频/麦克风/音频/扬声器相关接口，请参考 [常见问题 - 如何实现开关摄像头/视频画面/麦克风/音频/扬声器？](https://doc-zh.zego.im/faq/device-switching)。</Note>
...
调用 [startPlayingStream ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/javascript_uni-app/classes/_zegoexpressengineapp_.zegoexpressengine.html) 接口，根据传入的流 ID 参数 “streamID”，拉取远端推送的音视频流。
...
为方便体验，ZEGO 提供了一个 [Web 端调试示例](https://zegoim.github.io/express-demo-web/src/Examples/QuickStart/VideoTalk/index.html?lang=zh)，在该页面下，输入相同的 AppID、RoomID，输入不同的 UserID、以及对应的 [Token](/console/development-assistance/temporary-token)，即可加入同一房间与真机设备互通。当成功开始音视频通话时，可以听到远端的音频，看到远端的视频画面。
...
调用 [stopPublishingStream ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/javascript_uni-app/classes/_zegoexpressengineapp_.zegoexpressengine.html) 接口停止发送本地的音视频流，结束通话。
...
如果启用了本地预览，开发者可以在停止推流后根据业务需要调用 [stopPreview ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/javascript_uni-app/classes/_zegoexpressengineapp_.zegoexpressengine.html) 接口停止预览。
...
调用 [stopPlayingStream ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/javascript_uni-app/classes/_zegoexpressengineapp_.zegoexpressengine.html) 接口，停止拉取远端的音视频流。
...
调用 [destroyEngine ](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/javascript_uni-app/classes/_zegoexpressengineapp_.zegoexpressengine.html) 接口销毁引擎，用于释放 SDK 使用的资源。
