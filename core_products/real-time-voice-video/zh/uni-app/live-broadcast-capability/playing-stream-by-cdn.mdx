...
直接通过 CDN 的 URL 地址拉流，需要使用 [ZegoCDNConfig\|_blank](/unique-api/express-video-sdk/zh/javascript_uni-app/classes/_zegoexpressdefines_.zegocdnconfig.html) 对象来填入 URL 参数，如果对应的 CDN 配置了拉流鉴权，还需要通过 `authParam` 字段填入鉴权参数。  
...
通过调用 [startPlayingStream\|_blank](/unique-api/express-video-sdk/zh/javascript_uni-app/classes/_zegoexpressengine_.zegoexpressengine.html#startplayingstream) 接口开始拉流。

拉流时，如果出现错误，请参考 [常见错误码 - 1004xxx 拉流相关的错误码\|_blank](10429#5)。
...
若要停止拉流，调用 [stopPlayingStream\|_blank](/unique-api/express-video-sdk/zh/javascript_uni-app/classes/_zegoexpressengine_.zegoexpressengine.html#stopplayingstream) 接口。
...
可以通过 [playerStateUpdate\|_blank](/unique-api/express-video-sdk/zh/javascript_uni-app/interfaces/_zegoexpresseventhandler_.zegoeventlistener.html#playerstateupdate) 来监听从 CDN 拉流的结果。

<div class = 'mk-warning'>

如果不是使用 ZEGO SDK 进行推流，而是使用第三方推流工具直接进行推流、但是使用 ZEGO SDK 进行拉流，这种场景下推流方没有使用 ZEGO SDK 加入房间，拉流方默认收不到 [roomStreamUpdate\|_blank](/unique-api/express-video-sdk/zh/javascript_uni-app/interfaces/_zegoexpresseventhandler_.zegoeventlistener.html#roomstreamupdate) 的回调，可以使用 [增加房间流\|_blank](#19561) 与 [删除房间流\|_blank](#19563) 的功能，让拉流端可以收到 [roomStreamUpdate \|_blank](/unique-api/express-video-sdk/zh/javascript_uni-app/interfaces/_zegoexpresseventhandler_.zegoeventlistener.html#roomstreamupdate) 的回调。
</div>
