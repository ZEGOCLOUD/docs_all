...
- 浏览器的兼容性，请参考 [ZEGO Express Web SDK 支持哪些浏览器？\|_blank](https://doc-zh.zego.im/faq/browser_support)。  
</div>
...
1. 下载 [Express-Video SDK\|_blank](#7776) 到本地，解压缩 “zego-ZegoExpressUniAppSDK” 文件。

2. 将解压缩后的文件夹，直接复制到项目工程根目录下的 “nativeplugins” 文件夹中，如果没有该目录，请手动创建。

    <img src="/Pics/QuickStart/Import_SDK_uniapp.png">

#### 方式 2：通过 uni-app 插件市场获取 [Express-Video SDK\|_blank](https://ext.dcloud.net.cn/plugin?id=3617)。
...
1. 单击项目目录的 “manifest.json” 文件后，单击 “App 原生插件配置” 中的“选择本地插件”或“选择云端插件”。
    ...
    其他常见问题，请参考 [在 Windows 平台上集成 Express uni-app SDK 后，使用 dev 证书云打包运行 iOS 应用时出错，该如何处理？\|_blank](/faq/RTC_uniapp_packing_within_Windows?product=ExpressVideo&platform=uni-app)
    </div>
...
uni-app 官方自定义调试基座使用说明请参考 [什么是自定义调试基座及使用说明\|_blank](https://uniapp.dcloud.net.cn/tutorial/run/run-app.html#customplayground)。 
</div>
...
2. 在弹出的界面中，按照 [uni-app 教程\|_blank](https://uniapp.dcloud.net.cn/tutorial/run/run-app.html#customplayground)，填写相关信息，并单击“打包”进行云打包。
...
1. 导入 JS 封装层。
    ...
    在插件市场的 [ZegoExpressEngine 音视频插件（JS）\|_blank](https://ext.dcloud.net.cn/plugin?id=7748) 界面，单击右侧的“使用 HBuilderX 导入插件”。
