...
收到 Token 即将过期回调或者收到 Token 过期错误码后，开发者需要从自己的服务端获取新的有效 Token，并调用 SDK 提供的 [renewToken](@renewToken) 接口更新 Token。若未处理，则：
  - 已登录的用户不会被踢出房间。
