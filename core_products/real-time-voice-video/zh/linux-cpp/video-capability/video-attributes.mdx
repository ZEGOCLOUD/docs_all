...
推流前调用 [setVideoConfig\|_blank](/zh/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#set-video-config) 接口设置视频分辨率、帧率和码率，可以使用 ZEGO Express SDK 提供的预设组合值，也可以自行设置 ZegoVideoConfig 对应字段。
...
推流前调用 [setVideoMirrorMode\|_blank](/zh/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#set-video-mirror-mode) 接口设置本地预览视频和推送的视频是否开启镜像模式。若开启则渲染出来的画面，物体运动时方向和照镜子时一致，若关闭镜像则相反。
