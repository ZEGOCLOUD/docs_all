...
SDK 只有在 [onStart ](@onStart-IZegoCustomVideoCaptureHandler) 触发后、[onStop ](@onStop-IZegoCustomVideoCaptureHandler) 触发前，才会接受用户传递的视频数据，调用发送自定义采集的视频帧数据接口向 SDK 发送视频帧数据，自定义采集的视频帧数据接口与 [1 开启自定义视频采集](!VideoAdvanced/CustomVideoCapture#3_1) 中向 SDK 提供视频帧数据类型 [bufferType ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~CPP_linux~struct~zego-express-zego-custom-video-capture-config#buffer-type) 一一对应：
