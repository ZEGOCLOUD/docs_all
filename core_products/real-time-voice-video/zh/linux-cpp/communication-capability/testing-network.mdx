...
测速结果将在 [onNetworkSpeedTestQualityUpdate](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-event-handler#on-network-speed-test-quality-update) 中的网络测速质量 [ZegoNetworkSpeedTestQuality](/article/api?doc=Express_Video_SDK_API~CPP_linux~struct~zego-express-zego-network-speed-test-quality) 返回，通过分析各参数值，可得知当前网络质量是否良好。
