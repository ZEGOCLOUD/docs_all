...
直接通过 CDN 的 URL 地址拉流，需要使用 [ZegoCDNConfig](/zh/api?doc=Express_Video_SDK_API~cpp_linux~struct~ZegoCDNConfig) 对象来填入 URL 参数，当从 CDN 拉流时存在拉流鉴权时，还需要通过 `authParam` 字段填入鉴权参数。  

<div class = 'mk-hint'>

* 鉴权参数即 URL 的问号“?” 之后的字符串（无需填写“?”），例如从 CDN 拉流的 URL 为 `rtmp://xxxx.yyy.zzz?a=qqq&b=www` 时，则鉴权参数为 `a=qqq&b=www` 部分。  
* 拉流 URL 的鉴权参数主要用于防盗链，具体鉴权规则开发者可以与具体 CDN 厂商或 ZEGO 技术人员沟通。若无鉴权参数，请忽略 `authParam` 字段。
</div>
...
通过调用 [startPlayingStream](/zh/api?doc=Express_Video_SDK_API~cpp_linux~class~IZegoExpressEngine#start-playing-stream-1) 接口开始拉流。

拉流时，如果出现错误，请参考 [拉流错误码](#4383)。
...
若要停止拉流，调用 [stopPlayingStream](/zh/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#stop-playing-stream) 接口。
...
可以通过 [onPlayerStateUpdate](/zh/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-event-handler#on-player-state-update) 来监听从 CDN 拉流的结果。

<div class = 'mk-warning'>

如果不是使用 ZEGO SDK 进行推流，而是使用第三方推流工具直接进行推流、但是使用 ZEGO SDK 进行拉流，这种场景下推流方没有使用 ZEGO SDK 加入房间，拉流方默认收不到 [onRoomStreamUpdate](@onRoomStreamUpdate) 的回调，可以使用 [增加房间流](#19561) 与 [删除房间流](#19563) 的功能，让拉流端可以收到 [onRoomStreamUpdate ](@onRoomStreamUpdate) 的回调。
</div>
