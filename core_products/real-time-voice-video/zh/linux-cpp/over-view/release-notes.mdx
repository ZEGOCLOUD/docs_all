...
相关 API 请参考 [getAudioDeviceList](@getAudioDeviceList), [useAudioDevice](@useAudioDevice)
...
相关 API 请参考 [addMediaFilePath](@addMediaFilePath)
...
媒体播放器在播放本地文件之外，还支持播放在线资源并将媒体播放器中的流推到远端 ，详情请参考 [媒体播放器](!Other_Functions/MediaPlayer)。
...
相关 API 请参考 [enableCustomAudioIO](@enableCustomAudioIO), [sendCustomAudioCaptureAACData](@sendCustomAudioCaptureAACData), [sendCustomAudioCapturePCMData](@sendCustomAudioCapturePCMData), [fetchCustomAudioRenderPCMData](@fetchCustomAudioRenderPCMData)
...
相关 API 请参考 [sendBroadcastMessage](@sendBroadcastMessage), [sendBarrageMessage](@sendBarrageMessage), [sendCustomCommand](@sendCustomCommand)
