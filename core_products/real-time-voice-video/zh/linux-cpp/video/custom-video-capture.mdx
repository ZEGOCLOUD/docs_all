...
请参考桌面端跨 C++ 平台的 [示例源码文档](/real-time-video-windows-cpp/quick-start/run-example-code) 获取源码。
...
- 已在项目中集成 ZEGO Express SDK，并实现了基本的音视频推拉流功能，详情请参考 [快速开始 - 集成](/real-time-video-linux-cpp/quick-start/integrating-sdk) 和 [快速开始 - 实现流程](/real-time-video-linux-cpp/quick-start/implementing-video-call)。
...
SDK 只有在 [onStart ](@onStart-IZegoCustomVideoCaptureHandler) 触发后、[onStop ](@onStop-IZegoCustomVideoCaptureHandler) 触发前，才会接受用户传递的视频数据，调用发送自定义采集的视频帧数据接口向 SDK 发送视频帧数据，自定义采集的视频帧数据接口与 [1 开启自定义视频采集](/real-time-video-linux-cpp/video/custom-video-capture) 中向 SDK 提供视频帧数据类型 [bufferType ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~CPP_linux~struct~zego-express-zego-custom-video-capture-config#buffer-type) 一一对应：
