...
1. SDK 可以通过 [getScreenCaptureSources](@getScreenCaptureSources) 获取当前可共享的所有窗口信息。
...
2. 通过上述窗口信息里的窗口 ID 与窗口类型，调用 [createScreenCaptureSource](@createScreenCaptureSource) 接口创建屏幕共享源对象。
    ```cpp
    if (infoList.size() > 0) {
...
- **是否激活窗口**    
    若需要采集的窗口被其他窗口挡住，可以调用 [enableWindowActivate](@enableWindowActivate) 接口，将需要采集的窗口放到最前面，以激活当前共享的窗口。
    ```cpp
