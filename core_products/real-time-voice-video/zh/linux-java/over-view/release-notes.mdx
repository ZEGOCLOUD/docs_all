...
#### 1. 修改了媒体推流器 ZegoMediaDataPublisher 类的一些 API 名称

在 3.8.0 版本，对媒体推流器 [ZegoMediaDataPublisher] 类的 API 命名进行变更：将原来的成员函数 [setMediaDataPublisherEventHandler] 更名为 [setEventHandler]，将原来的抽象类 [IMediaDataPublisherEventHandler] 更名为 [IZegoMediaDataPublisherEventHandler]。

API 名称修改后可能存在兼容性问题，详情请参考 [3.8.0 及以上版本升级指南\|_blank](!upgrade_guide5)。
...
#### 3. 新增 Linux Java 示例代码

官网提供 <a href="https://doc-zh.zego.im/article/14512" target="_blank" rel="noopener noreferrer">Linux Java 示例代码</a>，帮助开发这快速接入 SDK。
...
#### 2. 修复媒体数据推流器 (ZegoMediaDataPublisher) 功能异常的问题
...
相关 API 请参考 [createMediaPlayer\|_blank](@createMediaPlayer), [destroyMediaPlayer\|_blank](@destroyMediaPlayer)

#### 2. 新增音效播放器（AudioPlayer）模块

音效是指为了增强真实感或者烘托场景氛围播放的简短效果音，例如：在直播期间播放掌声、礼物音效、提示音等；在游戏中播放子弹声、碰撞打击声。
