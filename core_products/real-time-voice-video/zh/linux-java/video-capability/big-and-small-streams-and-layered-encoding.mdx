...
在推流端开启了分层视频编码后，拉流端用户在拉流前后均可调用 [setPlayStreamVideoType\|_blank](@setPlayStreamVideoType) 接口，传入具体的拉流参数以拉取特定的视频分层。如果没有指定，拉流端默认会根据网络情况拉取合适的视频分层，例如弱网只拉取基本层。目前支持的视频分层如下：
...
在推流（[startPublishingStream\|_blank](@startPublishingStream)）前，调用 [setVideoConfig\|_blank](@setVideoConfig-ZegoExpressEngine) 接口设置 [ZegoVideoConfig\|_blank](@-ZegoVideoConfig) 类中的参数 `codecID` 为 `ZegoVideoCodecIDH264DualStream`，即可开启大小流视频编码。
