...
- 若您集成的是 2.18.0 之前版本的 ZEGO Express SDK（使用 AppSign 鉴权），现在想升级到 2.18.0 版本且使用 Token 鉴权，可以通过 [如何从 AppSign 鉴权升级为 Token 鉴权\|_blank](https://doc-zh.zego.im/faq/token_upgrade) 文档了解 AppSign 鉴权和 Token 鉴权更多信息。
...
若您接入的是 **2.18.0** 及以上版本的 ZEGO Express SDK，在 Token 过期后，未调用 [renewToken\|_blank](@renewToken) 接口更新 Token，则当权限过期时：
  - 已登录的用户不会被踢出房间。
