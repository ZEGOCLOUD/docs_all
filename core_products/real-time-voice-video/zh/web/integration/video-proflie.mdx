...
创建并初始化一个 [ZegoExpressEngine ](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine) 的实例， [ZegoOptions](@-ZegoOptions) 参数中的 `scenario` 字段需要指定一个场景（[ZegoScenario](@-ZegoScenario)），您可以根据实际音视频业务场景进行选择。
...
- 若您需要有多种音视频业务场景，例如，同时有 1v1 音视频通话场景和秀场直播场景。您可以在不创建新的引擎实例的前提下，在退出房间后，通过调用 [setRoomScenario](@setRoomScenario) 接口切换场景，然后再登录其他房间。
</div>
...
- [setRoomScenario](@setRoomScenario) 修改场景成功后，并不会对修改场景前 [createZegoStream](@createZegoStream) 已创建的流有影响，只会对于修改场景后再通过 [createZegoStream](@createZegoStream) 创建的流产生影响。
...
- 若先进行场景化配置，再通过 [createZegoStream](@createZegoStream) 进行相关音视频的精细化配置，则新的参数配置，会覆盖场景化设置中的音视频相关配置。
