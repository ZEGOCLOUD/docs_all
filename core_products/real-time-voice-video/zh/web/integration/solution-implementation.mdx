...
您可以调用 [checkSystemRequirements\|_blank](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#check-system-requirements) 接口检测浏览器的兼容性，检测结果的含义，请参考 [ZegoCapabilityDetection\|_blank](@-ZegoCapabilityDetection) 接口下的参数描述。
...
登录房间需要用于验证身份的 Token，开发者可直接在 [ZEGO 控制台](https://console.zego.im/dashboard)获取临时 Token（有效期为 24 小时）来使用，详情请参考 [控制台 - 项目管理\|_blank](#12107) 中的 “项目信息”。
...
开始推流前需要创建本端的音视频流，调用 [createZegoStream\|_blank](@createZegoStream) 接口获取 [ZegoLocalStream\|_balank](@-ZegoLocalStream) 实例对象 localStream，默认会采集摄像头画面和麦克风声音。

通过 localStream 的 [playVideo\|_blank](@playVideo)、[playAudio\|_blank](@playAudio) 接口，创建本地媒体流播放组件，播放待推送或者已经成功推流的音视频。

2. 需等待 [createZegoStream\|_blank](@createZegoStream) 接口返回 [ZegoLocalStream](@-ZegoLocalStream) 实例对象 localStream 后，再将自己的音视频流推送到 ZEGO 音视频云。
...
房间内有其他用户加入时，SDK 会触发 [roomStreamUpdate\|_blank](@roomStreamUpdate) 回调，通知房间内有流新增，基于此可获取其他用户的 “streamID”。此时，调用 [startPlayingStream\|_blank](@startPlayingStream) 接口根据传入的其他用户的 “streamID”，拉取远端已推送到 ZEGO 服务器的音视频画面。若需要从 CDN 拉流，可参考 [使用 CDN 直播\|_blank](#1218)。

<div class="mk-warning">

推荐用 [createRemoteStreamView\|_blank](@createRemoteStreamView) 播放媒体流，不建议使用 audio或 video 标签播放媒体流。  
</div>
