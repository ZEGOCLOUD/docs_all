...
您可以监听 [roomStateChanged](@roomStateChanged) 回调实时监控自己与房间的连接状态。只有当房间状态是连接成功时，才能进行推流、拉流等操作。如果登录房间失败，可参考 [错误码](!Error_Code/Error_Code) 进行操作。
...
开始推流前需要创建本端的音视频流，调用 [createZegoStream](@createZegoStream) 接口获取 [ZegoLocalStream\|_balank](@-ZegoLocalStream) 实例对象 localStream，默认会采集摄像头画面和麦克风声音。
...
调用 [startPublishingStream](@startPublishingStream) 接口，传入 “streamID” 和创建流得到的流对象 “localStream”，向远端用户发送本端的音视频流。
...
可根据需要通过 [createZegoStream](@createZegoStream) 接口中的如下属性设置音视频相关采集参数，详情可参考 [自定义视频采集](#3679)：
...
房间内有其他用户加入时，SDK 会触发 [roomStreamUpdate](@roomStreamUpdate) 回调，通知房间内有流新增，基于此可获取其他用户的 “streamID”。此时，调用 [startPlayingStream](@startPlayingStream) 接口根据传入的其他用户的 “streamID”，拉取远端已推送到 ZEGO 服务器的音视频画面。若需要从 CDN 拉流，可参考 [使用 CDN 直播](#1218)。
