...
- 调用 [enableAiDenoise](@enableAiDenoise) 接口前，需要先调用 [createZegoStream](@createZegoStream) 接口，创建 [ZegoLocalStream](@-ZegoLocalStream) 实例对象 localStream。
...
1. 调用 [createZegoStream](@createZegoStream) 接口，创建 [ZegoLocalStream](@-ZegoLocalStream) 实例对象 localStream。
...
2. （可选）开发者可通过调用 [setAiDenoiseMode](@setAiDenoiseMode) 接口，设置降噪模式。
...
3. 开发者可通过调用 [enableAiDenoise](@enableAiDenoise) 接口，传入需要进行 AI 降噪的媒体流，开启或关闭 AI 降噪功能，完成 AI 降噪的相关设置。
