...
调用 [createZegoStream](@createZegoStream) 接口，创建 [ZegoLocalStream](@-ZegoLocalStream) 实例对象 localStream，通过 [camera](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoStreamOptions#camera) 视频数据采集方式配置 3A 开关。
