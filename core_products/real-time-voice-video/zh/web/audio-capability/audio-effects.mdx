...
1. 调用 [createZegoStream\|_blank](@createZegoStream) 接口，创建 [ZegoLocalStream\|_blank](@-ZegoLocalStream) 媒体流。
...
2. 设置变声效果。调用 [setVoiceChangerPreset \|_blank](@setVoiceChangerPreset) 接口，传入需要进行变声的媒体流，开始变声处理，并完成变声处理的相关设置。
...
若 SDK 预置的变声效果无法满足需求，开发者可以调用 [setVoiceChangerParam\|_blank](@setVoiceChangerParam) 中 "voiceParam" 参数值设置自定义变声。该参数取值范围为 [-12.0, 12.0]，值越大声音越尖锐，默认值为 “0.0”（即无变声）。
...
调用 [createZegoStream\|_blank](@createZegoStream) 接口，创建 [ZegoLocalStream\|_blank](@-ZegoLocalStream) 媒体流，示例代码请参考 [变声](18146#3_1) 的第 1 步。
...
如果需要开启虚拟立体声功能，必须在创建 [ZegoLocalStream\|_blank](@-ZegoLocalStream) 媒体流时设置双声道，即调用 [createZegoStream \|_blank](@createZegoStream) 方法时，设置 ZegoStreamOptions.camera.audio.channelCount 为 2（默认为单声道）。
