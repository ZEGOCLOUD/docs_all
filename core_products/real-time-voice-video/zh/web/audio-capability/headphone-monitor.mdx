...
调用 [createZegoStream](@createZegoStream) 接口，创建 [ZegoLocalStream](@-ZegoLocalStream) 实例对象 localStream，用于预览本地媒体流。
...
开启预览或开始推流后，可调用 localStream 的 [playAudio](@playAudio) 接口，开始播放音频，即开启耳返功能，主播方讲话后，会听到自己的声音。
...
开启预览或开始推流后，可调用 localStream 的 [setVolume](@setVolume) 接口，调整耳返的音量。
...
调用 [createZegoStream ](@createZegoStream) 接口创建流，并通过 `camera` 对象下 `audio` 的 `channelCount` 参数设置声道数，其中 channelCount 取值：
