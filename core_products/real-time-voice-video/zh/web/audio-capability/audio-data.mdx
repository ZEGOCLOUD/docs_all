...
开发者可调用 [setCaptureAudioFrameCallback ](@setCaptureAudioFrameCallback) 接口开启原始音频数据回调监测，回调参数 [ZegoAudioFrame](@-ZegoAudioFrame)  包含了音频原始数据（channels），声道数（channelCount）和音频采样率（sampleRate）。当回调函数设置为 `null` 时会停止监听此条音频数据。
...
开发者可调用 [setAudioFrameCallback ](@setAudioFrameCallback) 接口开启原始音频数据回调监测，回调参数 [ZegoAudioFrame](@-ZegoAudioFrame) 包含了音频原始数据（channels），声道数（channelCount）和音频采样率（sampleRate）。当回调函数设置为 `null` 时会停止监听此条音频数据。
