...
- - -
<Warning title="注意">
单流转码功能归属于服务端混流服务，统一按服务端混流标准进行计费。
</Warning>
...
- 需使用 [L3 拉流](/real-time-video-web/live-streaming/low-latency-live-streaming)、[转推 CDN 拉流（通过 streamID 拉流）](/real-time-video-web/live-streaming/using-cdn-for-live-streaming) 或 RTC 拉流，且未使用混流能力。
...
1. 添加转码模板。
2. 创建引擎并登录房间。
3. 按需拉取转码流，根据需要选择拉流方式为 [L3 拉流](/real-time-video-web/live-streaming/low-latency-live-streaming)、[转推 CDN 拉流（通过 streamID 拉流）](/real-time-video-web/live-streaming/using-cdn-for-live-streaming) 或 RTC，并传入转码模板 ID 触发转码。
...
具体流程请参考“实现视频通话”文档的 [创建引擎](/real-time-video-web/quick-start/implementing-video-call) 和 [登录房间](/real-time-video-web/quick-start/implementing-video-call) 章节。
...
配置 [ZegoWebPlayOption > codecTemplateID](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#codec-template-id) 属性，填写 [步骤 1 添加转码配置模板](/real-time-video-web/live-streaming/single-stream-transcoding) 中获取的转码模板 ID，并开始拉流。
...
配置 [ZegoWebPlayOption > transcodeOnDecodeFail](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#transcode-on-decode-fail) 属性为 true 开启降级转码功能。
开启后正常情况直接拉原流， SDK 会在视频解码失败时再使用单流转码为其他视频编码，但拉流检测视频编码可用性会增加重试耗时。
