...
为防止攻击者盗取或伪造您的推流 URL 地址，您可以参考 [CDN 推流鉴权](/real-time-video-web/live-streaming/cdn-stream-publishing-authentication)，提升您推流使用的安全性。
...
请参考 [快速开始 - 实现流程](/real-time-video-web/quick-start/implementing-video-call) 的 “推流”，将音视频流推送到 ZEGO 实时音视频云上。
...
ZEGO 自研 Web 端播放器插件，支持拉取 H.265 和 H.264 格式的 CDN 直播流，快速集成到 Web 应用中，为开发者提供稳定、流畅的 CDN 直播播放能力，详情请参考 [播放器简介](/real-time-video-web/player-plugin/overview)。
...
1. **请参考 [播放器插件 - 接入指引](/real-time-video-web/player-plugin/integration)，集成播放器插件，并完成实例化播放器插件等操作。**
...
- CDN 相关地址默认为 HTTP 格式，若您出于安全考虑，或者想使用 Web 端进行 CDN 观众拉流，则需要配置 HTTPS，详情请参考：[控制台 - 服务配置 - CDN](/console/service-configuration/l3)。
...
自 Express Web SDK 3.3.0 版本起，可通过使用 ZEGO 自研 Web 端 [播放器插件（1.3.0 及以上版本）](/real-time-video-web/player-plugin/release-notes) 播放 CDN 直播流。该播放器插件支持拉取 H.265 和 H.264 格式的 CDN 直播流，可快速集成到 Web 应用中，为开发者提供稳定、流畅的 CDN 直播播放能力，详情请参考 [播放器简介](/real-time-video-web/player-plugin/overview)。
...
[将流转推到 CDN 过程中，连接断开后如何处理？](https://doc-zh.zego.im/faq/cdn-relay-disconnect-handling)
