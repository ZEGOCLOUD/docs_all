...
WebRTC 技术在 Chrome 平台支持度最好，建议开发者使用最新版本的 [Chrome 浏览器](https://www.google.com/intl/zh-CN/chrome/)，下载最新版本的 [Web SDK](#3209)。
</div>
...
部分 Android 设备不支持 H.264 编解码。开发者可以通过 [checkSystemRequirements](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#check-system-requirements) 接口检测，如果当前设备不支持该编码格式时，可以使用 “VP8” 编码。同时会有部分浏览器不支持 WebRTC 的技术，建议开发者使用 Chrome 浏览器。
...
开发者可以通过 ZEGO Express Web SDK 提供了 [checkSystemRequirements](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#check-system-requirements-t-extends-keyof-zego-check-single-type) 接口，检测当前浏览器的兼容性情况，该接口可检测 WebRTC、视频编码、自定义采集、摄像头、麦克风以及屏幕共享的支持情况。
