...
- 推荐使用 56 或以上版本的 Chrome 浏览器，不同浏览器的兼容性说明请参考 [7 浏览器兼容性](/real-time-video-web/communication/range-audio)。
...
sequenceDiagram
    ...
    A->>Z: 创建引擎 <br/> new ZegoExpressEngine
    A->>Z: 创建范围语音实例对象 <br/> createRangeAudioInstance
    A->>Z: 监听范围语音事件回调 <br/> on('microphoneStateUpdate')
    A->>Z: 启用内部 AudioContext 对象 <br/> resumeAudioContext
    A->>Z: 登录房间 <br/> loginRoom
    critical 关键步骤
        A->>Z: 设置收听者的当前位置 <br/> updateSelfPosition
        A->>Z: 添加/更新发声者位置信息 <br/> updateAudioSource
        A->>Z: 开启麦克风、扬声器 <br/> enableMicrophone、enableSpeaker
        Z-->>A: 回调麦克风使用状态 <br/> microphoneStateUpdate
    end
...
- 范围语音的浏览器兼容性请参考 [ZEGO Express Web SDK 支持哪些浏览器](https://doc-zh.zego.im/faq/web-sdk-browser-support_1) 。
- 3D 音效的浏览器兼容性如下：
<table>
