...
```mermaid
sequenceDiagram
    participant P as 推流客户端 App
    participant Z as ZEGO Express SDK
    participant L as 拉流客户端 App

    P->>Z: 初始化 SDK <br/> new ZegoExpressEngine

    P->>Z: 登录房间 <br/> loginRoom

    P->>Z: 开始推流 <br/> startPublishingStream

    L->>Z: 初始化 SDK <br/> new ZegoExpressEngine

    L->>Z: 实现相关回调监听 <br/> on('playerRecvSEI')

    L->>Z: 登录房间 <br/> loginRoom

    L->>Z: 开始拉流 <br/> startPlayingStream

    critical 关键步骤
        P->>Z: 发送 SEI 信息 <br/> sendSEI
        Z-->>L: 接收 SEI 信息 <br/> playerRecvSEI
    end
```
{/*
