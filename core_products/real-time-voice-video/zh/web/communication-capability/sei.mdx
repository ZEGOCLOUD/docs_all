...
<img src="/Pics/Common/ZegoExpressEngine/send_and_recv_sei_web_new.png" />
...
- 推流时，若调用 [enableVideoCaptureDevice\|_blank](@enableVideoCaptureDevice) 接口关闭摄像头采集，将不会发送 SEI 数据。
- 拉流时，如果只拉纯音频，即 [startPlayingStream\|_blank](@startPlayingStream) 的 playOption 参数指定仅拉取音频流，或者调用 [mutePlayStreamVideo\|_blank](@mutePlayStreamVideo) 接口停止拉取视频流时，将无法接收 SEI 信息。
