...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/Common/ZegoExpressEngine/send_and_recv_sei_web_new.png" /></Frame>
...
1. 调用 [on](@on) 监听 [playerRecvSEI](@playerRecvSEI) 回调。
2. 调用 [loginRoom](@loginRoom) 接口登录房间。
3. 调用 [startPlayingStream](@startPlayingStream) 接口拉流。
4. 在拉流成功后，接收到推流端发送的 SEI 信息之后触发 [playerRecvSEI](@playerRecvSEI) 回调。
...
- 推流时，若调用 [enableVideoCaptureDevice](@enableVideoCaptureDevice) 接口关闭摄像头采集，将不会发送 SEI 数据。
- 拉流时，如果只拉纯音频，即 [startPlayingStream](@startPlayingStream) 的 playOption 参数指定仅拉取音频流，或者调用 [mutePlayStreamVideo](@mutePlayStreamVideo) 接口停止拉取视频流时，将无法接收 SEI 信息。
...
- 但当开发者需要使用第三方解码器解码时（如 FFmpeg），会导致无法解出正确的 SEI，此时需要在推流 [startPublishingStream](@startPublishingStream) 时指定发送的 SEI 类型为 UserUnregister 的 SEI（nalu type = 6, payload type = 5），且在推流前和拉流前调用 [setSEIConfig](@setSEIConfig) 接口设置 uuid(UserUnregisterID)来区分是视频编码器自身产生的 SEI 还是业务 SEI。
