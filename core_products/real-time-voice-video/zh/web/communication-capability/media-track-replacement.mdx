...
调用 [createZegoStream](@createZegoStream) 接口，设置 `camera` 属性，创建摄像头麦克风采集源数据，并得到一个 [ZegoLocalStream](@-ZegoLocalStream) 实例对象 localStream。

调用 [startPublishingStream](@startPublishingStream) 接口将本地流推送到远端（ZEGO 服务器），通过 `videoCodec` 参数设置推流视频属性，只能传入 “VP8” 或 “H264”，默认值为 “H264”。
...
调用 [startCaptureCustomAudio](@startCaptureCustomAudio) 接口，替换麦克风为背景音乐；调用 [updatePublishingStream](@updatePublishingStream) 接口，同步更新推流音频，由麦克风替换为背景音乐。
...
调用 [startCaptureScreen](@startCaptureScreen) 接口，替换摄像头为屏幕共享；调用 [updatePublishingStream](@updatePublishingStream) 接口，同步更新推流视频，由摄像头替换为屏幕共享。
