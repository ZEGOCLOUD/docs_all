...
- 范围语音功能模块中推拉流的相关回调（[publisherStateUpdate\|_blank](@publisherStateUpdate) 和 [playerStateUpdate\|_blank](@playerStateUpdate)）不再生效。
- 范围语音功能模块中调用 ZegoExpressEngine 的 [on\|_blank](@on) 接口注册了 [roomStreamUpdate\|_blank](@roomStreamUpdate) 和 [streamExtraInfoUpdate\|_blank](@streamExtraInfoUpdate) 事件回调后，您接入范围语音功能时请勿调用 ZegoExpressEngine 的 [off\|_blank](@off) 接口把所有事件回调注销了。
...
如果需要注册回调，开发者可根据实际需要，实现 [ZegoRangeAudioEvent\|_blank](@-ZegoRangeAudioEvent) 中的某些方法，通过范围语音实例 rangeAudio 可调用 [on](@on-ZegoExpressRangeAudio) 接口设置回调。
...
通过 [isAudioContextRunning\|_blank](@isAudioContextRunning) 接口判断浏览器是否支持自动播放声音，如果 `isSupport` 为 `false`，可以引导用户点击界面的 DOM 元素来触发点击事件，并在点击事件中调用 [resumeAudioContext\|_blank](@resumeAudioContext) 接口来启动自动播放。
...
登录房间需要用于验证身份的 Token，获取方式请参考 [用户权限控制\|_blank](#7646)。如需快速调试，可使用控制台生成临时 Token。

**2. 登录房间**  

通过 ZegoExpressEngine 实例 zg 调用 [loginRoom \|_blank](@loginRoom) 接口，传入房间 ID 参数 `roomID`、`token` 和用户参数 `user`，根据实际情况传入参数 `config`，登录房间。
...
调用 [updateSelfPosition\|_blank](@updateSelfPosition) 接口可以添加自身方位，或者在自身方位发生变化时更新自己在世界坐标系中的位置和朝向。
...
登录房间成功后调用 [updateAudioSource\|_blank](@updateAudioSource) 接口来更新发声者位置。
...
调用 [setAudioReceiveRange\|_blank](@setAudioReceiveRange) 接口设置听者接收音频距离的最大范围，即以自身为起点，3D 空间中以设置的距离为立体空间。设置该范围后，在开启 3D 音效的情况下，声音将会随距离的增加而衰减，直至超出所设置的范围，则不再有声音。小队内的语音，不会受到该值的限制，也不会有 3D 音效。
...
- 调用 [enableMicrophone\|_blank](@enableMicrophone) 接口设置是否开启麦克风，当 `enable` 取值为 `true` 时表示开启，取值为 `false` 时表示关闭。（可随时开启或关闭）
    
    开发者可以通过监听 [microphoneStateUpdate\|_blank](@microphoneStateUpdate) 事件回调，获取麦克风更新后的状态。

- 调用 [enableSpeaker\|_blank](@enableSpeaker) 接口设置是否开启扬声器，`enable` 取值为 `true` 时表示开始拉取和播放音频流，为 “false” 时表示停止拉取和播放音频流。（可随时开启或关闭）
...
调用 [setRangeAudioMode\|_blank](@setRangeAudioMode) 接口设置范围语音模式，`mode` 参数取值为 `0` 时表示可以听到所有人的声音，取值为 `1` 时表示只能听到同一小队内其他成员的声音。
...
- 范围语音的浏览器兼容性请参考 [ZEGO Express Web SDK 支持哪些浏览器\|_blank](https://doc-zh.zego.im/faq/browser_support) 。
- 3D 音效的浏览器兼容性如下：
<table>
  <colgroup>
    <col>
