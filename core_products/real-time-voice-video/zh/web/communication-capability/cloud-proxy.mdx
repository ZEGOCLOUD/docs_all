...
此接口必须在创建 [ZegoExpressEngine\|_blank](@-ZegoExpressEngine) 引擎实例前，**通过类静态方法的方式调用**。
</div>
...
- 确保在创建 [ZegoExpressEngine\|_blank](@-ZegoExpressEngine) 引擎实例前，正确设置云代理，不支持中途修改。

### 是否提供本地代理方案？

提供，详情请参考 [本地代理\|_blank](#19011)。
