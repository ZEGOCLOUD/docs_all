...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/Common/ZegoExpressEngine/Network_speed_test_web.png" /></Frame>
...
1. 创建 [ZegoExpressEngine](@-ZegoExpressEngine) 实例对象 zg。
2. 调用 [loginRoom](@loginRoom) 接口登录用于测试网络的房间。
3. 调用 [createZegoStream](@createZegoStream) 接口，在 `quality` 参数中设置期望网络满足的视频质量参数和码率，创建摄像头相关的 [ZegoLocalStream](@-ZegoLocalStream) 实例对象。
4. 调用 [startPublishingStream](@startPublishingStream) 接口进行推流，通过监听 [publishQualityUpdate](@publishQualityUpdate) 事件获取上行网络质量 `stats.video.videoQuality`。
5. 调用 [startPlayingStream](@startPlayingStream) 接口进行拉流，通过监听 [playQualityUpdate](@playQualityUpdate) 事件获取下行网络质量 `stats.video.videoQuality`。
...
通过如上步骤，获取到上行平均网络质量和下行平均网络质量。网络质量 [videoQuality](@videoQuality) 的枚举值含义如下：
