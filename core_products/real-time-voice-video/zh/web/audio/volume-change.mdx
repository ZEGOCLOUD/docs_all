...
1. 调用 [startMixerTask](@startMixerTask) 时，需进行混流配置 [ZegoMixStreamConfig](@-ZegoMixStreamConfig)，需要将 `enableSoundLevel` 配置为 true，并为 [inputList](@inputList) 配置`soundLevelID`，详情请参考 [混流](/real-time-video-web/live-streaming/stream-mixing)。
    ```js
    const mixConfig = {
        "taskID": "custom-task",
...
2. 调用 [mixerSoundLevelUpdate](@mixerSoundLevelUpdate) 接口监听混流音量变化。
    ...
    ```
3. 根据混流 streamID 拉流，若需要获取混流音量值，需要开启 SEI，详情请参考 [媒体补充增强信息（SEI）](/real-time-video-web/communication/sei)。
