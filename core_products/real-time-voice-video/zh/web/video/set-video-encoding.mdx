...
在推流（[startPublishingStream](@startPublishingStream)）前，调用 [enableDualStream](@enableDualStream) 接口，开启或关闭视频小流编码功能。
...
可根据需要，在推流（[startPublishingStream](@startPublishingStream)）和 调用 [enableDualStream](@enableDualStream)开启小流前，调用 [setLowStreamParameter](@setLowStreamParameter) 接口设置小流分辨率、帧率及宽高。
...
在推流端开启了视频大小流编码后，拉流端在拉流调用 [startPlayingStream](@startPlayingStream) 接口时，可在可选拉流参数 `streamType` 中，选择指定的视频分层类型，此时拉流端默认会根据网络情况，拉取合适的视频分层，例如，弱网只拉取基本层。
