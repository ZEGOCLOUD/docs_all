...
1. 点击下载 [ZEGO 共享插件](https://storage.zego.im/downloads/jZego-screen-extention.zip)，并解压。
...
通过调用 [on](/zh/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#on) 接口监听停止屏幕共享的 [screenSharingEnded ](@screenSharingEnded) 回调。
...
调用 [createZegoStream](@createZegoStream) 方法创建屏幕共享流。

[createZegoStream ](@createZegoStream) 接口参数 “source” 包含的对象 “screen”，可以对屏幕捕捉采集流进行相关配置。
...
调用 [startPublishingStream\| _blank](/zh/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#start-publishing-stream) 方法将 [createZegoStream ](@createZegoStream) 创建的屏幕共享流推送到 ZEGO 实时音视频云。当同时推多路流时，建议只推 1 路音频，防止出现回音。
...
当屏幕共享流推送到 ZEGO 实时音视频云后，其他用户可以使用 [startPlayingStream](@startPlayingStream) 接口拉取屏幕共享流。

### 6 销毁屏幕共享流

#### 6.1 停止推流

若要销毁屏幕共享流，需要先调用 [stopPublishingStream ](/zh/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#stop-publishing-stream) 方法停止推流，否则对端画面会卡住。
...
停止推流后，需要及时调用 [destroyStream ](/zh/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#destroy-stream) 方法销毁屏幕共享流数据，释放被占用的资源。
