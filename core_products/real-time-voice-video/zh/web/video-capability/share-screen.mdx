...
1. 点击下载 [ZEGO 共享插件\|_blank](https://storage.zego.im/downloads/jZego-screen-extention.zip)，并解压。
...
<details class="zg-primary">
    <summary>监听事件回调</summary>

通过调用 [on\|_blank](/zh/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#on) 接口监听停止屏幕共享的 [screenSharingEnded \|_blank](@screenSharingEnded) 回调。
...
调用 [createZegoStream\|_blank](@createZegoStream) 方法创建屏幕共享流。

[createZegoStream \|_blank](@createZegoStream) 接口参数 “source” 包含的对象 “screen”，可以对屏幕捕捉采集流进行相关配置。
...
<details class="zg-primary">
    <summary>Google Chrome 浏览器</summary>


在 Google Chrome 浏览器中，“audio” 设置为 “true” 后，还需要在浏览器的弹窗中勾选“分享音频”，才能将声音推送给对端用户。开发者在选择不同的共享内容时，浏览器弹窗中的“分享音频”选项是否展示，请参考如下：
![/Pics/Web/ExpressSDK/Common/web_screenSharing_audioSharing.png](/Pics/Web/ExpressSDK/Common/web_screenSharing_audioSharing.png)
...
<details class="zg-primary">
    <summary>Microsoft Edge 浏览器</summary>


在 Microsoft Edge 浏览器中，“audio” 设置为 “true” 后，还需要在浏览器的弹窗中勾选“共享音频”，才能将声音推送给对端用户。开发者在选择不同的共享内容时，浏览器弹窗中的“共享音频”选项是否展示，请参考如下：
![/Pics/Web/ExpressSDK/Common/web_screenSharing_audioSharing.png](http://doc.oa.zego.im/Pics/Web/ExpressSDK/Common/web_screenSharing_audioSharing_Edge.png)
...
![/Pics/Web/ExpressSDK/Common/web_screenSharing_audioSharing_Firefox.png](http://doc.oa.zego.im/Pics/Web/ExpressSDK/Common/web_screenSharing_audioSharing_Firefox.png)
</details>
...
调用 [startPublishingStream\| _blank](/zh/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#start-publishing-stream) 方法将 [createZegoStream \|_blank](@createZegoStream) 创建的屏幕共享流推送到 ZEGO 实时音视频云。当同时推多路流时，建议只推 1 路音频，防止出现回音。
...
当屏幕共享流推送到 ZEGO 实时音视频云后，其他用户可以使用 [startPlayingStream\|_blank](@startPlayingStream) 接口拉取屏幕共享流。

### 6 销毁屏幕共享流

#### 6.1 停止推流

若要销毁屏幕共享流，需要先调用 [stopPublishingStream \|_blank](/zh/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#stop-publishing-stream) 方法停止推流，否则对端画面会卡住。
...
停止推流后，需要及时调用 [destroyStream \|_blank](/zh/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#destroy-stream) 方法销毁屏幕共享流数据，释放被占用的资源。
...
- [如何切换屏幕共享流和摄像头视频流？\|_blank](https://doc-zh.zego.im/faq/web_switch_screen_camera)
- [如何解决 Web 平台屏幕共享的质量问题？\|_blank](https://doc-zh.zego.im/faq/web_screen_share)
