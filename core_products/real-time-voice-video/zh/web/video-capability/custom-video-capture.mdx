...
1. 在调用 [createZegoStream](@createZegoStream) 接口创建 [ZegoLocalStream](@-ZegoLocalStream) 流实例对象时，通过设置 “source” 参数来指定第三方数据源。
2. 调用 [startPublishingStream](@startPublishingStream) 推流。
