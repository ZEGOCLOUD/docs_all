...
### 推流
主播端推流后混流的前置条件为推流，请参考 [快速开始 - 实现流程](!Integration/Solution_Implementation#publishingStream) 的 “推流”。

### 设置混流配置

#### 设置混流输入流

根据实际业务场景，调用 [ZegoMixStreamConfig](@-ZegoMixStreamConfig) 中 [inputList](@inputList) 的定义输入的视频流列表，列表中是 [ZegoMixStreamInput](@-ZegoMixStreamInput) 对象，表示为 “streamID” 的流。根据所需的输入流个数构建 N 个 [ZegoMixStreamInput](@-ZegoMixStreamInput) 放入输入流列表。
...
[ZegoMixStreamInput](@-ZegoMixStreamInput) 的使用示例：
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/iOS/ZegoLiveRoom/ZegoLiveRoom-MixStream/mixStreamInfo.png" /></Frame>
...
在混流参数 “outputList” 中，输出流的 [target](@-ZegoMixStreamOutput) 可为流名 “streamID” 或 “URL”。如果需要将流推到 CDN 上，采用 “URL”；不需要将流推到 CDN 上时，由调用者决定采用 “URL” 或者 “streamID”。
...
如果需要背景图片 [ZegoMixerImageInfo](@-ZegoMixerImageInfo) 的 URL，请联系 ZEGO 技术支持获取, 或者可以用网络图片，但只支持 HTTP 协议的。
...
可通过设置 [enableSoundLevel](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoMixStreamConfig#enable-sound-level) 参数选择是否开启混流的声浪回调通知，开启后（取值为 “True”）用户拉混流时可通过 [mixerSoundLevelUpdate](@mixerSoundLevelUpdate) 回调收到每条单流的音量变化（声浪）信息。
...
停止混流是通过调用 [stopMixerTask](@stopMixerTask) 实现。
...
通过 [channel](@-ZegoMixerAudioConfig) 参数可以修改音频声道，目前支持如下音频声道:
...
可通过设置 [enableSoundLevel](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoAutoMixerTask#enable-sound-level) 参数选择是否开启自动混流的声浪回调通知，开启后（取值为 “True”）用户拉混流时可通过 [AutoMixerSoundLevelUpdate](@AutoMixerSoundLevelUpdate) 回调收到每条单流的音量变化（声浪）信息。
