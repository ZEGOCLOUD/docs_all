...
### 1 推流
主播端推流后混流的前置条件为推流，请参考 [快速开始 - 实现流程\|_blank](!Integration/Solution_Implementation#publishingStream) 的 “推流”。

### 2 设置混流配置

#### 2.1 设置混流输入流

根据实际业务场景，调用 [ZegoMixStreamConfig\|_blank](@-ZegoMixStreamConfig) 中 [inputList\|_blank](@inputList) 的定义输入的视频流列表，列表中是 [ZegoMixStreamInput\|_blank](@-ZegoMixStreamInput) 对象，表示为 “streamID” 的流。根据所需的输入流个数构建 N 个 [ZegoMixStreamInput\|_blank](@-ZegoMixStreamInput) 放入输入流列表。 
...
[ZegoMixStreamInput\|_blank](@-ZegoMixStreamInput) 的使用示例：
...
在混流参数 “outputList” 中，输出流的 [target\|_blank](@-ZegoMixStreamOutput) 可为流名 “streamID” 或 “URL”。如果需要将流推到 CDN 上，采用 “URL”；不需要将流推到 CDN 上时，由调用者决定采用 “URL” 或者 “streamID”。
...
如果需要背景图片 [ZegoMixerImageInfo\|_blank](@-ZegoMixerImageInfo) 的 URL，请联系 ZEGO 技术支持获取, 或者可以用网络图片，但只支持 HTTP 协议的。
...
可通过设置 [enableSoundLevel\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoMixStreamConfig#enable-sound-level) 参数选择是否开启混流的声浪回调通知，开启后（取值为 “True”）用户拉混流时可通过 [mixerSoundLevelUpdate\|_blank](@mixerSoundLevelUpdate) 回调收到每条单流的音量变化（声浪）信息。
...
当混流信息发生变更时，例如混流的输入流列表发生增减、调整混流视频输出码率等，修改该混流任务对象的参数，然后再调用一次 [startMixerTask|_blank](/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#start-mixer-task) 即可更新 ZEGO 混流服务器上的混流配置。
...
通过 [channel\|_blank](@-ZegoMixerAudioConfig) 参数可以修改音频声道，目前支持如下音频声道:
...
可通过设置 [enableSoundLevel\|_blank](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoAutoMixerTask#enable-sound-level) 参数选择是否开启自动混流的声浪回调通知，开启后（取值为 “True”）用户拉混流时可通过 [AutoMixerSoundLevelUpdate\|_blank](@AutoMixerSoundLevelUpdate) 回调收到每条单流的音量变化（声浪）信息。
