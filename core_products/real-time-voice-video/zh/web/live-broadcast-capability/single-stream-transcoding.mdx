...
- 需使用 [L3 拉流\|_blank](!ExpressVideoSDK-Publisher_Player_Advanced/Low_Latency_Live)、[转推 CDN 拉流（通过 streamID 拉流）\|_blank](!ExpressVideoSDK-Publisher_Player_Advanced/RelayToCDN) 或 RTC 拉流，且未使用混流能力。
...
1. 添加转码模板。
2. 创建引擎并登录房间。
3. 按需拉取转码流，根据需要选择拉流方式为 [L3 拉流\|_blank](!ExpressVideoSDK-Publisher_Player_Advanced/Low_Latency_Live)、[转推 CDN 拉流（通过 streamID 拉流）\|_blank](!ExpressVideoSDK-Publisher_Player_Advanced/RelayToCDN) 或 RTC，并传入转码模板 ID 触发转码。
...
具体流程请参考“实现视频通话”文档的 [创建引擎\|_blank](!Integration/Solution_Implementation#createEngine) 和 [登录房间\|_blank](!Integration/Solution_Implementation#loginRoom) 章节。
...
配置 [ZegoWebPlayOption > codecTemplateID](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#codec-template-id) 属性，填写 [步骤 1 添加转码配置模板](!Single_stream_transcoding#step1) 中获取的转码模板 ID，并开始拉流。
