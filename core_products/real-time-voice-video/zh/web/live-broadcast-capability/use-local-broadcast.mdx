...
在 [下载](!DownloadSDK/DownloadSDK) 页面获取最新的包，解压后的文件中包含了本地导播插件包。找到您项目目录下的 “index.html” 文件，添加如下代码，手动集成本地导播插件包。
...
创建并初始化一个 [ZegoExpressEngine](@-ZegoExpressEngine) 实例，然后调用 [createStreamCompositor](@createStreamCompositor) 接口，创建本地导播插件处理实例。
...
调用 [createZegoStream](@createZegoStream) 接口，传入不同的参数，分别创建摄像头、屏幕共享的本地数据流。
...
3. 调用 [setTransparentBackgroundOptions](@setTransparentBackgroundOptions) 接口，设置摄像头流采集的带人像画面背景的透明效果；再调用 [enableBackgroundProcess](@enableBackgroundProcess) 接口，开启背景处理。
...
调用 [setInputEndpoint](@setInputEndpoint) 接口，设置输入流的布局效果、渲染模式、层级等。
...
调用 [addImage](@addImage) 接口，设置图片图层的位置。
...
- 图片加载需要时间，您需要在图片加载完成（`onload` 事件）后，再调用 [addImage](@addImage) 接口。
</div>

### 7 设置输出参数和布局

调用 [setOutputConfig](@setOutputConfig) 接口，设置输出流的宽、高、帧率等。
...
调用 [startComposingStream](@startComposingStream) 接口，开始混合媒体流。
...
调用 [stopComposingStream](@stopComposingStream) 接口，停止混合媒体流；再调用 [destroyStream](@destroyStream) 销毁流数据，释放资源。
