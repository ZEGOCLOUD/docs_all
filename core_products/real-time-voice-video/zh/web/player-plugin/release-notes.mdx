...
1. 新增配置项 [keepScreenOn](/real-time-video-web/client-sdk/api-reference/interface#keepscreenon) 设置在移动端播放视频时，保持播放状态不自动息屏。

**改进优化**

1. 优化拉流的首帧耗时，不等待缓冲数据，拉取到数据后直接解码和渲染首帧画面。


**问题修复**

1. 修复跨域引用播放器代码出现自动播放失败时，不会触发 [onError](@onError) 回调的问题。

2. 修复播放器获取 [muted](@muted) 属性值为 `undefined` 的问题。

3. 修复用 MSE 的解码方式进入页面时，无交互状态下自动播放音视频会导致后续播放异常的问题。

4. 修复播放器插件初始化后，先调 [stop](/real-time-video-web/client-sdk/api-reference/class#stop-zegoexpressplayer-class) 停止播放资源，再调用 [play](/real-time-video-web/client-sdk/api-reference/class#play-zegoexpressplayer-class) 播放资源偶现播放失败的问题。
