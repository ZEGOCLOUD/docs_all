...
ZEGO 播放器插件，由深圳市即构科技有限公司提供，您可以在本页面获取适用于 Web 平台的播放器插件资源，当前可下载版本为 1.5.1，发布日志请参考 [播放器插件 - 发布日志](/real-time-video-web/player-plugin/release-notes)，合规事宜请参考 [ZEGO 安全合规白皮书](/policies-and-agreements/zego-security-and-compliance-white-paper)。

<CardGroup cols={2}>
<Card title="播放器插件 1.5.1" href="https://webrepo.zego.im/webplatform/ZegoExpressPlayer.zip">
本地下载
</Card>
...
3. 在 [本页面](/real-time-video-web/player-plugin/integration) 开头获取最新的播放器插件包。解压后的目录中所有文件都需要用到，需要在服务器上放在同一个目录下，并且不能与页面地址跨域部署。
...
- 如果您没有使用 Express Web SDK 的 [loginRoom](@loginRoom) 接口登录房间，此处必须对播放器插件进行鉴权，Token 的获取方式请参考 [使用 Token 鉴权](/real-time-video-web/communication/using-token-authentication)。
