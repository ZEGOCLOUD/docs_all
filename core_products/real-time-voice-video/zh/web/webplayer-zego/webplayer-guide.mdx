...
ZEGO 播放器插件，由深圳市即构科技有限公司提供，您可以在本页面获取适用于 Web 平台的播放器插件资源，当前可下载版本为 1.3.1，发布日志请参考 [播放器插件 - 发布日志](!webplayer_changeLog)，合规事宜请参考 [ZEGO 安全合规白皮书](https://doc-zh.zego.im/article/16142)。
...
- 已生成推流转推的 CDN 地址，详情请参考 [使用 CDN 直播](https://doc-zh.zego.im/article/1218)。
...
2. 在 [下载](https://doc-zh.zego.im/article/3209) 页面获取最新的 Express SDK 的 JS 集成包（**必须是 3.0.0 或以上版本**）；
