...
ZEGO 播放器插件，由深圳市即构科技有限公司提供，您可以在本页面获取适用于 Web 平台的播放器插件资源，当前可下载版本为 1.3.1，发布日志请参考 [播放器插件 - 发布日志\|_blank](!webplayer_changeLog)，合规事宜请参考 [ZEGO 安全合规白皮书\|_blank](#16142)。

<div class="download-area">
  <div class="sdk-download-item-wrapper">
    <div class="sdk-fun">Beta</div>
    <div class="download-title">播放器插件 1.3.1</div>
    <div class="download-selector">
      <div class="download-select-wrapper" style="padding-right: 0;">
        ...
        <ul class="download-dropdown-list">
          <li data-href="https://storage.zego.im/webplatform/ZegoExpressPlayer.zip"
  ...
  <div class="sdk-download-item-wrapper">
    ...
    <div class="download-selector">
      <div class="download-select-wrapper" style="padding-right: 0;">
        <a class="download-target-btn" href="javascript:void(0);">前往体验</a>
        <ul class="download-dropdown-list">
          <li data-href="https://zegoim.github.io/express-demo-web/src/Examples/AdvancedStreaming/StreamByCDN/index.html"
            class="dropdown-option">
            前往体验
          </li>
        </ul>
...
- 已生成推流转推的 CDN 地址，详情请参考 [使用 CDN 直播\|_blank](#1218)。
...
2. 在 [下载\|_blank](#3209) 页面获取最新的 Express SDK 的 JS 集成包（**必须是 3.0.0 或以上版本**）；
3. 在 [本页面](!webplayer_guide#1) 开头获取最新的播放器插件包。解压后的目录中所有文件都需要用到，需要在服务器上放在同一个目录下，并且不能与页面地址跨域部署。
...
- 如果您已通过 Express Web SDK 的 [loginRoom\|_blank](@loginRoom) 接口成功登录房间，此处无需再对播放器插件进行鉴权。
- 如果您没有使用 Express Web SDK 的 [loginRoom\|_blank](@loginRoom) 接口登录房间，此处必须对播放器插件进行鉴权，Token 的获取方式请参考 [使用 Token 鉴权\|_blank](!Integration/User_Access_Control)。
</div>
...
播放器插件还支持：暂停或中断请求资源、全屏显示、拉流时 SEI 回调、媒体信息回调等功能。更多功能说明，请参考 [ZegoExpressPlayer\|_blank](@-ZegoExpressPlayer) 的 API 接口文档说明。
...
<details class="zg-primary">
    <summary>集成播放器插件的完整示例代码</summary>
