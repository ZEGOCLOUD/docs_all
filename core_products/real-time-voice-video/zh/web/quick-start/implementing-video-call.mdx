...
2. 用户 B 创建实例，登录同一个房间。登录成功后，用户 B 开始推流，此时 SDK 会触发 [roomStreamUpdate ](@roomStreamUpdate) 回调，表示房间内有流的变化。
3. 用户 A 可通过监听 [roomStreamUpdate ](@roomStreamUpdate) 回调，当回调通知有流新增时，获取用户 B 的流 ID，来拉取播放用户 B 刚刚推送的流。
...
sequenceDiagram
    ...
    participant 用户B
    用户A->>ZEGO Express SDK: 1.用户A创建引擎 <br/> new ZegoExpressEngine
    用户A->>ZEGO Express SDK: 2.用户A登录房间 <br/> loginRoom
    用户B->>ZEGO Express SDK: 3.用户B创建引擎 <br/> new ZegoExpressEngine
    用户B->>ZEGO Express SDK: 4.用户B登录房间 <br/> loginRoom
    用户B->>ZEGO Express SDK: 5.用户B推流 <br/> startPublishingStream
    ZEGO Express SDK->>用户A: 6.用户A收到roomStreamUpdate回调 <br/> (获取 用户B 的streamID)
    用户A->>ZEGO Express SDK: 7.用户A传入用户B的streamID拉流 <br/> startPlayStream
```
{/*
...
1. 创建并初始化一个 [ZegoExpressEngine ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine) 的实例，将您项目的 `AppID` 传入参数 “appID”，`Server 地址` 传入参数 “server”。

<Warning title="注意">
- 若使用 3.7.0 及以上版本 SDK，`server` 参数可填写控制台获取的`Server 地址`或者直接填空字符串。
...
您可以调用 [checkSystemRequirements](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#check-system-requirements) 接口检测浏览器的兼容性，检测结果的含义，请参考 [ZegoCapabilityDetection](@-ZegoCapabilityDetection) 接口下的参数描述。
...
您还可以通过 ZEGO 提供的 [在线检测工具](https://zegoim.github.io/express-demo-web/src/Examples/Others/WebrtcCheck/index.html?lang=zh)，在需要检测的浏览器中打开，直接检测浏览器的兼容性。请参考 [浏览器兼容性和已知问题](/real-time-video-web/introduction/browser-restrictions) 获取 SDK 支持的浏览器兼容版本。
...
您可以监听 [roomStateChanged](@roomStateChanged) 回调实时监控自己与房间的连接状态。只有当房间状态是连接成功时，才能进行推流、拉流等操作。如果登录房间失败，可参考 [错误码](/real-time-video-web/client-sdk/error-code) 进行操作。
...
开始推流前需要创建本端的音视频流，调用 [createZegoStream](@createZegoStream) 接口获取 [ZegoLocalStream](@-ZegoLocalStream) 实例对象 localStream，默认会采集摄像头画面和麦克风声音。

通过 localStream 的 [playVideo](@playVideo)、[playAudio](@playAudio) 接口，创建本地媒体流播放组件，播放待推送或者已经成功推流的音视频。

2. 需等待 [createZegoStream](@createZegoStream) 接口返回 [ZegoLocalStream](@-ZegoLocalStream) 实例对象 localStream 后，再将自己的音视频流推送到 ZEGO 音视频云。

调用 [startPublishingStream](@startPublishingStream) 接口，传入 “streamID” 和创建流得到的流对象 “localStream”，向远端用户发送本端的音视频流。您可通过监听 [PublisherStateUpdateCallBack](@PublisherStateUpdateCallBack) 回调知晓推流是否成功。
...
可根据需要通过 [createZegoStream](@createZegoStream) 接口中的如下属性设置音视频相关采集参数，详情可参考 [自定义视频采集](/real-time-video-web/video/custom-video-capture)：
...
房间内有其他用户加入时，SDK 会触发 [roomStreamUpdate](@roomStreamUpdate) 回调，通知房间内有流新增，基于此可获取其他用户的 “streamID”。此时，调用 [startPlayingStream](@startPlayingStream) 接口根据传入的其他用户的 “streamID”，拉取远端已推送到 ZEGO 服务器的音视频画面。您可通过监听 [PlayerStateUpdateCallBack](@PlayerStateUpdateCallBack) 回调知晓是否成功拉取音视频。若需要从 CDN 拉流，可参考 [使用 CDN 直播](/real-time-video-web/live-streaming/using-cdn-for-live-streaming)。

<Warning title="注意">


推荐用 [createRemoteStreamView](@createRemoteStreamView) 播放媒体流，不建议使用 audio或 video 标签播放媒体流。
...
- 部分浏览器因自动播放限制策略问题，使用 [ZegoStreamView](@-ZegoStreamView) 媒体流播放组件进行播放媒体流可能受阻，SDK 默认会在界面上弹窗提示恢复播放。
- 您可以将 ZegoStreamView.play() 方法的第二个参数 options.enableAutoplayDialog 设置为 false 关闭自动弹窗，通过在 [autoplayFailed](@autoplayFailed) 事件回调中，在页面上显示一个按钮，引导用户点击恢复播放。
...
调用 [stopPublishingStream](@stopPublishingStream) 接口停止向远端用户发送本端的音视频流。调用 [destroyStream](@destroyStream) 接口销毁创建的流数据。
...
如果开发者通过 [roomStreamUpdate](@roomStreamUpdate) 回调收到了音视频流 “减少” 的通知，请及时调用 [stopPlayingStream ](@stopPlayingStream) 接口停止拉流，避免拉到空流、产生额外的成本；或者，开发者可以根据自己的业务需求，选择合适的时机，主动调用 [stopPlayingStream ](@stopPlayingStream) 接口停止拉流。
...
sequenceDiagram
    participant App
    participant ZEGO Express SDK
    App->>ZEGO Express SDK: 创建引擎 <br/> new ZegoExpressEngine
    App->>ZEGO Express SDK: 登录房间 <br/> loginRoom
    ZEGO Express SDK-->>App: 房间/用户/流状态更新回调 <br/> roomStateChanged/roomUserUpdate/roomStreamUpdate
    App->>ZEGO Express SDK: 创建流（采集） <br/> createZegoStream
    App->>ZEGO Express SDK: 推流 <br/> startPublishingStream
    ZEGO Express SDK-->>App: 推流状态更新/质量回调 <br/> publisherStateUpdate/publishQualityUpdate
    App->>ZEGO Express SDK: 拉流 <br/> startPlayingStream
    ZEGO Express SDK-->>App: 拉流状态更新/质量回调 <br/> playerStateUpdate/playQualityUpdate
    App->>ZEGO Express SDK: 停止推流 <br/> stopPublishingStream
    ZEGO Express SDK-->>App: 推流状态更新回调 <br/> publisherStateUpdate
    App->>ZEGO Express SDK: 销毁流（停止采集） <br/> destroyStream
    ...
    ZEGO Express SDK-->>App: 房间状态更新回调 <br/> roomStateChanged
...
## 相关文档
- [错误码文档](/real-time-video-web/client-sdk/error-code)
- [如何设置和获取 SDK 的日志、堆栈信息？](https://doc-zh.zego.im/faq/express-logs-stacktrace)
- [移动端如何使用 ZEGO Express Web SDK？](https://doc-zh.zego.im/faq/mobile-web-sdk-usage)
- [如何处理 Web 平台直播过程中出现黑屏或无声？](https://doc-zh.zego.im/faq/web-live-failure)
