...
- 已在 [ZEGO 控制台\|_blank](https://console.zego.im) 创建项目，并申请有效的 AppID 和 Server 地址，详情请参考 [控制台 - 项目管理\|_blank](#12107)。
- 已在项目中集成 ZEGO Express SDK（2.9.0 版本及以上），并实现了基本的音视频推拉流功能，详情请参考 [快速开始 - 集成\|_blank](!ExpressVideoSDK-Integration/SDK_Integration) 和 [快速开始 - 实现流程\|_blank](!ExpressVideoSDK-Integration/Solution_Implementation)。
...
![/Pics/Common/MultiRoom/MultiRoom_Web_new.png](//doc.oa.zego.im/Pics/Common/MultiRoom/MultiRoom_Web_new.png)
...
必须在初始化 SDK 之后，登录房间前调用，调用 [enableMultiRoom\|_blank](@enableMultiRoom) 设置为true开启多房间
...
通过 [loginRoom\|_blank](@loginRoom) 接口成功登录第一个房间后，再次调用 [loginRoom\|_blank](@loginRoom) 接口登录第二个房间。
...
登录第一个房间后，也可以通过 [switchRoom\|_blank](@switchRoom) 接口切换到另一个房间。
```javascript
// 先登录第一个房间
zg.loginRoom('first', token, {userID: 'zegoUser'});
...
传入房间 ID 等参数，调用 [startPublishingStream\|_blank](@startPublishingStream) 接口在指定房间内推流。
...
传入的流 ID，调用 [startPlayingStream\|_blank](@startPlayingStream)  接口即可进行拉流。
...
调用 [sendBroadcastMessage\|_blank](@sendBroadcastMessage) 接口发送广播消息，房间其他用户通过 [IMRecvBroadcastMessage\|_blank](@IMRecvBroadcastMessage)
...
调用 [sendBarrageMessage\|_blank](@sendBarrageMessage) 接口发送弹幕消息，房间其他用户通过 [IMRecvBarrageMessage\|_blank](@IMRecvBarrageMessage)
 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
调用 [sendCustomCommand\|_blank](@sendCustomCommand) 接口发送自定义信令，房间其他用户通过 [IMRecvCustomCommand\|_blank](@IMRecvCustomCommand)
 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
调用 [setRoomExtraInfo\|_blank](@setRoomExtraInfo) 接口发送房间附加信息，房间其他用户通过 [roomExtraInfoUpdate\|_blank](@roomExtraInfoUpdate)
 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
发送实时消息的大小和频率限制可参考 [限制说明\|_blank](!ExpressVideoSDK-OverView/Restrictions)。
</div>

### 7 退出房间

传入房间 ID ，调用 [logoutRoom\|_blank](@logoutRoom) 接口退出房间。
...
#### 1. 调用 enableMultiRoom 接口返回 false 失败，是什么原因？

- 是否在初始化 SDK 之后 登录房间前调用 enableMultiRoom 接口。
- 是否重复调用，一次完整的生命周期中仅允许该接口调用 1 次
- 是否传入正确的参数。

#### 2. 调用 loginRoom 登录失败，错误码为 “1100001016”。是什么原因？

在没有开通多房间功能的情况下设置了多房间模式，登录房间会失败，返回错误码 1100001016。

#### 3. 调用 loginRoom 登录失败，错误码为 “1000005022”。是什么原因？
