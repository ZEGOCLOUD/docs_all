...
- 已在项目中集成 ZEGO Express SDK（2.9.0 版本及以上），并实现了基本的音视频推拉流功能，详情请参考 [快速开始 - 集成](/real-time-video-web/quick-start/integrating-sdk) 和 [快速开始 - 实现流程](/real-time-video-web/quick-start/implementing-video-call)。
...
请参考 [快速开始 - 实现流程](/real-time-video-web/quick-start/implementing-video-call) 的 “创建引擎”  进行 SDK 的初始化。

### 2 设置多房间模式

必须在初始化 SDK 之后，登录房间前调用，调用 [enableMultiRoom](@enableMultiRoom) 设置为true开启多房间
...
登录第一个房间后，也可以通过 [switchRoom](@switchRoom) 接口切换到另一个房间。
```javascript
// 先登录第一个房间
zg.loginRoom('first', token, {userID: 'zegoUser'});
...
传入房间 ID 等参数，调用 [startPublishingStream](@startPublishingStream) 接口在指定房间内推流。
...
传入的流 ID，调用 [startPlayingStream](@startPlayingStream)  接口即可进行拉流。
...
调用 [sendBroadcastMessage](@sendBroadcastMessage) 接口发送广播消息，房间其他用户通过 [IMRecvBroadcastMessage](@IMRecvBroadcastMessage)
...
调用 [sendBarrageMessage](@sendBarrageMessage) 接口发送弹幕消息，房间其他用户通过 [IMRecvBarrageMessage](@IMRecvBarrageMessage)
 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
调用 [sendCustomCommand](@sendCustomCommand) 接口发送自定义信令，房间其他用户通过 [IMRecvCustomCommand](@IMRecvCustomCommand)
 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
调用 [setRoomExtraInfo](@setRoomExtraInfo) 接口发送房间附加信息，房间其他用户通过 [roomExtraInfoUpdate](@roomExtraInfoUpdate)
 回调接收消息，可通过房间 ID 来区分是哪个房间发送的。
...
发送实时消息的大小和频率限制可参考 [限制说明](/real-time-video-web/best-practice/restrictions)。
