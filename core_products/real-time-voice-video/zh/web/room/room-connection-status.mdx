...
如果因为网络质量不佳导致房间连接中断，SDK 内部会自动进行重试，详情请参考 [房间断线重连](/real-time-video-web/room/room-connection-status) 章节。
...
因网络质量不佳而导致的房间连接中断，SDK 内部会自动进行重试，详情请参考 [房间断线重连](/real-time-video-web/room/room-connection-status) 章节。

#### 房间状态说明

```mermaid
stateDiagram-v2
    state "Error 状态 <br/> 未连接状态" as unconnected {
        [*] --> Logout
        Logout: Logout（初始状态）
    }
    state "连接状态" as connected {
        Logined
        Reconnected
    }
    Logout --> Logining: ① loginRoom 登录房间 <br/> ② switchRoom 切换房间时，登录目标房间
    Logining --> Logined: ① loginRoom 登录房间成功 <br/> ② switchRoom 切换房间成功
    Logining --> LoginFailed: ① loginRoom 登录房间失败 <br/> ② switchRoom 切换房间失败
    LoginFailed --> Logout
    Logined --> Logout: ① logoutRoom 登出房间 <br/> ② switchRoom 切换房间时，登出当前房间
    Logined --> LogoutFailed: ① logoutRoom 登出房间失败 <br/> ② switchRoom 切换房间时，登出当前房间失败
    LogoutFailed --> Logout
    Logined --> Reconnecting: 网络质量不佳
    Reconnecting --> Reconnected: 重连成功
    Reconnecting --> ReconnectFailed: 重连失败
    ReconnectFailed --> Logout
    Logined --> KickOut: 被服务器踢出房间
    KickOut --> Logout
    Reconnected --> Logout: ① logoutRoom 登出房间 <br/> ② switchRoom 切换房间时，登出当前房间
    Reconnected --> LogoutFailed: ① logoutRoom 登出房间失败 <br/> ② switchRoom 切换房间时，登出当前房间失败
    Reconnected --> Reconnecting: 网络质量不佳
    Reconnected --> KickOut: 被服务器踢出房间
```
{/*
...
如果用户 A 短暂断线，但是在 100s 内重连成功了，那么用户 B 不会收到 [roomUserUpdate](@roomUserUpdate) 的回调通知。
...
- T5 = T3 + time（小于 100s）：用户 A 在重连时间内恢复网络，重连成功。会收到 [roomStateChanged](@roomStateChanged) 回调以通知客户端用户 A 重连成功。
...
- T0、T1、T2、T3 时刻的情况同 [情况一：在 ZEGO 服务端判断用户 A 心跳超时前重连成功](/real-time-video-web/room/room-connection-status) 中的 T0、T1、T2、T3。
- T4' ≈ T3 + 100s：用户 B 此时收到 [roomUserUpdate](@roomUserUpdate) 回调以通知 用户 A 已断线。
- T5' = T3 + time（大于 100s，小于 5 min）：用户 A 在重连时间内恢复网络，重连成功。此时客户端会收到 [roomStateChanged](@roomStateChanged) 回调以通知客户端用户 A 重连成功。
...
[Web 平台上，Express SDK 是否支持断线重连机制？](/faq/web-reconnect)
