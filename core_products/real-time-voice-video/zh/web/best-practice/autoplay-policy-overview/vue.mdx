...
登录房间需要用于验证身份的 Token，获取方式请参考 [使用 Token 鉴权](https://doc-zh.zego.im/article/7646)。如需快速调试，可使用控制台生成临时 Token。
...
调用 [destroyStream|_blank](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#destroy-stream) 接口销毁创建的流数据，销毁流后开发需自行销毁 video（停止采集）。
