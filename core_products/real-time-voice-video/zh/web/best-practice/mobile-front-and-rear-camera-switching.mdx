...
开发者创建流成功后，可以通过 [useVideoDevice\|_blank](@useVideoDevice) 接口切换采集摄像头。但在 H5 上直接使用时，部分机型可能产生一些不符合预期的问题，例如黑屏、切换摄像头失败等。
...
1. 调用 [createZegoStream\|_blank](@createZegoStream) 接口，创建后置摄像头采集流 zegoLocalStream，用 zegoLocalStream 推流。
2. 调用 [useFrontCamera\|_blank](@useFrontCamera) 接口，切换为前置或后置摄像头。
...
1. 调用 [createZegoStream\|_blank](@createZegoStream) 接口，创建前置摄像头采集流 zegoLocalStream，用 zegoLocalStream 推流。

2. 调用 [stopCaptureVideo\|_blank](@stopCaptureVideo) 接口，停止当前摄像头采集。

3. 调用 [startCaptureCamera\|_blank](@startCaptureCamera) 接口，切换为后置摄像头，重新开始采集。

4. 调用 [updatePublishingStream\|_blank](@updatePublishingStream) 接口，更新到推流中。
...
1. 调用 [createStream\|_blank](@createStream) 接口，创建前置摄像头采集流 localStream，用 localStream 推流。
...
2. 切换为后置摄像头。
    ...
    2. 调用 [createStream\|_blank](@createStream) 接口，创建后置摄像头采集流。

    3. 调用 [replaceTrack\|_blank](@replaceTrack) 接口，将 localStream 的视轨替换成后置摄像头视轨即可。
