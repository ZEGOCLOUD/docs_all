...
- [Unity\|_blank](https://unity.cn/tuanjie/tuanjieyinqing) 2018.4.21f1 或以上版本（建议通过 Unity Hub 下载最新的 LTS 版本）。
- 确保运行设备已经连接到 Internet。

<div class="mk-hint">

本文说的 Unity 是指 [团结引擎\|_blank](https://unity.cn/tuanjie/tuanjieyinqing)，目前只有 Unity 中国团队的团结引擎支持 HarmonyOS 平台。
</div>
...
1. [下载 ZEGO Express Unity3D SDK\|_blank](!DownloadSDK/DownloadSDK)，并解压 ZegoExpressUnity3D.zip 文件。
...
#### 1. 在 HarmonyOS 平台可以使用 WebCamTexture 吗？
 暂不支持

#### 2. HarmonyOS 平台如何使用媒体播放器模块？
...
#### 3. 真机调试时，出现“code:9568322 error: signature verification failed due to not trusted app source”报错，该如何处理？

签名问题，每次使用新的测试机器，都需要 [重新确认签名配置\|_blank](https://developer.huawei.com/consumer/cn/doc/harmonyos-faqs-V5/faqs-app-debugging-9-V5)。

#### 4. 真机调试时，出现“TypeError：Cannot read property Zego_TsApi_InitNapiFunc of undefined”报错，该如何处理？

请确保 [导入 SDK 步骤](21309#2) 执行正确。并确保导出的工程中，“entry/libs/” 目录下，包含 “libZegoExpressEngine.so” 文件；“entry/src/main/ets/” 目录下，包含 “zegoets” 以及相关文件。
