...
- [Unity\|_blank](https://unity.com/download) 2018.4.21f1 或以上版本（建议通过 Unity Hub 下载最新的 LTS 版本）。安装 Unity 时，请根据需要运行的平台，勾选对应的 "Plaforms" 模块一起下载。
...
<img src="/Pics/Unity3D/ZegoExpressEngine/QuickStart/int-create.png">
...
1. 请参考 [下载 SDK 包\|_blank](!Integration/SDK_Integration)，下载最新版本的 SDK，下载完成后进行解压。
...
3. 对于不同的运行平台，需要做一些额外处理。
    ...
    - **Windows**
        ...
        <img src="/Pics/Unity3D/ZegoExpressEngine/multiple_dll_error.png">
        ...
        <img src="/Pics/Unity3D/ZegoExpressEngine/windows_all_archs.png">

        <img src="/Pics/Unity3D/ZegoExpressEngine/windows_one_arch.png">
    ...
    - **macOS**
        ...
        <img src="/Pics/Unity3D/ZegoExpressEngine/macos_dylib.png">

    - **iOS**

        导出 Xcode 工程后，直接运行可能会报如下错误：

        <img src="/Pics/Unity3D/ZegoExpressEngine/ios_multiple_archs_error.png">

        iOS 平台的 XCFramework 中有三个文件夹：

        <img src="/Pics/Unity3D/ZegoExpressEngine/QuickStart/ios_xcframework.png">
        ...
        - "ios-arm64_×86_64-maccatalyst" 是 iOS MacCatalyst 架构，目前 Unity 暂不支持此架构，**请直接删除**，详情请参考 [Unity 官网说明\|_blank](https://forum.unity.com/threads/mac-catalyst.1183951/)。

    - **Android**

        <img src="/Pics/Unity3D/ZegoExpressEngine/android_win_dll_error.png">
...
<img src="/Pics/Unity3D/ZegoExpressEngine/unity_set_ios_privacy.png">
...
2. **导出 iOS Xcode 工程后还需要集成 ZegoExpressEngine.framework 吗？**
    ...
    <img src="/Pics/Unity3D/ZegoExpressEngine/ios_add_embedded_framework.png" width="100%">
...
3. **为什么集成 SDK 后，在 Unity Editor 调试过程中创建 SDK 经常失败？**
    ...
    ![/Pics/Unity3D/ZegoExpressEngine/Unity_Editor.png](http://doc.oa.zego.im/Pics/Unity3D/ZegoExpressEngine/Unity_Editor.png)

    如果按图中勾选了对应设置，Unity Editor 在停止调试后一段时间会缓存静态变量，导致 SDK 内部静态变量没有重置而出现异常，表现为首次 [CreateEngine\|_blank](@CreateEngine) 成功，Unity Player 停止运行后再启动 [CreateEngine\|_blank](@CreateEngine) 失败，详情请参考 [域重新加载\|_blank](https://docs.unity3d.com/cn/current/Manual/DomainReloading.html)。
