...
打开了声浪和频谱监控之后，SDK 将周期性的通过相关回调（[OnCapturedSoundLevelUpdate\|_blank](@OnCapturedSoundLevelUpdate)、[OnCapturedSoundLevelInfoUpdate\|_blank](@OnCapturedSoundLevelInfoUpdate)、[OnRemoteSoundLevelUpdate\|_blank](@OnRemoteSoundLevelUpdate)、[OnRemoteSoundLevelInfoUpdate\|_blank](@OnRemoteSoundLevelInfoUpdate)、[OnCapturedAudioSpectrumUpdate\|_blank](@OnCapturedAudioSpectrumUpdate)、[OnRemoteAudioSpectrumUpdate\|_blank](@OnRemoteAudioSpectrumUpdate)）以通知用户当前的声浪和频谱数据，用户只需重写相关回调函数实现 UI 展示即可。
...
- [怎么处理无声问题？\|_blank](https://doc-zh.zego.im/faq/noaudio?product=all&platform=unity3d)
- [怎么处理音量太小问题？\|_blank](https://doc-zh.zego.im/faq/audio_low?product=all&platform=unity3d)
