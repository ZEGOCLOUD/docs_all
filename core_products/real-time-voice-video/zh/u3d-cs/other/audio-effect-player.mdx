...
调用  [LoadResource](@LoadResource-ZegoAudioEffectPlayer) 方法加载音效资源，可通过 “callback” 参数来监听加载的结果，显示加载成功后方可播放。
...
当加载的音效使用完成后，可以调用 [UnloadResource](@UnloadResource-ZegoAudioEffectPlayer) 接口卸载，以释放相关资源，否则 SDK 将在 [ZegoAudioEffectPlayer](@-ZegoAudioEffectPlayer) 实例释放时，自动卸载已加载的音效。
...
可以根据需要监听音效播放器的 [OnAudioEffectPlayStateUpdate](@OnAudioEffectPlayStateUpdate) 方法为播放器设置事件回调，用于监听“音效播放状态改变”等通知。
...
调用 [Start](@Start-ZegoAudioEffectPlayer) 方法播放音效，目前仅支持同时播放 12 个，且只能为本地文件，不支持播放网络资源。

* 如果已通过 [LoadResource](@LoadResource-ZegoAudioEffectPlayer) 方法预先加载了音效，则只需要传入预加载时的 “audioEffectID”，“path”（音效资源的路径）字段传空即可。
...
* 若需要重复播放可以通过 [ZegoAudioEffectPlayConfig](@-ZegoAudioEffectPlayConfig) 中 “playCount” 配置重复次数。如果设置为 “0”，则表示无限重复播放，直到用户手动调用 [Stop](@Stop-ZegoAudioEffectPlayer) 停止。
...
- **暂停播放**：调用 [Pause](@Pause-ZegoAudioEffectPlayer) 方法可以暂停播放 “audioEffectID” 指定的音效，调用 [PauseAll](@PauseAll-ZegoAudioEffectPlayer) 方法则暂停所有正在播放的音效。
- **恢复播放**：音效暂停播放后，调用 [Resume](@Resume-ZegoAudioEffectPlayer) 方法可以恢复播放 “audioEffectID” 指定的音效，调用 [ResumeAll](@ResumeAll-ZegoAudioEffectPlayer) 方法则恢复所有已暂停音效。
- **停止播放**：调用 [Stop](@Stop-ZegoAudioEffectPlayer) 方法可以停止播放 “audioEffectID” 指定的音效，调用 [StopAll](@StopAll-ZegoAudioEffectPlayer) 方法则停止播放所有音效。
...
- 调用 [SetVolume](@SetVolume-ZegoAudioEffectPlayer) 方法可以设置 “audioEffectID” 指定的音效音量，取值范围为 [0, 200]，默认值为 “100”。
- 调用 [SetVolumeAll](@SetVolumeAll-ZegoAudioEffectPlayer) 方法则同时设置所有音效音量，取值范围为 [0, 200]，默认值为 “100”。
...
- 调用 [GetTotalDuration](@GetTotalDuration-ZegoAudioEffectPlayer) 方法获取单个音效的总时长。
- 调用 [GetCurrentProgress](@GetCurrentProgress-ZegoAudioEffectPlayer) 方法获取音效当前播放进度。
- 调用 [SeekTo](@SeekTo-ZegoAudioEffectPlayer) 方法可以根据需要设置播放进度。
