...
调用 [LoginScene\|_blank](@LoginScene-ZegoRangeScene) 接口，传入场景参数：sceneID、user、position、broadcastMode，即可登录场景。

<div class="mk-warning">

- 同一个 AppID 内，需保证 userID 全局唯一，建议开发者将其设置成一个有意义的值，可将 userID 与自己业务账号系统进行关联。
- userID 不能为空，否则会导致登录场景失败。
- 请勿在 [LoginScene\|_blank](@LoginScene-ZegoRangeScene) 接口的回调 [OnLoginSceneCallback\|_blank](@OnLoginSceneCallback) 中调用 SDK 接口，否则会出错崩溃（可以切线程调用）。
</div>
...
通过 [UpdateUserStatus\|_blank](@UpdateUserStatus-ZegoRangeScene)、[UpdateUserCommand\|_blank](@UpdateUserCommand-ZegoRangeScene) 接口更新用户状态及用户命令。通过 [OnUserStatusUpdate\|_blank](@OnUserStatusUpdate-IZegoRangeSceneEventHandler)、[OnUserCommandUpdate\|_blank](@OnUserCommandUpdate-IZegoRangeSceneEventHandler) 回调接收 AOI 范围内的远端用户位置、命令等状态信息。
...
可以根据需要调用 [GetUserCount\|_blank](@GetUserCount-ZegoRangeScene)、[GetUserListInView\|_blank](@GetUserListInView-ZegoRangeScene) 接口获取场景内用户数、AOI 范围内用户列表。
...
调用 [LogoutScene\|_blank](@LogoutScene-ZegoRangeScene) 接口退出场景。
