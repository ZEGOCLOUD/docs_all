...
您可以根据需要调用 [SetReceiveRange\|_blank](@SetReceiveRange-ZegoRangeSceneStream) 以及 [EnableRangeSpatializer\|_blank](@EnableRangeSpatializer-ZegoRangeSceneStream) 接口设置范围场景参数，控制当前用户音视频的接收范围、设置 3D 音效距离的衰减范围区间 [min, max]、是否开启 3D 音效。
...
调用 [UpdateUserPosition\|_blank](@UpdateUserPosition-ZegoRangeScene) 接口，更新当前用户的位置。
