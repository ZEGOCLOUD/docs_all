...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/Express/express_AOI.png" /></Frame>
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/Express/rangeScene_u3d.png" /></Frame>
...
您可以根据需要调用 [SetReceiveRange](@SetReceiveRange-ZegoRangeSceneStream) 以及 [EnableRangeSpatializer](@EnableRangeSpatializer-ZegoRangeSceneStream) 接口设置范围场景参数，控制当前用户音视频的接收范围、设置 3D 音效距离的衰减范围区间 [min, max]、是否开启 3D 音效。
...
调用 [LoginScene](@LoginScene-ZegoRangeScene) 接口，传入场景参数：`sceneID`、`user`、`position`、`broadcastMode`，即可登录场景。
...
- 请勿在 [LoginScene](@LoginScene-ZegoRangeScene) 接口的回调 [OnLoginSceneCallback](@OnLoginSceneCallback) 中调用 SDK 接口，否则会出错崩溃（可以切线程调用）。
...
调用 [UpdateUserPosition](@UpdateUserPosition-ZegoRangeScene) 接口，更新当前用户的位置。
...
调用 [LogoutScene](@LogoutScene-ZegoRangeScene) 接口退出场景。
