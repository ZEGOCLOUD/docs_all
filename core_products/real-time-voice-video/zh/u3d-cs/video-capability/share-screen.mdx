...
2. 设置采集源为屏幕共享源。
    ...
    - SDK 推流的“视频源”默认为摄像头，如果需要推屏幕共享源，需要通过 [SetVideoSource\|_blank](@SetVideoSource) 切换为屏幕共享。

    ```C#
    engine.SetVideoSource(ZegoVideoSourceType.ScreenCapture, ZegoPublishChannel.Main);
    ```

    - SDK 推流的“音频源”默认为麦克风，如果需要推屏幕共享源，需要通过 [SetAudioSource\|_blank](@SetAudioSource) 切换为屏幕共享。

    ```C#
    engine.SetAudioSource(ZegoAudioSourceType.ScreenCapture, ZegoPublishChannel.Main);
    ```

3. 调用 [StartScreenCapture\|_blank](@StartScreenCapture) 接口共享整个系统的画面、采集第三方应用的音频，开始屏幕共享。

    ```C#
    engine.StartScreenCapture();
    ```

    开发者还可以使用 [ZegoScreenCaptureConfig\|_blank](@-ZegoScreenCaptureConfig) 参数设置是否采集视频、是否采集音频、设置音频采集时的采样率和采样通道等。
...
4. 调用 [UpdateScreenCaptureConfig\|_blank](@UpdateScreenCaptureConfig) 接口，可以更新屏幕共享的配置。
...
5. 开发者需要监听 [IZegoEventHandler\|_blank](@-IZegoEventHandler) 类中的 [OnMobileScreenCaptureExceptionOccurred\|_blank](@OnMobileScreenCaptureExceptionOccurred) 回调，接收屏幕共享过程中的异常信息通知。
...
iOS 平台是基于苹果的 [Replaykit\|_blank](https://developer.apple.com/documentation/ReplayKit) 框架实现屏幕录制，能够分享整个系统的屏幕内容。但需要当前 App （主 App 进程）额外提供一个 Extension 扩展组件（Extension 进程），用于录制屏幕，再结合 ZEGO Express SDK 相关 API 来实现屏幕共享功能，详情请参考 [屏幕共享（iOS）\|_blank](10460#4) 的 “实现流程” 说明。
...
2. 应用内屏幕共享。若用户只在应用内共享画面与声音，可以调用 [StartScreenCaptureInApp\|_blank](@StartScreenCaptureInApp) 接口开启屏幕共享。也可调用 [broadcastFinished\|_blank](https://developer.apple.com/documentation/replaykit/rpbroadcastsamplehandler/2143169-broadcastfinished/) 接口回调屏幕共享结束通知，若屏幕采集失败可接收到失败的原因。
...
3. 跨应用屏幕共享。若用户需要共享整个系统的画面与声音，可以调用 [StartScreenCapture\|_blank](@StartScreenCapture) 接口开启屏幕共享。
    ...
    1. 新建 Broadcast Upload Extension，详情请参考 [屏幕共享（iOS）\|_blank](10460#5_2_2) 的 “跨应用屏幕共享” 步骤 1。
    2. 设置 Extension，详情请参考 [屏幕共享（iOS）\|_blank](10460#5_2_2) 的 “跨应用屏幕共享” 步骤 2。
    3. 开始屏幕共享，详情请参考 [屏幕共享（iOS）\|_blank](10460#5_2_2) 的 “跨应用屏幕共享” 步骤 3。
    4. 启动 Extension 扩展进程，详情请参考 [屏幕共享（iOS）\|_blank](10460#5_2_2) 的 “跨应用屏幕共享” 步骤 4。
    5. 获取屏幕数据，详情请参考 [屏幕共享（iOS）\|_blank](10460#5_2_2) 的 “跨应用屏幕共享” 步骤 5。
...
2. 获取窗口（包括屏幕）列表信息，您可以通过 [GetScreenCaptureSources\|_blank](@GetScreenCaptureSources) 接口，获取当前可共享的所有窗口信息。

    ```C#
    List<ZegoScreenCaptureSourceInfo> captureSources = engine.GetScreenCaptureSources(400, 400, 100, 100);
    ```

3. 通过上述窗口信息里的窗口 ID 与窗口类型，调用 [CreateScreenCaptureSource\|_blank](@CreateScreenCaptureSource) 接口创建屏幕共享源对象。
...
4. 设置采集源为屏幕共享源。SDK 推流的视频源默认为摄像头源，如果需要推屏幕共享源，需要通过 [SetVideoSource\|_blank](@SetVideoSource) 进行切换为屏幕共享。

    ```C#
    engine.SetVideoSource(ZegoVideoSourceType.ScreenCapture, (uint)captureSource.GetIndex(), ZegoPublishChannel.Main);
    ```

5. 启动屏幕共享，调用 [StartCapture\|_blank](@StartCapture) 接口可以共享窗口画面。

    ```C#
    captureSource.StartCapture();
    ```

6. 调用 [UpdateCaptureSource\|_blank](@UpdateCaptureSource) 接口可以更新共享窗口画面。
...
7. 更新共享源区域。用户可以调用 [UpdateCaptureRegion\|_blank](@UpdateCaptureRegion) 接口动态更新共享窗口区域，其中设置为 (0, 0, 0, 0) 可恢复整个窗口共享。
...
8. 过滤指定窗口。调用 [SetExcludeWindowList\|_blank](@SetExcludeWindowList) 接口可以过滤掉共享的屏幕中指定窗口画面，只适用于共享整个屏幕时进行设置。
...
9. 用户可以调用 [EnableWindowActivate\|_blank](@EnableWindowActivate) 接口来激活当前共享的窗口。

    ```C#
    captureSource.EnableWindowActivate(true);
    ```

10. 调用 [EnableCursorVisible\|_blank](@EnableCursorVisible) 接口进行显示或隐藏鼠标。
...
11. 监听事件回调。
    ...
    通过 [IZegoScreenCaptureSourceEventHandler\|_blank](@-IZegoScreenCaptureSourceEventHandler) 中的方法 [OnAvailableFrame\|_blank](@OnAvailableFrame)、[OnExceptionOccurred\|_blank](@OnExceptionOccurred)、[OnWindowStateChanged\|_blank](@OnWindowStateChanged)、[OnRectChanged\|_blank](@OnRectChanged) 监听事件回调。
...
调用 [LoginRoom\|_blank](@LoginRoom) 接口，传入房间 ID 参数 “roomID” 和用户参数 “user”，登录房间。
...
完成以上步骤之后，其他用户可以使用 [StartPlayingStream\|_blank](@StartPlayingStream) 接口拉取屏幕共享流。
...
调用 [StopScreenCapture\|_blank](@StopScreenCapture) 接口停止共享。
