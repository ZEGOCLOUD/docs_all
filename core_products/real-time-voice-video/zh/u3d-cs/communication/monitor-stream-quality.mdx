...
您可以通过监听 [OnNetworkQuality](@OnNetworkQuality) 回调，收到房间内用户（包括您自己）的上下行网络质量。此回调每隔两秒会收到一次，网络质量等级请参考 [ZegoStreamQualityLevel](@-ZegoStreamQualityLevel)。

<Note title="说明">

上下行网络质量：
- 上行网络质量：表示本端推流（上行）到 ZEGO 服务端的网络质量。
- 下行网络质量：表示本端拉流（下行）时从 ZEGO 服务端接收的网络质量。

回调场景示例：
- 场景一：用户 A 同时推流与拉流，远端用户通过从 ZEGO 服务端拉取 A 的流，就可以获取到用户 A 的上行网络质量与下行网络质量；用户 A 也会回调自身的上下行网络质量。
- 场景二：用户 B 只推流不拉流，因为没有拉流，所以无法算出本端的下行网络质量，因此，远端用户通过从 ZEGO 服务端拉取 B 的流，就可以获取到用户 B 的上行网络质量，下行网络质量未知，因此回调为 unknown（枚举值为 5）。在该场景下，用户 B 也只会回调自身的上行网络质量，下行网络质量回调为 unknown（枚举值为 5）。
- 场景三：用户 C 只拉流不推流，因为没有推流，所以无法将自己的网络情况广播出去，因此，远端用户无法获取用户 C 的上行网络质量与下行网络质量。在该场景下，用户 C 只会回调自身的下行网络质量，上行网络质量回调为 unknown（枚举值为 5）。
...
<tr>
<td>2.22.0 及以上</td>
<td>基于 2.14.0 ～ 2.21.1 的 onNetworkQuality 接口的回调逻辑上，如果远端推流用户心跳丢失 1 次，回调其网络质量为 unknown；如果远端推流用户心跳丢失达到 3 次，回调其网络质量为 die。</td>
</tr>
<tr>
<td>2.14.0 ～ 2.21.1</td>
<td>
...
[OnNetworkQuality](@OnNetworkQuality) 不适用于使用 CDN 进行直播的场景，您可以参考 [进阶质量报告 - 推流质量报告](/real-time-video-u3d-cs/communication/monitor-stream-quality) 监测 CDN 的推流质量。
...
- 大多数情况下，您只需关注 “quality” 的 “level” 参数，以 “level” 枚举值来判断推流的综合质量，详情可参考 [ZegoStreamQualityLevel](@-ZegoStreamQualityLevel)。
...
您可以通过注册 [OnPlayerQualityUpdate](@OnPlayerQualityUpdate) 接收拉流质量回调，拉流成功后每隔三秒会收到此回调。开发者可根据 quality([ZegoPlayStreamQuality](@-ZegoPlayStreamQuality)) 参数实时了解拉取的音视频流的健康情况。
...
在推流成功后，您可以通过 [OnPublisherStateUpdate](@OnPublisherStateUpdate) 获取推流状态变更的通知。
...
- [怎么处理视频卡顿问题？](https://doc-zh.zego.im/faq/video-freeze-issue)
- [怎么处理音频卡顿问题？](https://doc-zh.zego.im/faq/audio-stutter)
