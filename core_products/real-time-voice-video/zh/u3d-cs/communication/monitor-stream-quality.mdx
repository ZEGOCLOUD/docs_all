...
- 场景二：用户 B 只推流不拉流，因为没有拉流，所以无法算出本端的下行网络质量，因此，远端用户通过从 ZEGO 服务端拉取 B 的流，就可以获取到用户 B 的上行网络质量，下行网络质量未知，因此回调为 unknown。在该场景下，用户 B 也只会回调自身的上行网络质量，下行网络质量回调为 unknown。
- 场景三：用户 C 只拉流不推流，因为没有推流，所以无法将自己的网络情况广播出去，因此，远端用户无法获取用户 C 的上行网络质量与下行网络质量。在该场景下，用户 C 只会回调自身的下行网络质量，上行网络质量回调为 unknown。
