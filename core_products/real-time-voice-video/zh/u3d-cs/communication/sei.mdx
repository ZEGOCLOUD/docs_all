...
```mermaid
sequenceDiagram
    participant 推流客户端 App
    participant ZEGO Express SDK
    participant 拉流客户端 App

    推流客户端 App->>ZEGO Express SDK: 初始化 SDK <br/> createEngine
    推流客户端 App->>ZEGO Express SDK: 登录房间 <br/> loginRoom
    推流客户端 App->>ZEGO Express SDK: 开始推流 <br/> startPublishingStream

    拉流客户端 App->>ZEGO Express SDK: 初始化 SDK <br/> createEngine
    拉流客户端 App->>ZEGO Express SDK: 注册相关回调监听 <br/> setEventHandler
    拉流客户端 App->>ZEGO Express SDK: 登录房间 <br/> loginRoom
    拉流客户端 App->>ZEGO Express SDK: 开始拉流 <br/> startPlayingStream

    critical 关键步骤
    推流客户端 App->>ZEGO Express SDK: 发送 SEI 信息 <br/> sendSEI
    ZEGO Express SDK-->>拉流客户端 App: 接收 SEI 信息 <br/> onPlayerRecvSEI
    end
```
{/*
...
由于 SDK 默认使用 ZEGO 自行定义的 SEI（nalu type = 6, payload type = 243）类型打包，且此类型是 SEI 标准未规定的类型，因此跟视频编码器或者视频文件中的 SEI 不存在冲突。但当开发者需要使用第三方解码器解码时（如 FFmpeg），会导致解不出正确的 SEI，此时需要在推流前调用 [setSEIConfig ](@setSEIConfig) 接口更换 SDK 发送 SEI 的类型，使用 UserUnregister 的 SEI（nalu type = 6, payload type = 5）类型打包。
...
1. 调用 [CreateEngine](@CreateEngine) 接口创建 engine 对象。
2. 调用 [LoginRoom](@LoginRoom) 接口登录房间。
3. 调用 [StartPublishingStream](@StartPublishingStream) 接口推流。
4. 在推流成功后，调用 [SendSEI](@SendSEI) 接口发送 SEI 信息。
...
1. 调用 [CreateEngine ](CreateEngine) 接口创建 engine 对象。
2. 实现 engine 对象上接收 SEI 信息的 [OnPlayerSyncRecvSEI](@OnPlayerSyncRecvSEI) 方法。
3. 调用 [LoginRoom](@LoginRoom) 接口登录房间。
4. 调用 [StartPlayingStream](@StartPlayingStream) 接口拉流。
5. 在拉流成功后，接收到推流端发送的 SEI 信息之后触发 [OnPlayerSyncRecvSEI](@OnPlayerSyncRecvSEI) 回调。
