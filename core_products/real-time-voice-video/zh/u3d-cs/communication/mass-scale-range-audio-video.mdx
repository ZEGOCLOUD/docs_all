...
sequenceDiagram
    ...
    App->>SDK: 创建范围场景模块 <br/> CreateRangeScene

    App->>SDK: 监听范围场景事件回调 <br/> IZegoRangeSceneStreamEventHandler

    critical 关键步骤
        App->>SDK: 设置范围场景流管理参数 <br/> SetReceiveRange、EnableRangeSpatializer

        App->>SDK: 登录场景 <br/> LoginScene

        App->>SDK: 推流到场景内 <br/> StartPublishingStreamInScene

        App->>SDK: 更新用户位置 <br/> UpdateUserPosition

        App->>SDK: 退出场景 <br/> LogoutScene
    end

    App->>SDK: 销毁范围场景模块 <br/> DestroyRangeScene

    App->>SDK: 销毁引擎 <br/> DestroyEngine
```
{/*
