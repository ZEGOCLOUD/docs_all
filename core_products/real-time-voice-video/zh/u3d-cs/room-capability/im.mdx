...
1. **发送广播消息**
  ...
  通过 [OnIMSendBroadcastMessageResult \|_blank](@OnIMSendBroadcastMessageResult) 回调获取消息发送结果。
...
1. **发送弹幕消息**
  ...
  通过 [OnIMSendBarrageMessageResult \|_blank](@OnIMSendBarrageMessageResult) 回调获取消息发送结果。
...
2. **接收弹幕消息**
  ...
  实现 [IZegoEventHandler\|_blank](@-IZegoEventHandler) 代理中的 [OnIMRecvBarrageMessage \|_blank](@OnIMRecvBarrageMessage) 回调，当发送方成功发送弹幕消息后，同一房间内的其他用户通过此回调接收相关信息，包括消息内容、消息 ID、发送时间及发送方信息。
...
1. **发送自定义信令**
  ...
  通过 [OnIMSendCustomCommandResult \|_blank](@OnIMSendCustomCommandResult) 回调获取消息发送结果。
...
2. **接收自定义信令**
  ...
  实现 [IZegoEventHandler\|_blank](@-IZegoEventHandler) 代理中的 [OnIMRecvCustomCommand \|_blank](@OnIMRecvCustomCommand) 回调，当发送方成功发送自定义消息后，同一房间内的指定用户通过此回调接收相关信息，包括消息内容和消息发送方信息。
