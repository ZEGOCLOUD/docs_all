...
为防止攻击者盗取或伪造您的推流 URL 地址，您可以参考 [CDN 推流鉴权](/real-time-video-u3d-cs/live-streaming/cdn-stream-publishing-authentication)，提升您推流使用的安全性。
...
若选择使用直推 CDN 功能，则无需执行本节所有步骤，请参考 [直推 CDN](/real-time-video-u3d-cs/live-streaming/using-cdn-for-live-streaming)。
...
请参考 [快速开始 - 实现流程](/real-time-video-u3d-cs/quick-start/implementing-video-call) 的 “创建引擎”、“登录房间”。

### 开始推流

请参考 [快速开始 - 实现流程](/real-time-video-u3d-cs/quick-start/implementing-video-call) 的 “推流”。
...
- 开发者转推到多家第三方 CDN 后，可从 CDN 回调状态通知 [OnPublisherRelayCDNStateUpdate](@OnPublisherRelayCDNStateUpdate) 的列表参数中获取到每条转推流的状态变更通知。**在 WebGL 平台上运行时，不支持该回调状态通知。**
...
开发者可通过设置 [OnPublisherRelayCDNStateUpdate](@OnPublisherRelayCDNStateUpdate) 委托获取添加/删除转推 CDN 地址状态回调。在 ZEGO RTC 服务器将音视频流转推到 CDN 后，如果 CDN 转推状态发生变化，例如出现转推停止或转推重试，将会收到此回调。
...
在 WebGL 平台上运行时，没有转推 CDN 信息 [ZegoStreamRelayCDNInfo](@-ZegoStreamRelayCDNInfo)。
...
- 若选择使用转推 CDN 功能，则无需执行本节所有步骤，请参考 [转推 CDN](/real-time-video-u3d-cs/live-streaming/using-cdn-for-live-streaming)。
...
- 当推流方直推 CDN 时，拉流方可以直接通过 streamID 进行拉流，请参考 [快速开始 - 实现流程](/real-time-video-u3d-cs/quick-start/implementing-video-call) 的 “拉流”。

- 当音视频流转推 CDN 成功后，开发者希望用户从 CDN 进行拉流时，需要使用传入 URL 的自定义拉流方式进行拉流，而不能通过流 ID 进行拉流。URL 拉流的操作步骤可参考 [通过 URL 拉流](/real-time-video-u3d-cs/live-streaming/playing-stream-by-url) 中的 “1 配置拉流参数” 和 “2 开始拉流”。
