...
1. 请参考 [下载 SDK 包](/real-time-video-u3d-cs/quick-start/integrating-sdk)，下载最新版本的 SDK，下载完成后进行解压。
...
3. 对于不同的运行平台，需要做一些额外处理。
    ...
    - **Android**
        ...
        构建 Android App 时也可能会提示存在多个 Windows dll 文件的问题，请按照上述 [Windows 的处理方式](/real-time-video-u3d-cs/quick-start/integrating-sdk) 操作。
...
如果您需要开发一个 Web KTV 项目，在 [在线 KTV - 下载](/online-ktv-u3d/downloads) 获取包含版权音乐的 Express SDK 后，完成对上述引入步骤，请打开 `Assets/WebGLTemplates/zegoTemplates/index.html` 文件，在 `head` 标签内引入以下两个 js 文件。
