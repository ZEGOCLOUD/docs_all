...
- 已在项目中集成 ZEGO Express SDK，实现基本的实时音视频功能，详情请参考 [快速开始 - 集成](/real-time-video-u3d-cs/quick-start/integrating-sdk)。
...
sequenceDiagram
    ...
    ZEGO Express SDK-->>App: 房间/用户/流状态更新回调 <br/> OnRoomStateUpdate/OnRoomUserUpdate/OnRoomStreamUpdate

    App->>ZEGO Express SDK: 预览/推流 <br/> StartPreview/StartPublishingStream
    ZEGO Express SDK-->>App: 推流状态更新/质量回调 <br/> OnPublisherStateUpdate/OnPublisherQualityUpdate

    App->>ZEGO Express SDK: 拉流 <br/> StartPlayingStream
    ZEGO Express SDK-->>App: 拉流状态更新/质量/流媒体回调 <br/> OnPlayerStateUpdate/OnPlayerQualityUpdate/OnPlayerMediaEvent
    ...
    ZEGO Express SDK-->>App: 房间状态更新回调 <br/> OnRoomStateUpdate
...
// 如果您使用 AppSign 的方式鉴权，Token 参数不需填写（除 WebGL 平台外）；如果需要使用更加安全的 鉴权方式：Token 鉴权，请参考[如何从 AppSign 鉴权升级为 Token 鉴权](https://doc-zh.zego.im/faq/express-token-upgrade)
