...
To play streams directly through the CDN's URL address, you need to use the [ZegoCDNConfig](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoCDNConfig-class.html) object to fill in the URL parameters. If the corresponding CDN is configured with playing stream authentication, you also need to fill in the authentication parameters through the `authParam` field.
...
Call the [startPlayingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePlayer/startPlayingStream.html) interface to start playing stream.
...
To stop playing stream, call the [stopPlayingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePlayer/stopPlayingStream.html) interface.
...
You can listen to the result of playing stream from CDN through [onPlayerStateUpdate](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/onPlayerStateUpdate.html).

<Warning title="Caution">

If you are not using ZEGO SDK to publish stream, but using third-party publishing tools to publish stream directly, and using ZEGO SDK to play stream, in this scenario, the publishing side has not used ZEGO SDK to join the room, and the playing side will not receive the callback of [onRoomStreamUpdate](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/onRoomStreamUpdate.html) by default.
