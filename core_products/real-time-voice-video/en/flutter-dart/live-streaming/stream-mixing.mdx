...
The stream mixing feature is not enabled by default. Please enable it in the [ZEGOCLOUD Console](https://console.zegocloud.com) before use (for enabling steps, please refer to "Stream Mixing" in [Project Management - Service Configuration](/console/service-configuration/enable-stream-mixing-service)), or contact ZEGOCLOUD technical support to enable it.
...
[ZegoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerTask-class.html) is the mixing task configuration object defined in the ZegoExpressEngine SDK, which includes information such as input streams and output streams.

**Create a mixing task object**

Create a new mixing task object through the constructor [ZegoMixerTask ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerTask-class.html), then call instance methods to set input, output, and other parameters respectively.
...
Developers can call the [ZegoMixerVideoConfig](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerVideoConfig-class.html) method to configure video parameters (frame rate, bitrate, resolution) for the mixing task.
...
Developers can call the [ZegoMixerAudioConfig](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerAudioConfig-class.html) method to configure audio bitrate, channel count, and audio encoding for the mixing task.

The default value for audio bitrate [bitrate](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerAudioConfig-class.html) is 48 kbps.
...
Define the list of input video streams [ZegoMixerInput](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerInput-class.html) based on actual business scenarios, set the "layout" parameter for each input video stream to layout each input stream's frame, and the ZEGOCLOUD Video Call server will mix the input streams and output a mixed stream in one frame.
...
<Accordion title="Stream mixing layout example 3: One large picture filled and two small pictures floating" defaultOpen="false">
...
<Accordion title="Stream mixing sound wave callback settings" defaultOpen="false">
<Warning title="Warning">
...
After completing the configuration of the [ZegoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerTask-class.html) mixing task object, call the start mixing interface to start this mixing task, and handle the logic of starting the mixing task failure in the callback.
...
[ZegoAutoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoAutoMixerTask-class.html) is the auto mixing task configuration object defined in the SDK. You can customize auto mixing tasks by configuring this object.
...
Set up auto mixing audio-related configurations through [ZegoMixerAudioConfig](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerAudioConfig-class.html), mainly including audio bitrate, channel count, and encoding ID.
...
You can modify the audio channel through the [channel](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoAudioChannel.html) parameter. Currently, the following audio channels are supported:
...
You can modify the encoding ID through the [codecID](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoAudioCodecID.html) parameter. Currently, the following encoding IDs are supported:

|Enum Value|Description|Use Cases|
|-|-|-|
|Default|Default value.|Determined by the [scenario] when calling [createEngineWithProfile](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/createEngineWithProfile.html).|
...
Set up the auto mixing output list through [ZegoMixerOutput](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerOutput-class.html), and users can play the mixed stream from the output targets in the list.
...
<Accordion title="Automatic stream mixing sound wave callback settings" defaultOpen="false">
<Warning title="Warning">
...
You can choose whether to enable auto mixing sound level callback notifications by setting the [enableSoundLevel](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoAutoMixerTask/enableSoundLevel.html) parameter. After enabling (set to "True"), users can receive sound level information for each individual stream through the [onAutoMixerSoundLevelUpdate](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/onAutoMixerSoundLevelUpdate.html) callback when playing the mixed stream.
...
After completing the configuration of the [ZegoAutoMixerTask ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoAutoMixerTask/ZegoAutoMixerTask.html) auto mixing task object, call the [startAutoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngineMixer/startAutoMixerTask.html) interface to start this auto mixing task, and receive the start auto mixing task result in the [ZegoMixerStartCallback ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerStartResult-class.html) callback.
...
1. **Can I push the mixed stream to a third-party CDN? How to forward to multiple CDNs?**
  ...
  If you need to push the mixed stream to a third-party CDN, you can fill in the CDN URL in the "target" parameter of [ZegoMixerOutput](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerOutput/ZegoMixerOutput.html).

  The URL format must be RTMP format: "rtmp://xxxxxxxx".

  To push to multiple CDNs, create N output stream objects [ZegoMixerOutput](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerOutput/ZegoMixerOutput.html) and put them in the "outputList" output list of [ZegoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerTask-class.html).

2. **How to set the layout of each stream in the mixing?**

  Example of using the "layout" parameter of [ZegoMixerInput](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerInput-class.html).
    ...
    - Assuming the resolution "width" in the "videoConfig" parameter of [ZegoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerTask-class.html) is "375" and "height" is "667".
...
3. **When the aspect ratio of the "ZegoRect" layout of the mixing input object "ZegoMixerInput" does not match the resolution of the stream itself, how will the frame be cropped?**
  ...
  The SDK will perform proportional scaling. Assuming the resolution of an input stream is "720 × 1280", that is, the aspect ratio is "9:16", and the "layout" parameter of this stream's [ZegoMixerInput](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMixerInput-class.html) is "[left:0 top:0 right:100 bottom:100]", that is, the aspect ratio is "1:1", the frame will display the middle part of this stream, with the upper and lower parts cropped out.
