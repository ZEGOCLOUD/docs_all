...
CDN live streaming is not enabled by default. Please enable it in the [ZEGOCLOUD Console](https://console.zegocloud.com) before using (for enabling steps, please refer to "CDN" in [Project Management - Service Configuration](/console/service-configuration/l3)), or contact ZEGOCLOUD Technical Support to enable it.
...
- After developers relay to multiple third-party CDNs, they can obtain status change notifications for each relayed stream from the list parameter of the CDN callback status notification [onPublisherRelayCDNStateUpdate](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/onPublisherRelayCDNStateUpdate.html).
...
Developers can register [onPublisherRelayCDNStateUpdate](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/onPublisherRelayCDNStateUpdate.html) to get the add/remove relay CDN address status callback. After the ZEGO RTC server relays the audio and video stream to the CDN, if the CDN relay status changes, such as relay stopping or relay retrying, this callback will be received.
...
If you need to stop direct publishing to CDN, call the [stopPublishingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePublisher/stopPublishingStream.html) interface to stop publishing.

After stopping publishing, if the next publishing does not need direct CDN publishing, you can call the [enablePublishDirectToCDN](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePublisher/enablePublishDirectToCDN.html) interface and pass the value "false" to turn off the direct CDN publishing function. Calling this interface during publishing will not affect this publishing.
