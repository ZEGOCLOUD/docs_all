...
By calling the [setVideoConfig](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePublisher/setVideoConfig.html) interface to modify video configuration, users can customize parameters or use preset values for setting.

<Warning title="Warning">


Need to set relevant video configurations before publishing ([startPublishingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePublisher/startPublishingStream.html)) or preview ([startPreview](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePublisher/startPreview.html)). After publishing, only encoding resolution and bitrate modification are supported.
...
If the playing end needs to pull 60-frame streams, you need to contact technical support. For details, please refer to [Does ZEGO Express SDK support pulling 60-frame streams?](/faq/60fps-stream-support)
...
In addition to setting by yourself, you can also use the preset combination values provided by ZEGO Express SDK. The preset combination values of [ZegoVideoConfigPreset](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoVideoConfigPreset.html) are as follows:
...
When calling the [setVideoConfig](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePublisher/setVideoConfig.html) interface to use preset values, the example code is as follows:
...
By modifying the "viewMode" parameter of the view object [ZegoCanvas](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoCanvas-class.html), you can modify the view mode of the video. Currently supports three video rendering fill modes:
...
Before publishing or after stopping publishing, you can call [setVideoMirrorMode](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePublisher/setVideoMirrorMode.html) to set whether the local preview video and published video enable mirror mode. Currently supports the following four mirror modes:
...
- [How to deal with video enlargement or black edge issues?](/faq/video-zoom-or-black-bars-issue)
- [How to deal with video blur issues?](/faq/video-blur-issue)
