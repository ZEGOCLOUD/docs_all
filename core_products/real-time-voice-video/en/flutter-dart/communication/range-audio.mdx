...
<p>The above image takes the range audio mode as "World" as an example. For more information about sound reachability under different mode combinations, please refer to <a href="#optional-set-general-voice-mode">Set General Voice Mode</a>.</p>
</td>
</tr>
<tr>
...
<Note title="Note">
<ul><li>Communication between teammates is not affected by "range" and "3D spatial audio". For details, please refer to <a href="#optional-set-general-voice-mode">Set General Voice Mode</a>.</li><li>If you want to customize audio send/receive capabilities, please refer to <a href="#optional-set-custom-voice-mode">Set Custom Voice Mode</a>.</li></ul>
</Note>
</td>
</tr>
</tbody></table>
...
sequenceDiagram
    ...
    A->>B: Create range audio instance object <br/> [createRangeAudio]
    A->>B: Listen to range audio event callbacks <br/> [setEventHandler]
    A->>B: Login room <br/> [loginRoom]
    critical Critical steps
        A->>B: Set listener's current position <br/> [updateSelfPosition]
        A->>B: Add/Update audio source position info <br/> [updateAudioSource]
        A->>B: Enable microphone, speaker <br/> [enableMicrophone / enableSpeaker]
        B-->>A: Callback microphone usage status <br/> [microphoneStateUpdate]
    end

    A->>B: Destroy range audio module <br/> [destroyRangeAudio]
    A->>B: Logout room <br/> [logoutRoom]
```
{/*
...
Call the [createEngineWithProfile](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/createEngineWithProfile.html) interface, pass the applied AppID to the appID parameter, and create an engine singleton object.
...
Call the [createRangeAudio](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngineRangeAudio/createRangeAudio.html) method to create a range audio instance.
...
After creating a ZegoUser user object by passing in the userID and userName parameters, call the [loginRoom](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngineRoom/loginRoom.html) interface, pass in the room ID parameter roomID and the user parameter user, and log in to the room.
...
When a user has successfully logged in to the room, if the application exits abnormally, after restarting the application, the developer needs to first call the [logoutRoom](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngineRoom/logoutRoom.html) interface to leave the room, then call the [loginRoom](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngineRoom/loginRoom.html) interface to log in to the room again.
...
After successfully logging in to the room, you can call the [updateAudioSource](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoRangeAudio/updateAudioSource.html) interface to add or update speaker position information.
...
1. Call the [enableSpatializer](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoRangeAudio/enableSpatializer.html) interface to enable 3D spatial audio.
...
<Note title="Note">
        ...
        1. Call the [createMediaPlayer](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngineMediaPlayer/createMediaPlayer.html) interface to create a media player.
        2. Call the [ZegoMediaPlayer.updatePosition](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMediaPlayer/updatePosition.html) interface to set the media player's position in the world coordinate system.
        3. Call the [loadResourceWithConfig](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoMediaPlayer/loadResourceWithConfig.html) interface to load media resources.
    - Audio Effect Player:
...
<Note title="Note">
        ...
        1. Call the [createAudioEffectPlayer](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngineAudioEffectPlayer/createAudioEffectPlayer.html) interface to create an audio effect player.
        2. Call the [ZegoAudioEffectPlayer.start](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoAudioEffectPlayer/start.html) interface to play audio effect resources.
        3. After receiving the [onAudioEffectPlayStateUpdate](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/onAudioEffectPlayStateUpdate.html) callback status as `ZegoAudioEffectPlayState.Playing`, call the [ZegoAudioEffectPlayer.updatePosition](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoAudioEffectPlayer/updatePosition.html) interface to set the position of the currently playing audio effect resource in the world coordinate system.
...
When the range audio module is no longer in use, call the [destroyRangeAudio](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngineRangeAudio/destroyRangeAudio.html) interface to destroy it and release the resources occupied by the range audio module.
