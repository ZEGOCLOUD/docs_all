...
2. Integrate the Video Call SDK and implement the basic video feature. For details, refer to [Quick start\|_blank](!Quick_start_Integrate_Implementation).
...
```dart
ZegoExpressEngine.instance.renewToken('roomID', 'new_token');
```
...
ZegoExpressEngine.onRoomTokenWillExpire = (String roomID, int remainTimeInSecond) {
    String token = 'new_token'; // Get the new token；
