...
<Note title="Note">
For SEI-related concepts and principles, please refer to [How to Understand and Use SEI (Media Supplemental Enhancement Information)](/faq/media-sei-usage).
</Note>
...
```mermaid
sequenceDiagram
    participant P as Publishing Client App
    participant Z as ZEGO Express SDK
    participant L as Playing Client App

    P->>Z: Initialize SDK <br/> [new ZegoExpressEngine]

    P->>Z: Login room <br/> [loginRoom]

    P->>Z: Start publishing stream <br/> [startPublishingStream]

    L->>Z: Initialize SDK <br/> [new ZegoExpressEngine]

    L->>Z: Implement related callback listeners <br/> [on('playerRecvSEI')]

    L->>Z: Login room <br/> [loginRoom]

    L->>Z: Start playing stream <br/> [startPlayingStream]

    critical Critical steps
        P->>Z: Send SEI info <br/> [sendSEI]
        Z-->>L: Receive SEI info <br/> [playerRecvSEI]
    end
```
{/*
...
1. Call the [createEngineWithProfile ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/createEngineWithProfile.html) interface to create an engine object.

2. Call the [loginRoom ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngineRoom/loginRoom.html) interface to log into the room.

3. Call the [startPublishingStream ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePublisher/startPublishingStream.html) interface to publish stream.

4. After successfully publishing the stream, call the [sendSEI ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePublisher/sendSEI.html) interface to send SEI information.

Playing side:

1. Call the [createEngineWithProfile ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/createEngineWithProfile.html) interface to create an engine object.

2. Implement the [ZegoExpressEngine.onPlayerRecvSEI ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/onPlayerRecvSEI.html) callback.

3. Call the [loginRoom ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngineRoom/loginRoom.html) interface to log into the room.

4. Call the [startPlayingStream ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePlayer/startPlayingStream.html) interface to play stream.

5. After successfully playing the stream, the [onPlayerRecvSEI ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEngine/onPlayerRecvSEI.html) callback is triggered after receiving SEI information sent from the publishing side.

<Note title="Note">


    When playing stream, if the developer sets to play only audio stream by calling [mutePlayStreamVideo](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePlayer/mutePlayStreamVideo.html) or [muteAllPlayStreamVideo](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePlayer/muteAllPlayStreamVideo.html) interfaces, SEI information cannot be received.
...
However, when developers need to use third-party decoders to decode (such as FFmpeg), it will result in inability to decode the correct SEI. At this time, before publishing stream, you need to call the [setSEIConfig ](https://www.zegocloud.com/unique-api/express-video-sdk/en/dart_flutter/zego_express_engine/ZegoExpressEnginePublisher/setSEIConfig.html) interface to change the SEI type sent by the SDK, using UserUnregister's SEI (nalu type = 6, payload type = 5) type packaging.
...
[How to Understand and Use SEI (Media Supplemental Enhancement Information)?](/faq/media-sei-usage)
