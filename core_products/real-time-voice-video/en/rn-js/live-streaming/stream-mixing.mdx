...
[ZegoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixertask.html) is a stream mixing task configuration object defined in the SDK, which contains information such as input stream layout and output stream.
...
Create a new stream mixing task object [ZegoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixertask.html), and then set input, output and other parameters respectively.
...
Developers can call the [ZegoMixerVideoConfig](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixervideoconfig.html) method to configure the video parameters (frame rate, bitrate, resolution) of the stream mixing task.
...
Developers can call the [ZegoMixerAudioConfig](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixeraudioconfig.html) method to configure the audio bitrate, number of audio channels, and audio encoding of the stream mixing task.
...
After completing the configuration of the [ZegoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixertask.html) stream mixing task object, call the [startMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startmixertask) interface to start this stream mixing task, and handle the logic of failing to start the stream mixing task in the asynchronous result return.
...
When stream mixing information changes (for example, the input stream list of stream mixing increases or decreases, adjust stream mixing video output bitrate, etc., modify the parameters of this stream mixing task object), and then call the [startMixerTask ](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startmixertask) interface once again to update the configuration.
...
Call the [stopMixerTask ](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#stopmixertask) interface to stop stream mixing.
...
If you need to push the mixed stream to a third-party CDN, you can fill in the CDN URL in the "target" parameter of [ZegoMixerOutput ](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixeroutput.html). The filled URL format needs to be RTMP format, such as "rtmp://xxxxxxxx".

To relay to multiple CDNs, create N output stream objects [ZegoMixerOutput ](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/modules/_index_.html#zegomixeroutput) and put them in the "outputList" output list in "ZegoMixerTask".

**2. How to set the layout of each stream in stream mixing?**

Usage example of the "layout" parameter of [ZegoMixerInput ](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixerinput.html):
...
**3. When the aspect ratio of the "ZegoRect" layout of the stream mixing input object [ZegoMixerInput ](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixerinput.html) does not match the resolution of the stream itself, how will the picture be cropped?**
