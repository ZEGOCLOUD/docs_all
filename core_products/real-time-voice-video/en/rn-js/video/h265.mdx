...
- Co-hosting: A form of interaction between users in a room. By calling the [startPublishingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startpublishingstream) interface to publish your own stream while also calling the [startPlayingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startplayingstream) interface to play the other party's stream. After two users successfully co-host, they can engage in interactive calls.
...
Some older or low-end mobile models do not support H.265 video **encoding**. In this case, developers need to **before publishing stream** first determine whether the device supports H.265 video encoding capability through the [isVideoEncoderSupported](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#isvideoencodersupported) interface. If supported, you can set H.265 video encoding type through the [setVideoConfig](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#setvideoconfig) interface before publishing stream; otherwise, it will not take effect.

#### Detect H.265 Decoding Capability

Some older or low-end mobile models do not support H.265 video **decoding**. In scenarios where different video bitstreams can be played, such as CDN scenarios, developers need to **before playing stream** first determine whether the device supports H.265 video decoding capability through the [isVideoDecoderSupported](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#isvideodecodersupported) interface. If supported, H.265 video bitstreams can be played; otherwise, only other video bitstream formats, such as H.264, can be played.
...
1. Create a new mixer task object through the constructor [ZegoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixertask.html#constructor), then call instance methods to set input, output, and other parameters respectively.
2. Set the mixer task input stream list through the [inputList](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixertask.html#inputlist) property in the mixer task object [ZegoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixertask.html) (the encoding format of streams in stream mixing input supports H.264 and H.265), default supports up to 9 input streams, please handle stream layout yourself.
3. Set the mixer task output stream list through the [outputList](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixertask.html#outputlist) property in the mixer task object [ZegoMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressdefines_.zegomixertask.html). Assuming in the current scenario, the stream mixing service directly transcodes one H.264 mixed stream and one H.265 mixed stream, that is, the stream mixing output video encoding format is H.264 and H.265.
4. Call the [startMixerTask](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startmixertask) interface to initiate a mixer task.
...
2. Call the [isVideoDecoderSupported](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#isvideodecodersupported) interface to query whether the audience's device supports H.265 decoding format.
    - If supported, call the [startPlayingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startplayingstream) interface to play H.265 bitstreams from CDN.
    - If not supported, call the [startPlayingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startplayingstream) interface to play H.264 bitstreams from CDN.
...
<Steps>
<Step>
After creating the engine, call the [enableHardwareEncoder](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#enablehardwareencoder) interface to enable hardware encoding (if configuration is changed after publishing stream, you need to wait for the next publishing stream to take effect). After enabling, GPU will be used for encoding, reducing CPU usage.
</Step>
<Step>
Call the [isVideoEncoderSupported](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#isvideoencodersupported) interface to query whether the host's device supports the specified video encoding type.
</Step>
<Step>
...
<Steps>
<Step>
You can call the [enableH265EncodeFallback](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#enableh265encodefallback) interface to enable automatic fallback to H.264 encoding when H.265 encoding fails (default enabled). After enabling, when H.265 encoding is not supported or H.265 encoding fails, the SDK will internally attempt to downgrade to H.264 encoding for publishing stream. After disabling, when H.265 encoding is not supported or H.265 encoding fails, publishing stream will fail directly.
</Step>
<Step>
Call the [addPublishCdnUrl](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#addpublishcdnurl) interface to add the URL address for relaying the audio and video stream from ZEGOCLOUD Video Call cloud to CDN.
</Step>
<Step>
Call the [startPublishingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startpublishingstream) interface to start publishing stream. After successful publishing, other users in the same room can get stream addition notifications by listening to the [roomStreamUpdate](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/interfaces/_zegoexpresseventhandler_.zegoeventlistener.html#roomstreamupdate) callback.
</Step>
<Step>
...
1. After the host publishes stream, the audience end receives stream added notification through the [roomStreamUpdate](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/interfaces/_zegoexpresseventhandler_.zegoeventlistener.html#roomstreamupdate) interface.
2. The audience end gets the encoding format of the publishing stream from the app's business server.
- If it is H.264 format, the audience can call the [startPlayingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startplayingstream) interface to play the host's stream directly from CDN.
- If it is H.265 format, you need to first call the [isVideoDecoderSupported](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#isvideodecodersupported) interface to query whether the audience's device supports H.265 decoding format.
    - If supported, call the [startPlayingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startplayingstream) interface to play H.265 bitstreams from CDN.
    - If not supported, call the [startPlayingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startplayingstream) interface to play H.264 bitstreams from CDN.
...
1. After the host publishes stream, co-hosting guests receive stream added notifications through the [roomStreamUpdate](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/interfaces/_zegoexpresseventhandler_.zegoeventlistener.html#roomstreamupdate) interface.
2. Co-hosting guests get the encoding format of the publishing stream from the app's business server.
- If it is H.264 format, co-hosting guests can call the [startPlayingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startplayingstream) interface to play the host's stream directly from ZEGOCLOUD Video Call cloud.
- If it is H.265 format, you need to first call the [isVideoDecoderSupported](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#isvideodecodersupported) interface to query whether their device supports H.265 decoding format.
    - If supported, call the [startPlayingStream](https://www.zegocloud.com/unique-api/express-video-sdk/en/javascript_react-native/classes/_zegoexpressengine_.zegoexpressengine.html#startplayingstream) interface to play H.265 bitstreams from ZEGOCLOUD Video Call cloud.
