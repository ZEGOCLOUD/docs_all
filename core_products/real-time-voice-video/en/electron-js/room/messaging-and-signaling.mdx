...
1. **Send custom signals**
  ...
  Call the [sendCustomCommand](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-zego-express-engine#send-custom-command) interface to send custom signals to users specified by "toUserList" in the same room. The length cannot exceed 1024 bytes.
