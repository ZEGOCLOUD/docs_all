...
Call the [startPlayingStream](https://www.zegocloud.com/article/api?doc=express-video-sdk_API~javascript_electron~class~ZegoExpressEngine#start-playing-stream) interface to pull the remote audio and video stream according to the passed stream ID parameter "streamID".
