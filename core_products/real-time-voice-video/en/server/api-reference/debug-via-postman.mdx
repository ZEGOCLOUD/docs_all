...
本文已不再更新，推荐使用 [如何在线调试服务端 API](/real-time-video-server/api-reference/call-api-online) 进行调试。
...
3. 打开 “RTC” 环境，将 [前提条件](/real-time-video-server/api-reference/debug-via-postman) 中获取的 AppId 和 ServerSecret 设置到对应环境变量的 “CURRENT VALUE” 中，然后保存。
...
1. 在 Collections 中选择 “ SendBroadcastMessage”，根据 [推送广播消息](/real-time-video-server/api-reference/room/send-broadcast-message) 中的参数说明修改 Param 中参数的取值。双大括号包裹的参数值为环境变量，无需手动修改。
...
1. 在 Collections 中选择 “KickoutUser”，根据 [踢出房间用户](/real-time-video-server/api-reference/room/kick-out-user) 中的参数说明修改 Param 中参数的取值。UserId[] 为踢出房间的用户 ID，可携带一个或多个，最大支持 5 个用户 ID，具体含义请参考参数说明。
...
1. 在 Collections 中选择 “StartMix”，根据 [开始混流](/real-time-video-server/api-reference/stream-mixing/start-mix) 中的参数说明修改 Body 中参数的取值。
