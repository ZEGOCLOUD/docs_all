...
当开发者需要了解当前 App 的单流转码任务，可以参考 [回调配置说明](/real-time-video-server/callback/receiving-callback) 配置相关回调接口。当发起 [停止单流转码](/real-time-video-server/api-reference/stream-mixing/stop-cloud-transcode) 请求后，可以通过该回调，查看停止转码任务操作的结果。
