...
当开发者有视频流内容审核的需求时，在 [开始视频流审核](/real-time-video-server/api-reference/moderation/start-censor-video) 任务后，将会以 POST 的形式对回调地址进行请求，获取音频流审核识别结果。当 [开始视频流审核](/real-time-video-server/api-reference/moderation/start-censor-video) 任务中的 `ReturnFinishInfo` 参数为 1 时，审核任务结束后，会发起审核任务状态回调。
...
| Nonce | String | 随机数。 |
| Signature | String | 签名，具体见  <a href="/real-time-video-server/callback/receiving-callback" target="_blank">检验说明</a>。 |

| 业务参数 | 类型 | 描述 |
|---|---|---|
| Code | Number | 返回码。0 为成功。 |
| Message | String | 操作结果描述。 |
| TaskId | String | 审核任务的 TaskId。<br/>对应 [开始视频流审核](/real-time-video-server/api-reference/moderation/start-censor-video) 响应参数中返回的 TaskId。 |
...
| Code | Number | 返回码。0 为成功。 |
| Message | String | 操作结果描述。 |
| TaskId | String | 审核任务的 TaskId。<br/>对应 [开始视频流审核](/real-time-video-server/api-reference/moderation/start-censor-video) 中响应参数中返回的 TaskId。 |
| ResultTaskId | String | 此次回调信息的 ID。 |
...
当 [开始视频流审核](/real-time-video-server/api-reference/moderation/start-censor-video) 任务中的 `ReturnFinishInfo` 参数为 1 时，视频流审核任务结束后，会发起审核任务状态回调。
...
| Message | String | 操作结果描述。 |
| TaskId | String | 审核任务的 TaskId。<br/>对应 [开始视频流审核](/real-time-video-server/api-reference/moderation/start-censor-video) 返回的 TaskId。 |
...
| Code | Number | 返回码，0 表示成功。 |
| Message | String | 操作结果描述。 |
| TaskId | String | 审核任务的 TaskId，对应 [开始视频流审核](/real-time-video-server/api-reference/moderation/start-censor-video) 返回的 TaskId。 |
| Status | Number | 审核状态。<br/>0：审核结束。 |
| AuxInfo | Object | 辅助信息。 |
| └ RoomId | String | 房间 ID。 |
| └ CensorStreamTime | Number | 本次任务审核流的总时长（单位：秒）。 |
