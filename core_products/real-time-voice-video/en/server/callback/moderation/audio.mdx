...
当开发者有音频流内容审核的需求时，在 [开始音频流审核](/real-time-video-server/api-reference/moderation/start-censor-audio) 任务后，将会以 POST 的形式对回调地址进行请求，获取音频流审核识别结果。当 [开始音频流审核](/real-time-video-server/api-reference/moderation/start-censor-audio) 任务中的 `ReturnFinishInfo` 参数为 1 时，审核任务结束后，会发起审核任务状态回调。
...
| TaskId | String | 审核任务的 TaskId。<br/>对应 [开始音频流审核](/real-time-video-server/api-reference/moderation/start-censor-audio) 中响应参数中返回的 TaskId。 |
...
当 [开始音频流审核](/real-time-video-server/api-reference/moderation/start-censor-audio) 任务中的 `ReturnFinishInfo` 参数为 1 时，音频流审核任务结束后，会发起审核任务状态回调。
...
| Message | String | 操作结果描述。 |
| TaskId | String | 审核任务的 TaskId，对应 [开始音频流审核](/real-time-video-server/api-reference/moderation/start-censor-audio) 返回的 TaskId。 |
