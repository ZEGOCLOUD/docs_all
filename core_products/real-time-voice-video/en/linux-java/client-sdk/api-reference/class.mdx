...
<Note title="">
- **Use cases**: Developers can use this callback to determine the status of the current user in the room.
 When to trigger: Users will receive this notification when they call room functions (refer to [Related APIs]). 2. This notification may also be received when the user device's network conditions change (SDK will automatically log in to the room again when the connection is disconnected, refer to [https://www.zegocloud.com/docs/faq/express-reconnect](https://www.zegocloud.com/docs/faq/express-reconnect) ).
- **Caution**: If the connection is being requested for a long time, the general probability is that the user's network is unstable.
- **Related APIs**: [loginRoom], [logoutRoom], [switchRoom]
</Note>
...
<ParamField
  ...
  desc="[Deprecated] Create ZegoExpressEngine singleton object and initialize SDK. Deprecated since 2.14.0, please use the method with the same name without [isTestEnv] parameter instead. Please refer to [Testing environment deprecation](https://docs.zegocloud.com/article/13315) for more details."
  prefixes={["static"]}
  suffixes={["deprecated"]}
  parent_file="Declared in \`ZegoExpressEngine.java\`"
  parent_name="ZegoExpressEngine"
...
<Note title="">
- **Use cases**: Recommended configuration in different business scenarios [https://www.zegocloud.com/docs/real-time-video-ios-oc/introduction/overview](https://www.zegocloud.com/docs/real-time-video-ios-oc/introduction/overview)
...
<Note title="">
- **When to call**: After the callback [onStart] is received.
- **References**: Please refer to the documentation [https://www.zegocloud.com/docs/real-time-video-android-java/introduction/overview](https://www.zegocloud.com/docs/real-time-video-android-java/introduction/overview) for details.
