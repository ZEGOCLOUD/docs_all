...
Before publishing, call the [enableTrafficControl](/real-time-video-ios-oc/client-sdk/api-reference/class#enabletrafficcontrolproperty-zegoexpressengine-class) interface and set the `enable` parameter to true to enable traffic control, and set adjustable traffic control properties (bitrate, frame rate, resolution) through the `property` parameter. Multiple options are supported, and the default value is `ADAPTIVE_FPS`, which is adaptive (reducing) video frame rate. When uplink bandwidth is insufficient, the SDK will reduce the final uplink bitrate of publishing according to the current network environment and the set `property` parameter to adapt to the uplink bandwidth.
...
After enabling traffic control, call the [setMinVideoBitrateForTrafficControl](/real-time-video-ios-oc/client-sdk/api-reference/class#setminvideobitratefortrafficcontrolmode-zegoexpressengine-class) method to set the minimum video bitrate (default value is "0") and video sending mode. This allows the SDK to not send video or send video at an extremely low frame rate when the network does not reach the minimum bitrate for sending video.

<Warning title="Warning">

You can call [setMinVideoBitrateForTrafficControl](/real-time-video-ios-oc/client-sdk/api-reference/class#setminvideobitratefortrafficcontrolmode-zegoexpressengine-class) at any time after initializing the SDK and before publishing stream [startPublishingStream](@startPublishingStream), but this setting only takes effect after traffic control is enabled.
</Warning>
