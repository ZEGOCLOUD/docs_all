...
<p>The above diagram only shows an example where the range voice mode is "World". For more information about voice reachability under different mode combinations, please refer to <a href="#set-common-voice-mode">Set Common Voice Mode</a>.</p>
</td>
</tr>
<tr>
...
<Note title="Note">
<ul><li>Communication between teammates is not affected by "Range" and "3D Audio Effects". For details, please refer to <a href="#set-common-voice-mode">Set Common Voice Mode</a>.</li><li>If you want to customize voice sending and receiving capabilities, please refer to <a href="#set-custom-voice-mode">Set Custom Voice Mode</a>.</li></ul>
</Note>
</td>
</tr>
</tbody></table>
...
sequenceDiagram
    ...
    App->>SDK: Create range audio module <br/> [CreateRangeAudio]
    App->>SDK: Listen to range audio event callbacks <br/> [IZegoRangeAudioHandler]
...
- Call the [enableMicrophone](@enableMicrophone) interface to set whether to enable the microphone. When enable is true, it means enabled. At this time, the SDK will automatically use the main channel to publish audio stream. When false, it means disabled. (Can be enabled or disabled at any time)
    ...
    Developers can listen to the [microphoneStateUpdate](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~objective-c_ios~protocol~ZegoRangeAudioEventHandler#range-audio-microphone-state-update-error-code) event callback to get the updated microphone state.
