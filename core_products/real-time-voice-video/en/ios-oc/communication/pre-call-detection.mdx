...
Call the [startPreview](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine&jumpType=route#start-preview) interface to start audio capture without streaming.
...
Call the [startPreview](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine&jumpType=route#start-preview) interface to bind the view for camera preview, start video capture and preview without streaming.
...
If you can hear the corresponding audio, the playback device is functioning properly and can be used for calls. Call the [mediaPlayer:stateUpdate:errorCode:](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC~protocol~zego-media-player-event-handler#media-player-state-update-error-code) callback to check the media player status:
