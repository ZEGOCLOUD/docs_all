...
1. Upgraded Xcode version

    The SDK has been upgraded to Xcode 26 to meet Apple platform build and release requirements.

2. Reduced the number of TLS keys created by the SDK, lowering resource usage

3. Increased the default maximum number of stream playback from 12 to 20

4. Optimized the issue affecting word accuracy in low-latency mode
...
6. Super resolution performance optimization
    ...
    Related API: [enableVideoSuperResolution](@enableVideoSuperResolution)
...
1. Support custom encryption for audio and video streams
    ...
    For related APIs, see: [ZegoCustomMediaCrypto](@-ZegoCustomMediaCrypto), [setCustomMediaCryptoFactory](@setCustomMediaCryptoFactory)
