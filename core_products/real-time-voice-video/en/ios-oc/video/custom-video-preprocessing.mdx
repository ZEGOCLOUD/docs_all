...
sequenceDiagram
    ...
    A->>Z: Initialize Express SDK <br/> [createEngine]
    A->>Z: Login room <br/> [loginRoom]

    critical Critical steps
        A->>Z: Enable custom video preprocessing <br/> [enableCustomVideoProcessing]
        A->>Z: Set custom video preprocessing callback <br/> [setCustomVideoProcessHandler]
        A->>Z: Start preview, publish stream <br/> [startPreview, startPublishingStream]
        Z-->>A: Get raw video data,<br>perform video preprocessing <br/> [onCapturedUnprocessedCVPixelBuffer]
        A->>Z: Send processed image to Express <br/> [sendCustomVideoProcessedCVPixelBuffer]
    end

    A->>Z: Play processed video data <br/> [startPlayingStream]
    A->>Z: Stop preview, publish stream <br/> [stopPreview, stopPublishingStream]
    A->>Z: Logout room <br/> [logoutRoom]
    A->>Z: Destroy Express SDK <br/> [destroyEngine]
```
{/*
...
Before starting preview and starting publishing stream, call the [enableCustomVideoProcessing](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#enable-custom-video-processing-config) interface to enable the custom video preprocessing feature.
...
1. Use "ViewController" as the custom video preprocessing callback object and conform to the [ZegoCustomVideoProcessHandler ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~protocol~zego-custom-video-process-handler) protocol.
...
2. Call the [setCustomVideoProcessHandler ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#set-custom-video-process-handler) interface to set the custom video capture callback.
