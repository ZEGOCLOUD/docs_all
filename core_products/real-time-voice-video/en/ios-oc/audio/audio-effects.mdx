...
Call the [setVoiceChangerPreset ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#set-voice-changer-preset) method to use the SDK's preset voice changer effects.
...
If the SDK's preset voice changer effects cannot meet your needs, developers can call the [setVoiceChangerParam](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#set-voice-changer-param) method [ZegoVoiceChangerParam](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-voice-changer-param) to set custom voice changer through the pitch parameter "pitch". The value range of this parameter is [-12.0, 12.0]. The larger the value, the sharper the sound. The default value is "0.0" (that is, no voice changer).
...
Call the [setReverbPreset ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#set-reverb-preset) to set reverb through preset enumeration.
...
After setting custom reverb parameters, the preset reverb effects set when enabling reverb will become invalid. If you want to use the SDK preset parameters again, you can use the [setReverbPreset ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#set-reverb-preset) preset enumeration method to set them.
</Warning>

### Reverb Echo

Call the [setReverbEchoParam ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC_ios~class~zego-express-engine#set-reverb-echo-param) method to combine and set relevant parameters to achieve the reverb echo effects required by developers (for detailed parameter descriptions, please refer to the API documentation).
...
If you need to enable the virtual stereo feature, you must first call the [setAudioConfig ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC~class~zego-express-engine#set-audio-config) method before publishing stream to set the audio encoding channel to Stereo dual-channel (default is Mono mono-channel).

The example here constructs [ZegoAudioConfig ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC~class~zego-audio-config) through preset enumeration and sets it to dual-channel.
...
After setting the audio encoding channel to dual-channel, call the [enableVirtualStereo ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC~class~zego-express-engine#enable-virtual-stereo-angle) method, enable virtual stereo through the "enable" parameter, and set the sound source angle of virtual stereo through the "angle" parameter to have a stereo effect. The angle range is 0 to 360. Generally, it can be set to 90 degrees (that is, directly in front).
