...
- Interface prototypes
    ...
    - Remote played stream audio spectrum callback interface [onRemoteAudioSpectrumUpdate](@onRemoteAudioSpectrumUpdate):
        ...
        //
        // @param audioSpectrums Key-value pairs of remote audio spectrum, key is stream ID, value is the corresponding stream's audio spectrum value array, spectrum values range from [0-2^31]
...
- Call the [startSoundLevelMonitor](@startSoundLevelMonitor) interface to start monitoring sound level:
    ...
    ```
    <Note title="Note">Configuring the `millisecond` property of [ZegoSoundLevelConfig](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~objectivec_ios~class~ZegoSoundLevelConfig) can set the interval for sound level callbacks. The sound level monitoring time period is in milliseconds, ranging from [100, 3000]. Default is 100 ms.</Note>
...
- [What is the difference between media volume and call volume?](https://www.zegocloud.com/docs/faq/background-music-volume-issue)
- [How to handle no sound issue?](https://www.zegocloud.com/docs/faq/no-audio-issue)
- [How to handle low volume issue?](https://www.zegocloud.com/docs/faq/low-audio-volume-issue)
