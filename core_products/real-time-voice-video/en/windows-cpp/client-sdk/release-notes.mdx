...
1. Added video pre-encoding processing
...
2. Windows supports capturing non-system default audio output devices
    
<Warning title="Warning">

Only effective when sysplay_volume_follow_render=false (default is false).
...
5. When PC captures system sound card, it can be used as an independent source for streaming
    
    ZegoScreenCaptureSource adds enableAudioCapture interface to set whether to allow system audio capture. Before use, you need to call setAudioSource to set the capture source to ZegoAudioSourceTypeScreenCapture, and the screen capture and streaming channels need to be the same. 
    
<Warning title="Warning">

- It takes effect after startCapture starts capturing.
- After stopCapture stops capturing, audio capture will also stop synchronously. 
- It can be switched on and off at any time during capture. 
- Volume adjustment is not supported yet and follows system volume output. 
- Only applicable to macOS 10.15 and above and Windows 8 and above, capturing process sound only supports Windows 10 2004 and above.

</Warning>


For related API, please refer to [enableAudioCapture](@enableAudioCapture), [setAudioSource](@setAudioSource)

6. switchPlayingStream supports setting timeout
...
7. PK mixed streaming scenario supports blurring picture edges
...
8. The desktop supports setting pictures as custom video collection sources.
    
**Enhancements**

1. Optimized the performance and effect of Windows platform screen capture
    
    Added two configuration items that can be set before engine initialization: 
    
1. screencapture_enable_d3d_texture=true indicates starting texture capture 
2. screencapture_performance_mode=0/1/2 texture capture performance mode. 
    
    0 is the default mode, copying texture only once. Supports mouse display, border and other capabilities, used in normal screen capture scenarios.    
    1 is performance mode 1, copying texture only once without any processing.    
    2 is performance mode 2, passing texture directly to encoder without copying.   
    
      <Warning title="Warning">

      Performance modes 1 and 2 are used in high-performance scenarios such as game streaming, only supporting win10 18362 and above.
...
2. Flow control supports any resolution
    
    Optimize the flow control logic, remove the screen ratio limit, and currently support any resolution.
    
3. The flow control status callback interface supports distinguishing the disconnection status.
...
2. Fixed Windows system ear return duplex over-suppression issue
...
1. Screen sharing feature supports outline effect
    ...
    For related API, please refer to [enableHightLight](@enableHightLight)
    
2. Added a screen sharing exception callback with source type
    
    When the screen sharing source (such as a window or screen area) encounters an exception, the new [onCaptureTypeExceptionOccurred](@onCaptureTypeExceptionOccurred) callback will return the specific exception source type to help developers quickly locate the problem scenario.
    
    For related API, please refer to [onCaptureTypeExceptionOccurred](@onCaptureTypeExceptionOccurred)
    
3. Support customizing CDN stream playback timeout duration
...
4. Support setting the stream title when publishing a stream
...
5. Added an error code related to resource file permissions for the media player
...
4. Custom video capture supports developer control of external timestamps
...
1. Fixed the issue of echo leakage during audio and video calls.
    
2. Optimized login time.
    
3. Fixed the compatibility issue with older versions of the experimental API.
    
4. Fixed the issue of media player low probability status exceptions.
...
3. Reduce the time it takes to publish and play streams, and improve SDK performance.
...
2. Fix the issue of white stripes appearing in sampling under D3D11 rendering texture
...
2. When publishing streams, you can control whether the stream is allowed for review
...
3. Media Player supports playback speed as low as 0.3x.
...
4. Support for adaptive bitrate playback is available when streaming through L3.
...
5. Support smooth switching when using CDN for streaming
...
6. H.265 client encoding automatic compatibility strategy adds user-level negotiation range.
...
1. Fixed the issue of accessing a wild pointer during camera enumeration.
    
2. Fixed the issue of probabilistic crashes when stopping the audio and video engine with the input source from the auxiliary stream in the case of using the main stream for pushing.
...
2. ZegoMediaPlayer supports video mirroring.
    
    For related API, please refer to [enableViewMirror](@enableViewMirror)
    
3. Supports initiating mixed-streaming tasks normally when image resource verification fails.
...
4. Supports determining in advance whether the device can run the AI voice changing function.
...
2. Fixed the problem that the mediaPlayer takes a long time to open files in M3U8 format.
...
2. During screen sharing and live streaming, it supports capturing audio from specific processes.
    
    Note: This feature needs to be set before calling the [startCapture] interface; and when calling the [setAudioSource] interface, select the audio capture source type as ZEGO_AUDIO_SOURCE_TYPE_CUSTOM. 
    
    Added [enableAudioCapture] interface, which supports capturing audio from a specified process during screen sharing streaming.
    
    For related API, please refer to [enableAudioCapture](@enableAudioCapture), [startcapture](@startcapture), [setAudioSource](@setAudioSource)
...
5. Custom video rendering supports independent channel control.
...
6. Supports obtaining video data pre-processed by SDK, and passing different video data to SDK for preview and publish streaming respectively.
...
7. Supports low-light enhancement and color enhancement when using external capturing.
...
8. Support for H.265 automatic compatibility strategy.
...
2. Fixing the issue of occasional failure when using MJPEG format to capture with Windows camera.
    
3. Fixed compatibility issues with hardware encoding and decoding, addressing occasional crashes.
    
4. Fix known compatibility issues and null pointer problems.
...
1. Fix the issue that When the system is in sleep mode, the SDK does not receive notifications for network status changes, resulting in inaccurate determination of network type.
...
1. Fix the crash issue when N card hardware decoding exits in certain situations.
    
2. Fixing the problem with the restart strategy when encountering camera abnormalities.
...
2. ScreenCapture supports setting publishing-stream area.
    ...
    For related API, please refer to [updatePublishRegion](@updatePublishRegion)
...
5. Windows supports the hard coding of the specified graphics card, and the texture input and output of hard coding and hard decoding.
...
1. Modified the setMediaDataPublisherEventHandler interface name of the media publisher IZegoMediaDataPublisher class.
    ...
    In version 3.8.0, the name of an API interface of the media pusher [IZegoMediaDataPublisher] class is changed: the original member function [setMediaDataPublisherEventHandler] is renamed to [setEventHandler]. There may be compatibility issues after the interface name is modified. For details, please refer to [Upgrade guide 3.8.0+](!upgrade_guide5).
...
2. Added Enhanced KTV reverb effect.
...
3. Game voice function supports media player and audio player to use 3D sound effects.
...
4. The media player supports obtaining media stream video information.
...
5. Media player double speed function supports up to 4x.
...
1. Optimize the performance of the subject segmentation function when the microphone overlaps with the human body.
...
2. Fixed an issue where a green screen may occasionally appear in screen rendering when using the Windows SDK to preview or stream.
...
2. Added [setSpeakerSimpleMute] and [getSpeakerSimpleMute] interfaces for setting speaker mute and obtaining whether speakers are muted.
...
3. Media player supports playing transparent special effects.
    ...
    The media player supports the playback of transparent special effects files through the rendering of alpha channels.
    
    For related API, please refer to [loadResourceWithConfig](@loadResourceWithConfig)
    
4. State synchronization and active streaming support for audio and video over a 10000 person range.
...
5. The media player supports functions such as downloading, retrieving data decryption, and playing at the same time.
...
6. Support for platform based switch body segmentation function.
...
7. Support dynamic switching of flow control strategies
...
1. Optimize the adaptation function of the resolution and frame rate of the external camera on the Electron end.
    
2. Optimize SDK memory usage
...
3. The single-stream transcoding function supports L3 or CDN streaming.
...
4. The same mixed-stream task supports the output of multiple resolution video streams.
...
5. Mixed flow task supports input of super whiteboard information.
...
6. Scenario-based audio and video configuration adds [StandardVoiceCall] standard voice call scenario.
...
1. Fixed hardware encoding access null pointer crash.
    
2. Fixed network time module retry failure.
...
2. Fixed the problem of occasional access field pointer crash when video external capture is in memory mode.
...
This version contains breaking changes, please refer to [v3.0.0 Upgrade Guide](!upgrade_guide4) for details.
...
1. Fixed a crash caused by enumerating video hardware encoding capabilities on Windows.
    
2. Fixed an issue where calling [loginRoom] during network switching (Wi-Fi or cellular) may not have a callback in multi-room mode.
    
3. Fixed the issue that the voice of people in the original squad could be heard outside the range of the range voice function after exiting the squad.
...
3. The H.264 and H.265 hardware codec function supports using AMD/NVIDIA/Intel graphics cards.
    ...
    For related API, please refer to [setEngineConfig](@setEngineConfig), [enableHardwareEncoder](@enableHardwareEncoder), [enableHardwareDecoder](@enableHardwareDecoder)
...
1. Optimize the related error codes of Token exceptions when mandatory login authentication is enabled.
    ...
    Added 1002074, 1002075, 1002076, 1002077, 1002078, 1002079, 1002080 and other error codes. After enabling mandatory login authentication, if the Token is incorrect, these error codes will be returned. For details, please refer to <a href="https://docs.zegocloud.com/article/5549#3" target="_blank" rel="noopener noreferrer">Error codes</a>.
    
2. Windows supports H.265 hardware codec for Nvidia cards.
    
    Note: To enable this feature, you need to contact ZEGOCLOUD technical support.
    
3. Optimized the alignment between vocal and accompaniment for real-time chorus scenario.
...
1. Fix the exception of Windows Device Manager automatically switching devices.
    
    Fixed an issue where the Windows Device Manager would not automatically switch devices sometimes when the user selected multiple devices on the same channel in the "Auto Mode".
    
2. Fixed an issue where [onStop] callbacks would be triggered repeatedly when custom video processing was turned off.
    
    For related API, please refer to [enableCustomVideoProcessing](@enableCustomVideoProcessing)
    
3. Fixed an issue where play stream with L3 failed.
...
1. The authentication method is optimized.
    ...
    For versions 2.17.0 and above, pass the AppSign blank or not when creating the engine, and you must pass in the Token when logging in to the room. After the authentication is passed, you can use the real-time audio and video function. For details, please refer to <a href="https://docs.zegocloud.com/article/11649" target="_blank" rel="noopener noreferrer">Control user mos</a>.
...
3. Support ROI (region of interest) intelligent video coding, improve the quality of the specified area.
    ...
    This feature is often used for remote control, <a href="https://docs.zegocloud.com/article/9954" target="_blank" rel="noopener noreferrer">Face detection</a > etc. scene.
...
3. Local media recording supports file paths in utf8 format.
    
    After previewing or streaming is successful, when calling the [startRecordingCapturedData] API to record audio and video data locally, it supports saving the recorded file to a file path in utf8 format (recommended for developers to use this format).
...
1. Fixed an issue where switching audio devices did not work.
...
5. Support to obtain the audio device information currently in use.
    ...
    For related API, please refer to [getCurrentAudioDevice](@getCurrentAudioDevice)
    
6. Mixing supports spatial audio capabilities.
...
5. Media player added support for repeat play times.
...
2. Media player added screenshot function.
...
3. Support mute or unmute audio devices.
    
    You can mute or unmute audio input/output devices as needed.
    
    For related API, please refer to [muteAudioDevice](@muteAudioDevice), [isAudioDeviceMuted](@isAudioDeviceMuted)
    
4. Supports monitoring of audio device volume.
    
    You can monitor the volume of audio input or output devices.
    
    For related API, please refer to [startAudioDeviceVolumeMonitor](@startAudioDeviceVolumeMonitor), [stopAudioDeviceVolumeMonitor](@stopAudioDeviceVolumeMonitor)
    
5. Support setting the collection volume of the system sound card.
    
    The volume of the system sound card can be set as required, the effective range is [0, 200], and the default is 100.
    
    For related API, please refer to [setMixSystemPlayoutVolume](@setMixSystemPlayoutVolume)
    
6. Support setting and obtaining audio quality configuration.
...
7. Support for setting the factors of concern that trigger flow control.
...
8. Add local preview first frame rendering callback.
...
9. Media player adds precise search function.
...
10. Support to mute or resume all streams' audio data being playing
...
11. Support to mute or resume all streams' video data being playing
...
1. Optimize the status of the monitoring system's microphone and camera and other hardware device permissions change.
...
2. Add setAudioDeviceVolume interface to set audio device volume
    
    It is supported to set the audio device (speaker or microphone) collection volume through the setAudioDeviceVolume interface before pushing and pulling the stream. However, due to system limitations, this interface call may fail. It is recommended to directly use the setCaptureVolume and setPlayVolume interfaces to adjust the volume of the push and pull stream.
    
    For related API, please refer to [setAudioDeviceVolume](@setAudioDeviceVolume), [getAudioDeviceVolume](@getAudioDeviceVolume), [setCaptureVolume](@setCaptureVolume), [setPlayVolume](@setPlayVolume)
...
1. Fix the problem that Android loses precision when converting long types on 32-bit machines.
...
3. Added desktop sound card capture function `enableMixSystemPlayout` (only supports macOS/Windows)
    
4. Added the support for headphone monitoring. A new API `enableHeadphoneMonitor` was added to enable this feature.
    
5. Added the support for processing encoded data in the custom video capture module. A new API `sendCustomVideoCaptureEncodedData` was introduced to send encoded video data to the SDK.
