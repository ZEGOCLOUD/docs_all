...
Call the [setVoiceChangerPreset](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#set-voice-changer-preset) method to use the SDK's preset voice changer effects.
...
Call [setReverbPreset](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#set-reverb-preset) to set reverb through preset enumeration.
...
If the SDK's preset reverb types cannot meet the requirements, developers can call the [setReverbAdvancedParam](@setReverbAdvancedParam) method in [ZegoReverbAdvancedParam](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_windows~struct~zego-express-zego-reverb-advanced-param) and achieve the reverb effects needed by developers through relevant parameter combinations (for detailed parameter descriptions, please refer to the API documentation).
...
When custom reverb parameters are set, the preset reverb effect set when enabling reverb will become invalid. If you want to use SDK preset parameters again, you can use the [setReverbPreset](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#set-reverb-preset) preset enumeration method to set.
</Warning>

### Reverb Echo

Call the [setReverbEchoParam](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#set-reverb-echo-param) method and achieve the reverb echo effects needed by developers through relevant parameter combinations (for detailed parameter descriptions, please refer to the API documentation).
