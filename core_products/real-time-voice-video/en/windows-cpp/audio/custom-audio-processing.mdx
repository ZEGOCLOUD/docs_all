...
Call the [createEngine ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-zego-express-sdk#create-engine) interface to create an SDK engine instance. For details, please refer to "Create the engine" in [Quick Start - Implementation Process](/real-time-video-windows-cpp/quick-start/implementing-video-call).
...
Call the [setCustomAudioProcessHandler ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#set-custom-audio-process-handler) interface to set the audio custom processing handler, and implement the callback methods: custom audio processing local captured PCM audio frame callback [onProcessCapturedAudioData](@onProcessCapturedAudioData) and custom audio processing remote stream playing PCM audio frame callback [onProcessRemoteAudioData](@onProcessRemoteAudioData). By directly processing the obtained `data` in the callback method, you can implement processing of published and played audio data.
...
- Before starting to publish stream or starting local preview, call the [enableCustomAudioCaptureProcessing ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#enable-custom-audio-capture-processing) interface to enable custom audio processing for local capture. After enabling, developers can receive locally captured audio frames through the [onProcessCapturedAudioData ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-custom-audio-process-handler#on-process-captured-audio-data) callback and can modify the audio data.

- Before starting to play stream, call the [enableCustomAudioRemoteProcessing ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-express-engine#enable-custom-audio-remote-processing) interface to enable custom audio processing for remote stream playing. After enabling, developers can receive remote stream audio frames through [onProcessRemoteAudioData ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP~class~zego-express-i-zego-custom-audio-process-handler#on-process-remote-audio-data) and can modify the audio data.
