...
In multi-room mode, you must use [ZegoPlayerConfig](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~ObjectiveC~class~zego-player-config) to specify the "roomID" associated with "streamID". After exiting the specified room, the stream playing operation for that room will be stopped.
</Warning>
