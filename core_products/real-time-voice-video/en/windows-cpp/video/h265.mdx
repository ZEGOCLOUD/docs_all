...
- 混流：是一种把多路音视频流从云端混合成一路流的技术，支持手动混流、自动混流和全自动混流三种方式，详情请参考 [混流](/real-time-video-windows-cpp/live-streaming/stream-mixing)。
- 转推 CDN：将音视频流从 ZEGO 音视频云推送到 CDN 的过程，详情请参考 [通过 CDN 推流/拉流](/real-time-video-windows-cpp/live-streaming/using-cdn-for-live-streaming)。
...
<Step>
推流前调用 [setVideoConfig](@setVideoConfig) 接口，通过 codecID 设置视频编码格式（若推流后更改配置，需要等下一次推流才生效）。如果设置了 H.265 编码，则：
<Steps>
<Step>
