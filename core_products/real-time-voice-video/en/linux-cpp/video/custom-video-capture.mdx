...
After calling the start preview interface [startPreview](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#start-preview) or start publishing stream interface [startPublishingStream](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#start-publishing-stream), the [onStart](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-custom-video-capture-handler#on-start) callback will be triggered; after stopping publishing/preview, exiting the room, or destroying the engine, the [onStop](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-custom-video-capture-handler#on-stop) callback will be triggered.

The SDK will only accept video data passed by users after [onStart ](@onStart-IZegoCustomVideoCaptureHandler) is triggered and before [onStop ](@onStop-IZegoCustomVideoCaptureHandler) is triggered. Call the custom captured video frame data interface to send video frame data to the SDK. The custom captured video frame data interfaces correspond one-to-one with the video frame data types [bufferType ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~struct~zego-express-zego-custom-video-capture-config#buffer-type) provided to the SDK in [1 Enable Custom Video Capture](/real-time-video-linux-cpp/video/custom-video-capture):

|Video Frame Type|bufferType|Send Video Frame Data Interface|
|-|-|-|
|Raw data type|ZEGO_VIDEO_BUFFER_TYPE_RAW_DATA|[sendCustomVideoCaptureRawData](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#send-custom-video-capture-raw-data) |
|Encoded type| ZEGO_VIDEO_BUFFER_TYPE_ENCODED_DATA|[sendCustomVideoCaptureEncodedData](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#send-custom-video-capture-encoded-data) |
