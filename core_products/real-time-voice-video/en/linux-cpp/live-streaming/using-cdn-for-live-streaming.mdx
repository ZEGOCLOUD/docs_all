...
After `publishing stream successfully`, call the [addPublishCdnUrl](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#add-publish-cdn-url) interface to add the URL for dynamic relaying to CDN, so that the audio/video stream that has been successfully published to ZEGO real-time cloud can be dynamically relayed to third-party CDN.

<Note title="Note">


* If developers need to relay to multiple third-party CDN vendors, they can call [addPublishCdnUrl](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#add-publish-cdn-url) multiple times using the same stream ID (URLs need to be different).
...
* If developers relay to multiple third-party CDNs, they can obtain status change notifications for each relayed stream from the list parameter of the CDN callback status notification [onPublisherRelayCDNStateUpdate](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-event-handler#on-publisher-relay-cdn-state-update).
...
Developers can register [onPublisherRelayCDNStateUpdate](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~cpp_linux~class~IZegoEventHandler#on-publisher-relay-cdn-state-update) to get callbacks for adding/deleting relay to CDN URL status. After ZEGO RTC server relays the audio/video stream to CDN, if the CDN relay status changes, such as relay stopping or relay retry, this callback will be received.
...
Call [removePublishCdnUrl](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#remove-publish-cdn-url) to delete the URL for dynamic relaying to CDN. **When calling the [removePublishCdnUrl](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#remove-publish-cdn-url) interface to stop relaying, please ensure that the current streamID exists.**
