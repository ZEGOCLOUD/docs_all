...
Call the [createEngine](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-zego-express-sdk#create-engine) interface to create an SDK engine instance. For details, please refer to "Create Engine" in [Quick Start - Implementation Flow](/real-time-video-linux-cpp/quick-start/implementing-video-call).
...
Call the [setCustomAudioProcessHandler](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#set-custom-audio-process-handler) interface to set the audio custom processing object, and implement callback methods: custom audio processing local captured PCM audio frame callback [onProcessCapturedAudioData](@onProcessCapturedAudioData) and custom audio processing remote played stream PCM audio frame callback [onProcessRemoteAudioData](@onProcessRemoteAudioData). In the callback method, directly processing the passed `data` can achieve processing of publishing and playing stream audio data.
...
- Before starting publishing stream or starting local preview, call the [enableCustomAudioCaptureProcessing](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#enable-custom-audio-capture-processing) interface to enable local capture custom audio processing. After enabling, developers can receive locally captured audio frames through the [onProcessCapturedAudioData](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-custom-audio-process-handler#on-process-captured-audio-data) callback and modify the audio data.

- Before starting playing stream, call the [enableCustomAudioRemoteProcessing](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#enable-custom-audio-remote-processing) interface to enable remote playing stream custom audio processing. After enabling, developers can receive remote playing stream audio frames through [onProcessRemoteAudioData](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-custom-audio-process-handler#on-process-remote-audio-data) and modify the audio data.
