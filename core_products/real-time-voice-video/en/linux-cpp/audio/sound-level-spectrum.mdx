...
- Interface prototype:
    ...
    * @param audioSpectrum Locally captured audio spectrum value array, spectrum value range is [0-2^31]
    *
    */
    ...
    * @param audioSpectrums Remote audio spectrum key-value pairs, key is stream ID, value is the audio spectrum value array of the corresponding stream, spectrum value range is [0-2^31]
    *
    */
...
The local captured callback will be triggered immediately, and the callback value is 0 when not publishing stream; the remote playing stream callback will only be triggered after successfully playing stream [startPlayingStream](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#start-playing-stream).
