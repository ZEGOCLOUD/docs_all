...
The local captured callback will be triggered immediately, and the callback value is 0 when not publishing stream; the remote playing stream callback will only be triggered after successfully playing stream [startPlayingStream](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#start-playing-stream).
