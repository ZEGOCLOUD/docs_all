...
Call the [setVoiceChangerPreset](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#set-voice-changer-preset) method to use SDK's preset voice changer effects.
...
If the SDK's preset voice changer effects cannot meet requirements, developers can call the [ZegoVoiceChangerParam](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~struct~zego-express-zego-voice-changer-param#zego-voice-changer-param) method and set custom voice changer through the pitch parameter `pitch`. The value range of this parameter is [-12.0, 12.0]. The larger the value, the sharper the sound. The default value is "0.0" (i.e., no voice changer).
...
Call [setReverbPreset](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#set-reverb-preset) to set reverb through preset enumeration.
...
If the SDK's preset reverb types cannot meet requirements, developers can call the [ZegoReverbAdvancedParam](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~struct~zego-express-zego-reverb-advanced-param#zego-reverb-advanced-param) method and set relevant parameters in combination to achieve the reverb effects developers need (for detailed parameter descriptions, please refer to the API documentation).
...
After setting custom reverb parameters, the preset reverb effect set when enabling reverb will become invalid. If you want to use SDK preset parameters again, you can use the [setReverbPreset](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#set-reverb-preset) preset enumeration method to set.
...
Call the [setReverbEchoParam](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#set-reverb-echo-param) method and set relevant parameters in combination to achieve the reverb echo effects developers need (for detailed parameter descriptions, please refer to the API documentation).
...
If you need to enable virtual stereo functionality, you must call the [setAudioConfig](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#set-audio-config) method before publishing stream to set audio encoding channels to Stereo dual channel (default is Mono single channel).

The following example uses preset enumeration to construct [ZegoAudioConfig](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~struct~zego-express-zego-audio-config#zego-audio-config) and sets it to dual channel:
...
After setting audio encoding channels to dual channel, call the [enableVirtualStereo](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#enable-virtual-stereo) method and set the sound source angle of virtual stereo through the `enable` parameter to have stereo effect. The angle range is 0 ~ 180. Generally, it can be set to 90 degrees (i.e., straight ahead).
