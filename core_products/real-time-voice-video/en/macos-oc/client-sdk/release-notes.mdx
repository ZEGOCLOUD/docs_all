...
2. onPublisherQualityUpdate callback provides traffic control status information
...
3. setDummyCaptureImagePath supports passing network image addresses
    
    For related API, please refer to [setDummyCaptureImagePath](@setDummyCaptureImagePath)
    
4. When PC captures system sound card, it can be used as an independent source for streaming
...
For related API, please refer to [enableAudioCapture](@enableAudioCaptureWithConfig), [setAudioSource](@setAudioSource)
...
7. The desktop supports setting pictures as custom video collection sources.
...
2. Added a screen sharing exception callback with source type
    ...
    When the screen sharing source (such as a window or screen area) encounters an exception, the new callback (Objetive-C interface: [screenCapture:captureType:exceptionOccurred](/article/api?doc=Express_Video_SDK_API~objectivec_macos~class~ZegoScreenCaptureSourceEventHandler#screen-capture-capture-type-exception-occurred); C++ interface: [onCaptureTypeExceptionOccurred](/article/api?doc=Express_Video_SDK_API~cpp_macos~class~IZegoScreenCaptureSourceEventHandler#on-capture-type-exception-occurred)) will return the specific exception source type to help developers quickly locate the problem scenario.
...
3. Custom Rendering Supports Outputting RGBA Data
...
5. Custom video capture supports developer control of external timestamps
...
1. Fixed occasional freezing issues when destroying the engine on macOS platform
...
2. Fixed the issue of macOS throwing a status error when disconnecting the currently used media device.
...
1. Fixed the issue of playback problems when hardware decoding CDN live video streams containing B-frames.
...
3. Fixed the occasional playback failure issue when playing network materials with the media player.
...
2. Fix known compatibility issues and null pointer problems.
...
1. The virtual background of subject segmentation supports video materials.
...
4. Custom video capture supports transparent channel transmission.
...
1. ScreenCapture supports setting publishing-stream area.
...
7. Object-segmentation supports Intel devices to expand device coverage capabilities.
...
5. Supports self-maintaining reference signal for AEC.
...
1. Starting from version 3.5.0, support for macOS 10.13 and below has been discontinued, and the macOS Deployment Target (minimum supported version) has been upgraded to macOS 10.13.
...
3. State synchronization and active streaming support for audio and video over a 10000 person range.
...
5. Support for platform based switch body segmentation function.
...
6. Support dynamic switching of flow control strategies
...
5. Support for obtaining the codec capability of the specified video codec of the current device
...
7. New video first frame callback based on camera opening
...
1. Fixed an issue where a crash could occur when repeatedly start publishing or playing stream when using the [ZegoScenarioCommunication] scenario.
...
2. Support binaural capture.
    
    Supports dual-channel internal capture of external audio devices. The audio capture channel mode can be set through the [setAudioCaptureStereoMode] interface. When developers enable dual-channel capture, they can use a dual-channel audio device to capture dual-channel audio. audio data and push it.
    
    This function is often used in scenarios where the user is more sensitive to the effect of sound, such as voice radio, musical instrument performance.
    
    For related API, please refer to [setAudioCaptureStereoMode](@setAudioCaptureStereoMode)
    
3. Pull stream quality callback [onPlayerQualityUpdate] added MOS sound quality score.
...
4. Support ROI (region of interest) intelligent video coding, improve the quality of the specified area.
...
1. Fixed the crash issue of the iPad App when starting and stopping the camera on M1 chip (Apple Silicon) model.
    
2. Fixes the problem that [onDeviceError] may repeat callbacks.
...
1. Starting from version 2.1.0, the delivery product is [Normal Framework] + [XCFramework]. From this version, it is changed to deliver only [XCFramework].
...
2. Support mute or unmute audio devices.
...
3. Supports monitoring of audio device volume.
...
4. Support setting the collection volume of the system sound card.
...
5. Support setting and obtaining audio quality configuration.
...
6. Support for setting the factors of concern that trigger flow control.
...
7. Add local preview first frame rendering callback.
...
8. Media player adds precise search function.
...
9. Support to mute or resume all streams' audio data being playing
...
10. Support to mute or resume all streams' video data being playing
