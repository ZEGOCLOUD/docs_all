...
After publishing the stream, call the [startMixingAudio](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#start-mixing-audio) method to start audio mixing, and mix audio into the stream being published through `streamID` and `mediaList` (media tag list: `video` or `audio`).
...
Call the [stopMixingAudio](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#stop-mixing-audio) method to stop audio mixing.
