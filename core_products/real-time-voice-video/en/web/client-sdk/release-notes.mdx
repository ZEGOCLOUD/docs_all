...
1. Added Support for Audio and Video Calls in Chrome Picture-in-Picture Mode
    ...
    For details, please refer to the [Video Picture-in-Picture Solution](/article/18155).
...
4. Added WebRTC support for H265
    
    WebRTC has newly added support for H265. Currently, only higher versions of browsers support it (for example, Chrome 136 and above). You can use the [checkSystemRequirements](@checkSystemRequirements) interface to determine whether the current browser supports the H265 function.
    
    For related APIs, please refer to: [checkSystemRequirements](@checkSystemRequirements)
...
1. Optimized Stuttering Rate Calculation
    
    Optimized the calculation rules for stuttering rate. Stuttering caused by B-frames generated on the streaming side will no longer be counted into the playback stuttering duration, avoiding confusion with network-induced stuttering.
...
2. Support for single-stream transcoding feature
...
3. Fixed the occasional issue of encoding frame rate configuration not taking effect.
4. Fixed the issue where users did not receive the roomStateChanged callback when exiting a room they were not logged into.
...
1. Added callback for updating audio and video tracks of playing stream
    
    Added callback for updating audio track ([playerAudioTrackUpdate](@playerAudioTrackUpdate)) and video track ([playerVideoTrackUpdate](@playerVideoTrackUpdate)) of playing stream. Use this callback to obtain the audio and video tracks of the playing stream and perform audio and video rendering processing as needed.
    
<Note title="Note">

If there is no special requirement for audio and video rendering, it is recommended to use [ZegoStreamView](@-ZegoStreamView) for rendering in most scenarios.
...
1. Fixed the issue where the video screen of the playing stream would turn black and cannot be restored when using the `video` tag to render media streams, and then switching off the camera ([enableVideoCaptureDevice](@enableVideoCaptureDevice)) or video ([mutePublishStreamVideo](@mutePublishStreamVideo)) at the publishing end of the stream.
2. Fixed the issue where the callback for face beautification was triggered before the output of the face beautification video track when successfully enabling face beautification using [setEffectBeauty](@setEffectBeauty).
...
1. Support for quick room switching
    ...
    - Switching rooms will automatically stop all streaming and playback in the current room, but preview will not be stopped. If you want to stop preview, you can call [destroyStream](@destroyStream) to stop preview. If you don't want to stop playback, you can contact ZEGOCloud technical support for settings.
...
Before publishing, you can enable traffic control by using [ZegoWebPublishOption > enableTrafficControl](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPublishOption#enable-traffic-control) to adapt to the current network environment and fluctuations, and achieve adaptive bitrate to ensure smooth audio and video streaming. You can also set the minimum video bitrate by using [ZegoWebPublishOption > trafficControlMinVideoBitrate](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPublishOption#traffic-control-min-video-bitrate). 

Please refer to the related API for more information: [enableTrafficControl](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPublishOption#enable-traffic-control), [trafficControlMinVideoBitrate](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPublishOption#traffic-control-min-video-bitrate)
...
During RTC playback, you can enable ultra-low latency playback by setting the [ZegoWebPlayOption > enableLowLatency](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#enable-low-latency) parameter. When ultra-low latency playback is enabled, low latency is prioritized, but there may be buffering in weak network conditions.

Please refer to the related API for more information: [ZegoWebPlayOption > enableLowLatency](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#enable-low-latency)  

4. Support for setting playback buffer time
    
    In scenarios with poor network conditions, you can set the [ZegoWebPlayOption > jitterBufferTarget](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#jitter-buffer-target) parameter to set the audio and video playback delay buffer time for playback, reducing buffering.
    - This parameter is invalid when [ZegoWebPlayOption > enableLowLatency](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#enable-low-latency) is set to `true`.
    - This feature will increase latency, so please use it with caution according to your business scenario.
    
    Please refer to the related API for more information: [ZegoWebPlayOption > jitterBufferTarget](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#jitter-buffer-target) 
...
5. Support for customizing the resource type for audience before and after going on stage
    ...
    Please refer to the related API for more information: [startPlayingStream](@startPlayingStream), [ZegoWebPlayOption > customResourceConfig](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#custom-resource-config)
...
7. New error codes for authentication failure in publishing and playback
    ...
    New error codes for authentication failure in playback (1104046) and publishing (1103019). If the [ZegoWebPublishOption > streamParams](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPublishOption#stream-params) parameter is set incorrectly when enabling publishing or playback authentication, the authentication will fail. For more details, please refer to the [Error Code document](./error-code/error-code.mdx).
    
**Deprecated**

1. Deprecated [playoutDelayHint](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#playout-delay-hint) parameter of the [startPlayingStream](@startPlayingStream) playback interface
    
    The [playoutDelayHint](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#playout-delay-hint) parameter is deprecated. Please use the [ZegoWebPlayOption > jitterBufferTarget](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#jitter-buffer-target) parameter to set the audio and video playback delay for playback.
...
1. Support for setting display timing of image resources in mixing streams
    
    Note: To display image resources, make sure to provide a valid image URL in [ZegoMixerImageInfo > url](@url-ZegoMixerImageInfo).
    
    When setting image content for mixing streams, you can now set the display timing of the image using [ZegoMixerImageInfo > displayMode](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoMixerImageInfo#display-mode). The `displayMode` options are as follows:
    - Value 0: Default value. When [ZegoMixerImageInfo > url](@url-ZegoMixerImageInfo) is not empty, the image will be displayed.
    - Value 1: Reference the status of the camera switch. If the camera is turned off, the image will be displayed; otherwise, the video will be displayed.
    - Value 2: Reference the availability of video data in the input stream. If there is no video data for 3 consecutive seconds, the image will be displayed; otherwise, the video will be displayed. The default duration for determining empty video data is 3 seconds. If you need to configure it differently, please contact ZEGO technical support.
    
    For related APIs, please refer to [ZegoMixerImageInfo > displayMode](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoMixerImageInfo#display-mode)
...
1. Independent Control of Stream Auditing When Audit Service is Enabled
    
    When the audit service is enabled, by default, all streams in the room will be audited. If you need to control whether a specific stream is audited individually, you can call the [startPublishStream](@startPublishStream) interface on the client side to start the stream, and control whether this stream is audited by the audit flag parameter `inspectFlag` (0: allowed, default; 1: not allowed).
    
    Refer to the related API [startPublishStream](@startPublishStream).
    
2. New Automatic Mixing Stream Feature
...
3. New Feature Configuration and Status Callback for Mixing Stream Feature
...
2. Fixed the issue with the [checkSystemRequirements](@checkSystemRequirements) interface, which provided inaccurate results for the capability check of video encoding.
...
1. Screen sharing supports video footage sharing only a certain DOM element in the tab.
    
    - Only Chrome 104 or above browsers are supported; if the shared DOM elements contain multimedia elements (img, video, etc.), Chrome 116 or above browsers are required. For a better experience, it is recommended that you use the latest version of Chrome directly.
    - It is recommended that you use CSS to fix the position of the DOM element to prevent the size of the DOM in the viewport from changing when the browser page is scrolled, causing frequent changes in the resolution of the captured video and causing play-stream decoding failure on some Android devices.
    
    For related APIs, please refer to [createZegoStream](@createZegoStream), [startCaptureScreen](@startCaptureScreen), [startCaptureScreenWithAudio](@startCaptureScreenWithAudio)
    
2. Supports the introduction of functions of each module of the SDK on demand.
...
4. Mixed flow task supports input of super whiteboard information.
    ...
    For related APIs, please refer to [startMixerTask](@startMixerTask)、[ZegoMixStreamConfig](@-ZegoMixStreamConfig)
    
5. Mobile devices support quick switching between front and rear cameras.
    
    Newly added [useFrontCamera](@useFrontCamera) interface, which is used to switch between the front and rear cameras on mobile devices. Compared to the previous version's mode of switching cameras using facingMode, the interface call is simpler.
    
    For related API, please refer to [useFrontCamera](@useFrontCamera)
...
4. Optimize scenario-based audio and video configuration [ZegoScenario](@-ZegoScenario) to improve user experience.
...
6. Optimize video's SEI
    ...
    For related APIs, please refer to  [sendSEI](@sendSEI)、[playerRecvSEI](@playerRecvSEI)
...
- The old version of [createStream](@createStream) and a series of interfaces will be officially abandoned in version 3.0.0. Please update your code logic in time to avoid affecting the normal use of your business. For details, please refer to [Upgrade guide v3.0.0+](./upgrade-guide/upgrade-to-v3.mdx).
...
2. This function is recommended to be used in conjunction with `stream-compositor`.
    ...
    For related APIs, please refer to [setTransparentBackgroundOptions](@setTransparentBackgroundOptions)
...
1. Fix the problem that when using the cloud recording function, the SDK's room user update callback [roomUserUpdate](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoRTMEvent#room-user-update) will throw unexpected cloud recording robot user information
2. Fix other known issues.
...
2. Fix the problem that the SDK calculates the validity period of the Token incorrectly due to the wrong system time, thus triggering the [tokenWillExpire](@tokenWillExpire) callback.
...
1. Add background blur and virtual background function
    ...
    For related APIs, please refer to [initBackgroundModule](@initBackgroundModule), [setBackgroundBlurOptions](@setBackgroundBlurOptions), [setVirtualBackgroundOptions](@setVirtualBackgroundOptions), [enableBackgroundProcess](@enableBackgroundProcess)
...
5. The web side supports triggering the [networkQuality](@networkQuality) event, which is used to obtain the push-pull streaming quality of local users and remote users.
...
1. Support the function of adding or deleting audio and Video tracks after creating a stream.
    ...
    For related APIs, please refer to [addTrack](@addTrack), [removeTrack](@removeTrack)
...
2. When creating a Screen Sharing stream, you can enable 3A configuration through parameters.
    ...
    When creating a Screen Sharing stream, you can enable 3A configuration through the `ANS`, `AGC` and `AEC` parameters of the [createStream](@createStream) interface. When creating Audio for a Screen Sharing stream, the default 3A configuration only has AEC turned on, and AGC and noise cancellation processing turned off.
    
    For related APIs, please refer to [createStream](@createStream)
...
2. Support to enable Video hardware encoding function
    ...
    A new interface for enabling Video hardware encoding [enableHardwareEncoder](@enableHardwareEncoder) has been added. Developers can enable this configuration to improve encoding efficiency.
    
    Developers can choose whether to enable this switch according to the support of the model, and can check the [ZegoPublishVideoStats.isHardwareEncode](@publishQualityUpdate) logo through the publish streams quality interface [publishQualityUpdate](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoPublishVideoStats#is-hardware-encode) to confirm the current hardware encoding is enabled or off state.
    ...
    For related APIs, please refer to [enableHardwareEncoder](@enableHardwareEncoder), [ZegoPublishVideoStats.isHardwareEncode](/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoPublishVideoStats#is-hardware-encode)
...
3. When the SDK implements the [Range Audio](../communication/range-audio.mdx) function, there is no sound or noise in some devices or browsers when play streams and switching to the background.
