...
[ZegoMixerTask](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-entity-zego-mixer-task) is the mixing task configuration object defined in ZegoExpressEngine SDK, which includes information such as input streams and output streams.

<a name="5_2_1"></a>

#### Create Mixing Task Object

Create a new mixing task object through the constructor [ZegoMixerTask ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-entity-zego-mixer-task#zego-mixer-task), then call instance methods to set input, output and other parameters respectively.
...
The default value of audio bitrate [bitrate](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoMixerAudioConfig#bitrate) is 48 kbps.
...
According to actual business scenarios, define the input video stream [ZegoMixerInput](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-entity-zego-mixer-input) list, set the "layout" parameter of each video stream to layout the image of each input stream, and the ZEGOCLOUD server will mix the input streams and output the mixed stream in one image.
...
After completing the configuration of the [ZegoMixerTask](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-entity-zego-mixer-task) mixing task object, call the start mixing interface to start this mixing task, and handle the logic of starting mixing task failure in the callback.
...
When mixing information changes, such as adding or removing input streams in the mixing list, adjusting mixing video output bitrate, etc., modify the parameters of this mixing task object, then call the [startMixerTask](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#start-mixer-task) interface once to update the configuration.
...
1. **Can I push mixed streams to third-party CDNs? How to relay to multiple CDNs?**
  ...
  If you need to push mixed streams to third-party CDNs, you can fill in the CDN URL in the "target" parameter of [ZegoMixerOutput](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-entity-zego-mixer-output).

  The filled URL format needs to be in RTMP format: "rtmp://xxxxxxxx".

  To push to multiple CDNs, create N output stream objects [ZegoMixerOutput](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-entity-zego-mixer-output) and put them in the "outputList" output list in [ZegoMixerTask](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-entity-zego-mixer-task).

2. **How to set the layout of each stream in the mix?**

  Example of using the "layout" parameter of [ZegoMixerInput](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-entity-zego-mixer-input).
    ...
    - Assume the resolution "resolution" in the "videoConfig" parameter of [ZegoMixerTask](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-entity-zego-mixer-task) is "CGSizeMake(375, 667)".
...
3. **When the aspect ratio of the "ZegoRect" layout of the mixing input object "ZegoMixerInput" does not match the resolution of the stream itself, how will the image be cropped?**
  ...
  The SDK will perform proportional scaling. Assume an input stream has a resolution of "720 × 1280", that is, an aspect ratio of "9:16", and the "layout" parameter of this stream's [ZegoMixerInput](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-entity-zego-mixer-input) is "[left:0 top:0 right:100 bottom:100]", that is, an aspect ratio of "1:1", the image will display the middle part of this stream, that is, the top and bottom parts are cropped.
