...
<Note title="说明">
- 使用 [setLowlightEnhancement](@setLowlightEnhancement) 接口前，需确保 SDK 包含**视频内部采集**与**视频内部滤镜**模块。
- 若业务采用外部采集视频流，仅支持特定数据类型 `PIXEL_BUFFER_TYPE_GL_TEXTURE_2D` 和 `PIXEL_BUFFER_TYPE_MEM`。
</Note>
...
| ZegoExpLowlightEnhancementType.AI | AI 低照度增强。<Note title="说明">如需要使用此功能，需要联系 ZEGO 技术支持。</Note>|
...
<Warning title="注意">
- 使用 [setVideoDenoiseParams](@setVideoDenoiseParams) 接口前，需确保 SDK 包含**视频内部采集**与**视频内部滤镜**模块。
- 若业务采用外部采集视频流，仅支持特定数据类型 `PIXEL_BUFFER_TYPE_GL_TEXTURE_2D` 和 `PIXEL_BUFFER_TYPE_MEM`。
