...
2. Copy the "release/Library/ZegoExpressEngine.aar" in the decompressed AAR file to your project directory, such as "app/libs".
    ...
    - Other files: for your use when using [Option 3: Manual integration by copying SDK JAR + SO files](/article/15545#Integration_3).
...
1. Download the latest version of SDK. For details, see [SDK downloads (#3690).
...
call the [`createEngineWithProfile`(/article/api?doc=express-video-sdk_API~java_android~class~ZegoExpressEngine#create-engine-with-profile-event-handler) method to initialize the Video Call SDK.  And config the following:
...
To start the local video preview and to render it, call the [`startPreview`](/article/api?doc=express-video-sdk_API~java_android~class~ZegoExpressEngine#start-preview) method.
...
You can call [`startPlayingStream`](/article/api?doc=express-video-sdk_API~java_android~class~ZegoExpressEngine#start-playing-stream-canvas) method to start playing a remote video stream.
