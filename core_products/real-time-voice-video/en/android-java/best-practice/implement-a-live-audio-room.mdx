...
The usage of basic SDK functions has been introduced by [Quick start](!Quick_start_Integrate_Implementation). If you are not familiar with the concept of stream publishing/playing, please read the document carefully again.
...
In addition to implementing the above logic, the live audio room also needs to manage speaker seats. The speaker seat management function can usually be implemented using the [room attribute](#14207) feature of the ZIM SDK.
...
The ZIM SDK provides rich functionality for sending and receiving messages, see [Send & Receive messages (roomrequest)](!zim-sendMessages). And here, you will need to use the customizable roomrequest message: `ZIMCommandMessage`
...
For details, refer to [Visualize the sound level](!ExpressVideoSDK-AudioAdvanced/SoundLevelSpectrum).
