...
* @param errorCode Error code, please refer to common error codes: /real-time-video-android-java/client-sdk/error-code#1006xxx-device-error.
*/
...
Call the [startPreview](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-zego-express-engine&jumpType=route#start-preview) interface to bind the camera preview view and start video capture and preview without publishing stream.
...
If you can hear the corresponding audio, the playback device is normal and can be used for calls. Call the [onMediaPlayerStateUpdate](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-callback-i-zego-media-player-event-handler&jumpType=route#on-media-player-state-update) callback to view the player status callback:
...
* @param errorCode Error code, for details please refer to the common error codes document: /real-time-video-android-java/client-sdk/error-code#1006xxx-device-error.
*/
