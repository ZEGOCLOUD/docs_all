...
- Scenario 1: User A both publishes and pulls streams. By pulling A's streams from the ZEGOCLOUD server, they can get User A's upstream and downstream network quality; User A will also receive callbacks of its own upstream and downstream network quality.
- Scenario 2: User B only publishes streams and does not pull streams. Since there is no stream pulling, the local downstream network quality cannot be calculated. Therefore, by pulling B's streams from the ZEGOCLOUD server, remote users can get User B's upstream network quality, while the downstream network quality is unknown, so the callback is unknown (enum value 5). In this scenario, User B will only receive callbacks of its own upstream network quality, and the downstream network quality callback is unknown (enum value 5).
- Scenario 3: User C only pulls streams and does not publish streams. Since there is no stream publishing, the local network condition cannot be broadcast. Therefore, remote users cannot get User C's upstream or downstream network quality. In this scenario, User C will only receive callbacks of its own downstream network quality, and the upstream network quality callback is unknown (enum value 5).
