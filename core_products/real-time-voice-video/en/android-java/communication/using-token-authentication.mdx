...
2. Integrate the Video Call SDK and implement the basic video feature. For details, refer to [Quick start](!Quick_start_Integrate_Implementation).
...
:::if{props.platform="swift"}
```swift
let roomID: String = "xxx" // The room ID of the room to log in to.
let user = ZegoUser(userID: "xxxx")
let config = ZegoRoomConfig()
config.token = "xxxxxxx" // The Token you get from your app server.
...
```cpp
std::string roomID = 'xxx'; // The room ID of the room to log in to.
ZegoUser user;
user.userID = 'xxxx';
user.userName = 'xxxx';
...
let loginResult = await zg.loginRoom(roomID, token, user) //login room
```
:::
...
:::if{props.platform="swift"}
```swift
let token: String = MyToken.getToken()// Request a new Token from app server.
...
[[ZegoExpressEngine sharedEngine] renewToken:
...
30 seconds before a Token expires, the SDK sends out a notification through the {props.platform === "web" ? <a target="_blank" href="@tokenWillExpire">tokenWillExpire</a> : <a target="_blank" href="@onRoomTokenWillExpire">onRoomTokenWillExpire</a>} callback.
