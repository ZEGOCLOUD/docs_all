...
<Note title="Note">
For related concepts and principles of SEI, please refer to [How to understand and use SEI (Supplemental Enhancement Information)](/faq/media-sei-usage).
</Note>
...
1. Call the [createEngine ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#create-engine) interface to create an engine object.

2. Call the [loginRoom ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#login-room) interface to log in to the room.

3. Call the [startPublishingStream ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#start-publishing-stream) interface to publish stream.

4. After successful publishing, call the [sendSEI ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#send-sei) interface to send SEI information.

Play stream end:

1. Call the [createEngine ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#create-engine) interface to create an engine object.

2. Create an [IZegoEventHandler ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler) object and override the [onPlayerRecvSEI ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler#on-player-recv-sei) method for receiving SEI information. Call the [setEventHandler](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#set-event-handler) interface and pass in the created [IZegoEventHandler ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler) to listen to the [onPlayerRecvSEI ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler#on-player-recv-sei) callback.

3. Call the [loginRoom ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#login-room) interface to log in to the room.

4. Call the [startPlayingStream ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#start-playing-stream) interface to play stream.

5. After successfully playing stream, when receiving SEI information sent by the publish stream end, the [onPlayerRecvSEI ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler#on-player-recv-sei) callback will be triggered.
...
Since the SDK uses ZEGO's self-defined SEI (nalu type = 6, payload type = 243) type by default for packaging, and this type is not specified in the SEI standard, it does not conflict with SEI in video encoders or video files. However, when developers need to use third-party decoders to decode (such as FFmpeg), it will result in inability to decode the correct SEI. At this time, before publishing stream, you need to call the [setSEIConfig ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-zego-express-engine#set-sei-config) interface to change the SEI type sent by the SDK, using UserUnregister's SEI (nalu type = 6, payload type = 5) type for packaging.
<Note title="Note">Only when developers use third-party decoders to decode SEI, this step needs to be performed.</Note>
```java
...
[How to understand and use SEI (Supplemental Enhancement Information)?](/faq/media-sei-usage)
