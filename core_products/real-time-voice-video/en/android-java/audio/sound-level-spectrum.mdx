...
- Interface prototypes
    ...
    - Local captured audio spectrum callback interface [onCapturedAudioSpectrumUpdate](@onCapturedAudioSpectrumUpdate):
         ...
         *
         * Callback notification period is 100 ms.
         * @param audioSpectrum Array of local captured audio spectrum values, spectrum value range is [0-2^31]
         */
    ...
    - Remote play stream audio spectrum callback interface [onRemoteAudioSpectrumUpdate](@onRemoteAudioSpectrumUpdate):
         ...
         *
         * Callback notification period is 100 ms.
         * @param audioSpectrums Key-value pairs of remote audio spectrums, key is stream ID, value is the array of audio spectrum values of the corresponding stream, spectrum value range is [0-2^31]
         */
...
- Call the [startSoundLevelMonitor](@startSoundLevelMonitor) interface to start sound level monitoring:
    ...
    In version 1.15.0 and above, when calling the [startSoundLevelMonitor](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#start-sound-level-monitor-1) interface, you can set the `millisecond` parameter to adjust the sound level callback interval.
    ```java
    ...
    After calling the above interface, [onCapturedSoundLevelUpdate ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler#on-captured-sound-level-update) will be triggered immediately, the callback value is 0 when not publishing and not previewing; [onRemoteSoundLevelUpdate ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler#on-remote-sound-level-update) will only callback after starting to play stream [startPlayingStream ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#start-playing-stream).
...
- Call the [startAudioSpectrumMonitor](@startAudioSpectrumMonitor) interface to start audio spectrum monitoring:
    ...
    After calling the above interface, [onCapturedAudioSpectrumUpdate ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler#on-captured-audio-spectrum-update) will be triggered immediately, the callback value is 0 when not publishing and not previewing; [onRemoteAudioSpectrumUpdate ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler#on-remote-audio-spectrum-update) will only callback after starting to play stream [startPlayingStream ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#start-playing-stream).
...
- Call the [stopSoundLevelMonitor](@stopSoundLevelMonitor) interface to stop sound level monitoring:
    ...
    After calling the above interface, [onCapturedSoundLevelUpdate ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler#on-captured-sound-level-update) and [onRemoteSoundLevelUpdate ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler#on-remote-sound-level-update) will no longer callback.
...
- Call the [stopAudioSpectrumMonitor](@stopAudioSpectrumMonitor) interface to stop audio spectrum monitoring:
    ...
    After calling the above interface, [onCapturedAudioSpectrumUpdate ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler#on-captured-audio-spectrum-update) and [onRemoteAudioSpectrumUpdate ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-event-handler#on-remote-audio-spectrum-update) will no longer callback.
...
- [How to handle no sound issue?](/faq/no-audio-issue)
- [How to handle low volume issue?](/faq/low-audio-volume-issue)
