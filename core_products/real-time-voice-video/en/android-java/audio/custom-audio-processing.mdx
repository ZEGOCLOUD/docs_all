...
Call the [createEngine ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#create-engine) interface to create an SDK engine instance. For details, please refer to "Create Engine" in [Quick Start - Implementation](/real-time-video-android-java/quick-start/implementing-video-call).
...
Call the [setCustomAudioProcessHandler](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#set-custom-audio-process-handler) interface to set the audio custom processing handler, and implement the callback methods: custom audio processing local captured PCM audio frame callback [onProcessCapturedAudioData](@onProcessCapturedAudioData) and custom audio processing remote playing stream PCM audio frame callback [onProcessRemoteAudioData](@onProcessRemoteAudioData). Directly process the obtained `data` in the callback method to implement processing of publishing and playing stream audio data.
...
- Before starting publishing stream or starting local preview, call the [enableCustomAudioCaptureProcessing ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#enable-custom-audio-capture-processing) interface to enable **local captured** custom audio processing. After enabling, developers can receive locally captured audio frames through the [onProcessCapturedAudioData ](@onProcessCapturedAudioData) callback and modify the audio data.

- Before starting playing stream, call the [enableCustomAudioRemoteProcessing ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#enable-custom-audio-remote-processing) interface to enable **remote playing stream** custom audio processing. After enabling, developers can receive remote playing stream audio frames through [onProcessRemoteAudioData ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-callback-i-zego-custom-audio-process-handler#on-process-remote-audio-data) and modify the audio data.
