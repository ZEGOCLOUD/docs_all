...
Call the [setVoiceChangerPreset ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#set-voice-changer-preset) method to use the SDK's preset voice changer effects.
...
If the SDK's preset voice changer effects cannot meet your needs, developers can call the [setVoiceChangerParam](@setVoiceChangerParam) method in [ZegoVoiceChangerParam ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-entity-zego-voice-changer-param) to set custom voice changer through the pitch parameter "pitch". The value range of this parameter is [-12.0, 12.0]. The larger the value, the sharper the sound. The default value is "0.0" (i.e., no voice changer).
...
If the SDK's preset reverb types cannot meet your needs, developers can call the [setReverbAdvancedParam](@setReverbAdvancedParam) method in [ZegoReverbAdvancedParam ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java_android~class~im-zego-zegoexpress-entity-zego-reverb-advanced-param) to achieve the required reverb effect through the combination of relevant parameters (for detailed parameter description, please refer to the API documentation).
...
After setting custom reverb parameters, the preset reverb effect set when enabling reverb will become invalid. If you want to use SDK preset parameters again, you can use the [setReverbPreset ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~java_android~class~ZegoExpressEngine#set-reverb-preset) preset enumeration method to set.
</Warning>
...
If you need to enable the virtual stereo function, you must first call the [setAudioConfig ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#set-audio-config) method to set the audio encoding channel to Stereo dual-channel (default is Mono mono-channel) before publishing stream.

This example uses preset enumeration to construct [ZegoAudioConfig ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-entity-zego-audio-config) to set to dual-channel.
...
After setting the audio encoding channel to dual-channel, call the [enableVirtualStereo ](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~Java~class~im-zego-zegoexpress-zego-express-engine#enable-virtual-stereo) method, enable virtual stereo through the "enable" parameter, and set the sound source angle of virtual stereo through the "angle" parameter to achieve the stereo effect. The angle range is 0 to 360, generally can be set to 90 degrees (i.e., directly in front).
