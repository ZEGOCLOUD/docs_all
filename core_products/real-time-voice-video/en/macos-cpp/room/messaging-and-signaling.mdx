...
1. **Send Broadcast Message**
    ...
    Call the [sendBroadcastMessage](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#send-broadcast-message) interface to send broadcast messages to other users in the same room. The length cannot exceed 1024 bytes.
...
1. **Send Barrage Message**
    ...
    Call the [sendBarrageMessage](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#send-barrage-message) interface to send barrage messages to other users in the same room. The length cannot exceed 1024 bytes.
...
1. **Send Custom Signaling**
    ...
    Call the [sendCustomCommand](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_windows~class~zego-express-i-zego-express-engine#send-custom-command) interface to send custom signaling to users specified by "toUserList" in the same room. The length cannot exceed 1024 bytes.
