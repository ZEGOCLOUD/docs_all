...
Call the [startPreview](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-express-engine&jumpType=route#start-preview) interface to start audio capture without publishing streams.
...
Call the [startPreview](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_macOS~class~zego-express-i-zego-express-engine&jumpType=route#start-preview) interface to bind the view for camera preview, and start video capture and preview without publishing streams.
...
Call the [IZegoMediaPlayer](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-media-player&jumpType=route#public-func-lists) interface to play the audio file you use for testing.
...
If you can hear the corresponding audio, the playback device is working properly and can be used for calls. Call the [onMediaPlayerStateUpdate](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-media-player-event-handler&jumpType=route#on-media-player-state-update) callback to view the media player status:
