...
Based on the [call invitation (signaling)](https://www.zegocloud.com/docs/zim-ios/guides/call-invitation-signaling) feature provided by the In-app Chat (referred to as ZIM SDK), which provides the capability of call invitation, allowing you to send, cancel, accept, and reject an invitation, you can achieve PK battle invitations, room invitations, and similar functions - you can use the `extendedData` field provided by [ZIMCallInviteConfig](https://docs.zegocloud.com/article/api?doc=zim_API~objectivec_ios~class~ZIMCallInviteConfig), which allows you to customize the type of this invitation, thus achieving different functions.
...
For the specific usage of these interfaces, please refer to [Call invitation (signaling)](/zim-ios/guides/call-invitation-signaling).
...
For more details, refer to [stream mixing](/live-streaming-ios/live-streaming/stream-mixing)

</Accordion>

The PK Battles solution requires the use of [stream mixing](/live-streaming-ios/live-streaming/stream-mixing): stream mixing refers to combining multiple streams into one, so that audiences only need to play this stream to watch the footage of multiple hosts. The necessity and significant advantages of using stream mixing are as follows:
...
If you need a more complex custom layout, please refer to [this complete document on mixing layouts](/live-streaming-ios/live-streaming/stream-mixing) to understand the way of mixing layouts and use the `ZEGOLiveStreamingManager.shared.eventDelegates.add(self)` in the Demo to modify the layout:
...
When the PK battle starts, it is necessary to notify the audience that the PK battle has begun. After receiving the notification, the audience can handle the logic of watching the PK. How to notify the audience? We recommend using the [room attribute](/zim-android/guides/room/room-properties) feature of the ZIM SDK to achieve this.
...
When the PK starts, the host needs to call [setRoomAttributes](/zim-ios/client-sdks/api-reference/protocol#setroomattributesroomattributesroomidconfigcallback-zim-class) to set the additional attributes of the room, indicating that the room has entered the PK state. It is recommended to include the following information in the room's additional attributes:
...
After the settings are completed, the audience will receive the [roomAttributesUpdated](/zim-ios/client-sdks/api-reference/protocol#zimroomattributesupdatedroomid-zimeventhandler-class) callback. Now let's explain the logic on the audience side.


## Start PK battle - Audience logic


After receiving the [roomAttributesUpdated](/zim-ios/client-sdks/api-reference/protocol#zimroomattributesupdatedroomid-zimeventhandler-class) callback, if the audience finds that there are newly added fields related to PK battle, they can start processing the logic of watching PK.
...
3. Remove the PK-related attributes from the [room attribute](/zim-ios/guides/room/room-properties).
4. Adjust the UI to return to the state of a single host's live stream.

**Audience:**


When the audience receives the callback [roomAttributesUpdated](/zim-ios/client-sdks/api-reference/protocol#zimroomattributesupdatedroomid-zimeventhandler-class) indicating that the PK-related attributes have been removed, they can start handling the following logic:
...
See [PKBattleView.swift](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/master/best_practice/ZegoCloudSDKDemo/Components/PK/PKBattleView.swift)
...
See [LiveStreamingViewController.swift](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/blob/master/best_practice/ZegoCloudSDKDemo/ViewControllers/LiveStreamingViewController.swift)
...
It is important to note that this feature needs to be enabled by calling [setEngineConfig](@setEngineConfig) after calling [createEngineWithProfile](@createEngineWithProfile). Here is an example of the code:
...
In this case, the audience cannot receive the callbacks mentioned in "Scenario 1". Therefore, when the host is publishing the stream, they need to update their device status using [sendSEI](@sendSEI), and the playing side will receive the callback [onPlayerRecvSEI](@onPlayerRecvSEI).
...
> You can refer to the [relevant code](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/tree/master/best_practice/ZegoCloudSDKDemo/Internal/Business/PK/PKService.swift) in the demo for the specific implementation of this part.
...
In general, the sound level can be rendered using [Visualize the sound level](/live-streaming-ios/audio/sound-level-spectrum).
...
2. To ensure that the audience in the room also cannot hear the audio of the other host, you can update the mixer task by calling [startMixerTask](@startMixerTask) again. Set the `contentType` of the other host's [input stream](@-ZegoMixerInput) to `ZegoMixerInputContentTypeVideoOnly` (There is no need to call `StopMixerTask`, just call `startMixerTask` directly).
...
You can use [git diff](https://github.com/ZEGOCLOUD/zegocloud_sdk_demo_ios/compare/master..pk_1v1) to view these changes and follow our documentation to check the changes item by item and add them to your project.
