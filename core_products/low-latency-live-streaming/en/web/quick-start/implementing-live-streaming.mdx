...
1. Create and initialize an instance of [ZegoExpressEngine ](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine), pass your project's AppID to the parameter "appID", and pass Server to the parameter "server".
...
2. The SDK provides notification callbacks such as room connection status, audio and video stream changes, and user entry/exit. After creating the engine, you can register callbacks through the [on](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~javascript_web~class~ZegoExpressEngine#on) method of the engine instance.
...
Call the [startPlayingStream](@startPlayingStream) interface, and set the [resourceMode](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoWebPlayOption#resource-mode) parameter to the number "2", indicating Live Streaming playing.
