...
- 服务器：对应 RTMP 推流地址，即 `rtmp://hosts/AppName/`
    ...
    **请注意，RTMP 推流地址 URL，与 [CDN 推流鉴权](/real-time-video-web/live-streaming/cdn-stream-publishing-authentication) 中的推流地址 URL 不同，不可混用，使用时请您注意区分。**
...
- 可以使用 ZEGO 服务端的 [增加房间流](/real-time-video-server/api-reference/room/add-stream) 与 [删除房间流](/real-time-video-server/api-reference/room/delete-stream) 接口向指定房间增加或删除一路流信息，将 “streamID” 信息下发给房间内的其他用户，此时其他用户可以就通过 [RoomStreamUpdate](@RoomStreamUpdate) 回调收到音视频流新增或删除的通知。
...
调用 [startPlayingStream](@startPlayingStream) 接口，传入 RTMP 推流的 “streamID” （即设置推流地址中设置的“串流密码”）拉流播放，下文以超低延迟直播拉流为例，其他拉流方式可参考 [快速实现超低延迟直播](/live-streaming-web/quick-start/implementing-live-streaming)。
