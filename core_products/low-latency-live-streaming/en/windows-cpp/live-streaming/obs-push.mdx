...
- Server: Corresponds to the RTMP push stream address, that is, `rtmp://hosts/AppName/`
    ...
    **Please note that the RTMP push stream address URL is different from the push stream address URL in [CDN Push Stream Authentication](/real-time-video-windows-cpp/live-streaming/cdn-stream-publishing-authentication) and cannot be mixed. Please pay attention to distinguish them when using.**
