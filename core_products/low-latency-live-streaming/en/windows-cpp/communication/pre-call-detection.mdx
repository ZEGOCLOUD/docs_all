...
Call the [startPreview](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-express-engine&jumpType=route#start-preview) interface to start audio capture without publishing stream.
...
Call the [startSoundLevelMonitor](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-express-engine&jumpType=route#start-sound-level-monitor) interface to get the energy value of the sound collected by the microphone. If the data is normal, the microphone is normal and can be used for calls or live streaming.
...
Call the [startPreview](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-express-engine&jumpType=route#start-preview) interface to bind the camera preview view, and start video capture and preview without publishing stream.
...
Call the [IZegoMediaPlayer](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-media-player&jumpType=route#public-func-lists) interface to play your audio file for testing.
...
| Method | Description |
|----|----|
|[startPreview](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-express-engine&jumpType=route#start-preview)|Start local preview|
|[onLocalDeviceExceptionOccurred](@onLocalDeviceExceptionOccurred)|Local device exception notification|
|[startSoundLevelMonitor](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-express-engine&jumpType=route#start-sound-level-monitor)|Start volume change monitoring|
|[onMediaPlayerStateUpdate](https://zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_macos~class~zego-express-i-zego-media-player-event-handler&jumpType=route#on-media-player-state-update)|Player playing state callback|
