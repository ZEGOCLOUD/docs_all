...
- A project has been created in the [ZEGOCLOUD Console](https://console.zego.im) and a valid AppID has been applied for. 
...
You can listen to and process the event callbacks you care about by instantiating a class that implements the [IZegoEventHandler](@-IZegoEventHandler) interface and implementing the required callback methods, and then passing the instantiated object as the `eventHandler` parameter to the [createEngine ](@createEngine) method or to the [setEventHandler](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~cpp_windows~class~IZegoExpressEngine#set-event-handler) method to register the callback.
...
<table>
  ...
  <tr>
    ...
    <td>0</td>
    <td>Logging in to the room. When calling [loginRoom](@loginRoom) to log in to the room or [switchRoom](@switchRoom) to switch to the target room, enter this state, indicating that a connection to the server is being requested. Usually, this state is used to display the application interface.</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>7</td>
    <td>Logged out of the room successfully. This is the default state before logging in to the room. When calling [logoutRoom](@logoutRoom) to log out of the room successfully or [switchRoom](@switchRoom) internally logs out of the current room successfully, enter this state.</td>
  </tr>
  <tr>
    <td>ZEGO_ROOM_STATE_CHANGED_REASON_LOGOUT_FAILED</td>
    <td>8</td>
    <td>Failed to log out of the room. When calling [logoutRoom](@logoutRoom) to log out of the room fails or [switchRoom](@switchRoom) internally fails to log out of the current room, enter this state.</td>
  </tr>
</tbody></table>
...
<Note title="Note">
If you need to know about Express's microphone/audio/speaker related interfaces, please refer to [FAQ - How to Implement Camera/Video Screen/Microphone/Audio/Speaker Switch?](/faq/device-switching).
</Note>
...
- [How to Handle Room and User Related Issues?](/faq/express-room-maintenance)
- [How to Set and Get SDK Logs and Stack Information?](/faq/express-logs-stacktrace?product=ExpressVideo&platform=all)
- [Does SDK Support Reconnection After Disconnection?](/faq/express-reconnect?product=ExpressVideo&platform=all)
- [How to Monitor Remote Audience Role Users Login/Exit Room Events in Live Streaming Scenarios?](/faq/audience-room-events)
- [How to Adjust Camera Focal Length (Zoom Feature)?](/faq/camera-focal-length-adjustment)

- [Why Can't I Open the Camera?](/faq/camera-access-failure)

- [How to Ensure Smooth Audio and Video in Poor Network Environments (Traffic Control Feature)?](/faq/flow-control-in-weak-network)
