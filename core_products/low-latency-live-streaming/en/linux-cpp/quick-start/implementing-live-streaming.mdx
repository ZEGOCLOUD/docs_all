...
You can listen to and handle event callbacks you care about by instantiating a class that implements the [IZegoEventHandler](@IZegoEventHandler) interface and implementing the required callback methods. Then pass the instantiated object as the `eventHandler` parameter to [createEngine](@createEngine) or to [setEventHandler](@setEventHandler) to register the callback.
...
<table>
  ...
  <tr>
    ...
    <td>0</td>
    <td>Logging in to the room. When calling [loginRoom](@loginRoom) to log in to the room or [switchRoom](@switchRoom) to switch to the target room, enter this state, indicating that a request is being made to connect to the server. Usually, this state is used to display the application interface.</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>7</td>
    <td>Successfully logged out of the room. This is the default state before logging in to the room. After successfully calling [logoutRoom](@logoutRoom) to log out of the room or successfully logging out of the current room internally by [switchRoom](@switchRoom), enter this state.</td>
  </tr>
  <tr>
    <td>ZEGO_ROOM_STATE_CHANGED_REASON_LOGOUT_FAILED</td>
    <td>8</td>
    <td>Failed to log out of the room. After failing to call [logoutRoom](@logoutRoom) to log out of the room or failing to log out of the current room internally by [switchRoom](@switchRoom), enter this state.</td>
  </tr>
</tbody></table>
...
<Note title="Note">
If you need to understand Express-related microphone/audio/speaker interfaces, please refer to [FAQ - How to implement turning camera/video screen/microphone/audio/speaker on or off?](https://www.zegocloud.com/docs/faq/device-switching).
</Note>
...
- Live Streaming is not enabled by default. Please enable it yourself in the ZEGOCLOUD Console or contact ZEGOCLOUD Technical Support. For details, please refer to [Enable Service](#enable-service).
...
- The "streamID" pushed by remote users can be obtained from the [onRoomStreamUpdate](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-event-handler#on-room-stream-update) callback in [ZegoEventHandler](https://www.zegocloud.com/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-event-handler).
...
For convenience, ZEGO provides a [Web platform for debugging](https://zegoim.github.io/express-demo-web/src/Examples/QuickStart/VideoTalk/index.html?lang=en). On this page, enter the same AppID, RoomID, Server address, and [Token](/console/development-assistance/temporary-token) to join the same room and communicate with the real device. When live streaming is successfully started, you can hear remote audio and see remote video screens.
...
- [In live streaming scenarios, how to listen for events when remote audience role users log in/log out of the room?](https://www.zegocloud.com/docs/faq/audience-room-events)
- [How to adjust the camera focus (zoom function)?](https://www.zegocloud.com/docs/faq/audience-room-events)

- [Why can't I open the camera?](https://www.zegocloud.com/docs/faq/camera-access-failure)

- [How to ensure smooth audio and video in poor network environments (traffic control feature)?](https://www.zegocloud.com/docs/faq/flow-control-in-weak-network)
