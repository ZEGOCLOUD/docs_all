...
A project has been created in [ZEGOCLOUD Console](https://console.zegocloud.com), and a valid AppID and AppSign have been applied for. 
...
1. **When Android Studio runs the downloaded demo source code, it reports an error "Error: Illegal expression start :9". How to handle it?**
    ...
    In the downloaded demo source code, AppID, UserID and Token do not have specific values. Please refer to step 5 in the "[Run Demo Source Code](#run-demo-source-code)" section of this article.
...
2. **When Android Studio runs the downloaded demo source code, it reports an error "Error: Package im.zego.zegoexpress does not exist". How to handle it?**
    ...
    Since accessing GitHub in mainland China is relatively slow, the demo source code downloaded from [Github](https://github.com/zegoim/zego-express-example-topics-android) does not contain the "ZegoExpressEngine" SDK, so it will report that the specified package does not exist. Please refer to step 4 in the "[Run Demo Source Code](#run-demo-source-code)" section of this article.
