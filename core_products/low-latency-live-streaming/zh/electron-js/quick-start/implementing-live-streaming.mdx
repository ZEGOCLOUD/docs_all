...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成](/live-streaming-electron/quick-start/integrating-sdk)。
...
关于回调的注册和取消，请参考 [设置回调](/live-streaming-electron/quick-start/setting-callback)
...
临时 Token 仅供调试，正式上线时，请从开发者的业务服务器生成 token，详情可参考 [使用 Token 鉴权](/live-streaming-electron/communication/use-token-authentication)。如果 Token 错误，请参考 [错误码](/live-streaming-electron/client-sdk/error-code) 文档中的 1002067 和 1003072 排查问题。
...
调用登录房间接口之后，您可通过监听 [onRoomStateChanged](/live-streaming-electron/quick-start/setting-callback) 回调实时监控自己在本房间内的连接状态。
...
在用户调用 [loginRoom ](@loginRoom) 接口后，可以直接调用 [startPublishingStream ](@startPublishingStream) 接口，传入 “streamID”，将自己的音视频流推送到 ZEGO 音视频云。您可通过监听 [onPublisherStateUpdate ](/live-streaming-electron/quick-start/setting-callback) 回调知晓推流是否成功。
...
在同一房间内的其他用户将音视频流推送到 ZEGO 音视频云时，我们会在 [onRoomStreamUpdate](/live-streaming-electron/quick-start/setting-callback) 回调中收到音视频流新增的通知，并可以通过 ZegoStream 获取到某条流的 “streamID”。

我们可以在该回调中，调用 [startPlayingStream](@startPlayingStream) 接口，传入 “streamID” 拉取播放该用户的音视频。您可通过监听 [onPlayerStateUpdate ](/live-streaming-electron/quick-start/setting-callback) 回调知晓是否成功拉取音视频。您可以使用如下拉流方式。
...
- 如果用户在直播的过程中，遇到相关错误，可查询 [错误码](/live-streaming-electron/client-sdk/error-code)。
