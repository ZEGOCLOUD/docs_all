...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成](/live-streaming-linux/quick-start/integrating-sdk)。
...
调用 [createEngine ](@createEngine) 接口，将申请到的 AppID 传入参数 “appID” 和，创建引擎单例对象。
...
您可以通过实例化实现 [IZegoEventHandler ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-event-handler) 接口的类，并实现需要的回调方法，以监听并处理所关注的事件回调。然后将实例化对象作为`eventHandler`参数传递给 [createEngine ](@createEngine) 或者传递给 [setEventHandler ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine) 注册回调。
...
临时 Token 仅供调试，正式上线时，请从开发者的业务服务器生成 token，详情可参考 [使用 Token 鉴权](/live-streaming-linux/communication/use-token-authentication)。如果 Token 错误，请参考 [错误码](/live-streaming-linux/client-sdk/error-code) 文档中的 1002067 和 1003072 排查问题。
...
我们可以在该回调中，调用 [startPlayingStream](@startPlayingStream) 接口，传入 “streamID” 拉取播放该用户的音视频。您可通过监听 [onPlayerStateUpdate ](@onPlayerStateUpdate) 回调知晓是否成功拉取音视频。

<Warning title="注意">


- 超低延迟直播不是默认开启的，请在 ZEGO 控制台自助开通或联系 ZEGO 技术支持，详情请参考 [开通服务](https://doc-zh.zego.im/live-streaming-linux/quick-start/implementing-live-streaming)。
- 如果用户在直播的过程中，遇到相关错误，可查询 [错误码](/live-streaming-linux/client-sdk/error-code)。
...
调用 [startPlayingStream](@startPlayingStream) 接口，并将 [resourceMode](@resourceMode-ZegoPlayerConfig) 参数设置为 “ZEGO_STREAM_RESOURCE_MODE_ONLY_L3”，表示超低延迟直播拉流。
...
## 相关文档
- [常见错误码](/real-time-video-linux-cpp/client-sdk/error-code)
