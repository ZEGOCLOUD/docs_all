...
调用 [createEngine \|_blank](@createEngine) 接口，将申请到的 AppID 传入参数 “appID” 和，创建引擎单例对象。
...
您可以通过实例化实现 [IZegoEventHandler \|_blank](/zh/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-event-handler) 接口的类，并实现需要的回调方法，以监听并处理所关注的事件回调。然后将实例化对象作为`eventHandler`参数传递给 [createEngine \|_blank](@createEngine) 或者传递给 [setEventHandler \|_blank](/zh/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#set-event-handler) 注册回调。
...
[onRoomUserUpdate\|_blank](@onRoomUserUpdate): 同一房间内的其他用户进出房间时，您可通过此回调收到通知。

<div class="mk-warning">

- 只有在登录房间 [loginRoom\|_blank](@loginRoom) 时传的配置 [ZegoRoomConfig\|_blank](@-ZegoRoomConfig) 中的 isUserStatusNotify 参数为 true 时，用户才能收到房间内其他用户的回调。

- 回调中的参数 [ZegoUpdateType\|_blank](@-ZegoUpdateType) 为 ZEGO_UPDATE_TYPE_ADD 时，表示有用户进入了房间；[ZegoUpdateType\|_blank](@-ZegoUpdateType) 为 ZEGO_UPDATE_TYPE_DELETE 时，表示有用户退出了房间。
</div>
...
[onPublisherStateUpdate\|_blank](@onPublisherStateUpdate)：根据实际应用需要，用户推送音视频流之后，当推送视频流的状态发生变更时（如出现网络中断导致推流异常等情况），您会收到该回调，同时 SDK 会自动进行重试。
...
[onPlayerStateUpdate\|_blank](@onPlayerStateUpdate)：根据实际应用需要，用户拉取音视频流之后，当拉取视频流的状态发生变更时（如出现网络中断导致拉流异常等情况），您会收到该回调，同时 SDK 会自动进行重试。
...
@@@Express_Video_Login_Room_Notes@@@
  ...
  传入用户 ID 参数 “userID” 创建 [ZegoUser \|_blank](@ZegoUser) 用户对象后，调用 [loginRoom \|_blank](@loginRoom) 接口，传入房间 ID 参数 “roomID” 和用户参数 “user”，登录房间。如果房间不存在，调用该接口时会创建并登录此房间。
...
[ZegoUser \|_blank](@ZegoUser) 不能为默认值，否则会导致登录房间失败。
...
仅当房间状态是登录成功或重连成功时，推流（[startPublishingStream\|_blank](@startPublishingStream)）、拉流（[startPlayingStream\|_blank](@startPlayingStream)）才能正常收发音视频。
...
我们可以在该回调中，调用 [startPlayingStream\|_blank](@startPlayingStream) 接口，传入 “streamID” 拉取播放该用户的音视频。您可通过监听 [onPlayerStateUpdate |_blank](@onPlayerStateUpdate) 回调知晓是否成功拉取音视频。

<div class="mk-warning">

- 超低延迟直播不是默认开启的，请在 ZEGO 控制台自助开通或联系 ZEGO 技术支持，详情请参考 [3.1 开通服务](21031#3_1)。
...
调用 [startPlayingStream\|_blank](@startPlayingStream) 接口，并将 [resourceMode\|_blank](@resourceMode-ZegoPlayerConfig) 参数设置为 “ZEGO_STREAM_RESOURCE_MODE_ONLY_L3”，表示超低延迟直播拉流。
