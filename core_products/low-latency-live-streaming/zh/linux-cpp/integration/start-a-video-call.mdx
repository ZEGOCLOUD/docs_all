...
您可以通过实例化实现 [IZegoEventHandler ](/zh/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-event-handler) 接口的类，并实现需要的回调方法，以监听并处理所关注的事件回调。然后将实例化对象作为`eventHandler`参数传递给 [createEngine ](@createEngine) 或者传递给 [setEventHandler ](/zh/api?doc=Express_Video_SDK_API~CPP_linux~class~zego-express-i-zego-express-engine#set-event-handler) 注册回调。
...
我们可以在该回调中，调用 [startPlayingStream](@startPlayingStream) 接口，传入 “streamID” 拉取播放该用户的音视频。您可通过监听 [onPlayerStateUpdate |_blank](@onPlayerStateUpdate) 回调知晓是否成功拉取音视频。
