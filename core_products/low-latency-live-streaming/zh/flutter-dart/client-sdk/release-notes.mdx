...
1. 新增 AI 低照度增强功能
    ...
    开发者可通过 [setLowlightEnhancementParams](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEnginePublisher/setLowlightEnhancementParams.html) 指定使用 AI 算法，当设备性能不足以支持开启 AI 算法时，SDK 会自动使用传统算法。一般中高端机型可稳定运行低照度增强 AI 算法，详细请参考 [推流视频增强](/live-streaming-flutter/video/publish-video-enhancement) 文档。
...
1. 删除 AndroidManifest.xml 文件中声明的屏幕共享默认权限
    ...
    详情请参考 [屏幕共享](/live-streaming-flutter/video/screen-sharing) 文档。
...
1. iOS 平台支持前后摄像头同时采集视频
    ...
    新增枚举值 `ZegoVideoSourceTypeSecondaryCamera` 用于标识第二个摄像头的视频，支持通过 [setVideoSource] 接口同时将前后摄像头设置为视频采集源并分别推流。开发者可以通过 [useFrontCamera] 将 `ZegoVideoSourceTypeCamera` 及 `ZegoVideoSourceTypeSecondaryCamera` 对应的前后画面进行置换。该能力可应用于使用双摄像头直播等场景，更多内容请参考 [多源采集](/live-streaming-flutter/communication/multi-source-capture)。
...
3. Android 平台新增屏幕采集权限的申请结果回调通知
    ...
    开发者可根据回调结果做后续业务处理，包括 UI 提示或应用跳转等，更多内容请参考 [屏幕共享](/live-streaming-flutter/video/screen-sharing)。
...
4. iOS 和 Android 平台的视频超分辨率功能支持 1.33 倍和 1.5 倍放大倍数
    ...
    视频分辨率高于设备支持上限会引入额外损伤，导致视频质量下降。因此视频超分辨率能力新增 1.33 倍和 1.5 倍的放大倍数，以适配不同设备的最佳效果，更多内容请参考 [超分辨率](/live-streaming-flutter/video/super-resolution)。

5. AI 降噪新增低延迟模式

    注意：如需使用该功能，请联系 ZEGO 技术支持。

    在 10ms 延迟下，依然保持纯净的降噪效果以及高保真的人声音质，适用于游戏语音、游戏开黑、实时合唱等对延迟较为敏感的场景，目前 AI 降噪已支持均衡模式、低延迟模式以及轻量模式，详情请参考 [场景化 AI 降噪](/live-streaming-flutter/audio/scenario-based-ai-noise-reduction)。
...
7. 媒体播放器播放倍速最低支持至 0.3 倍速
    ...
    媒体播放器的播放速度区间从 [0.5,4.0]，扩大为 [0.3,4.0]，更多内容请参考 [媒体播放器](/live-streaming-flutter/other/media-player)。
...
1. 优化 AI 场景化降噪中，“均衡模式”的降噪效果
    ...
    优化 AI 场景化降噪中，“均衡模式”的降噪效果，在性能不变的情况下，人声清晰度、平稳度进一步提升，且噪音抑制更干净，详情请参考 [场景化 AI 降噪](/live-streaming-flutter/audio/scenario-based-ai-noise-reduction)。
...
2. 推流视频支持色彩增强
    ...
    针对各种摄像头等设备采集到的画面色彩偏灰、或饱和度偏低的情况，支持在保护人体肤色的同时，增强画面色彩，使其更加鲜艳明亮，更符合人眼真实的视觉感受，详情请参考 [推流视频增强](/live-streaming-flutter/video/publish-video-enhancement)。
...
2. 当前官网 SDK 不包含此功能，如有需要，请联系 ZEGO 技术支持特殊编包。
    ...
    新增 AI 变声功能，实时通话中的“柯南变声领结”，完美重现目标角色的音色与韵律，同时保留用户的语速、情感、语调，随心所欲切换音色，超低延迟让用户畅享社交语聊、直播、游戏语音等场景，详情请参考 [AI 变声](/live-streaming-flutter/audio/ai-voice-changer)。
...
2. 虚拟背景的视频填充方式为居中及等比例缩放，视频过大时，超出部分会被裁减。
    ...
    使用主体分割功能时，虚拟背景支持使用视频素材，视频素材的最终帧率会与编码帧率保持一致，且循环播放。视频素材的限制说明，请参考 [主体分割](/live-streaming-flutter/video/object-segmentation)。
...
1. 新增支持“智能云代理”模式
    ...
    开发者设置“智能云代理”模式后，在 RTC 或 L3 拉流时，会优先使用直连网络模式进行尝试。如果直连网络不可用、且当前是蜂窝网络，则继续留在直连模式重试；如果直连网络不可用、且当前是非蜂窝网络，则切到云代理模式，详情请参考 [云代理](/live-streaming-flutter/communication/cloud-proxy)。
...
1. 废弃了一些 API 接口
    ...
    API 接口变更后可能存在兼容性问题，详情请参考 [3.8.1 及以上版本升级指南](/live-streaming-flutter/client-sdk/upgrade-guide/upgrade-to-v3-81)。
...
2. 设置大流、小流的分辨率的 “比例” 需要保持一致，否则调用接口会出错。
    ...
    在指定编码格式为 “大小流编码” 的情况下，支持分别设置大流和小流的分辨率、帧率和码率，详情请参考 [视频大小流和分层编码](/live-streaming-flutter/video/small-large-video-stream-and-layered-encoding)。
...
1. 新增支持背景虚化、虚拟背景功能
    ...
    在实景或绿幕场景中，开发者可以通过该功能对用户的背景进行虚化模糊处理，或替换为自定义图片背景，详情请参考 [主体分割](/live-streaming-flutter/video/object-segmentation)。
...
3. 游戏语音支持自定义设置发声模式和收听模式
    ...
    游戏语音支持自定义设置发声模式和收听模式，可用于实现加入小队后，需屏蔽非范围内的同一小队玩家的场景，详情请参考 [游戏语音](/live-streaming-flutter/communication/range-audio)。
...
7. 场景化音视频配置新增 [StandardVoiceCall] 标准语音通话场景
    ...
    场景化音视频配置新增 [StandardVoiceCall] 标准语音通话场景，适用于 1v1 纯语音通话场景，详情请参考 [场景化音视频配置](/live-streaming-flutter/quick-start/scenario-based-audio-video-configuration)。
...
1. 优化超分算法，大幅提升机型覆盖率
    ...
    注意：[enableVideoSuperResolution](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEnginePlayer/enableVideoSuperResolution.html) 修改了调用时机，需要在 [initVideoSuperResolution](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEnginePlayer/initVideoSuperResolution.html) 后才能调用，详情请参考 [超分辨率](/live-streaming-flutter/video/super-resolution)。
...
1. Flutter Web 支持使用屏幕共享功能
    ...
    Flutter Web 支持使用屏幕共享功能，详情请参考 [屏幕共享](/live-streaming-flutter/video/screen-sharing)。
...
2. Flutter Web 支持使用美颜功能
    ...
    Flutter Web 支持使用美颜功能，详情请参考 [推流视频增强 - 基础美颜](/live-streaming-flutter/video/publish-video-enhancement)。
...
3. Flutter Web 支持使用多房间功能
    ...
    Flutter Web 支持使用多房间功能，详情请参考 [登录多房间](/live-streaming-flutter/room/multi-room-login)。
...
1. 支持多源采集能力
    ...
    多源采集能力对屏幕共享、混音等常见能力的实现路径，进行缩短优化及归一化设计，从 3.1.0 版本后，您可以不用再通过自定义采集实现上述复杂的能力，详情请参考 [多源采集文档](/live-streaming-flutter/communication/multi-source-capture)。主要能力特性如下：
    - 推流通道支持设置或切换多种音视频源。
    - 支持屏幕共享、混音等常见能力。
...
2. SDK 支持设置云代理
    ...
    通过设置 SDK 的云代理接口，将 SDK 对应的所有流量通过云端的代理服务器进行中转，实现与 RTC 通信，详情请参考  [云代理文档](/live-streaming-flutter/communication/cloud-proxy) 。
...
本次更新包含不兼容改动，详情请参考 [3.0.0 及以上版本升级指南](/live-streaming-flutter/client-sdk/upgrade-guide/upgrade-to-v3)。
...
1. 新增视频超分辨率能力
    ...
    新增 [enableVideoSuperResolution](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEnginePlayer/enableVideoSuperResolution.html) 接口支持对某一条视频流进行超分辨率处理，从而得到更好的画质。超分辨率，简称超分，是客户端对拉取的视频流的宽和高进行实时倍增处理的技术，例如，从 640x360 超分为 1280x720，详情请参考 [超分辨率](/live-streaming-flutter/video/super-resolution) 文档。
...
2. 场景化 AI 降噪新增在音乐场景下降噪的能力
    ...
    场景化 AI 降噪功能，在之前针对所有非人声进行降噪的基础上，新增支持在音乐场景下的降噪能力，通过识别音乐，智能调整降噪效果还原音乐音质。SDK 会实时对麦克风输入内容进行音乐检测，在声卡、弹唱或近场音乐场景下，自动调整降噪等级，保证音乐的高保真音质，详情请参考 [场景化 AI 降噪](/live-streaming-flutter/audio/scenario-based-ai-noise-reduction) 文档。
...
3. 新增房间维度的场景 Scenario
    ...
    当前支持场景包括秀场直播、KTV、标准 1v1 音视频通话、高画质 1v1 音视频通话、标准语聊房、高音质语聊房（Web 端暂不支持高品质语聊房场景及 KTV 场景），详情请参考 [场景化音视频配置](/live-streaming-flutter/quick-start/scenario-based-audio-video-configuration) 文档。
...
1. 废弃了 [ZegoScenario] 的三种旧版本场景
    废弃 [ZegoScenario](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoScenario.html) 场景枚举中的 `General`，`Communication`， `Live` 三种场景，详情请参考 [场景化音视频配置](/live-streaming-flutter/quick-start/scenario-based-audio-video-configuration) 文档。

2. 删除了 `setDebugVerbose`、`setPlayStreamVideoLayer`、`enableAudioDataCallback` 等接口，详情请参考 [3.0.0及以上版本升级指南](/live-streaming-flutter/client-sdk/upgrade-guide/upgrade-to-v3)。
