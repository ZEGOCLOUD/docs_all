...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成](/live-streaming-ios-swift/quick-start/integrating-sdk)。
...
//更多信息请参考 https://doc-zh.zego.im/real-time-video-ios-oc/room/room-connection-status
func onRoomStateChanged(_ reason: ZegoRoomStateChangedReason, errorCode: Int32, extendedData: [AnyHashable : Any], roomID: String) {
    if reason == .logining {
...
- 如果用户在音视频直播的过程中，遇到相关错误，可查询 [错误码](/live-streaming-ios-swift/client-sdk/error-code)。
...
为方便体验，ZEGO 提供了一个 [Web 端调试示例 ](https://zegoim.github.io/express-demo-web/src/Examples/QuickStart/VideoTalk/index.html?lang=zh)，在该页面下，输入相同的 AppID、RoomID、Server 地址和 Token，即可加入同一房间与真机设备互通。当成功开直播时，可以听到远端的音频，看到远端的视频画面。
