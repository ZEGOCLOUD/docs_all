...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成](/live-streaming-android/quick-start/integrating-sdk)。
...
[onRoomUserUpdate ](@onRoomUserUpdate)：同一房间内的其他用户进出房间时，您可通过此回调收到通知。回调中的参数 [ZegoUpdateType ](@-ZegoUpdateType) 为 ZegoUpdateType.ADD 时，表示有用户进入了房间；[ZegoUpdateType ](@-ZegoUpdateType) 为 ZegoUpdateType.DELETE 时，表示有用户退出了房间。
...
开发者可在 [ZEGO 控制台](https://console.zego.im/dashboard) 获取临时 Token（有效期为 24 小时），详情请参考 [控制台 - 开发辅助](https://doc-zh.zego.im/console/development-assistance/temporary-token)。

<Warning title="注意">

- 临时 Token 仅供调试，正式上线时，请从开发者的业务服务器生成 token，详情可参考 [使用 Token 鉴权](/live-streaming-android/communication/use-token-authentication)。
- 如果 Token 错误，请参考 [错误码](/live-streaming-android/client-sdk/error-code) 文档中的 1002067 和 1003072 排查问题。
...
我们可以在该回调中，调用 [startPlayingStream](@startPlayingStream) 接口，传入 “streamID” 拉取播放该用户的音视频。您可通过监听 [onPlayerStateUpdate ](@onPlayerStateUpdate) 回调知晓是否成功拉取音视频。您可以使用如下拉流方式。

<Warning title="注意">


- 超低延迟直播不是默认开启的，请在 ZEGO 控制台自助开通或联系 ZEGO 技术支持，详情请参考 [开通服务](https://doc-zh.zego.im/live-streaming-android/quick-start/implementing-live-streaming)。
- 如果用户在直播的过程中，遇到相关错误，可查询 [错误码](/live-streaming-android/client-sdk/error-code)。
