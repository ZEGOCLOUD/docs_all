...
<tr>
<td>[RTMP 推流到 ZEGO 服务器|_blank](!OBS_Push)</td>
<td>在直播场景下，主播可以通过 RTMP 推流工具将音视频流推送到 ZEGO 服务器，实现低延迟的效果。</td>
<td>直播场景。</td>
</tr>
<tr>
<td>[混流|_blank](!Other_Functions/Mixer)</td>
...
<tr>
<td>[单流转码\|_blank](!Single_stream_transcoding)</td>
...
<tr>
<td>[直播连麦|_blank](!hybrid_hierarchical_delivery_system-Integration/Solution_Implementation)</td>
...
<tr>
<td>[房间连接状态说明\|_blank](!Room_Connection_Status_Description)</td>
<td>判断用户在房间内的连接状态，以及各个连接状态的转化过程。</td>
<td>-</td>
</tr>
<tr>
<td>[实时消息与信令|_blank](!AdvancedModule/IM)</td>
...
<tr>
<td>[登录多房间\|_blank](!Other_Functions/Multiple_Rooms)</td>
...
<tr>
<td>[音频频谱与音量变化|_blank](!hybrid_hierarchical_delivery_system-Audio_Spectrum_and_Volume_Change)</td>
<td>音频频谱：即数字音频信号在各频点的能量值。音量变化：即某条流音量的大小。</td>
<td>- 判断麦上的用户谁在说话、麦克风、扬声器等是否可用。
- 音频频谱动画展示。</td>
</tr>
<tr>
<td>[耳返与声道设置|_blank](!hybrid_hierarchical_delivery_system-Inear_and_Channel_Settings)</td>
...
<tr>
<td>[音频 3A 处理\|_blank](!AudioAdvanced/Audio_3A_Processing) </td>
...
<tr>
<td>[变声/混响/立体声\|_blank](!AudioAdvanced/AudioEffects)  </td>
...
<tr>
<td>[场景化 AI 降噪|_blank](!AIDenoise)</td>
...
<tr>
<td>[自定义音频采集\|_blank](!AudioAdvanced/custom_audio_io)</td>
<td>开发者可以自行获取音频信息后，交给 Zego Express SDK 进行传输。</td>
<td>- 在线或本地音频文件传输。
- 定制采集系统的音频文件进行传输。</td>
</tr>
<tr>
<td>[自定义音频渲染\|_blank](!AudioAdvanced/custom_audio_io)</td>
<td>音频的由开发者自行渲染后进行播放。</td>
<td>开发者有自己的特殊渲染需求。</td>
</tr>
<tr>
<td>[自定义音频处理|_blank](!Custom_Audio_Processing)</td>
<td>开发者可以自行进行音频特殊处理。</td>
<td>有 Zego Express SDK 无法满足的特殊的声音处理需求时，如特殊变声。</td>
</tr>
<tr>
<td>[AI 变声|_blank](!AI_VoiceChanger)</td>
...
<tr>
<td>[常用视频配置|_blank](!AdvancedModule/Common_Video_Configuration)</td>
<td>在直播时，自定义设置采集及播放的视频相关配置，如视频采集分辨率、视频编码输出分辨率、视频帧率、视频码率、视图模式和镜像模式等。</td>
<td>对分辨率、码率、镜像等有特殊要求的场景。</td>
</tr>
<tr>
<td>[视频采集旋转|_blank](!collection_rotate)</td>
<td>用户使用移动设备进行直播时，可以采用不同的视频采集方向，并且可以根据对端用户的摄像头方向和角度进行适配调整。</td>
<td>多类型设备直播场景，或需要最佳视频播放角度的场景。</td>
</tr>
<tr>
<td>[屏幕共享|_blank](!Other_Functions/Share_Screen)</td>
<td>视频通话或互动直播过程中将屏幕内容以视频的方式共享给房间内的其他用户。</td>
<td>- 视频会议
- 游戏直播</td>
</tr>
<tr>
<td>[水印与截图|_blank](!hybrid_hierarchical_delivery_system-Watermark_Local_Screenshot)</td>
...
<tr>
<td>[设置视频编码方式\|_blank](!VideoAdvanced/video_codec_svc) </td>
<td>可对视频的编解码进行详细设置，包括启用分层视频编码、使用硬件编解码和设置编码方式等。</td>
<td>对编解码有特殊要求时。</td>
</tr>
<tr>
<td>[自定义视频采集|_blank](!VideoAdvanced/CustomVideoCapture)</td>
<td>自定义向 Zego Express SDK 提供视频输入源输入视频数据，并由 Zego Express SDK 进行编码推流的功能。</td>
<td>- 使用了第三方美颜厂商的美颜 SDK。
- 摄像头无法被 SDK 正常使用。
- 直播非摄像头采集的数据。</td>
</tr>
<tr>
<td>[自定义视频渲染|_blank](!VideoAdvanced/CustomVideoRender)</td>
<td>自定义视频渲染指的是 Zego Express SDK 向外部提供本地预览及远端拉流的视频帧数据，供用户自行渲染。</td>
<td>- App 使用了跨平台界面框架或游戏引擎。
- App 需要获取 Zego Express SDK 采集或拉流的视频帧数据进行特殊处理。</td>
</tr>
<tr>
<td>[自定义视频前处理|_blank](!hybrid_hierarchical_delivery_system-Custom_Video_Preprocessing)</td>
<td>开发者自行对视频数据进行自定义的前处理。</td>
<td>美颜、添加挂件等操作。</td>
</tr>
<tr>
<td>[超分辨率|_blank](!super_resolution)</td>
<td>在拉流端对拉取到的视频流画面的宽和高进行倍增。例如：拉流端拉取到的原始画面分辨率为 640p x 360p，对画面进行超分处理后分辨率将提升为 1280p x 720p。</td>
<td>1V1 视频通话场景、直播场景、在线教育。</td>
</tr>
<tr>
<td>[主体分割|_blank](!background_segment)</td>
<td>ZEGO 业内领先技术。在推流端将矩形视频内的主体（多数情况下是人）通过 AI 算法分离出来并在 RTC 网络中传输、在拉流端渲染。</td>
<td>多人异地同台、秀场直播同台 PK、多人在线自习等多人同台场景。</td>
</tr>
<td>[H.265|_blank](!hybrid_hierarchical_delivery_system-H265)</td>
<td>通过更加先进的 H.265 编码技术，在相同码率的情况下，提供更高的清晰度。</td>
<td>网络环境较差下需要较高的音视频通话、直播体验对带宽较为敏感。</td>
</tr>
<tr>
<td>[视频大小流和分层编码|_blank](!Big_and_small_streams_and_Layered_Encoding)</td>
<td>将码流分为基本层和扩展层，可以为不同网络状态、不同设备性能的用户提供更好的体验。</td>
<td>视频通话。</td>
</tr>
<tr>
<td>[推流视频增强\|_blank](!Publish_Video_Enhanced)</td>
...
<tr>
<td>[媒体播放器|_blank](!Other_Functions/MediaPlayer)</td>
...
<tr>
<td>[音效文件播放器|_blank](!hybrid_hierarchical_delivery_system-AudioAdvanced/Audio_Player)</td>
<td>提供音效播放器，并进行音效统一管理，实现对增强真实感或者烘托场景氛围的简短效果音的播放/控制等功能。</td>
<td>- 秀场直播。
- 游戏娱乐。</td>
</tr>
<tr>
<td>[音视频录制|_blank](!Other_Functions/Local_Media_Recording)</td>
...
<tr>
<td>[播放透明礼物特效\|_blank](!gift_special_effects)</td>
