...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成](/live-streaming-windows/quick-start/integrating-sdk)。
...
注册回调，可将实现了 [ZegoEventHandler](@-IZegoEventHandler) 的对象（例如 “self”）传入参数 “eventHandler”。
...
临时 Token 仅供调试，正式上线时，请从开发者的业务服务器生成 token，详情可参考 [使用 Token 鉴权](/live-streaming-windows/communication/use-token-authentication)。如果 Token 错误，请参考 [错误码](/live-streaming-windows/client-sdk/error-code) 文档中的 1002067 和 1003072 排查问题。
...
- 超低延迟直播不是默认开启的，请在 ZEGO 控制台自助开通或联系 ZEGO 技术支持，详情请参考 [开通服务](https://doc-zh.zego.im/live-streaming-windows/quick-start/implementing-live-streaming)。
- 如果用户在直播的过程中，遇到相关错误，可查询 [错误码](/live-streaming-windows/client-sdk/error-code)。
