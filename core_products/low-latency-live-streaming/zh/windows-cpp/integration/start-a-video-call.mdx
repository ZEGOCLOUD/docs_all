...
注册回调，可将实现了 [ZegoEventHandler\|_blank](@-IZegoEventHandler) 的对象（例如 “self”）传入参数 “eventHandler”。
...
您可以通过实例化实现 [IZegoEventHandler\|_blank](@-IZegoEventHandler) 接口的类并实现需要的回调方法，以监听并处理所关注的事件回调，然后将实例化对象作为`eventHandler`参数传递给 [createEngine \|_blank](@createEngine) 方法或者传递给 [setEventHandler](/article/api?doc=Express_Video_SDK_API~cpp_windows~class~IZegoExpressEngine#set-event-handler) 注册回调。
...
- 超低延迟直播不是默认开启的，请在 ZEGO 控制台自助开通或联系 ZEGO 技术支持，详情请参考 [3.1 开通服务](13416#3_1)。
