...
可参考 [快速开始 - 实现流程\|_blank](!Integration/Solution_Implementation#3_1) 的 “3.1 创建引擎” 和 “3.2 登录房间” 完成房间登录。
...
[ZegoMixerTask\|_blank](@-ZegoMixerTask) 是 SDK 中定义的混流任务配置对象，通过配置该对象可定制化手动混流任务。
...
通过 [ZegoMixerVideoConfig\|_blank](@-ZegoMixerVideoConfig) 设置混流视频相关配置，主要包括视频分辨率、视频帧率和码率。
...
通过 [ZegoMixerAudioConfig\|_blank](@-ZegoMixerAudioConfig) 设置混流音频相关配置，主要包括音频码率、声道数、编码ID。 
...
通过 [ZegoMixerOutput\|_blank](@-ZegoMixerOutput) 设置混流输出列表，用户可以从列表中的输出目的地拉取混流。    
...
完成了 [ZegoMixerTask\|_blank](@-ZegoMixerTask) 混流任务对象的配置后，调用 [startMixerTask\|_blank](@startMixerTask) 接口开始这个混流任务，并在回调中接收开始混流任务结果。
...
调用 [stopAutoMixerTask\|_blank](@stopAutoMixerTask) 接口停止混流。
...
```c++
task.audioConfig.bitrate = 48;/** 音频码率，单位为 kbps，默认为 48 kbps，开始混流任务后不能修改 */
task.audioConfig.channel = ZEGO_AUDIO_CHANNEL_MONO;/** 音频声道，默认为 Mono 单声道 */
task.audioConfig.codecID = ZEGO_AUDIO_CODEC_ID_DEFAULT;/** 编码 ID，默认为 ZEGO_AUDIO_CODEC_ID_DEFAULT */
task.audioConfig.mixMode = ZEGO_AUDIO_MIX_MODE_RAW;/** 多路音频流混音模式，默认为 ZEGO_AUDIO_MIX_MODE_RAW */
```
...
ZegoMixerOutput mixerOutput;
// 输出流名为 "output-stream"
mixerOutput.target = "output-stream";/** 混流输出目标，URL 或者流 ID */
task.outputList = {mixerOutput};
```
