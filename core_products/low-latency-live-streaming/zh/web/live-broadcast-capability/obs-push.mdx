...
- 可以使用 ZEGO 服务端的 [增加房间流](#19561) 与 [删除房间流](#19563) 接口向指定房间增加或删除一路流信息，将 “streamID” 信息下发给房间内的其他用户，此时其他用户可以就通过 [RoomStreamUpdate](@RoomStreamUpdate) 回调收到音视频流新增或删除的通知。
</div>

### 3.3 使用 SDK 拉流/停止拉流

调用 [startPlayingStream](@startPlayingStream) 接口，传入 RTMP 推流的 “streamID” （即 3.2.1 章节中设置的“串流密码”）拉流播放，下文以超低延迟直播拉流为例，其他拉流方式可参考 [快速实现超低延迟直播](!Integration/Solution_Implementation)。
