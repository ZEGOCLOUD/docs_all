...
- 已在项目中集成 ZEGO Express SDK，详情请参考 [快速开始 - 集成](https://doc-zh.zego.im/article/199)。
...
超低延迟直播功能不是默认开启的，使用前请在 [ZEGO 控制台](https://console.zego.im) 自助开通（开通步骤请参考 [项目管理 - 服务配置](https://doc-zh.zego.im/article/14337) 中的“超低延迟直播”），或联系 ZEGO 技术支持开通。
...
您还可以通过 ZEGO 提供的 [在线检测工具](https://zegodev.github.io/zego-express-webrtc-sample/webrtcCheck/index.html)，在需要检测的浏览器中打开，直接检测浏览器的兼容性。请参考 [浏览器兼容性和已知问题](https://doc-zh.zego.im/article/12047) 获取 SDK 支持的浏览器兼容版本。
...
临时 Token 仅供调试，正式上线前，请从开发者的业务服务器生成 Token，详情可参考 [使用 Token 鉴权](https://doc-zh.zego.im/article/7646)。
...
可根据需要通过 [createZegoStream](@createZegoStream) 接口中的如下属性设置音视频相关采集参数，详情可参考 [自定义视频采集](https://doc-zh.zego.im/article/3679)：
