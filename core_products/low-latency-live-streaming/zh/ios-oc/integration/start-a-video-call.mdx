...
我们可以在该回调中，调用 [startPlayingStream](@startPlayingStream) ，传入 “streamID” 拉取拉取播放该用户的音视频。您可通过监听 [onPlayerStateUpdate |_blank](@onPlayerStateUpdate) 回调知晓是否成功拉取音视频。您可以选择如下拉流方式。
