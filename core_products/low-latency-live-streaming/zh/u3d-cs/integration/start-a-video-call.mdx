...
[OnRoomUserUpdate\|_blank](@OnRoomUserUpdate)：同一房间内的其他用户进出房间时，您可通过此回调收到通知。回调中的参数 [ZegoUpdateType\|_blank](@-ZegoUpdateType) 为 ZegoUpdateType.ZegoUpdateTypeAdd 时，表示有用户进入了房间；[ZegoUpdateType\|_blank](@-ZegoUpdateType) 为 ZegoUpdateType.ZegoUpdateTypeDelete 时，表示有用户退出了房间。
...
- 房间人数大于 500 人的情况下 [OnRoomUserUpdate\|_blank](@OnRoomUserUpdate) 回调不保证有效。若业务场景存在房间人数大于 500 的情况，请联系 ZEGO 技术支持。
...
- 在 WebGL 环境中运行时，仅支持 Token 鉴权，Token 不能为空，请参考 [使用 Token 鉴权\|_blank](#14355)。
...
调用 [StartPlayingStream\|_blank](@StartPlayingStream) 接口，并将 [resourceMode\|_blank](@resourceMode-ZegoScenePlayerConfig) 参数设置为 “ZegoScenePlayerConfig.ZegoStreamResourceModeOnlyL3”，表示超低延迟直播拉流。
