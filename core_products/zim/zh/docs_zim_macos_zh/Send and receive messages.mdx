...
1. 请参考 [下载 SDK](./Client SDKs/SDK downloads.mdx)，下载最新版本的 SDK。
...
首先，请创建用户个人登录配置（[ZIMLoginConfig](@-ZIMLoginConfig) 对象）；创建完毕后，调用 [loginWithUserID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_macos~protocol~ZIM#login-with-user-id-config-callback-2) 接口，传入上述对象，即可登录 ZIM。
...
客户端 B 登录 ZIM 后，将会收到在 [setEventHandler](@setEventHandler) 回调中设置的 [peerMessageReceived](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_macos~protocol~ZIMEventHandler#zim-peer-message-received-info-from-user-id) 监听接口，收到客户端 A 发送过来的消息。
