...
在无网络且应用已被清理的情况下，可以调用 [loginWithUserID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_macos~protocol~ZIM#login-with-user-id-config-callback-2) 接口时，[ZIMLoginConfig](@-ZIMLoginConfig) 中的 [isOfflineLogin](@isOfflineLogin) 传入 `YES`，实现使用上一次的用户信息离线登录 App。当通过 [ZIMLoggedInCallback](@ZIMLoggedInCallback) 回调确定登录成功后，即可访问本地 SDK 数据。 

<Note title="说明">
建议您缓存每次登录使用的用户信息。当 App 打开时，读取缓存，实现以下逻辑：
- 如果判断用户不是使用上一次在线登录的 UserID ，此时 [isOfflineLogin](@isOfflineLogin) 应为 `NO`，此时离线登录会失败，不允许用户访问该 UserID 的本地 SDK 数据。
- 如果用户使用上一次在线登录的 UserID，为了实现自动登录、加速进入 App 主页以及提前渲染 UI，[isOfflineLogin](@isOfflineLogin) 应为 `YES`，实现离线登录。
</Note>
