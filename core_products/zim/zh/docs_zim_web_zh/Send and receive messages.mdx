...
详细的接口介绍，请参考 [connectionStateChanged](@connectionStateChanged)、[messageReceived](@messageReceived)、[tokenWillExpire](@tokenWillExpire)。
...
客户端需要使用各自的用户信息和 Token 进行登录。调用 [login](@login__2) 接口进行登录，传入 `userID` 和 [ZIMLoginConfig](@-ZIMLoginConfig) 对象（必须包含 `Token`），等待 Token 鉴权通过后才能登录成功。
...
客户端 B 登录 ZIM 后，将会收到在 [on](@on) 回调中设置的 [messageReceived](@messageReceived) 监听接口，收到客户端 A 发送过来的消息。
