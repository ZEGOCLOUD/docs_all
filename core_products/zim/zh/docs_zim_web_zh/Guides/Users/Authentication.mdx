...
在 Token 过期前 30 秒，SDK 会通过 [tokenWillExpire](@tokenWillExpire) 回调发出通知。若登录成功后 Token 有效期不足 30 秒，则会立即回调。

收到该回调后，开发者需要从自己的服务端获取新的有效 Token，并调用 SDK 提供的 [renewToken](@renewToken) 接口更新 Token。

<Note title="说明">

当 Token 过期且未得到更新时，用户会掉线，收到 [connectionStateChanged](@connectionStateChanged) 回调，其中 `event` 为 `TokenExpired`，`state` 为 `Disconnected`。
</Note>
...
```javascript
// 注册监听“令牌即将过期”的回调
zim.onTokenWillExpire(function(zim, second) {
    const token = ''; // 有效时长不能超过 24 天，重新请求开发者服务端获取 Token
    zim.renewToken(token)    
        .then(function({ token }) {
            // 更新成功
