...
[messageReactionsChanged](@messageReactionsChanged) 现在也会在操作端调用 [addMessageReaction](@addMessageReaction)、[deleteMessageReaction](@deleteMessageReaction) 或 [deleteAllMessageReactions](@deleteAllMessageReactions) 成功后回调，请开发者检查相应业务逻辑在升级 SDK 之后是否产生影响。
</Step>
...
优化 SDK 本地存储策略，更加符合[小程序本地文件存储限制](https://developers.weixin.qq.com/miniprogram/dev/framework/ability/file-system.html)，不支持本地消息存储。
- 过期的消息查询不到
- 查询列表类接口，耗时相对增加
