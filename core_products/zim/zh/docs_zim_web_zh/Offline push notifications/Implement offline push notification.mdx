...
1. 请参考 [ZPNs SDK](./../Client%20SDKs/SDK%20downloads.mdx)，下载最新版本的 ZPNs SDK。
...
2. 如果不再需要离线推送功能，开发者可以调用 [unregister](@unregister) 接口，注销该功能，注销后用户将不再能收到推送消息。
...
<Warning title="注意">
实现离线推送之前：
- 请参考 [发送消息](./../Send%20and%20receive%20messages.mdx)，实现发送单聊/群聊消息的功能。
- 请参考 [呼叫邀请](./../Guides/Call%20invitation%20signaling.mdx)，实现呼叫邀请。
</Warning>
...
3. 发送方调用 [sendPeerMessage](@sendPeerMessage)，传入 “sendConfig”，向接收方发送单聊消息。
...
3. 发送方调用 [sendGroupMessage](@sendGroupMessage)，传入 “sendConfig”，向群组内的所有用户发送消息。
...
1. 从 [下载](./../Client%20SDKs/SDK%20downloads.mdx) 获取 2.5.0 或以上版本的 ZPNs SDK，将其中的 `firebase-messaging-sw.js` 文件，拷贝到您的项目的根目录。
