...
ZIM SDK 2.28.0 版本调整了事件回调触发的时机。从旧版本升级到 2.28.0 版本时，请参考 [ZIM 升级指南](./ZIM%20upgrade%20guide.mdx#2280-升级指南)。
</Warning>
...
|优化项-25% | 优化描述-40% | 相关接口-25%|
|-----|-----|-----|
|优化删除会话接口| 删除会话时，发起删除的客户端会触发会话变更的相关回调。详细请参考[删除会话](../Guides/Conversation/Delete%20conversations.mdx#删除单个会话)。|<ul><li>[deleteConversation](@deleteConversation)</li><li>[conversationChanged](@conversationChanged)</li><li>[deleteAllConversations](@deleteAllConversations)</li><li>[conversationsAllDeleted](@conversationsAllDeleted)</li></ul>|
|优化消息置顶功能| 消息置顶超过最大消息数时，支持设置是否覆盖当前置顶列表。|[pinMessage](@pinMessage)|
|优化本地存储策略| SDK 本地存储策略适应[小程序本地文件存储限制](https://developers.weixin.qq.com/miniprogram/dev/framework/ability/file-system.html)，**不再支持本地消息存储**。|-|
...
|网络连接优化| 微信小程序基础库版本升级到最新版本时，修复可能存在的无限重连问题 | [onConnectionStateChanged](@onConnectionStateChanged) |
...
|-----|-----|-----|
|消息置顶| 用户可以将会话中的消息固定在会话顶部，方便会话中的所有用户快速查看重要消息。详细请参考 [消息置顶](../Guides/Messaging/pin-message.mdx)。 | <ul><li>[messagePinStatusChanged](@messagePinStatusChanged)</li><li>[pinMessage](@pinMessage)</li><li>[queryPinnedMessageList](@queryPinnedMessageList)</li></ul> |
...
|功能项-25% | 功能描述-40% | 相关接口-25%|
|-----|-----|-----|
| 支持设置用户在客户端发送的消息不计入未读数 | 当用户发送一些特殊场景的客户端消息时，可以设置此消息不计入未读数。例如，主播发起 PK 连麦的申请消息，接收方接收后不会增加对应的未读数。详细请参考 [收发消息 - 发送不计未读消息](./../Guides/Messaging/Send%20and%20receive%20messages.mdx#发送不计未读数的消息)。 | <ul><li>[sendMessage](@sendMessage)</li><li>[disableUnreadMessageCount](@disableUnreadMessageCount)</li></ul> |
...
| 获取房间用户扩展字段 | 用户在加入房间后，可获取此用户进房前的用户扩展信息。详细请参考 [获取房间用户扩展字段信息](./../Guides/Room/Room%20info.mdx#获取房间用户扩展字段信息)。 | [queryRoomMembers](@queryRoomMembers)、[queryRoomMemberList](@queryRoomMemberList)、[roomMemberJoined](@roomMemberJoined)、[roomMemberLeft](@roomMemberLeft) |
...
| 退群后查询群成员用户名称和头像逻辑优化 | 群成员用户名和头像信息与用户数据库关联，保障获取本地最新的用户信息。| [queryGroupMemberInfo](@queryGroupMemberInfo) |
...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
| -----  | ---- | ----- |
| 群备注  | 支持用户对某个群组设置仅自己可见的群备注。详情请参考 [群资料管理 - 修改群备注](./../Guides/Group/Group%20info.mdx#修改群备注)。| <ul><li>[updateGroupAlias](@updateGroupAlias)</li><li>[groupAliasUpdated](@groupAliasUpdated)</li></ul> |
...
| 切换房间  | 新增 `switchRoom` 接口以切换房间，适用于秒开等需要平滑切换房间的业务场景。详情请参考 [房间管理 - 切换房间](./../Guides/Room/Manage%20rooms.mdx#切换房间)。 | [switchRoom](@switchRoom) |
...
| -----  | ---- | ----- |
| 废弃原有单聊、群聊和房间消息接收回调 | 废弃 `receivePeerMessage`、`receiveGroupMessage` 和 `receiveRoomMessage`，请使用以下新回调替代：[onPeerMessageReceived](@onPeerMessageReceived)、[onGroupMessageReceived](@onGroupMessageReceived) 和 [onRoomMessageReceived](@onRoomMessageReceived)。<Note title="说明">接口变更详情请参考 [升级指南 - ZIM](./ZIM%20upgrade%20guide.mdx)。</Note>| <ul><li>[onPeerMessageReceived](@onPeerMessageReceived)</li><li>[onGroupMessageReceived](@onGroupMessageReceived)</li><li>[onRoomMessageReceived](@onRoomMessageReceived)</li></ul> |
...
| Tips 消息 | 支持将群内用户的某些群组操作（如创建群组、解散群组等） 转换为特殊类型消息（Tips）。凭借群组消息回调获得操作信息后，开发者可以自行构造并在 UI 上展示相关事件的描述文本。<br />如需了解 Tips 消息种类和扩展信息，以及调接收后的处理操作，请参考 [接收 Tips 消息](./../Guides/Messaging/Receive%20tip%20messages.mdx). | <ul><li>[ZIMMessageType > Tips](@Tips)</li><li>[receiveGroupMessage]()</li></ul> |
...
| 优化 [onRoomStateChanged](@onRoomStateChanged) 回调逻辑  | 当开发者调用 ZIM 服务端接口在后台销毁房间后，[onRoomStateChanged](@onRoomStateChanged) 回调描述房间状态变化的原因修改为 `RoomNotExist`。 | [onRoomStateChanged](@onRoomStateChanged) |
...
| -----  | ---- | ----- |
| <ul><li>[callInvitationAccepted]()</li><li>[callInvitationRejected](@callInvitationRejected)</li><li>[callInviteesAnsweredTimeout]()</li></ul> | 为便利开发者监听呼叫邀请中用户的呼叫状态变化，新增 [onCallUserStateChanged](@onCallUserStateChanged)，替代原呼叫邀请回调 `callInvitationAccepted`、`callInvitationRejected` 和 `callInviteesAnsweredTimeout`。 | 2.9.0 |
...
| <div style={{width:"130px"}}>废弃项</div>  | 废弃描述 | 相关接口 |
| -----  | ---- | ----- |
| 废弃系统消息类型 | [ZIMMessageType](@-ZIMMessageType) 废弃了系统消息（值为 30），开发者请使用功能更完善的自定义消息（值为 200）替代。 | [ZIMMessageType > Custom](https://doc-zh.zego.im/article/api?doc=zim_API~javascript_wxxcx~enum~ZIMMessageType#custom) |
...
| <div style={{width:"130px"}}>优化项</div>  | 优化描述 | 相关接口 |
| -----  | ---- | ----- |
| 变更 [ZIMPushConfig](@-ZIMPushConfig) 的额外字段命名| 为避免开发者接入概念混淆，[ZIMPushConfig](@-ZIMPushConfig) 的 extendedData 参数修改为 payload 参数，功能不变。<br />说明：<ul><li>若集成 ZIM SDK 2.5.0 版本后，开发者使用了离线推送功能后出现了接口编译失败的问题，请将 extendedData 改为 payload。</li><li>Web 端获取 payload 透传字段，需使用 **2.5.0** 或以上版本的 **ZPNs SDK**。</li></ul> | - |
...
| 收发富媒体消息 | 支持用户发送和接收图片、语音、视频和文件等类型的富媒体消息。| [sendMediaMessage]()|
...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
|-------|-------|-------|
| 安全审核 | 支持对单聊、群聊和房间内的通讯消息（文本消息）进行安全审核，审核范围包含：鉴黄、暴恐、违禁、涉政、广告等。| <ul><li>[sendPeerMessage](@sendPeerMessage)</li><li>[sendGroupMessage](@sendGroupMessage)</li><li>[sendRoomMessage](@sendRoomMessage)</li></ul> |
...
| 设置房间属性 | <p>房间属性指房间内保存的 `Key-Value` 属性对，开发者可以使用此功能为房间设置自定义信息。</p><p>此功能可用于语聊房、直播等场景，用来记录房间背景图地址、各用户的角色身份、设备状态等自定义信息。</p> | <ul><li>[createRoom](@createRoom)</li><li>[setRoomAttributes](@setRoomAttributes)</li><li>[queryRoomAllAttributes](@queryRoomAllAttributes)</li></ul> |
...
<tr>
<td>API 接口命名优化</td>
<td><b>查询房间成员</b> 接口命名优化：queryRoomMember 重命名为 [queryRoomMemberList](@queryRoomMemberList)。</td>
...
<td><ul><li>[sendPeerMessage](@sendPeerMessage)</li><li>[sendRoomMessage](@sendRoomMessage)</li></ul></td>
</tr>
<tr>
<td>部分回调接口，增加 roomID 参数，便于开发者在调用相关接口时，获取当前操作结果来源的房间 roomID。</td>
<td><ul><li>[ZIMRoomLeftResult](@-ZIMRoomLeftResult)</li><li>[ZIMRoomMemberQueriedResult](@-ZIMRoomMemberQueriedResult)</li><li>[ZIMRoomOnlineMemberCountQueriedResult](@-ZIMRoomOnlineMemberCountQueriedResult)</li></ul></td>
</tr>
</tbody></table>
