...
|查询历史消息支持配置优先同步服务端数据 | [queryHistoryMessage](@queryHistoryMessage) `config` 新增 `isPreferQueryFromServer` 参数，用于设置是否优先从服务端获取历史消息（默认优先使用本地缓存）。| [ZIMMessageQueryConfig](@-ZIMMessageQueryConfig)|
