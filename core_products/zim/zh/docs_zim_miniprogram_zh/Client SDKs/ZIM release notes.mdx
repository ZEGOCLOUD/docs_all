...
|网络连接优化| 微信小程序基础库版本升级到最新版本时，修复可能存在的无限重连问题 | [connectionStateChanged](@connectionStateChanged) |
...
| -----  | ---- | ----- |
| 废弃原有单聊、群聊和房间消息接收回调 | 废弃 `receivePeerMessage`、`receiveGroupMessage` 和 `receiveRoomMessage`，请使用以下新回调替代：[peerMessageReceived](@peerMessageReceived)、[groupMessageReceived](@groupMessageReceived) 和 [roomMessageReceived](@roomMessageReceived)。<Note title="说明">接口变更详情请参考 [升级指南 - ZIM](./ZIM%20upgrade%20guide.mdx)。</Note>| <ul><li>[peerMessageReceived](@peerMessageReceived)</li><li>[groupMessageReceived](@groupMessageReceived)</li><li>[roomMessageReceived](@roomMessageReceived)</li></ul> |
...
| 收发富媒体消息 | 支持用户发送和接收图片、语音、视频和文件等类型的富媒体消息。| [sendMediaMessage](@sendMediaMessage)|
