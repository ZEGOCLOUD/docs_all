...
废弃的 `sendPeerMessage`、`sendRoomMessage`、`sendGroupMessage` 以及 `sendMediaMessage` 接口已被移除，请统一使用带 `notification` 参数的 [sendMessage](@sendMessage) 接口代替。
...
`ZIMMessageType.System`（值为 30）已从 `ZIMMessageType` 枚举中移除，同时 `ZIMSystemMessage` 类型也已被移除。如需发送系统级指令，请使用 [ZIMCommandMessage](@ZIMCommandMessage) 代替。
...
```typescript 旧版本用法
// ZIMMessageType.System（值为 30）和 ZIMSystemMessage 已被移除
...
```typescript 旧版本用法
const avatarUrl: string = userFullInfo.userAvatarUrl; // 已废弃
```
</CodeGroup>
...
```typescript 旧版本用法
const seq: number = message.conversationSeq; // 已移除
```
</CodeGroup>
...
```typescript 旧版本用法
const isDeleteAll: boolean = deletedInfo.isDeleteConversationAllMessage; // 已移除
```
</CodeGroup>
...
```typescript 旧版本用法
const avatarUrl: string = groupMemberInfo.memberAvatarUrl; // 已移除
```
</CodeGroup>
...
zim.callInvite(invitees, config)
    .then((res: ZIMCallInvitationSentResult) => {
        for (const errorUser of res.info.errorUserList) {
...
zim.callInvite(invitees, config)
    .then((res: ZIMCallInvitationSentResult) => {
        for (const errorInvitee of res.info.errorInvitees) {
...
[ZIMMediaDownloadingProgress](@ZIMMediaDownloadingProgress) 以及 [ZIMMediaDownloadedResult](@-ZIMMediaDownloadedResult) 中，参数 `message` 的类型从 `ZIMMediaMessage` 变更为 `ZIMMessage`，以适应组合消息使用，TypeScript 开发者需要根据 IDE 的编译错误提示修正调用。
