...
| sendMessage 及相关回调 | <ul><li>自 2.19.0 版本后，发送多媒体消息也使用 [sendMessage](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIM/sendMessage.html) 接口，原发送多媒体消息接口 `sendMediaMessage` 被废弃，减少开发者的维护成本。</li><li>[ZIMMessageSendNotification](@-ZIMMessageSendNotification) 中的 [onMediaUploadingProgress](@onMediaUploadingProgress-ZIMMessageSendNotification) 回调方法参数 `message` 类型从 `ZIMMessage` 变更为 `ZIMMediaMessage`，以确保仅媒体消息会触发此回调。</li></ul> |
