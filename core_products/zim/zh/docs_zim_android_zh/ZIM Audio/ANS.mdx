...
export const initMap = {
  ...
  'iOS,UTS': <a href='@initWithLicense' target='_blank'>initWithLicense</a>,
...
export const enableANSMap={
  "Android,iOS,RN,UTS": <a href="@enableANS" target="_blank">enableANS</a>,
...
export const setANSParamMap={
  "Android,iOS,RN,UTS": <a href="@setANSParam" target="_blank">setANSParam</a>,
...
启用噪声控制功能后，可开始 [录制音频文件](./Send%20and%20receive%20audio%20messages.mdx#2-录制音频文件)。
...
```typescript
// 调整降噪模式为 aggressive
ZIMAudio.getInstance().setANSParam({mode: 2} as ZIMAudioANSParam);
```
:::