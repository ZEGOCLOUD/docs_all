...
1. 参考安卓厂商推送集成指南（[小米](./Integrate%20Xiaomi.mdx)/[华为](./Integrate%20Huawei.mdx)/[OPPO](./Integrate%20OPPO.mdx)/[vivo](./Integrate%20vivo.mdx)/[Google FCM](./Integrate%20FCM.mdx)），为 2 个应用分别获取适用于各厂商的 2 份离线推送证书。
2. 参考 [控制台 - 离线推送配置](https://doc-zh.zego.im/console/service-configuration/im/offline-push-config)，在 ZEGO 控制台上为各厂商配置上述获取的 2 份推送证书。
