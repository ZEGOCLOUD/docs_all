...
export const registerPushMap = {
  'Android,RN,UTS': <a href='@registerPush' target='_blank'>registerPush</a>,
...
export const ZPNsEventHandlerMap = {
  'Android,RN,UTS': <a href='@-ZPNsEventHandler' target='_blank'>ZPNsEventHandler</a>,
...
export const onNotificationClickedMap = {
  ...
  'RN,UTS': <a href='@notificationClicked' target='_blank'>notificationClicked</a>,
...
<Note title="说明">本文档仅适用于华为、小米、OPPO、VIVO 推送通道。</Note>
:::
:::if{props.platform="Flutter|RN|UTS"}
<Note title="说明">本文档仅适用于华为、小米、OPPO、VIVO 推送通道以及 Apple 推送服务（APNs）。</Note>
:::
...
:::if{props.platform="undefined|Flutter|RN|UTS"}
- 自定义前：
    <Video src="https://doc-media.zego.im/docuo/7a952bc33f.mp4" />
- 自定义后：
    <Video src="https://doc-media.zego.im/docuo/7b35ed7b40.mp4" />
:::
...
- 已在控制台配置离线推送证书，详情请参考控制台文档 [服务配置 - 即时通讯 - 离线推送配置](https://doc-zh.zego.im/console/service-configuration/im/offline-push-config)。

## 实现流程

<Steps>
<Step title="开通功能">
开启自定义点击跳转功能请参考 [服务配置 - 即时通讯 - 自定义点击跳转配置](/console/service-configuration/im/custom-notification-click-redirection)。
</Step>
:::if{props.platform=undefined}
<Step title="重写回调">
请在自定义的 [ZPNsMessageReceiver](@-ZPNsMessageReceiver) 广播类派生类中（请参考 [实现离线推送->注册离线推送](./Implement%20offline%20push%20notification.mdx#注册离线推送) 的类似内容），重写 {getPlatformData(props,onNotificationClickedMap)} 方法，以便获取跳转目标页面的路径。
```java
...
ZPNsEventHandler.onNotificationClicked = (ZPNsMessage message){
    if(zpnsMessage.pushSourceType == ZPNsPushSourceType.FCM){
        ...
        String payload = message.extras['payload']; 
        // 您可以根据上述信息编写代码实现跳转
    }
};
```
...
ZPNs.getInstance().on('notificationClicked', message => {
    if (message.pushSourceType == ZPNsPushSourceType.FCM){
        ...
        const payload = message.extras.payload; 
        // 您可以根据上述信息编写代码实现跳转
    }
});
```
...
:::
<Step title="配置透传字段">
消息发送者在发送消息时，需要通过 `ZIMPushConfig > payload` 参数设置透传字段（例如需要跳转到的目标页面路径）。相关代码请参考 [实现离线推送 - 使用 ZIM SDK 实现离线推送功能](./Implement%20offline%20push%20notification.mdx#使用-zim-sdk-实现离线推送功能) 章节。
</Step>
<Step title="触发回调">
...
<Warning title="注意">
    Android 平台仅当在 ZEGO 控制台配置了 `点击后续动作` 为 `打开应用内指定页面` 后，才会触发此回调。
</Warning>
</Step>
</Steps>
