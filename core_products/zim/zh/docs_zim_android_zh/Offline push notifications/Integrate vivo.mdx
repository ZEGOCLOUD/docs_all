...
- 已完成如下步骤：
    ...
    <Tabs>
    <Tab title="uni-app x">
    编辑 `uni_modules/zego-zpns-uts/utssdk/app-android/AndroidManifest.xml` 文件，填入上一步获取到的 AppID、AppKey。

    ```xml
    <application>
        <!-- 配置 vivo -->
        <meta-data android:name="com.vivo.push.app_id" android:value="填入 AppID" />
        <meta-data android:name="com.vivo.push.api_key" android:value="填入 AppKey" />
    </application>
    ```
    </Tab>
    <Tab title="uni-app">
    打开 “manifest.json” 文件，选择“App原生插件配置”一栏，填入上一步获取到的 AppID、AppKey。
    ...
    5. 以上配置完成后，请在 [ZEGO 控制台](https://console.zego.im/) 自助配置 ZIM 离线推送证书（详情请参考 [项目管理 - 即时通讯 - 离线推送配置](https://doc-zh.zego.im/console/service-configuration/im/offline-push-config)），若无法配置，请联系 ZEGO 技术支持处理。
    </div>
    :::
...
1. 请参考 [resourcesID 说明 - 配置](./ResourcesID%20introduction.mdx#配置)，在 ZEGO 控制台上使用如下信息配置 resourcesID： 
    ...
    <tr>
    <td>category</td>
    <td>推送类别。详细请参考 [vivo 开发平台](https://dev.vivo.com.cn/documentCenter/doc/359#s-jfhljivk)。<Warning title="注意"><ul><li>填写 category 后，可以不填写 classification。</li><li>但若填写 classification，请保证 category 与 classification 是正确对应关系，否则无法推送。</li></ul></Warning></td>
    </tr>
    <tr>
...
3. 在发起推送时，可以指定使用该 Resource ID 配置系统消息通道进行推送。您需要将配置的 Resource ID 填写到 ZIMPushConfig 中，实现代码请参考 [实现离线推送 - 发送单聊消息时使用离线推送功能](./Implement%20offline%20push%20notification.mdx#场景-1发送单聊消息时使用离线推送功能)。
