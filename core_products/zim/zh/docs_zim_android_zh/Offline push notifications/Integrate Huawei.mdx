...
1. 下载并配置 agconnect-services.json。
    ...
    登录 <a href="https://developer.huawei.com/consumer/cn/" target="_blank" rel="noreferrer noopener">华为开发者后台</a> ，在 “管理中心 > PUSH > 选择您的应用 > 项目设置 > 常规”，找到 “agconnect-services.json” 文件，点击下载，如下图：
...
<div>
    <Tabs>
    <Tab title="uni-app x">
    将下载好的 “agconnect-services.json” 文件，复制到自己项目根目录下的 “nativeResources/android” 目录。
    </Tab>
    <Tab title="uni-app">
    将下载好的 “agconnect-services.json” 文件，复制到自己项目根目录下的 “nativeplugins/zego-ZPNsUniPlugin/android/assets” 目录。
    </Tab>
    </Tabs>

2. 自助配置 ZIM 离线推送证书

以上配置完成后，请在 [ZEGO 控制台](https://console.zego.im/) 自助配置 ZIM 离线推送证书（详情请参考 [项目管理 - 即时通讯 - 离线推送配置](https://doc-zh.zego.im/console/service-configuration/im/offline-push-config)），若无法配置，请联系 ZEGO 技术支持处理。
</div>
:::
:::if{props.platform="undefined|RN"}
    <div>
    将下载好的 “agconnect-services.json” 文件，复制到自己项目的 “app” 目录下，如下图：

    <Frame width="512" height="auto" caption=""><img src="https://doc-media.zego.im/sdk-doc/Pics/ZIM/offline_push_huawei_json.png" /></Frame>

2. 配置 HMS Core SDK 与 mavenCentral 的 Maven 仓地址。
...
3. 在应用级别的 “build.gradle” 文件中添加 agcp 插件配置。
...
2. 完成上述申请后，请参考 [resourcesID 说明 - 配置](./ResourcesID%20introduction.mdx#配置)，在 ZEGO 控制台上使用如下信息配置 resourcesID：
    - `resourcesID`：由您自定义，不超过 32 个字符）。
    - `category`：推送类别。详细请参考 [华为 HarmonyOS 开发平台](https://developer.huawei.com/consumer/cn/doc/HMSCore-Guides/message-classification-0000001149358835#section1653845862216)。
...
3. 在发起推送时，可以指定使用该 resourcesID 对应的通道进行推送。您需要将配置的 Resource ID 填写到 ZIMPushConfig 中，实现代码请参考 [实现离线推送 - 发送单聊消息时使用离线推送功能](./Implement%20offline%20push%20notification.mdx#场景-1发送单聊消息时使用离线推送功能)。
