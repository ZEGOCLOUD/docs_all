...
- 已完成如下步骤：
    ...
    <div>
    4. 以上配置完成后，请在 [ZEGO 控制台](https://console.zego.im/) 自助配置 ZIM 离线推送证书（详情请参考 [项目管理 - 即时通讯 - 离线推送配置](https://doc-zh.zego.im/console/service-configuration/im/offline-push-config)），若无法配置，请联系 ZEGO 技术支持处理。
    </div>
    :::
...
2. 创建 Channel 后，请参考 [resourcesID 说明 - 配置](./ResourcesID%20introduction.mdx#配置)，在 ZEGO 控制台上使用如下信息配置 resourcesID：
...
3. 在发起推送时，可以指定使用该 Resource ID 对应的通道进行推送。您需要将配置的 Resource ID 填写到 ZIMPushConfig 中，实现代码请参考 [实现离线推送 - 发送单聊消息时使用离线推送功能](../Offline%20push%20notifications/Implement%20offline%20push%20notification.mdx)。
