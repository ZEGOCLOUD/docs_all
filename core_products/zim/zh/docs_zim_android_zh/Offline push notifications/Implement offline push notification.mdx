...
请参考 Android 推送集成指南（[小米](./Integrate%20Xiaomi.mdx)/[华为](./Integrate%20Huawei.mdx)/[OPPO](./Integrate%20OPPO.mdx)/[vivo](./Integrate%20vivo.mdx)/[FCM](./Integrate%20FCM.mdx)），集成需要使用到的第三方厂商离线推送 SDK，接入各厂商的离线推送通道。
...
2. 进入 “app” 目录，打开 “build.gradle” 文件，在 “dependencies” 中添加 `implementation 'im.zego:zpns:x.y.z'`。“x.y.z” 为 SDK 的版本号，请参考 [发布日志](./../Client%20SDKs/ZPNs%20release%20notes.mdx) 中的发布历史获取。
...
<Tab title="方式2">
1. 请参考 [下载 SDK](./../Client%20SDKs/SDK%20downloads.mdx)，下载最新版本的 ZPNs SDK。
...
1. 开发者创建新的 Java 类 MyZIMPushReceiver 继承自 ZPNs 中的 [ZPNsMessageReceiver](@-ZPNsMessageReceiver) 广播类，用于接收厂商推送消息。
...
<Accordion title="展开了解如何获取 payload 透传字段" defaultOpen="false">
    ...
    在上述方法触发时将 [ZPNsMessage](@-ZPNsMessage) 对象传给该方法，取出 payload 字段。
...
3. 配置第三方推送通道。
    ...
    根据 [前提条件](#接入第三方厂商离线推送通道) 集成的第三方厂商离线推送 SDK，调用 [enableHWPush](@enableHWPush)/[enableMiPush](@enableMiPush)/[enableOppoPush](@enableOppoPush)/[enableVivoPush](@enableVivoPush) 接口，启用各厂商的推送功能，然后调用 [setPushConfig](@setPushConfig) 接口，配置第三方推送通道。
...
4. 注册离线推送功能。
    ...
    注册离线推送功能后，可以通过继承 [ZPNsMessageReceiver](@-ZPNsMessageReceiver) 类中的 [onRegistered](@onRegistered) 回调，获取到离线推送的 `pushID`，向指定设备推送离线消息。

5. 如果不再需要离线推送功能，开发者可以调用 [unregisterPush](@unregisterPush) 接口，注销该功能，注销后用户将不再能收到推送消息。
...
3. 发送方调用 [sendMessage](@sendMessage)，传入 “sentConfig”，向接收方发送单聊消息。
...
4. 被邀请的用户若处于离线状态，将会收到对应的离线推送，上线后，若呼叫邀请还未结束，将会收到 [onCallInvitationReceived](@onCallInvitationReceived) 的回调。
