...
4. 注册离线推送。
    ...
    注册离线推送后，可以通过继承 [ZPNsMessageReceiver](@-ZPNsMessageReceiver) 类中的 [onRegistered](@onRegistered) 回调，获取到离线推送的 `pushID`，向指定设备推送离线消息。

### 注销离线推送

如果不再需要离线推送功能，开发者可以调用 [unregisterPush](@unregisterPush) 接口，注销该功能，注销后用户将不再能收到推送消息。
