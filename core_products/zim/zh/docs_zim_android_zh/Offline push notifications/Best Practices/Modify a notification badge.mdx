...
4. 开启华为角标
    ...
    在调用 [registerPush](@registerPush) 注册离线推送之前，调用 [setPushConfig](@setPushConfig) 接口，开启华为角标功能。
...
ZPNs 封装了支持修改本地角标数的厂商（华为、vivo、OPPO）的相关 API。当您需要修改本地角标时。请调用 [setApplicationIconBadgeNumber](@setApplicationIconBadgeNumber) 接口。
