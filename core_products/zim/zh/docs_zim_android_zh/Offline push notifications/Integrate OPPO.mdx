...
3. 在发起推送时，可以指定使用该 Resource ID 对应的通道进行推送。您需要将配置的 Resource ID 填写到 ZIMPushConfig 中，实现代码请参考 [实现离线推送 - 发送单聊消息时使用离线推送功能](./Implement%20offline%20push%20notification.mdx#场景-1发送单聊消息时使用离线推送功能)。
...
2. 完成上述申请后，请参考[resourcesID 说明 - 配置](./ResourcesID%20introduction.mdx#配置)，在 ZEGO 控制台上使用如下信息配置 resourcesID： 
    - `resourcesID`：由您自定义，不超过 32 个字符。
    - `category`：消息类别。详细请参考 [OPPO 开放平台](https://open.oppomobile.com/documentation/page/info?id=13189)。
...
4. 根据 [OPUSH私信模版校验能力接入说明](https://open.oppomobile.com/documentation/page/info?id=12391)，OPPO 在新消息分类的基础上新增了推送私信模版。当您在 OPPO 控制台配置好私信模版后，如需在推送时应用对应模版，可在 [ZIMPushConfig](@-ZIMPushConfig) 为 privateMessageTemplate 赋值。
:::if{props.platform="undefined|Flutter"}
```java
...
```typescript
const template: ZIMOfflinePushPrivateMessageTemplate = {
  //OPPO 推送私信模板 ID，下发对应私信模板时必须携带。如果resourceID 中 OPPO 的 category 设置分类为内容与营销，则此字段无效。
  templateID: "",
  //json 字符串， OPPO 推送标题模板填充参数。例：私信模板 ID 标题模板为：欢迎来到$ {city} $ ，$ {city} $ 欢迎您。此参数内容为："{"city":"北京"}"
  titleParameters: "",
