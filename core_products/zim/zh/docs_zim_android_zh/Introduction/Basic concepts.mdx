...
[群组](./../Guides/Group/Manage%20groups.mdx)，指两个或以上的用户一起进行聊天。有成员在群组内发送消息时，其他成员会收到消息；当 App 在后台运行或者 App 进程被杀死后，不在线的群内成员也可以接收到离线消息的推送通知。
...
[房间](./../Guides/Room/Manage%20rooms.mdx)指两个或以上的用户一起进行聊天。有用户在房间内发送消息时，其他用户会收到消息；当 App 在后台运行或者 App 进程被杀死后，不会接收到消息的推送通知。
...
每个单聊、群聊会有单独的会话 ID。[会话列表](./../Guides/Conversation/Get%20the%20conversation%20list.mdx)指各类会话 ID 根据时间顺序排列的列表。列表的顺序会按照各个会话最近一条消息的时间、置顶等因素影响。房间聊天不属于会话列表范畴。
...
目前 ZIM 已支持 [Apple](./../Offline%20push%20notifications/Integrate%20APNs.mdx)、华为、小米、vivo、OPPO、Google 等主流手机厂商。
:::
:::if{props.platform=undefined}
目前 ZIM 已支持 Apple、[华为](./../Offline%20push%20notifications/Integrate%20Huawei.mdx)、[小米](./../Offline%20push%20notifications/Integrate%20Xiaomi.mdx)、[vivo](./../Offline%20push%20notifications/Integrate%20vivo.mdx)、[OPPO](./../Offline%20push%20notifications/Integrate%20OPPO.mdx)、[Google](./../Offline%20push%20notifications/Integrate%20FCM.mdx) 等主流手机厂商。
:::
:::if{props.platform="Flutter|RN|uniapp"}
目前 ZIM 已支持 [Apple](./../Offline%20push%20notifications/Integrate%20APNs.mdx)、[华为](./../Offline%20push%20notifications/Integrate%20Huawei.mdx)、[小米](./../Offline%20push%20notifications/Integrate%20Xiaomi.mdx)、[vivo](./../Offline%20push%20notifications/Integrate%20vivo.mdx)、[OPPO](./../Offline%20push%20notifications/Integrate%20OPPO.mdx)、[Google](./../Offline%20push%20notifications/Integrate%20FCM.mdx) 等主流手机厂商。
:::
...
:::if{props.platform="undefined|iOS|mac|Windows|RN"}

## 社群

社群（Community）指通过即时通讯能力搭建的一个大型互动社区，业内的开拓者 Discord 的创建了这一社交模式，并且定义和实现了 Server-Chanel-Thread 的三层社交组织结构实时互动社区。满足游戏玩家、兴趣小组、粉丝运营、教育培训等各个类型用户进行大范围实时互动聊天的场景。
