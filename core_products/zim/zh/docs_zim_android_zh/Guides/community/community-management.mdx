...
- `communityID` 支持开发者自定义，仅支持数字、英文字符及以下特殊字符：`! # $ % & ( ) + - : ; < = . > ? @ [ ] ^ _ { } | ~`，且不能以 `#` 开头。若不填，ZIM 服务端会创建以 #B 开头的 communityID 的社群。
- 创建成功后即已加入社群，无需再调用加入接口。每个用户最多同时加入 1000 个社群。同一 AppID 下同时存在的社群数量上限为 1000000。
</Warning>
