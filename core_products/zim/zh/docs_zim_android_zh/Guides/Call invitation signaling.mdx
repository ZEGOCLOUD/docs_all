...
```javascript
// 监听呼叫邀请相关用户的状态变化
zim.onCallUserStateChanged((zim, info) => {
    // 相关的 callID
    const changeCallID = info.callID;
    info.callUserList.forEach(userInfo => {
        // 状态变化用户的 userID、最新用户状态、透传字段（与用户该调用接受、拒绝、退出呼叫时携带的 extended data 一致）
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/ZIM/send_1.jpeg" /></Frame>
...
2. 客户端 A 通过调用 {getPlatformData(props,callInviteMap)} 接口，发起呼叫邀请，客户端 B 在收到邀请信息后，可以选择 <a href="#4-接受呼叫邀请">接受</a> 或者 <a href="#5-拒绝呼叫邀请">拒绝</a>。
    ...
    - 在 timeout 外登录，请调用 {getPlatformData(props,queryCallInvitationListMap)} 来 <a href="#6">查询呼叫邀请列表</a>。
    </Note>
...
:::if{props.platform="Web"}
<CodeGroup>
```javascript title="向在线用户发送呼叫邀请"
/** 向在线用户发送呼叫邀请 */
const invitees = ['xxxx'];  // 被邀请人ID列表
const config = { timeout: 200 }; //邀请超时时间，单位为秒，范围1-600   
...
```javascript
/** 邀请发起者收到呼叫邀请已创建的通知 */
zim.onCallInvitationCreated((zim, info) => {
...
```javascript
/** 被邀请者收到邀请后的回调通知 */
zim.onCallInvitationReceived((zim, { callID, inviter, timeout, extendedData }) => {
    // console.log('onCallInvitationReceived', { callID, inviter, timeout, extendedData })
})
```
:::
...
const callID = 'xxxx';
const invitees = ['xxxx'];  // 被邀请人ID列表
const config = { extendedData: 'xxxx' }; 
...
```javascript
// 被邀请者收到取消邀请后的回调通知
zim.onCallInvitationCancelled((zim, { callID, inviter, extendedData }) => {
    // console.log('onCallInvitationCancelled', { callID, inviter, extendedData })
})
```
:::
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/ZIM/timeout_new.png" /></Frame>
...
```javascript
// 被邀请者响应超时后，“被邀请者”收到的回调通知，超时时间单位：秒
zim.onCallInvitationTimeout(zim, { callID }) => {
    // console.log('onCallInvitationTimeout', { callID })
})
```
:::
...
const config = { timeout: 200, mode: 1 };
zim.callInvite(invitees, config)
    .then(({ callID, timeout, errorInvitees }) => {
        // 操作成功
        // 此处的 callID 是用户发起呼叫后，SDK 内部生成的 ID，用于唯一标识一次呼叫邀请；之后发起人取消呼叫、被邀请人接受/拒绝呼叫，都会使用此 callID
...
```javascript
// 呼叫成员收到的回调通知
zim.onCallInvitationEnded((zim, info) => {
...
<Note title="说明">
请参考 <a href="#1-监听呼叫邀请相关用户的状态变化">如何监听呼叫邀请相关用户的状态变化</a> 了解更多。
</Note>
