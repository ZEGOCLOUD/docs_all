...
```typescript
/** 被邀请者收到邀请后的回调通知 */
zim.onCallInvitationReceived(({ callID, inviter, timeout, extendedData }) => {
...
```typescript
// 被邀请者收到取消邀请后的回调通知
zim.onCallInvitationCancelled(({ callID, inviter, extendedData }) => {
...
```typescript
// 被邀请者响应超时后，“被邀请者”收到的回调通知，超时时间单位：秒
zim.onCallInvitationTimeout({ callID }) => {
