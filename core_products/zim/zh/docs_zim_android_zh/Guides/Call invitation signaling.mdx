...
| Accepted  | 已接受邀请。 | <ul><li>用户发起呼叫邀请。</li><li>用户接受呼叫邀请。</li></ul>                           | !mu                |
| Rejected  | 已拒绝邀请。 | 用户拒绝呼叫邀请。                         | !mu                |
| Cancelled | 已取消邀请。 | <ul><li>用户主动取消呼叫邀请。</li><li>被叫尚未应答邀请，而主叫掉线且心跳超时。</li></ul> | !mu                |
| Received  | 已收到邀请。 |  <ul><li>在线用户收到邀请。</li><li>离线用户在超时范围内上线。</li></ul>                   | !mu                |
| Timeout   | 超时未接受。 | 被叫用户超时未响应邀请。                                                | !mu                |
| Quit      | 退出。       | 呼叫实现后，用户退出呼叫。                                              | 进阶模式           |
| Unknown   | 未知。       | 请联系 ZEGO 技术支持。                                                  | !ml                |
...
```typescript
// 监听邀请者和被邀请者的状态变化
zim.on('callUserStateChanged', (zim: ZIM, info: ZIMCallUserStateChangedEventResult) => {
    // 相关的 callID
...
```typescript
/** 邀请发起者收到呼叫邀请已创建的通知 */
zim.on('callInvitationCreated', (zim: ZIM, info: ZIMCallInvitationCreatedEventResult) => {
...
```typescript
/** 被邀请者收到邀请后的回调通知 */
zim.on('callInvitationReceived', (zim: ZIM, data: ZIMCallInvitationReceivedEventResult) => {
})
```
:::
:::if{props.platform="UTS"}
...
```typescript
// 被邀请者收到取消邀请后的回调通知
zim.on('callInvitationCancelled',(zim: ZIM, data: ZIMCallInvitationCancelledEventResult) => {
})
```
:::
:::if{props.platform="UTS"}
...
```typescript
// 被邀请者响应超时后，“被邀请者”收到的回调通知，超时时间单位：秒
zim.on('callInvitationTimeout', (zim: ZIM, data: ZIMCallInvitationTimeoutEventResult) => {
})
```
:::
:::if{props.platform="UTS"}
...
```typescript
// 呼叫成员收到的回调通知
zim.on('callInvitationEnded', (zim: ZIM, info: ZIMCallInvitationEndedEventResult) => {
