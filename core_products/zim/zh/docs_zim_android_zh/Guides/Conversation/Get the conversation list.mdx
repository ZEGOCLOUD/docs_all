...
```typescript
zim.on('conversationSyncStateChanged', (zim: ZIM, data: ZIMEventOfConversationSyncStateChangedResult) => {
...
zim.queryConversationList(config)
    .then((res: ZIMConversationListQueriedResult) => {
        ...
        // 分页查询第二页会话
        config.nextConversation = res.conversationList[res.conversationList.length - 1];
        zim.queryConversationList(config).then((res: ZIMConversationListQueriedResult) => {
        });        
    })
    .catch((err: ZIMError) => {
        // 查询失败
...
zim.queryConversationList(config, null)
    .then((res: ZIMConversationListQueriedResult) => {
        ...
        config.nextConversation = conv.type.toString() + conv.conversationID;
        zim.queryConversationList(config, null).then((res: ZIMConversationListQueriedResult) => {
        });
    })
    .catch((err: ZIMError) => {
        // 查询失败
