...
zim.onRroomAttributesUpdated(function (zim, { roomID, infos }) {
    console.log('roomAttributesUpdated', roomID, infos);
});
```
</CodeGroup>
...
zim.onRoomAttributesUpdated(function (zim, { roomID, infos }) {
    console.log('roomAttributesUpdated', roomID, infos);
});
```
</CodeGroup>
...
:::if{props.platform="U3d"}
通过调用 [QueryRoomAllAttributes](@QueryRoomAllAttributes) 接口，可以获取房间的所有属性。
:::
...
zim.onRroomAttributesBatchUpdated(function (zim, { roomID, infos }) {
    console.log('roomAttributesBatchUpdated', roomID, infos);
});
```
</CodeGroup>
