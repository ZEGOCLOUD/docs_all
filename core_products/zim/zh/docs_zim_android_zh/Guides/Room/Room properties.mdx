...
:::if{props.platform="Flutter"}
设置房间属性后，可以通过 [ZIMRoomAttributesOperatedResult](/zim-flutter/client-sdks/api-reference/struct#zimroomattributesoperatedresult) 类型的异步返回值，返回设置房间属性的操作结果。
:::
:::if{props.platform=undefined}
<CodeGroup>
```java title="示例代码"
...
zim.on('roomAttributesUpdated', (zim: ZIM, data: ZIMRoomAttributesUpdatedEventResult) => {
...
zim.on('roomAttributesBatchUpdated', (zim: ZIM, data: ZIMRoomAttributesUpdatedEventResult) => {
