...
用户加入房间后，通过 `ZIMUserInfo.userExtendedData` 字段可获取到该用户进房前的扩展信息快照，实现房间用户等级、VIP 用户标识等业务场景。

支持的接口：{getPlatformData(props,queryRoomMemberListMap)}、{getPlatformData(props,queryRoomMembersMap)}、{getPlatformData(props,onRoomMemberJoinedMap)}、{getPlatformData(props,onRoomMemberLeftMap)}。

<Note title="说明">
- 2.22.0 版本开始支持。
- 用户加入房间后再次修改的扩展字段信息，通过该接口获取时，不会生效（比如：进房前的 userExtendedData 为 “extended1”，进房后修改为“extended2”，通过该接口获取的内容为 “extended1”）。
</Note>
