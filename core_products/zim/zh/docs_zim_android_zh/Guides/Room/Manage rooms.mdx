...
export const createRoomMap = {
  ...
  'UTS': <a href='@createRoom1' target='_blank'>createRoom</a>,
...
export const platformMap = {
  'Android':<a href="https://doc-zh.zego.im/faq/zim-reconnect-support" target="_blank" > 场景 4 : ZIM 房间断网 </a>,
  'window':<a href="https://doc-zh.zego.im/faq/zim-reconnect-support" target="_blank" > 场景 4 : ZIM 房间断网 </a>,
  'Flutter':<a href="https://doc-zh.zego.im/faq/zim-reconnect-support" target="_blank" > 场景 4 : ZIM 房间断网 </a>,
  'mac':<a href="https://doc-zh.zego.im/faq/zim-reconnect-support" target="_blank" > 场景 4 : ZIM 房间断网 </a>,
  'iOS':<a href="https://doc-zh.zego.im/faq/zim-reconnect-support" target="_blank" > 场景 4 : ZIM 房间断网 </a>,
  'Web':<a href="https://doc-zh.zego.im/faq/zim-reconnect-support" target="_blank" > 场景 4 : ZIM 房间断网 </a>,
  'U3d':<a href="https://doc-zh.zego.im/faq/zim-reconnect-support" target="_blank" > 场景 4 : ZIM 房间断网 </a>,
  'RN':<a href="https://doc-zh.zego.im/faq/zim-reconnect-support" target="_blank" > 场景 4 : ZIM 房间断网 </a>,
  'uniapp,UTS':<a href="https://doc-zh.zego.im/faq/zim-reconnect-support" target="_blank" > 场景 4 : ZIM 房间断网 </a>,
  'wxxcx':<a href="https://doc-zh.zego.im/faq/zim-reconnect-support" target="_blank" > 场景 4 : ZIM 房间断网 </a>,
}
...
房间内的用户，可以通过 {getPlatformData(props,sendMessageMap)} 接口，向房间内发送消息，详情请参考 [收发房间消息](./../../Send%20and%20receive%20messages.mdx)。

<Note title="说明">

如果 roomID 已存在：

- 调用 {getPlatformData(props,createRoomMap)} 接口，会返回相关错误码，详情请参考 [常见错误码](./../../SDK%20Error%20Codes/ZIM.mdx)。
...
```typescript
zim.on('roomMemberJoined', (zim: ZIM, data: ZIMRoomMemberChangedEventResult) => {
    // to do something
});
```
:::
...
:::if{props.platform="Web|RN|uniapp"}
```typescript
zim.on('roomMemberLeft', (zim: ZIM, data: ZIMRoomMemberChangedEventResult) => {
    // to do something
});
```
:::
...
:::if{props.platform="Web|RN|uniapp"}
```typescript
zim.on('roomStateChanged', (zim: ZIM, data: ZIMRoomStateChangedEventResult) => {
    // to do something
});
```
:::
...
客户端 A 登录后，创建一个房间，可以调用 {getPlatformData(props,createRoomMap)} 接口，传入 {getPlatformData(props,ZIMRoomInfoMap)} 信息，即可创建并加入房间。同时可以通过错误码 `ZIMError` 的参数值，判断是否创建成功。相关错误码请查看 [常见错误码](./../../SDK%20Error%20Codes/ZIM.mdx)。
...
客户端 B 加入房间，可以调用 {getPlatformData(props,joinRoomMap)} 接口，传入由 A 创建的房间 roomID，即可加入房间。同时可以根据错误码 `ZIMError` 的参数值，判断用户是否加入成功。相关错误码请查看 [常见错误码](./../../SDK%20Error%20Codes/ZIM.mdx)。
...
5. 房间内的用户，同样可以使用 {getPlatformData(props,setEventHandlerMap)} 回调接口 {getPlatformData(props,onRoomMemberJoinedMap)} 方法，实现对房间内成员加入的监听。
:::
:::if{props.platform="UTS"}
...
:::if{props.platform="undefined|iOS|mac|window|Flutter|Web|RN|uniapp"}
房间切换成功后，原房间内的其他用户可通过 {getPlatformData(props,setEventHandlerMap)} 回调接口 {getPlatformData(props,onRoomMemberLeftMap)} ，得知有用户离开房间；目标房间内的其他用户可通过 {getPlatformData(props,setEventHandlerMap)} 回调接口 {getPlatformData(props,onRoomMemberJoinedMap)} ，得知有用户进入了房间。
:::
:::if{props.platform="UTS"}
...
:::if{props.platform="undefined|iOS|mac|window|Flutter|Web|RN|uniapp"}
用户如果想要离开房间，可以调用 {getPlatformData(props,leaveRoomMap)} 接口，传入房间的 roomID，即可退出此房间；房间内的其他用户可以通过 {getPlatformData(props,setEventHandlerMap)} 回调接口 {getPlatformData(props,onRoomMemberLeftMap)} ，收到成员变动通知。
:::
:::if{props.platform="UTS"}
...
:::if{props.platform="undefined|iOS|mac|window|Flutter|Web|RN|uniapp"}
房间内的其他用户可以通过 {getPlatformData(props,setEventHandlerMap)} 回调接口 {getPlatformData(props,onRoomMemberLeftMap)} ，收到成员变动通知。
:::
:::if{props.platform="UTS"}
