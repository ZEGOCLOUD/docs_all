...
export const createGroupMap = {
  ...
  'Web':<a href='@createGroup__1' target='_blank'>createGroup</a>,
  'Flutter': <a href='https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIM/createGroup.html' target='_blank'>createGroup</a>,
}
...
开发者可以通过 {getPlatformData(props,ZIMGroupCreatedCallbackMap)} ，判断群组是否创建成功。相关错误码请查看 [常见错误码](./../../SDK%20Error%20Codes/ZIM.mdx)。
...
- 同一 AppID 下 groupID 存在唯一性，一旦以某个 groupID 创建过群，此后无论该群是否解散，后续群均无法使用该 groupID。
</Warning>
...
// 注册监听“群成员状态变更”的回调
zim.on('groupMemberStateChanged', (zim: ZIM, data: ZIMGroupMemberStateChangedEventResult) => {
    console.log('groupMemberStateChanged', data);
});

// 注册监听“群状态变更”的回调
zim.on('groupStateChanged', (zim: ZIM, data: ZIMGroupStateChangedEventResult) => {
...
zim.on('groupApplicationListChanged', (zim: ZIM, data: ZIMGroupApplicationListChangedEventResult) => {
...
zim.on('groupApplicationUpdated', (zim: ZIM, data: ZIMGroupApplicationUpdatedEventResult) => {
...
用户登录 ZIM SDK 后，如果想要了解自己和他人共同加入了哪些群组，可以通过 {getPlatformData(props,queryGroupListMap)} 接口，获取已加入的共同群组列表。
...
<Frame width="auto" height="auto">
  <img src="https://doc-media.zego.im/docuo/303ae643b9.jpg" alt="Group_Muted_New.jpg"/>
</Frame>
...
如果您希望禁止特定群组成员发言，请参考 [群成员管理 - 设置群成员禁言状态](./Group%20members.mdx)。
</Note>
...
:::if{props.platform="Web"}
```typescript
zim.on('groupMutedInfoUpdated', (zim: ZIM, data: ZIMGroupMutedInfoUpdatedEventResult) => {
...
- 参考 [会话管理 - 拉取会话列表](../Conversation/Manage%20unread%20message%20counts.mdx#拉取会话列表) 主动拉取会话列表。
- 参考 [会话管理 - 更新会话列表](../Conversation/Manage%20unread%20message%20counts.mdx#更新会话列表)，监听会话变更回调，更新会话列表。
...
zim.on('groupVerifyInfoUpdated', (zim: ZIM, data: ZIMGroupVerifyInfoUpdatedEventResult) => {
});
