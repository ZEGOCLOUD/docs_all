...
export const mutedExpiredTimeMap = {
  ...
  'Flutter': <a href='/zim-flutter/client-sdks/api-reference/struct#selfmutedexpiredtime' target='_blank'>selfMutedExpiredTime</a>,
}
...
用户使用“群成员管理”功能之前，请先加入某个群组，否则无法使用相关功能，详情请参考 [加入群组](./Manage%20groups.mdx#加入群组)。
</Warning>
...
// 注册监听“群成员信息变更”的回调
zim.on('groupMemberInfoUpdated', (zim: ZIM, data: ZIMGroupMemberInfoUpdatedEventResult) => {
...
<Frame width="auto" height="auto">
  <img src="https://doc-media.zego.im/docuo/7155b076f7.jpg"/>
</Frame>
...
如果您希望禁止特定群成员角色发言，请参考 [群组管理 - 禁言或解禁群组](./Manage%20groups.mdx#禁言或解禁群组)。
</Note>
:::
...
<div>
- 参考 [会话管理 - 拉取会话列表](../Conversation/Get%20the%20conversation%20list.mdx#拉取会话列表) 主动拉取会话列表。
- 参考 [会话管理 - 更新会话列表](../Conversation/Get%20the%20conversation%20list.mdx#更新会话列表)，监听会话变更回调，更新会话列表。
</div>
...
<Note title="说明">
如果当前用户既因群角色被禁言（详见 [群组管理 - 禁言或解禁群组](./Manage%20groups.mdx#禁言或解禁群组)，又被单独禁言（详见本文 <a href="#设置群成员禁言状态">设置群成员禁言状态</a>），则 {getPlatformData(props,mutedExpiredTimeMap)} 值以“群禁言时间”与“单独被禁言时间”中的最大值为准。
</Note>
:::