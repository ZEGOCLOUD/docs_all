...
- 用户使用“群资料管理”功能之前，请先加入某个群组，否则无法使用相关功能，详情请参考 [加入群组](./Manage%20groups.mdx#加入群组)。
- 群内成员（包含群主）都可以使用“群资料管理”功能。
</Warning>

<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/ZIM/Common/GruopDocumentation.png" /></Frame>
...
zim_->queryGroupInfo(group_id, 
    [=](const ZIMGroupFullInfo &groupInfo, zim::ZIMError errorInfo) {
...
// 注册监听“群名更新”的回调
zim.onGroupNameUpdated(function (zim, { groupID, groupName, operatedInfo }) {
    console.log('groupNameUpdated', groupID, groupName, operatedInfo);
});
```
</CodeGroup>
...
// 注册监听“群头像 URL 更新”的回调
zim.onGroupAvatarUrlUpdated(function (zim, { groupID, groupAvatarUrl, operatedInfo }) {
    console.log('groupAvatarUrlUpdated', groupID, groupAvatarUrl, operatedInfo);
});
```
</CodeGroup>
...
// 注册监听“群公告更新”的回调
zim.onGroupNoticeUpdated(function (zim, { groupID, groupNotice, operatedInfo }) {
    console.log('groupNoticeUpdated', groupID, groupNotice, operatedInfo);
});
```
</CodeGroup>
...
```javascript
// 监听群备注修改通知
zim.onGroupAliasUpdated(function (zim, { groupID, groupAlias, operatedUserID }) {
    // 在这里可以处理群别名更新后的逻辑，例如更新 UI 或提示用户
});
```
:::
...
此外，该用户还会收到 {getPlatformData(props,onConversationChangedMap)} 回调，得知 `conversationAlias`（会话别名）更新，详情请参考 [获取会话列表 - 监听会话变更](./../Conversation/Get%20the%20conversation%20list.mdx)。
