...
// 注册监听“群名更新”的回调
zim.onGroupNameUpdated(function ({ groupID, groupName, operatedInfo }) {
    console.log('groupNameUpdated', groupID, groupName, operatedInfo);
});
```
</CodeGroup>
...
// 注册监听“群头像 URL 更新”的回调
zim.onGroupAvatarUrlUpdated(function ({ groupID, groupAvatarUrl, operatedInfo }) {
    console.log('groupAvatarUrlUpdated', groupID, groupAvatarUrl, operatedInfo);
});
```
</CodeGroup>
...
// 注册监听“群公告更新”的回调
zim.onGroupNoticeUpdated(function ({ groupID, groupNotice, operatedInfo }) {
    console.log('groupNoticeUpdated', groupID, groupNotice, operatedInfo);
});
```
</CodeGroup>
...
```typescript
// 监听群备注修改通知
zim.onGroupAliasUpdated(function ({ groupID, groupAlias, operatedUserID }) {
    // 在这里可以处理群别名更新后的逻辑，例如更新 UI 或提示用户
});
```
:::
