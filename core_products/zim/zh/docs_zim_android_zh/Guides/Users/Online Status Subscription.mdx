...
export const onUserStatusUpdatedMap = {
  ...
  "RN,uniapp,harmonyos": <a href="@userStatusUpdated">userStatusUpdated</a>,
  "UTS": <a href="@userStatusUpdated">onUserStatusUpdated</a>,
...
export const ZIMEventHandlerMap = {
  ...
  "RN,uniapp,UTS,harmonyos": <a href="@on">on</a>, 
}
...
<Frame width="auto" height="auto" >
  <img src="https://doc-media.zego.im/docuo/a8fa19c930.png" alt="1_用户状态_中文.png"/>
</Frame>
...
- `duration` 单位为 秒，传入 -1 时为永久保持该状态，直到下次修改。传入 0 时视为 24 小时。
...
config.customStatusDuration = 24 * 60 * 60; // 默认为 24 小时，若传 -1 则为永久，仅在 customStatus 为非空字符串时有效
```

```java title="更新自定义状态" {1}
ZIMUserCustomStatusUpdateConfig config = new ZIMUserCustomStatusUpdateConfig();
config.setDuration(24 * 60 * 60); // 默认为 24 小时，若传 -1 则为永久，仅在 customStatus 为非空字符串时有效

ZIM.getInstance().updateUserCustomStatus("请勿打扰", config, new ZIMUserCustomStatusUpdatedCallback() {
...
config.duration = 24 * 60 * 60; // 默认为 24 小时，若传 -1 则为永久，仅在 customStatus 为非空字符串时有效
...
config.duration = 24 * 60 * 60; // 默认为 24 小时，若传 -1 则为永久，仅在 customStatus 为非空字符串时有效
zim_->updateUserCustomStatus("请勿打扰", config, [=](std::string &customStatus, const ZIMError &errorInfo) {
...
const config: ZIMLoginConfig = {
    ...
    customStatusDuration = 24 * 60 * 60; // 默认为 24 小时，若传 -1 则为永久，仅在 customStatus 为非空字符串时有效
};
```
```typescript title="更新自定义状态" {1}
const config: ZIMUserCustomStatusUpdateConfig = {
    duration = 24 * 60 * 60; // 默认为 24 小时，若传 -1 则为永久，仅在 customStatus 为非空字符串时有效
};
zim.updateUserCustomStatus("请勿打扰", config)
...
var config = ZIMUserCustomStatusUpdateConfig(duration: 24 * 60 * 60); // 默认为 24 小时，若传 -1 则为永久，仅在 customStatus 为非空字符串时有效
ZIM.getInstance()?.updateUserCustomStatus("请勿打扰", config).then((ZIMUserCustomStatusUpdatedResult result) {
...
```typescript
// 监听用户状态更新事件
zim.on('userStatusUpdated', (zim: ZIM, data: ZIMEventOfUserStatusUpdatedResult) => {
