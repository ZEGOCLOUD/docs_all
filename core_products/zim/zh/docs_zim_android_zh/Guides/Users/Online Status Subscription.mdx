...
export const onUserStatusUpdatedMap = {
  ...
  "RN,uniapp": <a href="@userStatusUpdated">userStatusUpdated</a>,
...
export const onConnectionStateChangedMap = {
  ...
  "RN,uniapp": <a href='@connectionStateChanged' target='_blank'>connectionStateChanged</a>,
...
:::if{props.platform="undefined|iOS|mac|window|RN|Flutter"}
...
conifg.customStatus = "忙碌" //默认为空字符串，如果为空字符串，登录时不修改当前用户的自定义状态
```
...
```objc title="更新自定义状态" {1}
[[ZIM getInstance] updateUserCustomStatus:@"请勿打扰" callback:^(NSString * _Nonnull customStatus, ZIMError * _Nonnull errorInfo) {
...
```cpp title="更新自定义状态" {1}
zim->updateUserCustomStatus("请勿打扰",[](const std::string& customStatus, const ZIMError &errorInfo){
...
:::if{props.platform="undefined|iOS|mac|window|RN|Flutter"}
如需持续关注某些人的用户状态时，可以调用 {getPlatformData(props,subscribeUsersStatusMap)} 接口，向参数 `userIDs` 批量传入目标用户 ID（至多 100 名已注册用户，不得包含订阅者自身），在 config 中传入 `subscriptionDuration`（订阅有效时长，在此时段内会持续关注目标用户的用户状态变更），进而添加目标用户用户状态到当前用户的用户状态订阅表中。
:::
:::if{props.platform="uniapp"}
如需持续关注某些人的在线状态时，可以调用 {getPlatformData(props,subscribeUsersStatusMap)} 接口，向参数 `userIDs` 批量传入目标用户 ID（至多 100 名已注册用户，不得包含订阅者自身），在 config 中传入 `subscriptionDuration`（订阅有效时长，在此时段内会持续关注目标用户的在线状态变更），进而添加目标用户在线状态到当前用户的用户状态订阅表中。
:::

<Note title="说明">

- 单个用户最多可以订阅 3000 人，当订阅用户超过 3000 人时，将会覆盖订阅时间最早的用户。
- 订阅其他用户的用户状态后，即便用户在 `subscriptionDuration` 时限内登出并重新登录 ZIM SDK，订阅仍然有效，无需再次调用订阅方法。
</Note>
...
随后，在 `subscriptionDuration` 规定的时间内，目标用户的用户状态变更将会通过 {getPlatformData(props,ZIMEventHandlerMap)} 中的 {getPlatformData(props,onUserStatusUpdatedMap)} 事件回调返回。
...
zim.setEventHandler(new ZIMEventHandler(){
    public void onUserStatusUpdated(ZIM zim, ArrayList<ZIMUserStatus> userStatusList) {
        for (ZIMUserStatus userStatus : userStatusList) {
            ...
            String customStatus = userStatus.customStatus; // 用户自定义状态
            ArrayList<String> onlinePlatforms = userStatus.onlinePlatforms; // 用户在线平台列表
            long lastUpdateTime = userStatus.lastUpdateTime; // 上次状态变更时间
            long lastOnlineStatusUpdateTime = userStatus.lastOnlineStatusUpdateTime; //上次在线状态变更时间
            long lastCustomStatusUpdateTime = userStatus.lastCustomStatusUpdateTime; //上次自定义状态变更时间
        }
    }
});
```
...
- (void)zim:(ZIM *)zim userStatusUpdated:(NSArray<ZIMUserStatus *> *)userStatusList {
    for(ZIMUserStatus *userStatus in userStatusList){
        ...
        userStatus.customStatus; // 用户自定义状态
        userStatus.onlinePlatforms; // 用户在线平台列表
        userStatus.lastUpdateTime; // 用户状态的最后更新时间
        userStatus.lastOnlineStatusUpdateTime; // 用户在线状态的最后变更时间
        userStatus.lastCustomStatusUpdateTime; // 用户自定义状态的最后变更时间
    }
}
...
void onUserStatusUpdated(ZIM* zim, const std::vector<ZIMUserStatus>& userStatusList) {
    for (const ZIMUserStatus& userStatus : userStatusList) {
        ...
        userStatus.customStatus;      // 目标用户的自定义状态
        userStatus.onlinePlatforms;   // 用户在线平台列表
        userStatus.lastUpdateTime;    // 用户状态的最后更新时间
        userStatus.lastOnlineStatusUpdateTime; //用户在线状态的最后更新时间
        userStatus.lastCustomStatusUpdateTime; // 用户自定义状态的最后变更时间
    }
}
```
...
zim.on('userStatusUpdated', function (zim, userStatusList) {
    userStatusList.forEach((userStatus) => {
        const onlineStatus = userStatus.onlineStatus; // 目标用户的在线状态
        const customStatus = userStatus.customStatus; // 目标用户的自定义状态
        const onlinePlatforms = userStatus.onlinePlatforms; // 用户在线平台列表
        const lastUpdateTime = userStatus.lastUpdateTime; // 用户状态的最后更新时间
        const lastOnlineStatusUpdateTime = userStatus.lastOnlineStatusUpdateTime; // 用户在线状态的最后更新时间
        const lastCustomStatusUpdateTime = userStatus.lastCustomStatusUpdateTime; // 用户自定义状态的最后变更时间
    });
});
```
:::
...
ZIMEventHandler.onUserStatusUpdated = (ZIM zim, List<ZIMUserStatus> userStatusList){
    for (ZIMUserStatus userStatus in userStatusList) {
        ...
        userStatus.onlineStatus;      // 用户在线状态
        userStatus.customStatus;      // 自定义状态
        userStatus.onlinePlatforms;   // 用户在线平台列表
        userStatus.lastUpdateTime;    // 上次状态变更时间
        userStatus.lastOnlineStatusUpdateTime; // 上次在线状态变更时间
        userStatus.lastCustomStatusUpdateTime; // 上次自定义状态变更时间
    }
};
```
:::
...
若无需关注某些用户的用户状态，可调用 {getPlatformData(props,unsubscribeUsersStatusMap)} 接口，向 `userIDs` 参数传入目标用户 ID（至多 100 名用户）。
...
如仅需获取一次目标用户的用户状态，无需持续关注时，可以调用 {getPlatformData(props,queryUsersStatusMap)} 接口并传入目标用户的 userIDs（至多 200 名用户，不得包括当前用户）。
...
如果用户希望了解自己订阅了哪些用户的用户状态，可以通过 {getPlatformData(props,querySubscribedUserStatusListMap)} 接口，参数 `config.userIDs` 传空，即可获取到完整的订阅列表，包含订阅目标用户当前的状态、以及订阅有效时长。

如果用户希望确认是否订阅了某些用户的用户状态，参数 `config.userIDs` 传入查询目标的 userID 即可。
