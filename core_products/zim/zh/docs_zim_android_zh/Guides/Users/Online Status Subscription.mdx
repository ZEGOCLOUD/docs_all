...
:::if{props.platform="undefined|iOS|mac|window|RN|UTS|Flutter"}
...
```
```js title="更新自定义状态" {1}
zim.updateUserCustomStatus("请勿打扰").then(res => {
...
```javascript
// 监听用户状态更新事件
zim.onUserStatusUpdated(function (zim, { userStatusList }) {
    userStatusList.forEach((userStatus) => {
        const { userID, onlineStatus, onlinePlatforms, lastUpdateTime } = userStatus;
...
:::if{props.platform="UTS"}
```javascript
zim.onConnectionStateChanged(function (zim, { state, event }) {
    switch (state) {
        case 2: // 已连接
            console.log('已连接');
            break;
