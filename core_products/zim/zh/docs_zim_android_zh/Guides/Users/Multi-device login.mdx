...
:::if{props.platform="UTS"}
```typescript
zim.onConversationChanged(({ infoList }) => {
    infoList.forEach((info) => {
        if (info.event == 3) {
            //对话已被删除
        }
...
:::if{props.platform="UTS"}
```typescript
zim.onConversationsAllDeleted(({ count }) => {
    console.log('已删除的对话数量', count);
});
```
:::
...
:::if{props.platform="UTS"}
```typescript
zim.onConversationTotalUnreadMessageCountUpdated(({ totalUnreadMessageCount }) => {
    //在其他设备上清除所有对话中的未读消息后，当前设备上的`totalUnreadMessageCount`值将为0。
});
```
:::
...
:::if{props.platform="UTS"}
```typescript
zim.onMessageDeleted(({ conversationID, conversationType, isDeleteConversationAllMessage, messageList }) => {
    if (messageDeleteType == 2 ) {
        // 会话中的所有消息都被删除
     } else if (messageDeleteType == 1 ) {
        // 会话中的所有当前消息都被删除
...
:::if{props.platform="UTS"}
```typescript
zim.onMessageReceiptChanged(({ infos }) => {
    infos.forEach((info) => {
        if (info.isSelfOperated) {
            // 用户将消息回执设置为已读
        }
...
zim.onConversationMessageReceiptChanged(({ infos }) => {
    infos.forEach((info) => {
        if (info.isSelfOperated) {
            // 用户将会话回执设置为已读
        }
...
```typescript
// When a user joins a room on device A, and then joins the same room on device B, device A will be kicked out of the room
zim.onRoomStateChanged(({ roomID, state, event }) => {
    if (state == 0 && event == 10) {
       // 由于多设备登录被踢出房间
    }
});
...
// 设备B监听callUserStateChanged事件
zim.onCallUserStateChanged(({ callID, callUserList }) => {
    if (curCallID == callID) {
        callUserList.forEach((item) => {
            if (item.userID == selfUserID && (item.state == 1 || item.state == 2)) {
                // 当设备A接受或拒绝时，关闭呼叫等待操作的弹出框。
