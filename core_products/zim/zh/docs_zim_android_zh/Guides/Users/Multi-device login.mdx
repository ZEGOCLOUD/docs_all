...
多端登录可支持在 Android、iOS、HarmonyOS、Windows、Mac、Linux、iPad、Web、小程序等 9 个平台上配置，其中支持在 Windows、Mac、Linux、Android、HarmonyOS、iOS、iPad 平台配置多设备登录，对于上述任一平台而言，一个账号在同一时间只能在最多两台设备上进行登录。在 Web 平台配置多实例登录，最多不超过 5 个登录实例。
