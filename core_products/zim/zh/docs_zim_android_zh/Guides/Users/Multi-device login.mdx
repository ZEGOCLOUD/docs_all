...
前往 ZEGO 控制台开通多端登录服务，详情请参考控制台文档 [服务配置 - 即时通讯 - 多端登录配置](https://doc-zh.zego.im/article/21499)。开通后，即可在多端调用 {getPlatformData(props,loginMap)} 登录 ZIM SDK，实现流程与单端登录无异。如何登录，详情请参考 [实现基本消息收发](../../Send%20and%20receive%20messages.mdx) 的“登录 ZIM“。 
...
```javascript
zim.onUserInfoUpdated((zim, { info }) => {
    console.log(info.baseInfo.userName, info.userAvatarUrl, info.extendedData);
});
```
:::
...
:::if{props.platform="UTS"}
```javascript
zim.onConversationChanged((zim, { infoList }) => {
    infoList.forEach((info) => {
        if (info.event == 3) {
            //对话已被删除
        }
...
zim.on('conversationsAllDeleted', (zim, { count }) => {
    console.log('已删除的对话数量', count);
});
```
:::
:::if{props.platform="UTS"}
```javascript
zim.onConversationsAllDeleted((zim, { count }) => {
    console.log('已删除的对话数量', count);
});
```
:::
...
zim.on('conversationTotalUnreadMessageCountUpdated', (zim, { totalUnreadMessageCount }) => {
    //在其他设备上清除所有对话中的未读消息后，当前设备上的`totalUnreadMessageCount`值将为0。
});
```
:::
:::if{props.platform="UTS"}
```javascript
zim.onConversationTotalUnreadMessageCountUpdated((zim, { totalUnreadMessageCount }) => {
    //在其他设备上清除所有对话中的未读消息后，当前设备上的`totalUnreadMessageCount`值将为0。
});
```
:::
...
当用户登录新设备后，SDK 不会自动将旧设备中的已有消息同步到新设备上，用户需要主动调用 {getPlatformData(props,queryHistoryMessageMap)} 接口，才能获取存储于 ZIM 服务端的消息，详情请参考 [获取历史消息](../Messaging/Get%20message%20history.mdx)。对于存储于旧设备的本地消息，则无法获取。
...
:::if{props.platform="UTS"}
```javascript
zim.onMessageDeleted((zim, { conversationID, conversationType, isDeleteConversationAllMessage, messageList }) => {
    if (messageDeleteType == 2 ) {
        // 会话中的所有消息都被删除
     } else if (messageDeleteType == 1 ) {
        // 会话中的所有当前消息都被删除
...
:::if{props.platform="UTS"}
```javascript
zim.onMessageReceiptChanged((zim, { infos }) => {
    infos.forEach((info) => {
        if (info.isSelfOperated) {
            // 用户将消息回执设置为已读
        }
...
zim.onConversationMessageReceiptChanged((zim, { infos }) => {
    infos.forEach((info) => {
        if (info.isSelfOperated) {
            // 用户将会话回执设置为已读
        }
...
房间模块相关接口和事件不支持多端登录。用户在 A 设备加入房间后，然后在 B 设备再加入相同的房间后，会把 A 设备踢出房间，A 设备会通过 {getPlatformData(props,onRoomStateChangedMap)} 回调收到 `event` 为 `KickedOutByMultiDevice`。
...
```javascript
// When a user joins a room on device A, and then joins the same room on device B, device A will be kicked out of the room
zim.onRoomStateChanged((zim, { roomID, state, event }) => {
    if (state == 0 && event == 10) {
       // 由于多设备登录被踢出房间
    }
});
...
// 设备A接受邀请
zim.callAccept(curCallID, { extendedData: '设备A接受邀请' }).then((res) => {
    // 关闭呼叫等待操作的弹出框。开始流发布和播放，进行音视频通话。
});
// 设备A拒绝邀请
zim.callReject(curCallID, { extendedData: '设备A拒绝邀请' }).then((res) => {
    // 关闭呼叫等待操作的弹出框。通话结束。
});
...
// 设备A接受邀请
zim.callAccept(curCallID, { extendedData: '设备A接受邀请' } as ZIMCallAcceptConfig).then((res) => {
    // 关闭呼叫等待操作的弹出框。开始流发布和播放，进行音视频通话。
});
// 设备A拒绝邀请
zim.callReject(curCallID, { extendedData: '设备A拒绝邀请' } as ZIMCallRejectConfig).then((res) => {
    // 关闭呼叫等待操作的弹出框。通话结束。
});

// 设备B监听callUserStateChanged事件
zim.onCallUserStateChanged((zim, { callID, callUserList }) => {
    if (curCallID == callID) {
        callUserList.forEach((item) => {
            if (item.userID == selfUserID && (item.state == 1 || item.state == 2)) {
                // 当设备A接受或拒绝时，关闭呼叫等待操作的弹出框。
