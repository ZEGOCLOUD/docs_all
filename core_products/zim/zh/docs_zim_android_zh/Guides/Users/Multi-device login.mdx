...
前往 ZEGO 控制台开通多端登录服务，详情请参考控制台文档 [服务配置 - 即时通讯 - 多端登录配置](https://doc-zh.zego.im/console/service-configuration/im/multi-device-login-config)。开通后，即可在多端调用 {getPlatformData(props,loginMap)} 登录 ZIM SDK，实现流程与单端登录无异。如何登录，详情请参考 [实现基本消息收发](../../Send%20and%20receive%20messages.mdx) 的“登录 ZIM“。 
...
```typescript
zim.on('userInfoUpdated', (zim: ZIM, data: ZIMUserInfoUpdatedEventResult) => {
});
```
:::
...
:::if{props.platform="Web"}
```typescript
zim.on('conversationsAllDeleted', (zim: ZIM, data: ZIMConversationsAllDeletedEventResult) => {
...
:::if{props.platform="Web"}
```typescript
zim.on('conversationTotalUnreadMessageCountUpdated', (zim: ZIM, data: ZIMConversationTotalUnreadMessageCountUpdatedEventResult) => {
...
如需支持同一用户在多个设备上同时登录房间，可在控制台自行开启该功能，详见控制台文档：[多端登录配置](https://doc-zh.zego.im/console/service-configuration/im/multi-device-login-config)。开启后，多个设备以相同 UserID 加入同一房间时将不会互相踢出。此时，你需维护房间成员列表，对 UserID 进行去重。  
