...
```javascript
// 好友列表变更回调
zim.onFriendListChanged((zim, data) => {
    // 待办事项
});

// 好友信息更新回调
zim.onFriendInfoUpdated((zim, data) => {
    // 待办事项
});

// 好友申请列表变更回调
zim.onFriendApplicationListChanged((zim, data) => {
    // 待办事项
});

// 好友申请更新回调
zim.onFriendApplicationUpdated((zim, data) => {
    // 待办事项
});
```
:::
...
```javascript
// 直接添加好友
const config = { wording: '你好！', friendAlias: '马克', friendAttributes: { k0: 'SZ' } };
...
添加好友成功后，相关用户会收到回调 {getPlatformData(props,onFriendListChangedMap)} 得知该用户已成为好友。
...
```javascript
// 接受好友申请
const config = { friendAlias: 'Mark', friendAttributes: { k0: 'SZ' } };
...
:::if{props.platform="Web"}
```javascript
const config = { keywords: ['a', 'b'], isAlsoMatchFriendAlias: true, count: 100, nextFlag: 0 };
