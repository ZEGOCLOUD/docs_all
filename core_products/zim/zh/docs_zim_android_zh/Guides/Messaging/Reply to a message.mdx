...
zim.on('messageRepliedCountChanged', (zim: ZIM, data: ZIMMessageRepliedCountChangedEventResult) => {
...
// 监听回复消息的源消息变更事件
// 事件触发时机：当源消息被 “删除” 和 “撤回” 后，引用其的回复消息的 repliedInfo 属性会发生改变
zim.on('messageRepliedInfoChanged', (zim: ZIM, data: ZIMMessageRepliedInfoChangedEventResult) => {
