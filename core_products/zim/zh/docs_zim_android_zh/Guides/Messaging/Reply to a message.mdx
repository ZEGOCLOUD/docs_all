...
export const messageReceivedMap = {
  ...
  'iOS': <a href='https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-message-received' target='_blank'>messageReceived</a>,
  'mac': <a href='https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_macos~protocol~ZIMEventHandler#zim-message-received' target='_blank'>messageReceived</a>,
...
<Frame width="auto" height="auto">
  <img src="https://doc-media.zego.im/docuo/2fe84e2d81.jpg"/>
</Frame>
...
登录 ZIM 后，用户可以监听 {getPlatformData(props,messageReceivedMap)} 回调接收单聊会话和群聊会话的新消息，或调用 {getPlatformData(props,queryHistoryMessageMap)} 接口拉取历史消息。
...
// 回复一条消息，引用的源消息对象 toOriginalMessage 可以通过 queryHistoryMessage 或者 onMessageReceived 获取。
...
// 引用的源消息对象 toOriginalMessage 可以通过 queryHistoryMessage 或者 onMessageReceived 获取。
...
监听 {getPlatformData(props,messageReceivedMap)} 回调在单聊会话和群聊会话中收到新消息后，需要通过判断消息是否存在 {getPlatformData(props,repliedInfoMap)} （源消息基本信息）：
...
// 接收消息的回调
void MyClass::onMessageReceived(ZIM * zim, const ZIMMessageReceivedEventResult &result) {
...
zim.on('messageRepliedCountChanged', (zim: ZIM, data: ZIMEventOfMessageRepliedCountChangedResult) => {
...
// 监听回复消息的源消息变更事件
// 事件触发时机：当源消息被 “删除” 和 “撤回” 后，引用其的回复消息的 repliedInfo 属性会发生改变
zim.on('messageRepliedInfoChanged', (zim: ZIM, data: ZIMEventOfMessageRepliedInfoChangedResult) => {
...
// replyMessage 可为根消息或任意回复，一般通过 queryHistoryMessage 接口或者 onMessageReceived 获取
...
const originalMessage: ZIMMessage = {};
const config: ZIMMessageQueryConfig = {
    nextMessage: originalMessage.localMessageID,
    count: 20,
    reverse: false,
};
