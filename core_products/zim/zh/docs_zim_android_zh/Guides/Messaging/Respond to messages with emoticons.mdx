...
- 2.28.0 以下版本时，重复调用本接口，对同一条消息进行相同类型的表态会报错。
- 2.28.0 及以上版本时，对同一条消息进行相同类型的表态，通过 config 参数可实现重复表态和计数功能。
...
也可以调用 {getPlatformData(props,deleteMessageAllReactionsMap)} 删除所有表态。

<Warning title="注意">

调用本接口只能删除自己所作的表态。如果有实现表态计数，删除后，该表态自己的计数也会清零。
</Warning>
