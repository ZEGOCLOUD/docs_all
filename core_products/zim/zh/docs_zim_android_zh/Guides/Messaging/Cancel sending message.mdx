...
<Note title="注意">
- 2.22.0 版本开始支持。
- 仅支持取消发送的[消息类型](/zim-server/messaging/messagebody-introduction)为包含本地媒体文件的 `Multiple(10)、Image(11)、File(12)、Audio(13)、Video(14)` 类型。
</Note>


## 取消正在发送的媒体消息

当用户发送包含本地媒体文件的消息时，由于文件上传需要一定的耗时，在这段时间内调用 {getPlatformData(props,cancelSendingMessageMap)} 取消发送此条消息终止文件上传。

<Warning title="注意">
当包含普通文本和媒体文件的[组合消息](/zim-server/messaging/messagebody-introduction#组合消息)被取消时，整个消息（包含普通文本）都会被取消发送。
</Warning>
