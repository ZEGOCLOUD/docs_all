...
删除成功后，操作者与会话内其他成员会分别收到删除结果通知：

- 操作者通过 {getPlatformData(props,ZIMMessageDeletedCallbackMap)} 回调得知删除结果。
- 会话内的其他成员通过 {getPlatformData(props,messageDeletedMap)} 回调收到通知，回调参数 {getPlatformData(props,ZIMMessageDeletedInfoMap)} 中，`messageDeleteType` 为“删除指定消息”类型，`messageList` 为被删除的消息列表。

以下示例代码演示了如何为所有人删除消息，以及操作者和其他成员如何接收删除结果通知：
...
zim.on('messageDeleted', (zim: ZIM, data: ZIMMessageDeletedEventResult) => {
    if (data.messageDeleteType == 0) {
        // data.messageList 为被删除的消息列表，可于此处刷新 UI
    }
});
```
</CodeGroup>
