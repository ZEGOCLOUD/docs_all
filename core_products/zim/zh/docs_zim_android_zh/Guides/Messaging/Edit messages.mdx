...
**成功登录 ZIM SDK 后**，会话参与者可以调用 {getPlatformData(props,editMessageMap)} 接口编辑自身已发送的消息，支持编辑的属性如下：
- extendedData：消息拓展字段。
...
<Note title="说明">
- 仅支持编辑 24 小时内的消息。
- 仅支持以下类型的消息：{getPlatformData(props,ZIMTextMessageMap)}、{getPlatformData(props,ZIMCustomMessageMap)}、{getPlatformData(props,ZIMMultipleMessageMap)}。
- 不能修改消息类型，比如不能把文本消息改为自定义消息等。不支持修改消息类型，如 {getPlatformData(props,ZIMTextMessageMap)} 改为 {getPlatformData(props,ZIMCustomMessageMap)}。
