...
:::
- 接收消息时：
  + 单聊消息（Peer 类型），通过 {getPlatformData(props, peerMessageReceivedMap)} 回调接收。
  + 房间消息（Room 类型），通过 {getPlatformData(props, roomMessageReceivedMap)} 回调接收。
  + 群组消息（Group 类型），通过 {getPlatformData(props, groupMessageReceivedMap)} 回调接收。
