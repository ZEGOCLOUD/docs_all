...
import MarkMessageTypeOld from '/core_products/zim/zh/snippets/MessageTypeOld.mdx'
...
{props.platform === 'harmonyos' || props.platform === 'uniapp' || props.platform === 'UTS' ? <MarkMessageTypeOld /> : <MarkMessageType />}
...
1. 客户端 A、B 分别创建自己的 ZIM 实例，并注册 {getPlatformData(props,setEventHandlerMap)} 监听的 {getPlatformData(props,messageReceivedMap)} 回调接口，用于接收单聊消息通知。
:::
:::if{props.platform="UTS"}
1. 客户端 A、B 分别创建自己的 ZIM 实例，并监听 {getPlatformData(props,messageReceivedMap)} 回调接口，用于接收单聊消息通知。
:::
...
4. 客户端 B 将通过 {getPlatformData(props,messageReceivedMap)} 回调接口，收到客户端 A 的消息。
</div>

<Warning title="注意">
发送消息时，统一使用 {getPlatformData(props, sendMessageMap)} 接口，并根据会话类型传入对应的 conversationType 取值。
...
<Note title="说明">
1. 本地图片（通过 `fileLocalPath` 发送）
图片将上传至 ZEGO 服务器，并自动生成 `fileDownloadURL`（发送方需在消息发送成功的回调中，通过 ZIMMessage 对象获取该 URL）。发送方和接收方均能通过 `fileDownloadURL` 下载原图（即使发送方本地图片已删除，仍可通过该 URL 重新获取）。
2. 网络图片（通过 `fileDownloadURL` 直接发送）
不会经过 ZEGO 服务器中转，SDK 仅透传该 URL，不会生成新的服务端下载地址。需要确保该 URL 是公网可访问的有效地址，接收方需自行处理图片加载（如缓存、防失效等）。
...
if (kIsWeb) {
  ...
  // 其他平台：本地文件绝对路径  
  fileLocalPath = '/xxx/xxx.mov';
}

ZIMVideoMessage videoMessage = ZIMVideoMessage(fileLocalPath);
ZIMMessageSendConfig sendConfig = ZIMMessageSendConfig();
ZIMMessageSendNotification notification = ZIMMessageSendNotification(onMessageAttached: (message){
...
if (kIsWeb) {
  ...
  // 其他平台：本地文件绝对路径  
  fileLocalPath = '/xxx/xxx.mp3';
}

ZIMAudioMessage audioMessage  = ZIMAudioMessage(fileLocalPath);
ZIMMessageSendConfig sendConfig = ZIMMessageSendConfig();
ZIMMessageSendNotification notification = ZIMMessageSendNotification(onMessageAttached: (message){
...
if (kIsWeb) {
  ...
  // 其他平台：本地文件绝对路径  
  fileLocalPath = '/xxx/xxx.txt';
}

ZIMFileMessage fileMessage = ZIMFileMessage(fileLocalPath);
ZIMMessageSendConfig sendConfig = ZIMMessageSendConfig();
ZIMMessageSendNotification notification = ZIMMessageSendNotification(onMessageAttached: (message){
...
- 通过 {getPlatformData(props,onMediaUploadingProgressMap)} 回调，接收富媒体消息的上传文件进度的相关通知。
...
```
:::
:::if{props.platform="UTS|harmonyos"}
<CodeGroup>
...
// uni-app x UTS 语言
// const uint8Array = Array.from(new TextEncoder().encode(jsonText));

// uni-app js 语言
// const uint8Array = Array.from(unescape(encodeURIComponent(jsonText))).map((val) => val.charCodeAt(0));
...
ZIM SDK 支持开发者实现自定义类型的消息收发，开发者可以通过 {getPlatformData(props,ZIMCustomMessageMap)} 对象创建自定义类型消息，例如投票类型、接龙类型、视频卡片类型等。
...
ZIM SDK 支持开发者实现组合类型的消息收发，开发者可以通过 {getPlatformData(props,ZIMMultipleMessageMap)} 对象组合多种消息类型（文本、图片、音频、视频、文件和自定义消息），实现图文消息等场景。
...
<Frame width="60%" height="auto" >
  <img src="https://doc-media.zego.im/docuo/22bdff3b4b.png" alt="ZIMMultipleMessage_zh.png"/>
</Frame>

#### 上传文件进度回调

通过 {getPlatformData(props,multipleMediaUploadingProgressMap)} 回调，接收组合消息中富媒体文件的上传进度的相关通知。在此回调中，您可以了解到以下字段：
...
通过将发送设置中的 isAllowMultipleMessagePartialFailed 字段设置为 true，可以使得发送组合消息时允许部分子项发送失败时不影响组合消息自身发送成功。
<Note title="说明">
- 仅 2.27.0 及以上版本的 ZIM SDK 支持发送组合类型消息时允许部分子项发送失败的特性。
- 在发送组合消息存在部分失败时，仅发送端可以看到包含失败子项在内的所有子项以及发送失败的子项 index 列表 failedIndexList，发送者的多端以及接受者仅能看见所有发送成功的子项。
</Note>
...
const config: ZIMMessageSendConfig = { 
    priority: 1, // 设置消息优先级，取值为 低：1（默认），中：2，高：3
    isAllowMultipleMessagePartialFailed: true, // 设置发送组合消息时允许部分子项发送失败
};
...
<Frame width="auto" height="auto">
  <img src="https://doc-media.zego.im/docuo/e130538572.jpg"/>
</Frame>
...
在调用 {getPlatformData(props,sendMessageMap)} 发送消息时，可以通过以下属性或方法（可同时使用）将一条消息设置为 @ 信息：
...
:::if{props.platform="undefined|iOS|mac|win|flutter|Web|miniprogram|rn|uniapp"}
<div>
- 开发者可以通过 {getPlatformData(props,setEventHandlerMap)} 注册监听，用于接收消息相关通知。
</div>
:::
:::if{props.platform="UTS"}
<div>
...
:::
- 接收消息时，通过 {getPlatformData(props, messageReceivedMap)} 回调接收所有类型的消息（单聊、群组、房间），并通过 conversationType 区分消息来源。
- 接收消息时，收到的消息类型是基类 {getPlatformData(props,ZIMMessageMap)} 。开发者需要根据其中的 `type`（具体请参考 {getPlatformData(props,ZIMMessageTypeMap)} ）字段，判断消息类型，然后强转基类为具体的子类。
...
zim.onMessageReceived((zim, result) => {
    ...
    messageList.forEach((msg) => {
        if (msg.type == 2) {
            // 信令消息，这里以 JSON 字符串 为例，将 Uint8Array 消息内容转为 JSON 字符串
            const byteArr = msg.message;

            // uni-app x UTS 语言
            // const jsonText = new TextDecoder().decode(new Uint8Array(byteArr));
            // const jsonObj = JSON.parse(jsonText);

            // uni-app js 语言
            // const jsonText = decodeURIComponent(escape(String.fromCharCode(...byteArr)));
            // const jsonObj = JSON.parse(jsonText);
...
- 通过 {getPlatformData2(props,onMediaDownloadingProgressMap)} 回调，接收富媒体消息的下载文件进度的相关通知。
...
```
:::
:::if{props.platform="UTS|rn|uniapp|harmonyos"}
```typescript
...
:::if{props.platform="undefined|iOS|mac|win|flutter|rn|uniapp|UTS|harmonyos"}
### 组合消息

如果需要下载组合消息的 Item 中的富媒体文件，可遍历 `messageInfoList` 列表并判断为富媒体文件类型时，调用 `downloadMediaFile` 接口下载。开发者可参考 [接收消息 - 富媒体消息](#富媒体消息-2) 了解此接口参数详情。
:::
...
```typescript
// 用户接收全员推送消息
zim.on('broadcastMessageReceived', (zim: ZIM, data: ZIMBroadcastMessageReceivedEventResult) => {
...
在调用 {getPlatformData(props,sendMessageMap)} 发送消息时，可以通过以下参数设置发送的消息不会被计入到未读消息数中：
...
```typescript
// 监听消息状态
zim.on('messageSentStatusChanged', (zim: ZIM, data: ZIMMessageSentStatusChangedEventResult) => {
