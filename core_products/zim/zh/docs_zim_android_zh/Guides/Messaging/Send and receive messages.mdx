...
1. 在消息接收回调中，若消息类型为 `ZIMMessageType.COMBINE`，调用 {getPlatformData(props,queryCombineMessageDetailMap)} 接口查询组合消息详情。

2. 在 {getPlatformData(props,ZIMCombineMessageDetailQueriedCallbackMap)} 回调中，遍历子消息列表；若子消息为 `ZIMImageMessage` 类型，通过 {getPlatformData(props,downloadMediaFileMap)} 下载图片。
