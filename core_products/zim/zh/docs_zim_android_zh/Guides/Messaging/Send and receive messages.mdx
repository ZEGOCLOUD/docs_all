...
// 收到群组消息的回调
zim.onGroupMessageReceived(function ({ messageList, info, fromConversationID }) {
    console.log(messageList, info, fromConversationID);
});

// 收到房间消息的回调
zim.onRoomMessageReceived(function ({ messageList, info, fromConversationID }) {
    console.log(messageList, info, fromConversationID);
});
```
:::
...
:::if{props.platform="UTS"}
```typescript
zim.onConversationChanged(function ({ info }) {
    console.log(info.mentionInfoList);
});
```
:::
...
```typescript
// 用户接收全员推送消息
zim.onBroadcastMessageReceived(function ({ message }) {
    console.log(message);
});
```
:::
...
```typescript
// 监听消息状态
zim.onMessageSentStatusChanged(function ({ infos }) {
    infos.forEach(function (info) {
        console.warn(info.message, info.status);
    });  
});
