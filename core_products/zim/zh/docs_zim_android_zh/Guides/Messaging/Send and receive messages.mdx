...
在调用 {getPlatformData(props,sendMessageMap)} 发送消息时，可以通过以下参数设置发送不计未读数的消息：
- {getPlatformData2(props,disableUnreadMessageCountMap)}：标识该条消息是否为不计入未读计数的消息。

<Warning title="注意">

仅 2.23.0 及以上版本的 ZIM SDK 支持发送不计未读数的消息。
</Warning>

:::if{props.platform=undefined}
```java {7,8}
// 以下为用户在单聊会话中发送不计未读消息的示例代码：
...
const config: ZIMMessageSendConfig = { 
    priority: 1, // 设置消息优先级，取值为 低：1（默认），中:2，高：3
    disableUnreadMessageCount: true, // 设置消息为不计入未读计数消息
};
...
接收不计未读数的消息的回调接口与接收普通消息的回调接口一致，请参考 [收发普通消息 - 接收消息](#接收消息) 了解具体接口。
