...
在调用 {getPlatformData(props,sendMessageMap)} 发送消息时，可以通过以下设置发送不计未读的消息：
- {getPlatformData2(props,disableUnreadMessageCountMap)}：标识该条消息是否为不计入未读计数的消息。

<Note title="说明">

仅 2.23.0 及以上版本的 ZIM SDK 支持发送不计未读消息。
</Note>
...
接收不计未读消息的回调接口与接收普通消息的回调接口一致，请参考 [收发普通消息 - 接收消息](#接收消息) 了解具体接口。
