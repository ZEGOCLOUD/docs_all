...
```
:::
:::if{props.platform="undefined|iOS|mac|win|rn|flutter|Web|miniprogram"}
## 重发消息

<Note title="说明">如需使用本功能，请集成 2.20.0 及以上版本的 ZIM SDK。</Note>

对于因网络问题导致消息发送失败的情况，ZIM 提高自动重发和手动重发两种解决方案。
...
当自动重发失败时，用户可以手动重发该消息。此时，再次调用 {getPlatformData(props,sendMessageMap)} 接口，将参数 `config.isRetrySend` 设置为 true。

重发成功后，该消息会排序到当前最后位置。例如：当前消息排序为：A（成功）、B（失败）、C（成功），当重发消息 B 成功后，排序为 A、C、B。
:::
:::if{props.platform=undefined}
<CodeGroup>
```java title="示例代码" {10}
...
:::
:::if{props.platform="Web|miniprogram|rn|miniprogram"}
<CodeGroup>
```javascript title="示例代码" {7}
// 重发单聊 `Text` 信息
...
var messageTextObj; // 从 queryHistoryMessage 接口获取发送失败的消息
...
:::if{props.platform="undefined|iOS|mac|win|Web|flutter|miniprogram|rn"}
## 收发组合消息
