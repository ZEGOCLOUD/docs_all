...
export const onMessageReceivedMap = {
  ...
  'UTS': <a href='@onPeerMessageReceived' target='_blank'>onPeerMessageReceived</a>,
  'Flutter': <a href='/zim-flutter/client-sdks/api-reference/class#onmessagereceived' target='_blank'>onMessageReceived</a>,
  'iOS': <a href='@ZIMmessageReceivedWithResult' target='_blank'>messageReceivedWithResult</a>,
  'mac': <a href='@ZIMmessageReceivedWithResult' target='_blank'>messageReceivedWithResult</a>,
}
...
export const onConversationMessageReceiptChangedMap = {
  ...
  'UTS': <a href='@onConversationMessageReceiptChanged' target='_blank'>onConversationMessageReceiptChanged</a>,
...
<Frame width="512" height="auto" >
  <img src="https://doc-media.zego.im/docuo/7c17419de1.png" alt="3_消息回执_中文.png"/>
</Frame>

<Video src="https://doc-media.zego.im/core_products/zim/zh/docs_zim_android_zh/Guides/20260910-zim-1920x1080.mp4" />

本文档介绍了如何使用 ZIM SDK 的接口，实现发送一条附带回执的消息，以及查看和应答回执详情等功能。

<Warning title="注意">

ZIM SDK 目前支持发送“单聊”会话和“群组”会话的消息回执（仅支持普通消息、富媒体消息和自定义消息），暂不支持发送“房间”会话的消息回执。
...
消息发送端通过 ZIM SDK 发送一条消息，并通过设置 {getPlatformData(props,ZIMMessageSendConfigMap)} 的 `hasReceipt` 字段标记该消息是否需要带回执，接收端可根据消息的回执状态 `receiptStatus` 判断该消息是否带回执以及回执的当前进度，从而渲染不同的 UI 效果，而接收端可根据不同的场景进行不同的已读方式。

其中，`receiptStatus` 的取值来自枚举 [ZIMMessageReceiptStatus](@-ZIMMessageReceiptStatus)：

| 取值-30% | 说明 |
| --- | --- |
| NONE | 不是回执 |
| PROCESSING | 回执中 |
| DELIVERED | 已送达（3.1.0 及以上版本支持） |
| DONE | 已完成，即已被已读 |
| FAILED | 已失败 |
| EXPIRED | 已过期 |
| UNKNOWN | 未知 |
...
<div>
2. 客户端 A 调用 {getPlatformData(props,sendMessageMap)} 接口，向客户端 B 发送一条消息（仅支持“单聊”会话和“群组”会话的 {getPlatformData(props,ZIMTextMessageMap)}、 {getPlatformData(props,ZIMImageMessageMap)}、 {getPlatformData(props,ZIMFileMessageMap)}、 {getPlatformData(props,ZIMAudioMessageMap)}、 {getPlatformData(props,ZIMVideoMessageMap)}、 {getPlatformData(props,ZIMCombineMessageMap)}、 {getPlatformData(props,ZIMMultipleMessageMap)} 和 {getPlatformData(props,ZIMCustomMessageMap)}），并设置 {getPlatformData(props,ZIMMessageSendConfigMap)} 的 `hasReceipt` 字段为 true；
</div>
:::
:::if{props.platform="U3d"}
<div>
...
:::
3. 客户端 B 通过监听相关回调（{getPlatformData(props,onMessageReceivedMap)}）会收到一条 `receiptStatus` 为 `PROCESSING` 的消息。
...
- 如需对会话的历史消息进行相关操作，需先完成查询历史消息，对历史消息的回执状态进行判断，详情请参考 [查询历史消息](./Get%20message%20history.mdx)。

</Warning>

1. 客户端 B 通过相关回调（{getPlatformData(props,onMessageReceivedMap)}）收到客户端 A 发送的一条附带回执的消息；
...
1. 客户端 B 根据回调 {getPlatformData(props,onMessageReceivedMap)} 的 `receiptStatus` 字段判断该消息的回执状态。如果是该字段为 `PROCESSING`，则表示该消息处于回执中，开发者可根据自己的业务逻辑，调用 {getPlatformData(props,sendConversationMessageReceiptReadMap)} 接口将会话内客户端 A 已发送的所有消息都设置为已读。
...
支持的接口：{getPlatformData(props,queryMessageReceiptsInfoMap)}、{getPlatformData(props,onMessageReceiptChangedMap)}。

<Warning title="注意">

- 对于消息发送者：当该消息被会话内成员全部已读时，已读时间才会有值，且为最后一个成员已读时的服务端时间戳，否则为 0。
- 对于消息接收者：已读时间为调用 {getPlatformData(props,sendMessageReceiptsReadMap)} 成功时的服务端时间戳，否则为 0。
- 获取回执已读时间暂不支持[“会话已读”](#会话已读)。
- 2.22.0 版本开始支持。
</Warning>
...
发送者发送回执消息后，若接收端设备已收到该消息，那么消息的回执状态将变更为已送达。开发者可通过 {getPlatformData(props,onMessageReceiptChangedMap)} 回调监听回执状态的变化。

<Warning title="注意">
ZIM 3.1.0 版本起，消息回执在 “回执中” 和 “已完成” 之间新增了 “已送达” 状态，用于表示回执消息已送达，单聊和群聊均支持。此功能仅付费版支持，如需使用，请联系 ZEGO 技术支持开启。若使用 3.1.0 之前的版本需要此能力，请联系 ZEGO 技术支持评估。
</Warning>

以下场景的消息回执状态会变更为 “已送达” 并触发 {getPlatformData(props,onMessageReceiptChangedMap)} 回调。
...
如果您需要在消息接收方收到离线推送后使得消息回执状态在发送端变更为 “已送达”（iOS 可通过开启推送拦截，Android FCM 可通过使用 DataMessage 推送消息类型的方式，国内 Android 厂商暂不支持），可在收到推送后调用 [服务端 API - 设置群聊消息已送达回执](/zim-server/messaging/set-delivery-receipts-for-group-messages) 进行确认。

### 回执有效期与已过期状态

消息回执的有效期为 7 天。超出有效期后，消息的回执状态将流转为“已过期”（`EXPIRED`），且不再支持对该消息继续进行已读操作。

开发者可通过 {getPlatformData(props,onMessageReceiptChangedMap)} 回调实时感知回执过期状态的变化（2.16.0 及以上版本支持），建议在 UI 层将过期消息的回执标识置灰或移除。

### 查询群消息回执状态的成员列表

调用 {getPlatformData(props,queryGroupMessageReceiptMemberListMap)} 可查询群消息的已读、未读（未送达）、或已送达状态的成员列表。
...
| 返回值-30% | 说明 |
|--- | --- |
| [ZIMGroupMessageReceiptMemberListQueriedCallback](@-ZIMGroupMessageReceiptMemberListQueriedCallback) | 查询结果的回调，其中 `userList` 为符合查询条件的群成员列表，`nextFlag` 为下一次查询的起始标记（当 `nextFlag` 为 0 时表示查询完毕）。|
:::
