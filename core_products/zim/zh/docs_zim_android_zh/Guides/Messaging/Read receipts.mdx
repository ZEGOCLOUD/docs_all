...
自 ZIM 3.1.0 版本起，消息回执在"回执中"和"已完成"之间新增了"已送达"状态。发送者发送回执消息后，若接收端设备已收到该消息，那么消息的回执状态将变更为已送达。该功能默认为关闭状态，您可以通过联系技术支持开启此功能。
开发者可通过 [onMessageReceiptChanged](@onMessageReceiptChanged) 回调监听回执状态的变化。
...
调用该接口可一次性查询群消息的已读、未读（未送达）、或已送达状态的成员列表。
