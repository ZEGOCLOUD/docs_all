...
<Frame width="60%" height="auto" >
  <img src="https://doc-media.zego.im/docuo/b6034e2b18.png" alt="ZIMCombineMessage_zh.png"/>
</Frame>
...
2. 调用 {getPlatformData(props,sendMessageMap)} 接口发送合并消息。开发者可参考 [收发消息 - 发送消息](./Send%20and%20receive%20messages.mdx#发送消息) 了解此接口参数详情。
...
接收合并消息的回调接口与接收普通消息的回调接口一致，请参考 [收发消息 - 接收消息](./Send%20and%20receive%20messages.mdx#接收消息) 了解具体接口。

<Warning title="注意">

接收消息回调仅返回合并消息的标题、概要、合并 ID 的信息，如需了解合并的子消息内容，请参考 <a href="#查看合并消息详情">查看合并消息详情。</a> 
</Warning>
...
@Override
    public void onMessageReceived(ZIM zim, ZIMMessageReceivedEventResult result) {
        ...
        String conversationType = result.conversationType;
        if (conversationType == ZIMConversationType.PEER) {
...
- (void)zim:(ZIM *)zim messageReceivedWithResult:(ZIMMessageReceivedEventResult *)result {
    ...
    if (conversationType == ZIMConversationTypePeer) {
...
如果需要下载子消息中的富媒体文件，可遍历子消息列表并判断为富媒体文件消息时，调用 `downloadMediaFile` 接口下载。开发者可参考 [收发消息 - 富媒体消息](./Send%20and%20receive%20messages.mdx#富媒体消息-2) 了解此接口参数详情。
...
逐条转发消息本质上就是将已发送成功的消息作为参数发送到其他会话，与发送普通消息所用接口相同，开发者可参考 [收发消息 - 发送消息](./Send%20and%20receive%20messages.mdx#发送消息) 了解此接口参数详情。
