...
本文介绍了如何使用 ZIM SDK 在基本的单聊会话页面中渲染对话消息。
...
在获取消息数据前，还需要实现一个 `addMessage` 工具方法用于合并消息数据。
...
调用 {getPlatformData(props,queryHistoryMessageMap)} 接口从新分页到旧分页进行查询，获取双方历史发送的消息。
...
监听 {getPlatformData(props,peerMessageReceivedMap)}，获取实时的聊天消息，并更新消息到聊天界面。
...
```
:::
3. 发送消息时将消息展示到聊天页面，并监听消息发送的状态。

监听 {getPlatformData(props,messageSentStatusChangedMap)} ，获取当前用户发送的消息发送状态的变化（发送中、发送成功、发送失败），并更新 UI。
