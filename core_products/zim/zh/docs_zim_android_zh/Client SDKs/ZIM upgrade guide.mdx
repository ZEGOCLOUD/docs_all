...
ZIM Native 2.28.0 版本对修改了部分接口返回参数和增加了事件回调的时机。从旧版本升级到 2.28.0 版本时，请您阅读以下指南。
</Warning>
...
[onMessageReactionsChanged](@onMessageReactionsChanged) 现在也会在操作端调用 [addMessageReaction](@addMessageReaction)、[deleteMessageReaction](@deleteMessageReaction) 或 [deleteAllMessageReactions](@deleteAllMessageReactions) 成功后回调，请开发者检查相应业务逻辑在升级 SDK 之后是否产生影响。
</Step>

<Step title="onConversationChanged">
[onConversationChanged] (@onConversationChanged) 现在也会在操作端调用 [deleteConversation](@deleteConversation) 成功后回调，请开发者检查相应业务逻辑在升级 SDK 之后是否产生影响。
</Step>

<Step title="onConversationsAllDeleted">
[onConversationsAllDeleted] (@onConversationsAllDeleted) 现在也会在操作端调用 [deleteAllConversations](@deleteAllConversations) 成功后回调，请开发者检查相应业务逻辑在升级 SDK 之后是否产生影响。
</Step>
