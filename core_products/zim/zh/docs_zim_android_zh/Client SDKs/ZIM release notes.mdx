...
|-----|-----|-----|
|消息表态支持重复表态及统计表态次数| 支持单个用户对同一条消息进行多次相同类型的表态。查询消息表态时，可以获取此消息每种表情的累计表态次数，或者统计每个用户针对该消息每种表情的表态次数。详细请参考[消息表态](../Guides/Messaging/Respond%20to%20messages%20with%20emoticons.mdx#2-新增消息表态--重复表态计数)|[addMessageReaction](@addMessageReaction)|
...
|优化消息置顶功能| 消息置顶超过最大消息数时，支持指定是否覆盖当前置顶列表。|[pinMessage](@pinMessage)|
| 优化多端登录场景下用户离线推送的角标处理策略 | 用户在多端登录的场景下，其他端阅读了新消息，iOS、Android 端的离线通知角标会进行清除。 | - |
