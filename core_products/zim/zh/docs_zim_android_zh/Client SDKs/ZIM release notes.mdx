...
| 会话列表同步状态回调优化 | 当用户登录 ZIM 服务时或断线重连后，可通过 [onConversationListSyncStateChanged](@onConversationListSyncStateChanged) 回调，得知其从 ZIM 服务端同步最新会话列表的状态。详情请参考 [获取会话列表 - 监听会话列表同步状态变更](./../Guides/Conversation/Get%20the%20conversation%20list.mdx#监听会话列表同步状态变更)。 | [onConversationListSyncStateChanged](@onConversationListSyncStateChanged) |
