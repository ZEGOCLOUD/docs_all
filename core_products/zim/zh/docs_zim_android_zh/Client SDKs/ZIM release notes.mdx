...
|-----|-----|-----|
|本地会话支持过滤查询|本地会话搜索接口支持按照 conversationName 条件过滤查询|-|
...
|-----|-----|-----|
|组合消息数量和类型限制放开|组合消息目前可以支持一次性发送 10 个媒体消息，不限制媒体类型|-|
|组合消息的子消息发送逻辑优化|组合消息中如果包含多个子消息时，如果其中的一条发送失败，其他的消息可以正常发送|-|
|会话标记数量提升|单个会话的会话标记数量最大支持到 60 个|-|
