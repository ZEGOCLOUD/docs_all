...
| 获取房间用户扩展字段 | 用户在加入房间后，可获取此用户进房前的用户扩展信息。详细请参考 [获取房间用户扩展字段](./../Guides/Room/Room%20info.mdx#获取房间用户扩展字段)。 | [queryRoomMembers](@queryRoomMembers)、[queryRoomMemberList](@queryRoomMemberList)、[onRoomMemberJoined](@onRoomMemberJoined)、[onRoomMemberLeft](@onRoomMemberLeft) |
