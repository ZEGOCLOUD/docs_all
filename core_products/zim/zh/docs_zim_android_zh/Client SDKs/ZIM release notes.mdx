...
|-----|-----|-----|
|消息表态支持重复表态及表态次数统计| 支持单个用户对同一条消息进行多次相同类型的表态。查询消息表态时，将返回每种表情的累计表态次数，以及每个用户针对该消息每种表情的表态次数统计。|-|
| 查询历史消息支持配置优先同步服务端数据 |[queryHistoryMessage](@queryHistoryMessage) 新增 `isPreferQueryFromServer` 参数，用于设置是否优先从服务端获取历史消息（默认优先使用本地缓存）。|-|

**改进优化**
|优化项-25% | 优化描述-40% | 相关接口-25%|
|-----|-----|-----|
|优化删除会话接口| 调用 [deleteConversation](@deleteConversation) 接口时，会正确的触发 [onConversationChanged](@onConversationChanged) 回调，事件类型为 `deleted`。|-|
|优化消息置顶功能| 置顶列表在清空后仍可以查询到对应的消息。|-|
