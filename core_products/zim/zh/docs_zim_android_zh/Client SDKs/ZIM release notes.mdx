...
| 新增社群会话管理 | 会话列表新增社群类型会话，支持以社群模式展示会话。详细请参考 [频道会话管理 - 将频道会话同步到会话列表](./../Guides/community/channel-conversation-management.mdx#将频道会话同步到会话列表) | <ul><li> [queryConversationList](@queryConversationList)</li><li> [onConversationChanged](@onConversationChanged)</li></ul> |
...
| 消息回执新增已送达状态 | 消息回执新增已送达状态，支持在单聊和群聊中使用；群聊场景下可展示已送达状态列表。详细请参考 [消息回执 - 获取回执消息的已送达状态](../Guides/Messaging/Read%20receipts.mdx#获取回执消息的已送达状态) | [onMessageReceiptChanged](@onMessageReceiptChanged) |
...
|离线推送功能支持附带 OPPO 私信模版 | OPPO 推送在消息分类中新增了私信通道，ZIM 对此进行了适配。详细请参考 [OPPO 推送集成指南](./../Offline%20push%20notifications/Integrate%20OPPO.mdx#根据-oppo-消息分类新规申请消息分类)。 | [ZIMPushConfig](@-ZIMPushConfig) |
...
| 新增群主退群后的群组 Tips 消息 | 群主退群后，群成员会额外收到一条 Tips 消息 `xxx 成为新群主`。详细请参考 [接收群组 Tips 消息](./../Guides/Messaging/Receive%20tip%20messages.mdx)。 | [onGroupMessageReceived](@onGroupMessageReceived) |
...
| 获取房间用户扩展字段 | 用户在加入房间后，可获取此用户进房前的用户扩展信息。详细请参考 [获取房间用户扩展字段信息](./../Guides/Room/Room%20info.mdx#获取房间用户扩展字段信息)。 | [queryRoomMembers](@queryRoomMembers)、[queryRoomMemberList](@queryRoomMemberList)、[onRoomMemberJoined](@onRoomMemberJoined)、[onRoomMemberLeft](@onRoomMemberLeft) |
...
1. 修复在同一个 ZIM 实例的生命周期内，[onConversationSyncStateChanged](@onConversationSyncStateChanged) 事件通知在 “登录登出再登录” 后失效的问题。
...
| 群组定向消息 | ZIM 支持用户在调用 [ZIM 服务端接口](/zim-server/messaging/send-group-messages#targetoption) 发送群组消息时，指定对某些群用户是否可见。详情请参考 [群组定向消息](./../Guides/Messaging/group-targeted-message.mdx)。 | [onGroupMessageReceived](@onGroupMessageReceived) |
| 会话列表同步状态回调 | 当用户登录 ZIM 服务时或断线重连后，可通过 [onConversationSyncStateChanged](@onConversationSyncStateChanged) 回调，得知其从 ZIM 服务端同步最新会话列表的状态。详情请参考 [获取会话列表 - 监听会话列表同步状态变更](./../Guides/Conversation/Get%20the%20conversation%20list.mdx#监听会话列表同步状态变更)。 | [onConversationSyncStateChanged](@onConversationSyncStateChanged) |
...
| 优化项-25%  | 优化描述 | 相关接口 |
| -----  | ---- | ----- |
| OPPO 推送优化 | 为适配 OPPO 消息分类新规（[OPUSH 消息分类细则](https://open.oppomobile.com/documentation/page/info?id=13189)），支持开发者设置 `category` 和 `notify_level` 字段。详情请参考 [OPPO 推送集成指南](./../Offline%20push%20notifications/Integrate%20OPPO.mdx#根据-oppo-消息分类新规申请消息分类) | - |
...
| 自定义用户状态 | 支持开发者自定义用户状态，适用于登录或其他需要在线状态以外状态的等场景（如隐身、听歌中、通话中），详情请参考 [用户状态订阅 - 设置自定义状态](./../Guides/Users/Online%20Status%20Subscription.mdx#设置自定义状态)。 | <ul><li>[ZIMLoginConfig > customStatus](@customStatus-ZIMLoginConfig)</li><li>[updateUserCustomStatus](@updateUserCustomStatus)</li></ul> |
...
| 变更接口 | 变更描述 | 
| -----  | ---- | 
| downloadMediaFile 及相关回调| <ul><li>废弃原接口 `downloadMediaFile`，请使用同名 [downloadMediaFile](@downloadMediaFile) 代替。新版本的 [downloadMediaFile](@downloadMediaFile) 新增了 `config` 参数，新增了 `config` 参数，可用于指定下载组合消息中的单个媒体内容。</li><li>[ZIMMediaDownloadedCallback](@-ZIMMediaDownloadedCallback) 回调中 [onMediaDownloaded](@onMediaDownloaded) 和 [onMediaDownloadingProgress]() 的参数 `message` 类型从 `ZIMMediaMessage` 变更为 `ZIMMessage`。</li></ul> |
| [sendMessage](@sendMessage) | 废弃 `sendMediaMessage` 和旧的 `sendMessage` 方法，新增支持发送任意消息类型的 [sendMessage](@sendMessage) 方法。 |
| [ZIMMessageSentFullCallback](@-ZIMMessageSentFullCallback) |[ZIMMessageSentFullCallback](@-ZIMMessageSentFullCallback) 中的 [onMediaUploadingProgress](@onMediaUploadingProgress-ZIMMessageSentFullCallback) 参数 `message` 类型由 `ZIMMessage` 变更为 `ZIMMediaMessage`，以确保仅媒体消息会被回调通知。|
...
| 功能项-25% | 功能描述 | 相关接口 |
| ------------------------------  | ---- | ----- |
| 群备注  | 支持用户对某个群组设置仅自己可见的群备注。详情请参考 [群资料管理 - 修改群备注](./../Guides/Group/Group%20info.mdx)。| <ul><li>[updateGroupAlias](@updateGroupAlias)</li><li>[onGroupAliasUpdated](@onGroupAliasUpdated)</li></ul> |
| 用户在线状态 | 不仅支持用户通过订阅以持续关注其他用户的在线状态变化，还支持一次性查询其他用户的当下的在线状态，也支持用户查询自己的订阅列表。此外，在多端登录场景下，用户可以监听自己的在线平台变化。详情请参考 [用户状态订阅](./../Guides/Users/Online%20Status%20Subscription.mdx)。| <ul><li>[subscribeUsersStatus](@subscribeUsersStatus)</li><li>[onUserStatusUpdated](@onUserStatusUpdated)</li><li>[unsubscribeUsersStatus](@unsubscribeUsersStatus)</li><li>[queryUsersStatus](@queryUsersStatus)</li><li>[querySubscribedUserStatusList](@querySubscribedUserStatusList)</li></ul> |
...
| 会话列表查询 | 支持将标记、会话类型和是否包含未读消息作为过滤项获取会话列表。详情请参考 [标记会话](./../Guides/Conversation/Mark%20conversations.mdx)。 | [queryConversationList](@queryConversationList) |
...
| Tips 消息 | 支持将群内用户的某些群组操作（如创建群组、解散群组等） 转换为特殊类型消息（Tips）。凭借群组消息回调获得操作信息后，开发者可以自行构造并在 UI 上展示相关事件的描述文本。<br />如需了解 Tips 消息种类和扩展信息，以及调接收后的处理操作，请参考 [接收 Tips 消息](./../Guides/Messaging/Receive%20tip%20messages.mdx)。 | <ul><li>[ZIMMessageType > TIPS](@tips)</li><li>[onReceiveGroupMessage]()</li></ul> |
...
| 消息导出导入 | 支持将用户本地终端的历史消息导出作为备份，可用于更换设备时迁移聊天记录，或恢复被删除的消息。<br />如需了解接口调用细节，请参考 [导出导入消息](./../Guides/Messaging/Export%20and%20import%20messages.mdx)。 | <ul><li>[exportLocalMessages]()</li><li>[importLocalMessages]()</li></ul> |
...
| 功能项-25% | 功能描述 | 相关接口 |
|-------|-------|-------|
| 合并转发 | <ul><li>支持用户在收发消息后，通过构造合并消息体，传入发送消息接口即可实现转发合并消息；**注意：**如需实现逐条转发，只需将已有消息作为参数传入发送消息接口即可。</li><li>支持查询合并消息包含的子消息具体内容。</li></ul> | <ul><li>[ZIMCombineMessage](@-ZIMCombineMessage)</li><li>[sendMessage](@sendMessage)</li><li>[queryCombineMessageDetail](@queryCombineMessageDetail)</li></ul> |
...
| 优化项-25%  | 优化描述 | 相关接口 |
| -----  | ---- | ----- |
| 优化 ZIMMessage 和 ZIMConversation 数据结构 | ZIM 对 ZIMMessage 和 ZIMConversation做了可序列化实现，不同 Activity 之间传值更方便。 | <ul><li>[ZIMMessage](@-ZIMMessage)</li><li>[ZIMConversation](@-ZIMConversation)</li></ul> |
...
| 功能项-25% | 功能描述 | 相关接口 |
|-------|-------|-------|
| 多端登录 | “多平台登录”策略支持配置互踢逻辑，当前仅支持 android、iOS 设备互踢，Windows、Mac 设备互踢。 | [login](@login) |
| 离线登录 | 支持用户在离线状态登录 IM 服务，访问本地 SDK 数据。 | [login](@login) |
...
| 呼叫邀请 | 新增回调 [onCallInvitationCreated](@onCallInvitationCreated)，呼叫邀请发起者可通过监听此回调，得知呼叫邀请已创建。 | [onCallInvitationCreated](@onCallInvitationCreated) |
...
| 用户信息相关类新增用户头像字段 | [ZIMUserInfo](@-ZIMUserInfo) 和 [ZIMGroupMemberInfo](@-ZIMGroupMemberInfo) 新增 `userAvatarUrl` 字段，用于设置或说明用户头像地址。**说明**：暂不支持获取房间成员的用户头像字段，即通过 [queryRoomMemberList](@queryRoomMemberList) 和 [queryRoomMembers](@queryRoomMembers) 拿到的 `ZIMUserInfo` 的 `userAvatarUrl` 为空。 | <ul><li>[ZIMUserInfo](@-ZIMUserInfo)</li><li>[ZIMGroupMemberInfo](@-ZIMGroupMemberInfo)</li></ul> |
| 优化 [onRoomStateChanged](@onRoomStateChanged) 回调逻辑  | 当开发者调用 ZIM 服务端接口在后台销毁房间后，[onRoomStateChanged](@onRoomStateChanged) 回调描述房间状态变化的原因修改为 `ROOM_NOT_EXIST`。 | [onRoomStateChanged](@onRoomStateChanged) |
...
| 废弃项-25%  | 废弃描述 | 相关接口 |
| -----  | ---- | ----- |
| 废弃 `login` 接口 | 废弃旧版登录接口，新增新版登录接口 [login](@login)。新版登录接口支持通过 [ZIMLoginConfig](@-ZIMLoginConfig) 进行更多配置，如是否使用 Token 鉴权、是否离线登录。 | [login](@login) |
...
| 群消息管理 | 支持新进入群组成员自动获取群历史消息。**说明**：如需使用此功能，请联系 ZEGO 技术支持配置。 | <ul><li>[joinGroup](@joinGroup)</li><li>[inviteUsersIntoGroup](@inviteUsersIntoGroup)</li></ul> |
...
| 功能项-25% | 功能描述 | 相关接口 |
|-------|-------|-------|
| 多端登录 | 支持配置双平台或多平台登录策略，用户可在多平台、多设备上同时登录同一账户，实现会话、消息、群组等数据多端互通。如需了解多端登录对其他功能的影响，请参考 [多端登录](./../Guides/Users/Multi-device%20login.mdx)。<br />**注意**：仅支持专业版或旗舰版套餐用户使用此功能，且需联系 ZEGO 技术支持进行配置登录策略。 | [login](@login) |
...
| 服务端发送房间消息接口支持发送者有感知 | 新增 `SenderUnaware` 字段，允许通过服务端发送房间消息后，请求参数中的 `FromUserId`（发送方用户 ID）所对应的客户端对此次发送有感知。 | [发送房间消息](/zim-server/messaging/send-room-messages) |
| 服务端新增全员推送接口 | 支持向所有在线用户（包括消息发送用户自己）发送特定内容的消息，如文本、图片等。本功能适用于全员活动公告、送礼跨房间飘屏等场景。 | <ul><li>[全员推送](/zim-server/messaging/push-message-to-all-users)</li><li>[onBroadcastMessageReceived](@onBroadcastMessageReceived)</li></ul> |
...
| 优化项-25%  | 优化描述 | 相关接口 |
| -----  | ---- | ----- |
| 呼叫邀请接收回调 | 优化呼叫邀请普通模式，支持在呼叫邀请的超时时间内，离线用户收到呼叫邀请在上线后能够立即被通知。| [onCallUserStateChanged](@onCallUserStateChanged) |

**废弃删除**

| 废弃接口 | 废弃说明 | 废弃版本 |
| -----  | ---- | ----- |
| <ul><li>[onCallInvitationAccepted]()</li><li>[onCallInvitationRejected]()</li><li>[onCallInviteesAnsweredTimeout]()</li></ul> | 为便利开发者监听呼叫邀请中用户的呼叫状态变化，新增 [onCallUserStateChanged](@onCallUserStateChanged)，替代原呼叫邀请回调 `onCallInvitationAccepted`、`onCallInvitationRejected` 和 `onCallInviteesAnsweredTimeout`。 | 2.9.0 |
...
| 查询房间内用户状态 | 通过指定若干 userID 和 roomID，查询目标用户是否在指定房间内，从而进行业务逻辑设计，如邀请连麦。**说明**：最多支持一次性查询 10 名用户的信息。 | [queryRoomMembers](@queryRoomMembers) |
...
| 废弃项-25%  | 废弃描述 | 相关接口 |
| -----  | ---- | ----- |
| 废弃系统消息类型 | [ZIMMessageType](@-ZIMMessageType) 废弃了系统消息（值为 30），开发者请使用功能更完善的自定义消息（值为 200）替代。 | [ZIMMessageTypeCustom](https://doc-zh.zego.im/article/api?doc=zim_API~java_android~enum~ZIMMessageType#custom) |
...
| 新增发送消息状态回调 | 开发者可以通过监听此回调来完善消息发送状态的逻辑。根据消息状态的变更，开发者可以在 UI 上做相应提醒等。 | [onMessageSentStatusChanged](@onMessageSentStatusChanged) |
...
| 功能项-25% | 功能描述 | 相关接口 |
|-------|-------|-------|
|消息撤回|  撤回发送成功的消息，默认撤回 2 分钟内的消息。撤回操作仅支持撤回单聊和群聊的消息，不支持撤回房间内消息。**说明**：如需配置撤回时间，请联系 ZEGO 技术支持。| <ul><li>[revokeMessage](@revokeMessage)</li><li>[onMessageRevoked](@onMessageRevoked)</li></ul>|
...
| 新增接口 | 接口描述 | 上线版本 |
| :-------- | :------- | :----- |
| [login](@login) | 新增不带 Token 参数的 [login](@login) 接口。开发者使用 “AppSign 鉴权” 时，可直接使用该接口登录 ZIM，接入更加便捷。   | 2.4.0 |
| [sendMessage](@sendMessage) |  新增发送消息接口，支持发送单聊、房间、群组消息。  | 2.4.0 |
| [onMessageAttached](@onMessageAttached-ZIMMessageSentCallback) | 在 [ZIMMediaMessageSentCallback](@-ZIMMediaMessageSentCallback) 中新增 [onMessageAttached](@onMessageAttached-ZIMMessageSentCallback) 回调方法，用于监听消息对象发送前的相关信息，否则编译无法通过。  | 2.4.0 |
...
| 支持 AppSign 鉴权 | <p>iOS、Android、macOS、Windows 平台支持 AppSign 鉴权，降低开发者接入门槛。</p><p>开发者在调用 [create](@create) 接口，传入 AppSign，进行鉴权，鉴权通过即可使用 ZIM 相关功能。</p>| [create](@create)|
...
| 废弃接口 | 变更说明 | 废弃版本 |
| :-------- | :------- | :----- |
| [create](@create) | 原有的 `create` 接口废弃，替换为同名的 [create](@create) 接口，开发者可以通过传入 AppSign 进行鉴权，接入更加便捷。   | 2.3.0 |
...
| 功能项-25% | 功能描述 | 相关接口 |
|-------|-------|-------|
| 收发富媒体消息 | 支持用户发送和接收图片、语音、视频和文件等类型的富媒体消息。| <ul><li>[sendMediaMessage]()</li><li>[downloadMediaFile](@downloadMediaFile)</li></ul>|
...
| 离线消息推送 | <ul><li>支持“单聊/群组”消息发送给离线用户，离线用户上线后自动拉取。</li><li>支持通过手机厂商，将离线消息通知推送到手机设备上，支持主流厂商（华为、小米、OPPO、vivo、Apple）多数机型。</li></ul>| <ul><li>[setPushConfig](@setPushConfig)</li><li>[sendPeerMessage](@sendPeerMessage)</li><li>[sendGroupMessage](@sendGroupMessage)</li></ul>|
...
| 功能项-25% | 功能描述 | 相关接口 |
|-------|-------|-------|
| 新增“房间属性”功能 | <p>房间属性指房间内保存的 `Key-Value` 属性对，开发者可以使用此功能为房间设置自定义信息。</p><p>此功能可用于语聊房、直播等场景，用来记录房间背景图地址、各用户的角色身份、设备状态等自定义信息。</p> | <ul><li>[createRoom](@createRoom)</li><li>[setRoomAttributes](@setRoomAttributes)</li><li>[deleteRoomAttributes](@deleteRoomAttributes)</li><li>[queryRoomAllAttributes](@queryRoomAllAttributes)</li><li>[beginRoomAttributesBatchOperation](@beginRoomAttributesBatchOperation)</li><li>[endRoomAttributesBatchOperation](@endRoomAttributesBatchOperation)</li><li>[onRoomAttributesUpdated](@onRoomAttributesUpdated)</li><li>[onRoomAttributesBatchUpdated](@onRoomAttributesBatchUpdated)</li></ul> |
...
| 新增接口 | 接口描述 | 
|---------|---------|
| [createRoom](@createRoom) | 创建带高级设置的房间。 |
...
| [deleteRoomAttributes](@deleteRoomAttributes) | 删除房间属性。调用此接口，传入 Key 即可删除，支持同时删除多个 Key-Value 属性对。 |
| [queryRoomAllAttributes](@queryRoomAllAttributes) | 查询房间内设置的属性信息。 |
| [beginRoomAttributesBatchOperation](@beginRoomAttributesBatchOperation) | <p>开始进行房间属性的组合式操作，支持组合调用设置和删除房间属性。组合操作可将同一房间中，多个不同操作合并为一个原子操作进行执行，通常用于连续操作时不想被其他用户操作插入执行的场景中。</p><p>在此接口之后调用的 [setRoomAttributes](@setRoomAttributes) 和 [deleteRoomAttributes](@deleteRoomAttributes) 将会被同时发送到后台执行。</p> |
| [endRoomAttributesBatchOperation](@endRoomAttributesBatchOperation) | <p>结束进行房间属性的组合式操作。</p><p>调用此接口之后将请求后台执行组合式操作。</p> |
| [onRoomAttributesUpdated](@onRoomAttributesUpdated) | 房间内有属性对更新时，相关信息的回调。 |
|[onRoomAttributesBatchUpdated](@onRoomAttributesBatchUpdated) | 房间内有使用组合式操作来更新房间属性时，相关信息的回调。|
...
|  优化项-25%  | 优化描述 | 相关接口 |
| -----  | ---- | ----- |
| SDK 接口优化 | [onRoomStateChanged](@onRoomStateChanged) 回调新增 roomID 参数。 | [onRoomStateChanged](@onRoomStateChanged) |
