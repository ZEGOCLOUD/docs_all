...
设置此回调后，当用户发送单聊、群聊、房间或频道聊天消息时，ZIM 会向您的业务后台发起请求，您可以通过应答，对消息进行实时操作，如：
- 记录聊天消息，拦截违规发言请求；
...
- 请求地址：请在 [ZEGO 控制台](https://console.zego.im/) 上配置回调地址，配置流程请参考控制台文档 [ZIM 相关回调配置](https://doc-zh.zego.im/console/service-configuration/im/call-callback-config)。
...
| conv_type | Int | 目标会话类型：<ul><li>0：单聊。</li><li>1：房间。</li><li>2：群聊。</li><li>3：频道。</li></ul> |
...
<Warning title="注意">
- 如果需要此回调返回 ZIM 内容审核服务的拒绝原因，请联系 ZEGO 技术支持配置开通。
- 在 2.21.0 及以后的版本中，如果没有开通 ZIM 内容审核服务，此回调默认会返回业务服务端的拒绝原因。
</Warning>
