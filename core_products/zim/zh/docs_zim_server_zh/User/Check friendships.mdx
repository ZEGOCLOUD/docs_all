...
<td>Number</td>
<td>返回码。
<Note title="说明">当您发起请求同时检查 `FromUserId` 与多名用户的好友关系时：<ul><li>如果成功检查 `FromUserId` 与 1 名或更多用户的好友关系，Code 都会返回 0。此时请参考 Fails 中的具体信息，确认操作结果。</li><li>如果&nbsp; `FromUserId` 与全部目标用户的好友关系都检查失败，Code 会返回相关返回码，具体请参考 [全局返回码](../Return%20codes.mdx)。</li></ul></Note></td>
</tr>
<tr data-row-level="3">
<td>Message</td>
<td>String</td>
