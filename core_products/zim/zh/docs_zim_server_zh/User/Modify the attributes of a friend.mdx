...
| 参数                      | 类型            | 是否必选 | 描述                                                                                                                                                                       |
| ------------------------- | --------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FromUserId                | String          | 是       | 更新此 UserID（已在客户端调用 `login` 方法登录 ZIM 服务，或已调用 [服务端 API](./../User/Batch%20register%20users.mdx) 完成注册）对指定好友的属性设置。                    |
| UserId                    | String          | 是       | 好友属性待更新的用户。                                                                                                                                                     |
| [Attributes](#Attributes) | Array Of Object | 是       | 新的好友属性。默认一次性最多批量更新 5 个属性。<Note title="说明">Key 的总长度与 Value 的总长度上限为 1024 字节。如需上调，请联系 ZEGO 技术支持。建议不要将上限设置过大。</Note> |                                                                                                                                                                                                     
| Action                    | Number          | 否       | 对属性的操作。0：设置属性。                                                                                                                                                |

<a id="Attributes"></a>
**Attributes 结构如下：**
| 参数 | 类型 | 是否必选 | 描述 |
|------|------|----------|------|
| Key | String | 是 | 好友属性的键。取值仅能为 k0、k1、k2、k3、k4。 |
| Value | String | 是 | 好友属性的值。 |
