...
<td>String</td>
<td>是</td>
<td>消息发送者的用户 ID（已在客户端调用 `login` 方法登录 ZIM 服务，或已调用 [服务端 API](./../User/Batch%20register%20users.mdx) 完成注册）。</td>
</tr>
<tr>
<td>GroupId</td>
<td>String</td>
...
<td>Number</td>
<td>是</td>
<td>消息类型，群聊会话的适用类型请参考 [MessageBody 说明](./MessageBody%20Introduction.mdx)，导入消息时不支持信令消息。</td>
</tr>
<tr>
<td>Priority</td>
<td>Number</td>
...
<td>Object</td>
<td>是</td>
<td>消息内容，具体参数格式请参考 [MessageBody 说明](./MessageBody%20Introduction.mdx)，不支持离线推送 OfflinePush。</td>
</tr>
<tr>
<td>SubMsgType</td>
<td>Number</td>
...
- 传 0 或不传：取“当前时间”。
- 其他值：最早可取“当前时间 - 套餐包规定的历史消息保存天数（详情请参考 [计费说明 - 版本说明](/zh/zim-android/introduction/billing-instructions)）* 86400000”，最晚不得晚于“当前时间”。

不能早于套餐包规定的历史消息保存天数（详情请参考 [计费说明 - 版本说明](/zh/zim-android/introduction/billing-instructions)），也不能晚于当前时间。
</td>
</tr>
</tbody></table>
...
成功导入消息后，可调用 ZIM 服务端接口 [查询群聊会话消息列表](../Conversation/Query%20the%20message%20list%20of%20group%20chats.mdx)，确认群聊会话消息是否导入完整。
