...
<td>
消息 seq。获取方式：
- 若需要撤回由**客户端**发出的房间消息，通过 [消息发送后回调](../Callbacks/Message%20sent.mdx) 获取 MsgSeq。
- 若需要撤回由服务端 API [SendRoomMessage](./Send%20room%20messages.mdx) 发出的房间消息，通过接口响应数据获取 MsgSeq。
</td>
</tr>
<tr>
<td>Payload</td>
