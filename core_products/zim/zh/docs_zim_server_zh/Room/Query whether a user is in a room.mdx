...
| UserList | Object | 查询结果，即在指定房间的用户列表。 |
| └UserId | String | 在指定房间的用户的 ID。 |
| └UserName | String | 在指定房间的用户的名称。 |
| └Avatar | String | 用户头像信息。 |
| └Extra | String | 扩展信息。 | 
| ErrorUser | Object | 不在指定房间的用户列表。<ul><li>Code 为 0：<ul><li>ErrorUser 为空，全部用户都在指定房间。</li><li>ErrorUser 不为空，表示部分用户不在指定房间，请参考 SubCode 处理。</li></ul></li><li>Code 不为 0：<ul><li>ErrorUser 为空，表示参数错误、接口频率限制、系统错误。</li><li>ErrorUser 不为空，表示全部用户都不在指定房间。</li></ul></li></ul> |
