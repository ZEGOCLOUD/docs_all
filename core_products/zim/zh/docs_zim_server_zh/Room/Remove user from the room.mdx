...
<table>
  ...
  <tr>
    ...
    <td>是</td>
    <td><p>需要移除的用户 ID 列表（已在客户端调用 `login` 方法登录 ZIM 服务，或已调用 [服务端 API](./../User/Batch%20register%20users.mdx) 完成注册）），最大支持 50 个用户 ID。</p><p>
