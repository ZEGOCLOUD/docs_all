...
<Steps>
<Step title="监听注册回调">
监听 [ZPNsEventHandler.registered](@registered-ZPNsEventHandler) 回调，获取 ZPNs 注册结果，并在该方法触发时打印 `pushID`（用于向指定设备推送离线消息）。
```typescript
...
<Warning title="注意">
在实现离线推送前，请确保：
- 参考 [发送消息](./../Send%20and%20receive%20messages.mdx)，实现发送单聊或群聊消息的功能。
