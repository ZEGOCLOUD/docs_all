...
- 由于 uni-app 云端打包因限制而无法适用于 FCM，因此，如需使用 FCM 请使用离线方式打包工程，并通过本地包手动集成 FCM 推送 SDK，详情请参考 [FCM 推送集成指南](./Integrate%20FCM.mdx)。
...
8.在 [下载](./../Client SDKs/SDK downloads.mdx) 取最新版本的 ZPNs JS 插件 SDK ，并解压得到的 “zego-ZPNsUniPlugin-JS.zip” 文件。
...
3. 调用 [registerPush](@registerPush) 接口注册离线推送。

    <Note title="说明">
    - 在 iOS 端调用 [registerPush](@registerPush) 接口时，您需要根据打包时选择的证书为 development 还是 description 而提前填写 `ZPNsIOSEnvironment`。切换证书时，请更改此枚举。
    - 证书为 development 时，`ZPNsIOSEnvironment` 为 `Development`。
    - 证书为 distribution 时，`ZPNsIOSEnvironment` 为 `Production`。
    - 如果您尚不清楚当前所处的 `ZPNsIOSEnvironment`，请填 `ZPNsIOSEnvironment.Automatic`。`Automaic` 可能受 iOS 系统版本影响，若 iOS 系统进行大版本更新后，请留意 [ZPNs 发布日志](./../Client%20SDKs/ZPNs%20release%20notes.mdx)是否存在相关更新说明。
    </Note>
    ...
    注册离线推送后，可以通过 [ZPNsEventHandler](@-ZPNsEventHandler) 类中的 [registered](@registered) 回调，获取到离线推送的 `pushID`，向指定设备推送离线消息。
...
集成 ZPNs SDK 和获取 Push ID 后，您可以在 [ZEGO 控制台](https://console.zego.im/) 在线调试 ZIM 离线推送功能，详情请参考控制台的 [ZIM 离线推送调试](https://doc-zh.zego.im/console/development-assistance/zim-offline-push-debugging)。
<Note title="说明">
