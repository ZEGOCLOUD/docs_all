...
3. 打开 “pages/index/index.uvue” 文件，并使用本文 [前提条件](#前提条件) 已获取的 AppID 、AppSign 和 ServerSecret 正确填写，并保存。
    ...
    **若您的项目已切换为 “Token 鉴权”，请在 [ZEGO 控制台](https://console.zego.im) 上，申请临时 Token 用于调试。**
    ...
    ```
    在 `uni_modules/zego-zim-uts` 目录下运行 `npm install` 安装 npm 加密库，用于生成 Token。
...
1. [下载示例源码](https://artifact-demo.zego.im/core_products/zim/zh/docs_zim_uniapp_x_zh/ZIMUniAppUtsExample.zip)。
