...
- <b>方式一：从 ZEGO 官网下载</b>
1. 请参考 [下载](./../Client SDKs/SDK downloads.mdx) 页面，获取最新版本的 SDK 到本地，并将得到的 “zego-zpns-uts.zip” 文件解压缩。
...
1. 请参考 [下载](./../Client SDKs/SDK downloads.mdx) 页面，获取最新版本的 ZPNs 原生插件 SDK，并解压得到的 “zego-ZPNsUniPlugin.zip” 文件。
...
6. 在 [下载](./../Client SDKs/SDK downloads.mdx) 取最新版本的 ZPNs JS 插件 SDK ，并解压得到的 “zego-ZPNsUniPlugin-JS.zip” 文件。
...
4. 调用 [registerPush](@registerPush) 接口注册离线推送。
    ...
    注册离线推送后，可以通过 [ZPNsEventHandler](@-ZPNsEventHandler) 类中的 [onRegistered](@registered) 回调，获取到离线推送的 `pushID`，向指定设备推送离线消息。
