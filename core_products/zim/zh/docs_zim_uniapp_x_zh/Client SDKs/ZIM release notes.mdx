...
|优化项-25% | 优化描述-40% | 相关接口-35%|
|-----|-----|-----|
|Android、iOS 本地依赖库改为线上依赖| 将 Native 本地依赖库改为线上依赖，避免云打包体积过大 | - |
...
|功能项-25% | 功能描述-40% | 相关接口-35%|
|-----|-----|-----|
|消息置顶| 用户可以将会话中的消息固定在会话顶部，方便会话中的所有用户快速查看重要消息。详细请参考 [消息置顶](../Guides/Messaging/pin-message.mdx)。 | <ul><li>[onMessagePinStatusChanged](@messagePinStatusChanged)</li><li>[pinMessage](@pinMessage)</li><li>[queryPinnedMessageList](@queryPinnedMessageList)</li></ul> |
|uniapp x SDK 支持开发鸿蒙应用| 用户可以通过 uniapp x SDK 开发鸿蒙操作系统的应用，进一步扩充 ZIM uniapp x SDK 的应用开发能力 | - |
...
<Note title="说明">
ZIM SDK 首次发布，对齐 iOS/Android/Web/小程序 等平台的 2.21.0 版本。
</Note>
...
| 消息编辑 | 支持编辑自定义消息的检索字段（[ZIMCustomMessage > searchedContent](@searchedContent-ZIMCustomMessage)），和支持编辑单聊消息和群聊消息的内容和其他属性。详情请参考 [编辑消息](./../Guides/Messaging/Edit%20messages.mdx)。<Note>此接口默认仅适用于过去 24 小时内的消息。</Note>  | [editMessage](@editMessage) |
...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
|-------|-------|-------|
| 多端登录 | 支持配置双平台或多平台登录策略，用户可在多平台、多设备上同时登录同一账户，实现会话、消息、群组等数据多端互通。如需了解多端登录对其他功能的影响，请参考 [多端登录](../Guides/Users/Multi-device%20login.mdx)。<Note title="说明"><p>仅支持专业版或旗舰版套餐用户使用此功能，且需联系 ZEGO 技术支持进行配置登录策略。</p></Note> | [login](@login__1) |
...
1. 从 2.8.1 版本开始，ZIM 不再支持 iOS 11.0 以下版本，开发者的 iOS Deployment Target（最低支持版本）提升到 iOS 11.0
...
2. 从 2.8.1 版本开始，ZIM iOS SDK 不再支持 32 位 armv7 架构
...
修复调用 [sendMessageReceiptsRead](@sendMessageReceiptsRead) 接口报错问题。
...
| <div style={{width:"130px"}}>优化项</div>  | 优化描述 | 相关接口 |
| -----  | ---- | ----- |
| 变更 [ZIMPushConfig](@-ZIMPushConfig) 的额外字段命名| 为避免开发者接入概念混淆，[ZIMPushConfig](@-ZIMPushConfig) 的 extendedData 参数修改为 payload 参数，功能不变。| - |
...
ZIM SDK 版本首次发布，对齐 iOS/Android/macOS/Windows 等平台的 2.1.6 版本。
</Note>
