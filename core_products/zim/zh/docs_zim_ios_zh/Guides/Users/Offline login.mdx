...
在无网络且应用已被清理的情况下，可以调用 [loginWithUserID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#login-with-user-id-config-callback-2) 接口时，[ZIMLoginConfig](@-ZIMLoginConfig) 中的 [isOfflineLogin](@isOfflineLogin) 传入 `YES`，实现使用上一次的用户信息离线登录 App。当通过 [ZIMLoggedInCallback](@ZIMLoggedInCallback) 回调确定登录成功后，即可访问本地 SDK 数据。 
