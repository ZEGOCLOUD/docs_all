...
在使用 ZIM Audio SDK 其他接口前，请先调用 [initWithLicense](@initWithLicense) 进行初始化。
...
1. 创建一个声明 [ZIMAudioEventHandler](@-ZIMAudioEventHandler) 协议的类，并实现 [ZIMAudioEventHandler](@-ZIMAudioEventHandler) 中的方法。
...
1. 消息发送客户端调用 [startRecordWithConfig](@startRecordWithConfig) 接口定义音频文件存放的本地绝对路径（需包含音频文件名称与后缀，如 xxx/xxx/xxx.m4a，仅支持 .m4a 和 .mp3）和录制时长上限，开始录制音频文件。
...
1. 消息接收客户端调用 [startPlayWithConfig](@startPlayWithConfig) 接口，传入音频文件的本地绝对路径，设置音频输出的路由类型，开始播放音频文件。
