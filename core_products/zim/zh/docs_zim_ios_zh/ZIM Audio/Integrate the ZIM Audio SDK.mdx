---
slug: zim-audio/integrate-the-zim-audio-sdk
---
# 集成 SDK
...
4. 执行 `pod repo update` 命令更新本地索引，确保能安装最新版本的 SDK，最新版本号请参考 [发布日志](https://doc-zh.zego.im/zim-ios/client-sdks/zim-audio-release-notes)。
...
1. 请参考 [下载 SDK](https://doc-zh.zego.im/zim-ios/client-sdks/sdk-downloads)，下载最新版本的 SDK。
...
<Accordion title="Express SDK 和 ZIM Audio SDK混用时，出现音量波动和使用麦克风耗时较长的异常现象如何解决？" defaultOpen="false">
&nbsp;&nbsp;&nbsp;&nbsp;由于 Express SDK 和 ZIM Audio SDK 的配置不一致，会导致 iOS 的 `audio session` 被反复修改从而产生异常现象。
解决此问题需要通过 ZIM Audio SDK 调用 [setAdvancedConfigWithKey](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMAudio#set-advanced-config-with-key-key-value) 将 audio_session_mix_with_others、audio_session_do_nothing 配置项和 Express SDK 对齐。
<Warning title="注意">
如果 Express SDK 设置了 bluetooth_capture_only_voip 配置项， ZIM Audio SDK 的 bluetooth_capture_only_voip 配置项也需要和 Express SDK 对齐。
</Warning>
</Accordion>