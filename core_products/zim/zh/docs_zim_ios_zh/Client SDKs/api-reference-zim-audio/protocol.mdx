...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| license | NSString * | 鉴权信息。开发者如果没有通过服务端 API 获取到的情况下，可以传入空字符串，但后续使用部分需要鉴权的功能时会受限。 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| eventHandler | id\<[ZIMAudioEventHandler](#zimaudioeventhandler)\> | 事件通知回调。传 [null] 则清空已设置的回调。开发者应根据自身业务场景，监听相应的回调。SDK 主要的回调函数都在这里。 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| param | [ZIMAudioANSParam](./class#zimaudioansparam) | ANS 参数，包含 ANS 模式 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| config | [ZIMAudioRecordConfig](./class#zimaudiorecordconfig) | 录制配置 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| config | [ZIMAudioPlayConfig](./class#zimaudioplayconfig) | 播放参数 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| currentDuration | int | 当前录制时长，单位为毫秒 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| currentDuration | int | 当前播放时长，单位为毫秒 |
