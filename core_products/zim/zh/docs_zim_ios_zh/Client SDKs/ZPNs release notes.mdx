...
| 更新图标角标 | 支持用户离线情况下，更新 App 图标角标，提醒用户消息未读数，详情请参考 [更新图标角标](./../Offline%20push%20notifications/Best%20Practices/Modify%20a%20notification%20badge.mdx)。 | <ul><li>[setBadge](@setBadge)</li><li>[ZIMPushConfig > enableBadge](@enableBadge-ZIMPushConfig)</li><li>[ZIMPushConfig > badgeIncrement](@badgeIncrement-ZIMPushConfig)</li></ul> |
...
| 注销推送 | 支持注销离线推送，关闭推送通知。 | [unregisterAPNs](@unregisterAPNs) |
...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
|-------|-------|-------|
| 注册 APNs 推送 | 在开发者正确配置证书和工程信息后，调用注册 APNs 推送的接口，使系统生成 deviceToken，并将 deviceToken 传入 SDK 绑定即可使用。 | <ul><li>[registerAPNs](@registerAPNs)</li><li>[setDeviceToken](@setDeviceToken)</li><li>[onRegistered](@onRegistered)</li></ul> |
