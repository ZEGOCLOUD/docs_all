...
[onMessageReactionsChanged](@onMessageReactionsChanged) 回调会在客户端调用 [addMessageReaction](@addMessageReaction)、[deleteMessageReaction](@deleteMessageReaction) 或 [deleteAllMessageReactions](@deleteAllMessageReactions) 成功后触发。
</Step>

<Step title="onConversationChanged">
[onConversationChanged](@onConversationChanged) 回调会在客户端调用 [deleteConversation](@deleteConversation) 成功后触发。
</Step>

<Step title="onConversationsAllDeleted">
[onConversationsAllDeleted](@onConversationsAllDeleted) 回调会在客户端调用 [deleteAllConversations](@deleteAllConversations) 成功后触发。
</Step>
