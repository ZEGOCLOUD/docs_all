...
请参考 [Apple 推送集成指南](./Integrate%20APNs.mdx)，获取 APNs 推送证书。
...
4. 执行 `pod repo update` 命令更新本地索引，确保能安装最新版本的 SDK，最新版本号请参考 [发布日志](./../Client%20SDKs/ZPNs%20release%20notes.mdx)。
...
1. 请在 [下载](./../Client%20SDKs/SDK%20downloads.mdx) 获取最新的 ZPNs SDK 包。
...
2. 开发者调用 [registerAPNs](@registerAPNs) 注册 APNs，注册成功后，系统会通过 `didRegisterForRemoteNotificationsWithDeviceToken` 代理回调，自动获取到 DeviceToken。
...
在 `didRegisterForRemoteNotificationsWithDeviceToken` 代理回调中，开发者需要通过 [setDeviceToken](@setDeviceToken) 接口，获取到 DeviceToken。
...
3. 发送方调用 [sendGroupMessage](@sendPeerMessage)，传入 “sentConfig”，向群组内的所有用户发送消息。
...
3. 发送方调用 [callInviteWithInvitees](@callInviteWithInvitees)，传入 “ callInviteConfig”，发起呼叫邀请。
...
#### 场景 4：iOS 通知分组 <a id="场景-4ios-通知分组" />
...
若开发者希望某台设备不再接收离线推送，可通过调用 [unregisterAPNs](@unregisterAPNs) 接口注销。注销后，发送弹窗推送、静默推送也将不再生效。
