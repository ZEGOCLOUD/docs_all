...
群组管理功能可应用于办公群、社交群、兴趣群以及粉丝群等场景中，群组成员数量上限请参考 [计费说明](../Content%20Moderation/Instruction.mdx)。
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/ZIM/Common/GroupManagement.png" /></Frame>

### 1 创建群组

客户端 A 登录 ZIM SDK 后，调用 [CreateGroup](@CreateGroup) 接口，创建一个群组，此时 A 就是 `群主`；其他客户端可以根据 A 创建的群组 groupID 加入群组。

开发者可以通过 [ZIMGroupCreatedCallback](@ZIMGroupCreatedCallback)，判断群组是否创建成功。相关错误码请查看 [常见错误码](./../../SDK%20Error%20Codes/ZIM.mdx)。

<Warning title="注意">

- “groupID” 支持开发者自定义规则生成，仅支持数字，英文字符和 {"'!'、'#'、'$'、'%'、'&'、'('、')'、'+'、'-'、':'、';'、'<'、'='、'.'、'>'、'?'、'@'、'['、']'、'^'、'_'、'{'、'}'、'|'、'~'"}，且不能以 ’#‘ 开头；若该字段为空，ZIM 服务器会自动生成。建议开发者将 “groupID” 设置为一个有意义的值，可将其与自己的业务账号系统进行关联。
- 调用 [CreateGroup](@CreateGroup) 接口创建群组后，会直接加入群组，无需再调用 [JoinGroup](@JoinGroup) 接口加入群组。
- 用户创建群组时，该用户即是该群组的“群主”。如果想要转让身份，请参考 [转让群主](./Group%20members.mdx#转让群主)。
- 创建群组后，群主可以通过 [SendMessage](@SendMessage) 接口，向群内发送消息，详情请参考 [收发群组消息](../Messaging/Send%20and%20receive%20messages.mdx)。
</Warning>
...
- 直接加入群组，即主动加入群组：
    ...
    用户直接调用 [JoinGroup](@JoinGroup) 接口，传入 groupID（groupID 必须已经存在，否则会操作失败），直接加入群组。加入成功后，全体群成员（包括自己）都会收到 [OnGroupMemberStateChanged](@OnGroupMemberStateChanged) 的回调通知。

- 由群内成员添加入群组，即被动加入群组。

    该群组内的成员，调用 [InviteUsersIntoGroup](@InviteUsersIntoGroup) 接口，传入 groupID（groupID 必须已经存在，否则会操作失败）、userIDs（需要被添加的用户列表），添加其他用户加入群组。加入成功后，全体群成员（包括自己、被添加的用户）都会收到 [OnGroupMemberStateChanged](@OnGroupMemberStateChanged) 的回调通知。

<Warning title="注意">

- 群内成员调用 [InviteUsersIntoGroup](@InviteUsersIntoGroup) 接口，邀请其他用户加入群组时：
    - 被添加的用户将直接进入群组，无需同意。
    - 被添加的用户 userID，必须是真实存在的（即已经通过 [login](@login) 接口登录注册过），否则会操作失败。
- 加入群组后，成员可以通过 [SendMessage](@SendMessage) 接口，向群内发送消息，详情请参考 [收发群组消息](../Messaging/Send%20and%20receive%20messages.mdx)。
</Warning>
...
- 直接退出群组，即主动退出群组。
    ...
    成员登录后，直接调用 [LeaveGroup](@LeaveGroup) 接口，传入 groupID（groupID 必须已经存在，否则会操作失败），主动退出群组。退出成功后，全体群成员（包括自己）都会收到 [OnGroupMemberStateChanged](@OnGroupMemberStateChanged) 的回调通知。
 
- 群主移除群内成员，即被动退出群组。

    群主调用 [KickGroupMembers](@KickGroupMembers) 接口，传入 groupID（groupID 必须已经存在，否则会操作失败）、userIDs（需要被移除的成员列表），移除这些成员。移除成功后，全体群成员（包括群主自己、被移除的成员）都会收到 [OnGroupMemberStateChanged](@OnGroupMemberStateChanged) 的回调通知。

<Warning title="注意">

- 只有群主可以调用 [KickGroupMembers](@KickGroupMembers) 接口，移除群内成员；且移除成员时，该成员无需登录在线，也无需经过该成员同意，即可直接移除。
...
群主登录后，如果想要解散群组，可以通过 [DismissGroup](@DismissGroup) 接口，解散群组。解散群组成功后，全体群成员都会收到 [OnGroupStateChanged](@OnGroupStateChanged) 回调通知。

<Note title="说明">

- 只有群主才能调用 [DismissGroup](@DismissGroup) 接口，解散群组。
- 所有成员退出群组时，群组将自动解散。
...
用户登录后，如果想要了解自己加入了哪些群组，可以通过 [QueryGroupList](@QueryGroupList) 接口，获取自己已加入的群组列表。
...
用户登录 ZIM SDK 后，如果想要根据条件对已加入的群组进行搜索，可以调用 [SearchLocalGroups](@SearchLocalGroups) 接口，传入 config、callback 搜索群组。

搜索结果将通过 [ZIMGroupsSearchedCallback](@ZIMGroupsSearchedCallback) 回调接口返回。
