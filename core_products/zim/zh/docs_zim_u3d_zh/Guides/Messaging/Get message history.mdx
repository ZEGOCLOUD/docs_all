...
- 历史消息存储天数与套餐版本相关，详情请参考 [计费说明](./../../Introduction/Pricing.mdx) 的 “版本差异”。
</Note>
...
ZIM SDK 支持获取 `单聊`、`群组` 和 `房间` 的会话历史记录，通过 [QueryHistoryMessage](@QueryHistoryMessage) 接口，传入参数 conversationID、config，进行获取。
...
2. 客户端 A 需要获取与 B 的会话记录时：
    ...
    2. 客户端 A 调用 [QueryHistoryMessage](@QueryHistoryMessage) 接口，传入参数 conversationID、config，开始获取。
    3. 获取的结果，将通过 [ZIMMessageQueriedCallback](@ZIMMessageQueriedCallback) 回调接口通知给客户端 A。
