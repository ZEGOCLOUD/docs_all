...
ZIM SDK 支持删除某个会话中指定的消息，或者删除某个会话的全部消息。删除消息分为“删除本地消息记录”和“删除服务端消息记录”，开发者可以通过 [ZIMMessageDeleteConfig](@-ZIMMessageDeleteConfig) 对象，设置删除消息相关的高级属性配置。
...
2. 客户端 A 需要删除与 B 的会话记录时：
    ...
    2. 客户端 A 调用 [DeleteMessages](@DeleteMessages) 接口，传入参数 messageList、config，删除指定的消息。
    3. 删除操作的结果，将通过 [ZIMMessageDeletedCallback](@ZIMMessageDeletedCallback) 回调接口通知给客户端 A。
...
2. 客户端 A 需要删除与 B 的会话记录时：
    1. 客户端 A 首先登录 ZIM SDK。
    2. 客户端 A 调用 [DeleteAllMessage](@DeleteAllMessage) 接口，传入参数 conversationID、conversationType、config，配置删除指定会话的全部消息。
    3. 删除操作的结果，将通过 [ZIMMessageDeletedCallback](@ZIMMessageDeletedCallback) 回调接口通知给客户端 A。
