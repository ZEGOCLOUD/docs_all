...
2. 客户端 B 通过注册 [OnCallInvitationReceived](@OnCallInvitationReceived) 回调接口，接收客户端 A 的邀请通知。
    ...
    - 在 timeout 外登录，请调用 [QueryCallInvitationList](@QueryCallInvitationList) 来 <a href="#查询呼叫邀请列表">查询呼叫邀请列表</a>。
    </Note>
...
ZIM.GetInstance().CallCancel(invitees, callid, config, (string callID, List<ZIMErrorUserInfo> errorUserList,
                ZIMError errorInfo) =>
{
    // 取消呼叫邀请的回调
});
