...
1. [下载](./Client%20SDKs/SDK%20downloads.mdx) 并解压 SDK 压缩包，将 解压后的 ZIMUnity3D 目录下的 “ZIM” 文件夹到拷贝到开发者项目的 "Assets" 目录下，即可集成 SDK ：
...
例如，客户端 A、B 分别调用 [Create](@Create) 接口，传入在 [前提条件](#前提条件) 中获取到的 AppID、AppSign（开发 Web 平台应用时无需使用 “appsign”，为防止暴露，请勿传入），创建了 A、B 的实例：
...
config.appID = (uint)appID; // 请通过 [ZEGO 控制台](https://console.zego.im/) 获取，格式为 123456789，详情请参考控制台的 [项目信息](/console/project-info)
config.appSign = (string)appSign;// 请通过 [ZEGO 控制台](https://console.zego.im/) 获取，格式为"0123456789012345678901234567890123456789012345678901234567890123"，64 个字符，详情请参考控制台的 [项目信息](/console/project-info)
...
详细的接口介绍，请参考 [OnConnectionStateChanged](@OnConnectionStateChanged)、[OnTokenWillExpire](@OnTokenWillExpire)、[OnReceivePeerMessage](@OnReceivePeerMessage)。
...
客户端需要使用各自的用户信息进行登录。调用 [Login](@Login) 接口，传入用户信息 [ZIMUserInfo](@-ZIMUserInfo) 对象，进行登录。
...
客户端 B 登录 ZIM 后，将会通过前面 “3.注册用于接收通知的回调” 中的 [OnReceivePeerMessage](@OnReceivePeerMessage) 接收到消息。
