...
|-----|-----|-----|
|消息表态支持重复表态和统计表态次数| 支持单个用户对同一条消息进行多次相同类型的表态。查询消息表态时，可以获取此消息每种表情的累计表态次数，或者统计每个用户针对该消息每种表情的表态次数。详细请参考[addMessageReaction API](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIM/addMessageReaction.html)。|[addMessageReaction](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIM/addMessageReaction.html)|
...
| 群组定向消息 | ZIM 支持用户在调用服务端接口发送群组消息时，指定对某些群用户是否可见。详情请参考 [群组定向消息](./../Guides/Messaging/group-targeted-message.mdx)。 | [onGroupMessageReceived](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIMEventHandler/onGroupMessageReceived.html) |
...
| 服务端离线消息推送无限频 | 支持开发者在调用服务端接口发送单聊消息和群组消息向国内 Android 厂商进行无限频离线推送。 | [MessageBody 说明 - OfflinePush - PushStrategyId](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIM/MessageBody.html) |
...
[升级到 2.9.0 版本的编译问题，升级前必看 >>](./ZIM upgrade guide.mdx#2.9.0-升级指南)
</Warning>
...
[升级到 2.3.0 版本的编译问题，升级前必看 >>](./ZIM upgrade guide.mdx#2.3.0-升级指南)
</Warning>
