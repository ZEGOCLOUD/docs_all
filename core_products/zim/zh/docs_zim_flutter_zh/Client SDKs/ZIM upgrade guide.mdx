...
ZIM Flutter SDK 2.28.0 版本对修改了部分接口的用法和增加了事件回调的时机。从旧版本升级到 2.28.0 版本时，请您阅读以下指南。
</Warning>
...
```dart 旧版本定义
Future<ZIMMessageReactionAddedResult> addMessageReaction(String reactionType, ZIMMessage message);
```
</CodeGroup>

<CodeGroup>
```dart 新版本用法
final result = await zim.addMessageReaction(reactionType: '❤️', message: message);
```

```dart 旧版本用法
final result = await zim.addMessageReaction('❤️', message);
```
</CodeGroup>
</Step>
...
```dart 旧版本定义
Future<ZIMMessageReactionDeletedResult> deleteMessageReaction(String reactionType, ZIMMessage message);
```
</CodeGroup>

<CodeGroup>
```dart 新版本用法
final result = await zim.deleteMessageReaction(reactionType: '❤️', message: message);
```

```dart 旧版本用法
final result = await zim.deleteMessageReaction('❤️', message);
```
</CodeGroup>
</Step>
...
<CodeGroup>
```dart 新版本用法
final result = await zim.pinMessage(message: message, isPinned: true);
```

```dart 旧版本用法
final result = await zim.pinMessage(message, true);
```
</CodeGroup>
</Step>
...
<Steps>
<Step title="onMessageReactionsChanged">
[onMessageReactionsChanged](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIMEventHandler/onMessageReactionsChanged.html) 现在也会在操作端调用 `addMessageReaction`、`deleteMessageReaction` 或 `deleteAllMessageReactions` 成功后回调，请开发者检查相应业务逻辑在升级 SDK 之后是否产生影响。
</Step>

<Step title="onConversationChanged">
[onConversationChanged] (https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIMEventHandler/onConversationChanged.html) 现在也会在操作端调用 `deleteConversation` 成功后回调，请开发者检查相应业务逻辑在升级 SDK 之后是否产生影响。
</Step>

<Step title="onConversationsAllDeleted">
[onConversationsAllDeleted] (https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIMEventHandler/onConversationsAllDeleted.html) 现在也会在操作端调用 `deleteAllConversations` 成功后回调，请开发者检查相应业务逻辑在升级 SDK 之后是否产生影响。
</Step>
