...
5. （可选）如果您要在 Web 浏览器运行示例源码，还需要在登录页需输入 Token 进行鉴权，才能体验 ZIM。您可以参考控制台的 [临时 Token 生成和 Token 校验](https://doc-zh.zego.im/console/development-assistance/temporary-token) 获取临时 Token。
