...
3. 注册离线推送功能。
    ...
    - 如果您尚不清楚当前所处的 `ZPNsIOSEnvironment`，请填 `ZPNsIOSEnvironment.Automatic`。`Automaic` 可能受 iOS 系统版本影响，若 iOS 系统进行大版本更新后，请留意 [ZPNs 发布日志](./../Client%20SDKs/ZPNs%20release%20notes.mdx)。
    </Note>
...
请参考 [Web 推送集成指南](./Integrate%20FCM%20(for%20web).mdx)，集成 FCM 离线推送 SDK。
...
开发者在通知发送端配置 [ZIMPushConfig](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIMPushConfig-class.html) 的 `threadID` 参数，即可实现 iOS 离线推送的通知分组效果。
