...
// Set up and listen for the callback for receiving messages.
zim.on('messageReceived', function (zim, { messageList, info, conversationID, conversationType }) {
    console.log('messageReceived', messageList, conversationID);
...
For a detailed introduction to the interfaces, please refer to [connectionStateChanged](@connectionStateChanged), [messageReceived](@messageReceived), [tokenWillExpire](@tokenWillExpire).
...
After client B logs in, he will receive client A's message through the callback [on](@on) which is already set in the [messageReceived](@messageReceived) method. 
