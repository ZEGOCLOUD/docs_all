...
| Conversation list sync state callback | When users log in to ZIM service or reconnect after disconnection, they can get the status of synchronizing the latest conversation list from ZIM server through the [onConversationListSyncStateChanged](@onConversationListSyncStateChanged) callback. For details, see [Get conversation list - Listen for conversation list sync state changes](./../Guides/Conversation/Get%20the%20conversation%20list.mdx#listen-for-conversation-list-sync-state-changes). | [onConversationListSyncStateChanged](@onConversationListSyncStateChanged) |
...
| Item  | Description | Method |
|-------|-------|-------|
| Group role | New group role "Administrator" has the upper limit of most group leaders. It can modify the nicknames of ordinary group members, withdraw messages from ordinary group members, kick people, and ban individual group members and specific group roles. <br></br>To learn about group roles and corresponding permissions, please refer to [Manage group members - Set a role for a group member](./../Guides/Group/Group%20members.mdx#set-a-role-for-a-group-member) | [setGroupMemberRole](@setGroupMemberRole). |
...
| Tips Message | Supports converting certain group operations of users in the group (such as creating groups, disbanding groups, etc.) into special types of messages (Tips). After obtaining the operation information through the group message callback, developers can construct and display the description text of the related events on the UI. <br></br>For information on Tips message types and extended information, as well as post-reception processing operations, please refer to [Receive tip messages](./../Guides/Messaging/Receive%20tip%20messages.mdx) | <ul><li>[ZIMMessageType > TIPS](@TIPS-ZIMMessageType)</li><li>[onReceiveGroupMessage](@onReceiveGroupMessage)</li></ul> |
| Custom push rules | Support users to decide the platforms that need to receive offline push in multi-terminal login scenarios, and query the current offline push rules. <br></br>For details on interface calling, please refer to [Set custom push rules](./../Offline%20push%20notifications/Set%20custom%20push%20rules.mdx) | <ul><li>[updateUserOfflinePushRule](@updateUserOfflinePushRule)</li><li >[querySelfUserInfo](@querySelfUserInfo)</li></ul> |
| Message export and import | Supports exporting historical messages from the user's local terminal as a backup, which can be used to migrate chat records when changing devices, or restore deleted messages. <br></br>For details on interface calling, please refer to [Export and import messages](./../Guides/Messaging/Export%20and%20import%20messages.mdx) | <ul><li>[exportLocalMessages](@exportLocalMessages)</li><li> [importLocalMessages](@importLocalMessages)</li></ul> |
| Cache Management | Supports querying the local cache file size of the currently logged in user and clearing the local cache. <br></br>For details on interface calling, please refer to [Cache management](./../Guides/Cache%20management.mdx) | <ul><li>[queryLocalFileCache](@queryLocalFileCache)</li><li>[ clearLocalFileCache](@clearLocalFileCache)</li></ul> |
