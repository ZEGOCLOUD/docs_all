...
|-----|-----|-----|
| Delete for Everyone members | Supports deleting messages for everyone in group chats. After deletion, the message is seamlessly deleted for all group members. For details, refer to [Delete messages - Delete for Everyone](./../Guides/Messaging/Delete%20messages.mdx#delete-for-everyone) | [deleteMessages](@deleteMessages) |
| Community conversation management | Community-type conversations are added to the conversation list, and conversations can be displayed in community mode. For details, refer to [Channel conversation management - Sync channel conversations to the conversation list](./../Guides/community/channel-conversation-management.mdx#sync-channel-conversations-to-the-conversation-list) | <ul><li> [queryConversationList](@queryConversationList)</li><li> [onConversationChanged](@onConversationChanged)</li></ul> |
| Community join review | Supports setting multiple community join modes, with the new "Review before joining" mode. For details, refer to [Community join applications](../Guides/community/community-join-application.mdx) | [updateCommunityJoinMode](@updateCommunityJoinMode) |

**Enhancements**

|Item-25% | Description-40% | Related APIs-25%|
|-----|-----|-----|
| Media message upload optimization | <ul><li>Image messages ([ZIMImageMessage](@-ZIMImageMessage)) support sending files up to 20 MB in size by default.</li> <li>File messages ([ZIMFileMessage](@-ZIMFileMessage)) and video messages ([ZIMVideoMessage](@-ZIMVideoMessage)) support sending files up to 1 GB in size.</li><li>Optional resumable upload capability is added for uploading media files, improving the file upload experience in weak network environments.</li></ul> | [sendMessage](@sendMessage) |
...
ZIM SDK 2.28.0 version adjusts some return parameters and event callback triggering mechanisms. When upgrading from an older version to 2.28.0, please refer to [ZIM upgrade guide](./ZIM%20upgrade%20guide.mdx#2280-upgrade-guide).
</Warning>
...
|Feature-25% | Description-40% | Related APIs-25%|
|-----|-----|-----|
|Message reaction supports repeated reaction and statistics| Supports users to repeatedly express the same type of reaction to the same message. When querying message reactions, you can get the cumulative number of reactions for each emotion of the message, or the number of reactions for each emotion of the message by each user. For details, see [Message reaction](../Guides/Messaging/Respond%20to%20messages%20with%20emoticons.mdx#2-add-an-emoticon-response--repeated-reaction-counting).|[addMessageReaction](@addMessageReaction)|
|Query history message supports configuring to synchronize server data first |[queryHistoryMessage](@queryHistoryMessage) adds a new `isPreferQueryFromServer` parameter, which is used to set whether to synchronize server data first (default to use local cache first).|[queryHistoryMessage](@queryHistoryMessage)|

**Enhancements**
|Item-25% | Description-40% | Related APIs-25%|
|-----|-----|-----|
|Optimize delete conversation interface| When deleting a conversation, the client that initiates the deletion will trigger the conversation change related callbacks. For details, see [Delete conversation](../Guides/Conversation/Delete%20conversations.mdx#delete-a-conversation).|<ul><li>[deleteConversation](@deleteConversation)</li><li>[onConversationChanged](@onConversationChanged)</li><li>[deleteAllConversations](@deleteAllConversations)</li><li>[onConversationsAllDeleted](@onConversationsAllDeleted)</li></ul>|
|Optimize message top feature| When the number of messages pinned exceeds the maximum number, the feature to set whether to overwrite the current pinned list is supported. |[pinMessage](@pinMessage)|
...
|-----|-----|-----|
|Message pinned to top| Users can pin messages in a conversation to the top of the conversation so that all users in the conversation can quickly view important messages. For details, please refer to [Message pinned to top](../Guides/Messaging/pin-message.mdx). | <ul><li>[onMessagePinStatusChanged](@onMessagePinStatusChanged)</li><li>[pinMessage](@pinMessage)</li><li>[queryPinnedMessageList](@queryPinnedMessageList)</li></ul> |
...
1. Fixed the issue where the [onConversationSyncStateChanged](@onConversationSyncStateChanged) event notification becomes invalid after "logging out and logging back in" within the same instance lifecycle.
...
| Group targeted messages | ZIM supports users to specify visibility for certain group users when calling [ZIM server API](/zim-server/messaging/send-group-messages#targetoption) to send group messages. For details, see [Group targeted messages](./../Guides/Messaging/group-targeted-message.mdx). | [onGroupMessageReceived](@onGroupMessageReceived) |
| Conversation list sync state callback | When users log in to ZIM service or reconnect after disconnection, they can get the status of synchronizing the latest conversation list from ZIM server through the [onConversationSyncStateChanged](@onConversationSyncStateChanged) callback. For details, see [Get conversation list - Listen for conversation list sync state changes](./../Guides/Conversation/Get%20the%20conversation%20list.mdx#listen-for-conversation-list-sync-state-changes). | [onConversationSyncStateChanged](@onConversationSyncStateChanged) |
...
| Message Resending | When a user encounters network disconnection while sending a message, if the network recovers within 30 seconds, ZIM SDK will automatically resend the message. If the network recovers after 30 seconds, users can manually resend the message. For details, see [Send and receive messages - Message retry](./../Guides/Messaging/Send%20and%20receive%20messages.mdx#message-retry). | [ZIMMessageSendConfig > isRetrySend](@isRetrySend-ZIMMessageSendConfig) |
...
| -----  | ---- | 
| downloadMediaFile and related callbacks | <ul><li>The original `downloadMediaFile` API is deprecated. Please use the new [downloadMediaFile](@downloadMediaFile) instead. The updated [downloadMediaFile](@downloadMediaFile) adds a new `config` parameter, which allows specifying the download of individual media content in multi-item messages.</li><li>In the callback [ZIMMediaDownloadedCallback](@-ZIMMediaDownloadedCallback), the parameter `message` in both [onMediaDownloaded](@onMediaDownloaded) and [onMediaDownloadingProgress]() has changed from `ZIMMediaMessage` to `ZIMMessage`.</li></ul> |
| [sendMessage](@sendMessage) | The `sendMediaMessage` and the old `sendMessage` methods are deprecated. A new [sendMessage](@sendMessage) method is introduced, which supports sending messages of any type. |
| [ZIMMessageSentFullCallback](@-ZIMMessageSentFullCallback) | In [ZIMMessageSentFullCallback](@-ZIMMessageSentFullCallback), the `message` parameter in [onMediaUploadingProgress](@onMediaUploadingProgress-ZIMMessageSentFullCallback) has been changed from `ZIMMessage` to `ZIMMediaMessage` to ensure that only media messages trigger callback notifications. |
...
| Item | Description | Method |
| -----  | ---- | ----- |
| Deprecation of callbacks for receiving message in one-to-one, room, and group chats | The callbacks of `onReceivePeerMessage`, `onReceiveGroupMessage` and `onReceiveRoomMessage` have been deprecated, please use the following callbacks instead: [onPeerMessageReceived](@onPeerMessageReceived), [onGroupMessageReceived](@onGroupMessageReceived), and [onRoomMessageReceived](@onRoomMessageReceived).<Note title="Note">Please refer to [ZIM Upgrade Guide](./ZIM%20upgrade%20guide.mdx#2180-upgrade-guide) for details on callback changes.</Note>| <ul><li>[onPeerMessageReceived](@onPeerMessageReceived)</li><li>[onGroupMessageReceived](@onGroupMessageReceived)</li><li>[onRoomMessageReceived](@onRoomMessageReceived)</li></ul> |
...
| Query Conversation List | Supports filtering conversation list by mark, conversation type, and whether it contains unread messages. For more details, please refer to [Mark Conversations](./../Guides/Conversation/Mark%20conversations.mdx). | [queryConversationList](@queryConversationList) |
...
| Get Specific Message List | Supports querying a specific list of historical messages, suitable for scenarios where users need to understand the context of a certain message. For more details, please refer to [Get message history](./../Guides/Messaging/Get%20message%20history.mdx) | [queryMessages](@queryMessages) |
...
| Item  | Description | Method |
|-------|-------|-------|
| Group role | New group role "Administrator" has the upper limit of most group leaders. It can modify the nicknames of ordinary group members, withdraw messages from ordinary group members, kick people, and ban individual group members and specific group roles. <br />To learn about group roles and corresponding permissions, please refer to [Manage group members - Set a role for a group member](./../Guides/Group/Group%20members.mdx#set-a-role-for-a-group-member) | [setGroupMemberRole](@setGroupMemberRole). |
...
| Tips Message | Supports converting certain group operations of users in the group (such as creating groups, disbanding groups, etc.) into special types of messages (Tips). After obtaining the operation information through the group message callback, developers can construct and display the description text of the related events on the UI. <br />For information on Tips message types and extended information, as well as post-reception processing operations, please refer to [Receive tip messages](./../Guides/Messaging/Receive%20tip%20messages.mdx) | <ul><li>[ZIMMessageType > TIPS](@tips)</li><li>[onReceiveGroupMessage]()</li></ul> |
...
| Item | Description | Method |
| -----  | ---- | ----- |
| Optimized ZIMMessage and ZIMConversation data structures | ZIM has implemented serializable implementations of ZIMMessage and ZIMConversation, making it easier to transfer values between different Activity. | <ul><li>[ZIMMessage](@-ZIMMessage)</li><li>[ZIMConversation](@-ZIMConversation)</li></ul> |
...
| Feature  | Description | Method |
|-------|-------|-------|
| Multi-device login | The "multi-device login" strategy supports configuring the logic for kicking each other out. Currently, it only supports kicking out between Android and iOS devices, as well as between Windows and Mac devices. | [login](@login) |
| Offline login | Supports users to login to the IM service and access local SDK data while being offline. | [login](@login)  |
...
| Call Invitation | Add callback [onCallInvitationCreated](@onCallInvitationCreated). The initiator of the call invitation can know that the call invitation has been created by listening to this callback. | [onCallInvitationCreated](@onCallInvitationCreated) |
...
| Added user avatar field to user information related classes | [ZIMUserInfo](@-ZIMUserInfo) and [ZIMGroupMemberInfo](@-ZIMGroupMemberInfo) added `userAvatarUrl` field, which is used to set or describe the user avatar address. <p>Warning: The user avatar field for room members is currently not supported. This means that the `userAvatarUrl` in the `ZIMUserInfo` obtained through [queryRoomMemberList](@queryRoomMemberList) and [queryRoomMembers](@queryRoomMembers) will be empty.</p> | <ul><li>[ZIMUserInfo](@-ZIMUserInfo)</li><li>[ZIMGroupMemberInfo](@-ZIMGroupMemberInfo)</li></ul> |
...
| Item | Description | Method |
| -----  | ---- | ----- |
| Abandon the `loginWithUserInfo` interface | Abandon the old login interface and add a new login interface [login](@login). The new version of the login interface supports more configurations through [ZIMLoginConfig](@-ZIMLoginConfig), such as whether to use Token authentication and whether to log in offline.| [login](@login) |
...
| Item | Description | Method |
| -----  | ---- | ----- |
| Deprecated system message types | [ZIMMessageType](@-ZIMMessageType) (value 30) have been deprecated. Developers are advised to use more versatile custom messages (value 200) instead. | [ZIMMessageTypeCustom](https://www.zegocloud.com/article/api?doc=zim_API~java_android~enum~ZIMMessageType#custom) |
...
| Added the callback for the message sending status | You can refine the logic of message sending status by listening for this callback. You can design UI logic to notify users when the message status changes. | [onMessageSentStatusChanged](@onMessageSentStatusChanged) |
...
| Feature  | Description | Method |
|-------|-------|-------|
|Message recall|  Sent messages can be recalled within 2 minutes by default. This is only supported in one-on-one chats and group chats. <p>Note: To customize the recall time, please contact ZEGOCLOUD technical support.</p>| <ul><li>[revokeMessage](@revokeMessage)</li><li>[onMessageRevoked](@onMessageRevoked)</li></ul>|
...
| Method | Description | Change version |
| :-------- | :------- | :----- |
| [login](@login) |  A new [login](@login) method without a Token parameter is added. When you use the "AppSign" authentication mode, you can directly use this method to log in to In-App Chat.  | 2.4.0 |
| [sendMessage](@sendMessage) |  New method to send messages, support one-to-one, in-room, and group messages. | 2.4.0 |
| [onMessageAttached](@onMessageAttached-ZIMMessageSentCallback) |A new [onMessageAttached](@onMessageAttached-ZIMMessageSentCallback) callback is added to the [ZIMMediaMessageSentCallback](@-ZIMMediaMessageSentCallback), used to listen for message object before sending information, otherwise, the compiler cannot pass.  | 2.4.0 |
...
| Optimized the ZPNs SDK adaption | ZPNs SDK can be used on Android 12.0. | - |
...
| Feature  | Description | Method |
|-------|-------|-------|
| Send & Receive rich media content| Supports sending and receiving messages of different rich media types, such as images, audio, video, and files. | <ul><li>[`sendMediaMessage`]()</li><li>[`downloadMediaFile`](@downloadMediaFile)</li></ul>|
...
| Call invitation | <ul><li>Supports sending and responding to call invitations.</li><li>Suitbale for multiple scenarios, such as, video calls, send seat-taking invitation in live audio room, and more.</li></ul>| <ul><li>[`callInvite`](@callInvite)</li><li>[`callCancel`](@callCancel)</li><li>[`callAccept`](@callAccept)</li><li>[`callReject`](@callReject)</li></ul> |
...
<table>
  ...
  <tr>
    <td>Enhanced API method name</td>
    <td><b>The method used to query room members:</b> Changed to <b>queryRoomMemberList</b> from <b>queryRoomMember</b>.</td>
    <td><a target="_blank" href="/article/api?doc=zim_API~java_android~class~ZIM#query-room-member-list">queryRoomMemberList</a></td>
  </tr>
  <tr>
    ...
    <td><ul><li><a target="_blank" href="/article/api?doc=zim_API~java_android~class~ZIM#send-peer-message">sendPeerMessage</a></li><li><a target="_blank" href="/article/api?doc=zim_API~java_android~class~ZIM#send-room-message">sendRoomMessage</a></li></ul></td>
  </tr>
  <tr>
    <td>A <b>roomID</b> parameter is added for some method callbacks. It is convenient for you to get the roomID from which the result of the current operation comes when calling the relevant method.</td>
    <td><ul><li><a target="_blank" href="/article/api?doc=zim_API~java_android~interface~ZIMRoomLeftCallback">ZIMRoomLeftCallback</a></li><li><a target="_blank" href="/article/api?doc=zim_API~java_android~interface~ZIMRoomMemberQueriedCallback">ZIMRoomMemberQueriedCallback</a></li><li><a target="_blank" href="/article/api?doc=zim_API~java_android~interface~ZIMRoomOnlineMemberCountQueriedCallback">ZIMRoomOnlineMemberCountQueriedCallback</a></li><li><a target="_blank" href="/article/api?doc=zim_API~java_android~interface~ZIMRoomAttributesBatchOperatedCallback">ZIMRoomAttributesBatchOperatedCallback</a></li><li><a target="_blank" href="/article/api?doc=zim_API~java_android~interface~ZIMRoomAttributesQueriedCallback">ZIMRoomAttributesQueriedCallback</a></li></ul></td>
  </tr>
  <tr>
    ...
    <td><a target="_blank" href="/article/api?doc=zim_API~java_android~interface~ZIMRoomAttributesOperatedCallback">ZIMRoomAttributesOperatedCallback</a></td>
  </tr>
</tbody></table>
