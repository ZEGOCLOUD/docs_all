...
Based on the conversation type (one-to-one chat, voice chatroom, or group chat), listen for the [onMessageReceived](@onMessageReceived) callback on the receiving client to receive the audio message notification and then call the [downloadMediaFile](@downloadMediaFile) method to download the audio file to the local database.

Below is the sample code for receiving and downloading an audio message in a one-to-one chat.

```java
// Sample codes for receiving an audio message
 @Override
