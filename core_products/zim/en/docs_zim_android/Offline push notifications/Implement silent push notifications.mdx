...
3. Create a class that inherits from [ZPNsMessageReceiver](@-ZPNsMessageReceiver) and declare it as the broadcast receiver for ZPNs. Refer to [Implementing Offline Push - Set up the offline push notification](./Implement%20offline%20push%20notification.mdx#set-up-the-offline-push-notification) for the declaration details. 
