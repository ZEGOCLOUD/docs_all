...
The newly released **In-app Chat 2.0.0** has been upgraded considerably (for detailed changes, see [Release notes](./../Client%20SDKs/ZIM%20release%20notes.mdx)). If the version of the SDK you are using is **earlier than 2.0.0**, for its error codes, see [Error codes for In-app Chat 1.x.x](https://docs.zegocloud.com/article/14297).
...
If the `errorInfo` field exists in the result after the method is called, which indicates that an error occurs. And the code will be like this:
```dart
...
| 6000021 | Blacklist restriction failed. | Handle the client UI behavior according to the normal blacklist logic. |
...
| 6000208 | The message is not pinned. | Check the current message pinning status. |
...
| 6000213 | Unsupported image/audio/video format. Please check the format first. For more details, refer to the [Message types](./../Guides/Messaging/Send%20and%20receive%20messages.mdx#message-types). |
| 6000214 | File size error. Please check the size first. For more about the size limit, refer to the [Message types](./../Guides/Messaging/Send%20and%20receive%20messages.mdx#message-types).|
| 6000215 | Audio duration error. Please check the duration first. For more about the duration limit, refer to the [Message types](./../Guides/Messaging/Send%20and%20receive%20messages.mdx#message-types). |
...
| 6000230 | The message was intercepted by the developer's server through the [Callback on message not sent yet](/zim-server/callbacks/message-not-sent-yet). No processing required.|
...
| 6000251 | Connection error occurred when the network is interrupted during upload/download. | Check the network, and resend or download the media/rich media message after the network is restored. |
| 6000252 | The URL of the upload/download is invalid. | Please provide logs to ZEGOCLOUD technical support for troubleshooting. |
| 6000253 | Certificate verification error. | Please provide logs to ZEGOCLOUD technical support for troubleshooting. |
| 6000254 | Client machine error. | This error code is usually a permission issue, disk space issue, etc. Please check the error message for troubleshooting suggestions. |
...
| 6000287 | The number of pinned messages exceeds the limit. | Check the number of pinned messages in the current conversation. |
...
| 6000310 | Failed to query room members. | Check the reason for the query failure based on the error message. |
| 6000311 | Completely failed to query room members. | Check the reason for the query failure based on the error message. |
...
| Return code | Possible cause and solutions |
|--|--|
| 6000288 | When deleting a message for everyone, the message has exceeded the valid deletion period. Please confirm whether the message to be deleted is within the validity period. The default validity period is 24 hours. To adjust it, contact ZEGO technical support. |
| 6000285 | When deleting a message for everyone in a one-to-one conversation, an attempt is made to delete a message sent by another user. In one-to-one conversations, only messages sent by yourself can be deleted for everyone. Check in advance in your code whether the current user is the message sender. |
| 6000541 | When deleting a message for everyone in a group conversation, an attempt is made to delete a message sent by a member whose role permission is higher than or equal to that of the operator. Use queryGroupMemberInfo to confirm the roles of the operator and the message sender in the group in advance. The group owner can delete messages sent by admins and regular members, and admins can delete messages sent by regular members. |
...
| Return code | Possible cause and solutions |
|--|--|
| 6000205 | General error for message editing failure. Please check the error message or contact ZEGOCLOUD technical support. |
| 6000285 | The user attempts to edit a message sent by another user. Check in the code whether the current user is the message sender in advance, allowing only the editing of messages sent by themselves. |
| 6000286 | Exceeded editing time limit. Please check your code to ensure that only messages within 24 hours can be edited. |
