...
# Displaying Messages in the App

---

## Overview

This article describes how to use the ZIM SDK to display messages in your app.
...
### Get message data and render it to the chat interface

1. Query historical messages displayed in the chat interface.

Call the {getPlatformData(props,queryHistoryMessageMap)} interface to query the historical messages sent by both parties from new to old.
...
2. Listen for real-time messages received and display them in the chat interface.

Listen to {getPlatformData(props,peerMessageReceivedMap)} to get real-time chat messages and update them to the chat interface.
...
```
:::
3. When sending messages, display them in the chat page and listen for the status of message sending.

Listen to {getPlatformData(props,messageSentStatusChangedMap)} to get the change of the sending status of the message sent by the current user (sending, sending successfully, sending failed), and update the UI.
