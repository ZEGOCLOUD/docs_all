...
Listen to user status update events, and the online status changes of subscribed users will be returned through {getPlatformData(props,onUserStatusUpdatedMap)}.
...
Use {getPlatformData(props,subscribeUsersStatusMap)} to subscribe to users of interest.
...
Use {getPlatformData(props,querySubscribedUserStatusListMap)} to query user subscription information.
