...
export const onMessageReceivedMap = {
  ...
  'iOS': <a href='https://docs.zegocloud.com/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-message-received-with-result' target='_blank'>messageReceived</a>,
  'mac': <a href='https://docs.zegocloud.com/article/api?doc=zim_API~objective-c_macos~protocol~ZIMEventHandler#zim-message-received-with-result' target='_blank'>messageReceived</a>,
  "Flutter": <a href='https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIMEventHandler/onMessageReceived.html' target='_blank'>onMessageReceived</a>,
}
...
export const ZIMMessageRepliedInfoStateDeletedMap = {
   'Android': <code>ZIMMessageRepliedInfoState.DELETED</code>,
   'iOS,mac': <code>ZIMMessageRepliedInfoState.ZIMMessageRepliedInfoStateDeleted</code>,
   "window": <code>ZIMMessageRepliedInfoState.ZIM_MESSAGE_REPLIED_INFO_STATE_DELETED</code>,
   "Web": <code>ZIMMessageRepliedInfoState.Deleted</code>, 
   "Flutter": <code>ZIMMessageRepliedInfoState.deleted</code>, 
}
export const ZIMMessageRepliedInfoStateNotFoundMap = {
   'Android': <code>ZIMMessageRepliedInfoState.NOTFOUND</code>,
   'iOS,mac': <code>ZIMMessageRepliedInfoState.ZIMMessageRepliedInfoStateNotFound</code>,
   "window": <code>ZIMMessageRepliedInfoState.ZIM_MESSAGE_REPLIED_INFO_STATE_NOT_FOUND</code>,
   "Web": <code>ZIMMessageRepliedInfoState.NotFound</code>, 
   "Flutter": <code>ZIMMessageRepliedInfoState.notFound</code>, 
}
...
<Frame width="auto" height="auto">
  <img src="https://doc-media.zego.im/docuo/26df1fb401.jpg"/>
</Frame>
...
After logging in to ZIM, users can listen to the {getPlatformData(props,onMessageReceivedMap)} callback to receive new messages from one-on-one and group conversations, or call the {getPlatformData(props,queryHistoryMessageMap)} interface to fetch historical messages.

At this point, users can choose a message to reply to by using that message as the `toOriginalMessage` parameter and constructing a new message as the `message` parameter. Then, call the {getPlatformData(props,replyMessageMap)} interface.

<Note title="Note">

Only the following types are supported for the `toOriginalMessage` and `message` parameters:
- Text message: {getPlatformData(props,ZIMTextMessageMap)}
- Image message: {getPlatformData(props,ZIMImageMessageMap)}
- File message: {getPlatformData(props,ZIMFileMessageMap)}
- Audio message: {getPlatformData(props,ZIMAudioMessageMap)}
- Video message: {getPlatformData(props,ZIMVideoMessageMap)}
- Multi-item message: {getPlatformData(props,ZIMMultipleMessageMap)}
- Combined message: {getPlatformData(props,ZIMCombineMessageMap)}
- Custom message: {getPlatformData(props,ZIMCustomMessageMap)}
</Note>
...
- {getPlatformData(props,onMessageAttachedMap)}: Before sending a reply, you can obtain a temporary ZIMMessage to add some business  logic.
- {getPlatformData(props,onMediaUploadingProgressMap)}: When sending a message containing rich media, this callback provides updates on the progress of file uploading.
</div>
:::
...
// Reply to a message, where the toOriginalMessage parameter refers to the source message obtained through queryHistoryMessage or peerMessageReceived and receiveGroupMessage.
...
// Reply to a message, where the toOriginalMessage parameter refers to the source message obtained through queryHistoryMessage or onMessageReceived.
ZIM::getInstance()->replyMessage(textMessage, toOriginalMessage, config, notification, [=](const std::shared_ptr<ZIMMessage> &message, const ZIMError &errorInfo) {
    // You can use this callback to listen for whether the message was sent successfully.
});
```
:::
...
// The toOriginalMessage parameter refers to the source message obtained through queryHistoryMessage or onMessageReceived.
...
When receiving new messages in one-on-one and group conversations through the {getPlatformData(props,onMessageReceivedMap)} callback, you need to determine if the message has {getPlatformData(props,repliedInfoMap)} (basic information of the source message):
...
:::if{props.platform="window"}
```cpp
// Register ZIMEventHandler callback
...
// replyMessage can be a root message or any reply, generally obtained through the queryHistoryMessage interface or onMessageReceived
...
// replyMessage can be a root message or any reply, generally obtained through the queryHistoryMessage interface or peerMessageReceived and GroupMessageReceived
...
- Since the `message` in `rootRepliedInfo` is a nullable object, when `state` is  {getPlatformData(props,ZIMMessageRepliedInfoStateDeletedMap)}, it means the root message has been deleted. At this time, developers should prompt "The root message has been deleted" on the UI. When `state` is {getPlatformData(props,ZIMMessageRepliedInfoStateNotFoundMap)}, it means that the root message cannot be found anymore. The possible reasons are that the message  exceeded the server storage time, or the group users query the reply list after joining the group but the root message was sent before they joined the group, so the message may not be able to be required anymore. At this time, developers should prompt "The root message cannot be located" or other informative statements on the UI.
</Note>
