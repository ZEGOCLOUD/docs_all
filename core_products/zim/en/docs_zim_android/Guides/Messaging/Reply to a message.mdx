...
<Frame width="auto" height="auto">
  <img src="https://media-resource.spreading.io/docuo/workspace564/27e54a759d23575969552654cb45bf89/26df1fb401.jpg"/>
</Frame>
...
After logging in to ZIM, users can listen to the {getPlatformData(props,onPeerMessageReceivedMap)} and {getPlatformData(props,onGroupMessageReceivedMap)} callback to receive new messages from one-on-one and group conversations, or call the {getPlatformData(props,queryHistoryMessageMap)} interface to fetch historical messages.
...
// Reply to a message, where the toOriginalMessage parameter refers to the source message obtained through queryHistoryMessage or onPeerMessageReceived and onReceiveGroupMessage.
...
// The toOriginalMessage parameter refers to the source message obtained through queryHistoryMessage or onPeerMessageReceived and onReceiveGroupMessage.
...
When receiving new messages in one-on-one and group conversations through the {getPlatformData(props,onPeerMessageReceivedMap)} and {getPlatformData(props,onGroupMessageReceivedMap)} callbacks, you need to determine if the message has {getPlatformData(props,repliedInfoMap)} (basic information of the source message):
...
// replyMessage can be a root message or any reply, generally obtained through the queryHistoryMessage interface or onPeerMessageReceived and onGroupMessageReceived
...
// replyMessage can be a root message or any reply, generally obtained through the queryHistoryMessage interface or onPeerMessageReceived and onPeerMessageReceived
