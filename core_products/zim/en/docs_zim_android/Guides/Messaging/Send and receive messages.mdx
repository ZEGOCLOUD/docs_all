...
:::if{props.platform="Web|rn"}
<CodeGroup>
```javascript title="Sample Code" {7,17}
// Resend a text message in a one-to-one conversation

var toConversationID = ''; // Opponent userID
var conversationType = 0; // Conversation type, value for One-on-one: 0, Room: 1, Group: 2
var config = { 
...
var messageTextObj; // Obtain the failed message by the queryHistoryMessage interface
