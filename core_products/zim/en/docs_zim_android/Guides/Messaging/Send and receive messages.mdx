...
<Note title="Description">To use this feature, please integrate ZIM SDK version 2.20.0 or higher.</Note>

When a user sends a message and the network disconnects:
...
When automatic resend fails, the user can manually resend the message after the network is restored. At this time, call the {getPlatformData(props,sendMessageMap)} interface again, re-enter the failed message object, and set the parameter `config.isRetrySend` to true.
...
<Note title="Description">When the ZIM SDK automatically resends a message, if the user attempts a manual resend simultaneously, the interface call will fail, and the ZIM SDK will indicate an incorrect parameter error.</Note>

:::if{props.platform=undefined}
<CodeGroup>
```java title="Example Code" {10,14}
// Resend a single chat `Text` message
String toConversationID = "xxxx1";

ZIMTextMessage zimMessage; // Obtain the failed message from the queryHistoryMessage interface

ZIMMessageSendConfig config = new ZIMMessageSendConfig();
// Set message priority
config.priority = ZIMMessagePriority.LOW;
// Must be set to true, indicating that this message is being resent
...
zim.sendMessage(zimMessage, toConversationID, type, config, new ZIMMessageSentFullCallback() {
    @Override
    public void onMessageAttached(ZIMMessage zimMessage) {
        // Developers can use this callback to monitor whether the message has started preparing for sending. Only messages that pass local basic parameter checks will trigger this callback; otherwise, errors will be thrown through the onMessageSent callback.
    }
...
:::if{props.platform="iOS|mac"}
<CodeGroup>
```objc title="Example Code" {10,19}
// Resend a single chat `Text` message
NSString *toConversationID = @"xxxx1";

ZIMTextMessage *textMessage; // Obtain the failed message from the queryHistoryMessage interface
...
ZIMMessageSendNotification *notification = [[ZIMMessageSendNotification alloc] init];
notification.onMessageAttached = ^(ZIMMessage * _Nonnull message) {
    // Callback before sending, clients can obtain a temporary object here. This object is the same as the zimMessage object created by the developer. Developers can use this feature to implement some business logic, such as pre-displaying UI.
};
...
:::if{props.platform="win"}
<CodeGroup>
```c++ title="Example Code" {9,16}
// Resend a single `Text` message

std::shared_ptr<zim::ZIMTextMessage> message; // Obtain the failed message from the queryHistoryMessage interface

// Set message priority
zim::ZIMMessageSendConfig config;
config.priority = zim::ZIM_MESSAGE_PRIORITY_LOW;
// Needs to be set to true, indicating that this message is being resent
config.isRetrySend = true;
...
:::
:::if{props.platform="Web|rn|miniprogram"}
<CodeGroup>
```javascript title="Example Code" {7,17}
// Resend a single `Text` message

var toConversationID = ''; // Opponent userID
var conversationType = 0; // Conversation type, value for Single Chat: 0, Room: 1, Group: 2
var config = { 
    priority: 1, // Set message priority, values Low: 1 (default), Medium: 2, High: 3
    isRetrySend: true, // Needs to be set to true, indicating that this message is being resent
};
...
var messageTextObj; // Obtain the failed message from the queryHistoryMessage interface
...
ZIMMessageSendNotification notification = ZIMMessageSendNotification(onMessageAttached: (message){
    // Callback before sending, developers can obtain a temporary object here. This object is the same as the zimMessage object created by the developer. Developers can use this feature for some business logic, such as pre-displaying UI.
});
