...
### Send messages that do not contribute to the unread count
...
- {getPlatformData2(props,disableUnreadMessageCountMap)}：Indicates whether the message is a message that does not contribute to the unread count.

<Warning title="Note">

Only ZIM SDK version 2.23.0 or higher supports sending messages that do not contribute to the unread count.
</Warning>

:::if{props.platform=undefined}
```java {7,8}
// The following is an example code for sending messages that do not contribute to the unread count in a one-to-one conversation:

ZIMMessageSendConfig config = new ZIMMessageSendConfig();
// Set the message priority
config.priority = ZIMMessagePriority.LOW;

// Set the message to not contribute to the unread count
...
const config: ZIMMessageSendConfig = { 
    ...
    disableUnreadMessageCount: true, // Set the message to not contribute to the unread count
};
...
### Receive messages that do not contribute to the unread count
