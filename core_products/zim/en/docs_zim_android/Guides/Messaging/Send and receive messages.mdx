...
import {
    ...
    sendMessageMap,
    setEventHandlerMap,
    ZIMCombineMessageDetailQueriedCallbackMap,
    queryCombineMessageDetailMap,
    disableUnreadMessageCountMap,
...
<Note title="Note">
For other business needs, please refer to [Get message history](./Get%20message%20history.mdx), [Delete messages](./Delete%20messages.mdx), etc.
</Note>
...
<Frame width="512" height="auto" caption=""><img src="https://doc-media.zego.im/sdk-doc/Pics/In_app_Chat/14226.png" /></Frame>

1. Client A and Client B create their own ZIM SDK instances, and set up an event handler {getPlatformData(props,setEventHandlerMap)} to listen for the callback on the result of message sending {getPlatformData(props,messageReceivedMap)}.
...
4. Client B listens for the {getPlatformData(props,messageReceivedMap)} callback to receive Client A's messages.
...
pushConfig.payload = "Extension info of the offline notification";
...
pushConfig.title = "Offline notification title";
pushConfig.content = "Offline notification content";
pushConfig.payload = "Extension info of the offline notificatio";
...
zim.on('messageReceived', function (zim, result) {
    ...
    console.log(messageList, info, conversationID, conversationType);
});
```
:::
:::if{props.platform="flutter"}
...
ZIMEventHandler().onMessageReceived = (ZIM zim, ZIMMessageReceivedEventResult result) {
    List<ZIMMessage> messageList = result.messageList;
    ZIMConversationType conversationType = result.conversationType;
    // Triggered when receiving messages.
};
```
:::
...
When sending messages by calling {getPlatformData(props,sendMessageMap)}, you can set the message to not contribute to the unread count by using the following parameters:
- {getPlatformData2(props,disableUnreadMessageCountMap)}：
...
// Set the message to not contribute to the unread message number
config.disableUnreadMessageCount = true;

// Send a one-to-one message as an example
ZIMConversationType type = ZIMConversationType.Peer;

zim.sendMessage(message, "conv_id", type, config, new ZIMMessageSentFullCallback() {
    @Override
    public void onMessageAttached(ZIMMessage zimMessage) {
        // Developers can use this callback to monitor whether the message starts preparing for sending. Only messages that pass the local basic parameter check will trigger this callback; otherwise, an error is thrown through the onMessageSent callback.               
    }
...
try {
    ...
    onError.code;// Handle the error code according to the error code table
...
The callback interface for receiving messages that do not contribute to the unread count is the same as the callback interface for receiving normal messages. Please refer to [Send & Receive messages - Receive messages](#receive-messages) for the specific interface.

After receiving the message, developers can implement the corresponding functions based on the business logic.

## Resend messages

<Note title="Note">To use this feature, please integrate ZIM SDK version 2.20.0 or higher.</Note>
...
When automatic resend fails, the user can manually resend the message after the network is restored. At this point, call the {getPlatformData(props,sendMessageMap)} interface again, re-enter the failed message object, and set the parameter `config.isRetrySend` to true.
...
<Note title="Note">When the ZIM SDK automatically resends a message, if the user simultaneously attempts a manual resend, the interface call will fail, and the ZIM SDK will prompt a parameter error.</Note>

:::if{props.platform=undefined}
<CodeGroup>
```java title="Sample Code" {10,14}
// Resend a text message in a one-to-one conversation 
String toConversationID = "xxxx1";

ZIMTextMessage zimMessage; // Obtain the failed message by the queryHistoryMessage interface

ZIMMessageSendConfig config = new ZIMMessageSendConfig();
// Set the message priority
config.priority = ZIMMessagePriority.LOW;
// Needs to be true, indicating that this message is resent
...
zim.sendMessage(zimMessage, toConversationID, type, config, new ZIMMessageSentFullCallback() {
    @Override
    public void onMessageAttached(ZIMMessage zimMessage) {
        // Developers can use this callback to monitor whether the message starts preparing for sending. Only messages that pass local basic parameter checks will trigger this callback; otherwise, errors are thrown through the onMessageSent callback.
    }
...
:::if{props.platform="iOS|mac"}
<CodeGroup>
```objc title="Sample Code" {10,19}
// Resend a text message in a one-to-one conversation 
NSString *toConversationID = @"xxxx1";

ZIMTextMessage *textMessage; // Obtain the failed message by the queryHistoryMessage interface
...
ZIMMessageSendNotification *notification = [[ZIMMessageSendNotification alloc] init];
notification.onMessageAttached = ^(ZIMMessage * _Nonnull message) {
// The callback before sending, where customers can obtain a temporary object. This object is the same as the zimMessage object created by the developer. Developers can use this feature to implement some business logic, such as pre-displaying UI, etc.};
...
:::if{props.platform="win"}
<CodeGroup>
```cpp title="Sample Code" {9,16}
// Resend a text message in a one-to-one conversation 
std::shared_ptr<zim::ZIMTextMessage> message; // Obtain the failed message by the queryHistoryMessage interface
...
:::if{props.platform="Web|rn"}
<CodeGroup>
```typescript title="Sample Code" {7,17}
// Resend a text message in a one-to-one conversation
...
ZIMMessageSendNotification notification = ZIMMessageSendNotification(onMessageAttached: (message){
    // Callback before sending, developers can obtain a temporary object here. This object is the same as the zimMessage object created by the developer. You can use this feature to implement some business logic, such as pre-displaying UI, etc.
});
...
ZIM.getInstance()!.sendMessage(textMessage, toConversationID, type, sendConfig).then((value) => {
    ...
    // Developers can catch exceptions for failed sends here.
});
});
```
</CodeGroup>
...
<Note title="Note"> 
1. **Local images** (sent via `fileLocalPath`) Images will be uploaded to ZEGOCLOUD servers, and a `fileDownloadURL` will be automatically generated (the sender can obtain this URL in the message sent success callback via the ZIMMessage object). Both the sender and receiver can download the original image using the `fileDownloadURL` (even if the sender deletes the local image, it can still be retrieved via this URL). 
2. **Network images** (sent directly via `fileDownloadURL`) These are not relayed through ZEGOCLOUD servers. The SDK only transmits the URL and does not generate a new server-side download address. Ensure the URL is a publicly accessible and valid address. Receivers need to handle image loading themselves (e.g., caching, preventing invalidation). 
...
ZIMVideoMessage message = new ZIMVideoMessage("/storage/emulated/0/Android/data/packagename/picture/xxx.mp4", 300);//The unit here is seconds
...
const file = ''; // The absolute path of the file to be uploaded
sendMessage(file);
```
</Accordion>
:::
...
After logging in, users should listen for the following callbacks based on the conversation type (one-to-one, room, group): {getPlatformData(props,messageReceivedMap)}, {getPlatformData(props,messageReceivedMap)}, {getPlatformData(props,messageReceivedMap)}, to receive the events of rich media messages. From the events, the URL of rich media content can be obtained directly.
...
<CodeGroup>
``` java SampleCode
// Receive rich media content - Receive images:
@Override
public void onMessageReceived(ZIM zim, ZIMMessageReceivedEventResult result) {
    List<ZIMMessage> messageList = result.messageList;
...
pushConfig.title = "Offline notification title";
pushConfig.content= "Offline notification content";
pushConfig.payload = "Extra information for the offline notification";
config.pushConfig = pushConfig;
...
zim.on('messageReceived', function (zim, result) {
    ...
    messageList.forEach(function (msg) {
        ...
        if (msg.type == 2) {
            ...
            console.log('messageReceived', jsonObj);
        }
    })
});
```
...
ZIM SDK supports sending multiple types of content in a single message, such as text, images, and more. This type of message is referred to as a "multi-message. You can define this type of message by using the {getPlatformData(props,ZIMMultipleMessageMap)}  object.
...
<Frame width="60%" height="auto">
  <img src="https://doc-media.zego.im/docuo/11fe886c32.png" alt="ZIMMultipleMessage_en.png"/>
</Frame>
...
**Allow Partial Failure of Sub-items**

By setting the isAllowMultipleMessagePartialFailed field in the send settings to true, you can allow some sub-items to fail to send without affecting the overall success of the multi-item message.
<Note title="Note">
- Only ZIM SDK version 2.27.0 and above support the feature that allows some sub-items to fail when sending multi-item messages.
- If some sub-items fail when sending a multi-item message, only the sender can see all sub-items, including the failed ones, as well as the failed sub-item index list `failedIndexList`. The sender's other clients and recipients can only see all successfully sent sub-items.
</Note>
...
// Message Priority
config.priority = ZIMMessagePriority.LOW;
// Enable partial failure of sub-items when sending multi-item messages.
config.isAllowMultipleMessagePartialFailed = true;
...
sendConfig.priority = ZIMMessagePriorityMedium;
// Enable partial failure of sub-items when sending multi-item messages.
sendConfig.isAllowMultipleMessagePartialFailed = YES;
...
sendConfig.pushConfig = &pushConfig;
// Enable partial failure of sub-items when sending multi-item messages.
sendConfig.isAllowMultipleMessagePartialFailed = true;
...
```typescript
// The method to select a local file
const file = ''; // Absolute path of the file to be uploaded
```
:::
:::if{props.platform="Web|rn"}
```typescript
...
const config: ZIMMessageSendConfig = { 
    priority: 1, // Message Priority, 1: low (default), 2: medium, 3: high
    isAllowMultipleMessagePartialFailed: true, // Enable partial failure of sub-items when sending multi-item messages.
};
...
The callback interface for receiving combined messages is the same as that for receiving regular messages. For details on the specific interface, please refer to [Send & Receive messages - Receive messages](./Send%20and%20receive%20messages.mdx#receive-messages).
...
1. In the message received callback, if the message type is ZIMMessageType.COMBINE, call the {getPlatformData(props,queryCombineMessageDetailMap)} API to query the combine message details.

2. In the {getPlatformData(props,ZIMCombineMessageDetailQueriedCallbackMap)} callback, iterate through the list of sub-messages. If a sub-message is of type ZIMImageMessage, download the image using the {getPlatformData(props,downloadMediaFileMap)} API.

The following is an example code for receiving multi-item messages in a one-to-one conversation:

:::if{props.platform=undefined}
```java
// User receives multi-item message
zim.setEventHandler(new ZIMEventHandler() {
    @Override
    public void onMessageReceived(ZIM zim, ZIMMessageReceivedEventResult result) {
        List<ZIMMessage> messageList = result.messageList;
...
<Frame width="auto" height="auto">
  <img src="https://doc-media.zego.im/docuo/d9fc3185c1.jpg" />
</Frame>
...
// Whether to forcefully push the notification to the alerted user (regardless of whether the other party has enabled Do Not Disturb mode), the default is true;
...
ZIMMessageSendConfig *config = [[ZIMMessageSendConfig alloc] init];
// Whether to forcefully push the notification to the alerted user (regardless of whether the other party has enabled Do Not Disturb mode), the default is YES;
...
:::if{props.platform="Web|rn"}
```typescript
const isMentionAll = message.getIsMentionAll;
```
:::
:::if{props.platform="flutter"}
```dart
