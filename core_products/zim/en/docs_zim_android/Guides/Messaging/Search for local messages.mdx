...
// Set the keyword to "zegocloud", supports up to 5 keywords..
...
config.count = 20;  // Number of search results
config.order = ZIMMessageOrder.Descending;  // Specify the search order as querying from the last (time) message stored locally
config.keywords.Add("zegocloud");  // Set the keyword as "zegocloud", supports up to 5 keywords.
config.messageTypes.Add(ZIMMessageType.Text); // Specify the message type as text
config.startTime = startTimestamp; // Start time of the search
config.endTime = endTimestamp; // End time of the search
...
After creating a ZIM object and logging in, call the {getPlatformData(props,searchLocalConversationsMap)} interface, passing in the parameter config, and set the search conditions related to the local conversation name to search all conversations that meet the criteria.

The list of conversations that match the criteria will be returned through the {getPlatformData(props,ZIMConversationsSearchedCallbackMap)} callback interface.
...
// Search for local conversations whose conversation name contains the keyword "zego" to get the corresponding conversation list
ZIMConversationSearchConfig *config = [[ZIMConversationSearchConfig alloc] init];
config.mode = ZIMConversationSearchModeMatchConversationName;
config.totalConversationCount = 10;
config.keywords = @[@"zego"];  // Set the keyword as "zego", up to 5 keywords are supported.
config.nextFlag = 0;
...
// Search for local conversations whose conversation name contains the keyword "zego" to get the corresponding conversation list
auto searchConfig = zim::ZIMConversationSearchConfig();
searchConfig.mode = zim::ZIMConversationSearchMode::ZIM_CONVERSATION_SEARCH_MODE_MATCH_CONVERSATION_NAME;
searchConfig.totalConversationCount = 10;
...
ZIMConversationSearchConfig config = ZIMConversationSearchConfig();
config.mode = ZIMConversationSearchMode.matchConversationName;
config.totalConversationCount = 20;
config.keywords.add('zego'); // Set the keyword as "zego", up to 5 keywords are supported.
