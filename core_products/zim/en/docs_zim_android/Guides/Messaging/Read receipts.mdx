...
export const onConversationMessageReceiptChangedMap = {
  ...
  'Web': <a href='@onConversationMessageReceiptChanged' target='_blank'>onConversationMessageReceiptChanged</a>,
  'Flutter': <a href='@onConversationMessageReceiptChanged' target='_blank'>onConversationMessageReceiptChanged</a>,
  'iOS': <a href='@ZIMConversationMessageReceiptChanged' target='_blank'>conversationMessageReceiptChanged</a>,
  'mac': <a href='@ZIMConversationMessageReceiptChanged' target='_blank'>conversationMessageReceiptChanged</a>,
}
...
export const ZIMCustomMessageMap = {
  ...
  'Flutter': <a href='@-ZIMCustomMessage' target='_blank'>ZIMCustomMessage</a>,
}
...
Message receipts refer to the ability of the sender, after sending a message with a receipt, to know the delivery and reading status of the message on the receiver side. The receipt status covers the complete lifecycle from "processing" to "delivered" to "completed (read)", as well as abnormal statuses such as "failed" and "expired". This feature applies to enterprise office businesses and other scenarios in which the message delivery and reading status needs to be known in real time.

<Frame width="512" height="auto">
  <img src="https://doc-media.zego.im/docuo/c9e82bb552.png" alt="Message receipt.png"/>
</Frame>

<Video src="https://doc-media.zego.im/core_products/zim/en/docs_zim_android/Guides/20260910-zim-en-1920x1080.mp4" />
...
The In-app Chat SDK supports reading receipts for one-to-one messages and group messages (only common messages, rich media messages, and custom messages) and does not support reading receipts for in-room messages.

</Warning>

## Implementation process

The sender sends a message through the In-app Chat SDK and sets the `hasReceipt` field of {getPlatformData(props,ZIMMessageSendConfigMap)} to identify whether a reading receipt is required for the message. Based on the `receiptStatus` field, the receiver determines whether the message requires a receipt and the current progress of the receipt, and renders different UI effects accordingly. The message receiver can use different reading methods based on the scenario.

The value of `receiptStatus` comes from the [ZIMMessageReceiptStatus](@-ZIMMessageReceiptStatus) enum:

| Value-30% | Description |
| --- | --- |
| NONE | Not a receipt. |
| PROCESSING | Receipt in progress. |
| DELIVERED | Delivered. Supported in ZIM 3.1.0 or later. |
| DONE | Completed, which means the message has been read. |
| FAILED | Failed. |
| EXPIRED | Expired. |
| UNKNOWN | Unknown. |
...
2. Client A calls the {getPlatformData(props,sendMessageMap)} API to send a message (common message, rich media message, or custom message in one-to-one or group chats) to Client B and sets the `hasReceipt` field of {getPlatformData(props,ZIMMessageSendConfigMap)} to **true**.
:::if{props.platform="undefined|Flutter|iOS|mac|window|Web"}
<div>
2. Client A calls the {getPlatformData(props,sendMessageMap)} API to send a message to Client B (only supports {getPlatformData(props,ZIMTextMessageMap)}, {getPlatformData(props,ZIMImageMessageMap)}, {getPlatformData(props,ZIMFileMessageMap)}, {getPlatformData(props,ZIMAudioMessageMap)}, {getPlatformData(props,ZIMVideoMessageMap)}, {getPlatformData(props,ZIMCombineMessageMap)}, {getPlatformData(props,ZIMMultipleMessageMap)}, and {getPlatformData(props,ZIMCustomMessageMap)} in "one-to-one" chats and "group" chats), and sets the `hasReceipt` field of {getPlatformData(props,ZIMMessageSendConfigMap)} to true;
</div>
:::
:::if{props.platform="U3d"}
<div>
...
:::
3. By listening for related callback ({getPlatformData(props,onMessageReceivedMap)}), Client B receives a message whose `receiptStatus` is set to `PROCESSING`.
...
1. Through related callback ({getPlatformData(props,onMessageReceivedMap)}), Client B receives a message that requires a reading receipt from Client A.
...
1. Based on the `receiptStatus` field of the {getPlatformData(props,onMessageReceivedMap)} callback, Client B determines the receipt status of the message. If this field is set to `PROCESSING`, the message is unread. Developers can call the {getPlatformData(props,sendConversationMessageReceiptReadMap)} API to set all messages sent by Client A in the conversation as read based on the service logic.
...
<Warning title="Note">
Since ZIM 3.1.0, a "delivered" status has been added between "processing" and "completed" for message receipts, indicating that the receipt message has been delivered. It is supported in both one-to-one and group chats. This feature is available only in the paid edition. To use it, contact ZEGOCLOUD technical support to enable it. If you need this capability on a version earlier than ZIM 3.1.0, contact ZEGOCLOUD technical support for evaluation.
</Warning>
...
## Receipt validity period and expired status

Message receipts have a validity period of 7 days. After the validity period expires, the receipt status of the message will change to "expired" (`EXPIRED`), and the message can no longer be set as read.

Developers can perceive the receipt expiry status change in real time through the {getPlatformData(props,onMessageReceiptChangedMap)} callback (supported since ZIM 2.16.0). It is recommended to gray out or remove the receipt indicator of expired messages in the UI.
...
|--- | --- |
| [ZIMGroupMessageReceiptMemberListQueriedCallback](@-ZIMGroupMessageReceiptMemberListQueriedCallback) | The callback that returns the query result, where `userList` is the group member list matching the query conditions, and `nextFlag` is the start marker for the next query (when `nextFlag` is 0, the query is complete).|
:::
