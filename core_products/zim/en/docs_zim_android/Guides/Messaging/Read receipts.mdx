...
<Warning title="Note">
Since ZIM 3.1.0, a "delivered" status has been added between "processing" and "completed" for message receipts, indicating that the receipt message has been delivered. It is supported in both one-to-one and group chats and requires no additional activation. If you need this capability on a version earlier than ZIM 3.1.0, contact ZEGOCLOUD technical support for evaluation.
</Warning>
