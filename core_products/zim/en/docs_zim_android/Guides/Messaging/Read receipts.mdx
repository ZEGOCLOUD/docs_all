...
2. Client A calls the {getPlatformData(props,sendMessageMap)} API to send a message (common message or rich media message in one-to-one or group chats) to Client B and sets the `hasReceipt` field of {getPlatformData(props,ZIMMessageSendConfigMap)} to **true**.
