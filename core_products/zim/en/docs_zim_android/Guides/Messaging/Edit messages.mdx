...
import { ZIMTextMessageMap, ZIMCustomMessageMap, ZIMMultipleMessageMap, setEventHandlerMap, messageEditedMap, editMessageMap } from "/snippets/zim/const-linkEN.mdx"
...
**After successfully logging into the ZIM SDK**, conversation participants can call the {getPlatformData(props,editMessageMap)} interface to edit messages they have already sent (only supports the following types of messages: {getPlatformData(props,ZIMTextMessageMap)}, {getPlatformData(props,ZIMCustomMessageMap)}, {getPlatformData(props,ZIMMultipleMessageMap)}). The editable attributes are as follows:
...
- If you have enabled ZIM content moderation, the edited message content will also be moderated, and the moderation process and limitations are the same as when sending a message.
    - When the message content is text (moderate before sending), failing the review will result in editing failure, and the message content will not be updated, remaining as the original content.
</Note>
:::if{props.platform=undefined}
```java title="Example Code"
