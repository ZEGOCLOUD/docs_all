...
Participants in a conversation register {getPlatformData(props,setEventHandlerMap)} listeners for {getPlatformData(props,messageEditedMap)} callback notifications related to message edits. When another user edits a message, you can directly obtain relevant information about the edited message, including the edit time and editor.
...
**After successfully logging into the ZIM SDK**, session participants can call the {getPlatformData(props,editMessageMap)} interface to edit messages they have already sent. The editable attributes are as follows:
...
<Note title="Note">
- Only supports editing messages within 24 hours.
- Only supports the following types of messages: {getPlatformData(props,ZIMTextMessageMap)}, {getPlatformData(props,ZIMCustomMessageMap)}, {getPlatformData(props,ZIMMultipleMessageMap)}.
- Cannot modify the message type, for example, cannot change a text message to a custom message. Does not support changing message types such as from {getPlatformData(props,ZIMTextMessageMap)} to {getPlatformData(props,ZIMCustomMessageMap)}.
- Message type cannot be changed, for example: {getPlatformData(props,ZIMTextMessageMap)} cannot be converted to {getPlatformData(props,ZIMCustomMessageMap)} or {getPlatformData(props,ZIMMultipleMessageMap)}.
- Restrictions on each attribute in the edit message interface are consistent with the relevant restrictions in the send message interface.
...
- If you have enabled ZIM content review, the edited message content will also be reviewed, and the review process and limitations are the same as when sending a message.
</Note>
:::if{props.platform=undefined}
```java title="Example Code"
// Edit text message content
ZIMTextMessage messageObj; // Obtain from the queryHistoryMessage interface
...
```cpp title="Example Code"
// Edit text message content
std::shared_ptr<zim::ZIMTextMessage> message_obj; // Obtain from the queryHistoryMessage interface
...
```javascript title="Example Code"
// Edit text message content
var messageObj; // Obtain from the queryHistoryMessage interface
messageObj.message = "Edited message content";
