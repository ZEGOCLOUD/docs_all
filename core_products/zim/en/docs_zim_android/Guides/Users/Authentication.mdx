...
The Token validity period cannot exceed 24 days. For security reasons, ZEGOCLOUD strongly recommends that developers generate Tokens on their own app server.
</Warning>
...
config.userName = @"YOUR_USER_NAME";
config.token = @"xxxx";// Obtain the token from the server.
