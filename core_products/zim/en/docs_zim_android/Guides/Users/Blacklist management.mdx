...
After a user is successfully blacklisted, you will no longer receive messages from the blacklisted user, but you can still send messages to them.
...
- After being blocked, the blocked user cannot **send messages** by default; to also restrict the blocked user from **editing messages, recalling messages, pinning messages, replying to messages, reacting to messages**, and **sending call invitations**, please contact technical support to enable the related blacklist configuration.
</Warning>
...
<Warning title="Warning">
Blocked users cannot directly check whether they have been blacklisted by other users. They can only determine if they have been blacklisted by receiving the error code `6000284` after sending a message.
</Warning>
