...
ZIM.getInstance().addFriend("zego", config, new ZIMFriendAddedCallback() {
    @Override
    public void onFriendAdded(ZIMFriendInfo friendInfo, ZIMError zimError) {
    }
);
```
</CodeGroup>
...
```typescript
// Add friends directly
const config: ZIMFriendAddConfig = { wording: 'Hello', friendAlias: 'Mark', friendAttributes: { k0: 'SZ' } };
zim.addFriend('userID', config).then(res => {
...
After successfully adding a friend, both users will receive the callback {getPlatformData(props,onFriendListChangedMap)} to notify them that they have become friends.
...
- One-way deletion: If user A deletes user B in a one-way manner, user B is no longer a friend of user A, but not vice versa.
    <Note title="Note">
    At this time, if user A [sends a friend request](#send-friend-requests) to user B, user B does not need to agree, and user A can become user B's friend directly.
    </Note>
...
zim.deleteFriends(['userID1', 'userID2'], config).then(res => {
...
:::if{props.platform="Web"}
```typescript
const config: ZIMFriendApplicationSendConfig = { wording: 'hello!', friendAlias: 'Mark', friendAttributes: { k0: 'SZ' } };
zim.sendFriendApplication('userID', config).then(res => {
    const errorUserList = res.errorUserList;
})
```
...
zim.acceptFriendApplication('userID', config).then(res => {
    const friendInfo = res.friendInfo;
})
```
:::
...
```typescript
// Reject friend application
zim.rejectFriendApplication('userID', {} as ZIMFriendApplicationRejectConfig).then(res => {
...
```typescript
// Query friend application information list
zim.queryFriendApplicationList({ count: 100, nextFlag: 0 } as ZIMFriendApplicationListQueryConfig).then(res => {
...
// Update friend alias
try {
    ZIMFriendAliasUpdatedResult result = await ZIM.getInstance()!.updateFriendAlias('userA', 'zego');
    // Handle success logic
} on PlatformException catch (onError) {
    // Handle failure logic
    onError.code; // Handle according to the official error code table
