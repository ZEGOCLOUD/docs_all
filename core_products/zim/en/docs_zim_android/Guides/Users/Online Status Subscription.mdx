...
<Frame width="auto" height="auto">
  <img src="https://media-resource.spreading.io/docuo/workspace740/af061ebc6eaf0f12ae9e7f72235bd04e/bee08a7ae3.png" alt=".png"/>
</Frame>
...
// 2. Update custom status
zim_->updateUserCustomStatus("Do Not Disturb", [](std::string &customStatus, const ZIMError &errorInfo) {
...
zim.updateUserCustomStatus("Do Not Disturb").then(res => {
});
```
</CodeGroup>
:::
...
config.customStatus = "Busy"; // Default is an empty string. If it's an empty string, the user's custom status will not be modified during login.
```
```dart title="Update Custom Statu" {1}
ZIM.getInstance()?.updateUserCustomStatus("Do Not Disturb").then((ZIMUserCustomStatusUpdatedResult result) {
