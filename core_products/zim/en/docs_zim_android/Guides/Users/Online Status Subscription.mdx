...
export const customStatusMap = {
  'Android': <a href='@customStatus' target='_blank'>ZIMLoginConfig.customStatus</a>,
...
To use this feature, please subscribe to the Premium Plan.
</Note>
...
<Note title="Note">
- The maximum length of `customStatus` is 64 bytes.
...
:::
## Subscription to User Status

To continuously monitor the status of certain users, you can call the {getPlatformData(props,subscribeUsersStatusMap)} interface. Pass the target user IDs in batches to the parameter `userIDs` (up to 100 registered users, excluding the subscriber themselves). In the config, pass the `subscriptionDuration` (the duration for which the subscription is valid, during which the online status changes of the target users will be continuously monitored), thereby adding the subscription of the target users' online status to the current user's subscription list.


<Note title="Note">

- A single user can subscribe to a maximum of 3000 people. When the number of subscribed users exceeds 3000, the earliest subscribed users will be overwritten.
- After subscribing to other users' online statuses, even if the user logs out and relogs into the ZIM SDK within the `subscriptionDuration`, the subscription remains valid, and there is no need to call the subscription method again.
</Note>
...
zim_->subscribeUsersStatus(userIDs, config, [](const std::vector<ZIMErrorUserInfo> &errorUserList, const ZIMError &errorInfo) {
    for (const ZIMErrorUserInfo &errorUserInfo : errorUserList) {
        errorUserInfo.userID;   // User ID for which subscription failed
        errorUserInfo.reason;   // Reason for failure
    }
});
```
:::
...
const config = {
    subscriptionDuration: 60, // Subscription duration is 60 minutes
};
...
try {
    ...
    // Check for users that failed to subscribe
    if (result.errorUserList.length > 0) {
        result.errorUserList.forEach((errorUserInfo) => {
            console.log('User ID that failed to subscribe:', errorUserInfo.userID);
            console.log('Error code for subscription failure:', errorUserInfo.reason);
        });
    } else {
        console.log('All users subscribed successfully!');
    }
} catch (error) {
    // Catch exceptions and handle error codes
...
ZIM.getInstance()?.subscribeUsersStatus(["userIdA","userIdB"], config).then((ZIMUsersStatusSubscribedResult result) {
    for(ZIMErrorUserInfo errorUserInfo in result.errorUserList){
        errorUserInfo.userID; // User ID that failed to subscribe
        errorUserInfo.reason; // Error code for subscription failure
    }
}).catchError((onError){
...
Subsequently, within the time frame specified by `subscriptionDuration`, updates to the target user's online status will be returned via the {getPlatformData(props,ZIMEventHandlerMap)} event callback in {getPlatformData(props,onUserStatusUpdatedMap)}.
...
If you no longer wish to monitor the online status of certain users, you can call the {getPlatformData(props,unsubscribeUsersStatusMap)} interface and pass the target user IDs (up to 100 users) into the `userIDs` parameter.
...
zim.unsubscribeUsersStatus(userIDs, new ZIMUsersStatusUnsubscribedCallback() {
    ...
    public void onUsersStatusUnsubscribed(ArrayList<ZIMErrorUserInfo> errorUserList, ZIMError errorInfo) {
        for (ZIMErrorUserInfo errorUserInfo : errorUserList) {
            System.out.println("User ID that failed to unsubscribe: " + errorUserInfo.userID);
            System.out.println("Error code for unsubscription failure: " + errorUserInfo.reason);
        }
    }
});
```
...
[zim unsubscribeUserStatus:@[@"userIdA",@"userIdB"]
                  callback:^(NSArray<ZIMErrorUserInfo *> *_Nonnull errorUserList,
                                ZIMError *_Nonnull errorInfo) {
        for(ZIMErrorUserInfo *errorUserInfo in errorUserList){
            errorUserInfo.userID; // Failed user ID to unsubscribe
            errorUserInfo.reason; // Error code for failed unsubscription
        }
}];
```
:::
...
zim_->unsubscribeUsersStatus(userIDs, [](const std::vector<ZIMErrorUserInfo> &errorUserList, const ZIMError &errorInfo) {
    for (const ZIMErrorUserInfo &errorUserInfo : errorUserList) {
        errorUserInfo.userID;   // User ID that could not be unsubscribed
        errorUserInfo.reason;   // Reason for failure
    }
});
```
...
try {
    ...
    // Check for users that could not be unsubscribed
    if (result.errorUserList.length > 0) {
        result.errorUserList.forEach((errorUserInfo) => {
            console.log('User ID that could not be unsubscribed:', errorUserInfo.userID);
            console.log('Error code for failed unsubscription:', errorUserInfo.reason);
        });
    } else {
        console.log('Successfully unsubscribed from all users');
    }
} catch (error) {
...
ZIM.getInstance()?.unsubscribeUsersStatus(["userIdA","userIdB"]).then((ZIMUsersStatusUnsubscribedResult result) {
    for(ZIMErrorUserInfo errorUserInfo in result.errorUserList){
        errorUserInfo.userID;   // User ID for which unsubscription failed
        errorUserInfo.reason;   // Error code for failed unsubscription
    }
}).catchError((onError){
...
If you only need to obtain the online status of a target user once without continuous attention, you can call the {getPlatformData(props, queryUsersStatusMap)} interface and pass in the userIDs of the target users (up to 200 users, excluding the current user).
...
:::if{props.platform="RN"}
```javascript
// List of user IDs whose status needs to be queried
const userIDs = ['userIdA', 'userIdB'];
...
try {
    ...
    // Iterate through the successfully queried user statuses
    result.userStatusList.forEach((userStatus) => {
        ...
        console.log('Online Platforms:', userStatus.onlinePlatforms); // User online platform list
        console.log('Last Update Time:', userStatus.lastUpdateTime); // Last time the online status was changed
    });

    // Check users with failed queries
    result.errorUserList.forEach((errorUserInfo) => {
        console.log('User ID with query failure:', errorUserInfo.userID);
        console.log('Error code for query failure:', errorUserInfo.reason);
    });
} catch (error) {
    // Catch exceptions and handle errors
    console.error('An error occurred while querying user status:', error);
}
```
:::
:::if{props.platform="Flutter"}
...
ZIM.getInstance()?.queryUsersStatus(["userIdA","userIdB"]).then((ZIMUsersStatusQueriedResult result) {
    for (ZIMUserStatus userStatus in result.userStatusList) {
      userStatus.userID;          // Target user ID
      userStatus.onlineStatus;    // User online status
      userStatus.onlinePlatforms; // List of user's online platforms
      userStatus.lastUpdateTime;  // Last time the online status was updated
    }
    for (ZIMErrorUserInfo errorUserInfo in result.errorUserList){
      errorUserInfo.userID; // User ID for which the query failed
      errorUserInfo.reason; // Error code for the failed query
...
## Query the Subscription List of Online Status Users

If a user wants to know which users' online statuses they have subscribed to, they can use the {getPlatformData(props,querySubscribedUserStatusListMap)} interface. By passing an empty `config.userIDs` parameter, the user can obtain the complete subscription list, including the current status of the subscribed target users and the duration of the subscription validity.

If a user wants to confirm whether they have subscribed to certain users' online statuses, they can pass the userID of the query target into the `config.userIDs` parameter.
...
// If not, the result callback will not include any relevant information
...
try {
    ...
    // Iterate through the subscribed user status information
    ...
    // Catch exceptions and handle errors
    console.error('An error occurred while querying the list of subscribed user statuses:', error);
}
```
:::
:::if{props.platform="Flutter"}
...
- (void)connectionStateChanged:(ZIMConnectionState)state event:(ZIMConnectionEvent)event extendedData:(NSDictionary *)extendedData{
    switch (state) {
        case ZIMConnectionStateConnected:
            // Connected, you can set the current user's status as online
            break;
        case ZIMConnectionStateConnecting:
            // Connecting, based on your business logic, you can set the current user's status as either online or offline
            break;
        case ZIMConnectionStateReconnecting:
            // Reconnecting, based on your business logic, you can set the current user's status as either online or offline
            break;
        case ZIMConnectionStateDisconnected:
            // Disconnected, you can set the current user's status as offline
            break;
        default:
            break;
    }
...
void onConnectionStateChanged(ZIM* /*zim*/, ZIMConnectionState state, ZIMConnectionEvent event, const std::string & /*extendedData*/) {
    switch (state) {
        case ZIM_CONNECTION_STATE_CONNECTED:
            // Connected, you can set the current user's status to online
            break;
        case ZIM_CONNECTION_STATE_CONNECTING:
            // Connecting, based on your business logic, you can set the current user's status to online or offline
            break;
        case ZIM_CONNECTION_STATE_RECONNECTING:
            // Reconnecting, based on your business logic, you can set the current user's status to online or offline
            break;
        case ZIM_CONNECTION_STATE_DISCONNECTED:
            // Disconnected, you can set the current user's status to offline
            break;
        default:
...
ZIMEventHandler.onConnectionStateChanged = (ZIM zim, ZIMConnectionState state,
    ...
    switch (state) {
        ...
        // Connected, you can map the current user status to online
            break;
        case ZIMConnectionState.connecting:
        // Connecting, you can map the current user status to online or offline based on your business logic
            break;
        case ZIMConnectionState.reconnecting:
        // Reconnecting, you can map the current user status to online or offline based on your business logic
            break;
        case ZIMConnectionState.disconnected:
        // Disconnected, you can map the current user status to offline
            break;
        default:
...
zim.setEventHandler(new ZIMEventHandler(){
    public void onUserStatusUpdated(ZIM zim, ArrayList<ZIMUserStatus> userStatusList) {
        String myUserID = "Current User ID"; // Replace with the actual current user ID
        for (ZIMUserStatus userStatus : userStatusList) {
            if (userStatus.userID.equals(myUserID)) {
                ...
                // The list of online platforms for the current user
...
- (void)zim:(ZIM *)zim userStatusUpdated:(NSArray<ZIMUserStatus *> *)userStatusList {
    for(ZIMUserStatus *userStatus in userStatusList){
        if([userStatus.userID isEqual:myUserID]){
            ...
            userStatus.onlinePlatforms; // The list of online platforms for the current user
...
```javascript
zim.on('userStatusUpdated', function (zim, userStatusList) {
    const myUserID = 'currentUserID'; // Replace with the actual ID of the current user

    userStatusList.forEach((userStatus) => {
        if (userStatus.userID === myUserID) {
            // For the current user, onlineStatus will be unknown and cannot accurately reflect the online status
            ...
            console.log('Online status of the current user:', onlineStatus);
            console.log('Online platforms of the current user:', onlinePlatforms);
