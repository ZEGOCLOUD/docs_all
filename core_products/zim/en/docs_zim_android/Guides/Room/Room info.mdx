...
export const onRoomMemberJoinedMap = {
  ...
  'iOS': <a href='/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-room-member-joined-room-id' target='_blank'>roomMemberJoined</a>,
  'mac': <a href='/article/api?doc=zim_API~objective-c_macos~protocol~ZIMEventHandler#zim-room-member-joined-room-id' target='_blank'>roomMemberJoined</a>,
...
export const onRoomMemberLeftMap = {
  ...
  'iOS': <a href='/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-room-member-left-room-id' target='_blank'>roomMemberLeft</a>,
  'mac': <a href='/article/api?doc=zim_API~objective-c_macos~protocol~ZIMEventHandler#zim-room-member-left-room-id' target='_blank'>roomMemberLeft</a>,
...
<Note title="Note">
When the number of room members exceeds 500, the result of querying the room member list can contain information of up to 500 members. To increase this limit, please contact ZEGOCLOUD Technical Support.
</Note>
...
After a user joins a room, the extended information snapshot of the user before joining the room can be obtained through the `ZIMUserInfo.userExtendedData` field. This feature can be used to implement business scenarios such as room user level and VIP user identification.

<Warning title="Warning">
This interface cannot get the extended field information modified after the user joins the room (for example: the `userExtendedData ` before entering the room is `extended1`, and the extended field information after entering the room is `extended2`, the content obtained through this interface is still `extended1`).
</Warning>

Related interfaces please refer to: {getPlatformData(props,queryRoomMemberListMap)}、{getPlatformData(props,queryRoomMembersMap)}、{getPlatformData(props,onRoomMemberJoinedMap)}、{getPlatformData(props,onRoomMemberLeftMap)}.
