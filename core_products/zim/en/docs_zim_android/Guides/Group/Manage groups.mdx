...
zim_->queryGroupList(count, config, [=](const std::vector<ZIMGroupInfo> &groupList, unsigned int nextFlag, zim::ZIMError errorInfo){
    int error_code = errorInfo.code;
});
```
:::
...
[zim queryGroupListWithCount:count config:config callback:^(NSArray<ZIMGroupInfo *> * _Nonnull groupList, unsigned int nextFlag, ZIMError * _Nonnull errorInfo) {
