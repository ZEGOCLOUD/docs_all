...
export const loginMap = {
  ...
  'mac': <a href='https://docs.zegocloud.com/article/api?doc=zim_API~objective-c_macos~protocol~ZIM#login-with-user-id-config-callback-2' target='_blank'>loginWithUserID</a>,
...
export const isDisabledMap = {
  ...
  'Flutter': <a href='/zim-flutter/client-sdks/api-reference/struct#isconversationdisabled' target='_blank'>isConversationDisabled</a>,
}
export const ZIMGroupConversationMap = {
  'Android': <a href='@-ZIMGroupConversation' target='_blank'>ZIMGroupConversation</a>,
  'Flutter': <a href='/zim-flutter/client-sdks/api-reference/struct#zimconversation' target='_blank'>ZIMConversation</a>,
}
...
You can check whether the group is created through the {getPlatformData(props,ZIMGroupCreatedCallbackMap)} callback. For more information about error codes, see [Error codes](./../../SDK%20Error%20Codes/ZIM.mdx).
...
- The user creating a group is the group owner. For more information about group ownership transfer, see [Transfer the group ownership](/zim-server/group/transfer-the-group-ownership#request-parameters).
...
<div>
- After creating a group, the group owner can use the [sendGroupMessage](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIM/sendGroupMessage.html) interface to send messages to the group. For details, please refer to [Send and Receive Group Messages](./../Messaging/Send%20and%20receive%20messages.mdx#receive-rich-media-content).
</div>
:::
- The `groupID` is unique under the same AppID. Once a group is created with a specific `groupID`, that `groupID` cannot be reused for any subsequent group, regardless of whether the original group has been disbanded.
</Warning>
...
- {getPlatformData(props,inviteUsersIntoGroupMap)}: Invite a user to a group without the user's approval.
...
```objc
// The user queries the list of joined groups.
[zim queryGroupListByGroupID:^(NSArray<ZIMGroupInfo *> * _Nonnull groupList, long long nextFlag, ZIMError * _Nonnull errorInfo) {
...
### Querying the List of Mutual Joined Groups with a Specified User

After logging in to the ZIM SDK, if you want to know which groups you and another user have both joined, you can obtain the list of mutual joined groups by calling the {getPlatformData(props,queryGroupListMap)} method.
...
zim.queryGroupList(count, config, new ZIMGroupListQueriedCallback() {
    @Override
    public void onGroupListQueried(ArrayList<ZIMGroupInfo> groupList, long nextFlag, ZIMError errorInfo) {
        // The list of mutual joined groups is returned here
    }
});
```
:::
...
```dart
ZIMGroupListQueryConfig config = ZIMGroupListQueryConfig();
config.userIDs = ["userIdA"];
config.nextFlag = 0;

ZIM.getInstance(count: 30, config: config)
  !.queryGroupList()
   .then((value){
       // Triggered when successful
   })
...
zim_->queryGroupList(count, config, [=](const std::vector<ZIMGroupInfo> &groupList, long long nextFlag, zim::ZIMError errorInfo){
    int error_code = errorInfo.code;
});
```
:::
...
```typescript
const config: ZIMGroupListQueryConfig = {
    userIDs: ["userIdA"],
    nextFlag: 0,
};

zim.queryGroupList(30, config)
...
ZIMGroupListQueryConfig *config = [[ZIMGroupListQueryConfig alloc] init];
config.count = 30;
config.nextFlag = 0;

[zim queryGroupListWithCount:count config:config callback:^(NSArray<ZIMGroupInfo *> * _Nonnull groupList, long long nextFlag, ZIMError * _Nonnull errorInfo) {
    // Write your business logic after calling the group list query method here
}];
```
:::
...
<Frame width="auto" height="auto">
  <img src="https://doc-media.zego.im/docuo/166e1eaa78.jpg" alt="Group_Muted_New_EN.jpg"/>
</Frame>
...
If you want to block specific group members from posting, please refer to [Set mute status for group members - Manage group members](./Group%20members.mdx#set-mute-status-for-group-members).
</Note>
...
- Pull the conversation list. For more information, see [Pull the conversation list](./../Conversation/Get%20the%20conversation%20list.mdx).
- Listen for the conversation change callback and update the conversation list. For more information, see [Update the conversation list](./../Conversation/Get%20the%20conversation%20list.mdx).
