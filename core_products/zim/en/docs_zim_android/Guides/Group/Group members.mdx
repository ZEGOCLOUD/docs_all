...
Assume there are two users, A and B, in a group. User A has already retrieved the group member list. At this point, if user B changes their username and avatar, will the latest group member information of user B be retrieved when calling the {getPlatformData(props, queryGroupMemberListMap)} API again?

You need to implement different operations based on the ZIM SDK version:

- **SDK Version ≥ 2.19.0**:
    - If A and B are friends, you can directly call the API.
    - If A and B are not friends, you can retrieve the latest information of user B in either following way:
        - First, call the server-side API to [query the user information of user B](/zim-server/user/query-user-information). Then call the API.
        - First, call the SDK API to [get the group member information of User B](#get-group-member-info). Then call the API.
- **SDK Version < 2.19.0**:
    - Calling the API again will only retrieve new members' information and will not retrieve updated information for existing members.
    - You need to call the API {getPlatformData(props, queryGroupMemberInfoMap)} to [get the group member information of User B](#get-group-member-info).
</Note>
...
<Frame width="auto" height="auto">
  <img src="https://media-resource.spreading.io/docuo/workspace564/27e54a759d23575969552654cb45bf89/64997b800a.jpg"/>
</Frame>
