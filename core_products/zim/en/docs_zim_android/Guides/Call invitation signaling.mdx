...
<Frame width="512" height="auto" caption="">
  <img src="https://doc-media.zego.im/sdk-doc/Pics/in_app_chat/14243_1.jpg" />
</Frame>

|state-15% | description | triggering event | applicable mode|
| --------- | ------------ | ----------------------------------------------------------------------- | ------------------ |
| Inviting  | Being invited.   | The user is being invited.                                                        | Common mode, Advanced mode |
| Accepted  | Invitation accepted. | <ul><li>A user initiates a call invitation.</li><li>A user accepts a call invitation.</li></ul>                           | !mu                |
| Rejected  | Invitation rejected. | A user rejects a call invitation.                         | !mu                |
| Cancelled | Invitation canceled. | <ul><li>A user cancels a call invitation.</li><li>The called user does not respond to the invitation. The caller gets offline and the heartbeat times out.</li></ul> | !mu                |
| Received  | Invitation received. |  <ul><li>An online user receives a call invitation.</li><li>An offline user gets online within the specified timeout period.</li></ul>                   | !mu                |
| Timeout   | Invitation times out. | The called user fails to respond to the invitation within the specified timeout period.                                                | !mu                |
| Quit      | Quit the call.       | A user in the call quit the call.                                              | Advanced mode           |
| Unknown   | Unknown       | Contact ZEGOCLOUD Technical Support.                                                  | !ml                |
...
<Frame width="512" height="auto" caption=""><img src="https://doc-media.zego.im/sdk-doc/Pics/in_app_chat/14243.jpeg" /></Frame>
...
- The client B logs in after the invitation times out. In this case, the client B can call the {getPlatformData(props,queryCallInvitationListMap)} operation to <a href="#query-the-list-of-call-invitations">query the invitation list</a>.
...
```typescript
/** Send a call invitation to offline users */
const invitees = ['xxxx'];  // List of invitees' IDs
...
<Frame width="512" height="auto" caption=""><img src="https://doc-media.zego.im/sdk-doc/Pics/In_app_Chat/14243_3_1.png" /></Frame>
...
ZIM.getInstance().callCancel(invitees, callid, config, new ZIMCallCancelSentCallback() {
    @Override
    public void onCallCancelSent(String callID, ArrayList<ZIMErrorUserInfo> errorUserList, ZIMError errorInfo) {
...
[zim callCancelWithInvitees:invitees callID:self.callID config:callCancelConfig callback:^(NSString * _Nonnull callID, NSArray<NSString *> * _Nonnull errorUserList, ZIMError * _Nonnull errorInfo) {
...
:::if{props.platform="window"}
```Cpp
auto callcancel_callback = [=](std::vector<ZIMErrorUserInfo> errorUserList, zim::ZIMError errorInfo) {
    if (errorInfo.code != 0) {
...
<Frame width="512" height="auto" caption=""><img src="https://doc-media.zego.im/sdk-doc/Pics/In_app_Chat/14243_4_1.png" /></Frame>
...
<Frame width="512" height="auto" caption=""><img src="https://doc-media.zego.im/sdk-doc/Pics/In_app_Chat/14243_5_1.png" /></Frame>
...
<Frame width="512" height="auto" caption=""><img src="https://doc-media.zego.im/sdk-doc/Pics/In_app_Chat/14243_6_1.png" /></Frame>
...
```typescript
/**Send a call invitation to an online user in advanced mode.**/
const invitees = ['xxxx'];  // List of Invitee User
const config: ZIMCallInviteConfig = { 
    //  Call invitation mode, 1 represents the advanced mode.
    mode: 1,
    // Invitation timeout period in seconds, ranging from 1 to 600
    timeout: 200, 
    extendedData: ''
};
zim.callInvite(invitees, config)
