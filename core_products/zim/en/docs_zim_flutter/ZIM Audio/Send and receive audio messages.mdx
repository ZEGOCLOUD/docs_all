...
```dart
    ...
    ZIMAudio.getInstance().startRecord(ZIMAudioRecordConfig("path",maxDuration: 10 * 1000));
        ...
        // The default duration is 60,000 ms, that is, 60 seconds. The maximum duration is 300,000 ms, that is, 5 minutes.
...
Based on the conversation type (one-to-one chat, voice chatroom, or group chat), listen for the [onMessageReceived](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIMEventHandler/onMessageReceived.html) callback on the receiving client to receive the audio message notification and then call the [downloadMediaFile](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIM/downloadMediaFile.html) method to download the audio file to the local database.

Below is the sample code for receiving and downloading an audio message in a one-to-one chat.

```dart
ZIMEventHandler().onMessageReceived = (ZIM zim, ZIMMessageReceivedEventResult result) async{
