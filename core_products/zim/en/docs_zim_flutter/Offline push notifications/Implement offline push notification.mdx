...
Open the `pubspec.yaml` file, and add the `zego_zpns` dependencies as follows:
...
You need to integrate the Google FCM offline push notification channel. For more, refer to [Integrate Google FCM push notification (Web)](./Integrate%20FCM%20(for%20Web).mdx).
...
By configuring the `threadID` parameter of [ZIMPushConfig](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIMPushConfig-class.html) on the sender side, you can achieve grouped notifications for iOS offline push notifications.

```dart
ZIMPushConfig pushConfig = ZIMPushConfig();
pushConfig.title = "offline push notification title";
pushConfig.content = "offline push notification content";
pushConfig.threadID = "thread identifier for notification grouping";
```
