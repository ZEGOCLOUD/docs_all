...
<td>Image message. Applicable formats includes JPG, PNG, BMP, TIFF, GIF, and WebP. The maximum size is 20 MB for ZIM 3.2.0 or later, and 10 MB for earlier versions. Up to 10 image messages can be sent per second per client.</td>
</tr>
<tr data-row-level="7">
<td>ZIMFileMessage(12)</td>
<td>File Message. A file message contains a file of any format. The maximum size is 1 GB for ZIM 3.2.0 or later, and 100 MB for earlier versions. Up to 10 file messages can be sent per second per client.</td>
</tr>
<tr data-row-level="8">
<td>ZIMAudioMessage(13)</td>
<td>Audio message. An audio message contains an MP3 or M4A audio of up to 300 seconds. The maximum size is 1 GB for ZIM 3.2.0 or later, and 6 MB for earlier versions. Up to 10 audio messages can be sent per second per client.</td>
</tr>
<tr data-row-level="9">
<td>ZIMVideoMessage(14)</td>
<td>A video message contains an MP4 or MOV video. The maximum size is 1 GB for ZIM 3.2.0 or later, and 100 MB for earlier versions. Up to 10 video messages can be sent per second per client.<Note title="Note">To retrieve the width and height of the first video frame after a video is successfully sent, the video must be encoded in H.264 or H.265.</Note></td>
</tr>
