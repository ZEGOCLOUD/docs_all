...
ZIM SDK 3.1.0 introduces new API deprecation markers. For details, please refer to [ZIM Upgrade Guide](./ZIM%20upgrade%20guide.mdx#310-upgrade-guide).
</Warning>
...
|Feature-25% | Description-40% | Related APIs-25%|
|-----|-----|-----|
| Added conversation-level message timed destruction feature | Supports setting a message timed destruction duration at the conversation level. You can customize the destruction duration, and messages will be automatically destroyed when the set time is reached. For details, see [Set conversation message timed destruction](../Guides/Conversation/Message%20destruct.mdx). | [setConversationMessageDestructDuration](@setConversationMessageDestructDuration) |
| Added "Delivered" status to message receipts | Message receipts now include a "Delivered" status, supported in both one-to-one and group chats; in group chats, the delivered status list can be displayed. For details, see [Read receipts - Get the delivered status of receipt messages](../Guides/Messaging/Read%20receipts.mdx#get-the-delivered-status-of-receipt-messages). | [onMessageReceiptChanged](@onMessageReceiptChanged) |
...
|Query history messages supports configuring priority to sync server data |[queryHistoryMessage](@queryHistoryMessage) adds the `isPreferQueryFromServer` parameter, used to set whether to prefer fetching history messages from the server (by default, local cache is preferred).|[queryHistoryMessage](@queryHistoryMessage)|
...
|Optimized badge handling strategy for offline push in multi-device login scenarios | In multi-device login scenarios, when other devices have read new messages, the offline notification badge on iOS will be cleared. For details, see [Implement offline push notification using ZIM SDK](./../Offline%20push%20notifications/use-zim-to-zpns.mdx#scenario-4ios-notification-badge-count). | [updateUserBadge](@updateUserBadge) |
...
1. Fixed the issue where the [conversationSyncStateChanged](@conversationSyncStateChanged) event notification becomes invalid after "logging out and logging back in" within the same instance lifecycle.
...
| Item | Description | Method |
| -----  | ---- | ----- |
| Deprecation of callbacks for receiving message in one-to-one, room, and group chats | The callbacks of `receivePeerMessage`, `receiveGroupMessage` and `receiveRoomMessage` have been deprecated, please use the following callbacks instead: [peerMessageReceived](@peerMessageReceived), [onGroupMessageReceived](@onGroupMessageReceived), and [roomMessageReceived](@roomMessageReceived).<Note title="Note">Please refer to [ZIM Upgrade Guide](./ZIM%20upgrade%20guide.mdx#2180-upgrade-guide) for details on callback changes.</Note>| <ul><li>[peerMessageReceived](@peerMessageReceived)</li><li>[onGroupMessageReceived](@onGroupMessageReceived)</li><li>[roomMessageReceived](@roomMessageReceived)</li></ul> |
---
...
| Tips Message | Supports converting certain group operations of users in the group (such as creating groups, disbanding groups, etc.) into special types of messages (Tips). After obtaining the operation information through the group message callback, developers can construct and display the description text of the related events on the UI. <br/>For information on Tips message types and extended information, as well as post-reception processing operations, please refer to [Receive Tips Message](./../Guides/Messaging/Receive%20tip%20messages.mdx). | <ul><li>[ZIMMessageType > Tips]()</li><li>[receiveGroupMessage]()</li></ul> |
...
| Optimize the call back logic of [onRoomStateChanged](@onRoomStateChanged)| When developers call the ZIM server API to destroy a room in the backend, the callback [onRoomStateChanged](@onRoomStateChanged) that describes the room state change reason is modified to`ZIMRoomEventRoomNotExist`. | [onRoomStateChanged](@onRoomStateChanged) |
...
| -----  | ---- | ----- |
| <ul><li>[callInvitationAccepted]()</li><li>[callInvitationRejected](@callInvitationRejected)</li><li>[callInviteesAnsweredTimeout]()</li></ul> | To facilitate developers to monitor the call status changes of users in call invitations, [onCallUserStateChanged](@onCallUserStateChanged) is added to replace the original call invitation callbacks `callInvitationAccepted`、`callInvitationRejected` and `callInviteesAnsweredTimeout`. | 2.9.0 |
...
| Item | Description | Method |
| -----  | ---- | ----- |
| Deprecated system message types | [ZIMMessageType](@-ZIMMessageType) (value 30) have been deprecated. Developers are advised to use more versatile custom messages (value 200) instead. | [ZIMMessageTypeCustom](https://www.zegocloud.com/article/api?doc=zim_API~javascript_web~enum~ZIMMessageType#custom) |
