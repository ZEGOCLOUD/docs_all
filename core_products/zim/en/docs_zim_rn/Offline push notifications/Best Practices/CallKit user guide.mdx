...
function handlePerformAnswerCallPush(action: CXAnswerCallAction) {
    console.log("handlePerformAnswerCallPush User clicks to accept",action);
...
function handlePerformEndCallAction(action: CXEndCallAction) {
   console.log("handlePerformEndCallAction User clicks to hang up", action);
