...
3. The call invitation sender calls the [`callInvite`](@callInvite) method with the `callInviteConfig` to send a call invitation.
    ...
    zim.callInvite(invitees, callInviteConfig)
        .then(function({ callID, timeout, errorUserList }){
            // Operation succeeded
...
By configuring the `threadID` parameter of [ZIMPushConfig](@-ZIMPushConfig) on the sender side, you can achieve grouped notifications for iOS offline push notifications.

```typescript
const pushConfig: ZIMPushConfig = {
    title: "offline push notification title",
    content: "offline push notification content",
    threadID: "thread identifier for notification grouping",
}
```

### Unregister Offline Push

If developers no longer want a certain device to receive offline push notifications, they can unregister it by calling the [unRegisterPush](@unRegisterPush) interface. After unregistering, both alert push and silent push will no longer take effect.
