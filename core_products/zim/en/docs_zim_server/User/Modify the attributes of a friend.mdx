...
| [Attributes](#Attributes) | Array Of Object   | Yes      | The new attributes. By default, up to five attributes of a friend can be modified at a time.<Note title="Note">The total length of the `Key` and `Value` parameters cannot exceed 1,024 bytes. If you need a higher limit, contact ZEGOCLOUD technical support. We recommend that you set a proper limit.</Note> |
