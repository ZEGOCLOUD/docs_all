...
| 50014 | The length of the UserName exceeds the limit.  Please make sure the length of the UserName is within 256 bytes. |
| 51102 | The user does not exist. The user has not logged in to In-app Chat. The user needs to log in to In-app Chat. |
| 100000000 | Incorrect Host in the request header. The Host in the URL is incorrect. Please check the Host in the Request endpoint. |
...
| 100000006 | Action is null. Please check the Request endpoint. |
| 100000007 | Unsupported Action. Please check the Request endpoint. |
...
| 660000028 | This feature is not supported for ZIM Free users. Please contact ZEGOCLOUD Technical Support to upgrade your plan.|
...
| 660700006 | The length of UserName exceeds the limit. The maximum length of UserName is 256 bytes. Please confirm the length of UserName. |
...
| 660800001 | If a one-to-one chat is to be pinned or unpinned, this indicates that the user in the conversation (`ConvID`) is not registered. Please Check whether the user in the conversation (`ConvID`) is registered. - If so, contact ZEGOCLOUD technical support. - If not, register the user. |
