...
凭借本回调，您可以实时查看用户加入特定群聊的信息，可用于通知后台有成员入群。
...
| event | String | 回调事件：<ul><li>`Apply_join_group`：用户通过申请入群。</li><li>`invited_join_group`：用户通过邀请进群。</li></ul> |
...
| operator_account | String | 入群操作者的用户 ID。<ul><li>当 `join_type` 为 `Apply` 时，此参数与 `new_user_ids` 相同；</li><li>当 `join_type` 为 `Invited` 时，此参数为邀请者的用户 ID。</li></ul>|
