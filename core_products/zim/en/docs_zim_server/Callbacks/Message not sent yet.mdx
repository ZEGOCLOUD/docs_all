...
<Warning title="Warning">
- If you need this callback to return the rejection reason from the ZIM content moderation service, please contact ZEGOCLOUD Technical Support to enable the feature.
- In versions 2.21.0 and later, if the ZIM content moderation service is not enabled, this callback will default to returning the rejection reason from the business server.
</Warning>
