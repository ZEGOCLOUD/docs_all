...
凭借本回调，您可以实时查看用户退出特定房间的信息，可用于通知后台有成员退出房间。
...
| operator_account | String | 退房操作者的用户 ID。<ul><li>当 `exit_type` 为 `Quit` 时，此参数与 `exit_user_ids` 相同；</li><li>当 `exit_type` 为 `Kickout` 时，此参数为踢人者的用户 ID。</li></ul> |
