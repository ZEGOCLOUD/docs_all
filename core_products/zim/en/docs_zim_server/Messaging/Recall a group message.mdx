...
<td>Number</td>
<td>Yes</td>
<td>The sequence of the message. Method for obtaining `MsgSeq`: - If a group message is sent from the **client**, obtain `MsgSeq` by using the [Callback on message sent](../Callbacks/Message%20sent.mdx) operation. - If a group message is sent by using the server API [`SendGroupMessage`](./Send%20group%20messages.mdx), obtain `MsgSeq` in the response data.</td>
</tr>
<tr>
<td>Payload</td>
<td>String</td>
