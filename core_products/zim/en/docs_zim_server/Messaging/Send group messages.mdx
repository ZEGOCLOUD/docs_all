...
| Parameter | Type | Required | Description |
|-------|------|----------|-------------|
| Inclusive | Bool | No | Targeting type: <ul><li>false: Reverse targeting, meaning users listed in `ReceiverUserIds` will not receive this message.</li><li>true: Forward targeting, meaning only users listed in `ReceiverUserIds` will receive this message.</li></ul> |
