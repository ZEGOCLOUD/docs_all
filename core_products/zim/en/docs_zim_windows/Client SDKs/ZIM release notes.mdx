...
|Query history messages supports configuring priority to sync from server |[queryHistoryMessage](@queryHistoryMessage) adds the `isPreferQueryFromServer` parameter, used to set whether to preferentially fetch history messages from the server (defaults to using local cache).|[queryHistoryMessage](@queryHistoryMessage)|

**Enhancements**
|Item-25% | Description-40% | Related APIs-25%|
|-----|-----|-----|
|Optimized delete conversation API| When deleting a conversation, the client that initiated the deletion will trigger related conversation change callbacks. For details, see [Delete conversations](../Guides/Conversation/Delete%20conversations.mdx#delete-a-conversation).|<ul><li>[deleteConversation](@deleteConversation)</li><li>[onConversationChanged](@onConversationChanged)</li><li>[deleteAllConversations](@deleteAllConversations)</li><li>[onConversationsAllDeleted](@onConversationsAllDeleted)</li></ul>|
...
1. Fixed the issue where the [onGroupMessageReceived](@onGroupMessageReceived) event notification becomes invalid after "logging out and logging back in" within the same instance lifecycle.
...
| Tips Message | Supports converting certain group operations of users in the group (such as creating groups, disbanding groups, etc.) into special types of messages (Tips). After obtaining the operation information through the group message callback, developers can construct and display the description text of the related events on the UI. <br />For information on Tips message types and extended information, as well as post-reception processing operations, please refer to [Receive tip messages](./../Guides/Messaging/Receive%20tip%20messages.mdx) | <ul><li>[ZIMMessageType > ZIM_MESSAGE_TYPE_TIPS\| _blank](/article/api?doc=zim_API~cpp_windows~enum~ZIMMessageType#zim-message-type-tips)</li><li>[onReceiveGroupMessage]()</li></ul> |
...
| Added user avatar field to user information related classes | [ZIMUserInfo](@-ZIMUserInfo) and [ZIMGroupMemberInfo](@-ZIMGroupMemberInfo) added `userAvatarUrl` field, which is used to set or describe the user avatar address.<p>Note: The user avatar field for room members is currently not supported. This means that the `userAvatarUrl` in the `ZIMUserInfo` obtained through [queryRoomMemberList](@queryRoomMemberList) and [queryRoomMembers](@queryRoomMembers) will be empty.</p> | <ul><li>[ZIMUserInfo](@-ZIMUserInfo)</li><li>[ZIMGroupMemberInfo](@-ZIMGroupMemberInfo)</li></ul> |
| Optimize the callback logic of [onRoomStateChanged](@onRoomStateChanged) | When developers call the ZIM server API to destroy a room in the backend, the callback [onRoomStateChanged](@onRoomStateChanged) that describes the room state change reason is modified to`ZIM_ROOM_EVENT_ROOM_NOT_EXIST`. | [onRoomStateChanged](@onRoomStateChanged) |
...
| Item | Description | Method |
| -----  | ---- | ----- |
| Abandon the `login` interface | Abandon the old login interface and add a new login interface [login](@login). The new version of the login interface supports more configurations through [ZIMLoginConfig](@-ZIMLoginConfig), such as whether to use Token authentication and whether to log in offline.| [login](@login) |
...
| Item | Description | Method |
| -----  | ---- | ----- |
| Deprecated system message types | [ZIMMessageType](@-ZIMMessageType) (value 30) have been deprecated. Developers are advised to use more versatile custom messages (value 200) instead. | [ZIM_MESSAGE_TYPE_CUSTOM](https://docs.zegocloud.com/article/api?doc=zim_API~cpp_windows~enum~ZIMMessageType#zim-message-type-custom) |
...
| [sendMediaMessage]()|  A new rich media message sending method [sendMediaMessage]() (overloaded with the same name) was added, which supports getting the information before media uploading and can send rich media information such as files, pictures, audio, and video.  | 2.4.0 |
...
<Note title="Note">In this iteration, we unified and optimized all existing and new methods of In-App Chat In terms of style. When you upgrade the API methods from V 1.x.x to V 2.0.0, you will need to adjust the code accordingly by referring to the table below.</Note>
