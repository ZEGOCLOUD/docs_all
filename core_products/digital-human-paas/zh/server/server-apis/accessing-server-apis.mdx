...
- 请求参数：您需要通过 Action 参数指定接口，例如 Action = CreateMetaHumanVideo；还需要指定接口的其他参数。
...
以下为 [`CreateMetaHumanVideo`\|_blank](#18546)（创建视频合成任务）接口的请求结构为例：
...
- `Action=CreateMetaHumanVideo`：指定了要调用的 API。
...
| 产品 | 接入地址 |
|-|-|
| 数字人 PaaS 服务 | aigc-api.zegotech.cn |
...
- 请勿直接拷贝下面的示例用于请求。
- 请求地址为：https://aigc-api.zegotech.cn/?Action=xxxxx&公共参数，其中 `Action=xxxxx` 需要替换为各接口文档“接口原型”章节中的请求地址。
- 各公共参数的取值请根据实际情况修改。
</Warning>
