# Desktop Screenshot Capture Template (@midscene/computer)
...
act() {
  local prompt="$1"
  npx -y @midscene/computer@1 act --prompt "$prompt" 2>&1
}

screenshot() {
  local filename="$1"
  local tmp_path
  tmp_path=$(npx -y @midscene/computer@1 take_screenshot 2>&1 | grep "Screenshot saved:" | awk '{print $NF}')
